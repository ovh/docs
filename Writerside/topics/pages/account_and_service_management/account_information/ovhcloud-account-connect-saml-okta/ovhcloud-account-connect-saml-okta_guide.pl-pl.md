...
![Dodaj aplikację SAML, etap 1](OKTA_add_application_step1.png){.thumbnail}

Kliknij `Create App Integration`{.action} i wybierz `SAML 2.0`{.action}.

![Dodaj aplikację SAML, etap 2](OKTA_add_application_step2.png){.thumbnail}

W etapie "General Settings" dodasz nazwę dla tej aplikacji, na przykład **OVHcloud** i logo, jeśli chcesz. Kliknij `Next`{.action}.

![Dodaj aplikację SAML, etap 3](OKTA_add_application_step3.png){.thumbnail}
...
![Dodaj aplikację SAML, etap 4](OKTA_add_application_step4.png){.thumbnail}
...
![Dodaj aplikację SAML, etap 5](OKTA_add_application_step5.png){.thumbnail}
...
![Przypisywanie użytkowników](OKTA_add_user.png){.thumbnail}
...
![Pobierz metadane](OKTA_retrieve_metadata.png){.thumbnail}
...
![User Informacje OVHcloud](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_infos.png){.thumbnail}
...
![Połączenie SSO OVHcloud etap 1](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_management_connect_sso_1.png){.thumbnail}
...
![Połączenie SSO OVHcloud etap 2](ovhcloud_add_federation.png){.thumbnail}

Teraz możesz odnaleźć Okta jako dostawcę tożsamości, a także grupy domyślne.

![Połączenie SSO OVHcloud etap 3](ovhcloud_add_federation_success.png){.thumbnail}

Aby uzyskać więcej informacji, kliknij link pod "URL usługi SSO".

![Połączenie SSO OVHcloud etap 4](ovhcloud_idp_details.png){.thumbnail}

Przycisk `...`{.action} pozwala na aktualizację lub usunięcie certyfikatu SSO i na zapoznanie się z jego szczegółami.

![Połączenie SSO OVHcloud etap 5](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_management_connect_sso_5.png){.thumbnail}
...
![Grupy zarządzania użytkownikami Okta](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_management_groups_1.png){.thumbnail}

![Grupy zarządzania użytkownikami Okta](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_management_groups_2.png){.thumbnail}

Następnie możesz sprawdzić, czy grupa została dodana do Twojego konta OVHcloud w sekcji "Grupy":

![Grupy zarządzania użytkownikami Okta](ovhcloud-account-connect-saml-okta_images_ovhcloud_user_management_groups_3.png){.thumbnail}
...
Na [stronie logowania OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pl/&ovhSubsidiary=pl) wpisz swój [identyfikator](ovhcloud-account-creation#jaki-jest-moj-identyfikator-klienta.), po którym następuje **/idp** bez hasła i kliknij przycisk `Logowanie`{.action}.

![Połączenie z federacją OVHcloud](ovhcloud-account-connect-saml-okta_images_ovhcloud_federation_login_1.png){.thumbnail}
...
![OVHcloud Federacja Login Przekierowanie Okta](OKTA_login.png){.thumbnail}
...
[Konfiguracja i zarządzanie hasłem do konta](manage-ovh-password1.)
