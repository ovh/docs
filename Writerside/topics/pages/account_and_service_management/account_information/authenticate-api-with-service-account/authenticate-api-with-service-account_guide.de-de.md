...
To carry out automations on your infrastructure, you need to provide credentials to the code that does it. With OVHcloud service accounts, it is possible to have a single ID per script for the use of the different APIs offered by OVHcloud products ([OVHcloud API](console-preview1.), [OpenStack API](starting_with_nova1.), etc. )
...
- You must have created a [service account via API](manage-service-account1.).
...
![API](actions-in-console.png){.thumbnail}
