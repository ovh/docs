...
![Windows Server tools menu](windows_server_tools_menu.png){.thumbnail}
...
![AD FS relying party trusts menu](adfs_relying_party_trusts_menu.png){.thumbnail}

Select `Claims aware`{.action} and confirm with the `Start`{.action} button.

![AD FS add relying party trust step 1](adfs_add_relying_party_trust_1.png){.thumbnail}
...
![AD FS add relying party trust step 2](adfs_add_relying_party_trust_2.png){.thumbnail}

Enter a display name for the relying party and click the `Next`{.action} button.

![AD FS add relying party trust step 3](adfs_add_relying_party_trust_3.png){.thumbnail}

Click `Next`{.action} in the Access Control window.

![AD FS add relying party trust step 4](adfs_add_relying_party_trust_4.png){.thumbnail}

Click `Next`{.action} again to proceed.

![AD FS add relying party trust step 5](adfs_add_relying_party_trust_5.png){.thumbnail}

Click the `Close`{.action} button in the last window. The OVHcloud relying party trust is now added to your AD FS.

![AD FS relying party trusts](adfs_relying_party_trusts.png){.thumbnail}
...
![AD FS relying party trust mapping step 1](adfs_relying_party_trusts_mapping_1.png){.thumbnail}

Then click on `Edit Claim Issuance Policy...`{.action}.

![AD FS relying party trust mapping step 2](adfs_relying_party_trusts_mapping_2.png){.thumbnail}

Click the `Add Rule...`{.action} button.

![AD FS relying party trust mapping step 3](adfs_relying_party_trusts_mapping_3.png){.thumbnail}
...
![AD FS relying party trust mapping step 4](adfs_relying_party_trusts_mapping_4.png){.thumbnail}

![AD FS relying party trust mapping step 5](adfs_relying_party_trusts_mapping_5.png){.thumbnail}

Click the `Apply`{.action} button and confirm with `OK`{.action}.

![AD FS relying party trust mapping step 6](adfs_relying_party_trusts_mapping_6.png){.thumbnail}
...
[Log in](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/asia/&ovhSubsidiary=asia) and click on your profile in the top-right corner.

![OVHcloud top menu](ovhcloud_top_menu.png){.thumbnail}

Click on your name to access your profile management page.

![OVHcloud user infos](ovhcloud_user_infos.png){.thumbnail}

Open the `User management`{.action} tab.

![OVHcloud profile menu](ovhcloud_profile_menu.png){.thumbnail}

Click on the `SSO connection`{.action} button.

![OVHcloud connect SSO step 1](ovhcloud_user_management_connect_sso_1.png){.thumbnail}
...
![OVHcloud connect SSO step 2](ovhcloud_user_management_connect_sso_2.png){.thumbnail}

You should now see your AD FS as identity provider, as well as the default groups.

![OVHcloud connect SSO step 3](ovhcloud_user_management_connect_sso_3.png){.thumbnail}

Click the link below `SSO service URL` to view more information on it.

![OVHcloud connect SSO step 4](ovhcloud_user_management_connect_sso_4.png){.thumbnail}

![OVHcloud connect SSO step 5](ovhcloud_user_management_connect_sso_5.png){.thumbnail}

The `...`{.action} button enables you to update or delete the SSO, and to see details.

![OVHcloud connect SSO step 6](ovhcloud_user_management_connect_sso_6.png){.thumbnail}
...
![AD FS user](adfs_user.png){.thumbnail}

Next, check the mapping in AD FS:

![AD FS relying party trust mapping](adfs_relying_party_trusts_mapping_4.png){.thumbnail}
...
![AD FS user management groups](ovhcloud_user_management_groups_1.png){.thumbnail}

![AD FS user management groups](ovhcloud_user_management_groups_2.png){.thumbnail}

You can then check that the group is added to your OVHcloud account in the `Groups` section:

![AD FS user management groups](ovhcloud_user_management_groups_3.png){.thumbnail}
...
Warning: if you give the `NONE` privilege, you will need to assign permissions to this group via the [IAM policies](iam-policy-ui1.).

You can then disconnect from your account and log in again with your AD FS as identity provider.

### Connect via SSO

On the [OVHcloud login page](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/asia/&ovhSubsidiary=asia), enter your [NIC handle](ovhcloud-account-creation#what-is-my-nic-handle.) followed by **/idp** without entering a password, and click the `Login`{.action} button.

![OVHcloud federation login](ovhcloud_federation_login_1.png){.thumbnail}
...
![OVHcloud federation login AD FS redirection](ovhcloud_federation_login_2.png){.thumbnail}

You are now logged in with the same NIC handle, but via your Active Directory user and using your AD FS SSO.

![OVHcloud user infos federation](ovhcloud_user_infos_federation.png){.thumbnail}
...
[How to use IAM policies using the OVHcloud Control Panel](iam-policy-ui1.).
