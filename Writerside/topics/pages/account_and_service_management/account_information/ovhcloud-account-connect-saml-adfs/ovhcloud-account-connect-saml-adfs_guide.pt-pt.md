...
![Ferramentas Windows Server](windows_server_tools_menu.png){.thumbnail}
...
![Mensagem de aprovações AD FS](adfs_relying_party_trusts_menu.png){.thumbnail}

Selecione `Claims aware`{.action} e valide com o botão `Start`{.action}.

![AD FS adicionar uma aprovação - etapa 1](adfs_add_relying_party_trust_1.png){.thumbnail}
...
![AD FS - adicionar uma aprovação - etapa 2](adfs_add_relying_party_trust_2.png){.thumbnail}

Introduza um nome de apresentação para o organismo terceiro de confiança e clique no botão `Next`{.action}.

![AD FS - adicionar uma aprovação - etapa 3](adfs_add_relying_party_trust_3.png){.thumbnail}

Clique em `Next`{.action} na janela do controlo de acesso.

![AD FS - adicionar uma aprovação - etapa 4](adfs_add_relying_party_trust_4.png){.thumbnail}

Clique novamente em `Next`{.action} para continuar.

![AD FS - adicionar uma aprovação - etapa 5](adfs_add_relying_party_trust_5.png){.thumbnail}
...
![Aprovação AD FS](adfs_relying_party_trusts.png){.thumbnail}
...
![Conjunto de aprovação AD FS etapa 1](adfs_relying_party_trusts_mapping_1.png){.thumbnail}

A seguir, clique em `Edit Claim Issuance Policy...`{.action}.

![Conjunto de aprovação AD FS etapa 2](adfs_relying_party_trusts_mapping_2.png){.thumbnail}

Clique no botão `Add Rule...`{.action}.

![Conjunto de aprovação AD FS etapa 3](adfs_relying_party_trusts_mapping_3.png){.thumbnail}
...
![Conjunto de aprovação AD FS etapa 4](adfs_relying_party_trusts_mapping_4.png){.thumbnail}

![Conjunto de aprovação AD FS etapa 5](adfs_relying_party_trusts_mapping_5.png){.thumbnail}

Clique no botão `Apply`{.action} e valide com `OK`{.action}.

![Conjunto de aprovação etapa 6](adfs_relying_party_trusts_mapping_6.png){.thumbnail}
...
![Informações do utilizador OVHcloud](ovhcloud_user_infos.png){.thumbnail}

Abra o separador `Gestão dos utilizadores`{.action}.

![Perfil menu da OVHcloud](ovhcloud_profile_menu.png){.thumbnail}

Clique no botão `SSO connection`{.action}.

![A ligação SSO da OVHcloud etapa 1](ovhcloud_user_management_connect_sso_1.png){.thumbnail}
...
![Ligação SSO OVHcloud etapa 2](ovhcloud_user_management_connect_sso_2.png){.thumbnail}

Agora deve encontrar o AD FS como fornecedor de identidade, assim como os grupos predefinidos.

![A ligação SSO da OVHcloud etapa 3](ovhcloud_user_management_connect_sso_3.png){.thumbnail}

Para mais informações, clique no link situado abaixo do `URL do serviço SSO`.

![A ligação SSO da OVHcloud etapa 4](ovhcloud_user_management_connect_sso_4.png){.thumbnail}

![Ligação SSO OVHcloud etapa 5](ovhcloud_user_management_connect_sso_5.png){.thumbnail}

O botão `...`{.action} permite atualizar ou eliminar o SO, e consultar os respetivos detalhes.

![Ligação SSO OVHcloud etapa 6](ovhcloud_user_management_connect_sso_6.png){.thumbnail}
...
![Utilizador AD FS](adfs_user.png){.thumbnail}

Verifique a tabela de correspondências em AD FS:

![Conjunto de aprovação da parte de confiança AD FS](adfs_relying_party_trusts_mapping_4.png){.thumbnail}
...
![Grupos de gestão dos utilizadores AD FS](ovhcloud_user_management_groups_1.png){.thumbnail}

![Grupos de gestão dos utilizadores AD FS](ovhcloud_user_management_groups_2.png){.thumbnail}

De seguida, poderá verificar que o grupo é adicionado à sua conta OVHcloud na secção `Grupos`:

![Grupos de gestão dos utilizadores AD FS](ovhcloud_user_management_groups_3.png){.thumbnail}
...
![Ligação à Federação OVHcloud](ovhcloud_federation_login_1.png){.thumbnail}
...
![OVHcloud Federation login Reencaminhamento AD FS](ovhcloud_federation_login_2.png){.thumbnail}

Já está conectado com o mesmo identificador de cliente, mas através do utilizador Active Directory e do SSO AD FS.

![Federação das informações utilizadores OVHcloud](ovhcloud_user_infos_federation.png){.thumbnail}
