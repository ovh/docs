...
![Menu Windows Server](windows_server_tools_menu.png){.thumbnail}

Klicken Sie auf `Relying Party Trusts`{.action}.

![AD FS](adfs_menu.png){.thumbnail}

Klicken Sie dann auf `Add Relying Party Trust...`{.action}.

![AD FS](adfs_relying_party_trusts_menu.png){.thumbnail}

Wählen Sie `Claims aware`{.action} aus und bestätigen Sie mit dem `Start`{.action} Button.

![AD FS eine Genehmigung hinzufügen - Schritt 1](adfs_add_relying_party_trust_1.png){.thumbnail}
...
![AD FS - Genehmigung hinzufügen - Schritt 2](adfs_add_relying_party_trust_2.png){.thumbnail}

Geben Sie einen Anzeigenamen für die Relying Party ein und klicken Sie auf den Button `Next`{.action}.

![AD FS - Genehmigung hinzufügen - Schritt 3](adfs_add_relying_party_trust_3.png){.thumbnail}

Klicken Sie im `Next`{.action} im Fenster der Zugriffskontrolle.

![AD FS - Genehmigung hinzufügen - Schritt 4](adfs_add_relying_party_trust_4.png){.thumbnail}

Klicken Sie erneut auf `Next`{.action}, um fortzufahren.

![AD FS - Genehmigung hinzufügen - Schritt 5](adfs_add_relying_party_trust_5.png){.thumbnail}
...
![AD FS](adfs_relying_party_trusts.png){.thumbnail}
...
![AD FS-Zulassungsschema Schritt 1](adfs_relying_party_trusts_mapping_1.png){.thumbnail}

Klicken Sie anschließend auf `Edit Claim Issuance Policy ...`{.action}.

![AD FS-Zulassungsschema Schritt 2](adfs_relying_party_trusts_mapping_2.png){.thumbnail}

Klicken Sie auf den Button `Add Rule...`{.action}.

![AD FS-Zulassungsschema Schritt 3](adfs_relying_party_trusts_mapping_3.png){.thumbnail}
...
![AD FS-Zulassungsschema Schritt 4](adfs_relying_party_trusts_mapping_4.png){.thumbnail}

![AD FS-Zulassungsschema Schritt 5](adfs_relying_party_trusts_mapping_5.png){.thumbnail}

Klicken Sie auf den Button `Apply`{.action} und bestätigen Sie mit `OK`{.action}.

![Zustimmungsschema Schritt 6](adfs_relying_party_trusts_mapping_6.png){.thumbnail}
...
![OVHcloud Top Menü](ovhcloud_top_menu.png){.thumbnail}

Klicken Sie auf die Initialen, um auf die Verwaltungsseite Ihres Accounts zuzugreifen.

![OVHcloud Benutzerinformationen](ovhcloud_user_infos.png){.thumbnail}

Öffnen Sie den Tab `Verwaltung der Nutzer`{.action}.

![OVHcloud Menüprofil](ovhcloud_profile_menu.png){.thumbnail}

Klicken Sie auf den Button `SSO-Verbindung`{.action}.

![OVHcloud SSO-Verbindung Schritt 1](ovhcloud_user_management_connect_sso_1.png){.thumbnail}
...
![OVHcloud SSO-Verbindung Schritt 2](ovhcloud_user_management_connect_sso_2.png){.thumbnail}

Sie sollten nun Ihren AD FS als Identity Provider sowie die Standardgruppen sehen können.

![OVHcloud SSO-Verbindung Schritt 3](ovhcloud_user_management_connect_sso_3.png){.thumbnail}

Für weitere Informationen klicken Sie auf den Link unter der `URL des SSO Dienstes`.

![OVHcloud SSO-Verbindung Schritt 4](ovhcloud_user_management_connect_sso_4.png){.thumbnail}

![OVHcloud SSO-Verbindung Schritt 5](ovhcloud_user_management_connect_sso_5.png){.thumbnail}

Mit dem Button `...`{.action} können Sie den SSO aktualisieren oder löschen und die Details einsehen.

![OVHcloud SSO-Verbindung Schritt 6](ovhcloud_user_management_connect_sso_6.png){.thumbnail}
...
![AD FS Benutzer](adfs_user.png){.thumbnail}

Überprüfen Sie die Entsprechungstabelle in AD FS:

![AD FS-Bestätigungsschema](adfs_relying_party_trusts_mapping_4.png){.thumbnail}
...
![AD FS Benutzer-Verwaltungsgruppen](ovhcloud_user_management_groups_1.png){.thumbnail}

![AD FS Benutzer-Verwaltungsgruppen](ovhcloud_user_management_groups_2.png){.thumbnail}

Anschließend können Sie überprüfen, ob die Gruppe zu Ihrem OVHcloud Account im Bereich `Gruppen` hinzugefügt wurde:

![AD FS Benutzer-Verwaltungsgruppen](ovhcloud_user_management_groups_3.png){.thumbnail}
...
![Verbindung zum Verband OVHcloud](ovhcloud_federation_login_1.png){.thumbnail}
...
![OVHcloud Federation Login Weiterleitung AD FS](ovhcloud_federation_login_2.png){.thumbnail}

Sie sind nun mit derselben Kundenkennung eingeloggt, jedoch über AD FS SSO und mit Ihrem Active Directory Benutzer.

![OVHcloud User Info](ovhcloud_user_infos_federation.png){.thumbnail}
