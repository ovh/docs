...
![Menu Strumenti Windows Server](windows_server_tools_menu.png){.thumbnail}

Clicca su `Relying Party Trusts`{.action}.

![AD FS Menu](adfs_menu.png){.thumbnail}

Clicca su `Add Relying Party Trust`{.action}.

![Menu di approvazione AD FS](adfs_relying_party_trusts_menu.png){.thumbnail}

Seleziona `Claims aware`{.action} e conferma cliccando sul pulsante `Start`{.action}.

![AD FS aggiungi un'approvazione - Step 1](adfs_add_relying_party_trust_1.png){.thumbnail}
...
![AD FS - Aggiungere un'approvazione - Step 2](adfs_add_relying_party_trust_2.png){.thumbnail}

Inserisci il nome visualizzato per l'organismo terzo di fiducia e clicca sul pulsante `Next`{.action}.

![AD FS - Aggiungere un'approvazione - Step 3](adfs_add_relying_party_trust_3.png){.thumbnail}

Clicca su `Next`{.action} nella finestra di controllo degli accessi.

![AD FS - Aggiungere un'approvazione - Step 4](adfs_add_relying_party_trust_4.png){.thumbnail}

Clicca di nuovo su `Next`{.action} per continuare

![AD FS - Aggiungere un'approvazione - Step 5](adfs_add_relying_party_trust_5.png){.thumbnail}
...
![Approvazione AD FS](adfs_relying_party_trusts.png){.thumbnail}
...
![Passaggio di approvazione AD FS Step 1](adfs_relying_party_trusts_mapping_1.png){.thumbnail}

Clicca su `Edit Claim Issuance Policy...`{.action}.

![Passaggio di approvazione AD FS Step 2](adfs_relying_party_trusts_mapping_2.png){.thumbnail}

Clicca sul pulsante `Add Rule...`{.action}.

![Passaggio di approvazione AD FS 3](adfs_relying_party_trusts_mapping_3.png){.thumbnail}
...
![Passaggio di approvazione AD FS 4](adfs_relying_party_trusts_mapping_4.png){.thumbnail}

![Passaggio di approvazione AD FS 5](adfs_relying_party_trusts_mapping_5.png){.thumbnail}

Clicca sul pulsante `Apply`{.action} e conferma con `OK`{.action}.

![Passaggio di approvazione 6](adfs_relying_party_trusts_mapping_6.png){.thumbnail}
...
![Informazioni utente OVHcloud](ovhcloud_user_infos.png){.thumbnail}

Apri la scheda `Gestione utenti`{.action}.

![Profilo menu OVHcloud](ovhcloud_profile_menu.png){.thumbnail}

Clicca sul pulsante `SSO connection`{.action}.

![OVHcloud di connessione SSO Step 1](ovhcloud_user_management_connect_sso_1.png){.thumbnail}
...
![OVHcloud di connessione SSO Step 2](ovhcloud_user_management_connect_sso_2.png){.thumbnail}

A questo punto è necessario recuperare il tuo AD FS come provider di identità e i gruppi predefiniti.

![OVHcloud di connessione SSO Step 3](ovhcloud_user_management_connect_sso_3.png){.thumbnail}

Per maggiori informazioni, clicca sul link sotto l'`URL del servizio SSO`.

![OVHcloud di connessione SSO Step 4](ovhcloud_user_management_connect_sso_4.png){.thumbnail}

![OVHcloud di connessione SSO step 5](ovhcloud_user_management_connect_sso_5.png){.thumbnail}

Il pulsante `...`{.action} permette di aggiornare o eliminare l'SO e di consultarne i dettagli.

![OVHcloud di connessione SSO Step 6](ovhcloud_user_management_connect_sso_6.png){.thumbnail}
...
![Utente AD FS](adfs_user.png){.thumbnail}

Verifica la tabella di corrispondenza in AD FS:

![Mappatura di approvazione della parte di fiducia AD FS](adfs_relying_party_trusts_mapping_4.png){.thumbnail}
...
![Gruppi di gestione utenti AD FS](ovhcloud_user_management_groups_1.png){.thumbnail}

![Gruppi di gestione utenti AD FS](ovhcloud_user_management_groups_2.png){.thumbnail}

Verifica che il gruppo sia aggiunto al tuo account OVHcloud nella sezione `Gruppi`:

![Gruppi di gestione utenti AD FS](ovhcloud_user_management_groups_3.png){.thumbnail}
...
Attenzione: se concedi il privilegio `Nessuno`, sarà necessario assegnare i diritti a questo gruppo tramite le [politiche IAM](iam-policy-ui1.).

In seguito, potrai disconnetterti dal tuo account e collegarti con il tuo AD FS come provider di identità.

### Connessione via SSO

Nella [pagina di identificazione OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.it/&ovhSubsidiary=it), inserisci il tuo [identificativo cliente](ovhcloud-account-creation#qual-e-il-mio-identificativo-cliente.) seguito da **/idp** senza password e clicca sul pulsante `Login`{.action}.

![Connessione alla federazione OVHcloud](ovhcloud_federation_login_1.png){.thumbnail}
...
![OVHcloud Federation login Reindirizzamenti AD FS](ovhcloud_federation_login_2.png){.thumbnail}

Adesso sei connesso con lo stesso identificativo cliente, ma tramite l'utente Active Directory e il tuo SSO AD FS.

![Federazione delle informazioni degli utenti OVHcloud](ovhcloud_user_infos_federation.png){.thumbnail}

## Per saperne di più

[Crea un account OVHcloud](ovhcloud-account-creation1.)

[Rendere sicuro il tuo account OVHcloud e gestire le tue informazioni personali](all_about_username1.)

[Definizione e gestione della password del tuo account](manage-ovh-password1.)

[Rendere sicuro il tuo account OVHcloud con la doppia autenticazione](secure-ovhcloud-account-with-2fa1.)

[Come utilizzare le politiche IAM dallo Spazio Cliente](iam-policy-ui1.).
