...
This document is the reversibility policy for the **[Dedicated Servers](https://www.ovhcloud.com/en-ca/bare-metal/)** product range.
...
|---|-----|---|-----|-----|
|Supplying dedicated servers|Supplying different ranges of high-performance dedicated servers|N/A|**Inbound migration**: Order a dedicated server in the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca), backup and migrate data, re-install software (or use automated installation).<br><br>**Outbound migration**: Order dedicated servers, backup and migrate data, re-install software|[Getting started with a dedicated server](getting-started-with-dedicated-server1.)|
|Backup storage|Backup storage delivered by default with every dedicated server|NFS/CIFS/FTP|**Inbound migration**: Migrate data on your dedicated servers and activate backup storage on those servers through the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca).<br><br>**Outbound migration**: enable access from outside your account through OVHcloud API, then migrate data using standard file transfer protocols, such as FTP.|[Using backup storage on a dedicated server](services_backup_storage1.)|
|Operating systems and software installed on dedicated server|Automatically install operating systems, databases, administration interfaces and virtualization software on a new dedicated server.<br>See [here](https://www.ovhcloud.com/en-ca/bare-metal/os/) the complete list.|For customized images used with "bring your own image": <br>- Boot type: **uefi** or **legacy**<br>- Partition type: **MBR** or **GPT**<br>- Image format: **qcow2** or **raw**|**Inbound migration**: Order dedicated servers and choose operating systems/software to install on the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca). Alternatively, use the"bring your own image" feature to install customized images.<br><br>**Outbound migration**: Export your image and install on another dedicated server.|[Installing or reinstalling your dedicated server](getting-started-with-dedicated-server#installing-or-reinstalling-your-dedicated-server.)<br><br>[How to use the Bring Your Own Image feature](bring-your-own-image1.)|

### OVHcloud implementation <a name="ovhcloud-implementation"></a>

|Feature|Description|Available formats|Migration model|Available documentation|
|---|-----|---|-----|-----|
|Floating IPs|Additional IP that can be switched from one service to another in the same datacenter.|IPv4, IPv6|**Inbound migration**: Plan the network architecture, configure the Floating IPs in the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca) or through OVHcloud API.<br><br>**Outbound migration**: Record the configured setup in the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca) and configure a similar setup at another provider.|[Moving an Additional IP](move-failover-ip1.)<br><br>[Assigning a virtual MAC to an Additional IP](network_virtual_mac1.)|
...
All components of the OVHcloud Dedicated Server product are accessible through the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca). this allows to visualize and manage the dedicated servers, installed images, network configuration ... as well as the features that are attached to these components.
