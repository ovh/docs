...
- Ter [criado os ID para a API OVHcloud -EN](first-steps1.).

## Instruções

Para utilizar a [API de renovação](renew#GET.){.external}, vai precisar do `serviceId`. Esta informação está disponível na API `serviceInfos`; por exemplo:
