...
L'espace client OVHcloud vous permet de créer des [sauvegardes de vos instances](save_an_instance1.) en quelques clics et d'automatiser ce processus.
...
- Disposer d'une sauvegarde d'une instance Public Cloud. À cet effet, consultez [le guide dédié à la création d'une sauvegarde](save_an_instance1.).
...
Pour plus d'informations sur la création d'une instance, consultez [ce guide](public-cloud-first-steps#etape-3-creer-une-instance.).

> [!primary]
>
> Pour créer une instance dans un autre datacentre que celui de la sauvegarde, il faudra transférer celle-ci vers la zone correspondante. Référez-vous alors au [guide sur la sauvegarde d'une instance d'un datacentre à l'autre](transfer_instance_backup_from_one_datacentre_to_another1.).
>
...
[Création et connexion à une première instance Public Cloud](public-cloud-first-steps1.)
