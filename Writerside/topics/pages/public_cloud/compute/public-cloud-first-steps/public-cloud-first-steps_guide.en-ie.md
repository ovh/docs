...
> [!primary]
>
Stored SSH keys are useful for a faster creation of your instances. To change key pairs and add users afterwards, please refer to the guide on [Configuring additional SSH keys](configuring_additional_ssh_keys1.).
>

Log in to the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.ie/&ovhSubsidiary=ie), go to the `Public Cloud`{.action} section and select the Public Cloud project concerned. Then, click on `SSH Keys`{.action} in the left-hand navigation bar under "Project Management".
...
> [!success]
>
It is preferable to create a private network before creating an instance. For more information on creating a private network, see the [Configuring vRack for Public Cloud](getting-started-07-creating-vrack1.) guide.
>
...
```
You can now use these credentials to log in via the `VNC console`{.action} in your [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.ie/&ovhSubsidiary=ie). Next, switch to the "root" user and set a secure password, then switch back to the previous user:
...
[Backing up an instance](save_an_instance1.)

[Increasing Public Cloud quota](increasing_public_cloud_quota1.)

[Switching from hourly to monthly billing](changing_hourly_monthly_billing1.)

[Configuring additional SSH keys](configuring_additional_ssh_keys1.)
