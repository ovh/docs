...
Per effettuare questa operazione, consulta la guida [Creare e connettersi a un’istanza Public Cloud](public-cloud-first-steps1.).
...
In caso di perdita, consulta la guida [Sostituisci la tua chiave SSH in caso di perdita](replacing_lost_ssh_key1.).
...
Per eseguire questa operazione, consulta la guida [Effettuare lo Snapshot di un’istanza](save_an_instance1.).
...
Per effettuare questa operazione, consulta la guida [Creare e rimuovere un utente OpenStack](create_and_delete_a_user1.).
...
Per passare da una modalità di fatturazione all'altra, consulta la nostra guida [Passare dalla fatturazione oraria a quella mensile](changing_hourly_monthly_billing1.).
...
Per maggiori informazioni, consulta la guida [Creare e configurare un gruppo di sicurezza su Horizon](setup_security_group1.).
...
Per maggiori informazioni, consulta la guida [Configurazione della vRack Public Cloud](getting-started-07-creating-vrack1.).
...
Per maggiori informazioni, consulta la guida di [gestione delle regole di firewall e porta security sulle reti OpenStack CLI](security_group_private_network1.).
...
Per maggiori informazioni, consulta la guida [Acquista un Additional IP](additional-ip-buy1.).
...
Tuttavia, è possibile mitigare questa falla direttamente nell'istanza eseguendo un hard reboot dell'istanza [tramite lo Spazio Cliente OVHcloud](first_steps_with_public_cloud_instance1.) o utilizzando un comando come questo:
