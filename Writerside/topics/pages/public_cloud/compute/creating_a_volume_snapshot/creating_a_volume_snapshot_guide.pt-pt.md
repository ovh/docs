...
A criação de um backup de volume requer que o volume seja desassociado da instância. Para mais informações sobre esta opção, consulte por favor este [guia](volume-backup1.).
...
À direita do volume em questão, clique em `...`{.action} e depois em `Criar uma snapshot`{.action} (não é necessário desassociar primeiro o volume da sua instância.) No entanto, se pretender desassociar o seu volume, consulte [esta secção](create_and_configure_an_additional_disk_on_an_instance#em-linux_1.) do guia correspondente para Linux e [esta secção](create_and_configure_an_additional_disk_on_an_instance#em-windows_1.) para Windows.
...
Clique no botão `...`{.action} para eliminar uma snapshot ou `Criar um volume`{.action} a partir da snapshot correspondente. Para mais informações, consulte [este guia](create-volume-from-snapshot1.).

## Quer saber mais?

[Criar uma cópia de segurança de um volume](volume-backup1.)

[Criar um volume a partir de um backup](create-volume-from-snapshot1.)

[Criar e configurar um disco suplementar numa instância](create_and_configure_an_additional_disk_on_an_instance1.)

[Aumentar o tamanho de um disco suplementar](increase_the_size_of_an_additional_disk1.)
