...
Per creare un backup del volume è necessario scollegarlo dall'istanza. Per ulteriori informazioni su questa opzione, consultare questa [guida](volume-backup1.).
...
Clicca sul pulsante `...`{.action} per eliminare uno Snapshot o `Creare un volume`{.action} a partire dallo Snapshot corrispondente. Per maggiori informazioni, consulta [questa guida](create-volume-from-snapshot1.).

## Per saperne di più

[Creare un backup di un volume](volume-backup1.)
...
[Aumenta la spazio del tuo disco aggiuntivo](increase_the_size_of_an_additional_disk1.)
