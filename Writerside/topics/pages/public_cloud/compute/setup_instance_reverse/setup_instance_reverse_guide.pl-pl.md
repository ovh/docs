...
![Rewers DNS](images_reversecp01.png){.thumbnail}

Kliknij `...`{.action} w linii odpowiedniego adresu IP i wybierz `Zmień rewers`{.action}.

![Rewers DNS](images_reversecp02.png){.thumbnail}
...
> [!primary]
>
Jeśli modyfikacja nie działa zgodnie z oczekiwaniami, sprawdź, czy pole `A` jest poprawnie skonfigurowane w strefie DNS Twojej domeny. Uwaga, modyfikacja [strefy DNS](dns_zone_edit1.) może trwać do 24 godzin, jeśli ostatnio zmieniłeś pole `A`.
>
