...
- [Configuring user access to Horizon](introducing_horizon1.)
...
- [Your OVHcloud API identifiers and authorisation key](first-steps1.)
- [An SSH key](public-cloud-first-steps1.)
...
You now need to [create a new OpenStack user](create_and_delete_a_user1.), then [generate the OpenRC file](loading_openstack_environment_variables#etapa-1-recuperar-as-variaveis.) containing all of the credentials you want to export as environment variables.
...
Now, log in to the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pt/&ovhSubsidiary=pt), go to the `Public Cloud`{.action} section and click on `Instances`{.action}.
...
In that case, the only solution is to use the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pt/&ovhSubsidiary=pt) to create a project.<br>
