...
- [Estar conectado a Horizon](introducing_horizon1.){.external}. 

## Procedimiento

En primer lugar, conéctese a Horizon. Si necesita ayuda, consulte nuestra [guía](introducing_horizon1.).

A continuación, haga clic en `Compute`{.action} en la columna izquierda y seleccione `Instances`{.action}.

![Crear una instancia](create-instance-step1.png){.thumbnail}
...
![Crear una instancia](create-instance-step2.png){.thumbnail}
...
> [!warning]
> 
> Aunque el campo "Key Pair" no es obligatorio en la interfaz Horizon al crear una instancia, es absolutamente necesario registrar una llave SSH para poder conectarse a una instancia. Sin una llave SSH, deberá reiniciar la instancia en modo de rescate para poder crear una contraseña o añadir una llave SSH al archivo correspondiente (para más información, consulte la guía [Modificar su llave SSH en caso de pérdida](replacing_lost_ssh_key#procedimiento.)).
>
...
![Crear una instancia](create-instance-step3.png){.thumbnail}
