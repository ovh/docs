...
<br>Infine, proponiamo risorse per [spingervi oltre](00-essential-info-to-get-started-on-public-cloud_#gofurther.).
...
- [Apri un account OVHcloud](ovhcloud-account-creation1.)
- [Crea il tuo primo progetto Public Cloud](create_a_public_cloud_project1.)
- [Scopri l'interfaccia Public Cloud](03-public-cloud-interface-walk-me1.)
- [Crea la tua prima istanza](public-cloud-first-steps1.)
- [Gestisci le quote](increasing_public_cloud_quota1.)
...
Per iniziare, ti serve un [account cliente OVHcloud](ovhcloud-account-creation1.).

A questo punto [crei un progetto Public Cloud](create_a_public_cloud_project1.). Un progetto è un ambiente che voi dedicherete a un contesto.
...
Una volta registrato il metodo di pagamento, verrà utilizzato per addebitare l'importo calcolato per la tua [fatturazione alla fine del mese](analyze_billing1.). Questo calcolo è effettuato in base al tempo di utilizzo di ogni risorsa, in funzione del prezzo delle risorse.
...
<br>Tali quote sono assegnate per localizzazione (regione in senso OpenStack). È quindi possibile che tu raggiunga il massimo delle risorse possibili sul tuo progetto e che sia necessario [aumentare queste quote](increasing_public_cloud_quota1.).
...
- Per utilizzare le API OpenStack o S3 o l'interfaccia Horizon, è necessario [creare degli utenti](create_and_delete_a_user1.). Gli utenti possono avere diritti limitati per proteggere i perimetri di azione.
- Se non hai bisogno di accedere alle API o a Horizon, puoi [associare al tuo progetto un altro account cliente OVHcloud](delegate_projects1.).
...
|---|---|
|[FAQ](faq_pci1.)|Le domande più frequenti su Public Cloud|
|[Lessico](introduction_about_instances1.)|I concetti e le definizioni di cui avete bisogno per progredire.|
...
|[Changelog delle immagini](image_changelog1.)|Modifiche alle immagini di sistema disponibili al pubblico.|

Nella pratica, ecco alcune guide che ti aiuteranno ad avviare:

|Documentazione|Descrizione|
|---|---|
|[Crea la tua prima istanza](public-cloud-first-steps1.)|La prima guida pratica per avviare un server Cloud dallo Spazio Cliente OVHcloud.|
|[Utilizzo di una chiave SSH](public-cloud-first-steps#step-2-salva-le-chiavi-pubbliche-nello-spazio-cliente-ovhcloud_1.)| Per connetterti a un'istanza Linux, dovrai connetterti in SSH tramite questa guida.|
|[Configura la rete privata](getting-started-07-creating-vrack1.)|In OVHcloud le reti private sono basate sulla tecnologia vRack. Questa guida ti mostra come eseguire questa operazione.|
|[Associa un disco aggiuntivo a un'istanza](create_and_configure_an_additional_disk_on_an_instance1.)|Questa guida ti mostra come aggiungere spazio di storage supplementare alla tua prima istanza.|
|[Accedere all’interfaccia Horizon](introducing_horizon1.)|L'interfaccia Horizon di OpenStack permette alcune azioni avanzate: ecco come connettersi.|
|[Crea un cluster Kubernetes](creating-a-cluster1.) (EN)|Questa guida ti mostra come creare il tuo primo cluster Kubernetes.|
|[Configura un Additional IP](getting-started-04-configure-additional-ip-to-instance1.)|Gli Additional IP ti permettono di trasferire il traffico da un'istanza all'altra, questa guida ti mostra come configurarlo.|
|[Usare l'interfaccia Horizon](introducing_horizon1.)|Horizon è la classica interfaccia di OpenStack e questa guida spiega come accedervi.|
|[Installazione della CLI OpenStack](prepare_the_environment_for_using_the_openstack_api1.)|OpenStack può essere utilizzato anche da riga di comando, come installare gli strumenti.|
