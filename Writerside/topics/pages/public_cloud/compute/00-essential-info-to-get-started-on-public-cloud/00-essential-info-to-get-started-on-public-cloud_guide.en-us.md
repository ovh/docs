...
<br>Finally, we offer resources for [going further](00-essential-info-to-get-started-on-public-cloud_#gofurther.).
...
- [Open an OVHcloud account](ovhcloud-account-creation1.)
- [Create your first Public Cloud project](create_a_public_cloud_project1.)
- [Familiarise yourself with the Public Cloud interface](03-public-cloud-interface-walk-me1.)
- [Create your first instance](public-cloud-first-steps1.)
- [Manage quotas](increasing_public_cloud_quota1.)
...
To get started, you will first need an OVHcloud [customer account](ovhcloud-account-creation1.).

You will then need to [create a Public Cloud project](create_a_public_cloud_project1.). A project is an environment that you will dedicate to a context.
...
Since your payment method has been saved, it will be used to debit the amount calculated for your [billing at the end of the month](analyze_billing1.). This calculation is based on the usage time of each resource, depending on the price of the resources.
...
<br>These quotas are allocated by region (in the OpenStack sense). As a result, you may reach the maximum possible resources for your project, and you may need to [increase these quotas](increasing_public_cloud_quota1.).
...
- If you would like to use the OpenStack or S3 APIs, or the Horizon interface, you will need to [create users](create_and_delete_a_user1.) to do this. Users may have limited permissions to secure the action perimeters.
- If you do not need access to the APIs or Horizon, you can [link another OVHcloud](delegate_projects1.) customer account as an add-on to your project.
...
|---|---|
|[FAQ](faq_pci1.)|Frequently asked questions about Public Cloud.|
|[Glossary](introduction_about_instances1.)|The concepts and definitions you will need to move forward.|
...
|[Image Changelog](image_changelog1.)|Changes to publicly available system images.|

Here are some guides to help you with the first steps:

|Documentation|Details|
|---|---|
|[Create your first instance](public-cloud-first-steps1.)|First practical guide to starting a cloud server from the OVHcloud Control Panel.|
|[Using an SSH key](public-cloud-first-steps#step-2-storing-public-keys-in-the-ovhcloud-control-panel_1.)| To log in to a Linux instance, you will need to log in via an SSH connection. This guide details the procedure.|
|[Private Network Configuration](getting-started-07-creating-vrack1.)|At OVHcloud, private networks are powered by vRack technology. This guide will help you get started.|
|[Attach an additional disk to an instance](create_and_configure_an_additional_disk_on_an_instance1.)|This guide will help you add additional storage to your first instance.|
|[Accessing the Horizon interface](introducing_horizon1.)|The Horizon interface of OpenStack allows some advanced actions, here is how to connect to it.|
|[Creating a Kubernetes cluster](creating-a-cluster1.)|This guide will help you step by step in creating your first Kubernetes cluster.|
|[Configuring an Additional IP](getting-started-04-configure-additional-ip-to-instance1.)|Additional IPs can allow you to switch traffic from one instance to another. This guide will explain how to configure this.|
|[Installing the OpenStack CLI](prepare_the_environment_for_using_the_openstack_api1.)|OpenStack can also be used in the command line. Here is how to install the tools.|
