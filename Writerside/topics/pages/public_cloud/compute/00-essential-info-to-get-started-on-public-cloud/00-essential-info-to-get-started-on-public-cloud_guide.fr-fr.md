...
<br>Enfin, nous vous proposons des ressources pour [aller plus loin](00-essential-info-to-get-started-on-public-cloud_#gofurther.).
...
- [Ouvrir un compte OVHcloud](ovhcloud-account-creation1.)
...
- [Créer sa première instance](public-cloud-first-steps1.)
- [Gérer ses quotas](increasing_public_cloud_quota1.)
...
Pour démarrer, vous aurez d'abord besoin d'un [compte client OVHcloud](ovhcloud-account-creation1.).

Vous devrez ensuite [créer un projet Public Cloud](create_a_public_cloud_project1.). Un projet est un environnement que vous allez dédier à un contexte.
...
Votre moyen de payement étant enregistré, il servira à débiter le montant calculé pour votre [facturation en fin de mois](analyze_billing1.). Ce calcul est fait sur la base du temps d'utilisation de chaque ressource, en fonction du prix des ressources.
...
<br>Ces quotas sont attribués par localisation (région au sens OpenStack). Il se peut donc que vous atteigniez le maximum de ressources possibles sur votre projet et qu'il faille [augmenter ces quotas](increasing_public_cloud_quota1.).
...
- Si vous souhaitez utiliser les APIs OpenStack ou S3, ou l'interface Horizon, vous aurez à [créer des utilisateurs](create_and_delete_a_user1.) pour cela. Les utilisateurs peuvent éventuellement avoir des droits limités afin de sécuriser les périmètres d'action.
- Si vous n'avez pas besoin d'accéder aux APIs ou à Horizon, vous pouvez [associer un autre compte client OVHcloud](delegate_projects1.) en complément sur votre projet.
...
|---|---|
|[FAQ](faq_pci1.)|Les questions les plus fréquentes au sujet de Public Cloud.|
|[Lexique](introduction_about_instances1.)|Les concepts et définitions dont vous aurez besoin pour avancer.|
...
|[Changelog des images](image_changelog1.)|Les changements apportés aux images système disponibles publiquement.|

Dans la pratique, voici également quelques guides qui vous aideront au démarrage :

|Documentation|Détails|
|---|---|
|[Créer sa première instance](public-cloud-first-steps1.)|Premier guide pratique pour démarrer un serveur cloud depuis l'espace client OVHcloud.|
|[Utilisation d'une clé SSH](public-cloud-first-steps#etape-2-stocker-les-cles-publiques-dans-lespace-client-ovhcloud_1.)| Pour vous connecter à une instance Linux, vous aurez besoin de passer par une connexion SSH, ce guide vous en détaille l'utilisation.|
|[Configuration du réseau privé](getting-started-07-creating-vrack1.)|Chez OVHcloud, les réseaux privés sont portés par la technologie vRack. Ce guide vous accompagne dans cette mise en place.|
|[Attacher un disque supplémentaire à une instance](create_and_configure_an_additional_disk_on_an_instance1.)|Ce guide vous accompagne pour ajouter du stockage additionnel à votre première instance.|
|[Accéder à l'interface Horizon](introducing_horizon1.)|L'interface Horizon d'OpenStack permet certaines actions avancées, voici comment vous y connecter.|
|[Créer un cluster Kubernetes](creating-a-cluster1.) (EN)|Ce guide vous accompagne pas à pas dans la création de votre premier cluster Kubernetes.|
|[Configurer une Additional IP](getting-started-04-configure-additional-ip-to-instance1.)|Les Additional IP peuvent vous permettre de basculer le trafic d'une instance à une autre, ce guide vous expliquera comment configurer cela.|
|[Utilisation de l'interface Horizon](introducing_horizon1.)|Horizon est l'interface classique d'OpenStack, ce guide vous expliquera comment y accéder.|
|[Installation de la CLI OpenStack](prepare_the_environment_for_using_the_openstack_api1.)|OpenStack peut également être utilisé en ligne de commande, voici comment installer les outils.|
