...
<br>Enfim, propomos-lhe recursos para [ir mais longe](00-essential-info-to-get-started-on-public-cloud_#gofurther.).
...
- [Abrir uma conta OVHcloud](ovhcloud-account-creation1.)
...
- [Criar a primeira instância](public-cloud-first-steps1.)
- [Gerir as quotas](increasing_public_cloud_quota1.)
...
Para começar, precisará de uma [conta de cliente OVHcloud](ovhcloud-account-creation1.).

De seguida, terá de [criar um projeto Public Cloud](create_a_public_cloud_project1.). Um projeto é um ambiente que vão dedicar a um contexto.
...
Com o seu método de pagamento registado, este servirá para debitar o montante calculado para a sua [faturação no final do mês](analyze_billing1.). Este cálculo é feito com base no tempo de utilização de cada recurso, em função do preço dos recursos.
...
<br>Estas quotas são atribuídas por localização (região no sentido OpenStack). Por isso, é possível que atinja o máximo de recursos possíveis no seu projeto e que seja necessário [aumentar estas quotas](increasing_public_cloud_quota1.).
...
- Se deseja utilizar as APIs OpenStack ou S3, ou a interface Horizon, terá de [criar utilizadores](create_and_delete_a_user1.) para isso. Os utilizadores podem ter permissões limitadas para proteger os perímetros de ação.
- Se não tem necessidade de aceder às API ou ao Horizon, pode [associar uma outra conta de cliente OVHcloud](delegate_projects1.) em complemento ao seu projeto.
...
|---|---|
|[FAQ](faq_pci1.)|As questões mais frequentes sobre o Public Cloud.|
|[Léxico](introduction_about_instances1.)|Os conceitos e definições de que necessitará para avançar.|
...
|[Muelog das imagens](image_changelog1.)|Alterações nas imagens de sistema disponíveis publicamente.|

Na prática, pode consultar os seguintes manuais:

|Documentação|Detalhes|
|---|---|
|[Criar a primeira instância](public-cloud-first-steps1.)|Primeiro guia prático para iniciar um servidor cloud a partir da Área de Cliente OVHcloud.|
|[Utilização de uma chave SSH](public-cloud-first-steps#2o-passo-armazenamento-de-chaves-publicas-na-area-de-cliente-ovhcloud_1.)| Para se ligar a uma instância Linux, terá de passar por uma ligação SSH, este manual explica-lhe a utilização.|
|[Configuração da rede privada](getting-started-07-creating-vrack1.)|Na OVHcloud, as redes privadas são suportadas pela tecnologia vRack. Este guia acompanha-o nesta implementação.|
|[Associar um disco suplementar a uma instância](create_and_configure_an_additional_disk_on_an_instance1.)|Este manual fornece instruções para adicionar espaço de armazenamento adicional à primeira instância.|
|[Aceder à interface Horizon](introducing_horizon1.)|A interface Horizon da OpenStack permite algumas ações avançadas. Aqui está como conectá-lo.|
|[Criar um cluster Kubernetes](creating-a-cluster1.) (EN)|Este guia acompanha-o passo a passo na criação do seu primeiro cluster Kubernetes.|
|[Configurar um Additional IP](getting-started-04-configure-additional-ip-to-instance1.)|Os endereços Additional IP permitem-lhe transferir o tráfego de uma instância para outra, este guia explica-lhe como configurar isto.|
|[Usando a interface Horizon](introducing_horizon1.)|Horizon é a interface clássica do OpenStack, e este guia explicará como acessá-la.|
|[Instalação da CLI OpenStack](prepare_the_environment_for_using_the_openstack_api1.)|OpenStack pode também ser utilizado em linha de comandos: eis como instalar as ferramentas.|
