...
First, follow the guide [Creating a private registry](creating-a-private-registry1.) to create a new private registry named `my-new-registry` for example, with the latest version of Harbor.

![My new private registry](my-new-registry-02.png)

Follow the guide [Connecting to the UI](connecting-to-the-ui1.) to connect to your new private registry. 
...
![Create a replication between two registries](registries.png){.thumbnail}
...
![New private registry endpoint](new-registry-endpoint.png){.thumbnail}

Click on the button `Test connection`{.action} to ping the private registry and test your information.

![New private registry endpoint](connection-tested-ok.png){.thumbnail}

Click on the button `OK`{.action} to save your information.

![New private registry endpoint](new-registry-endpoint-created.png){.thumbnail}

In the `Administration` menu, click on `Replications`{.action}.

![Replications](replications.png){.thumbnail}
...
![New replicaton rule](new-replication-rule.png){.thumbnail}

Click on `Save`{.action} to save your information.

![Replication rule created](replication-rule-created.png){.thumbnail}

To run manually a replication rule, select your replication rule and click on the `Replicate`{.action} button.

![Replication rule](replicate-button.png){.thumbnail}

Then click on `Replicate`{.action} to run the replication.

![Replication rule](are-you-sure.png){.thumbnail}
...
![Replication rule](execution-status.png){.thumbnail}
![Replication rule](execution-status-success.png){.thumbnail}
