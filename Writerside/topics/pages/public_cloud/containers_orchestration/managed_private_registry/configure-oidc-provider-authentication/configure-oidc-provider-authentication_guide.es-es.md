...
![OIDC](images_oidc.png){.thumbnail}
...
![Keycloak](images_keycloak.png){.thumbnail}
...
- An OVHcloud [Managed Private Registry](creating-a-private-registry1.).
...
Follow the official guide if you want to [install Keycloack on an OVHcloud Managed Kubernetes cluster](installing-keycloak1.).
...
![Log in to the API Explorer](configure-oidc-provider-authentication_images_kubernetes-quickstart-api-ovh-com-001.png){.thumbnail}
...
![Kubernetes section of the API Explorer](configure-oidc-provider-authentication_images_kubernetes-quickstart-api-ovh-com-002.png){.thumbnail}
...
![Harbor login via oidc provider](configure-oidc-provider-authentication_images_login.png){.thumbnail}

When you click the `Login via OIDC provider`{.action} button, you are redirected to the Keycloack interface. 

![keycloack login](keycloack-login.png){.thumbnail}
...
![Harbor login via OIDC provider](autoonboard_off.png){.thumbnail}
...
![keycloack group](keycloack_groups.png){.thumbnail}

Check you have at least one member in the group:

![keycloack group member](keycloack_group_member.png){.thumbnail}
...
![admin user](admin.png){.thumbnail}
...
To have an overview of OVHcloud Managed Private Registry service, read the [OVHcloud Managed Private Registry documentation](public-cloud-containers-orchestration-managed-private-registry1.).
