...
This is detailed in the [quickstart with OVH API](deploying-hello-world-ovh-api1.) guide.

GPU (`T1-*`) instances are now supported! If you want to know [how to deploy GPU applications on Kubernetes Managed Service](deploying-gpu-application1.), please read our tutorial.
...
All these `Storage Classes` are based on Cinder, the OpenStack block storage service. The difference between them is the associated physical storage device, and the fact that `csi-cinder-high-speed` uses SSD, while `csi-cinder-classic` uses traditional spinning disks. This is detailed in the [Persistent Volumes ](persistent-volumes-on-ovh-managed-kubernetes1.) guide.
