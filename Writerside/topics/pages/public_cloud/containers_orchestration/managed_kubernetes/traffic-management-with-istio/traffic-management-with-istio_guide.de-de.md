...
![Bookinfo architecture](bookinfo-architecture.png){.thumbnail}
...
![A/B Testing - At every refresh you will see that stars change alternatively between back (`v2`) and red (`v3`)](ab-testing-001.png){.thumbnail}
...
![Canary Release v1 to v2 progressing release](canary-schema.png){.thumbnail}
...
![Canary Testing - all traffic to v1](canary-testing-001.png){.thumbnail}
...
![Canary Testing - one tenth of times we get v2](canary-testing-002.png){.thumbnail}
...
![Rolling Deployment](rolling-deployment-001.png){.thumbnail}
...
![Rolling deployment - At this moment traffic will be equally split between the two releases of `reviews`](rolling-deployment-002.png){.thumbnail}
...
![Rolling deployment - And now the `v2` of `reviews` receives all the traffic](rolling-deployment-003.png){.thumbnail}
