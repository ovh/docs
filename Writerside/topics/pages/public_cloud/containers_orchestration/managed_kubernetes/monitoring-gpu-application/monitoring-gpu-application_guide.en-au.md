...
![GPU Metrics Visualization](gpu-metrics-visualization.png){.thumbnail}
...
And you also need to follow [Deploying a GPU application on OVHcloud Managed Kubernetes](deploying-gpu-application1.) tutorial to install NVIDIA GPU operator and configure your cluster correctly with needed components for this guide.
...
As you have already followed the [Deploying a GPU application on OVHcloud Managed Kubernetes](deploying-gpu-application1.) tutorial, you should already have `dcgm-exporter` running in your cluster.
...
![Prometheus Architecture](images_prometheus-scraping-schema.png){.thumbnail}
...
![Prometheus](images_prometheus.png){.thumbnail}
...
![Grafana](monitoring-gpu-application_images_grafana.png){.thumbnail}

![Grafana Home Page](images_grafana-home-page.png){.thumbnail}
...
![Grafana Import menu](grafana-import.png){.thumbnail}

Import the NVIDIA dashboard from https://grafana.com/grafana/dashboards/12239, click on the `Load`{.action} button:

![Grafana Import menu](grafana-import-dashboard.png){.thumbnail}

Then choose `Prometheus`{.action} as the data source in the drop down menu and click on the `Import`{.action} button:

![Grafana Import menu](grafana-import-dashboard-2.png){.thumbnail}

You should now see your new dashboard:

![Grafana NVIDIA DCGM Exporter Dashboard](grafana-dcgm-exporter-dashboard.png){.thumbnail}

If you followed the [Deploying a GPU application on OVHcloud Managed Kubernetes](deploying-gpu-application1.) tutorial, you should see metrics like in our example.

You can click on the `instance`{.action} drop down menu in order to visualize GPU metrics for another Node for example:

![Grafana NVIDIA DCGM Exporter Dashboard other node](grafana-dcgm-exporter-dashboard-2.png){.thumbnail}
...
![Grafana Demo GPU metrics](grafana-gpu-metrics.png){.thumbnail}
![Grafana Demo GPU metrics](grafana-gpu-metrics-2.png){.thumbnail}
![Grafana Demo GPU metrics](grafana-gpu-metrics-3.png){.thumbnail}
