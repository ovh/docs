...
- [Requirements](backup-and-restore-cluster-namespace-and-applications-with-trilio_#requirements.)
...
- To create Target for the `OVHcloud Object Storage using S3 Swift API`, use [this link](pcs_create_container#creating-an-object-storage-container-from-the-ovhcloud-control-panel.).
- To create Target for the `OVHcloud Object Storage using High Performance`, use [this link](s3_getting_started_with_object_storage1.)
...
Next, create an Access Key and Secret Key to access the S3 Object Storage Container using the [Getting Started with the Swift S3 API](pcs_getting_started_with_the_swift_s3_api1.) tutorial.
 
> [!primary]
>
> If you have created a container with High Performance then follow the [Getting started with S3 High Performance](s3_getting_started_with_object_storage#using-aws-cli.) documentation.
...
![TVK Home Cluster Dashboard](tvk_cluster_dashboard.png){.thumbnail}
...
- `Backup & Recovery`{.action}: This is the main dashboard which gives you a general overview for the whole cluster, like: Discovered namespaces, Applications, Backupplans list, Targets, Hooks, Policies etc.
    - `Namespaces`{.action}:
  ![TVK Cluster Namespaces](tvk_console_home_namespaces.png){.thumbnail}
    - `Applications`{.action}:
  ![TVK Auto-discovered Applications](tvk_auto_discovered_applications.png){.thumbnail}
    - `Backupplans`{.action}:
  ![TVK Backupplans](tvk_backupplans.png){.thumbnail}
    - `Targets`{.action}:
  ![TVK Target List](tvk_target_list.png){.thumbnail}
    - `Scheduling Policy`{.action}:
  ![TVK Default Scheduling Policy](tvk_default_scheduling_policies.png){.thumbnail}
    - `Retention Policy`{.action}:
  ![TVK Default Retention Policy](tvk_default_retention_policies.png){.thumbnail}
  ...
  ![TVK TrilioVault Monitoring Backups and Restores](tvk_triliovault_monitoring.png){.thumbnail}
    - `Velero Monitoring`{.action}:
  ![TVK Velero Monitoring](tvk_velero_monitoring.png){.thumbnail}
- `Disaster Recovery`{.action}: Allows you to manage and perform disaster recovery operations.
  ![TVK Disaster Recovery](tvk_disaster_recovery.png){.thumbnail}
...
![TVK Target Browser](tvk_target_browser.png){.thumbnail}
...
Next, re-create the cluster as described in [Creating a OVHcloud Managed Kubernetes Cluster](creating-a-cluster#instructions.).
...
>
> ![TVK Default Scheduling Policies](tvk_default_scheduling_policies.png){.thumbnail}
>
