...
If you want to know more on those topics, please look at the [OVHcloud Managed Kubernetes Service Quickstart](deploying-hello-world1.).
...
![persistent-volumes](working-with-persistent-volumes-01.png){.thumbnail}
...
Since Kubernetes 1.11, support for expanding `PersistentVolumeClaims` (PVCs) is enabled by default, and it works on Cinder volumes. In order to learn how to resize them, please refer to the [Resizing PersistentVolumes](resizing-persistent-volumes1.) tutorial. Kubernetes PVCs resizing only allows to expand volumes, nor to decrease them.
...
![Kubernetes PV Schema](pv-schema.png)
