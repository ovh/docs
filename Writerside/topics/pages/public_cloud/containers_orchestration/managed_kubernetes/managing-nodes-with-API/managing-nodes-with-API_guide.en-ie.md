...
You can also enable the `autoscale` feature, and the `desiredNodes` will be automatically updated at runtime within the `minNodes` and `maxNodes` boundaries, depending on the resource reservations of your workload (see [Using the cluster autoscaler](using-cluster-autoscaler1.)).
...
![Log in to the API Explorer](managing-nodes-with-API_images_kubernetes-quickstart-api-ovh-com-001.png){.thumbnail}
...
Otherwise to skip it and push to deploy your first application on your Kubernetes cluster, we invite you to follow our guide to [configuring default settings for `kubectl`](configuring-kubectl-on-an-ovh-managed-kubernetes-cluster1.) and [deploying an application](deploying-an-application1.).
