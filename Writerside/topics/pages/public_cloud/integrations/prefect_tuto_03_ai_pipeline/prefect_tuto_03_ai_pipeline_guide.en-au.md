...
The model used will be a classical **PyTorch** image classification model. If you want to have more details about this model, don't hesitate to consult [our tutorial](training_tuto_01_train_your_first_model1.). The AI pipeline can be represented as follows:

![image](flows_email_job.png){.thumbnail}

## Requirements

- A Prefect cloud profile and an open Prefect workspace. See this [tutorial](prefect_guide_01_getting_started1.)
- A coding environment with Prefect configured. See this [tutorial](prefect_guide_01_getting_started1.)
- Access to the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com.au/&ovhSubsidiary=au)
- A [Public Cloud project](https://www.ovhcloud.com/en-au/public-cloud/)
- An S3 data store configured (you can do this easily with cli: `ovhai data store ls`. If you have never done this before, you can check [our tutorial](gi_08_s3_compliance1.)
...
Follow our [S3 official documentaiton for credentials here](s3_identity_and_access_management1.). 
Keep the access key and the secret key, we will need them later. 

Next, let's synchronize our S3 credentials with the boto3 client. To do this, we have to create a folder and two files in it. To configurate the boto3 client, we recommend following this [part](s3_getting_started_with_object_storage#configuration.) of the tutorial. Once the two files are created and you have filled them with your credentials, we can start coding and create our S3 object storage with a Prefect flow.
...
We require certain credentials to use this protocol. You can get these credentials as S3 users. You can create them in the OVHcloud control panel, go to your public cloud project and open the object storage tab. To know more about these credentials, consult this [guide](s3_identity_and_access_management1.).
...
In this flow, we will need a Python client that can connect to the OVHcloud API. This is our first task. If you have followed the [getting started tutorial](prefect_guide_01_getting_started1.), you should already be able to perform this task.
...
![image](button_job.png){.thumbnail}
...
- Learn about AI Training jobs with tutorials : [AI Training - Tutorials](public-cloud-ai-and-machine-learning-ai-training-tutorials1.)
