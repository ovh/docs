...
![image](marine-mammals-categories.png){.thumbnail}
...
- Two [Object Storage containers](pcs_create_container1.) to store the data and the model
- Your own dataset
...
If you want to upload it from the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/fr/&ovhSubsidiary=qc), go to the Object Storage section and [create a new object container](pcs_create_container1.) by clicking `Object Storage`{.action} > `Create an object container`{.action}.

![image](notebook_tuto_06_marine_mammal_sounds_classification_images_new-object-container.png){.thumbnail}
...
If you want to run it with the CLI, just follow this [this guide](cli_17_how_to_cli_data_notebooks1.). You have to choose the region, the name of your container and the path where your data is located and use the following command:
...
You need to attach a volume if your data is in your OVHcloud Object Storage and you want to use it during your experiment. For more information on data, volumes and permissions, see [our guide on data](cli_17_how_to_cli_data_notebooks1.).
...
If you want to launch it with the CLI, choose the [volume](cli_17_how_to_cli_data_notebooks1.) you want to attach and the number of GPUs (`<nb-gpus>`) to use on your notebook and use the following command:
...
- If you want to train your model, please check out this [AI Training tutorial](training_tuto_08_train_marine_mammal_sound1.).
- If you want to deploy a **Streamlit** app in order to classify marine mammal sounds using your model, please check out this [notebook](deploy_tuto_03_streamlit_sounds_classification1.).
- You can also compare two methods for audio classification task by following this [tutorial](training_tuto_06_models_comparaison_weights_and_biases1.).
