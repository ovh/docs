...
During its lifetime, the [AI Training job](training_guide_03_concepts_jobs1.) will go through the following statuses:
...
- `FAILED`: The job ended with an error, e.g. the process in the job finished with a non 0 exit code, Docker image could not be pulled. For more information, refer to [this section](training_guide_07_troubleshooting#why-has-my-job-failed.) of our [Troubleshooting documentation](training_guide_07_troubleshooting1.).
- `ERROR`: The job ended due to a backend error. You may contact our support.

![lifecycle](ai.training.lifecycle.png){.thumbnail}
...
![items](ai.training.items.png){.thumbnail}

A more detailed view:

![billing](ai.training.billing.png){.thumbnail}
