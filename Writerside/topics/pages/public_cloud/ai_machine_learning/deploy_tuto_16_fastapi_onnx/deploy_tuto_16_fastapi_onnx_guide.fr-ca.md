...
For more information on how to train DenseNet on a CIFAR-10 dataset, refer to the following [documentation](notebook_tuto_15_finetune_export_onnx_model1.).

Here is an overview of the **image classification** API:

![APIOverview](overview-fastapi-image-classificaion.png){.thumbnail}
...
> [!warning]
> **Warning**
> You must have previously created a `densenet-cifar10-onnx-model` Object Storage bucket when training your model via [AI Notebooks](notebook_tuto_15_finetune_export_onnx_model1.).
>
...
![APIAccess](access-fastapi-url.png){.thumbnail}
...
![APIDocs](fastapi-docs-endpoint.png){.thumbnail}
...
![APIImage](fastapi-send-image.png){.thumbnail}

To get the result of the prediction, click on the `Execute`{.action} button.

![APIPrediction](fastapi-get-prediction.png){.thumbnail}

Congratulations! You have obtained the results of the prediction with the labels and the confidence scores.

## Go further

- You can imagine deploying an image segmentation app through this [tutorial](deploy_tuto_14_img_segmentation_app1.).
- Feel free to use **Streamlit** to [deploy a Speech-to-Text app](deploy_tuto_09_streamlit_speech_to_text_app1.).
