...
![app add label](01-app-add-label.png){.thumbnail}
...
![token generation input read](05-token-gen-input-read.png){.thumbnail}
...
![token generation result](06-token-gen-result-read.png){.thumbnail}
...
An operator token grants read access along with management access for the matching apps. This means that you can manage the AI Deploy app lifecycle (start/stop/delete) using either the CLI (more info [here](cli_10_howto_install_cli1.)) or the [AI Training API](https://gra.ai.cloud.ovh.net/) by providing this token.
...
![token generation input operator](07-token-gen-input-op.png){.thumbnail}

Additional information about the use of a token to manage an AI Training resource can be found [here](cli_13_howto_app_token_cli#use-the-app-token.).
...
![app access login](11-app-access-credentials.png){.thumbnail}
...
![app response](12-app-access-result.png){.thumbnail}
...
![token regenerate](08-token-list-regen.png){.thumbnail}

Then click on `Regenerate`{.action} to confirm.

![token regenerate confirm](09-token-regen-confirm.png){.thumbnail}
...
![token delete](10-token-list-delete.png)
