...
![image](overview_tensorboard.png){.thumbnail}
...
Alternatively, you can have a job already `RUNNING` that is plugged with that object store container and is writting metric logs inside it (exemple: `my_tf_metrics@GRA:/runs:RW:cache`). In that last case, don't forget the `cache` parameter indicating that the volume is cached and sharable among jobs. More information about volumes configuration in jobs can be found [here](cli_12_howto_run_job_cli#attaching-volumes.), information about volume caching can be found [here](gi_02_concepts_data#capabilities.).
...
- Check how to run Tensorflow code with GPUs [here](training_tuto_03_tensorflow_gpu1.).
