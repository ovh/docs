...
For more information on how to train U-Net on the [BraTS2020 Dataset](https://www.kaggle.com/datasets/awsaf49/brats20-dataset-training-validation), refer to the following [documentation](notebook_tuto_12_image-segmentation-unet-tumors1.).

Overview of our image segmentation app:

![Overview](img_seg_app_overview.png){.thumbnail}
...
- [The OVHcloud AI CLI](cli_10_howto_install_cli1.) installed on your local computer
...
You can upload your data to the cloud either by using the [OVHcloud Control Panel (UI)](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca) or with the [OVHcloud AI CLI](cli_10_howto_install_cli1.).
...
Then, select the `Object Storage`{.action} section (in the Storage category) and [create a new object container](pcs_create_container1.) by clicking `Storage`{.action} > `Object Storage`{.action} > `Create an object container`{.action}.
...
- Do you want to use Streamlit to create an audio classification app? [Learn how to do it](deploy_tuto_03_streamlit_sounds_classification1.).
- Learn how to create & deploy a Speech-to-Text app [here](deploy_tuto_09_streamlit_speech_to_text_app1.).
