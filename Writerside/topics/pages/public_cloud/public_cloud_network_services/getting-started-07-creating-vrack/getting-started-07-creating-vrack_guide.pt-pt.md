...
O [vRack](https://www.ovh.pt/solucoes/vrack/) é uma rede privada que lhe permite configurar o direcionamento entre vários servidores dedicados OVHcloud. Mas permite-lhe também adicionar [instâncias Public Cloud](public-cloud-first-steps1.) à sua rede privada a fim de criar uma infraestrutura de recursos físicos e virtuais.
...
- um [utilizador OpenStack](create_and_delete_a_user1.) (opcional)
...
Consulte o guia: [Criar um acesso à interface Horizon](introducing_horizon1.) para se familiarizar com o Horizon.
...
Consulte o guia [Primeiros passos com as API OVHcloud](first-steps1.), para se familiarizar com a utilização das APIv6 OVHcloud.
...
Para isso, deve escolher o bom fornecedor e o bom recurso Terraform. Encontre mais informações no nosso [guia de utilização do Terraform (EN)](terraform-at-ovhcloud1.).
...
![seleção do projeto](vrack2021-05.png){.thumbnail}
...
Para ativar e gerir um vRack a partir das APIv6 OVHcloud, clique [aqui](getting-started-08-creating-vrack-with-api#step-1-activating-and-managing-a-vrack.) (EN), para consultar o guia específico deste método.
...
>
> Para mais informações sobre a gestão das VLAN do vRack dos servidores dedicados, pode consultar este guia: [Criar várias VLAN no vRack](creating-multiple-vlans-in-a-vrack1.).
...
![VLAN criação](vrack2022-03.png){.thumbnail}
...
Selecione esta opção se pretende criar instâncias apenas com uma rede privada. Para mais informações, consulte os seguintes guias: [Creating a private network with Gateway (EN)](getting-started-02-create-private-network-gateway1.) e [Criação e conexão a uma primeira instância Public Cloud](public-cloud-first-steps#etapa-3-criacao-de-uma-instancia.).
...
Caso seja necessário fazer comunicar servidores dedicados OVHcloud com VLAN taggado, consulte o seguinte guia: [Criar várias VLAN no vRack](creating-multiple-vlans-in-a-vrack1.).
...
Para criar uma VLAN a partir da APIv6 OVHcloud, clique [aqui](getting-started-08-creating-vrack-with-api#step-3-creating-a-vlan-in-the-vrack.) (EN), para consultar o guia específico deste método.
...
Consulte o guia: [Criar uma instância a partir da Área de Cliente](public-cloud-first-steps#create-instance.). Ao criar uma instância, poderá especificar, na etapa 5, pode escolher um modo de rede e depois uma rede privada na qual possa integrar a sua instância.
...
Clique [aqui](getting-started-08-creating-vrack-with-api#step-4-integrating-an-instance-into-the-vrack.) (EN), para consultar o guia específico deste método.
...
Tenha em atenção as seguintes informações, como indicado no [guia de utilização da API Nova](starting_with_nova1.):
...
![ligação Horizon](images_horizon1.png){.thumbnail}

Selecione `Compute` e, em seguida, `Instances` no menu.

![Horizon compute instâncias](images_horizon2.png){.thumbnail}
...
[Configuração do vRack Public Cloud a partir das APIv6 OVHcloud](getting-started-08-creating-vrack-with-api1.) (EN)

[Servidores dedicados - Criar várias VLAN no vRack](creating-multiple-vlans-in-a-vrack1.)
