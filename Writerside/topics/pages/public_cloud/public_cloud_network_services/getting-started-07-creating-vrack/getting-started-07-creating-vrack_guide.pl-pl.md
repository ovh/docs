...
Usługa [vRack](https://www.ovh.pl/rozwiazania/vrack/) to prywatna sieć, która umożliwia klientom kierowanie ruchu między serwerami dedykowanymi OVHcloud i innymi usługami OVHcloud. Umożliwia on również dodawanie [instancji Public Cloud](public-cloud-first-steps1.) do sieci prywatnej w celu utworzenia infrastruktury zasobów fizycznych i wirtualnych.

**Dowiedz się, jak skonfigurować instancje Public Cloud w ramach sieci vRack.**

## Wymagania początkowe

- [projekt Public Cloud](create_a_public_cloud_project1.) na koncie OVHcloud
- dostęp do [Panelu klienta OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pl/&ovhSubsidiary=pl)
- [użytkownika OpenStack](create_and_delete_a_user1.) (opcjonalnie)
...
Zapoznaj się z następującą [instrukcją](introducing_horizon1.), aby zapoznać się z Horizon.
...
Zobacz przewodnik [Pierwsze kroki z API OVHcloud](first-steps1.), aby rozpocząć korzystanie z interfejsu APIv6 OVHcloud.
...
W tym celu należy wybrać właściwego dostawcę terraform i zasobów. Więcej informacji zawiera przewodnik [używanie Terraform z OVHcloud (EN)](terraform-at-ovhcloud1.).
...
![wybór projektu](vrack2021-05.png){.thumbnail}

Kliknij pozycję `Private Network`{.action} na lewym pasku nawigacyjnym.

![Prywatne sieci](vrack2021-02.png){.thumbnail}
...
![Utworzenie vRack](vrack3.png){.thumbnail}
...
Aby włączyć i zarządzać usługą vRack za pomocą interfejsu APIv6 OVHcloud, zapoznaj się z [tą częścią](getting-started-08-creating-vrack-with-api#step-1-activating-and-managing-a-vrack.) (EN), odpowiedniego przewodnika.
...
>
> Więcej informacji na ten temat zawiera przewodnik [Tworzenie kilku sieci VLAN w prywatnej sieci vRack.](creating-multiple-vlans-in-a-vrack1.)
...
![Tworzenie sieci VLAN](vrack2022-03.png){.thumbnail}
...
![tworzenie network](vrack6-2022.png){.thumbnail}

**Utwórz Gateway i połącz się z prywatną siecią**

Wybierz tę opcję, jeśli chcesz utworzyć instancje wyłącznie z siecią prywatną. Więcej informacji znajdziesz w przewodnikach: [Creating a private network with Gateway (EN)](getting-started-02-create-private-network-gateway1.) i [Tworzenie pierwszej instancji Public Cloud i łączenie się z nią](public-cloud-first-steps#krok-3-tworzenie-instancji.).
...
Jeśli potrzebujesz komunikacji z serwerami dedykowanymi w tej sieci VLAN, zapoznaj się z przewodnikiem [Tworzenie kilku sieci VLAN w prywatnej sieci vRack](creating-multiple-vlans-in-a-vrack1.).
...
Aby utworzyć sieć VLAN przy użyciu interfejsu APIv6 OVHcloud, zapoznaj się z [tą sekcją](getting-started-08-creating-vrack-with-api#step-3-creating-a-vlan-in-the-vrack.) (EN), odpowiedniego przewodnika.
...
Jeśli potrzebujesz pomocy, zapoznaj się z tym przewodnikiem: [Tworzenie instancji](public-cloud-first-steps#create-instance.) Podczas tworzenia instancji w etapie 5 możesz wybrać tryb sieci, a następnie prywatną sieć, do której chcesz zintegrować instancję.

![przypisz nową instancję](images_network-selection.png){.thumbnail}
...
Przejdź do [tej sekcji](getting-started-08-creating-vrack-with-api#step-4-integrating-an-instance-into-the-vrack.) (EN), odpowiedniego przewodnika.
...
Należy również zwrócić uwagę na następujące informacje, jak wskazano w [Podręcznik użytkownika API Nova](starting_with_nova1.):
...
![wystąpienie szczegółowe](vrack2021.png){.thumbnail}
...
![przypisz sieć](vrack2021-01.png){.thumbnail}
...
![przypisz sieć](vrack9.png){.thumbnail}

##### **Zarządzanie interfejsami sieciowymi za pomocą interfejsu APIv6 OVHcloud**

Przejdź do [tej sekcji](getting-started-08-creating-vrack-with-api#in-case-of-an-existing-instance.) (EN), odpowiedniego przewodnika.
...
![connection Horizon](images_horizon1.png){.thumbnail}

Wybierz polecenie `Compute`, a następnie polecenie `Instances`.

![Instancje obliczeniowe Horizon](images_horizon2.png){.thumbnail}
...
![Interfejs odłączania Horizon](horizon5.png){.thumbnail}

Wybierz interfejs i zatwierdź.

![Interfejs odłączania Horizon](horizon6.png){.thumbnail}
...
[Konfiguracja vRack dla Public Cloud przy użyciu interfejsu APIv6 OVHcloud](getting-started-08-creating-vrack-with-api1.) (EN)

[Serwery dedykowane - Tworzenie kilku sieci VLAN w prywatnej sieci vRack](creating-multiple-vlans-in-a-vrack1.)
