...
>
> Niniejszy przewodnik ułatwi Ci realizację bieżących zadań. W przypadku trudności lub wątpliwości związanych z administrowaniem, użytkowaniem lub uruchomieniem usługi na serwerze zalecamy skorzystanie z pomocy specjalisty. Więcej informacji znajduje się w sekcji [Sprawdź](changing_dns_servers_of_an_instance_#gofurther.) ten przewodnik.
>
...
Zaloguj się do instancji przez SSH. Więcej informacji na ten temat znajdziesz w przewodniku "[Logowanie do instancji Public Cloud](public-cloud-first-steps#connect-to-instance.)".

Przejdź do użytkownika root. Jeśli potrzebujesz, skorzystaj z naszego przewodnika, aby [zmienić hasło root](become_root_and_change_password1.).
...
Zaloguj się do instancji za pomocą sesji zdalnego pulpitu lub konsoli VNC. Więcej informacji na ten temat znajdziesz w przewodniku "[Logowanie do instancji Public Cloud](public-cloud-first-steps#connect-to-instance.)".

Otwórz `Parametry sieci`{.action}.

![zmień serwery dns](changednsservers1.png){.thumbnail}

Za pomocą panelu konfiguracyjnego przejdź do konfiguracji IPv4 Twojej publicznej karty sieciowej.

![zmień serwery dns](changednsservers2.png){.thumbnail}

Dodaj serwery, których chcesz użyć w zaawansowanych `ustawieniach`{.action}.

![zmień serwery dns](changednsservers3.png){.thumbnail}
...
[Logowanie jako użytkownik root i ustawienie hasła](become_root_and_change_password1.)

[Zmiana hostname instancji Public Cloud](changing_the_hostname_of_an_instance1.)
