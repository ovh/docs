...
Les classes de stockages objet S3 offrent un large support des API S3. L'ensemble des API compatibles sont décrites dans notre [guide de compatibilité S3](s3_s3_compliancy1.).
...
Oui, l'Object Storage S3 est largement compatible avec les API S3 et peut être intégré avec les outils du marché comme par exemple [Veeam](s3_veeam1.), [Owncloud](s3_owncloud1.), [Nextcloud](s3_nextcloud1.).
...
La liste des endpoints est disponible dans le guide « [Endpoints et géo-disponibilité de l’Object Storage](s3_location1.) ».
...
Il est possible de gérer plusieurs profils utilisateurs grâce aux S3 Policies. Consultez notre guide « [Gestion des identités et des accès](s3_identity_and_access_management1.) ».

### Comment configurer les droits d'accès par objet ou par bucket ?

Il est possible de configurer des droits d'accès par profil utilisateur et par objet. Consultez notre guide « [Gestion des identités et des accès](s3_identity_and_access_management1.) ».
...
Retrouvez plus d'informations dans notre guide « [Chiffrez vos objets côté serveur avec SSE-C](s3_encrypt_your_objects_with_sse_c1.) ».
...
Retrouvez plus d'informations dans notre guide « [Gestion de l’immuabilité des objets avec Object Lock (WORM)](s3_managing_object_lock1.) ».
...
- [Utiliser S3 Object Storage avec rClone](s3_rclone1.)
- [Utiliser Object Storage Swift avec rClone](pcs_sync_rclone_object_storage1.)
