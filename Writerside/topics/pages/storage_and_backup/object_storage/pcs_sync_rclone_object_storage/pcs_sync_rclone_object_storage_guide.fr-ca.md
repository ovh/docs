...
- Avoir créé son container *Stockage d’objet* (depuis [l'espace client](pcs_create_container1.){.external}.
- Avoir créé un [utilisateur OpenStack](create_and_delete_a_user1.){.external}.
...
Une fois votre [utilisateur OpenStack](create_and_delete_a_user1.){.external} créé vous pourrez choisir, dans votre espace client, de récupérer le fichier de configuration nécessaire pour rClone.
...
![Télécharger un fichier de configuration rClone](download_file.png){.thumbnail}
