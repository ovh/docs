...
- Avoir accès aux API v6 (reportez à notre guide de [premiers pas avec les API OVHcloud](first-steps1.)). Une fois les accès aux API créés, exportez les 4 IDs dans les variables, tel que décrit ci-dessous :
...
- Le bloc *variable* définit la région et le *s3 endpoint* qui sont utilisés pour créer le bucket. Vous pouvez l'adapter en fonction de votre besoin : [Object Storage - Endpoints et géo-disponibilité de l’Object Storage](s3_location1.) to know what region / endpoints are available..
