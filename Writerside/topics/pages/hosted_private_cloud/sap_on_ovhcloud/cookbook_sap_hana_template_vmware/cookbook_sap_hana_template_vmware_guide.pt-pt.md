...
- A [Public Cloud project](create_a_public_cloud_project1.) in your OVHcloud account with:
    - [An S3 Object Storage bucket](s3_create_bucket1.) and [an S3 user](s3_identity_and_access_management#creation-dun-utilsateur.) with *read* right
    - [A second S3 Object Storage bucket](s3_create_bucket1.) and [an S3 user](s3_identity_and_access_management#creation-dun-utilsateur.) with *read/write* rights
...
We recommend following our [S3 Object Storage guide](s3_getting_started_with_object_storage1.) if it's your first time with S3 Object Storage.
...
![sap-hana-installation](step-9.png){.thumbnail}
...
3\. Create the scheduling backups for your SAP HANA database. We present an example of the scheduling backups with OVHcloud Backint Agent for SAP HANA in [our guide](cookbook_install_ovhcloud_backint_agent1.).
