...
Vous pouvez indiquer un nom de serveur qui sera visible via la commande `hostname`. Vous avez également la possibilité d'[ajouter votre clé SSH](getting-started-with-dedicated-server#ajout-dune-cle-ssh-facultatif.).
...
Une fois l'installation de l'image SLES 15 for SAP réalisée, vous pouvez [vous connecter à votre serveur dédié](getting-started-with-dedicated-server#connexion-a-votre-serveur.).
