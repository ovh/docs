...
- An interconnection between two clusters, for example using these technologies:
    - via an IPsec VPN as detailed in the guide [Interconnecting IPsec between two sites](44-ipsec-interconnection1.)
...
Using **Prism Central**, connect to **Prism Element** on the cluster in France. You can use the [Nutanix Hyperconvergence guide](03-nutanix-hci1.) for more information.

In the top left-hand corner of the **Prism Element** dashboard, click `the cluster name`{.action}.

![00 Add iSCSI address in FRANCE01](00-isci-add-address-france01.png){.thumbnail}

Check the IP address in the **iSCSI Data Service IP** field.

![00 Add iSCSI address in FRANCE01](00-isci-add-address-france02.png){.thumbnail}
...
![01 Add iSCSI address in Canada01](01-isci-add-address-canada01.png){.thumbnail}

Check the IP address in the **iSCSI Data Service IP** field.

![01 Add iSCSI address in Canada02](01-isci-add-address-canada02.png){.thumbnail}
...
![Activate Recovery 01](02-activate-recovery01.png){.thumbnail}

Click `Enable Leap`{.action}.

![Activate Recovery 02](02-activate-recovery02.png){.thumbnail}

When you see `Prechecks successful`, click `Enable`{.action}.

![Activate Recovery 03](02-activate-recovery03.png){.thumbnail}
...
![Connect clusters 01](03-connect-cluster01.png){.thumbnail}

Then click `Connect to Availability Zone`{.action}.

![Connect clusters 01](03-connect-cluster02.png){.thumbnail}
...
![Connect clusters 01](03-connect-cluster03.png){.thumbnail}

The remote cluster is named after the private IP address of the **Prism Central** in France, preceded by **PC_**.

![Connect clusters 02](03-connect-cluster04.png){.thumbnail}
...
![Connect clusters 03](03-connect-cluster05.png){.thumbnail}
...
![Create CATEGORIE 01](04-create-category01.png){.thumbnail}

Click `New Category`{.action}.

![Create CATEGORIE 02](04-create-category02.png){.thumbnail}
...
![Create CATEGORIE 03](04-create-category03.png){.thumbnail}

Go back to the main menu and select `VMs`{.action} under `Compute and Storage`.

![Create CATEGORIE 04](04-create-category04.png){.thumbnail}

Select the virtual machine by using the `check box`{.action} to its left, then click the `Actions`{.action} button.

![Create CATEGORIE 05](04-create-category05.png){.thumbnail}

In this menu, click `Manage Categories`{.action}.

![Create CATEGORIE 06](04-create-category06.png){.thumbnail}

Select the category you created and click the blue `+`{.action} button.

![Create CATEGORY 07](04-create-category07.png){.thumbnail}

Click `Save`{.action}.

![Create CATEGORIE 08](04-create-category08.png){.thumbnail}
...
![Create Protection policy 01](05-create-protection-policy01.png){.thumbnail}

Click `Create Protection Policy`{.action}. 

![Create Protection policy 02](05-create-protection-policy02.png){.thumbnail}
...
![Create Protection policy 03](05-create-protection-policy03.png){.thumbnail}
...
![Create Protection policy 04](05-create-protection-policy04.png){.thumbnail}

Then click `+ Add schedule`{.action}.

![Create Protection policy 05](05-create-protection-policy05.png){.thumbnail}
...
![Create Protection policy 06](05-create-protection-policy06.png){.thumbnail}

Click `Next`{.action}.

![Create Protection policy 07](05-create-protection-policy07.png){.thumbnail}

Select the `category` where the virtual machine was added and click `Add`{.action}.

![Create Protection policy 08](05-create-protection-policy08.png){.thumbnail}

Click `Create`{.action}.

![Create Protection policy 09](05-create-protection-policy09.png){.thumbnail}

The protection policy is enabled and appears in the **Protection Policies** dashboard.

![Create Protection policy 10](05-create-protection-policy10.png){.thumbnail}
...
![Create Recovery Plan 01](06-create-recovery-plan01.png){.thumbnail}

Click `Create New Recovery Plan`{.action}. 

![Create Recovery Plan 02](06-create-recovery-plan02.png){.thumbnail}
...
![Create Recovery Plan 03](06-create-recovery-plan03.png){.thumbnail}

Click `Add Entities`{.action}. 

![Create Recovery Plan 04](06-create-recovery-plan04.png){.thumbnail}

Select the `checkbox`{.action} next to the virtual machine that is part of the disaster recovery plan, then click `Add`{.action}.

![Create Recovery Plan 05](06-create-recovery-plan05.png){.thumbnail}
...
![Create Recovery Plan 06](06-create-recovery-plan06.png){.thumbnail}

After reviewing the Disaster Recovery Plan presentation, click `OK, Got it`{.action}. 

![Create Recovery Plan 07](06-create-recovery-plan07.png){.thumbnail}
...
![Create Recovery Plan 08](06-create-recovery-plan08.png){.thumbnail}

The disaster recovery plan appears in the list.

![Create Recovery Plan 09](06-create-recovery-plan09.png){.thumbnail}
...
![Planned Migration 01](07-planned-migration01.png){.thumbnail}
...
![Planned Migration 02](07-planned-migration02.png){.thumbnail}

Click `Failover`{.action}. 

![Planned Migration 03](07-planned-migration03.png){.thumbnail}

Select `Planned Failover`{.action} as the type of Failover and click the `Failover`{.action} button.

![Planned Migration 04](07-planned-migration04.png){.thumbnail}

Type `Failover`{.action} in the confirmation field and click the `Failover`{.action} button.

![Planned Migration 05](07-planned-migration05.png){.thumbnail}
...
![Planned Migration 06](07-planned-migration06.png){.thumbnail}
...
![Replication Inversion 01](08-replication-inversion01.png){.thumbnail}

Select the `checkbox`{.action} to the left of the protection policy and click the `Actions`{.action} button.

![Replication Inversion 02](08-replication-inversion02.png){.thumbnail}

Click the `Update`{.action} button.

![Replication Inversion 03](08-replication-inversion03.png){.thumbnail}

Move left under `Primary Location` until a menu appears. Click `Edit`{.action}.

![Replication Inversion 04](08-replication-inversion04.png){.thumbnail}
...
![Replication Inversion 05](08-replication-inversion05.png){.thumbnail}

Click `Update Location`{.action}.

![Replication Inversion 06](08-replication-inversion06.png){.thumbnail}

Move to the right below the destination until a menu appears. Click `Edit`{.action}.

![Replication Inversion 07](08-replication-inversion07.png){.thumbnail}
...
![Replication Inversion 08](08-replication-inversion08.png){.thumbnail}

Click `Update Location`{.action}.

![Replication Inversion 09](08-replication-inversion09.png){.thumbnail}

Click `Next`{.action}. 

![Replication Inversion 10](08-replication-inversion10.png){.thumbnail}

Click `Update`{.action} to complete the inversion.

![Replication Inversion 11](08-replication-inversion11.png){.thumbnail}
...
![Recovery Plan Inversion 01](09-recovery-plan-inversion01.png){.thumbnail}
...
![Recovery Plan Inversion 02](09-recovery-plan-inversion02.png){.thumbnail}

From the actions menu, click `Update`{.action}.

![Recovery Plan Inversion 03](09-recovery-plan-inversion03.png){.thumbnail}

In the **Primary Location** drop-down menu on the left, replace the remote cluster with `Local AZ`{.action}.

![Recovery Plan Inversion 04](09-recovery-plan-inversion04.png){.thumbnail}

Choose `The remote area`{.action} in **Recovery Location** on the right instead of Local AZ.

![Recovery Plan Inversion 05](09-recovery-plan-inversion05.png){.thumbnail}

Now that the source and destination of the disaster recovery plan have changed, click `Next`{.action}.

![Recovery Plan Inversion 06](09-recovery-plan-inversion06.png){.thumbnail}

Click `Proceed`{.action}.

![Recovery Plan Inversion 07](09-recovery-plan-inversion07.png){.thumbnail}

Select the `Virtual Machine` that you want to start during the disaster recovery plan and click `Next`{.action}.

![Recovery Plan Inversion 08](09-recovery-plan-inversion08.png){.thumbnail}
...
![Recovery Plan Inversion 09](09-recovery-plan-inversion09.png){.thumbnail}
...
![Recovery Plan Inversion 10](09-recovery-plan-inversion10.png){.thumbnail}
...
![Unplanned Recovery 01](10-unplanned-recovery01.png){.thumbnail}

Select the Disaster Recovery Plan with the `checkbox`{.action} and click the `Actions`{.action} button. 

![Unplanned Recovery 02](10-unplanned-recovery02.png){.thumbnail}

Choose `Failover`{.action} from the menu. 

![Unplanned Recovery 03](10-unplanned-recovery03.png){.thumbnail}

Check `Unplanned Failover`{.action} and click `Failover`{.action}

![Unplanned Recovery 04](10-unplanned-recovery04.png){.thumbnail}

Type `Failover` in the confirmation field and click `Failover`{.action}

![Unplanned Recovery 05](10-unplanned-recovery05.png){.thumbnail}
...
[IPsec interconnect between two sites](44-ipsec-interconnection1.)
