...
Na sua [interface vSphere](vsphere_interface_connexion1.), coloque-se na vista `Storage`{.action}.

![para armazenamento](register-vmx-01.png){.thumbnail}

Selecione um datastore da lista.

![selecionar datastore](register-vmx-02.png){.thumbnail}

Nos ficheiros deste datastore, selecione o ficheiro `.vmx` e clique em `Register VM`{.action}.

![registar VM](register-vmx-03.png){.thumbnail}

Introduza as informações necessárias e clique em `Finish`{.action}.

![registar VM](register-vmx-04.png){.thumbnail}

Estas operações devem ser repetidas em cada datastore e em cada VM a re-registar.

Verifique os parâmetros das suas VM (nome, [portgroup](creation_vlan1.), etc...) ao clicar com o botão direito do rato sobre cada uma delas e ao selecionar o `Edit Settings`{.action}.

![modificar parâmetros](register-vmx-06.png){.thumbnail}
...
![registar VM](register-vmx-05.png){.thumbnail}
