...
In your [vSphere interface](vsphere_interface_connexion1.), go to the `Storage`{.action} view.

![storage view](register-vmx-01.png){.thumbnail}
...
![reregister VM](register-vmx-03.png){.thumbnail}
...
Check the settings of your VMs (name, [portgroup](creation_vlan1.), etc...) by right-clicking on each one and then selecting `Edit Settings`{.action}.

![change](register-vmx-06.png){.thumbnail}
