...
En su [interfaz vSphere](vsphere_interface_connexion1.), sitúese en la vista `Almacenamiento`{.action}.

![vista de almacenamiento](register-vmx-01.png){.thumbnail}

Seleccione un datastore de la lista.

![seleccionar datastore](register-vmx-02.png){.thumbnail}

En las carpetas de este datastore, seleccione el archivo `.vmx` y haga clic en `Register VM`{.action}.

![enregistrer VM](register-vmx-03.png){.thumbnail}

Introduzca la información solicitada y haga clic en `Finish`{.action}

![enregistrer VM](register-vmx-04.png){.thumbnail}

Estas operaciones se repiten en cada datastore y por cada MV que se va a volver a registrar.

Compruebe los parámetros de sus MV (nombre, [portgroup](creation_vlan1.), etc.) haciendo clic derecho sobre cada una de ellas y seleccionando `Edit Settings`{.action}.

![modificar parámetros](register-vmx-06.png){.thumbnail}
...
![enregistrer VM](register-vmx-05.png){.thumbnail}
