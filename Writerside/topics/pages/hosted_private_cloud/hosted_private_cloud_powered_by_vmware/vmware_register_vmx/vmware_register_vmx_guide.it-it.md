...
Nella tua [interfaccia vSphere](vsphere_interface_connexion1.), posizionati nella vista `Storage`{.action}.

![vista archiviazione](register-vmx-01.png){.thumbnail}

Seleziona un datastore nella lista

![selezionare datastore](register-vmx-02.png){.thumbnail}

Nelle cartelle di questo datastore, seleziona il file `.vmx` e clicca su `Register VM`{.action}.

![salva VM](register-vmx-03.png){.thumbnail}

Inserisci le informazioni richieste e clicca su `Finish`{.action}.

![salva VM](register-vmx-04.png){.thumbnail}

Le stesse operazioni devono essere ripetute su ogni datastore e per ogni VM da ri-registrare.

Verifica le impostazioni delle tue VM (nome, [portgroup](creation_vlan1.), ecc...) cliccando con il tasto destro su ciascuna di esse e selezionando `Edit Settings`{.action}.

![modificare i parametri](register-vmx-06.png){.thumbnail}
...
![salva VM](register-vmx-05.png){.thumbnail}
