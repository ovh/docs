...
Gehen Sie im [vSphere Interface](vsphere_interface_connexion1.) in die Ansicht `Storage`{.action}.

![Storage](register-vmx-01.png){.thumbnail}

Wählen Sie einen Datastore in der Liste aus.

![Datastore auswählen](register-vmx-02.png){.thumbnail}

Wählen Sie in den Ordnern dieses Datastores die `.vmx` Datei aus und klicken Sie auf `Register VM`{.action}.

![VM speichern](register-vmx-03.png){.thumbnail}

Füllen Sie die erforderlichen Informationen aus und klicken Sie auf `Finish`{.action}.

![VM speichern](register-vmx-04.png){.thumbnail}

Diese Operationen sind auf jedem Datastore und für jede neu zu registrierende VM zu wiederholen.

Überprüfen Sie die Einstellungen Ihrer VMs (Name, [Portgroup](creation_vlan1.) etc.), indem Sie mit der rechten Maustaste auf jede Ihrer VMs klicken und dann auf `Edit Settings`{.action}.

![Ändern](register-vmx-06.png){.thumbnail}
...
![VM speichern](register-vmx-05.png){.thumbnail}
