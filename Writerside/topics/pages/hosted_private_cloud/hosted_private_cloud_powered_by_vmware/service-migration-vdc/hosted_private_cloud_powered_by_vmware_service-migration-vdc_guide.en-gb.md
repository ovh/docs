...
![decision tree](tree.png){.thumbnail}
...
![add a Virtual Datacenter](add-vDC.gif){.thumbnail}
...
For connections to the VMware platform, you can choose to block access to vSphere by default. Please refer to our guide on the [vCenter access policy](vcenter-modify-access-policy1.) for details.
...
To do this, please refer to our guides on [Changing user rights](change_users_rights1.), [Changing the User Password](changement_du_mot_de_passe_utilisateur1.) and [Associating an email with a vSphere user](vsphere_edit_user1.).
...
As part of a migration process, by default, the new vDC will be linked to the same vRack as the source vDC. Please consult our guide to [Using Private Cloud within a vRack](using_private_cloud_in_vrack1.).
...
Setting up a new vDC involves reconfiguring VMware High Availability (HA), including boot order and priority. Please consult our guide about [VMware HA configuration](vmware_ha_high_availability2.).
...
Setting up a new vDC involves reconfiguring the VMware Distributed Resource Scheduler (DRS) feature, in particular the affinity or anti-affinity rules for groups of hosts and VMs. Please consult our guide about [configuring VMware DRS](vmware_drs_distributed_ressource_scheduler_new1.).
...
As a first step, please read our documentation on [getting started with NSX](nsx-01-first-steps1.).
...
For more information, read our documentation on [Managing the distributed firewall in NSX](nsx-05-manage-distributed-firewall1.).
...
To recreate DHCP and associate them with your segments and T1 Gateways, refer to our guide on [configuring DHCP in NSX](nsx-03-configure-dhcp-onsegment1.).

<a name="dns"></a>
##### Step 4.8.3.3 DNS

To recreate DNS and associate them with your T1 Gateways, please refer to our guide on [configuring the DNS forwarder in NSX](nsx-04-configure-dns-forwarder1.).

<a name="nat"></a>
##### Step 4.8.3.4 NAT Rules

To recreate your NAT rules and associate them with the T1 Gateways, please refer to our guide on [Setting up NAT for port redirections with NSX](nsx-07-configure-nat-redirection1.).

<a name="lb"></a>
##### Step 4.8.3.5 NSX Load Balancing

To recreate your Load Balancers, follow the instructions in our guide on [configuring Load Balancing in NSX](nsx-09-configure-loadbalancing1.).

<a name="fwgw"></a>
##### Step 4.8.3.6 T0 T1 Gateway Firewall

To recreate the firewall rules associated with your previous edges, please refer to our guide on [Gateway Firewall Management in NSX](nsx-06-manage-gateway-firewall1.).
...
To recreate your IPSec sessions, please refer to our guide on [setting up an IPsec Tunnel with NSX](nsx-12-configure-ipsec1.).
...
Next, in the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager), follow the instructions in our [Move an Additional IP](move-failover-ip1.) guide to move the initial NSX-V block to the PCC service you are migrating, but specify the VIP IP of the T0 as the "next hop", as shown in the example below:

![NSX IP Migration](MoveIPNextHop.png){.thumbnail}
...
The next step depends on the current configuration per [Virtual Protection Group](zerto_virtual_replication_as_a_service1.):
...
To "move" Private Gateway to destination vDC, you must first disable it by following the steps in [Disable the private gateway](private_gateway#disablegw.).
...
Then enable it again by following the steps in [Enable the private gateway](private_gateway#enablegw.) and choose the datacentreId of the new vDC.
