...
> [!warning]
>
> Tanzu Community Edition is coming to an end of life at the end of 2022 and will no longer be available. VMware has decided to offer free download of VMware Tanzu Kubernetes Grid. You can use our [introduction to Tanzu Kubernetes Grid](tanzu_tkgm_01presentation1.) guide to deploy it.
>
...
- [DHCP](nsx_configurer_le_dhcp_sur_une_edge_gateway1.) services activated on the NSX Gateway
...
Check out our [Adding an IP block](ajout_de_bloc_ip1.) documentation if you are out of usable public IPs.

![network](en00ipblocks.png){.thumbnail}
...
![internal interface](en01nsxinter.png){.thumbnail}
- DHCP service distributing VLAN13 address scope 172.16.13.10 through 172.16.13.100<br>
![DHCP](en02nsxdhcp.png){.thumbnail}
- a SNAT rule to translate adress range 172.16.13.1/24 into secondary public IP xxx.xxx.xxx.226 for external access <br>
![SNAT rule](en03nsxsnat.png){.thumbnail}

In the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/asia/&ovhSubsidiary=asia), go to the `Security`{.action} tab of your Hosted Private Cloud and add the public IPs to the allowed adresses to connect.<br>
The addresses should appear as "Authorised and set up".

![addresses](en00ipsec.png){.thumbnail}
...
You can deploy a VM [from an ISO](deploiement_d_une_machine_virtuelle1.) or [from an OVF template](ovf_template1.).<br>

Make sure the VM is set on the VLAN that will be used for the TCE clusters (VLAN13 in our case).<br>

![VLAN](en04bootvlan.png){.thumbnail}
...
![Tanzu CLI](en05tanzucli.png){.thumbnail}
...
![NSX - Metadata](en12optional.png){.thumbnail}

Choose your resource locations.<br>
Click `Next`{.action}.

![Resources](en13resources.png){.thumbnail}

Enter the Kubernetes Network settings.<br>
Click `Next`{.action}.

![Kubernetes](en14kubnet.png){.thumbnail}

Disable Identity Management.<br>
Click `Next`{.action}.

![Identity](en15identity.png){.thumbnail}

Select the node template to be used.<br>
Click `Next`{.action}.

![node template](en16os.png){.thumbnail}

Click `Review Configuration`{.action}.

![Review](en17review.png){.thumbnail}

When ready, click `Deploy Management Cluster`{.action}.

![Deploying cluster](en18deploy.png){.thumbnail}

Upon completion, verify you get a creation message and the nodes are visible in vSphere.

![Creation message](en20created.png){.thumbnail}

![Visible nodes](en21vsphere.png){.thumbnail}
...
![configuration file name](en26copyconf.png){.thumbnail}
...
![Creation message](en27created.png){.thumbnail}

![Visible nodes](en28vsphere.png){.thumbnail}
