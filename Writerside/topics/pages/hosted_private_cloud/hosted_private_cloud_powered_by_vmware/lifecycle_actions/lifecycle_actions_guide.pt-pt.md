...
- Access to the [OVHcloud API](https://api.ovh.com/){.external}. If required, please refer to our guide on [Getting started with OVHcloud](first-steps1.) APIs.
- You need to have [created your login credentials for the OVHcloud API](first-steps1.){.external}.

## Instructions

### Step 1 - Retrieve your Hosted Private Cloud service information

Log in to the OVHcloud APIv6 using our guide on [Getting started with the OVHcloud API](first-steps1.).
...
Start by adding new datastores, using our guide on [adding storage](how_to_add_storage1.).
...
Finally, delete your old datastores by following our guide on [deleting a datastore](delete_datastore1.).
...
Start by adding new datastores by following the guide on [adding storage](how_to_add_storage1.).
...
To complete your migration, you can follow this guide: [Migrate to a new infrastructure](service-migration-vdc2.).

### Software component lifecycle

#### Retrieving your PCC information in the OVHcloud Control Panel

Log in to the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pt/&ovhSubsidiary=pt) with an administrator account.
...
![control panel - retrieving vsphere version](manager-version-vsphere.png){.thumbnail}
...
![vSphere - data recovery](vsphere-version-nsx-v.png){.thumbnail}
