...
A desativação é necessária para a realização de operações de atualização com o [Update Manager](vmware_update_manager1.), por exemplo. Neste caso específico, o host é isolado.

![Ativação HA](images_HA.png){.thumbnail}
...
![Falha do host](images_HAparam1.PNG){.thumbnail}
...
![Isolamento do host](images_HAparam2.PNG){.thumbnail}
...
![Datastore com PDL](images_HAparam3.PNG){.thumbnail}
...
![Datastore com APD](images_HAparam4.PNG){.thumbnail}

##### Monitorização das máquinas virtuais

A monitorização das MV passa a estar disponível após a instalação das [VMware Tools](vmware_tools_install2.).
...
![Monitorização das MV](images_HAparam5.PNG){.thumbnail}
