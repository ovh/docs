...
Disabling it is required to perform update operations with [Update Manager](vmware_update_manager1.).

![](images_HA.png){.thumbnail}
...
![](images_HAparam4.PNG){.thumbnail}

##### VM Monitoring

VM monitoring is available after [VMware tools](vmware_tools_install2.){.external} is installed, in case of non-response via the **tools** (heartbeat signals) the virtual machine will be automatically restarted. Advanced configuration is possible for this feature (for example, reboot interval).

![](images_HAparam5.PNG){.thumbnail}
