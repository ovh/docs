...
- [active Windows licences](manager_ovh_private_cloud#windows-licence.){.external} (activate via the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.ie/&ovhSubsidiary=ie){.external}, in the `Windows licence`{.action} tab for the datacentre)
