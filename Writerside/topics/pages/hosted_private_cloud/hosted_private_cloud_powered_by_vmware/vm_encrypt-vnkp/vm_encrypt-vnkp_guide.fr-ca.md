...
La clé **DEK** est chiffrée à l'aide de la **KDK**. Elle est stockée et chiffrée avec la machine virtuelle. Vous trouverez plus de détails sur le chiffrement **VMware** en consultant les documentations officielles dans la section « [Aller plus loin](vm_encrypt-vnkp_#gofurther.) » de ce guide.
...
![00 add right from manager 01](00-add-right-from-manager01.png){.thumbnail}

Cliquez sur `Modifier`{.action}.

![00 add right from manager 02](00-add-right-from-manager02.png){.thumbnail}

Activez la `Gestion du chiffrement`{.action} et cliquez sur `Valider`{.action}.

![00 add right from manager 03](00-add-right-from-manager03.png){.thumbnail}

Attendez que la fenêtre de modification disparaisse.

![00 add right from manager 04](00-add-right-from-manager04.png){.thumbnail}

Les droits de gestion du chiffrement ont été modifiés, comme on peut le constater dans la colonne `Gestion du chiffrement`.

![00 add right from manager 05](00-add-right-from-manager05.png){.thumbnail}
...
Connectez-vous à l'interface **vSphere**. Au besoin, aidez-vous du guide « [Se connecter à l'interface vSphere](vsphere_interface_connexion1.) ».
...
![01 Create KEY 06](01-create-key06.png){.thumbnail}

Il est maintenant possible d'utiliser la clé pour chiffrer des machines virtuelles.

![01 Create KEY 07](01-create-key07.png){.thumbnail}
...
![02 encrypt VM 03](02-encrypt-vm03.png){.thumbnail} 
...
![03 migrate-from-kms-to-vnkp 01](03-migrate-from-kms-to-vnkp01.png){.thumbnail}

Confirmez votre choix en cliquant sur `DÉFINIR COMME VALEUR PAR DÉFAUT`{.action}.

![03 migrate-from-kms-to-vnkp 02](03-migrate-from-kms-to-vnkp02.png){.thumbnail}

La clé **vSphere Native Key Provider** est alors définie par défaut.

![03 migrate-from-kms-to-vnkp 03](03-migrate-from-kms-to-vnkp03.png){.thumbnail}
...
![03 migrate-from-kms-to-vnkp 04](03-migrate-from-kms-to-vnkp04.png){.thumbnail}
...
![03 migrate-from-kms-to-vnkp 05](03-migrate-from-kms-to-vnkp05.png){.thumbnail}
...
![03 migrate-from-kms-to-vnkp 06](03-migrate-from-kms-to-vnkp06.png){.thumbnail}
...
![03 migrate-from-kms-to-vnkp 07](03-migrate-from-kms-to-vnkp07.png){.thumbnail}
...
![Activate vSAN data at rest encryption 01](04-activate-vsan-data-at-rest-encryption-01.png)
...
![Activate vSAN data at rest encryption 02](04-activate-vsan-data-at-rest-encryption-02.png)

Revenez sur **Services de données** et vous constaterez que le **Chiffrement de données au repos** est activé avec votre clé.

![Activate vSAN data at rest encryption 03](04-activate-vsan-data-at-rest-encryption-03.png)
