...
Vous pouvez retrouver les informations générales concernant Spectre et Meltdown sur notre [guide détaillé](meltdown_spectre_fixes1.){.external} et notre [guide par OS](meltdown_spectre_fixes_os1.){.external}.
...
Pour vérifier la version de votre hôte, vous pouvez vous connecter à l'[interface vSphere](vsphere_interface_connexion1.){.external} et accéder au résumé de l'hôte, partie `Configuration`{.action} :

![Partie configuration de l'hôte](spectre1.JPG)
...
La mise à jour d'un hôte s'effectue en plaçant celui-ci en maintenance (un redémarrage sera effectué lors de la procédure) et en utilisant le [plugin Update Manager](vmware_update_manager1.){.external}.
