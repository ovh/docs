...
![Alerte anti spam](alertblockedip.png){.thumbnail}
...
![Débloquer IP](unblockip.png){.thumbnail}
...
Connectez-vous à l'interface [API d'OVHcloud](https://api.ovh.com/) et suivez les étapes ci-dessous. Pour plus d'informations sur l'utilisation des API OVHcloud, consultez notre guide « [Premiers pas avec les API OVHcloud](first-steps1.) ».
...
- configurer correctement les [Reverses](mail_sending_optimization#configurer-le-reverse-ip.) sur toutes les adresses IP. 
...
Pour plus d'informations sur les services ci-dessus, consultez notre guide « [Optimiser l’envoi d’e-mails](mail_sending_optimization1.) ».

#### Cas spécifiques d'envois d'e-mails

- **Vers un serveur Microsoft (Outlook, etc.)**

Microsoft utilise une politique de liste blanche. Cela signifie que tout serveur se trouve initialement sur une liste noire. Une procédure spécifique est alors nécessaire pour faire valider votre serveur e-mail. Pour plus d’informations, nous vous invitons à consulter [cette section](mail_sending_optimization#vers-un-serveur-microsoft-outlook-etc.) du guide correspondant.
...
- Le fichier .EML de l'e-mail fourni. Celui-ci doit inclure **l'en-tête** et **le pied de page** de l'e-mail. Si vous ne savez pas comment extraire un fichier .EML, nous vous invitons à consulter le guide suivant : [Récupérer l’en-tête d’un e-mail](diagnostic_headers1.).
