...
- The easiest way is to use a suitable software such as [SFTP](comment-deposer-ou-recuperer-des-donnees-sur-un-serveur-dedie-via-sftp1.).
- The other option is to [synchronise the two servers with each other](how-to-copy-data-from-one-dedicated-server-to-another-using-rsync1.).
...
See our guide on [using Backup Storage on a dedicated server](services_backup_storage1.){.external} for more information.
...
If the reputation of your IP addresses is important, we strongly recommend using [Additional IPs](https://www.ovhcloud.com/en/bare-metal/ip/), as they can be kept if you migrate.

Once you have these IPs, you simply need to move them to the new server.
To do this, please refer to our dedicated guide: [Moving an Additional IP](move-failover-ip1.).
...
For more information, please read our documentation on [domains and Dns](web-cloud-domains-domain-names1.).
