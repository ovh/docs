...
- La plus simple est d'utiliser un logiciel adapté tel que [SFTP](comment-deposer-ou-recuperer-des-donnees-sur-un-serveur-dedie-via-sftp1.).
- L'autre option est de [synchroniser les deux serveurs entres-eux](how-to-copy-data-from-one-dedicated-server-to-another-using-rsync1.).
...
Consultez notre guide « [utiliser backup storage sur un serveur dédié](services_backup_storage1.){.external} » pour plus d'informations.
...
À cet effet, consultez notre guide dédié : [Déplacer une Additional IP](move-failover-ip1.).
...
Pour plus d'informations, consultez notre documentation sur les [domaines et DNS](web-cloud-domains-domain-names1.).
