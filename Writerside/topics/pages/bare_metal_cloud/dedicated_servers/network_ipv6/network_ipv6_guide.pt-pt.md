...
- Ter conhecimentos básicos de [SSH](ssh_introduction1.) e redes.
...
- [Obter as informações de rede através da Área de Cliente](network_ipv6_#viacontrolpanel.).
- [Obter as informações de rede através das API](network_ipv6_#viaapi.).
...
Outra forma de recuperar as informações de rede do seu servidor é [utilizar a API OVHcloud](first-steps1.).
...
Se não conseguir que este endereço IPv6 faça ping, verifique a configuração e tente novamente. Além disso, assegure-se de que a máquina a partir da qual está a fazer o teste está conectada por IPv6. Se mesmo assim não for bem-sucedido, teste a configuração em [modo Rescue](rescue_mode1.).
...
Se não conseguir executar ping neste endereço IPv6, verifique a configuração e tente novamente. Além disso, certifique-se de que a máquina a partir da qual está a realizar o teste está ligada ao IPv6. Se ainda assim não funcionar, teste a sua configuração em [modo Rescue](rescue_mode1.).
...
[Encontre mais informações neste guia.](getting-started-with-dedicated-server#ligar-se-ao-servidor.)
...
Num primeiro tempo, [coloque o seu servidor em modo rescue](rescue_mode1.).
