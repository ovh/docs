...
- Basic knowledge of [SSH](ssh_introduction1.) and networking
...
The first step is to identify the IPv6 block and gateway assigned to your server. There are two ways of doing this:
  ...
  - [retrieving the network information via the OVHcloud Control Panel](network_ipv6_#viacontrolpanel.)
  - [retrieving the network information via the OVHcloud API](network_ipv6_#viaapi.)
...
Another way to retrieve the networking information for your server is to [use the OVHcloud API](first-steps1.).
...
If you are not able to ping this IPv6 address, check your configuration and try again. Also ensure that the machine you are testing from is connected with IPv6. If it still does not work, please test your configuration in [Rescue mode](rescue_mode1.).
...
Find more information in [this guide](getting-started-with-dedicated-server#logging-on-to-your-server.).
...
First, [put your server into rescue mode](rescue_mode1.).
