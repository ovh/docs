...
- Una conoscenza basilare di reti e [SSH](ssh_introduction1.).
...
- [Ottenere informazioni sulla rete tramite lo Spazio Cliente OVHcloud](network_ipv6_#viacontrolpanel.).
- [Ottenere informazioni sulla rete tramite API](network_ipv6_#viaapi.).
...
Un altro modo per recuperare le informazioni di rete del tuo server è [utilizzare l’API OVHcloud](first-steps1.).
...
Se non sei in grado di testare (effettuare il ping di) questo indirizzo IPv6, verifica la tua configurazione e riprova. Accertati anche che la macchina da cui stai effettuando il test sia connessa all’IPv6. Se ancora non funziona, testa la tua configurazione in [Modalità rescue](rescue_mode1.).
...
Per ulteriori informazioni fai riferimento a [questa guida](getting-started-with-dedicated-server#accedi-al-tuo-server.).
...
![Centro connessioni di rete e condivisione](ipv6_network_sharing_center.png){.thumbnail}

Clicca su `Modifica impostazioni scheda di rete`{.action}.

![Modifica impostazioni scheda di rete](ipv6_change_adapter_settings.png){.thumbnail}.

Clicca con il tasto destro sulla tua scheda di rete, quindi su `Proprietà`{.action}.

![Proprietà scheda di rete](ipv6_network_adapter_properties.png){.thumbnail}

Seleziona `Internet Protocol Versione 6`{.action}, quindi clicca su `Proprietà`{.action}.

![Proprietà](ipv6_properties.png){.thumbnail}
...
![Proprietà](ipv6_configuration.png){.thumbnail}
