...
- [Reset hasła użytkownika](replacing-user-password1.)
...
![Zmień tryb uruchamiania](rescue-mode-001.png){.thumbnail}
...
![Tryb rescue-customer](rescue-mode-08.png){.thumbnail}
...
![Zrestartuj serwer](rescue-mode-02.png){.thumbnail}
...
Zaloguj się do serwera za pomocą wiersza poleceń lub narzędzia [SSH](ssh_introduction1.), używając wygenerowanego hasła root dla trybu Rescue.
...
>
> Jednym ze sposobów na ominięcie tego problemu jest "komentowanie" znaku firmowego Twojego serwera poprzez dodanie znaku `#` przed jego linią w pliku `known_hosts`. Nie zapomnij anulować tej zmiany przed przywróceniem netbootu do trybu "normalnego".<br>Możesz również usunąć wiersz z pliku. Po ponownym zalogowaniu się klient SSH doda nowy wpis linii papilarnych do serwera. Jeśli potrzebujesz bardziej szczegółowych instrukcji, zapoznaj się z naszym przewodnikiem "[Wprowadzenie do SSH](ssh_introduction#login.)".
>
...
[Zmiana hasła administratora na serwerze dedykowanym z systemem Windows](changing-admin-password-on-windows1.)
