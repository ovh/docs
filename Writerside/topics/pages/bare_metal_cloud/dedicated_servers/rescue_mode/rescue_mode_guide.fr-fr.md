...
- [Réinitialisation du mot de passe de l'utilisateur](replacing-user-password1.)
...
> [!warning]
> Veuillez noter que si vous avez défini une clé SSH par défaut dans votre espace pour les produits dédiés, vous ne recevrez pas de mot de passe root lors du redémarrage d'un serveur en mode rescue. Dans ce cas, vous devez d'abord désactiver la clé SSH par défaut avant de redémarrer le serveur en mode rescue. Pour ce faire, nous vous invitons à consulter cette [section](creating-ssh-keys-dedicated#disablesshkey.) du guide correspondant.
>
...
![Changer le mode de démarrage](rescue-mode-001.png){.thumbnail}
...
![Mode rescue-customer](rescue-mode-08.png){.thumbnail}
...
![Redémarrer le serveur](rescue-mode-02.png){.thumbnail}
...
Vous devrez ensuite accéder à votre serveur en ligne de commande ou via un outil [SSH](ssh_introduction1.), en utilisant le mot de passe root généré pour le mode rescue.
...
[Modifier le mot de passe administrateur sur un serveur dédié Windows](changing-admin-password-on-windows1.)
