...
- [Resetting your user password](replacing-user-password1.)
...
![Modify boot mode](rescue-mode-001.png){.thumbnail}
...
![Reboot the server](rescue-mode-02.png){.thumbnail}
...
You will then need to access your server via the command line or an [SSH tool](ssh_introduction1.), using the root password generated for the rescue mode.
...
>
> One way around this is "commenting out" the fingerprint of your server by adding a `#` in front of its line in the `known_hosts` file. Remember to revert this change before switching the netboot back to "normal" mode.<br>You can also delete the line from the file. Your SSH client will then add a new fingerprint entry for the server when the connection is established anew. If you require more detailed instructions, please refer to our guide "[Getting started with SSH](ssh_introduction#login.)".
>
...
[Changing the admin password on a Windows dedicated server](changing-admin-password-on-windows1.)
