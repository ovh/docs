...
> [!primary]
>
> Per i dischi NVMe è necessario attivare sul server la modalità [Rescue](rescue_mode1.){.external} e utilizzare il tool preinstallato **nvme-cli**.
> 
...
Per maggiori informazioni relativamente ai comandi da utilizzare e a come eseguire test sugli hard disk, consulta [questa guida](raid_hard1.){.external} (in inglese).
...
Per identificare quale hard disk corrisponde alle diverse periferiche _sg_, consulta [questa guida](raid_hard1.){.external} (in inglese).
...
[Sostituire un disco difettoso](disk_replacement1.){.external}

[Configurare un RAID software](raid_soft1.){.external}

[Configurare un RAID hardware](raid_hard1.){.external} (in inglese)
