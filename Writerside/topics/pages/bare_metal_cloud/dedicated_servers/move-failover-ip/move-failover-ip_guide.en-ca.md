...
> See [this guide](network_support_virtual_mac1.) for details.
>
...
Log in to the [OVHcloud Control](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca), go to the `Bare Metal Cloud`{.action} menu and click on `Network`{.action}. Next, click on `IP`{.action}.

Click the `Additional IP`{.action} tab.

![manage IPs](manageIPs2022.png){.thumbnail}

Next, click on the `...`{.action} button to the right of the IP address you want to move and select `Move Additional IP`{.action}.

![move Additional](moveadditionalIP.png){.thumbnail}

In the pop-up window, select the service to move the IP address to from the menu.

![move Additional](moveadditionalIP2.png){.thumbnail}
