...
For detailed instructions, please refer to the [rescue mode guide](rescue_mode1.).

### Step 2: Mounting the system partition

Connect to your server via SSH. (Consult the [SSH introduction guide](ssh_introduction1.) if necessary.) Since it is a Windows server, you will see partitions labelled "Microsoft LDM data".
...
First, change the netboot back to **Boot from the hard disk** in your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=we) (see [step 1](#step-1-rebooting-the-server-into-rescue-mode.)).
...
In your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=we), navigate to the `IPMI`{.action} tab to open a KVM session.

![IPMI](adminpw_win_03.png){.thumbnail}

#### Step 5.1: For a newer version of Windows

There should be a password expiration message in the login interface.

![pwreset](adminpw_win_04.png){.thumbnail}
...
![enterpw](adminpw_win_05.png){.thumbnail}

Click on `OK`{.action} and you will be logged in.

![adminlogin](adminpw_win_06.png){.thumbnail}
...
![administratorpw](adminpw_win_07.png){.thumbnail}
...
![ntpwedit](adminpw_win_09.png){.thumbnail}
...
![ntpwedit](adminpw_win_10.png){.thumbnail}
...
[Using the IPMI with dedicated servers](using_ipmi_on_dedicated_servers1.)
