...
Lors de l’installation ou de la réinstallation d’un système d’exploitation Windows, un mot de passe pour l’accès administrateur vous est fourni. Nous vous conseillons vivement de le modifier, comme expliqué dans notre guide intitulé « [Sécuriser un serveur dédié](securing-a-dedicated-server1.){.external} ». Si vous avez perdu votre mot de passe admin, vous devrez le réinitialiser en mode rescue.
...
Pour plus d'informations sur le mode rescue, consultez [ce guide](rescue_mode1.).

### Étape 2 : monter la partition système

Connectez-vous à votre serveur via SSH. Si nécessaire, consultez le guide d'[introduction au SSH](ssh_introduction1.).
...
[Utilisation de l'IPMI pour les serveurs dédiés](using_ipmi_on_dedicated_servers1.)
