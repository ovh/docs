...
Durante l’installazione o la reinstallazione di una distribuzione Windows, viene fornita una password per accedere come amministratore. Ti consigliamo vivamente di modificarlo, come spiegato nella nostra guida Mettere in [sicurezza un server dedicato](securing-a-dedicated-server1.){.external}. Se hai perso la password admin, è necessario reimpostarla in modalità Rescue.
...
Per maggiori informazioni sulla modalità Rescue, consulta [questa guida](rescue_mode1.).

### Step 2: esegui il mount della partizione di sistema

Accedi al tuo server via SSH. Se necessario, consulta la guida [Introduzione a SSH](ssh_introduction1.)
...
[Utilizzare l’IPMI sui server dedicati](using_ipmi_on_dedicated_servers1.)
