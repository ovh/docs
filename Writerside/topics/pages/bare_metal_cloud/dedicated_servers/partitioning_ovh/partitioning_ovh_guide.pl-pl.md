...
With [OVHcloud Dedicated Servers](https://www.ovhcloud.com/pl/bare-metal/), you can configure Partitions, [software RAID](raid_soft1.), LVM, ZFS, etc. during [OS installation](getting-started-with-dedicated-server1.) from the [OVHcloud API](https://api.ovh.com/) as well as the [OVHcloud Control Panel](https://www.ovh.com/manager/#/dedicated/configuration). In this article, we will focus on the [OVHcloud API](https://api.ovh.com/). This will give us more details about the engine that is running in the background in order to create the partitioning on the dedicated server from the input data passed on to the OVHcloud API.
...
* A [dedicated server](https://www.ovhcloud.com/pl/bare-metal/) **ready to be installed/re-installed** in your OVHcloud account.
