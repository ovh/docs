...
>
> Per maggiori informazioni su questo attacco, consulta [le FAQ (EN)](faq-esxi1.).
>
...
- Disporre di un'offerta compatibile con la funzionalità [Network Firewall](firewall_network1.) per eseguire il filtraggio
...
>
> Per effettuare questa operazione, è necessario [riavviare](getting-started-with-dedicated-server#riavvio-del-tuo-server-dedicato.) il server ESXi attraverso lo Spazio Cliente OVHcloud.
> 
...
![interfaccia](gui_logs_.png){.thumbnail}
...
Ti proponiamo di attivare e utilizzare la nostra soluzione di filtraggio [Network Firewall](firewall_network1.).
...
![services_ssh](stop_service.png){.thumbnail}
...
![services_ssh](ssh_disabled_.png){.thumbnail} 

Applica gli stessi parametri per il servizio `slpd`:

![services_slp](slpd_.png){.thumbnail}
...
![rughe](firewall_web_.png){.thumbnail}
