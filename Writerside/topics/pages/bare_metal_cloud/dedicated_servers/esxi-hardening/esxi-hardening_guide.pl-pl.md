...
>
> Więcej informacji na temat tego ataku można znaleźć w [uzupełniającym FAQ (EN)](faq-esxi1.).
>
...
- Wykupienie usługi kompatybilnej z naszą funkcjonalnością [Network Firewall](firewall_network1.), jeśli chcesz korzystać z niej do filtrowania.
...
>
> W tym celu konieczne będzie [zrestartowanie](getting-started-with-dedicated-server#restart-serwera-dedykowanego.) serwera ESXi w Panelu klienta OVHcloud.
> 
...
![interfejs](gui_logs_.png){.thumbnail}
...
Proponujemy aktywację i korzystanie z naszego rozwiązania filtrowania [Network Firewall](firewall_network1.).
...
![usługi_ssh](stop_service.png){.thumbnail}
...
![usługi_ssh](ssh_disabled_.png){.thumbnail} 

Zastosuj te same parametry dla usługi `slpd`:

![usługi_slp](slpd_.png){.thumbnail}
...
![rule](firewall_web_.png){.thumbnail}
