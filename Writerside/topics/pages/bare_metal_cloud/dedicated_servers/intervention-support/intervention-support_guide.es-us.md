...
- Virtualisation
    ...
    - [XenServer](#xenserver.)
    - [ESXi](#esxi.)
    - [Windows (hyper-V)](intervention-support_#windows.)
...
1\. Reboot the server in [rescue mode](rescue_mode1.).<br>
...
1\. Reboot the server in [rescue mode](rescue_mode1.) and locate the partition `/`:
...
![proxmox](proxmox_edited.jpg){.thumbnail}

Do the following:

1\. Reboot the server in [rescue mode](rescue_mode1.).
...
- The new MAC address (viewable in the tab `Network interfaces`{.action} in the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=ws) or in the return of the command `ip link`).
...
1\. Restart the server in [rescue mode](rescue_mode1.) and use the `chroot` command:
...
![reset_network](restore.png){.thumbnail}

#### Version 6.7 or older

The procedure described below applies only to **versions 6.7 and earlier**.

1\. Restart the server in [rescue mode](rescue_mode1.) in order to mount the partition `/`:
...
1\. Log in as an administrator from the [IPMI/KVM](using_ipmi_on_dedicated_servers1.) interface.<br>
2\. Open the `Run`{.action} utility (Windows logo key + `R`{.action}), and through the `run` prompt, run the `devmgmt.msc` command:

![win_device_manager](win_device_manager_edited.png){.thumbnail}
...
![win_advanced](win_advanced_edited.png){.thumbnail}
...
![win_regedit_1](win_regedit_1_edited.png){.thumbnail}

4\. Then click `Load Hive...`{.action}.

![win_regedit_2](win_regedit_2_edited.png){.thumbnail}
...
![win_load](win_load_edited.png){.thumbnail}
...
![win_regedit_3](win_regedit_3_edited.png){.thumbnail}
...
1\. Restart the server in [rescue mode](rescue_mode1.), mount the partition `/` and then use the `chroot` command:
...
![generate_efi](generate_efi-v2.gif){.thumbnail}
...
![select_efi](select_efi-v2.gif){.thumbnail}
