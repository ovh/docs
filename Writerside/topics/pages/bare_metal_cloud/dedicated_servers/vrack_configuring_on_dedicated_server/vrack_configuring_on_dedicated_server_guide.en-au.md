...
![Order vrack](images_orderingvrack.png){.thumbnail}
...
![vRack selection](vrack_selection.png){.thumbnail}

### Step 3: Configuring your network interfaces

The following sections contain the configurations for the most commonly used recent distributions/operating systems. The first step is always to [log in to your server](getting-started-with-dedicated-server1.) via SSH or a RDP session (for Windows). The examples below presume you are logged in as a user with elevated permissions (Administrator/sudo).
...
![vRack interface](private_interface.png){.thumbnail}
...
![Windows Control Panel](windows_control_panel.png){.thumbnail}

Click on `Network and Internet`{.action}.

![Network and Internet](windows_network_and_internet.png){.thumbnail}

Open `Network and Sharing Center`{.action}.

![Network and Sharing Centre](windows_network_and_sharing_centre.png){.thumbnail}

Click on `Change Adapter Settings`{.action}.

![Change Adapter Settings](windows_change_adapter_settings.png){.thumbnail}
...
![Windows Properties](windows_properties_button.png){.thumbnail}

Double-click `Internet Protocol Version 4 (TCP/IPv4)`{.action}.

![Internet Protocol Version 4](windows_ipv4.png){.thumbnail}
...
![Use the following IP address](windows_use_following_ip_address.png){.thumbnail}
