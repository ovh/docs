...
![accesso server](listserversOVHcloud.png){.thumbnail}
...
- Monitoraggio: clicca su `Configura`{.action} per modificare le [impostazioni di monitoraggio del tuo server](getting-started-with-dedicated-server#monitoring-server.).
...
**DNS secondario**: configura qui il tuo DNS secondario. Per maggiori informazioni, consulta [questa guida](adding-secondary-dns-on-dedicated-server1.).

**Backup Storage** (disponibile solo sui server OVHcloud e sui server So you Start, inclusi quelli della linea Eco): ordina e configura il tuo backup storage qui. Per maggiori informazioni, consulta [questa guida](services_backup_storage1.).

**Interventi**: ritrova qui gli interventi in corso e passati sul tuo server.

**IPMI** (disponibile solo sui server OVHcloud e su alcuni server So you Start, inclusi quelli della linea Eco): accedi al tool IPMI o al KVM IP del tuo server. Per maggiori informazioni sull'utilizzo di questo strumento, consulta [questa guida](using_ipmi_on_dedicated_servers1.).

**Operazioni**: visualizza qui le operazioni recenti eseguite sul tuo server.

Per maggiori informazioni sulla gestione del tuo server dedicato, consulta [questa guida](getting-started-with-dedicated-server1.).
...
![sezione ip](manageIP2023.png){.thumbnail}
...
![licenza](managelicencesOVHcloud.png){.thumbnail}
...
- **Sicurezza**: questa sezione ti permette di gestire i parametri di sicurezza del tuo account. Per maggiori informazioni, consulta [questa guida](all_about_username1.).
...
- **Gestione degli utenti**: questa sezione ti permette di gestire i tuoi utenti. Per maggiori informazioni, consulta [questa guida](ovhcloud-users-management1.).
- **Le tue fatture**: da questa sezione è possibile consultare le fatture, seguire i pagamenti effettuati con il metodo di pagamento predefinito e consultare i tuoi averi. Per maggiori informazioni, consulta [questa guida](invoice_management1.).
- **I tuoi servizi**: questa sezione vi permette di visualizzare l'insieme dei vostri servizi e i vostri contratti.
- **Modalità di pagamento**: in questa sezione è possibile accedere al metodo di pagamento corrente, al conto prepagato e ai voucher OVHcloud. e aggiungere/rimuovere un metodo di pagamento. Per maggiori informazioni sulla gestione dei metodi di pagamento, consulta [questa guida](manage-payment-methods1.).
- **I tuoi ordini**: consulta i tuoi ordini da questa pagina. Per maggiori informazioni, consulta [questa guida](managing_ovh_orders1.).
- **I tuoi contatti**: in questa sezione è possibile visualizzare e gestire i contatti associati ai tuoi servizi. Nella scheda **Le tue richieste**, visualizzi le richieste di modifica dei contatti inviate dal tuo account e le richieste di gestione dei servizi ricevute. Per maggiori informazioni sulla gestione dei contatti di servizio, consulta [questa guida](managing_contacts1.).
...
[Accedi allo Spazio Cliente OVHcloud](ovhcloud-account-login1.).<br>
[Impostare e gestire la password di un account OVHcloud](manage-ovh-password1.).<br>
[Rendere sicuro il tuo account OVHcloud e gestire i tuoi dati personali](all_about_username1.).<br>
[Gestire i contatti dei servizi OVHcloud](managing_contacts1.).<br>
[Creare una chiave SSH](creating-ssh-keys-dedicated1.).<br>
[Quali sono gli indirizzi IP del monitoring OVHcloud?](network_ip_monitoring1.).
