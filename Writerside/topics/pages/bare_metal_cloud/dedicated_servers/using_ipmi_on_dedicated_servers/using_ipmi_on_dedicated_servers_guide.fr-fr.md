...
![IPMI Java initié](java_ipmi_initiate_2022.png){.thumbnail}

Téléchargez le fichier `kvm.jnlp` lorsque vous y êtes invité, puis lancez-le :

![Ouverture IPMI Java](java_ipmi_activation.png){.thumbnail}
...
![Activation de la déclaration d'intégrité IPMI](sol_ipmi_activation_2022.png){.thumbnail}
...
![Menu_démarrage](boot_menu.png){.thumbnail}
...
![Installation du KVM](kvm_install01.png){.thumbnail}

Cliquez sur `Start Media`{.action}. Cela préparera l'ISO pour le processus d'installation.

![Installation du KVM](kvm_install02.png){.thumbnail}
...
Lorsque le serveur est en [mode rescue](rescue_mode1.) et une fois que vous y êtes connecté, installez-le avec la commande suivante :
