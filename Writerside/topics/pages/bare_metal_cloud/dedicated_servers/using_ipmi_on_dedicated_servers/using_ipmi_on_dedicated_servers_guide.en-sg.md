...
To enable one of these methods, log in to your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg). In the `Bare Metal Cloud`{.action} section, select your server from `Dedicated Servers`{.action} and click on the `IPMI`{.action} tab.
...
![IPMI Java initiated](java_ipmi_initiate_2022.png){.thumbnail}

Download the file `kvm.jnlp` when you are prompted to do so, and run it:

![IPMI Java opening](java_ipmi_activation.png){.thumbnail}
...
![IPMI Java login](java_ipmi_login.png){.thumbnail}
...
![IPMI SoL activation](sol_ipmi_activation_2022.png){.thumbnail}
...
![IPMI test](ipmi_test_2022.png){.thumbnail}
...
![IPMI test](ipmi_reboot_2022.png){.thumbnail}
...
To begin, open [IPMI in a Java applet](#applet-java.) from the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg). Then, click `Device`{.action} from the menu bar and select `Redirect ISO`{.action} from the drop-down menu.
...
To begin, open [IPMI in a Java applet](#applet-java.) from the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg). Then, click `Virtual Media`{.action} and select `Virtual Storage`{.action}.

![Virtual_Storage](virtual_storage.png){.thumbnail}
...
![Boot_Menu](boot_menu.png){.thumbnail}
...
In the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg), open the [KVM web browser console](#kvm-browser.).
...
When the server is in [rescue mode](rescue_mode1.) and you have logged in, install it with the following commands:
...
Afterwards, access the [IPMI console](#procedure.) in your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg). You should see the server's BIOS menu displayed.
