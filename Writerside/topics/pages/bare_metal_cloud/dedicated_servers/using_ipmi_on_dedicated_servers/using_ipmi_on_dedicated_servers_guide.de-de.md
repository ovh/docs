...
![IPMI Java initiiert](java_ipmi_initiate_2022.png){.thumbnail}.

Laden Sie die `kvm.jnlp`-Datei herunter, wenn Sie dazu aufgefordert sind, und starten Sie sie.

![IPMI Java](java_ipmi_activation.png){.thumbnail}
...
![Java IPMI Verbindung](java_ipmi_login.png){.thumbnail}
...
![Aktivierung der IPMI Integritätserklärung](sol_ipmi_activation_2022.png){.thumbnail}
...
![Startmenü](boot_menu.png){.thumbnail}
...
![KVM installieren](kvm_install03.png){.thumbnail}

Klicken Sie auf `Power`{.action} und wählen Sie `Reset Server`{.action} im Drop-down-Menü aus.

![KVM installieren](kvm_install04.png){.thumbnail}
...
Wenn sich der Server im [Rescue-Modus](rescue_mode1.) befindet und Sie eingeloggt sind, installieren Sie es mit den folgenden Befehlen:
