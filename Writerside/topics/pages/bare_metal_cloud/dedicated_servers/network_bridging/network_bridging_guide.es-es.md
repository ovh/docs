...
>
> Vaya a la [página de configuración dedicada](proxmox-network-HG-Scale1.).
...
Conéctese al [área de cliente de OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.es/&ovhSubsidiary=es){.external}, haga clic en el menú `Bare Metal Cloud`{.action} y seleccione la sección `Network`{.action}. Haga clic en `IP`{.action}.
...
![Añadir una MAC virtual (1)](addvmac.png){.thumbnail}
...
![Añadir una MAC virtual (2)](addvmac2.png){.thumbnail}
...
También puede recuperar la dirección de la pasarela a través de [su área de cliente](network_bridging_#viacontrolpanel.) o la [API de OVHcloud](network_bridging_#viaapi.).

#### Desde el área de cliente <a name="viacontrolpanel"></a>

Conéctese al [Panel de configuración de OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.es/&ovhSubsidiary=es), acceda a la sección `Bare Metal Cloud`{.action} y seleccione el servidor en la sección `Servidores dedicados`{.action}.
...
En la [página API de OVHcloud](https://api.ovh.com/console/), haga clic en `Login`{.action}, situado en la esquina superior derecha. En la siguiente página, introduzca su ID de cliente de OVHcloud.
...
![navegar hasta el periférico de red](proxmox_01.png){.thumbnail}

A continuación, añada la dirección MAC que ha creado anteriormente.

![abrir un dispositivo de red](proxmox_02.png){.thumbnail}
...
![Menú contextual de MV](vmware_01.png){.thumbnail}
...
![Modificar los parámetros](vmware_02.png){.thumbnail}
