...
- A [dedicated server](https://www.ovhcloud.com/asia/bare-metal/), a [VPS](https://www.ovhcloud.com/asia/vps/) or a [Public Cloud instance](https://www.ovhcloud.com/asia/public-cloud/) in your OVHcloud account (excluding Windows systems)
...
> 
> To install WordPress on a server, you can follow [this tutorial](install_wordpress_on_an_instance1.).
...
[Configuring a firewall](firewall-Linux-iptable1.) (*iptables*) will enhance the security of your server. This process can be simplified by using the frontend "Uncomplicated Firewall" (UFW) and its preset of profiles. Install UFW:
...
In order to access your web server installation via a domain name, you need to attach it to your service. This is done by editing the DNS zone which you can do in your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/asia/&ovhSubsidiary=asia), provided OVHcloud is your domain registrar **and** the domain name uses OVHcloud DNS servers.

Please refer to our guide on [Editing a DNS zone](dns_zone_edit1.) for further instructions. If the domain name is currently in use, only configure DNS after your website or application is ready.
