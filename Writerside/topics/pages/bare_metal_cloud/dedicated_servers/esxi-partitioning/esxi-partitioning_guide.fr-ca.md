...
Malgré le fait que votre serveur ait plusieurs disques, l'installation d'ESXi n'est possible que sur le premier disque de la grappe de disques sélectionnée pour l'installation (voir « [API OVHcloud et installation d'un OS - Grappes de disques](api-os-installation#disk-group.) » ), les autres disques pouvant être configurés par la suite par l'utilisateur pour être utilisés pour stocker les machines virtuelles (voir « [Configuration du stockage d'un serveur HGR-STOR-2](hgrstor2_system_configuration#add-datastore.) » ).
...
⁴ Le client pourra toujours [ajouter un datastore](hgrstor2_system_configuration#add-datastore.) par la suite, mais uniquement sur les autres disques.<br />
...
> [!primary]
>
> Cette procédure est très similaire à celle [des autres OS](getting-started-with-dedicated-server1.), à l'exception qu'il n'est pas possible de sélectionner `Personnaliser la configuration des partitions`{.action} et qu'il y a une liste déroulante permettant de sélectionner le schéma de partitionnment à la quatrième et dernière étape.
>
...
![Bouton Réinstaller](reinstalling-your-server-00.png){.thumbnail}
...
![Sélection de template](reinstalling-your-server-01.png){.thumbnail}
...
Choisissez la grappe de disques sur laquelle vous souhaitez qu'ESXi soit installé. Notez que seul le premier disque de la grappe va être utilisé pour installer le système d'exploitation. Retrouvez plus d'information dans [ce guide](api-os-installation#disk-group.).

Cliquez sur `Suivant`{.action} pour continuer.

![Choix d'ESXi comme OS à installer](reinstalling-your-server-02.png){.thumbnail}
...
![Choix du schéma de partitionnement](esxi-custom-scheme-00.png){.thumbnail}
