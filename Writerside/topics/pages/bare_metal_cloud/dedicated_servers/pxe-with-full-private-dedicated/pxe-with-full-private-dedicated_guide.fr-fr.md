...
Vous avez donc la possiblité d'utiliser/déclarer vos réseaux *publics* et *privés* en passant par notre solution [vRack](vrack_configuring_on_dedicated_server1.).
...
La seule différence majeure notable est que les [réseaux privés](occ-concepts-overview#prive.) n'ont donc pas accès à tout ce qui n'appartient pas à votre infrastructure.<br>
...
![Netboot en action](netboot_steps.png)
...
- Avoir toutes les interfaces réseau de ce serveur en mode **privé**, ce qui sous-entend que vous avez préalablement configuré [notre fonctionnalité OLA](ola-enable-manager1.).<br>
...
![iPXE en action](animation.gif){.thumbnail}
