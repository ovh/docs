...
* Aver creato un'[istanza Public Cloud OVHcloud](public-cloud-first-steps1.)
...
Una volta configurato il tuo [progetto Public Cloud](create_a_public_cloud_project1.), è necessario aggiungerlo alla vRack. Ciò può avvenire in due modi:
...
![Ordina la vRack](orderingvrack.png){.thumbnail}
...
![aggiungi un progetto alla vrack](addprojectvrack.png){.thumbnail}
...
Se hai bisogno di aiuto, consulta la guida: [Creare un'istanza Public Cloud](public-cloud-first-steps#step-3-crea-unistanza.){.external}. Durante la creazione di un'istanza, potrai specificare, nello Step 4, una rete privata nella quale integrare la tua istanza. Nel menu a tendina presentato seleziona la tua vRack creata precedentemente.

#### Caso di un'istanza già esistente

È possibile associare un'istanza esistente a una rete privata. Per maggiori informazioni, consulta [questa sezione](getting-started-07-creating-vrack#in-caso-di-istanza-esistente.) della guida corrispondente.
...
Sui server dedicati, di default, sei sulla VLAN **0**. Per utilizzare questo ID, sarà necessario « taglare » la rete privata associata alla tua istanza con VLAN **0**. Per farlo, è necessario utilizzare l'[API OVHcloud (EN)](getting-started-08-creating-vrack-with-api#step-3-creating-a-vlan-in-the-vrack.).
...
Ad esempio: se hai definito la rete privata associata alla tua istanza con VLAN 2, l'interfaccia di rete privata del tuo server dedicato deve essere « tag » con VLAN 2. Per maggiori informazioni, consulta questa guida: [Creare due o più VLAN nella vRack](creating-multiple-vlans-in-a-vrack1.).

### Configura le interfacce di rete

Configura le interfacce di rete sulla tua nuova istanza Public Cloud e sul tuo server dedicato utilizzando questa guida: [Configurare diversi server dedicati nella vRack](vrack_configuring_on_dedicated_server1.){.external}.
