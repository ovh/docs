...
**Ce guide vous montre comment configurer le réseau privé entre une [instance Public Cloud](public-cloud-first-steps#etape-3-creer-une-instance.) et un [Serveur dédié](https://www.ovh.com/ca/fr/serveurs_dedies/).**

## Prérequis

* Avoir créé une [instance Public Cloud OVHcloud](public-cloud-first-steps1.)
...
Une fois votre [projet Public Cloud](create_a_public_cloud_project1.) configuré, il est nécessaire de l’ajouter au vRack. Cela peut se faire de deux manières :
...
![Commander le vrack](orderingvrack.png){.thumbnail}
...
![ajouter un projet au vrack](addprojectvrack.png){.thumbnail}
...
Si vous avez besoin d’aide, consultez le guide [Créer une instance Public Cloud](public-cloud-first-steps#etape-3-creer-une-instance.){.external}. Lors de la création d’une instance, vous pourrez spécifier, durant l’étape 4, un réseau privé dans lequel intégrer votre instance. Choisissez alors, dans le menu déroulant présenté, votre vRack précédemment créé.

#### Cas d’une instance déjà existante

Vous pouvez associer une instance existante à un réseau privé. Pour plus d'informations, vous pouvez consulter [cette section du guide correspondant](getting-started-07-creating-vrack#cas-dune-instance-deja-existante.).
...
Sur les serveurs dédiés, par défaut, vous êtes sur le VLAN **0**. Si vous souhaitez utiliser cet ID, il sera nécessaire de « tagguer » le réseau privé lié à votre instance avec le VLAN **0**. Pour cela, vous devez passer par [l’API OVHcloud](getting-started-08-creating-vrack-with-api#etape-3-creer-un-vlan-dans-le-vrack.).
...
Par exemple : si vous avez défini le réseau privé lié à votre instance avec VLAN 2, l'interface réseau privée de votre serveur dédié doit être « tagguée » avec VLAN 2. Pour plus d'informations consultez le guide suivant : [Créer plusieurs VLAN dans le vRack](creating-multiple-vlans-in-a-vrack1.).

### Configurer vos interfaces réseau

Configurez ensuite les interfaces réseau sur votre nouvelle instance Public Cloud et votre serveur dédié à l'aide de ce guide : [Configurer plusieurs serveurs dédiés dans le vRack](vrack_configuring_on_dedicated_server1.).
