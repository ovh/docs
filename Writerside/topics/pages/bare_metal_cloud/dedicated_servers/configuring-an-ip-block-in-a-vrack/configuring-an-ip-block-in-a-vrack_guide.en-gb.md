...
![check the second network interface](win-ip-vrack-1.png){.thumbnail}

Then we must check the properties:

![properties of the second network interface](win-ip-vrack-2.png){.thumbnail}

![properties of the second network interface](win-ip-vrack-3.png){.thumbnail}

#### Step 2: IP Configuration

We must select the ```Use the following IP address``` option:

![ip configuration](win-ip-vrack-4.png){.thumbnail}

And we can finally define the IP information:

![ip configuration](win-ip-vrack-5b.png){.thumbnail}

#### Step 3: Rebooting the network interface

First we do the disabling process

![disabling network](win-ip-vrack-6.png){.thumbnail}

Then we do the enabling process

![enabling network](win-ip-vrack-7.png){.thumbnail}
...
[Configuring the vRack on your dedicated servers](vrack_configuring_on_dedicated_server1.){.external}

[Creating multiple vLANs in a vRack](creating-multiple-vlans-in-a-vrack1.){.external}

[Configuring the vRack between the Public Cloud and a Dedicated Server](configuring-the-vrack-between-the-public-cloud-and-a-dedicated-server1.){.external}
