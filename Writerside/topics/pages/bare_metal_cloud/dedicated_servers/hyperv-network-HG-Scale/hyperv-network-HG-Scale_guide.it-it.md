...
![Statistiche IP](static_ip_4.png){.thumbnail}
...
![Configura RRAS](configure_rras_1.png){.thumbnail}

Scegli `Custom configurazione` e clicca su `Next`{.action}.

![Configura RRAS](configure_rras_2.png){.thumbnail}

Seleziona `LAN Routing` e clicca su `Next`{.action}.

![Configura RRAS](configure_rras_3.png){.thumbnail}

Nella nuova finestra, clicca su `Finish`{.action} e poi su `Start Service`{.action}.

![Configura RRAS](configure_rras_4.png){.thumbnail}
...
![Statistiche IP](static_ip_5.png){.thumbnail}
...
![Statistiche IP](static_ip_6.png){.thumbnail}
...
> [!warning]
>
> Questa fase può provocare la perdita di connessione. Se questo si verifica, accedi utilizzando l'[IPMI](using_ipmi_on_dedicated_servers1.) e modifica di nuovo la configurazione. Il tuo gateway di default è tornato vuoto. Aggiungi il gateway 100.64.0.1.
>
