...
![Instalacja](install_roles_4.png){.thumbnail}
...
![Konfiguracja RRAS](configure_rras_1.png){.thumbnail}

Wybierz `Custom konfiguracji` i kliknij `Next`{.action}.

![Konfiguracja RRAS](configure_rras_2.png){.thumbnail}

Następnie wybierz `LAN Routing` i kliknij `Next`{.action}.

![Konfiguracja RRAS](configure_rras_3.png){.thumbnail}

Następnie w oknie, które się wyświetli kliknij `Finish`{.action}, a następnie `Start Service`{.action}.

![Konfiguracja RRAS](configure_rras_4.png){.thumbnail}
...
![Status IP](static_ip_5.png){.thumbnail}
...
![Status IP](static_ip_6.png){.thumbnail}
...
> [!warning]
>
> Ten etap może spowodować utratę połączenia. Jeśli tak się stanie, zaloguj się za pomocą [IPMI](using_ipmi_on_dedicated_servers1.) i zmodyfikuj ponownie konfigurację. Zauważcie, że domyślny mostek powrócił do stanu pustego. Dodaj gateway 100.64.0.1.
>
