...
> [!primary]
>
> Se o seu servidor pertence à linha de produtos **Eco**, consulte [este guia](getting-started-with-dedicated-server-eco1.).

## Instruções

### Índice

- [Instalação ou reinstalação de um sistema operativo](getting-started-with-dedicated-server_#install.)
- [Ligação ao seu servidor](getting-started-with-dedicated-server_#connect.)
- [Reinicialização do seu servidor dedicado](getting-started-with-dedicated-server_#reboot.)
- [Segurança do seu servidor dedicado](getting-started-with-dedicated-server_#secure.)
- [Monitorização OVHcloud](getting-started-with-dedicated-server_#monitoring-server.)
- [Configuração de rede](getting-started-with-dedicated-server_#network.)
- [Modo rescue](getting-started-with-dedicated-server_#rescue.)
- [Acesso à ajuda do IPMI](getting-started-with-dedicated-server_#console.)
...
![Botão Reinstalar](getting-started-with-dedicated-server_images_reinstalling-your-server-01.png){.thumbnail}
...
- `Instalar a partir de uma imagem personalizada`{.action}: esta opção permite-lhe instalar uma imagem externa no servidor. Para mais informações, consulte o [guia sobre a funcionalidade Bring Your Own Image](bring-your-own-image1.).
...
![Seleção de templates](getting-started-with-dedicated-server_images_reinstalling-your-server-02.png){.thumbnail}
...
![Seleção operacional](images_reinstalling-your-server-03.png){.thumbnail}
...
![Personalizar a configuração das partições](images_reinstalling-your-server-04.png){.thumbnail}
...
Para obter uma explicação detalhada sobre a geração de chaves SSH, consulte o nosso [guia](creating-ssh-keys-dedicated1.).     

![configuração SSH](images_reinstalling-your-server-05.png){.thumbnail}
...
Receberá um e-mail com as informações necessárias para estabelecer a primeira ligação em SSH. O SSH é um protocolo de comunicação segura que é utilizado para estabelecer ligações encriptadas com um host remoto. Para mais informações, consulte o nosso guia: [Primeiros passos em SSH](ssh_introduction1.).
...
Como solução de recurso, pode utilizar a [consola IPMI na sua Área de Cliente OVHcloud](getting-started-with-dedicated-server_#console.) para estabelecer ligação.
...
> 1. **Ligar ao servidor**
>>
>> Ligue-se ao servidor através de RDP ou [IPMI](getting-started-with-dedicated-server_#console.).<br>
>>
...
Para aceder ao ficheiro de logs em modo rescue, siga as instruções do [manual do modo rescue](rescue_mode1.).
...
![Reiniciar](images_rebooting-your-server.png){.thumbnail}

<a name="secure"></a>

### Segurança do seu servidor dedicado

Como explicado acima, o cliente é o administrador do seu servidor dedicado. Enquanto tal, é responsável pelos seus dados e pela sua segurança. Para saber mais sobre a segurança do seu servidor, consulte o nosso guia [Proteger um servidor dedicado](securing-a-dedicated-server1.).

Se utilizar um servidor Windows, consulte [este guia](activate-port-firewall-soft-win1.).
...
Para mais informações sobre o sistema de monitorização, consulte [este manual](network_ip_monitoring1.).
...
Para mais informações sobre o modo bridge, consulte o nosso manual: [Modo bridge IP](network_bridging1.).
...
Para obter instruções detalhadas sobre a configuração do alias IP, consulte o manual [Configurar o endereço IP em alias](network_ipaliasing1.).

#### Configuração IPv6

Todos os servidores dedicados OVHcloud são entregues com um bloco /64 IPv6. Para utilizar os endereços deste bloco, deve introduzir modificações na configuração da rede. Consulte o nosso guia [Configuração IPv6](network_ipv61.).
...
Consulte o manual "[Ativar e utilizar o modo rescue](rescue_mode1.)".
...
Para mais informações, consulte o nosso manual "[Utilização do IPMI com servidores dedicados](using_ipmi_on_dedicated_servers1.)".
...
Para ativar e utilizar a opção Backup Storage, consulte [este guia](services_backup_storage1.).
