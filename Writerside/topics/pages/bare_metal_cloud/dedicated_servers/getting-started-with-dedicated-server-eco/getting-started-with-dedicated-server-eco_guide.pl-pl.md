...
![Przycisk Reinstalacja](images_reinstalling-your-server-01.png){.thumbnail}
...
- `Instalacja na podstawie spersonalizowanego obrazu`{.action}: ta opcja pozwala na zainstalowanie zewnętrznego obrazu na serwerze. Więcej informacji na temat tej opcji znajdziesz w [przewodniku Bring Your Own Image](bring-your-own-image1.).
...
![Wybór szablonu](images_reinstalling-your-server-02.png){.thumbnail}

Po wybraniu `Instalacji z szablonu OVHcloud`{.action}, możesz wybrać system operacyjny z menu rozwijanego.

![Wybór operacyjny](reinstalling-your-server-03.png){.thumbnail}
...
![Spersonalizuj konfigurację partycji](reinstalling-your-server-04.png){.thumbnail}
...
Aby uzyskać szczegółowe informacje na temat generowania kluczy SSH, zapoznaj się z naszym [przewodnikiem](creating-ssh-keys-dedicated1.).     

![konfiguracja SSH](reinstalling-your-server-05.png){.thumbnail}
...
Otrzymasz wówczas e-mail z informacjami niezbędnymi do ustanowienia pierwszego połączenia przez SSH. SSH to bezpieczny protokół komunikacyjny używany do ustanawiania szyfrowanych połączeń ze zdalnym hostem. Więcej informacji znajdziesz w naszym przewodniku: [Pierwsze kroki z SSH](ssh_introduction1.).
...
W zależności od Twoich potrzeb w zakresie bezpieczeństwa, mobilności i wygody, klucze SSH mogą służyć jako dodatkowa metoda logowania lub nawet zastąpić identyfikację za pomocą nazwy użytkownika i hasła. Dowiedz się, jak z nich korzystać w naszym przewodniku: [Tworzenie i używanie kluczy SSH](creating-ssh-keys-dedicated1.).
...
Aby uzyskać dostęp do pliku logów w trybie ratunkowym, należy postępować zgodnie z instrukcjami zawartymi w [przewodniku trybu ratunkowego](rescue_mode1.).
...
Więcej informacji na temat monitoringu OVHcloud znajdziesz w [tym przewodniku](network_ip_monitoring1.).
...
Więcej informacji na temat trybu bridge znajdziesz w naszym przewodniku: [Tryb bridge IP](network_bridging1.).
...
Szczegółowe instrukcje dotyczące konfiguracji aliasu IP znajdziesz w przewodniku "[Konfiguracja adresu IP jako aliasu](network_ipaliasing1.)".
...
Wszystkie serwery dedykowane OVHcloud są dostarczane z blokiem /64 IPv6. Aby korzystać z adresów tego bloku, należy wprowadzić zmiany w konfiguracji sieci. Zapoznaj się z naszym przewodnikiem "[Konfiguracja IPv6](network_ipv61.)".
...
Zapoznaj się z przewodnikiem "[Włącz i użyj trybu Rescue](rescue_mode1.)".
...
Aby uzyskać więcej informacji, zapoznaj się z naszym przewodnikiem "[Korzystanie z IPMI z serwerów dedykowanych](using_ipmi_on_dedicated_servers1.)".
...
Aby włączyć i korzystać z opcji Backup Storage, zapoznaj się [z tym przewodnikiem](services_backup_storage1.).

## Sprawdź również

[Konfiguracja kont użytkowników i dostępu root na serwerze](changing_root_password_linux_ds1.)
