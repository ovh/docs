...
> [!primary]
> Ce guide ne s'applique pas aux installations **Windows Server** standard car elles reposent sur le `Remote Desktop Protocol` (RDP) pour les connexions. Les connexions SSH sont cependant utilisées pour le mode rescue d’OVHcloud. Plus d'informations dans la section [Aller plus loin](creating-ssh-keys-dedicated_#gofurther.) de ce guide.
>
...
Nous vous invitons à consulter également le guide de présentation du [protocole SSH](ssh_introduction1.).
...
Pour en savoir plus sur les connexions SSH, consultez les guides de [premiers pas](#getstarted.) » et notre introduction au [protocole SSH](ssh_introduction1.).
...
[Connectez-vous](ssh_introduction1.) à votre serveur et assurez-vous que vous vous trouvez dans le répertoire `$HOME` de votre utilisateur. S'il n'existe pas déjà, créez le dossier `.ssh` :
...
![clé PuTTy](puttygen_04.png){.thumbnail}
...
![clé PuTTy](puttygen_05.png){.thumbnail}
...
![Espace de gestion des clés SSH](SSH_keys_panel_2022.png){.thumbnail}

Dans `Mes services`, positionnez-vous sur l'onglet `Clés SSH`{.action} et cliquez sur `Ajouter une clé SSH`{.action}.

![Espace de gestion des clés SSH](SSH_keys_panel_2.1.png){.thumbnail}
...
![Espace de gestion des clés SSH](SSH_keys_panel_3.png){.thumbnail}
...
> [!primary]
>
> Toutes les clés enregistrées dans la section `Dédié` sont disponibles en pré-installation sur un serveur dédié ou un VPS. En ce qui concerne les clés SSH pour les services Public Cloud, veuillez vous référer à [ce guide](public-cloud-first-steps1.).
>
...
![Espace de gestion des clés SSH](defaultsshkey.png){.thumbnail}
...
![Espace de gestion des clés SSH](defaultsshkey1.png){.thumbnail}
...
[Introduction au protocole SSH](ssh_introduction1.)

[Mode rescue sur serveur dédié](rescue_mode1.)

[Mode Rescue sur VPS](rescue1.)
