...
> [!primary]
> Diese Anleitung gilt nicht für Standardinstallationen von **Windows Server**, da sich diese auf das `Remote Desktop Protocol` (RDP) für Verbindungen stützen. SSH-Verbindungen werden jedoch für den Rescue-Modus von OVHcloud verwendet. Weitere Informationen finden Sie im Abschnitt [Weiterführende Informationen](creating-ssh-keys-dedicated_#gofurther.) dieser Anleitung.
>
...
Lesen Sie auch die Einführung zum [SSH Protokoll](ssh_introduction1.).
...
Weitere Informationen zu SSH-Verbindungen finden Sie in den "[Erste Schritte](#getstarted.)"-Anleitungen und unserer [Einführung in das SSH-Protokoll](ssh_introduction1.).
...
[Verbinden Sie sich mit Ihrem Server](ssh_introduction1.) und stellen Sie sicher, dass Sie sich im Verzeichnis `$HOME` Ihres Benutzers befinden. Wenn er noch nicht existiert, erstellen Sie den Ordner `.ssh`:
...
![SSH-Schlüsselverwaltungsbereich](SSH_keys_panel_2022.png){.thumbnail}

Gehen Sie in `Meine Dienste` auf den Tab `SSH-Schlüssel`{.action} und klicken Sie auf `SSH-Schlüssel hinzufügen`{.action}.

![SSH-Schlüsselverwaltungsbereich](SSH_keys_panel_2.1.png){.thumbnail}
...
![SSH-Schlüsselverwaltungsbereich](SSH_keys_panel_3.png){.thumbnail}
...
> [!primary]
>
> Alle unter `Dedicated` registrierten Schlüssel sind zur Vorinstallation auf einem Dedicated Server oder VPS verfügbar. Informationen zu SSH-Schlüsseln für Public-Cloud-Dienste finden Sie in [dieser Anleitung](public-cloud-first-steps1.).
>
...
![SSH-Schlüsselverwaltungsbereich](defaultsshkey.png){.thumbnail}
...
![SSH-Schlüsselverwaltungsbereich](defaultsshkey1.png){.thumbnail}
