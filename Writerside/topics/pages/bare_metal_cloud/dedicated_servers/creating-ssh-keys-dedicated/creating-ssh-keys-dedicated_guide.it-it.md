...
Per maggiori informazioni, consulta la guida introduttiva del [protocollo SSH](ssh_introduction1.).
...
Per saperne di più sulle connessioni SSH, consulta le guide di [primi passi](#getstarted.)" e la nostra introduzione al [protocollo SSH](ssh_introduction1.).
...
[Accedi](ssh_introduction1.) al server e assicurati di trovarti nella directory `$HOME` dell’utente. Se non esiste già, crea la cartella `.ssh`:
...
Nell'esempio precedente sono stati specificati solo l'IP del server e il file chiave, ma è possibile aggiungere ulteriori dettagli. Per configurare una connessione SSH a un secondo server con il nome utente "rocky", la [porta SSH modificata](secure_your_vps#changesshport.) "49160" e la chiave privata nel file "myserver_rsa", estendete il contenuto del file come indicato in questo esempio:
...
![chiave PuTTy](puttygen_04.png){.thumbnail}
...
![chiave PuTTy](puttygen_05.png){.thumbnail}
...
![Spazio di gestione delle chiavi SSH](SSH_keys_panel_2022.png){.thumbnail}

In `I miei servizi`, seleziona la scheda `Chiavi SSH`{.action} e clicca su `Aggiungi una chiave SSH`{.action}.

![Spazio di gestione delle chiavi SSH](SSH_keys_panel_2.1.png){.thumbnail}
...
![Spazio di gestione delle chiavi SSH](SSH_keys_panel_3.png){.thumbnail}
...
> [!primary]
>
> Tutte le chiavi registrate nella sezione `Dedicato` sono disponibili in preinstallazione su un server dedicato o un VPS. Per le chiavi SSH dei servizi Public Cloud, consulta la [presente guida](public-cloud-first-steps1.).
>
...
![Spazio di gestione delle chiavi SSH](defaultsshkey.png){.thumbnail}
...
![Spazio di gestione delle chiavi SSH](defaultsshkey1.png){.thumbnail}
...
[Introduzione al protocollo SSH](ssh_introduction1.)

[Modalità Rescue su server dedicato](rescue_mode1.)

[Modalità Rescue su VPS](rescue1.)
