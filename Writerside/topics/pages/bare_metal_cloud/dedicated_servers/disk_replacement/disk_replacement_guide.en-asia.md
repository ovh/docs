...
> 
> This guide is designed to assist you in common tasks as much as possible. However, we recommend that you call upon a [specialist service provider](https://partner.ovhcloud.com/asia/directory/) if you experience any issues or doubts when it comes to managing, using or securing your server. You can find more information in the “Go further” section of this guide.
> 
...
If you have a server that uses soft RAID, please refer to the [software RAID](raid_soft1.){.external} guide to find the disks installed on your server.
...
If you have a server that uses hard RAID, please refer to the [hardware RAID](raid_hard1.){.external} guide, and use the appropriate procedure for your RAID controller type to find the access paths to your disks.
...
If you have an NVMe disk, you will need to put the server into [rescue mode](rescue_mode1.){.external} and install the `nvme-cli` tool.
...
> [!primary]
>
> This replacement type is only possible for [Big-HG](https://www.ovh.com/asia/dedicated-servers/hg/){.external} servers that have a RAID card.
> 
...
If you have a server that uses soft RAID, we recommend that you resync your disks manually. To do this, you can refer to our [software RAID](raid_soft1.){.external} guide.

## Go further

[Software RAID](raid_soft1.){.external}

[Hardware RAID](raid_hard1.){.external}
