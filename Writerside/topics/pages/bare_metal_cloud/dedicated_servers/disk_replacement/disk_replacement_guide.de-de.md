...
Falls Sie einen Server mit Software-RAID besitzen, erfahren Sie in der Anleitung [Software-RAID](raid_soft1.){.external}, wie Sie die auf Ihrem Server installierten Festplatten finden.
...
Falls Sie einen Server mit Hardware RAID besitzen, lesen Sie unsere Anleitung [Managing Hardware RAID - EN](raid_hard1.){.external} und folgen Sie den Vorgaben für Ihren RAID-Controller-Typ, um die Zugriffspfade zu Ihren Festplatten zu identifizieren.
...
Bei einer NVMe-Disk muss der Server in erst in den [Rescue-Modus](rescue_mode1.){.external} versetzt werden. Installieren Sie dann das Tool `nvme-cli`.
...
- **die Seriennummer sowohl der zu ersetzenden Festplatte als auch aller weiteren intakten Festplatten**. Wie Sie die Seriennummer der zu ersetzenden Festplatte in Erfahrung bringen, sehen Sie in [dieser Anleitung](how_to_find_hdd_serial1.){.external} (Englisch). Sollten Sie aus irgendeinem Grund die Seriennummer der Festplatte nicht abrufen können, geben Sie dies bitte im Ticket an und teilen Sie uns die Seriennummern der nicht zu ersetzenden Festplatten mit. 
...
Wenn Sie über einen Software-RAID verfügen, muss die Resynchronisation Ihrer Festplatten manuell gestartet werden. Wie das geht, erfahren Sie in unserer Anleitung zum Thema [Software-RAID](raid_soft1.){.external}.

## Weiterführende Informationen

[Software-RAID](raid_soft1.)

[Hardware-Raid - EN](raid_hard1.)
