...
- [Obtenir les informations réseau via l'espace client](configure-ipv6_#viacontrolpanel.).
- [Obtenir les informations réseau via les API](configure-ipv6_#viaapi.).
...
L'adresse IPv6 et la gateway IPv6 assignées à votre serveur apparaissent dans la partie `IP`. Récupérez ces dernières puis poursuivez vers l'étape 2 « [Appliquer la configuration IPv6](configure-ipv6_#applyipv6.) ».
...
Une fois les adresses récupérées, continuez à l'étape 2 « [Appliquer la configuration IPv6](configure-ipv6_#applyipv6.) ».

### Étape 2 : appliquer la configuration IPv6 <a name="applyipv6"></a>

Une fois les informations nécessaires pour la configuration IPv6 en votre possession, connectez-vous en SSH à votre VPS. Si besoin, aidez-vous des informations de notre documentation « [Introduction au SSH](ssh_introduction1.){.external} ».
