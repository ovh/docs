...
- [Logowanie do serwera VPS (OS GNU/Linux)](starting_with_a_vps_#connect.)
- [Łączenie z serwerem VPS Windows](#winconnect.)
- [Zabezpiecz Twój serwer VPS](starting_with_a_vps_#secure.)
- [Przypisz domenę](starting_with_a_vps_#domain.)
...
>>
>> Więcej informacji na ten temat znajdziesz w naszym przewodniku o [trybie Rescue](rescue1.).
>>
> OS / Dystrybucja
>>
...
>>
>>
>> Po zainstalowaniu systemu wykonaj aktualizacje zabezpieczeń. Więcej informacji znajdziesz [poniżej](#reinstallvps.) i w naszym przewodniku [Zabezpiecz serwer VPS](secure_your_vps1.).
>> 
> Strefa / Lokalizacja
>>
...
> IPv4
>>
>> Główny publiczny adres IPv4 serwera VPS jest konfigurowany automatycznie podczas instalacji. Więcej informacji na temat zarządzania adresami IP znajdziesz w przewodniku [Konfiguracja IP aliasing](configuring-ip-aliasing1.).
>>
> IPv6 / Gateway
>> 
>> W tej części można zobaczyć publiczny adres IPv6 i adres przypisanej bramy. Są one automatycznie dołączane do serwera VPS podczas instalacji. Więcej informacji zawiera [ten przewodnik](configure-ipv61.).
>> 
> DNS secondary
>>
>> Funkcja ta jest przydatna przy instalowaniu usług DNS. Szczegółowo opisany jest [w przewodniku dotyczącym konfigurowania DNS secondary OVHcloud na serwerze VPS](adding-secondary-dns-on-vps1.).
...
Wszystkie informacje na temat dostępnych dla Twojej usługi rozwiązań do tworzenia kopii zapasowych można znaleźć na [stronie produktu](https://www.ovhcloud.com/pl/vps/options/) oraz w odpowiednich [przewodnikach](bare-metal-cloud-virtual-private-servers-backups1.).

#### Abonament

W tych sekcjach znajdują się najważniejsze informacje dotyczące fakturowania usługi. Wszystkie informacje na ten temat znajdziesz w odpowiedniej [dokumentacji](account-and-service-management-managing-billing-payments-and-services1.).
...
![Reinstalacja serwera VPS](2023panel_01.png){.thumbnail}

W oknie, które się pojawi, wybierz system operacyjny z rozwijanej listy. Proponowane opcje to obrazy [kompatybilne z serwerem VPS OVHcloud](image-life-cycle1.) i są gotowe do użytku natychmiast po instalacji.

Możesz również wybrać **klucz SSH** do zainstalowania w systemie, jeśli wcześniej przechowywałeś go w Panelu [klienta OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pl/&ovhSubsidiary=pl). Więcej informacji na ten temat znajdziesz w przewodniku Tworzenie i [używanie kluczy SSH](creating-ssh-keys-dedicated1.).
...
> 
O ile nie określono inaczej, wszystkie operacje administracyjne opisane w dokumentacji mogą być przeprowadzane przy użyciu domyślnego konta użytkownika, tzn. poprzez wpisanie `sudo` po odpowiednim poleceniu. Więcej informacji na ten temat znajdziesz w przewodniku [Konfiguracja kont użytkowników i dostępu root na serwerze](changing_root_password_linux_ds1.).
>

**Wykonaj następujące czynności**:

- Zapoznanie się z połączeniami SSH w przewodniku [Wprowadzenie do SSH](ssh_introduction1.).
- Rozważ użycie kluczy SSH jako zaawansowanej i wygodnej metody zdalnego połączenia za pomocą naszego przewodnika [Tworzenie i używanie kluczy SSH](creating-ssh-keys-dedicated1.).
- Zapoznaj się z przewodnikiem [Zabezpiecz serwer VPS](secure_your_vps1.), aby chronić Twój system przed automatycznymi atakami typu *brute force* i innymi typowymi zagrożeniami.

> [!primary]
>
Należy pamiętać, że w przypadku wyboru **dystrybucji z aplikacją** (Plesk, cPanel, Docker) ogólne środki bezpieczeństwa mogą nie mieć zastosowania do Twojego systemu. Zapoznaj się z przewodnikami Pierwsze [kroki z wstępnie zainstalowanymi](apps_first_steps1.) aplikacjami i [wdrażaj cPanel na serwerze VPS](cpanel1.), a także z oficjalną dokumentacją producenta.
>
...
Odbywa się to w konsoli VPS KVM: kliknij przycisk `...`{.action} obok nazwy serwera VPS w sekcji [Twój VPS](#yourvps.) i wybierz `KVM`{.action}. Więcej informacji na temat tego narzędzia znajdziesz w naszym [przewodniku KVM](using_kvm_for_vps1.).
...
>>
>> Połącz się z serwerem za pomocą zdalnego pulpitu lub sesji [KVM](using_kvm_for_vps1.).<br>
>>
...
Aby uzyskać dostęp do pliku dziennika w trybie ratunkowym, należy postępować zgodnie z instrukcjami zawartymi w przewodniku [tryb ratunkowy serwera VPS](rescue1.).
...
Zapoznaj się z naszym przewodnikiem [Zabezpieczanie VPS](secure_your_vps1.), aby uzyskać ważne wskazówki dotyczące ochrony systemu.
...
Aktywacja Twojego serwera VPS w Internecie zwykle polega na przypisaniu domeny i skonfigurowaniu DNS. Jeśli zarządzasz domeną w OVHcloud, zapoznaj się z naszym przewodnikiem [Edycja strefy DNS](dns_zone_edit1.) OVHcloud, aby uzyskać instrukcje.
...
W przypadku bardziej zautomatyzowanego procesu OVHcloud oferuje również rozwiązanie SSL Gateway. Więcej informacji znajdziesz na [stronie produktu](https://www.ovh.pl/ssl-gateway/) lub w [przewodniku](web-cloud-ssl-gateway1.) OVHcloud.
