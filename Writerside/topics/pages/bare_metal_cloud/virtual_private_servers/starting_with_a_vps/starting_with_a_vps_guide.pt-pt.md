...
- [Ligação ao seu VPS (OS GNU/Linux)](starting_with_a_vps_#connect.)
- [Ligação ao seu VPS Windows](#winconnect.)
- [Como proteger o VPS](starting_with_a_vps_#secure.)
- [Associar um nome de domínio](starting_with_a_vps_#domain.)
...
>>
>> Pode encontrar mais informações no nosso guia sobre o [modo rescue](rescue1.).
>>
> SO / Distribuição
>>
...
>>
>>
>> Uma vez instalado, o sistema deverá implementar as atualizações de segurança. Encontrará mais informações [abaixo](#reinstallvps.) e no nosso guia Como [Proteger um VPS](secure_your_vps1.).
>> 
> Zona / Localização
>>
...
> IPv4
>>
>> O endereço IPv4 público principal do VPS é configurado automaticamente durante a instalação. Encontre mais informações sobre a gestão dos IP no nosso guia [Configuring IP aliasing](configuring-ip-aliasing1.).
>>
> IPv6 / Gateway
>> 
>> Pode ver aqui o endereço IPv6 público e o endereço do gateway associado. Estes são automaticamente associados ao VPS aquando da instalação. Para mais informações, consulte [este guia](configure-ipv61.).
>> 
> DNS secundário
>>
>> Esta funcionalidade é útil para alojar serviços DNS. O nosso guia [Configurar o DNS secundário da OVHcloud num VPS](adding-secondary-dns-on-vps1.) descreve-o em pormenor.
...
Encontrará na [página do produto](https://www.ovhcloud.com/pt/vps/options/) e nos respetivos [manuais informações sobre as soluções de backup disponíveis para o serviço](bare-metal-cloud-virtual-private-servers-backups1.).

#### Subscrição

Estas secções apresentam as informações mais importantes sobre a faturação do serviço. Encontre todas as informações sobre este assumpto na [documentação correspondente](account-and-service-management-managing-billing-payments-and-services1.).
...
> 
Salvo indicação em contrário, todas as ações administrativas descritas na nossa documentação podem ser realizadas pela conta de utilizador predefinida, ou seja, introduzindo `sudo` seguido pelo comando correspondente. Saiba mais sobre este assumpto no guia "[Configuração das contas de utilizadores e acesso root num servidor](changing_root_password_linux_ds1.)".
>

**Recomendamos que efetue o seguinte procedimento**:

- Familiarizar-se com as ligações SSH consultando o guia [Introdução ao SSH](ssh_introduction1.).
- Considere a utilização de chaves SSH como um método avançado e mais prático para as ligações remotas através do nosso manual [Criar e utilizar chaves SSH](creating-ssh-keys-dedicated1.).
- Utilize o nosso guia [Tornar seguro um VPS](secure_your_vps1.) para proteger o seu sistema contra ataques automatizados de *brute force* e outras ameaças comuns.

> [!primary]
>
Tenha em conta que se selecionou uma **distribuição com aplicação** (Plesk, cPanel, Docker), as medidas de segurança genéricas podem não se aplicar ao seu sistema. Sugerimos que consulte os nossos manuais [Primeiros passos com as aplicações pré-instaladas](apps_first_steps1.) e [Implementar o cPanel num VPS](cpanel1.), assim como a documentação oficial do editor em causa.
>
...
Isto é feito na consola VPS KVM: clique no botão `...`{.action} ao lado do nome do seu VPS na secção [O seu VPS](#yourvps.) e selecione `KVM`{.action}. Pode encontrar mais informações sobre esta ferramenta no nosso [guia KVM](using_kvm_for_vps1.).
...
> 1. **Ligar ao servidor**
>>
>> Ligar-se ao seu servidor através de um ambiente de trabalho remoto ou de uma sessão [KVM](using_kvm_for_vps1.).<br>
>>
...
Para aceder ao ficheiro de registo em modo rescue, siga as instruções do manual do [modo rescue do VPS](rescue1.).
...
Consulte o nosso manual sobre Como [proteger um VPS](secure_your_vps1.) para obter conselhos essenciais sobre como proteger o seu sistema.
...
A passagem do seu VPS na web passa geralmente pela atribuição de um nome de domínio e a sua configuração DNS. Se gere o seu domínio na OVHcloud, pode consultar o nosso guia [Editar uma zona DNS da OVHcloud](dns_zone_edit1.) para obter instruções.
...
Para um processo mais automatizado, a OVHcloud propõe igualmente a solução SSL Gateway. Consulte a [página do produto](https://www.ovh.pt/ssl-gateway/) ou a [documentação de apoio](web-cloud-ssl-gateway1.) para obter mais informações.
...
[Como proteger um VPS](secure_your_vps1.)
