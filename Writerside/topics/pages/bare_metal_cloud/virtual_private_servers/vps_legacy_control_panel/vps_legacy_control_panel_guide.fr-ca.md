...
Cette option ouvrira une fenêtre contextuelle pour initier le processus de [résiliation de votre service](how_to_cancel_services1.).

### Redémarrer en mode rescue

Cliquez sur cette option pour redémarrer le VPS en mode rescue. Retrouvez tous les détails dans [notre guide](rescue1.).
...
- la langue ;
- une [clé SSH](creating-ssh-keys-dedicated1.) (facultatif)
...
Lors de la réinstallation de votre VPS, un e-mail contenant votre mot de passe root vous sera envoyé pour vous connecter à votre VPS en [SSH](ssh_introduction1.), sauf si vous avez sélectionné une [clé SSH](creating-ssh-keys-dedicated1.) à préinstaller.
...
Il est possible de désactiver les connexions d'utilisateurs root via le protocole SSH. Pensez à [créer un autre utilisateur](secure_your_vps#createuser.) avant de poursuivre les étapes ci-dessous.
...
Utilisez cette option pour vous connecter à votre VPS via KVM. Retrouvez tous les détails dans [notre guide](using_kvm_for_vps1.).
...
[Création et utilisation de clés SSH](creating-ssh-keys-dedicated1.)

[Sécuriser un VPS](secure_your_vps1.)

[Débuter avec un VPS](starting_with_a_vps1.)
