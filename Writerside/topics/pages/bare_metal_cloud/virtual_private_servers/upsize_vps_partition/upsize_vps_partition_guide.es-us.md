...
- Tener acceso de administrador al VPS ([Windows](upsize_vps_partition_#windows.)).
- Haber reiniciado el servidor en [modo de rescate](rescue1.) (solo Linux).
...
Si el VPS todavía no está en modo de rescate, actívelo gracias a [nuestra guía](rescue1.).
...
- **bad magic number in superblock**: No continúe. Para solucionar este problema, consulte el apartado [Cómo solucionar los errores «bad magic number in superblock»](upsize_vps_partition#como-solucionar-los-errores-bad-magic-number-in-superblock.){.external} de esta guía.
