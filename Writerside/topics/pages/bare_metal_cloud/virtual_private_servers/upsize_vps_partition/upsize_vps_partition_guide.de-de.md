...
- Sie haben administrativen Zugriff auf Ihren VPS (für [Windows-Systeme](upsize_vps_partition_#windows.)).
- Der Server wurde im [Rescue-Modus](rescue1.) neu gestartet (nur für GNU/Linux-Systeme notwendig).
...
Wenn sich der VPS noch nicht im Rescue-Modus befindet, aktivieren Sie ihn mithilfe von [unserer Anleitung](rescue1.).
...
- `bad magic number in superblock`: Fahren Sie nicht fort. Um dieses Problem zu beheben, befolgen Sie in der vorliegenden Anleitung die Anweisungen im Abschnitt “[Wie wird der Fehler *bad magic number in superblock* behoben?](upsize_vps_partition#wie-wird-der-fehler-bad-magic-number-in-superblock-behoben.)”.
