...
- [Reimposta la password dell'utente per recuperare l'accesso](replacing-user-password1.)
...
![configurazione della modalità rescue](rescue_new.png){.thumbnail}

Seleziona `Riavvia in modalità Rescue`{.action} nel menu.

Se il tuo Spazio Cliente è differente, consulta la nostra guida "[Gestire un VPS legacy](vps_legacy_control_panel1.)".
...
In seguito, dovrai [accedere al tuo server via SSH](ssh_introduction1.), utilizzando la password temporanea generata per la modalità rescue.
...
A questo punto è possibile apportare modifiche al sistema, ad esempio [reimpostare le password utente e le chiavi SSH](rescue_#gofurther.).
...
- Consulta il KVM nel tuo Spazio Cliente per ottenere informazioni utili sulle ragioni per cui il VPS non può essere avviato. Per maggiori informazioni su questa funzionalità, consulta la nostra [guida KVM](using_kvm_for_vps1.).
- Se il KVM indica che il VPS è bloccato all'avvio o che non riesce a trovare il disco, assicurati che [i log di avvio sono attivati](bootlog_display_kvm1.). Per ulteriori indagini, invia i log rilevanti al nostro team di supporto [crea una richiesta di assistenza](https://help.ovhcloud.com/csm?id=csm_get_help).
...
[Recupera l'accesso al server in caso di perdita della password utente](replacing-user-password1.)

[Sostituire una coppia di chiavi SSH](replacing-lost-ssh-key1.)

[Controllare il file system su un VPS](check-filesystem1.)
