...
HA is enabled by default in the first cluster that OVHcloud provides you when we deliver your Managed Bare Metal.
...
Disabling it is required to perform update operations with Update Manager.
...
![](HAparam4.PNG){.thumbnail}

##### VM Monitoring

VM monitoring is available after [VMware tools](vmware_tools_install1.){.external} is installed, in case of non-response via the **tools** (heartbeat signals) the virtual machine will be automatically restarted. Advanced configuration is possible for this feature (for example, reboot interval).

![](HAparam5.PNG){.thumbnail}
