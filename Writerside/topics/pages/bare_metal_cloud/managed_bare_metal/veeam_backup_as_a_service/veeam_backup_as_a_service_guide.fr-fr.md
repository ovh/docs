...
* [Donner le droit « Ajout de ressources »](change-user-rights1.){.external} pour le datacenter concerné à l'utilisateur depuis l'[espace client OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr){.external}.
* Être connecté au client vSphere.
* Avoir activé la [haute disponibilité (HA)](vmware_ha_high_availability1.){.external}.
* Avoir activé le [Distributed Ressource Scheduler (DRS)](vmware_drs_distributed_ressource_scheduler1.){.external} sur le ou les clusters.
...
![Activer le backup](veeam-managed-bare-metal.png){.thumbnail}
...
![Activer le backup](backuppcc_03_en.png){.thumbnail}
...
![Serveur de sauvegarde](backupserver.png){.thumbnail}
...
![Activation de la sauvegarde](backupvm_02.png){.thumbnail}

Une fenêtre de confirmation apparaît. Cliquez simplement sur `OK`{.action}.

![Confirmation de la sauvegarde](backupvm_03.png){.thumbnail}
...
![Restauration du backup](restorebackup_01.png){.thumbnail}

Sur le panneau à droite, cliquez alors sur `Restore Backup`{.action} pour lancer la demande de restauration.

![Restauration du backup](restorebackup_02.png){.thumbnail}
...
![Initialisation de la restauration](restorebackup_03.png){.thumbnail}
...
![Validation de la restauration](restorebackup_04.png){.thumbnail}

La machine est restaurée à côté de la machine source.

![La machine source est restaurée](restorebackup_05.png){.thumbnail}
...
![Conflit d'IP](restorebackup_06.png){.thumbnail}
...
![Désactivation de la machine](restorebackup_01.png){.thumbnail}

Sur le panneau à droite, cliquez alors sur `Disable Backup on this VM`{.action} pour désactiver la sauvegarde.

![Désactivation de la sauvegarde](restorebackup_02.png){.thumbnail}

Confirmez ensuite la désactivation en cliquant sur `OK`{.action}.

![Confirmation de désactivation](disablebackup_03.png){.thumbnail}
