...
Backups are performed using a virtual machine (VM) within your [Managed Bare Metal](https://www.ovhcloud.com/en-ie/managed-bare-metal/) infrastructure. The backup data is outsourced to an independent storage space at OVHcloud. Backups are performed during night-time, with a retention time depending on the solution level you choose.
...
- a user account with access to vSphere and the permission ["Add resources"](change-user-rights1.) for the pertinent data centre
- [vSphere High Availability (HA)](vmware_ha_high_availability1.) enabled
- [Distributed Resource Scheduler (DRS)](vmware_drs_distributed_ressource_scheduler1.) enabled
...
![Enable backup](veeam-managed-bare-metal.png){.thumbnail}
...
![Enable backup](backuppcc_03_en.png){.thumbnail}
...
![Backup Server](backupserver.png){.thumbnail}
...
![Backup Management](backupvm_01.png){.thumbnail}
...
![Enabling Backup](backupvm_02.png){.thumbnail}

In the confirmation window, click `OK`{.action}.

![Backup Confirmation](backupvm_03.png){.thumbnail}
...
![Restoring backup](restorebackup_01.png){.thumbnail}

On the right-hand panel, click `Restore Backup`{.action} to open the restore request.

![Restoring backup](restorebackup_02.png){.thumbnail}
...
![Initialising Restore](restorebackup_03.png){.thumbnail}
...
![Validating Restore](restorebackup_04.png){.thumbnail}

The VM is restored next to the source machine.

![Source machine is restored](restorebackup_05.png){.thumbnail}
...
![IP Conflict](restorebackup_06.png){.thumbnail}
...
![Machine Disable](restorebackup_01.png){.thumbnail}

On the right-hand panel, click `Disable Backup on this VM`{.action}.

![Disabling Backup](restorebackup_02.png){.thumbnail}

Then confirm the deactivation by clicking `OK`{.action}.

![Deactivation Confirmation](disablebackup_03.png){.thumbnail}
