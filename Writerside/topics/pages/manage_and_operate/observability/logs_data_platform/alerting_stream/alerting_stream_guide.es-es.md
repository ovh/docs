...
- [Quick start of Logs Data Platform](getting_started_quick_start1.).
- [Logstash collector on Logs Data Platform](ingestion_logstash_dedicated_input1.).
- [The Filebeat tutorial](ingestion_filebeat1.).
...
For this tutorial, we will configure the 3 alerts that we can use for a website. These 3 alerts can help you to react immediately in the case of a failure, detect unexpected problems or verify that all your websites are working correctly. But before going into the alerting feature itself, we need to configure our Apache Logging format to include all the information we need. We will also use [Filebeat](ingestion_filebeat1.) to send our logs to Logs Data Platform.
...
The configuration is inspired by the one you can find in [this tutorial](ingestion_apache1.).
...
![SSL input](ssl_input.png){.thumbnail}
...
![Navigate to alert](nav-alert.png){.thumbnail}
...
![Alert Creation](no-traffic-creation.png){.thumbnail}
...
![Alert Created](alert-created.png){.thumbnail}
...
![No traffic mail](mail-no-traffic.png){.thumbnail}
...
![Slow website alert](slow-alert.png){.thumbnail}
...
![Slow website alert](slow-mail.png){.thumbnail}
...
![Slow website alert](alert-500.png){.thumbnail}
...
![Slow website alert](apache-500.png){.thumbnail}
