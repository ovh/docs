...
- Log in to the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/asia/&ovhSubsidiary=asia), and navigate to the Bare Metal Cloud section located at the top left in the header.
- Once you have created your credentials, the main interface will appear:

![Main interface](manager_start.png){.thumbnail}
...
3. Edit your LDP access tokens (more on this functionality [here](security_tokens1.)).
...
![new Stream](newStream.png){.thumbnail}
...
![new Stream](streamMenu.png){.thumbnail}
...
- **Monitor in real-time** allows you to see the logs incoming into your stream in real-time. Note that Graylog also provides this functionality. On this page you can also **Test** different log formats from your computer to your stream. [More about](cli_ldp_tail1.)
- **Manage alerts** allows you to define your alert conditions on the logs routed to the stream. [More about](alerting_stream1.)
- **Archives** allows you to download the cold stored archives. [More about](archive_cold_storage1.)
...
![About page](about.png){.thumbnail}
...
![Graylog access](graylog-access.png){.thumbnail}

The Graylog login page looks like this:

![login Graylog](getting_started_quick_start_images_login.png){.thumbnail}

Once logged, you will be redirected to this page:

![Graylog Stream](graylog-stream.png){.thumbnail}
...
![Dashboard access](dashboard-access.png){.thumbnail}
...
![critical Dashboard](critical-dashboard.png)
...
![widget creation](widget-create.png)
...
![critical Dashboard 2](critical-dashboard-2.png)
...
- [Send correctly formatted logs](getting_started_field_naming_convention1.) to use custom types as number, boolean and other stuffs
- [Configure your syslog-ng](ingestion_syslog_ng1.) to send your Linux logs to Logs Data Platform.
- [Using roles](getting_started_roles_permission1.) to allow other users of the platform to let them see yours beautiful Dashboards or let them dig in your Streams instead of doing it for them.
- [Using OpenSearch Dashboards and aliases to unleash the power of OpenSearch](visualization_opensearch_dashboards1.)
