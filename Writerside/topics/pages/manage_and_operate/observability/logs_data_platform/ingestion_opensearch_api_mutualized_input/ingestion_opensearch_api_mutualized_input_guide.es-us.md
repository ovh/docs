...
Replace the `<user>`, `<password>` and `<ldp-cluster>` with your Logs Data Platform username, password and cluster. You can also use [tokens](security_tokens1.) in place of your credentials.  Sending this payload will result in this log:

![simple\_log](one_field.png){.thumbnail}
...
![gelf\_log](gelf_log.png){.thumbnail}

The system used the reserved fields associated with GELF to create the message and the source fields.

Logs Data Platform will also detect any typed field in the original data and convert them accordingly to our [field naming convention](getting_started_field_naming_convention1.). This last example illustrates it:
...
![gelf\_convention](gelf_convention.png){.thumbnail}

The OpenSearch input will also flatten any sub-object or array sent through it and also supports ingest pipelines that are used, for example, with [Filebeat integrations](ingestion_filebeat1.).

## Use case: Vector

[Vector](https://vector.dev/){.external} is a fast and lightweight log forwarder written in Rust. This software is quite similar to [Logstash](ingestion_logstash_dedicated_input1.) or [Fluent Bit](ingestion_kubernetes_fluent_bit1.). It takes logs from a source, transforms them and sends them in a format compatible with the configured output module.
...
- **auth.user** and **auth.password** must be set to the username of the Logs Data Platform account and its associated password. Note that you can use [tokens](security_tokens1.) in place of your credentials.

Once configured and launched you will immediately see this type of logs in Graylog:

![vector\_logs](vector_logs.png){.thumbnail}
