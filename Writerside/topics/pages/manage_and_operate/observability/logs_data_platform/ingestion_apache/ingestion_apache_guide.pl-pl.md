...
> [Shipping logs to Logs Data platform with Filebeat](ingestion_filebeat1.)
>
...
![apache Logs](apache-logs.png){.thumbnail}
...
To keep thing brief, this extract has only the parts relevant to the access log file. [The syslog-ng tutorial](ingestion_syslog_ng1.) covers the configuration for any syslog file (like the error log file). This configuration is only valid for syslog-ng 3.8+.

### Apache logs format

If you want to use your own log format and include some useful information here is a cheat sheet for you (Note that the labels follows [the field naming conventions](getting_started_field_naming_convention1.)).
...
The complete procedure of its installation is described [on this page](alerting_stream1.) in the Apache logs use case.
