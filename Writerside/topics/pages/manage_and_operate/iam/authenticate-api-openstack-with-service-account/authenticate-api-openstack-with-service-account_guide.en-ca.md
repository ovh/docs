...
To automate calls to protected APIs, you need to provide credentials to the code that handles them. With OVHcloud service accounts, it is possible to have a single ID per script for the use of the different APIs offered by OVHcloud products ([OVHcloud API](console-preview1.), [OpenStack API](starting_with_nova1.), etc.).
...
- You know [how to configure access policies via API](iam-policies-api1.).
- You know [how to use the OpenStack API](starting_with_nova1.)
- You have created a [service account via API](manage-service-account1.).
