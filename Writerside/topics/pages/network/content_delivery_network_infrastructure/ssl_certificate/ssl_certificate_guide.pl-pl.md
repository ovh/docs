...
Aby tworzenie certyfikatu przebiegało prawidłowo, konieczne jest, aby subdomena, którą właśnie dodałeś, wskazywała poprawnie CDN. Zapoznaj się z przewodnikiem, który wyjaśni, jak przeprowadzić [pierwszą konfigurację domeny](first_domain_name_configuration1.){.external}.
...
![Tworzenie certyfikat SSL w toku](ssl_in_progress.png){.thumbnail}
...
![Certyfikat SSL został utworzony](ssl_validated.png){.thumbnail}
...
![Dodanie certyfikatu SSL](add_ssl.png){.thumbnail}

- Jeśli posiadasz już certyfikat Let's Encrypt, możesz użyć opcji `Zastąp moim certyfikatem`{.action}:

![Zmiana certyfikatu SSL](change_ssl.png){.thumbnail}

## Sprawdź również

[Pierwsza konfiguracja domeny](first_domain_name_configuration1.){.external}
