...
![Service OVH Load Balancer](iplb_service.png){.thumbnail}
...
![Ajout d'une ferme de serveurs via le Manager](add_farm.png){.thumbnail}
...
![Détails de la ferme de serveurs créée](resume_farm.png){.thumbnail}
...
![Ajour d'un serveur dans une Ferme.](add_server.png){.thumbnail}
...
![Détails du serveur créé.](resume_server.png){.thumbnail}
...
![Ajout d'un frontend](add_frontend.png){.thumbnail}

Cliquez sur le bouton `Ajouter`{.action} une fois les champs remplis.

Votre Frontend devrait apparaître dans la liste, sous l'onglet `Frontends`{.action}.

![Détails du frontend créé](resume_frontend.png){.thumbnail}
...
![Connexion à SMTP via telnet](resume_validate.png){.thumbnail}
