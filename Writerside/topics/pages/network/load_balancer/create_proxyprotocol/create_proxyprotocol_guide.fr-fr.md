...
Si vous utilisez un frontend HTTP, nous vous recommandons le guide « [Configuration d'un service OVHcloud Load Balancer - Les en-têtes HTTP](create_headers1.) » qui décrit la manière standard d'utiliser les en-têtes HTTP pour retrouver l'adresse IP, le port ainsi que le protocole source.
...
![Adresse IPv4 de sortie de votre service OVHcloud Load Balancer](create_proxyprotocol_images_iplb_service.png){.thumbnail}
