...
Avant de vous lancer, si vous ne l'avez pas encore lue, nous vous conseillons de consulter la présentation générale du service OVHcloud Load Balancer: [Présentation de l'OVHcloud Load Balancer](use_presentation1.).
...
![Service OVHcloud Load Balancer](create_http_https_images_iplb_service.png){.thumbnail}

Pour plus d'informations sur les fonctionnalités de l'espace client OVHcloud, consultez la page « [Gestion du service Load Balancer via l’espace client](use-lb1.) ».
...
Pour plus d'informations sur les fonctionnalités de l'API, consultez la page [Détails des fonctions API](use_api_details1.){.ref}
...
![Ajout d'une ferme de serveurs via le Manager](add_backend.png){.thumbnail}
...
![Détails de la ferme de serveurs créée](resume_backend.png){.thumbnail}
...
![Ajour d'un serveur dans une Ferme.](images_add_server.png){.thumbnail}
...
![Détails du serveur créé.](images_resume_server.png){.thumbnail}
...
![Ajout d'un frontend](images_add_frontend.png){.thumbnail}

Cliquez sur le bouton `Ajouter`{.action} une fois les champs remplis.

Votre frontend devrait apparaître dans la liste, sous l'onglet `Frontends`{.action}.

![Détails du frontend créé](images_resume_frontend.png){.thumbnail}
...
![Boutons d'ajout de certificats](certificat.png){.thumbnail}
...
![Ajout d'un certificat existant](add_certificat_custom.png){.thumbnail}
...
![Command d'un certificat Let's Encrypt](add_certificat_letsencrypt.png){.thumbnail}
...
![Appliquer la configuration](btn_apply_configuration.png){.thumbnail}
...
![Appliquer la configuration zones](btn_apply_configuration_zones.png){.thumbnail}
