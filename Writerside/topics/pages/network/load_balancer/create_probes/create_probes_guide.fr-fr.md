...
Pour plus de détails sur le déploiement d'une architecture Blue-Green avec votre OVHcloud Load Balancer, référez-vous à [ce guide](case_blue_green1.).
...
![Paramètres avancés d'une ferme](farm_advanced_settings.png){.thumbnail}

Vous avez alors accès à la configuration du type de sonde.

![Paramètres d'une sonde](farm_advanced_settings_unfolded.png){.thumbnail}

Si le type de sonde sélectionné le permet, vous pouvez configurer les paramètres avancés spécifiques à cette sonde.

![Paramètres avancés d'une sonde](probe_settings.png){.thumbnail}

Une nouvelle fenêtre de configuration apparaît avec les paramètres de la sonde.

![Paramètres avancés d'une sonde](probe_settings_dialog.png){.thumbnail}
