...
![Ajouter une zone load balancer depuis le manager](add_Zone_IPLB.png){.thumbnail}

Sélectionnez alors le(s) zone(s) que vous souhaitez commander et cliquez sur `Ajouter`{.action}.
 
![Selection d'une zone load balancer depuis le manager](Select_Zone_IPLB.png){.thumbnail}

Un bon de commande est généré, il vous faudra le régler.

![Payer la commande zone load balancer depuis le manager](Paybill_Zone_IPLB.png){.thumbnail}
...
![Choix de la zone](Select-Datacenter.png){.thumbnail}
...
![Application d'une Configuration d'un Load Balancer](apply-configuration.PNG){.thumbnail}
...
![utiliser plusieurs zones](Edit-frontend-All-iplb.png){.thumbnail}
...
![Fonctionnement avec plusieurs zones et plusieurs fermes](multi_zones_multi_backends.png){.thumbnail}
