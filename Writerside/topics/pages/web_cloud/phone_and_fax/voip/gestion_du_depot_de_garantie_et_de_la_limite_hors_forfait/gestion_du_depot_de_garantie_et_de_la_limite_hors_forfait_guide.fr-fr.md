...
> Pour éviter ce dernier cas de figure, nous vous recommandons de [sécuriser votre ligne SIP](secure-sip-line1.).  
> 
...
![hors-forfait voip](HFvoip1.png){.thumbnail}
...
Vous pouvez augmenter la limite de base du groupe à 300€ sans verser d'argent, en [suivant ces instructions.](gestion_du_depot_de_garantie_et_de_la_limite_hors_forfait#augmenter-la-limite-de-base.)

Pour l'exemple cité plus haut :

![hors-forfait voip](HFvoip2.png){.thumbnail}

Vous pouvez également augmenter votre dépôt de garantie en [suivant cette méthode](gestion_du_depot_de_garantie_et_de_la_limite_hors_forfait#augmenter-le-depot-de-garantie.). 

Pour l'exemple cité plus haut :

![hors-forfait voip](HFvoip3.png){.thumbnail}
...
> Pour en savoir plus, reportez-vous à la partie [Qu'arrive t-il en cas de dépassement ?](gestion_du_depot_de_garantie_et_de_la_limite_hors_forfait#quarrive-t-il-en-cas-de-depassement.) en bas de ce guide.
>
...
Connectez-vous à votre [espace client OVHcloud](manager.){.external}, sélectionnez l'onglet `Telecom`{.action} puis la rubrique `Téléphonie`{.action} et enfin votre groupe de téléphonie.
Dans l'onglet `Facturation`{.action}, cliquez sur `Modifier votre limite de dépôt de garantie`{.action}.

![augmenter limite hors forfait](hflimit1.png){.thumbnail}
...
![augmenter limite hors forfait](hflimit2.png){.thumbnail}
...
![augmenter depot de garantie](securitydeposit1.png){.thumbnail}
...
![augmenter depot de garantie](securitydeposit2.png){.thumbnail}

Un bon de commande sera alors généré. Cliquez sur le numéro de bon de commande pour l'afficher et le payer.

![augmenter depot de garantie](securitydeposit3b.png){.thumbnail}
...
![Suppression groupe de téléphonie](SupprGroup.png)
...
> Pour cela, consultez le guide ["Gestion des groupes de téléphonie"](regrouper_services_telephonie1.) afin de créer un nouveau groupe ou de déplacer les services dans un groupe existant.
>
...
![transfert depot de garantie](transferdeposit1.png){.thumbnail}
...
![transfert depot de garantie](transferdeposit2.png){.thumbnail}
...
![consommation générale](voipconso.png){.thumbnail}

Si vous constatez des anomalies, commencez par [sécuriser votre ligne SIP](secure-sip-line1.). 

Une fois ces vérifications effectuées, vous pouvez débloquer le groupe en [augmentant le dépôt de garantie du groupe](gestion_du_depot_de_garantie_et_de_la_limite_hors_forfait#augmenter-le-depot-de-garantie.).
