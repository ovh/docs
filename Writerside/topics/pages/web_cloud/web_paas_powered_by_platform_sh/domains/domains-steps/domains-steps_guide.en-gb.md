...
If you are using an apex domain (`example.com`), see the additional information about [Apex domains and CNAME records](dns1.).

If you are planning to host multiple subdomains on different projects, see the additional information about [Subdomains](subdomains1.) *before* you add your domain to Web PaaS.
...
This step will tell the Web PaaS edge layer where to route requests for your web site. You can do this through the CLI with `webpaas domain:add example.com` or  [using the managment console](configure-project#domains.).
...
While not required, it's strongly recommended that you set up [health notifications](integrations-notifications1.) to advise you if your site is experiencing issues such as running low on disk space.  Notifications can be sent via email, Slack, or PagerDuty.
