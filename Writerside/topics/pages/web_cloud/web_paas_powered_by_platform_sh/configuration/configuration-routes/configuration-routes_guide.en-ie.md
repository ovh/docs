...
* `cache` controls [caching behavior of the route](cache1.).
* `ssi` controls whether Server Side Includes are enabled. For more information: see [SSI](ssi1.).
* `redirects` controls [redirect rules](redirects1.) associated with the route.

![Routes files](routes-configs.png "0.5")
...
In this example, we will route both the apex domain and the www subdomain to an [application called "app"](name1.), the www subdomain being redirected to the apex domain using an HTTP 301 `Moved Permanently` response.
...
If your project involves only a single apex domain with one app or multiple apps under subdomains, it's generally best to use the `{default}` placeholder.  If you are running [multiple applications](multi-app1.) on different apex domains then you will need to use a static domain for all but one of them.
...
![WebPaas Routes CLI](platform-routes-cli.png "0.3")
...
For your master environment, this would function as a catch-all domain once you [added the parent domain](configure-project#domains.) to the project settings.
...
You will also need to [disable request buffering](web#locations.) in the `.platform.app.yaml`.
