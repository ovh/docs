...
Set variables [in code](variables1.) using the `.platform.app.yaml` file.
...
Add secrets for all environments in project variables using [the management console](configure-project#variables.) or the CLI.
...
Set variables for specific environments using [the management console](configure-environment#variables.) or the CLI.
...
| PLATFORM_RELATIONSHIPS    | No    | Yes     | A base64-encoded JSON object of relationships. The keys are the relationship name and the values are arrays of relationship endpoint definitions. The exact format is defined for each [service](configuration-services1.). |
...
You can get a list of all variables defined on a given environment using either [the management console](configure-environment#variables.) or the CLI:
...
The following example shows nested structures introduced in an [app configuration](variables1.):
...
To apply a setting to all environments or to vary them between different PHP containers in the same project, specify the variables in the `.platform.app.yaml` file for your application. See the [PHP configuration page](ini1.) for more information.
