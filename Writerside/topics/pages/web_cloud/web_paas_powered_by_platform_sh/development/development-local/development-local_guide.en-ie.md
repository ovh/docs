...
You must have an [SSH key](development-tools#ssh.) already configured on your account, and have both [Git](development-tools#git.) and the [Web PaaS CLI](development-cli1.) installed before continuing.
...
Web PaaS supports whatever local development environment you wish to use.  There is no dependency on any particular tool so if you already have a local development workflow you're comfortable with you can keep using it without changes.  That's the "[untethered](untethered1.)" option.
