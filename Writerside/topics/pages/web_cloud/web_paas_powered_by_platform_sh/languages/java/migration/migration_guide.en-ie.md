...
> For more details, see [tuning](tuning1.).
> 

## Monolith/Single Application

To start a Java application, you need to understand the [Web PaaS structure](overview-structure1.).
At minimum, you to configure your [application](configuration-app1.)
and have two [YAML files](configuration-yaml1.) (though they can be blank if you don't need them):

* [Routes](configuration-routes1.)
* [Services](configuration-services1.)
...
1\. [A Java version](languages-java#supported-versions.), e,g.: `java:11`

2\. [Hooks define what happens when building the application](build#build.). This build process typically generates an executable file such as a uber-jar e.g.: `mvn clean package`

3\. [The commands key defines the command to launch the application](web#commands.). E.g.:  `java -jar file.jar`
...
> [!primary]  
> 
> Be aware that after the build, it creates a read-only system. You have the [mount option to create a writable folder](storage#mounts.).
> 
> 
...
* [Git submodules](development-submodules1.)

[Web PaaS supports multiple applications](multi-app1.) and there are two options:
...
[Web PaaS has services managed by Web PaaS itself such as database, cache and search engine](configuration-services1.). However, you can use a database or any services such as a transition process, just be aware of the [firewall](firewall1.).

When applications need to access a service, it is important to include the [Relationships key](relationships1.), because. by default an application may not talk to any other container within a project it includes others projects as a microservices architecture.
...
The services information is available in the [**PLATFORM_RELATIONSHIPS** environment variable](development-variables1.).
