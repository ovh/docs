...
You can also develop a microservice architecture mixing JavaScript and other apps with [multi-app projects](configuration-app1.).
...
To use a specific version in a container with a different language, [use a version manager](nvm1.).
...
Choose a version from the [list above](languages-nodejs_#supported-versions.)
and add it to your [app configuration](configuration-app1.):
...
If you have a `package.json` file in your code, the default [build flavor is run](build#build.):
...
The following examples show how to use Node.js to access various [services](configuration-services1.).
