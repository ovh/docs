...
- A [Web PaaS solution](web-paas.) in your OVHcloud account (find the details in [this guide](control-panel-creating-the-service1.))
...
![All projects](configure_manager01.png){.thumbnail}

Each project equals a service subscription. The button `New project`{.action} starts the process of ordering a [new project](control-panel-creating-the-service1.).
...
![General information](configure_manager02.png){.thumbnail}
...
Click on `Change solution`{.action} in order to choose a [different subscription](control-panel-creating-the-service1.). The only limitation is that you cannot downgrade to the `Start` plan.

![Change plan](configure_manager03.png){.thumbnail}
...
![Change CPU](configure_manager04.png){.thumbnail}
...
![Change storage](configure_manager05.png){.thumbnail}
...
![User licences](configure_manager06.png){.thumbnail}
...
![Change users](configure_manager07.png){.thumbnail}
