...
- Visiting the [product page](web-paas.) to decide which plan is suitable for your project's requirements

## Instructions

### Subscribing to the service

Log in to your [OVHcloud Control Panel](manager.) and switch to `Web Cloud`{.action} in the top navigation bar.

Select `Web PaaS`{.action} in the services bar on the left-hand side, then click on the button `New project`{.action}.

![order service](creation_manager01.png){.thumbnail}
...
Select a plan that best suits your needs after verifying the details and capabilities on the [product page](web-paas.). If necessary, increase the number of vCPUs in the drop-down menu of the chosen solution.
...
- If you click on `Select an existing template`{.action}, the list of available templates will be displayed along with their properties. You can check the [product page FAQ](web-paas.) to find out which templates are included in the respective plan. Select one and click on `Next`{.action}.

![project type](creation_manager04.png){.thumbnail}

#### Step 5: Configure your project

In this step you can choose additional resources for the project, according to the plan you have selected. Please refer to our [product page](web-paas.) for details about the available **Environments**, **Storage**, and **User licences** for each plan.

![configure details](creation_manager05.png){.thumbnail}
...
> [!primary]
> You can order the service from the [product page](web-paas.) as well.
>
...
![OVHcloud control panel](control_panel01.png){.thumbnail}
...
![console project](manage_console01.png){.thumbnail}

The project is ready when the status is set to `Active` on the Master environment.

![console project](manage_console02.png){.thumbnail}
...
- If you need to create a new key pair, we recommend to follow the steps in the [SSH guide](development-ssh1.).
...
[Configuring the CLI](development-cli1.)

[Using SSH](development-ssh1.)

[Using YAML](configuration-yaml1.)

[Managing Web PaaS from the OVHcloud Control Panel](control-panel-managing-the-service1.)
