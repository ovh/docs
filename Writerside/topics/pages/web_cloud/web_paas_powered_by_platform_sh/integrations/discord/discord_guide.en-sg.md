...
To use it, paste it as-is into a `.js` file and then add it as a new [script integration](integrations-activity#installing.).  Be sure to specify which events it should trigger on using the `--events` switch, and if desired which `--environments` you want.
...
Third, add that URL to your project as a [variable](development-variables1.) named `DISCORD_URL`.
