...
- Une des bases de données incluses avec un [hébergement web](hosting.).
- Une [instance Web Cloud Databases](databases.){.external} (incluse dans une offre d'[hébergement web Performance](hosting.)).
- Un pack de base de données [Start SQL](hosting-options-startsql.).
...
- Les services Web Cloud Databases, instances SGBD dédiées joignables via le réseau public ou via le réseau Web Hosting ([voir les annonces](clouddb-eos-eol1.)).

- Les services SharedSQL Web Hosting, bases de données MySQL joignables via le réseau Web Hosting ([voir les annonces](sql_eos_eol1.)).
