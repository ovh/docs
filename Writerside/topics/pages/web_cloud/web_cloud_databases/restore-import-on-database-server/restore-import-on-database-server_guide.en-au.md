...
- A [Web Cloud Databases instance](https://www.ovh.ie/cloud-databases/) (included in a [Performance web hosting plan](hosting.)) in your OVHcloud account
...
Log in to your [OVHcloud Control Panel](manager.) and select `Web Cloud`{.action} in the top navigation bar. Click `Web Cloud Databases`{.action}, then choose the database name concerned. Next, switch to the `Databases` tab.
...
This action is only possible via [SSH](ssh_on_webhosting1.) from an OVHcloud Web Hosting plan.
...
> [!warning]
>
> - To prevent someone from accessing this file containing sensitive data, you can [secure access to it](htaccess_protect_directory_by_password1.).
...
This action is only possible via [SSH](ssh_on_webhosting1.) from an OVHcloud Web Hosting plan, in stable or higher versions.
