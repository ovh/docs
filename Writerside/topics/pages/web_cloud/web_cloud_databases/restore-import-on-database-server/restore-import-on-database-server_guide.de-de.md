...
- Sie verfügen über eine [Web Cloud Databases Instanz](https://www.ovh.de/cloud-databases/) (auch in einem [Performance Webhosting](hosting.) Angebot enthalten)
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web Cloud`{.action} auf `Web Cloud Databases`{.action}. Wählen Sie den Datenbanknamen aus und gehen Sie dann auf den Tab `Datenbanken`{.action}.
...
Um Ihre Datenbank direkt über phpMyAdmin zu importieren, müssen Sie sich zuerst mit ihr verbinden. Folgen Sie hierzu den Schritten im Abschnitt ["Mit einer MySQL oder MariaDB Datenbank verbinden"](connecting-to-database-on-database-server#mit-einer-mysql-oder-mariadb-datenbank-verbinden.).
...
Diese Aktion ist nur über [SSH](ssh_on_webhosting1.) von einem [OVHcloud Webhosting](hosting.) aus möglich.
...
> [!warning]
>
> - Um zu vermeiden, dass jemand auf diese Datei mit sensiblen Daten zugreift, können Sie sie [entsprechend absichern](htaccess_protect_directory_by_password1.).
> - Diese Aktion ist nur von einem [OVHcloud Webhosting](hosting.) aus möglich.
>
...
Diese Aktion ist nur über [SSH](ssh_on_webhosting1.) von einem [OVHcloud Webhosting](hosting.) aus möglich (ab Version "stable").
