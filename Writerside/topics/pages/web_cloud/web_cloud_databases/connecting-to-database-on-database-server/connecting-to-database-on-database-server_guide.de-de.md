...
- Sie verfügen über eine [Web Cloud Databases Instanz](databases.){.external} (in einem [Performance Webhosting](hosting.) Angebot enthalten)
...
> [!primary]
>
> Beachten Sie, dass die [Web Cloud Databases Dienste](databases.) keinen Zugriff auf den Datenbankserver gewähren, sondern auf die darauf gehosteten Datenbanken. 
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web Cloud`{.action} links auf `Web Cloud Databases`{.action}. Wählen Sie den Namen Ihres Datenbankservers aus.
...
> [!warning]
>
> Wenn Sie Web Cloud Databases oder SQL Private für Datenbanken einsetzen, vergessen Sie nicht, Ihrer IP-Adresse mithilfe der Anleitung [Konfiguration Ihres Datenbankservers](configure-database-server1.) den Zugriff zu erlauben.
>
...
![config](add-alias.png){.thumbnail}
...
![config](general-dashboard.png){.thumbnail}
