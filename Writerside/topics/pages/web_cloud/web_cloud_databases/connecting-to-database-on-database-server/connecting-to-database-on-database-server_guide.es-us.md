...
- Tener una [instancia Web Cloud Databases](databases.) (incluida en un plan de [hosting Performance](hosting.))
...
> [!primary]
>
> Tenga en cuenta que las soluciones [Web Cloud Databases](databases.){.external} no permiten acceder al sistema de gestión de bases de datos, sino a las bases de datos alojadas en él.
...
Conéctese a su [área de cliente de OVHcloud](manager.){.external}  y acceda a la sección `Web Cloud`. En la columna izquierda, haga clic en la pestaña `Web Cloud Databases`{.action} y seleccione el nombre del servidor de bases de datos.
...
> [!warning]
>
> Si utiliza una solución Web Cloud Databases/ SQL Privado, no olvide autorizar su IP utilizando la guía sobre la [configuración de su servidor de bases de datos](configure-database-server1.).
>
...
![](aliases.png){.thumbnail} ejecutar SQuirreL SQL
...
![](add-alias.png){.thumbnail} conexión a la base de datos

- Confirme con el botón `{.action}Connect.

![](connect-to-mysql.png){.thumbnail} confirmación de la conexión

Se establecerá la conexión a la base de datos:

![](general-dashboard.png){.thumbnail} conexión a la base de datos
...
> [!primary]
>
> Para un servidor SQL privado, esta acción solo es posible por [SSH](ssh_on_webhosting1.){.external} desde un alojamiento compartido de OVHcloud.
