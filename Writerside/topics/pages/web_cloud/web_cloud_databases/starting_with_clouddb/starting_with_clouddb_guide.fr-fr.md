...
Si votre offre d'hébergement inclut l'option Web Cloud Databases, rendez-vous dans votre [espace client OVHcloud](manager.){.external}. Depuis la section `Web Cloud`{.action}, cliquez sur `Hébergements`{.action} dans la colonne de gauche.
...
Dans la barre de services à gauche de votre [espace client OVHcloud](manager.){.external}, rendez-vous dans la section `Web Cloud Databases`{.action}, puis sur l'instance concernée. Assurez-vous d'être bien positionné sur l'onglet `Informations générales`{.action}.
...
![Web Cloud Databases](database-import-new-file-step-1.png){.thumbnail}
...
[Créer vos bases de données et vos utilisateurs sur votre serveur de bases de données](create-db-and-user-on-db-server1.){.external}

[Se connecter à la base de données de votre serveur de bases de données](connecting-to-database-on-database-server1.){.external}
...
[Restaurer et importer une base de données sur votre serveur de bases de données](restore-import-on-database-server1.){.external}

[Configurer votre serveur de bases de données](configure-database-server1.){.external}
