...
- A [Web Cloud Databases instance](https://www.ovh.co.uk/cloud/cloud-databases/) (included in [Performance web hosting plans](hosting.)).
...
If your hosting plan includes the Web Cloud Databases option, go to the [OVHcloud Control Panel](manager.){.external}. In the `Web Cloud`{.action} section, click `Hosting plans`{.action} in the left-hand column.
...
![Informations générales](web-cloud-databases-enable.png){.thumbnail}
...
In the services bar on the left-hand side of the [OVHcloud Control Panel](manager.){.external}, go to the `Web Cloud Databases`{.action} section, then to the instance. Click on the `General information`{.action} tab.
...
To access the logs for your Web Cloud Databases solution, please refer to our guide "[Web Cloud Databases - How to retrieve logs](retrieve-logs1.)".
