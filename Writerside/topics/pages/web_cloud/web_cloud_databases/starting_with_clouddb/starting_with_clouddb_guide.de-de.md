...
- Sie verfügen über eine [Web Cloud Databases Instanz](https://www.ovh.de/cloud/cloud-databases/) (im [Webhosting Performance](hosting.)).
...
Wenn Ihr Hosting-Angebot die Option "Web Cloud Databases" beinhaltet, können Sie diese in Ihrem [OVHcloud Kundencenter](manager.){.external} aktivieren. Klicken Sie im Bereich `WebCloud`{.action} links auf `Hosting-Pakete`{.action}.
...
![Allgemeine Informationen](web-cloud-databases-enable.png){.thumbnail}
...
Gehen Sie In Ihrem [OVHcloud Kundencenter](manager.){.external} im linken Menü in den Bereich `Web Cloud Databases`{.action} und klicken Sie anschließend auf die entsprechende Instanz. Achten Sie darauf, dass Sie sich in dem Tab `Allgemeine Informationen`{.action} befinden.
