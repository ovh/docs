...
>
> Pour délier une offre Web Cloud Databases associée à un hébergement web **Performance**, rendez-vous dans votre [espace client OVHcloud](manager.). Cliquez sur l'onglet `Web Cloud` puis sur l'hébergement concerné dans l'onglet `Hébergements`{.action} qui apparaît dans la colonne de gauche. 
>
...
Pour modifier votre offre Web Cloud Databases, rendez-vous dans votre [espace client OVHcloud](manager.). Cliquez sur l'onglet `Web Cloud`{.action}, puis sur `Web Cloud Databases`{.action}. Sélectionnez le nom de votre serveur de bases de données.
