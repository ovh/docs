...
- Haber consultado la [guía de inicio de Web Cloud Databases](starting_with_clouddb1.).
...
Puede encontrar todos estos datos en el [área de cliente OVHcloud](manager.){.external}.

Para más información, puede consultar la guía [Primeros pasos con el servicio Cloud Databases](starting_with_clouddb1.).
...
![ejecutar SQuirreL SQL](images_aliases.png){.thumbnail}
...
![configuración de la conexión](images_add-alias.png){.thumbnail}

- Confirme con el botón `Connect`{.action}.

![confirmación de la conexión](images_connect-to-mysql.png){.thumbnail}

Se establecerá la conexión a la base de datos:

![conexión a la base de datos](images_general-dashboard.png){.thumbnail}
...
> [!primary]
>
> En algunos casos, la RAM disponible en la instancia Cloud Databases puede no permitir la exportación o importación deseadas. En ese caso, le recomendamos que utilice la herramienta OVHcloud en el área de cliente. Si lo necesita, consulte la guía ["Primeros pasos con el servicio Cloud Databases"](starting_with_clouddb1.).
>
