...
- Essere aggiornato nei [pagamenti](invoice_management#pay-bills.) di questo servizio e del dominio associato
...
![Diagnostica Exchange](img_4450.png){.thumbnail}
...
![Diagnostica Exchange](img_4451.png){.thumbnail}

La procedura di diagnostica richiederà circa 3-10 minuti. Ecco un esempio di risultati:

![Diagnostica Exchange](img_4471.png){.thumbnail}
...
Consulta la nostra guida [Cosa fare in caso di account bloccato per Spam?](locked_for_spam1.) per consentire ai nostri team di sicurezza di riattivare l'account.
...
Per riattivare il tuo abbonamento, è sufficiente riconfigurarne la [periodicità di fatturazione](manage_billing_exchange#periodicity.) nello [Spazio Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.it/&ovhSubsidiary=it).
...
Per maggiori informazioni su questa funzionalità, consulta la nostra [guida sulla politica di sicurezza](security-policy1.).

### L'autenticazione alla Webmail è fallita <a name="password"></a>

Questa operazione può essere dovuta all'inserimento di una password dell'account non corretta. Verifica che la password sia corretta, accedendo alla [Webmail](email_owa1.) e riavvia la diagnostica.
...
Puoi verificare queste impostazioni nella [zona DNS del tuo dominio](dns_zone_edit1.).
