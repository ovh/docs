...
- **DKIM**: Setting up an encrypted digital signature to secure email exchanges. Refer to our guide on [Configuring a DKIM record](dns_zone_dkim1.) for more information.
...
- **Manual configuration for a non-OVHcloud domain name**: Click on the red box to view the changes you will need to make.
    - *For a CNAME* record, you can use our guide on "[Creating a CNAME record when adding an associated domain](exchange_dns_cname1.)".
    - *For an MX* record, please refer to our guide on "[Adding an MX record to your domain name’s configuration](dns_zone_mx1.)".
    - *For an SRV* record, modify your DNS zone using the information provided when you click on the box. You can use our guide on "[Editing an OVHcloud DNS zone](dns_zone_edit1.)" to add this record.
    - *For an SPF* record, modify your DNS zone using the information provided when you click on the box. You can use our guide on "[Configuring an SPF record](dns_zone_spf1.)" to add this record.
    *For a DKIM* record, modify your DNS zone using the information provided when you click on the box. You can refer to our guide on "[Configuring a DKIM record](dns_zone_dkim1.)" to add this record.
...
If you would like to configure your email address on an email client or device (e.g. a smartphone or tablet), you can refer to our [configuration guides](web-cloud-email-collaborative-solutions-email-pro1.). If you simply need the information required to configure your Email Pro account, the settings to use are listed below:
...
[Creating inbox rules in OWA](creating-inbox-rules-in-owa-mx-plan1.)

[Adding an alias to your email account](feature_redirections1.)

[Creating automatic replies in OWA](owa_automatic_replies1.)

[Managing the billing for your Email Pro accounts](manage_billing_emailpro1.)

[Managing the security policy of an email service](security-policy1.)
