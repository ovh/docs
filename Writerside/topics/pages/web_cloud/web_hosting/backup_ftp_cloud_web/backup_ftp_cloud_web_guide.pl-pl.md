...
> 
> Wykonując kopię zapasową bezpieczeństwa Twojej strony WWW i korzystając z bazy danych, wykonaj również jej kopię zapasową. Aby [pobrać kopię zapasową bazy danych](sql_database_export1.), zapoznaj się z naszym przewodnikiem.
> 
...
- Hosting [Cloud Web](hosting-cloud-web-offer.){.external}
...
W przeciwieństwie do hostingu współdzielonego OVHcloud, przywracanie przestrzeni FTP nie jest możliwe za pomocą jednego kliknięcia w [Panelu klienta OVHcloud](manager.){.external}.
...
Aby wygenerować link do pobrania, zaloguj się do [Panelu klienta OVHcloud](manager.){.external}, przejdź do sekcji `Web Cloud`{.action}, kliknij `Hosting`{.action}, a następnie odpowiednią usługę Cloud Web. 

Wybierz kartę `FTP - SSH`{.action} i kliknij przycisk `Wygeneruj kopię zapasową`{.action} po prawej stronie.

![kopia zapasowa](generate-a-backup.png){.thumbnail}

W oknie, które się otworzy wybierz jedną z dostępnych kopii zapasowych, po czym kliknij `Dalej`{.action}.

![kopia zapasowa](generate-a-backup-step-1.png){.thumbnail}
...
![kopia zapasowa](generate-a-backup-step-2.png){.thumbnail}

Kliknij na `Zatwierdź`{.action}, aby potwierdzić zlecenie.

Jeśli wygenerowana zostanie kopia zapasowa, w [Panelu klienta OVHcloud](manager.){.external} pojawi się następujący komunikat:

![kopia zapasowa](message-backup-progress.png){.thumbnail}
...
![kopia zapasowa](backup-information.png){.thumbnail}

Pobrany plik jest w formacie *.tar.gz*.

### Przywróć kopię zapasową

Po pobraniu plików możesz [połączyć się z przestrzenią FTP](ftp_connection1.) przy użyciu programu FTP, takiego jak [Filezilla](ftp_filezilla_user_guide1.), a następnie wymienić wybrane pliki na pobrane pliki.

> [!primary]
>
> Korzystaj z portów podanych w [Panelu klienta OVHcloud](manager.){.external} do połączenia SFTP i SSH, ponieważ port 22 nie będzie działał dla Twojego hostingu Cloud Web.
>
...
[Logowanie za pomocą programu Filezilla](ftp_filezilla_user_guide1.)

Skontaktuj się z [partnerami OVHcloud](partner.), jeśli szukasz zaawansowanych rozwiązań (indeksowanie, rozwój, etc).

Jeśli chcesz otrzymywać wsparcie w zakresie konfiguracji i korzystania z rozwiązań OVHcloud, sprawdź naszą [ofertę wsparcia](support.).
