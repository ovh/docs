...
> 
> Ponemos a su disposición este tutorial para ayudarle lo mejor posible en tareas habituales. No obstante, si necesita ayuda, le recomendamos que contacte con un [proveedor especializado](partner.) o con el editor del CMS que haya elegido instalar. Nosotros no podremos asistirle. Más información en la sección ["Más información"](cms_manual_installation_#go-further.) de este tutorial.
>
...
> [!success]
>
> Para instalar su CMS **automáticamente** desde su [área de cliente de OVHcloud](manager.), consulte nuestra documentación sobre la instalación de un módulo en un clic](/pages/web_cloud/web_hosting/cms_install_1_click_modules).
>
...
Más información en nuestra página relativa a [módulo WordPress](hosting-wordpress.)
...
Más información en nuestra página relativa a [Joomla!](hosting-joomla.)
...
Más información en nuestra página relativa a [Drupal](hosting-drupal.)
...
Más información en nuestra página relativa a [PrestaShop](hosting-prestashop.)
...
Para instalar un CMS en su [plan de hosting](hosting.), es necesario realizar algunos preparativos.
...
Consulte nuestra guía sobre [cómo añadir un multisitio a un alojamiento web](multisites_configure_multisite1.) para determinar la carpeta raíz que debe utilizar con su CMS.
...
- Asegúrese de que el dominio que utilizará para acceder al CMS y el subdominio en "www" apuntan a la dirección IP de su [plan de hosting](hosting.).

Para obtener la dirección IP de su plan de hosting, conéctese a su [área de cliente de OVHcloud](manager.) en la sección `Web Cloud`{.action} y seleccione su plan de hosting en la sección `Alojamientos`{.action}.<br>
...
Si la zona DNS activa de su dominio está gestionada en su [área de cliente de OVHcloud](manager.), compare la dirección IP de su alojamiento con la presente en la zona DNS de su dominio, ayudándole a nuestra documentación sobre las [zonas DNS de OVHcloud](dns_zone_edit1.).

> [!warning]
>
> Si ha activado las opciones `CDN`{.action} o `IP del país`{.action} con su dominio, utilice la dirección IP adecuada con nuestra documentación que recoge [todas las direcciones IP de nuestros alojamientos compartidos](clusters_and_shared_hosting_IP1.).
...
- Obtenga [la información necesaria para conectarse al espacio FTP de su alojamiento web](ftp_connection#1-obtener-los-datos-de-conexion.).
- Consulte los accesos a la base de datos de su plan de hosting, si ya existe, o cree una con ayuda de [documentación](sql_create_database1.).

#### 1.3 - Instalar el cliente FTP gratuito "FileZilla"

Si todavía no utiliza un cliente FTP, puede utilizar Filezilla. Consulte la página de descarga gratuita y el tutorial sobre su uso en nuestra documentación sobre el [uso de FileZilla en el alojamiento de OVHcloud](ftp_filezilla_user_guide1.).

> [!primary]
>
> Existen otros clientes FTP que puede utilizar, por ejemplo, Cyberduck. Para más información, consulte nuestra guía relativa al [uso de Cyberduck con el plan de hosting de OVHcloud](ftp_cyberduck_user_guide_on_mac1.).
>
...
La mayoría de los CMS necesitan una base de datos para funcionar. Nuestros [planes de hosting](hosting.) contienen, a excepción de [Alojamiento gratuito 100M](domains-free-hosting.).

Utilice nuestra guía para [crear una base de datos desde su plan de hosting](sql_create_database1.).

Si tiene un plan Web Cloud Databases en MySQL o MariaDB y quiere utilizarlo para instalar manualmente su CMS, consulte nuestra documentación sobre la [creación de una base de datos en un servicio Web Cloud Databases](create-db-and-user-on-db-server1.).

Una vez que haya creado la base de datos, deberá descargar los parámetros de conexión (servidor, nombre de la base de datos, nombre de usuario y contraseña) y conservarlos para [el etapa 3](cms_manual_installation_#step3.) de esta guía.
...
>
> Si también se trata de un CMS idéntico al que tiene que instalar, puede utilizar [esta guía](sql_change_password#etapa-3-cambiar-la-contrasena-de-la-base-de-datos-del-sitio-web-en-el-archivo-de-configuracion.) para identificar los archivos de configuración en su [espacio de almacenamiento FTP](ftp_connection1.).
>
...
>
> - Para conectarse a la base de datos asociada a su plan de hosting, consulte [esta guía](sql_create_database#acceder-a-la-interfaz-phpmyadmin.).
> - Para conectarse a una base de datos en un databases de Web Cloud, consulte [esta guía](connecting-to-database-on-database-server1.).
>
...
>
>
> Configure la versión de PHP en su alojamiento web con nuestra guía sobre [el cambio de versión PHP de un alojamiento web](configure_your_web_hosting1.).
>
...
Una vez descomprimidos los archivos en su carpeta "**CMS**", [conéctese por FTP al espacio de almacenamiento](ftp_connection1.) utilizando el [cliente FTP FileZilla](ftp_filezilla_user_guide1.) y, a continuación, copie los archivos contenidos en la carpeta "**CMS**" en la "carpeta raíz" que haya establecido en su alojamiento durante el [etapa 1](cms_manual_installation_#step1.) de esta guía.
...
> [!primary]
>
Si la "carpeta raíz" que ha definido no se ha creado automáticamente en las acciones descritas en el [etapa 1](cms_manual_installation_#step1.), puede crearla a través de FileZilla.
>
...
Para conectarse a su alojamiento por SSH, consulte nuestra guía sobre la [conexión por SSH desde un alojamiento compartido de OVHcloud](ssh_on_webhosting1.).
...
- [Finalizar la instalación de WordPress](cms_manual_installation_wordpress1.)
- [Finalizar la instalación de Joomla!](cms_manual_installation_joomla1.)
- [Finalizar la instalación de Drupal](cms_manual_installation_drupal1.)
- [Finalizar la instalación de PrestaShop](cms_manual_installation_prestashop1.)
- [Finalizar la instalación de Pico](cms_manual_installation_pico1.)
- [Finalizar la instalación de Grav](cms_manual_installation_grav1.)
- [Finalizar la instalación de Typo3](cms_manual_installation_typo31.)
- [Finalizar la instalación de SPIP](cms_manual_installation_spip1.)

## Más información <a name="go-further"></a>

[Migración de su sitio web y de su correo a OVHcloud](hosting_migrating_to_ovh1.)

[Publicar un sitio web en internet en un alojamiento web](hosting_how_to_get_my_website_online1.)
