...
> 
> Nous mettons à votre disposition ce tutoriel afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) ou l'éditeur du CMS ue vous aurez choisi d'installer si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](cms_manual_installation_#go-further.) de ce tutoriel.
>
...
> [!success]
>
> Pour installer votre CMS **automatiquement** depuis votre [espace client OVHcloud](manager.), consultez notre documentation sur l'[installation d'un module « en un clic »](cms_install_1_click_modules1.).
>
...
Retrouvez plus d'informations sur notre page relative au [module WordPress](hosting-wordpress.)
...
Retrouvez plus d'informations sur notre page relative au [module Joomla!](hosting-joomla.)
...
Retrouvez plus d'informations sur notre page relative au [module Drupal](hosting-drupal.)
...
Retrouvez plus d'informations sur notre page relative au [module PrestaShop](hosting-prestashop.)
...
Pour installer un CMS sur votre offre d'[hébergement web](hosting.), quelques préparatifs sont nécessaires.
...
Consultez notre documentation qui décrit [comment ajouter un multisite sur son hébergement web](multisites_configure_multisite1.) pour définir le dossier racine à utiliser avec votre CMS.
...
- Assurez-vous que le nom de domaine que vous utiliserez pour accéder à votre CMS, ainsi que son sous-domaine en « www », pointent bien vers l'adresse IP de votre offre d'[hébergement web](hosting.).

Pour récupérer l'adresse IP de votre offre d'hébergement web, connectez-vous à votre [espace client OVHcloud](manager.) dans la partie `Web Cloud`{.action} puis sélectionnez votre offre d'hébergement web dans la section `Hébergements`{.action}.<br>
...
Si la zone DNS active de votre domaine est gérée dans votre [espace client OVHcloud](manager.), comparez l'adresse IP de votre hébergement avec celle présente dans la zone DNS de votre domaine, en vous aidant de notre documentation sur les [zones DNS OVHcloud](dns_zone_edit1.).

> [!warning]
>
> Si vous avez activé les options `CDN`{.action} ou `IP du pays`{.action} avec votre domaine, utilisez l'adresse IP adaptée en vous aidant de notre documentation recensant [l'ensemble des adresses IP de nos hébergements mutualisés](clusters_and_shared_hosting_IP1.).
>
...
- Récupérez [les informations nécessaires pour vous connecter à l'espace FTP de votre hébergement web](ftp_connection#etape-1-recuperer-les-informations-necessaires-pour-se-connecter.).
- Récupérez les accès à la base de données de votre offre d'hébergement web si elle existe déjà, ou créez-en une à l'aide de notre [documentation](sql_create_database1.).

#### 1.3 - Installer le client FTP gratuit « FileZilla »

Si vous n'utilisez pas déjà un client FTP, vous pouvez utiliser Filezilla. Retrouvez le lien de téléchargement gratuit ainsi qu'un tutoriel sur son utilisation dans notre documentation sur l'[utilisation de FileZilla avec votre offre d'hébergement OVHcloud](ftp_filezilla_user_guide1.).

> [!primary]
>
> Il existe d'autres clients FTP que vous pouvez utiliser comme, par exemple, Cyberduck. Retrouvez plus d'informations dans notre documentation relative à l'[utilisation de Cyberduck avec votre offre d'hébergement OVHcloud](ftp_cyberduck_user_guide_on_mac1.).
>
...
La plupart des CMS ont besoin d'une base de données pour fonctionner. Nos offres d'[hébergement web](hosting.) en contiennent.

Les CMS ont besoin d'une base de données pour fonctionner. Nos offres d'[hébergement web](hosting.) en contiennent.

Utilisez notre documentation pour [créer une base de données depuis votre offre d'hébergement web](sql_create_database1.).

Si vous disposez d'une offre Web Cloud Databases en MySQL ou MariaDB et que vous souhaitez l'utiliser pour installer manuellement votre CMS, consultez notre documentation sur la [création d'une base de données sur un service Web Cloud Databases](create-db-and-user-on-db-server#creer-une-base-de-donnees.).

Une fois la base de données créée, récupérez les paramètres de connexion (serveur, nom de la base de données, nom d'utilisateur et mot de passe) et conservez-les pour [l'étape 3](cms_manual_installation_#step3.) de ce guide.
...
>
> - Pour vous connecter à votre base de données liée à votre offre d'hébergement web, consultez [ce guide](sql_create_database#acceder-a-linterface-phpmyadmin.).
> - Pour vous connecter à une base de données présente sur un Web Cloud Databases, consultez [ce guide](connecting-to-database-on-database-server1.).
>
...
>
> Configurez ensuite la version de PHP sur votre hébergement web en vous aidant de notre documentation sur [le changement de version PHP d'un hébergement web](configure_your_web_hosting1.).
>
...
Une fois les fichiers décompressés dans votre dossier « **CMS** », [connectez-vous en FTP à votre espace de stockage](ftp_connection1.) à l'aide du [client FTP FileZilla](ftp_filezilla_user_guide1.) puis copiez les fichiers contenus dans le dossier « **CMS** » dans le « dossier racine » que vous avez défini sur votre hébergement lors de l'[étape 1](cms_manual_installation_#step1.) de ce guide.
...
>[!primary]
>
> Si le « dossier racine » que vous avez défini n'a pas été créé automatiquement lors des actions décrites dans l'[étape 1](cms_manual_installation_#step1.), vous pouvez le créer via FileZilla.
>
...
Pour vous connecter en SSH à votre hébergement, consultez notre guide sur la [connexion en SSH depuis un hébergement mutualisé OVHcloud](ssh_on_webhosting1.).
...
- [Finaliser l'installation de Wordpress](cms_manual_installation_wordpress1.)
- [Finaliser l'installation de Joomla!](cms_manual_installation_joomla1.)
- [Finaliser l'installation de Drupal](cms_manual_installation_drupal1.)
- [Finaliser l'installation de PrestaShop](cms_manual_installation_prestashop1.)
- [Finaliser l'installation de Pico](cms_manual_installation_pico1.)
- [Finaliser l'installation de Grav](cms_manual_installation_grav1.)
- [Finaliser l'installation de Typo3](cms_manual_installation_typo31.)
- [Finaliser l'installation de SPIP](cms_manual_installation_spip1.)

## Aller plus loin <a name="go-further"></a>

[Migration de votre site web et de vos e-mails vers OVHcloud](hosting_migrating_to_ovh1.)

[Mettre en ligne un site internet sur son hébergement web](hosting_how_to_get_my_website_online1.)
