...
> 
> Mettiamo a tua disposizione questo tutorial per supportarti nelle operazioni più frequenti. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a uno [specialista del settore](partner.) o all'amministratore del CMS che hai scelto di installare. OVHcloud non potrà fornirti alcuna assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](cms_manual_installation_#go-further.) di questo tutorial.
>
...
> [!success]
>
> Per installare il tuo CMS **automaticamente** dal tuo [Spazio Cliente OVHcloud](manager.), consulta la nostra guida sull'[installazione di un modulo "in un click"](cms_install_1_click_modules1.).
>
...
Per maggiori informazioni, consulta la nostra pagina relativa al [modulo WordPress](hosting-wordpress.)
...
Per maggiori informazioni, consulta la nostra pagina relativa a [modulo Joomla!](hosting-joomla.)
...
Per maggiori informazioni, consulta la pagina relativa al [modulo Drupal](hosting-drupal.)
...
Per maggiori informazioni, consulta la nostra pagina relativa al [modulo PrestaShop](hosting-prestashop.)
...
Per installare un CMS sulla tua offerta di [hosting Web](hosting.), sono necessari alcuni preparativi.
...
Consulta la nostra documentazione che descrive [come aggiungere un multisito su un hosting Web](multisites_configure_multisite1.) per definire la cartella di root da utilizzare con il tuo CMS.
...
- Assicurati che il dominio che utilizzerai per accedere al tuo CMS e il sottodominio in "www" puntino verso l'indirizzo IP della tua offerta di [hosting web](hosting.).

Accedi allo [Spazio Cliente OVHcloud](manager.) nella sezione `Web Cloud`{.action} e seleziona la tua soluzione di hosting Web nella sezione `Hosting`{.action}.<br>
...
Se la zona DNS attiva del tuo dominio è gestita dallo [Spazio Cliente OVHcloud](manager.), compara l'indirizzo IP del tuo hosting con quello presente nella zona DNS del tuo dominio, consultando la nostra documentazione sulle [zone DNS OVHcloud](dns_zone_edit1.).

> [!warning]
>
> Se hai attivato le opzioni `CDN`{.action} o `IP del paese`{.action} con il tuo dominio, utilizza l'indirizzo IP adattato seguendo la procedura descritta nella guida che contiene [tutti gli indirizzi IP dei nostri hosting condivisi](clusters_and_shared_hosting_IP1.).
>
...
- Recupera [le informazioni necessarie per accedere allo spazio FTP del tuo hosting web](ftp_connection#step-1-recupera-i-dati-necessari-a-effettuare-laccesso.).
- Recupera gli accessi al database della tua offerta di hosting Web se esiste già, o creane una utilizzando la nostra [guida](sql_create_database1.).

#### 1.3 - Installare il client FTP gratuito "FileZilla"

Se non utilizzi già un client FTP, puoi utilizzare Filezilla. Per maggiori informazioni, consulta la guida scaricare gratuitamente il link di download e una guida all'utilizzo nella nostra guida su [utilizzare FileZilla con la tua soluzione di hosting OVHcloud](ftp_filezilla_user_guide1.).

> [!primary]
>
> Ci sono altri client FTP che puoi utilizzare come, ad esempio, Cyberduck. Per maggiori informazioni, consulta la nostra documentazione relativa all'[utilizzo di Cyberduck con la tua soluzione di hosting OVHcloud](ftp_cyberduck_user_guide_on_mac1.).
>
...
Per funzionare correttamente, la maggior parte dei CMS ha bisogno di un database. Le nostre offerte di [hosting web](hosting.) ne contengono, ad eccezione di [Hosting gratuito 100M](domains-free-hosting.).

Utilizza la nostra documentazione per [creare un database dalla tua offerta di hosting web](sql_create_database1.).

Se disponi di un'offerta Web Cloud Databases in MySQL o MariaDB e desideri utilizzarla per installare manualmente il tuo CMS, consulta la nostra guida sulla [creazione di un database su un servizio Web Cloud Databases](create-db-and-user-on-db-server#crea-un-database.).

Una volta creato il database, recupera i parametri di connessione (server, nome del database, nome utente e password) e conservali per [step 3](cms_manual_installation_#step3.) di questa guida.
...
>
> Se il CMS è identico a quello da installare, puoi utilizzare [questa guida](sql_change_password#step-3-modifica-la-password-del-database-del-tuo-sito-nel-file-di-configurazione.) per identificare i file di configurazione nel tuo [spazio di storage FTP](ftp_connection1.).
>
...
>
> - Per connetterti al tuo database associato alla tua offerta di hosting Web, consulta [questa guida](sql_create_database#accedi-allinterfaccia-phpmyadmin.).
...
>
> Configura la versione di PHP sul tuo hosting Web consultando la nostra guida su [modifica della versione PHP di un hosting Web](configure_your_web_hosting1.).
>
...
Una volta che i file decomprimono la cartella "**CMS**", [collegati in FTP al tuo spazio di archiviazione](ftp_connection1.) con l'aiuto del [client FTP FileZilla](ftp_filezilla_user_guide1.) e copia i file contenuti nella cartella "**CMS**" nella "cartella root" definita sul tuo hosting durante lo [step 1](cms_manual_installation_#step1.) di questa guida.
...
> [!primary]
>
> Se la cartella root definita non è stata creata automaticamente durante le azioni descritte nello [step 1](cms_manual_installation_#step1.), puoi crearla via FileZilla.
>
...
Per connetterti in SSH al tuo hosting, consulta la nostra guida sulla [connessione in SSH da un hosting condiviso OVHcloud](ssh_on_webhosting1.).
...
- [Completare l'installazione di Wordpress](cms_manual_installation_wordpress1.)
- [Completare l'installazione di Joomla!](cms_manual_installation_joomla1.)
- [Completare l'installazione di Drupal](cms_manual_installation_drupal1.)
- [Completare l'installazione di PrestaShop](cms_manual_installation_prestashop1.)
- [Completare l'installazione di Pico](cms_manual_installation_pico1.)
- [Completare l'installazione di Grav](cms_manual_installation_grav1.)
- [Completare l'installazione di Typo3](cms_manual_installation_typo31.)
- [Completare l'installazione di SPIP](cms_manual_installation_spip1.)

## Per saperne di più <a name="go-further"></a>

[Migrazione del tuo sito Web e delle tue email verso OVHcloud](hosting_migrating_to_ovh1.)

[Pubblicare un sito Internet su un hosting Web](hosting_how_to_get_my_website_online1.)
