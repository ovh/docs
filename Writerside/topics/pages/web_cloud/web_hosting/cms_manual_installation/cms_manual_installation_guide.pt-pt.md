...
> 
> Colocamos à sua disposição este tutorial para o acompanhar o melhor possível em tarefas comuns. No entanto, se precisar de ajuda, recomendamos que recorra a um [fornecedor especializado](partner.) ou ao editor do CMS. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção [Quer saber mais?](cms_manual_installation_#go-further.) deste tutorial.
>
...
> [!success]
>
> Para instalar o seu CMS **automaticamente** a partir do seu [Área de Cliente OVHcloud](manager.), consulte o nosso manual sobre a [instalação de um módulo "num clique"](cms_install_1_click_modules1.).
>
...
Encontre mais informações na nossa página sobre o [módulo WordPress](hosting-wordpress.)
...
Encontre mais informações na nossa página sobre o [módulo Joomla!](hosting-joomla.)
...
Encontre mais informações na nossa página sobre o [módulo Drupal](hosting-drupal.)
...
Encontre mais informações na nossa página sobre o [módulo PrestaShop](hosting-prestashop.)
...
Para instalar um CMS na sua oferta de[alojamento web](hosting.), é necessário efetuar alguns preparativos.
...
Consulte o nosso manual que descreve [como adicionar um multi-site ao seu alojamento web](multisites_configure_multisite1.) para definir a pasta raiz a utilizar com o seu CMS.
...
- Certifique-se de que o domínio que utiliza para aceder ao seu CMS, bem como o subdomínio "www" apontam para o endereço IP do seu serviço de [alojamento web](hosting.).

Para obter o endereço IP da sua oferta de alojamento web, aceda à [Área de Cliente OVHcloud](manager.) na parte `Web Cloud`{.action} e selecione a sua oferta de alojamento web na secção `Alojamentos`{.action}.<br>
Na caixa `Informações gerais`{.action} à direita, encontrará o endereço IP do seu alojamento web no formulário `IPv4`{.action}.

Se a zona DNS ativa do seu domínio for gerida no seu [Área de Cliente OVHcloud](manager.), compare o endereço IP do seu alojamento com o endereço IP presente na zona DNS do seu domínio, através da nossa documentação sobre as [zonas DNS da OVHcloud](dns_zone_edit1.).

> [!warning]
>
> Se ativou as opções `CDN`{.action} ou `IP do país`{.action} com o seu domínio, utilize o endereço IP adaptado através da nossa documentação que regista [o conjunto dos endereços IP dos nossos alojamentos partilhados](clusters_and_shared_hosting_IP1.).
>
...
- Obtenha [as informações necessárias para se ligar ao espaço FTP do seu alojamento web](ftp_connection#1-recuperar-as-informacoes-de-acesso.).
- Obtenha os acessos à base de dados do alojamento web, caso já exista, ou crie um através da nossa [documentação](sql_create_database1.).

#### 1.3 - Instalar o cliente FTP gratuito "FileZilla"

Se ainda não utilizar um cliente FTP, pode utilizar o Filezilla. Encontre o link de download gratuito, assim como um tutorial sobre a sua utilização na nossa documentação sobre a [utilização do FileZilla com o seu plano de alojamento OVHcloud](ftp_filezilla_user_guide1.).

> [!primary]
>
> Existem outros clientes FTP que pode utilizar, como, por exemplo, o Cyberduck. Encontre mais informações na nossa documentação relativa à [utilização do Cyberduck com a sua oferta de alojamento OVHcloud](ftp_cyberduck_user_guide_on_mac1.).
>
...
A maioria dos CMS precisam de uma base de dados para funcionar. As nossas ofertas de [alojamento web](hosting.) incluem-no, com exceção de [alojamento gratuito 100M](domains-free-hosting.).

Utilize o nosso manual para [criar uma base de dados a partir do seu alojamento web](sql_create_database1.).

Se tiver à sua disposição uma oferta Web Cloud Databases em MySQL ou MariaDB e pretender utilizá-la para instalar manualmente o seu CMS, consulte o nosso manual sobre a [criação de uma base de dados num serviço Web Cloud Databases](create-db-and-user-on-db-server#criar-uma-base-de-dados.).

Depois de criar a base de dados, recupere os parâmetros de ligação (servidor, nome da base de dados, nome de utilizador e palavra-passe) e guarde-os para [etapa 3](cms_manual_installation_#step3.) deste guia.
...
>
> Se for também um CMS idêntico ao que deve instalar, pode utilizar [este guia](sql_change_password#etapa-3-alterar-a-palavra-passe-da-base-de-dados-do-seu-site-no-seu-ficheiro-de-configuracao.) para identificar os ficheiros de configuração no seu [espaço de armazenamento FTP](ftp_connection1.).
>
...
>
> - Para aceder à base de dados associada ao alojamento web, consulte o [guia](sql_create_database#aceder-a-interface-phpmyadmin.).
> - Para aceder a uma base de dados presente numa Web Cloud Databases, consulte [este guia](connecting-to-database-on-database-server1.).
>
...
>
> Configure a versão de PHP no seu alojamento web com a ajuda do nosso manual sobre a [alteração da versão PHP de um alojamento web](configure_your_web_hosting1.).
>
...
Depois de descomprimir os ficheiros na pasta "**CMS**", [ligue-se ao espaço de armazenamento com FTP](ftp_connection1.) através do [cliente FTP FileZilla](ftp_filezilla_user_guide1.), e depois copie os ficheiros na pasta "**CMS**" para a pasta raiz que definiu no seu alojamento durante a [etapa 1](cms_manual_installation_#step1.)) deste guia.
...
> [!primary]
>
> Se a pasta raiz que definiu não tiver sido criada automaticamente durante as ações descritas na [etapa 1](cms_manual_installation_#step1.), pode criá-la através do FileZilla.
>
...
Para se ligar em SSH ao seu alojamento, consulte o nosso guia sobre a [ligação em SSH a partir de um alojamento partilhado OVHcloud](ssh_on_webhosting1.).
...
- [Finalizar a instalação do Wordpress](cms_manual_installation_wordpress1.)
- [Finalizar a instalação do Joomla!](cms_manual_installation_joomla1.)
- [Finalizar a instalação de Drupal](cms_manual_installation_drupal1.)
- [Finalizar a instalação do PrestaShop](cms_manual_installation_prestashop1.)
- [Finalizar a instalação do Pico](cms_manual_installation_pico1.)
- [Finalizar a instalação do Grav](cms_manual_installation_grav1.)
- [Finalizar a instalação do Typo3](cms_manual_installation_typo31.)
- [Finalizar a instalação do SPIP](cms_manual_installation_spip1.)

## Quer saber mais? <a name="go-further"></a>

[Migração do seu website e dos seus e-mails para a OVHcloud](hosting_migrating_to_ovh1.)

[Publicar um website no alojamento web](hosting_how_to_get_my_website_online1.)
