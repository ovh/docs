...
> 
> Oddajemy do Twojej dyspozycji niniejszy tutorial, którego celem jest pomoc w jak najlepszym wykonywaniu bieżących zadań. Niemniej jednak, w przypadku trudności zalecamy skorzystanie z pomocy [wyspecjalizowanego webmastera](partner.) lub producenta systemu CMS, który wybrałeś. Niestety firma OVH nie będzie mogła udzielić wsparcia w tym zakresie. Więcej informacji znajduje się w sekcji ["Idź dalej"](cms_manual_installation_#go-further.) niniejszego tutoriala.
>
...
> [!success]
>
> Aby zainstalować CMS **automatycznie** z poziomu [Panelu klienta OVHcloud](manager.), zapoznaj się z naszą dokumentacją dotyczącą [instalacji modułu "jednym kliknięciem"](cms_install_1_click_modules1.).
>
...
Więcej informacji znajduje się na naszej stronie dotyczącej [modułu WordPress](hosting-wordpress.)
...
Więcej informacji znajduje się na naszej stronie dotyczącej [modułu Joomla!](hosting-joomla.)
...
Więcej informacji znajduje się na naszej stronie dotyczącej [modułu Drupal](hosting-drupal.)
...
Więcej informacji znajduje się na naszej stronie dotyczącej [modułu PrestaShop](hosting-prestashop.)
...
Aby zainstalować CMS na Twoim [hostingu](hosting.), potrzebne są jakieś przygotowania.
...
Zapoznaj się z naszą dokumentacją [jak dodać stronę podpiętą w opcji MultiSite na hostingu](multisites_configure_multisite1.), aby zdefiniować katalog główny, z którego chcesz korzystać w systemie CMS.
...
- Upewnij się, czy domena, której będziesz używał do uzyskania dostępu do CMS-a, jak również jej subdomena w www, wskazują na adres IP Twojej usługi [hosting](hosting.).

Aby uzyskać adres IP Twojego hostingu, zaloguj się do [Panelu klienta OVHcloud](manager.) w części `Web Cloud`{.action} i wybierz ofertę hostingu w sekcji `Hosting`{.action}.<br>
W ramce `Informacje ogólne`{.action} po prawej stronie znajdziesz adres IP Twojego hostingu w formularzu `IPv4`{.action}.

Jeśli aktywną strefą DNS Twojej domeny jest zarządzana w Twoim [Panelu klienta OVHcloud](manager.), porównaj adres IP Twojego hostingu z adresem IP w strefie DNS Twojej domeny, korzystając z naszej dokumentacji dotyczącej [stref DNS OVHcloud](dns_zone_edit1.).

> [!warning]
>
> Jeśli aktywowałeś opcje `CDN`{.action} lub `Geolokalizacja IP`{.action} dla Twojej domeny, użyj odpowiedniego adresu IP, korzystając z naszej dokumentacji zawierającej [wszystkie adresy IP naszych hostingów](clusters_and_shared_hosting_IP1.).
>
...
- Pobierz [informacje potrzebne do zalogowania się do przestrzeni FTP Twojego hostingu](ftp_connection#etap-1-pobranie-informacji-niezbednych-do-logowania.).
- Zdobądź dostęp do bazy danych Twojego hostingu, jeśli już istnieje, lub utwórz bazę danych przy użyciu naszej [dokumentacji](sql_create_database1.).

#### 1.3 - Zainstaluj darmowy program FTP "FileZilla"

Jeśli nie korzystasz już z klienta FTP, możesz użyć programu Filezilla. W dokumentacji OVH dotyczącej korzystania z tej usługi znajdziesz link do pobrania za darmo oraz samouczek dotyczący korzystania z niego [FileZilla wraz z hostingiem OVHcloud](ftp_filezilla_user_guide1.).

> [!primary]
>
> Są inni klienci FTP, których możesz użyć, na przykład Cyberduck. Więcej informacji znajdziesz w naszej dokumentacji dotyczącej [korzystania z Cyberduck w ramach Twojego hostingu OVHcloud](ftp_cyberduck_user_guide_on_mac1.).
>
...
Większość systemów CMS potrzebuje do działania bazy danych. Nasza oferta[hosting](hosting.) zawiera listę usług, z wyjątkiem [darmowy hosting 100M](domains-free-hosting.).

Skorzystaj z naszej dokumentacji, aby [utworzyć bazę danych w ramach Twojego hostingu](sql_create_database1.).

Jeśli dysponujesz usługą Web Cloud Databases w MySQL lub MariaDB i chcesz z niej korzystać do ręcznej instalacji CMS-a, zapoznaj się z naszą dokumentacją dotyczącą [utworzenia bazy danych w usłudze Web Cloud Databases](create-db-and-user-on-db-server#tworzenie-bazy-danych.).

Po utworzeniu bazy danych pobierz parametry połączenia (serwer, nazwa bazy danych, nazwa użytkownika i hasło) i zachowaj je dla [etap 3](cms_manual_installation_#step3.) tego przewodnika.
...
>
> Jeśli jest to również CMS identyczny z tym, który powinieneś zainstalować, możesz użyć [tego przewodnika](sql_change_password#etap-3-zmiana-hasla-do-bazy-danych-twojej-strony-www-w-pliku-konfiguracyjnym.) do identyfikacji plików konfiguracyjnych w [przestrzeni dyskowej FTP](ftp_connection1.).
>
...
>
> - Aby zalogować się do bazy danych powiązanej z hostingiem, zapoznaj się z [tym przewodnikiem](sql_create_database#dostep-do-interfejsu-phpmyadmin.).
> - Aby zalogować się do bazy danych na stronie WWW Cloud Databases, zapoznaj się z tym [przewodnikiem](connecting-to-database-on-database-server1.).
>
...
>
> Skonfiguruj następnie wersję PHP na Twoim hostingu, korzystając z dokumentacji [OVHcloud dotyczącej zmiany wersji PHP na hostingu](configure_your_web_hosting1.).
>
...
Po rozpakowaniu plików w Twoim katalogu "**CMS**", [zaloguj się przez FTP do przestrzeni dyskowej](ftp_connection1.) przy użyciu [klienta FTP FileZilla](ftp_filezilla_user_guide1.), a następnie skopiuj pliki zawarte w katalogu "**CMS**" do "katalogu głównego", który zdefiniowałeś na Twoim hostingu podczas [etap 1](cms_manual_installation_#step1.)) niniejszego przewodnika.
...
> [!primary]
>
> Jeśli "katalog główny", który zdefiniowałeś nie został utworzony automatycznie podczas operacji opisanych w [etap 1](cms_manual_installation_#step1.), możesz go utworzyć za pomocą FileZilla.
>
...
Aby zalogować się przez SSH do Twojego hostingu, zapoznaj się z naszym przewodnikiem dotyczącym [logowania przez SSH na hostingu OVHcloud](ssh_on_webhosting1.).
...
- [Zakończenie instalacji modułu Wordpress](cms_manual_installation_wordpress1.)
- [Zakończenie instalacji Joomla!](cms_manual_installation_joomla1.)
- [Zakończenie instalacji Drupala](cms_manual_installation_drupal1.)
- [Zakończenie instalacji PrestaShop](cms_manual_installation_prestashop1.)
- [Zakończenie instalacji Pico](cms_manual_installation_pico1.)
- [Zakończenie instalacji Grav](cms_manual_installation_grav1.)
- [Zakończenie instalacji Typo3](cms_manual_installation_typo31.)
- [Zakończenie instalacji SPIP](cms_manual_installation_spip1.)

## Sprawdź również <a name="go-further"></a>

[Przeniesienie strony WWW i kont e-mail do OVHcloud](hosting_migrating_to_ovh1.){.external}

[Uruchomienie strony WWW na hostingu](hosting_how_to_get_my_website_online1.){.external}

[Instalacja kilku stron WWW na jednym hostingu](multisites_configure_multisite1.){.external}

Zobacz nasze [rozwiązania hostingu WWW](hosting.){.external}
