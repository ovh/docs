...
> 
> Questa guida ti aiuta a eseguire le operazioni necessarie alla configurazione del tuo account. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di contattare un [provider specializzato](partner.). OVHcloud non sarà infatti in grado di fornirti assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](ssl_avoid_common_pitfalls_of_making_website_secure_#go-further.) di questa guida.
>
...
- Disporre di un [hosting Web OVHcloud](hosting.){.external}.
- Aver registrato almeno un [dominio](domains.){.external}.
- Avere accesso allo [Spazio Cliente OVHcloud](manager.){.external}, sezione "Web cloud".
...
> [!primary]
>
> Alcuni siti Web utilizzano [Content Delivery Network (CDN)](cdn_how_to_use_cdn1.) per ospitare, ad esempio, librerie *Javascript* (come *JQuery*). 
...
Ecco un esempio di reindirizzamento che potete aggiungere a un file "[.htaccess](htaccess_url_rewriting_using_mod_rewrite1.)", alla radice del vostro sito web (sostituendo l'URL *https://www.yourdomain.tld* con il vostro):
