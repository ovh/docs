...
- Dispor de um [alojamento web OVHcloud](hosting.){.external}.
- Ter registado, pelo menos, um [nome de domínio](domains.){.external}.
- Ter acesso à sua [Área de Cliente OVHcloud](manager.){.external}, parte "Web cloud".
...
> [!primary]
>
> Alguns websites utilizam [Content Delivery Network (CDN)](cdn_how_to_use_cdn1.) para alojar, por exemplo, bibliotecas *Javascript* (como *JQuery*). 
...
Eis um exemplo de reencaminhamento que pode adicionar num ficheiro "[.htaccess](htaccess_url_rewriting_using_mod_rewrite1.)", na raiz do seu website (substituindo o URL *https://www.yourdomain.tld* pelo seu):
