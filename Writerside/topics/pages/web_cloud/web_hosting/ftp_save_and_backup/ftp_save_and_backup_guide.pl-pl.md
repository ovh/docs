...
> 
> Wykonując kopię zapasową bezpieczeństwa Twojej strony WWW i używając bazy danych, wykonaj również kopię zapasową bazy danych. Zapoznaj się z naszym przewodnikiem, aby [tworzenie i pobieranie kopii zapasowej bazy danych na hostingu WWW](sql_database_export1.).
> 
...
- Posiadanie oferty [hostingu](hosting.){.external} (nie dotyczy hostingu [Cloud Web](hosting-cloud-web-offer.)).
- W zależności od użytej metody, posiadanie dostępu do interfejsu zarządzania usługą hostingu WWW w [Panelu klienta](manager.){.external} lub posiadanie hasła użytkownika FTP umożliwiającego zalogowanie do przestrzeni dyskowej. 
...
>
> Aby sprawdzić rozmiar używany na Twoim hostingu, zaloguj się do [Panelu klienta OVHcloud](manager.). Przejdź do sekcji `Web Cloud`{.action} i kliknij `Hosting`{.action}. Wybierz odpowiedni hosting. 
>
> Wykorzystany limit pojawia się na stronie, która się wyświetla:
>
>![ftp quota](images_find-disk-space.png){.thumbnail}
>
> Jeśli używana przestrzeń dyskowa FTP jest większa niż połowa całkowitej przestrzeni dyskowej FTP w Twojej ofercie hostingu, możesz lokalnie pobrać duże elementy Twojej strony WWW (w tym celu możesz użyć [Filezilla](ftp_filezilla_user_guide1.)). Usuń je z przestrzeni FTP. Zmniejszy to rozmiar przestrzeni dyskowej FTP, której używasz i będziesz mógł przywrócić dane z Panelu klienta OVHcloud.
>

Przejdź do sekcji dotyczącej wybranej przez Ciebie metody przywracania przestrzeni dyskowej. 

- [Przywracanie przestrzeni dyskowej w Panelu klienta](ftp_save_and_backup_#viacontrolpanel.)
...
> 
> Aby dowiedzieć się, czy Twoja strona została umieszczona w trybie konserwacji, zapoznaj się z naszym przewodnikiem [Co zrobić w przypadku strony “403 forbidden”?](diagnostic_403_forbidden1.). 
> 
> W tym przypadku:
>
> - Do [kontaktu administratora](managing_contacts#dostep-do-zarzadzania-kontaktami.) hostingu wysyłamy e-mail z naszymi zespołami. 
> - Status "konserwacji" pojawia się w Twoim [Panelu klienta OVHcloud](manager.){.external}. W sekcji `Web Cloud`{.action} kliknij Twoją usługę w sekcji `Hosting`{.action}, a następnie zakładkę `Informacje ogólne`{.action}.
> - Strona (-y), na której (-ych) hostowane są strony "403 Forbidden".
>

Przed rozpoczęciem operacji zaloguj się do [Panelu klienta](manager.){.external}, kliknij `Hosting`{.action} , następnie wybierz nazwę odpowiedniego hostingu. Przejdź do zakładki `FTP - SSH`{.action} i kliknij przycisk `Przywróć kopię zapasową`{.action}.

![kopia zapasowa](restore-backup.png){.thumbnail}
...
![kopia zapasowa](restore-backup-step-1.png){.thumbnail}
...
> [!primary]
>
>Automatyczne przywracanie może trwać od kilku minut do kilku godzin. Jeśli trwa to **więcej niż 24 godziny**, skontaktuj się z [OVHcloud support](support.).
>
...
- **program FileZilla**: pobierz program ze strony producenta. OVHcloud udostępnia przewodnik [Korzystanie z programu FileZilla na Twoim hostingu](ftp_filezilla_user_guide1.){.external}, w którym znajdziesz instrukcje obsługi programu. Pamiętaj, że przewodnik OVHcloud nie zastępuje oficjalnej dokumentacji producenta programu Filezilla.

- **program Cyberduck**: pobierz program ze strony producenta. OVHcloud udostępnia przewodnik [Korzystanie z programu Cyberduck na Twoim hostingu](ftp_cyberduck_user_guide_on_mac1.){.external}, w którym znajdziesz instrukcje obsługi programu. Pamiętaj, że przewodnik OVHcloud nie zastępuje oficjalnej dokumentacji producenta programu Filezilla.

- **interfejs FTP Explorer**: zaloguj się do interfejsu w [Panelu klienta](manager.){.external}. Po zalogowaniu, kliknij `Hosting`{.action}, następnie wybierz odpowiedni hosting. Przejdź do zakładki `FTP - SSH`{.action} i kliknij przycisk `FTP Explorer`{.action}.

Kiedy jesteś gotowy do rozpoczęcia operacji, przejdź do następnego etapu.

![kopia zapasowa](images_ftp-explorer.png){.thumbnail}
...
Możesz odnaleźć te informacje w zakładce `FTP - SSH`{.action} na Twoim hostingu. Jeśli nie posiadasz hasła dla użytkownika FTP, zapoznaj się z instrukcjami zawartymi w przewodniku [Zmiana hasła do konta FTP](ftp_change_password1.){.external}.

![kopia zapasowa](login-infos.png){.thumbnail}
...
![kopia zapasowa](login-interface-snap0.png){.thumbnail}
...
[Korzystanie z programu FileZilla na Twoim hostingu](ftp_filezilla_user_guide1.){.external}

[Korzystanie z programu Cyberduck na Twoim hostingu](ftp_cyberduck_user_guide_on_mac1.){.external}
