...
Aceda à Área de [Cliente OVHcloud](manager.) e selecione a opção `Web Cloud`{.action}. Clique em `Alojamentos`{.action} e selecione o plano correspondente. Clique em `...`{.action} à direita de "Opção CDN" e, a seguir, em `Encomendar um CDN`{.action} ou `Ativar a opção`{.action} se a opção CDN já estiver incluída no seu alojamento.
...
Aceda à Área de [Cliente OVHcloud](manager.) e selecione a opção `Web Cloud`{.action}. Clique em `Alojamentos`{.action} e selecione o plano correspondente. No separador `Multisite`{.action}, clique na roda dentada situada à direita da entrada multisite e depois em `Alterar`{.action}.

Assinale a opção "Ativar o CDN", clique em `Seguinte`{.action} e depois em `Validar`{.action}.

![CDN](activation.png){.thumbnail}
...
> Consulte a [lista de endereços IP dos clusters e alojamento Web](clusters_and_shared_hosting_IP1.) para encontrar o endereço IP específico do CDN do seu cluster.
...
![CDN](sharedcdn-clear-cache.png){.thumbnail}
...
> [!warning]
> 
> Algumas opções estão bloqueadas na oferta Basic e requerem a subscrição do [CDN security](hosting-options-cdn.) ou do [CDN Advanced](hosting-options-cdn.).

![CDN](sharedcdn-modify-cdn.png){.thumbnail}
...
![CDN](sharedcdn-option-settings.png){.thumbnail}
...
![CDN](sharedcdn-create-cache-rule.png){.thumbnail}
...
![CDN](sharedcdn-cache-rules.png){.thumbnail}
...
> [!warning]
>
> Para beneficiar de um limite de regras mais elevado e de vantagens de parâmetros sobre a criação de regras de cache, pode optar pela opção [CDN Advanced](hosting-options-cdn.).
...
> [!primary]
> As opções apresentadas abaixo requerem a subscrição do [CDN security](hosting-options-cdn.) ou do [CDN Advanced](hosting-options-cdn.).
...
- **Cross-Origin Resource Sharing (CORS)**: Indique na lista os domínios externos que serão autorizados a aceder aos recursos do seu website para os partilhar.
	...
	![CDN](security-cors.png){.thumbnail}
...
- **HTTPS-redirect** : Proteja a globalidade do tráfego do seu website reencaminhando-o para o protocolo HTTPS de forma temporária ou permanente.
	...
	![CDN](security-https-redirect.png){.thumbnail}
...
- **HTTP Strict Transport Security (HSTS)**: Imponha o acesso ao seu website unicamente em HTTPS. Desta forma, a sua solução web fica protegida contra os ataques de degradação (ou ataques de replicação).
	...
	![CDN](security-hsts.png){.thumbnail}
...
> [!warning]
>
> Para a instalação de um [módulo 1 clique da OVHcloud](cms_install_1_click_modules1.), o WAF deve ser desativado para evitar que a instalação do módulo seja bloqueada.
...
> [!primary]
>
>  As opções apresentadas abaixo requerem a subscrição do [CDN Advanced](hosting-options-cdn.)
...
- **Purga avançada**: Personalize a sua purga escolhendo os elementos da cache a esvaziar: todo o site, pasta, URI, extensão de ficheiros ou através de uma expressão personalizada e regular. 
	...
	![CDN](advanced-clear-cache-step-1.png){.thumbnail}
...
- **Prewarm**: Importe a cache permanente dos seus recursos primordiais. O CDN antecipa e refresca automaticamente a cache, sem esperar pelo pedido do utilizador. Esta função aplica-se apenas a conteúdos estáticos com um TTL superior a 0 e o recurso não deve exceder 1GB. Uma bitola indica-lhe o nível de consumo dos recursos em *Prewarm*, em função da lista dos seus URLs. O total dos recursos mobilizados por estes URLs não deve ultrapassar 1GB.
	...
	![CDN](advanced-prewarm.png){.thumbnail}
...
![CDN](advanced-create-cache-rule.png){.thumbnail}
...
![CDN](advanced-cache-rules.png){.thumbnail}
...
> Esta alteração é realizada automaticamente se a zona DNS ativa do seu domínio for também gerida a partir da [conta OVHcloud](manager.) que contém o seu alojamento web.
>
> Caso contrário, deverá executar as duas ações seguintes:
>
> - Obtenha o endereço IP **predefinido** do cluster onde se encontra o seu alojamento web através do nosso guia "[Alojamento web: os endereços IP dos nossos clusters](clusters_and_shared_hosting_IP1.)";
...
Aceda à [Área de Cliente OVHcloud](manager.) e selecione a opção `Web Cloud`{.action}. Clique em `Alojamentos`{.action} e selecione o plano correspondente. No separador `Multisite`{.action}, clique em `...`{.action} à direita da entrada multisite e depois em `Alterar`{.action}.

Desmarque a opção "Ativar o CDN", clique em `Seguinte`{.action} e depois em `Validar`{.action}.

![CDN](deactivation.png){.thumbnail}
...
Aceda à Área de [Cliente OVHcloud](manager.) e selecione a opção `Web Cloud`{.action}. Clique em `Alojamentos`{.action} e selecione o plano correspondente. Clique em `...`{.action} à direita de "Opção CDN" e depois em `Eliminar CDN`{.action}.

![CDN](resiliation.png){.thumbnail}
