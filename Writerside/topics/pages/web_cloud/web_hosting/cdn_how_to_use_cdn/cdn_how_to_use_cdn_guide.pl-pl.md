...
- Posiadanie [hostingu WWW](hosting.).
...
Zaloguj się do Panelu [klienta OVHcloud](manager.) i wybierz `Web Cloud`{.action}. Kliknij przycisk `Hostingi`{.action} i wybierz odpowiednie rozwiązanie. Kliknij `...`{.action} po prawej stronie "Opcji GeoCache", a następnie `Zamów CDN`{.action} lub `Włącz opcję`{.action}, jeśli opcja GeoCache jest już zawarta w Twoim hostingu.
...
Zaloguj się do Panelu [klienta OVHcloud](manager.) i wybierz `Web Cloud`{.action}. Kliknij przycisk `Hostingi`{.action} i wybierz odpowiednie rozwiązanie. W karcie `Multisite`{.action} kliknij ikonę koła zębatego po prawej stronie, a następnie wybierz `Zmień`{.action}.

Zaznacz opcję „Aktywuj GeoCache”, kliknij przycisk `Dalej`{.action}, a następnie `Zatwierdź`{.action}.

![GeoCache](activation.png){.thumbnail}
...
> Zapoznaj się z [listą adresów IP klastrów i hostingów WWW](clusters_and_shared_hosting_IP1.), aby znaleźć adres IP wykorzystywany przez usługę GeoCache w Twoim klastrze.
...
![GeoCache](sharedcdn-clear-cache.png){.thumbnail}
...
> [!warning]
> 
> Niektóre opcje są zablokowane dla oferty Basic i wymagają zamówienia usługi [CDN security](hosting-options-cdn.) lub [CDN Advanced](hosting-options-cdn.)

![GeoCache](sharedcdn-modify-cdn.png){.thumbnail}
...
![GeoCache](sharedcdn-option-settings.png){.thumbnail}
...
![GeoCache](sharedcdn-create-cache-rule.png){.thumbnail}
...
![GeoCache](sharedcdn-cache-rules.png){.thumbnail}
...
> [!warning]
>
> Aby korzystać z wyższego limitu reguł i dodatkowych parametrów podczas tworzenia reguły cache, możesz wybrać opcję [CDN Advanced](hosting-options-cdn.).
...
> [!primary]
>
> Poniższe opcje wymagają zamówienia na [CDN security](hosting-options-cdn.) lub [CDN Advanced](hosting-options-cdn.)
...
- **Cross-Origin Resource Sharing (CORS)**: Wpisz na liście nazwy domen zewnętrznych, które będą mogły uzyskać dostęp do zasobów Twojej strony WWW, aby je udostępnić. 
	...
	![GeoCache](security-cors.png){.thumbnail}
...
- **HTTPS-redirect**: Chroń cały ruch na Twojej stronie WWW poprzez przekierowanie go na protokół HTTPS tymczasowo lub na stałe.
	...
	![GeoCache](security-https-redirect.png){.thumbnail}
...
- **HTTP Strict Transport Security (HSTS)**: Zarządzaj dostępem do Twojej strony WWW za pomocą protokołu HTTPS. Rozwiązanie WWW jest więc zabezpieczone przed atakami przez retrogradację (lub ataki typu repli).
	...
	![GeoCache](security-hsts.png){.thumbnail}
...
> [!warning]
>
> Aby instalacja [modułu za pomocą 1 kliknięcia OVHcloud](cms_install_1_click_modules1.) mogła zostać wyłączona, należy wyłączyć WAF.
...
> [!primary]
>
> Poniższe opcje wymagają zamówienia usługi [CDN Advanced](hosting-options-cdn.).
...
- **Czyszczenie zaawansowane**: Spersonalizuj wyczyszczenie, wybierając elementy pamięci cache, które chcesz opróżnić: cała strona, folder, URI, rozszerzenie plików lub za pomocą regularnych ustawień. 
	...
	![GeoCache](advanced-clear-cache-step-1.png){.thumbnail}
...
- **Prewarm**: Wymuś stałe przechowywanie Twoich pierwotnych zasobów. CDN automatycznie przewiduje i odświeża cache, nie czekając na zapytanie użytkownika. Funkcja ta ma zastosowanie wyłącznie do treści statycznych o TTL powyżej 0, a zasoby nie mogą przekraczać 1GB. Przyrząd określi poziom zużycia zasobów w *Prewarm*, w zależności od listy Twoich URL. Łączne zasoby wywoływane przez te adresy URL nie mogą przekraczać 1GB.
	...
	![GeoCache](advanced-prewarm.png){.thumbnail}
...
![GeoCache](advanced-create-cache-rule.png){.thumbnail}
...
![GeoCache](advanced-cache-rules.png){.thumbnail}
...
![GeoCache](statistics.png){.thumbnail}
...
> Zmiana ta jest wykonywana automatycznie, jeśli aktywna strefa DNS Twojej domeny jest zarządzana również z poziomu [konto OVHcloud](manager.) zawierającego Twój hosting.
>
> W przeciwnym razie należy wykonać następujące dwa działania:
>
> - uzyskaj domyślny adres IP **klastra**, w którym znajduje się Twój hosting, korzystając z naszego przewodnika "[Hosting WWW: adresy IP klastrów OVHcloud](clusters_and_shared_hosting_IP1.)";
...
Przejdź do Panelu [klienta OVHcloud](manager.) i wybierz `Web Cloud`{.action}. Kliknij przycisk `Hostingi`{.action} i wybierz odpowiednie rozwiązanie. W zakładce `MultiSite`{.action} kliknij `...`{.action} po prawej stronie pozycji MultiSite, a następnie `Zmień`{.action}.

Usuń zaznaczenie opcji "Aktywuj GeoCache", kliknij przycisk `Dalej`{.action}, a następnie `Zatwierdź`{.action}.

![GeoCache](deactivation.png){.thumbnail}

### Usuń opcję CDN na Twoim hostingu

Celem tego działania jest usunięcie opcji GeoCache dla całego hostingu WWW.

Przejdź do Panelu [klienta OVHcloud](manager.) i wybierz `Web Cloud`{.action}. Kliknij przycisk `Hostingi`{.action} i wybierz odpowiednie rozwiązanie. Kliknij `...`{.action} po prawej stronie "Opcji GeoCache", a następnie `Rezygnacja z GeoCache`{.action}.

![GeoCache](resiliation.png){.thumbnail}
