...
- Posséder une [offre d'hébergement web](hosting.).
...
Connectez-vous à votre [espace client OVHcloud](manager.) puis sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action} et choisissez l'offre concernée. Dans l'onglet `Multisite`{.action}, cliquez sur la roue crantée à droite de l'entrée multisite puis cliquez sur `Modifier`{.action}.
...
>
> Consultez la [liste des adresses IP des clusters et hébergement web](clusters_and_shared_hosting_IP1.) pour retrouver l'adresse IP spécifique du CDN de votre cluster.
...
> [!warning]
> 
> Certaines options sont verrouillées sur l'offre Basic et nécessitent la souscription au [CDN security](hosting-options-cdn.) ou au [CDN Advanced](hosting-options-cdn.)
...
> [!warning]
>
> Pour bénéficier d'un quota de règles plus élevé et d'avantage de paramètres sur la création de règle de mise en cache, vous pouvez opter pour l'option [CDN Advanced](hosting-options-cdn.)
...
> [!primary]
>  les options présentées ci-dessous nécessitent la souscription au [CDN security](hosting-options-cdn.) ou au [CDN Advanced](hosting-options-cdn.)
...
> [!warning]
>
> Pour l'installation d'un [module en 1 clic OVHcloud](cms_install_1_click_modules1.), le WAF doit être désactivé afin d’éviter que l'installation du module ne soit bloquée.
...
> [!primary]
>
>  Les options présentées ci-dessous nécessitent la souscription au [CDN Advanced](hosting-options-cdn.)
...
> Ce changement se fait automatiquement si la zone DNS active de votre nom de domaine est également gérée depuis le [compte OVHcloud](manager.) contenant votre hébergement web.
>
> Dans le cas contraire, vous devrez effectuer les deux actions suivantes :
>
> - récupérez l'adresse IP **par défaut** du cluster où se trouve votre hébergement web à l'aide de notre guide « [Hébergement web : les adresses IP de nos clusters](clusters_and_shared_hosting_IP1.) » ;
...
Rendez-vous dans votre [espace client OVHcloud](manager.) puis sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action} et choisissez l'offre concernée. Dans l'onglet `Multisite`{.action}, cliquez sur `...`{.action} à droite de l'entrée multisite puis sur `Modifier`{.action}.
...
Rendez-vous dans votre [espace client OVHcloud](manager.) puis sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action} et choisissez l'offre concernée. Cliquez sur `...`{.action} à droite de « Option CDN » puis sur `Résilier le CDN`{.action}.
