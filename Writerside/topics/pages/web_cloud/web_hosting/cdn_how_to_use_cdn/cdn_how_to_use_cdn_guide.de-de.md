...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web Cloud`{.action} auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus und klicken Sie auf `...`{.action} rechts neben "CDN-Option" danach auf `CDN bestellen`{.action} oder `Option aktivieren`{.action}, wenn die CDN-Option bereits in Ihrem Hosting enthalten ist.
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web Cloud`{.action} auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus. Klicken Sie auf dem Tab `Multisite`{.action} auf `...`{.action} rechts neben dem Multisite-Eintrag und dann auf `Domain bearbeiten`{.action}.
...
>
> Im [Verzeichnis von IP-Adressen für die Webhosting Cluster](clusters_and_shared_hosting_IP1.) finden Sie die spezifische IP-Adresse des CDN Ihres Clusters.
...
> [!warning]
> 
> Einige Optionen sind für das Hosting-Angebot "Basic" nicht verfügbar und erfordern die Aktivierung von [CDN Security](hosting-options-cdn.) oder [CDN Advanced](hosting-options-cdn.).
...
> [!warning]
>
> Um eine höhere Quota an Regeln und zusätzliche Einstellungen für die Erstellung von Cache-Regeln zu erhalten, können Sie sich für die [CDN Advanced Option](hosting-options-cdn.) entscheiden.
...
> [!primary]
>  Für die unten aufgeführten Optionen ist [CDN Security](hosting-options-cdn.) oder [CDN Advanced](hosting-options-cdn.) erforderlich.
...
> [!warning]
>
> Vor der Installation eines OVHcloud [1-Klick-Moduls](cms_install_1_click_modules1.) muss WAF deaktiviert werden, um eine Blockierung der Installation des Moduls zu vermeiden.
...
> [!primary]
>
>  Die unten aufgeführten Optionen erfordern das Abo eines [CDN Advanced](hosting-options-cdn.).
...
> Diese Änderung erfolgt automatisch, wenn die aktive DNS-Zone Ihres Domainnamens auch über den [OVHcloud Account](manager.) verwaltet wird, der Ihr Webhosting enthält.
>
> Wenn dies nicht der Fall ist, müssen Sie die beiden folgenden Aktionen ausführen:
>
> - Ermitteln Sie mithilfe unserer Anleitung „[Webhosting: Die IP-Adressen unserer Cluster](clusters_and_shared_hosting_IP1.)“ die **Standard-IP-Adresse** des Clusters ab, auf dem sich Ihr Webhosting befindet.
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web`{.action} und dann auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus. Klicken Sie auf dem Tab `Multisite`{.action} auf `...`{.action} rechts neben dem Multisite-Eintrag und dann auf `Domain bearbeiten`{.action}.
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und klicken Sie im Bereich `Web Cloud`{.action} auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus und klicken Sie auf `...`{.action} rechts neben "CDN-Option" und danach auf `CDN kündigen`{.action}.
