...
- Tener contratado un [plan de alojamiento web](hosting.).
...
> Consulte la [lista de direcciones IP de los clústeres y alojamientos web](clusters_and_shared_hosting_IP1.) para saber qué dirección IP corresponde a la CDN de su clúster.
...
> [!warning]
>
> Algunas opciones están bloqueadas en la solución Basic y requieren la suscripción a la [CDN Security](hosting-options-cdn.) o a la [CDN Advanced](hosting-options-cdn.)
...
> [!warning]
>
> Para disfrutar de un mayor límite de reglas y de más parámetros en la creación de reglas de almacenamiento en caché, puede optar por la opción [CDN Advanced](hosting-options-cdn.)
...
> [!primary]
>
> Las opciones que se presentan a continuación requieren la suscripción a la [CDN Security](hosting-options-cdn.) o a la [CDN Advanced](hosting-options-cdn.)
...
> [!warning]
>
> Para instalar un [módulo en 1 clic OVHcloud](cms_install_1_click_modules1.), el WAF debe desactivarse para evitar que se bloquee la instalación del módulo.
...
> [!primary]
>
> Las opciones que se presentan a continuación requieren la suscripción a la [CDN Advanced](hosting-options-cdn.)
...
> Este cambio se realiza automáticamente si la zona DNS activa del dominio también se gestiona desde la [cuenta de OVHcloud](manager.) que contiene el alojamiento web.
>
> En caso contrario, deberá realizar las dos acciones siguientes:
>
> - Obtenga la dirección IP **por defecto** del cluster en el que se encuentra su alojamiento web, mediante la guía "[Web hosting: direcciones IP de nuestros clusters](clusters_and_shared_hosting_IP1.)";
...
Acceda al [área de cliente de OVHcloud](manager.) y seleccione `Web Cloud`{.action}.Haga clic en `Alojamientos`{.action} y seleccione el plan correspondiente. En la pestaña `Multisitio`{.action}, haga clic en `...`{.action} a la derecha de la entrada multisitio y seleccione `Editar`{.action}.
...
Acceda al [área de cliente de OVHcloud](manager.) y seleccione `Web Cloud`{.action}.Haga clic en `Alojamientos`{.action} y seleccione el plan correspondiente. Haga clic en `...`{.action} a la derecha de "Opción CDN" y, seguidamente, en `Anular la CDN`{.action}.
