...
> 
> Ponemos a su disposición esta guía para ayudarle a realizar las tareas más habituales. No obstante, si tiene alguna duda, le recomendamos que contacte con un [proveedor especializado](partner.). Nosotros no podremos asistirle. Más información en la sección ["Más información"](hosting_migrating_to_ovh_#go-further.) de esta guía.
>
...
- [Etapa 1: contratar el alojamiento y las direcciones de correo en OVHcloud](hosting_migrating_to_ovh_#step1.)
- [Etapa 2: crear y pre-configurar una zona DNS para su dominio en OVHcloud](hosting_migrating_to_ovh_#step2.)
- [Etapa 3: recuperar una copia de seguridad completa de su sitio web](hosting_migrating_to_ovh_#step3.)
- [Etapa 4: importar la copia de seguridad de su sitio web en su plan de hosting de OVHcloud](hosting_migrating_to_ovh_#step4.)
- [Etapa 5: crear sus direcciones de correo de forma idéntica en OVHcloud](hosting_migrating_to_ovh_#step5.)
- [Etapa 6: Declarar los servidores de correo de OVHcloud en la zona DNS activa de su dominio](hosting_migrating_to_ovh_#step6.)
...
- El alojamiento [Personal](hosting-personal-offer.) con **10 direcciones de correo** "MX Plan"
- El alojamiento [Pro](hosting-professional-offer.) con **100 direcciones de correo** "MX Plan"
- El alojamiento [Performance](hosting-performance-offer.) con **1000 direcciones de correo** "MX Plan". Esta oferta se divide en 4 "subproductos".
...
> [!primary]
>
Además de la solución MX Plan, OVHcloud ofrece otras soluciones de correo. Por ejemplo, puede combinar con direcciones de correo MX Plan direcciones cuentas ["Exchange"](emails-hosted-exchange.).
>

### Etapa 2: crear y preconfigurar una zona DNS para su dominio en OVHcloud <a name="step2"></a>

Una vez creado el alojamiento, conéctese a su [área de cliente de OVHcloud](manager.) y cree una zona DNS para su dominio **sin los "www"**. Si lo necesita, consulte nuestra guía sobre la [creación de una zona DNS en OVHcloud](dns_zone_create1.).

Una vez que haya creado la zona DNS, acceda a su gestión en la guía [Editar una zona DNS de OVHcloud](dns_zone_edit1.). Si no están presentes, introduzca las siguientes entradas:
...
- Su nombre de dominio sin los "www", hacia la dirección IP de destino de tipo "A" de su alojamiento OVHcloud. Para obtener la dirección IP correcta, consulte nuestra guía relativa a las [direcciones IP de los distintos clusters de alojamiento compartido](clusters_and_shared_hosting_IP1.).
...
![hosting](dashboard-mx-a-cname.png){.thumbnail}
...
Para importar la copia de seguridad del espacio de almacenamiento FTP de su anterior proveedor, [conéctese al espacio de almacenamiento FTP de su alojamiento de OVHcloud](ftp_connection1.) y envíe la copia de seguridad a la carpeta raíz "www" (o a otra carpeta raíz que haya creado previamente).

Le recomendamos que utilice el programa [FileZilla](ftp_filezilla_user_guide1.) para transferir su copia de seguridad FTP a su alojamiento.
...
Para la copia de seguridad de su base de datos, [cree una nueva base de datos](sql_create_database1.) y luego [importe la copia de seguridad en su nueva base de datos](sql_importing_mysql_database1.).
...
> [!success]
>
> Para asociar una nueva base de datos si utiliza un Content Management System (CMS) como WordPress, Joomla, Drupal o PrestaShop, consulte la información relativa a sus archivos de configuración desde **el etapa 2** de la guía "[Modificación de la contraseña de una base de datos](sql_change_password1.)".
>

Declare/autorice su dominio externo en su alojamiento web de OVHcloud en la guía "[gestión de multisitios de un alojamiento web de OVHcloud](multisites_configure_multisite1.)". Introduzca el nombre de la carpeta raíz que haya elegido al comienzo del[etapa 4](hosting_migrating_to_ovh_#step4.). Le recordamos que esta es la carpeta en la que ha guardado sus archivos en su espacio de almacenamiento FTP.
...
![hosting](dashboard-a-txt-cname.png){.thumbnail}
...
Utilice nuestra guía sobre la [creación de direcciones de correo electrónico MX Plan](email_creation1.).

Si ha optado por una solución Exchange, [consulte nuestra documentación sobre el asunto para crear sus direcciones de correo](exchange_starting_hosted1.)
...
**Opción 1**: utilice nuestra herramienta [OVH Mail Migrator (OMM)](https://omm.ovh.net/) {.external} que permite copiar el contenido de las direcciones de correo electrónico que tuviera con su anterior proveedor a las creadas en OVHcloud. Para más información, consulte nuestra guía [Migrar cuentas de correo electrónico a través de OVH Mail Migrator](migration_omm1.).
...
Una vez completada la migración, conéctese a su dirección de correo electrónico de OVHcloud utilizando el [webmail OVHcloud](email.) para comprobar que todos sus mensajes de correo estén presentes en la nueva cuenta.
...
Todos los parámetros de configuración se encuentran en la guía "[Información general sobre los MX Plan](email_generalities#32-configurar-la-direccion-de-correo-en-un-dispositivo.)". También encontrará enlaces a las guías de configuración personalizadas para los principales programas de mensajería.

#### Para las cuentas de correo "Exchange":

Consulte todas nuestras guías de ayuda a la configuración en `Configuración Exchange en ordenador` y `Configuración Exchange en smartphone` de [nuestra documentación sobre Exchange](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Etapa 9: sustituir los servidores DNS activos de su dominio por los de OVHcloud <a name="step9"></a>

La zona DNS preconfigurada durante el [etapa 2](hosting_migrating_to_ovh_#step2.) todavía no se ha aplicado al dominio.
...
Transfiera su dominio con ayuda de nuestra guía sobre [transferencia de un dominio a OVHcloud](transfer_incoming_generic_domain1.).
...
[Información general sobre los mensajes de correo en alojamiento compartido](email_generalities1.).

[Información general sobre los servidores DNS](dns_server_general_information1.).

[Crear una dirección de correo en alojamiento compartido](email_creation1.).

[Importar una base de datos MySQL](sql_importing_mysql_database1.).

[Gestión de una base de datos desde un alojamiento compartido](sql_create_database1.).
