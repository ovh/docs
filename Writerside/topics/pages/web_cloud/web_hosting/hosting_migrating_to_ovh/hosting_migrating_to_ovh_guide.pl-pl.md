...
> 
> Oddajemy w Twojej ręce niniejszy przewodnik, którego celem jest pomoc w jak najlepszym wykonywaniu bieżących zadań. W przypadku trudności zalecamy skorzystanie z pomocy [wyspecjalizowanego usługodawcy](partner.). Niestety firma OVHcloud nie będzie mogła udzielić wsparcia w tym zakresie. Więcej informacji znajduje się w sekcji ["Idź dalej"](hosting_migrating_to_ovh_#go-further.) niniejszego przewodnika.
>
...
- [Etap 1: zamów hosting i konta e-mail w OVHcloud](hosting_migrating_to_ovh_#step1.)
- [Etap 2: utworzyć i wstępnie skonfigurować strefę DNS dla Twojej domeny w OVHcloud](hosting_migrating_to_ovh_#step2.)
- [Etap 3: pobrać pełną kopię zapasową Twojej strony WWW](hosting_migrating_to_ovh_#step3.)
- [Etap 4: importować kopię zapasową Twojej strony WWW do Twojego hostingu OVHcloud](hosting_migrating_to_ovh_#step4.)
- [Etap 5: stwórz konta e-mail w serwisie OVHcloud](hosting_migrating_to_ovh_#step5.)
- [Etap 6: zadeklarować serwery e-mail OVHcloud w strefie DNS domeny](hosting_migrating_to_ovh_#step6.)
...
- Hosting [Perso](hosting-personal-offer.) z **10 adresów e-mail** "MX Plan"
- Hosting [Pro](hosting-professional-offer.) z **100 adresów e-mail** "MX Plan"
- Hosting [Performance](hosting-performance-offer.) z **1000 adresów e-mail** "MX Plan". Oferta jest podzielona na 4 "podoferty".
- Hosting [Cloud Web](hosting-cloud-web-offer.) z **200 adresów e-mail** "MX Plan". Ta oferta jest używana przez programistów aplikacji.
...
Zamówienie można złożyć w [Panelu klienta OVHcloud](manager.). Po zalogowaniu kliknij pozycję `Web Cloud`{.action}, następnie kliknij przycisk `Zamów`{.action} w lewym górnym rogu, a następnie kliknij `Hosting`{.action}. Przejdź przez kolejne etapy zamówienia **bez konieczności transferu domeny**.
...
> [!primary]
>
> OVHcloud poza ofertą "MX Plan" proponuje również inne usługi e-mail. Możesz na przykład połączyć konta e-mail "MX Plan" z adresami ["Email-Pro"](email-pro.) i/lub kontami ["Exchange"](emails-hosted-exchange.).
>

### Etap 2: utworzyć i wstępnie skonfigurować strefę DNS dla Twojej domeny w OVHcloud <a name="step2"></a>

Po utworzeniu hostingu zaloguj się do [Panelu klienta OVHcloud](manager.), następnie utwórz strefę DNS dla Twojej domeny **bez wpisywania jej na listę "www"**. Zapoznaj się z naszym przewodnikiem [dotyczącym tworzenia strefy DNS w OVHcloud](dns_zone_create1.).

Po utworzeniu strefy DNS zaloguj się do jej zarządzania, korzystając z naszego przewodnika "[Edycja strefy DNS OVHcloud](dns_zone_edit1.)". Jeśli nie są obecne, wprowadź następujące wpisy:
...
- Twoja domena bez "www" na adres IP "A" na hostingu OVHcloud. Aby uzyskać poprawny adres IP, zapoznaj się z naszym przewodnikiem dotyczącym adresów IP [IP różnych klastrów hostingowych](clusters_and_shared_hosting_IP1.).
...
Aby zaimportować kopię zapasową przestrzeni dyskowej FTP poprzedniego dostawcy, [zaloguj się do przestrzeni FTP Twojego hostingu OVHcloud](ftp_connection1.) i przesłać kopię zapasową do katalogu głównego "www" (lub innego katalogu głównego, który wcześniej utworzyłeś).

Zalecamy użycie programu [FileZilla](ftp_filezilla_user_guide1.) do pobrania kopii zapasowej FTP z Twojego hostingu.
...
Do tworzenia kopii zapasowej bazy danych [utwórz nową bazę danych](sql_create_database1.), a następnie [zaimportuj kopię zapasową do nowej bazy danych](sql_importing_mysql_database1.).
...
> [!success]
>
> Aby połączyć nową bazę danych, jeśli korzystasz z Content Management System (CMS), takiego jak WordPress, Joomla!, Drupal lub PrestaShop, znajdziesz informacje dotyczące plików konfiguracyjnych w pliku konfiguracyjnym w **etap 4** w przewodniku ["zmiana hasła do bazy danych"](sql_change_password1.).
>

Zarządzaj zewnętrzną domeną na hostingu OVHcloud za pomocą naszego przewodnika "[zarządzanie stronami podpiętymi w opcji MultiSite](multisites_configure_multisite1.)". Proszę podać nazwę katalogu głównego wybranego na początku [etap 4](hosting_migrating_to_ovh_#step4.). Przypominamy, że jest to folder, w którym umieściłeś pliki na przestrzeni FTP.
...
Skorzystaj z naszego przewodnika dotyczącego [tworzenia kont e-mail "MX Plan"](email_creation1.) i przypisz do konta e-mail hostowane u Twojego dostawcy.

Jeśli wybrałeś rozwiązanie "Email Pro" lub "Exchange", zapoznaj się z naszą dokumentacją dotyczącą tworzenia kont e-mail:

- Dla [Email-Pro](first_config1.)
- W przypadku [Exchange](exchange_starting_hosted1.)
...
Po zakończeniu migracji zaloguj się na Twój adres e-mail OVHcloud przy użyciu [webmail OVHcloud](email.), aby sprawdzić, czy wszystkie Twoje e-maile są obecne w nowym koncie.
...
- Wszystkie parametry konfiguracji znajdziesz w naszym przewodniku "[Informacje ogólne na temat e-maili MX Plan](email_generalities#2-skorzystaj-z-wybranego-programu.)". Znajdziesz tam również linki do przewodników dotyczących konfiguracji dla głównych programów pocztowych.

#### Dla kont "Email-Pro":

- Wszystkie nasze przewodniki dotyczące konfiguracji znajdują się w sekcjach `Konfiguracja na komputerze` i `Konfiguracja na smartphonie` [dokumentacja dotycząca oferty Email-Pro](web-cloud-email-collaborative-solutions-email-pro1.).

#### Dla kont e-mail "Exchange":

- Wszystkie nasze przewodniki dotyczące konfiguracji znajdują się w sekcjach `Konfiguracja Exchange na komputerze` oraz `Konfiguracja Exchange na smartphonie` [dokumentacja dotycząca oferty Exchange](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Etap 9: zastąpienie aktywnych serwerów DNS Twojej domeny serwerami OVHcloud <a name="step9"></a>

Strefa DNS wstępnie skonfigurowana podczas [etap 2](hosting_migrating_to_ovh_#step2.) nie została jeszcze zastosowana do Twojej domeny.
...
Przenieś domenę w naszym przewodniku dotyczącym [transferu domeny do OVHcloud](transfer_incoming_generic_domain1.).

Po zakończeniu transferu danych i usług zrezygnuj ze starych usług u poprzedniego dostawcy.

## Sprawdź również <a name="go-further"></a>

[Informacje ogólne na temat e-maili na hostingu](email_generalities1.).

[Informacje ogólne na temat serwerów DNS](dns_server_general_information1.).

[Tworzenie konta e-mail na hostingu](email_creation1.).

[Import bazy danych MySQL](sql_importing_mysql_database1.)

[Zarządzanie bazą danych na hostingu](sql_create_database1.).
