...
- [Step 1: ordina l'hosting e gli indirizzi email in OVHcloud](hosting_migrating_to_ovh_#step1.)
- [Step 2: creare e preconfigurare una zona DNS per il tuo dominio in OVHcloud](hosting_migrating_to_ovh_#step2.)
- [Step 3: recuperare un backup completo del tuo sito Web](hosting_migrating_to_ovh_#step3.)
- [Step 4: importare il backup del tuo sito Web sulla tua offerta di hosting OVHcloud](hosting_migrating_to_ovh_#step4.)
- [Step 5: creare i tuoi indirizzi email identici in OVHcloud](hosting_migrating_to_ovh_#step5.)
- [Step 6: dichiarare i server di posta OVHcloud nella zona DNS attiva del dominio](hosting_migrating_to_ovh_#step6.)
...
- L'hosting [Personale](hosting-personal-offer.) con **10 indirizzi email** "MX Plan"
- Hosting [Pro](hosting-professional-offer.) con **100 indirizzi email** "MX Plan"
- Hosting [Performance](hosting-performance-offer.) con **1000 indirizzi email** "MX Plan". Questa offerta è disponibile in 4 "sottolivello".
- L'hosting [Cloud Web](hosting-cloud-web-offer.) con **200 indirizzi email** "MX Plan". Questa offerta è utilizzata dagli sviluppatori di applicazioni.
...
> [!primary]
>
> OVHcloud propone altre offerte email in aggiunta al servizio "MX Plan". È possibile, ad esempio, combinare a degli indirizzi email "MX Plan" degli indirizzi ["Email-Pro"](email-pro.) e/o degli account ["Exchange"](emails-hosted-exchange.).
>

### Step 2: creare e preconfigurare una zona DNS per il tuo dominio presso OVHcloud <a name="step2"></a>

Una volta creato il tuo hosting, accedi allo [Spazio Cliente OVHcloud](manager.) e crea una zona DNS per il tuo dominio **senza le "www"**. Per maggiori informazioni, consulta la guida sulla [creazione di una zona DNS OVHcloud](dns_zone_create1.).

Una volta creata la zona DNS, consulta la guida [Modificare una zona DNS OVHcloud](dns_zone_edit1.). Se non sono presenti, inserisci le seguenti voci:
...
- Il dominio senza "www", verso l'indirizzo IP di destinazione di tipo "A" del tuo hosting OVHcloud Per recuperare l'indirizzo IP corretto, consulta la guida che elenca gli [indirizzi IP dei diversi cluster di hosting condivisi](clusters_and_shared_hosting_IP1.).
...
Per importare il backup dello spazio di storage FTP del tuo precedente provider, [accedi allo spazio di storage FTP del tuo hosting OVHcloud](ftp_connection1.) e trasferisci il backup nella cartella root "www" (o in un'altra cartella root creata precedentemente).

Ti consigliamo di utilizzare il software [FileZilla](ftp_filezilla_user_guide1.) per trasferire il tuo backup FTP sul tuo hosting.
...
Per il backup del tuo database, [crea un nuovo database](sql_create_database1.) e [importa il backup nel tuo nuovo database](sql_importing_mysql_database1.).
...
> [!success]
>
> Per collegare il tuo nuovo database se utilizzi un Content Management System (CMS) come WordPress, Joomla!, Drupal o PrestaShop, consulta le informazioni sui file di configurazione nello **Step 2** della guida ["Modificare la password di un database"](sql_change_password1.).
>

Dichiarare/autorizzare un dominio esterno su un hosting Web OVHcloud tramite la nostra guida ["Gestione dei multisiti di un hosting Web OVHcloud"](multisites_configure_multisite1.). Dichiarare correttamente il "nome" della cartella di root scelta all'inizio dello [step 4](hosting_migrating_to_ovh_#step4.). Ti ricordiamo che questa è la cartella in cui hai inserito i tuoi file nel tuo spazio di storage FTP.
...
Ricevi allo stesso modo gli indirizzi email presenti presso il tuo provider tramite la nostra guida sulla [creazione di indirizzi email "MX Plan"](email_creation1.).

Se hai scelto una soluzione Email Pro o Exchange, consulta la nostra documentazione per creare i tuoi indirizzi email:

- Per [Email-Pro](first_config1.)
- Per [Exchange](exchange_starting_hosted1.)
...
Una volta completata la migrazione, accedi al tuo indirizzo email OVHcloud utilizzando la [Webmail OVHcloud](email.) per verificare che tutte le tue email siano presenti nel nuovo account.
...
- Consulta la guida Per maggiori informazioni sui parametri di configurazione [Gestisci le email MX Plan](email_generalities#2-utilizza-il-software-che-preferisci.). e i link alle guide personalizzate per i principali client di posta.

#### Per gli account "Email-Pro":

- Consulta le nostre guide all'aiuto alla configurazione nelle sezioni `Configurazione su computer` e `Configurazione su smartphone` de [la nostra guida sull'offerta Email-Pro](web-cloud-email-collaborative-solutions-email-pro1.).

#### Per gli account email "Exchange":

- Tutte le nostre guide all'aiuto alla configurazione sono disponibili nelle sezioni `Configurazione di Exchange sul computer` e `Configurazione di Exchange su smartphone` di [la nostra guida sull'offerta Exchange](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Step 9: sostituire i server DNS attivi del tuo dominio con quelli di OVHcloud <a name="step9"></a>

La zona DNS preconfigurata durante lo [step 2](hosting_migrating_to_ovh_#step2.) non è ancora applicata al tuo dominio.
...
In seguito, trasferisci il tuo dominio nella guida su [trasferisci un dominio in OVHcloud](transfer_incoming_generic_domain1.).
...
[Generalità sulle email condivise](email_generalities1.).

[Generalità sui server DNS](dns_server_general_information1.).

[Creare un indirizzo email condiviso](email_creation1.).

[Importa un database MySQL](sql_importing_mysql_database1.).

[Gestire un database da un hosting condiviso](sql_create_database1.).
