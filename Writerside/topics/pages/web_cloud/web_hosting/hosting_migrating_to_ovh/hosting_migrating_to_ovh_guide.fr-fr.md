...
> 
> Nous mettons à votre disposition ce guide afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](hosting_migrating_to_ovh_#go-further.) de ce guide.
>
...
- [Etape 1 : commander l'hébergement et les adresses e-mail chez OVHcloud](hosting_migrating_to_ovh_#step1.)
- [Etape 2 : créer et pré-configurer une zone DNS pour votre nom de domaine chez OVHcloud](hosting_migrating_to_ovh_#step2.)
- [Etape 3 : récupérer une sauvegarde complète de votre site Web](hosting_migrating_to_ovh_#step3.)
- [Etape 4 : importer la sauvegarde de votre site Web sur votre offre d'hébergement OVHcloud](hosting_migrating_to_ovh_#step4.)
- [Etape 5 : recréer vos adresses e-mail à l'identique chez OVHcloud](hosting_migrating_to_ovh_#step5.)
- [Etape 6 : déclarer les serveurs e-mail OVHcloud dans la zone DNS active de votre nom de domaine](hosting_migrating_to_ovh_#step6.)
...
- L'hébergement [Perso](hosting-personal-offer.) avec **10 adresses e-mail** « MX Plan »
- L'hébergement [Pro](hosting-professional-offer.) avec **100 adresses e-mail** « MX Plan »
- L'hébergement [Performance](hosting-performance-offer.) avec **1000 adresses e-mail** « MX Plan ». Cette offre est déclinée en 4 « sous-offres ».
- L'hébergement [Cloud Web](hosting-cloud-web-offer.) avec **200 adresses e-mail** « MX Plan ». Cette offre est utilisée par les développeurs d'applications.
...
> [!primary]
>
> OVHcloud propose d'autres offres e-mail en plus de l'offre « MX Plan ». Vous pouvez, par exemple, combiner à des adresses e-mail « MX Plan » des adresses [« Email-Pro »](email-pro.) et/ou des comptes [« Exchange »](emails-hosted-exchange.).
>

### Etape 2 : créer et préconfigurer une zone DNS pour votre nom de domaine chez OVHcloud <a name="step2"></a>

Lorsque votre hébergement est créé, connectez-vous à votre [espace client OVHcloud](manager.) puis créez une zone DNS pour votre nom de domaine **sans les « www »**. Vous pouvez vous aider de notre guide sur la [création d'une zone DNS chez OVHcloud](dns_zone_create1.).

Une fois la zone DNS créée, accédez à sa gestion à l'aide de notre guide « [Éditer une zone DNS OVHcloud](dns_zone_edit1.) ». Si elles ne sont pas présentes, renseignez les entrées suivantes :
...
- Votre nom de domaine sans les « www », vers l'adresse IP cible de type « A » de votre hébergement OVHcloud. Pour récupérer la bonne adresse IP, consultez notre guide listant les [adresses IP des différents clusters d'hébergement mutualisés](clusters_and_shared_hosting_IP1.).
...
Pour importer la sauvegarde de l'espace de stockage FTP de votre ancien prestataire, [connectez-vous à l'espace de stockage FTP de votre hébergement OVHcloud](ftp_connection1.) et téléversez la sauvegarde dans le dossier racine « www » (ou dans un autre dossier racine que vous aurez préalablement créé).

Nous vous recommandons d'utiliser le logiciel [FileZilla](ftp_filezilla_user_guide1.) pour téléverser votre sauvegarde FTP sur votre hébergement.
...
Pour la sauvegarde de votre base de données, [créez une nouvelle base de données](sql_create_database1.) puis [importez votre sauvegarde dans votre nouvelle base de données](sql_importing_mysql_database1.).
...
> [!success]
>
> Pour lier votre nouvelle base de données si vous utilisez un Content Management System (CMS) comme WordPress, Joomla!, Drupal ou PrestaShop, retrouvez les informations sur leurs fichiers de configuration depuis **l'étape 2** du guide « [modification du mot de passe d'une base de données](sql_change_password1.) ».
>

Déclarez/autorisez votre nom de domaine externe sur votre hébergement web OVHcloud via notre guide « [gestion des multisites d'un hébergement web OVHcloud](multisites_configure_multisite1.) ». Déclarez bien le « nom » du dossier racine que vous avez choisi au début de l'[étape 4](hosting_migrating_to_ovh_#step4.). Pour rappel, il s'agit du dossier dans lequel vous avez placé vos fichiers dans votre espace de stockage FTP.
...
Recréez à l'identique les adresses e-mail présentes chez votre fournisseur e-mail à l'aide de notre guide sur la [création d'adresses e-mail « MX Plan »](email_creation1.).
...
- Pour [Email-Pro](first_config1.)
- Pour [Exchange](exchange_starting_hosted1.)
...
Une fois la migration terminée, connectez-vous à votre adresse e-mail OVHcloud à l'aide du [webmail OVHcloud](email.) pour vérifier que tous vos e-mails sont bien présents dans le nouveau compte.
...
- Retrouvez l'ensemble des paramètres de configuration dans notre guide « [Généralités sur les e-mails MX Plan](email_generalities#2-utiliser-le-logiciel-de-votre-choix.) ». Vous y trouverez également les liens vers les guides de configuration personnalisés pour les principaux logiciels de messagerie.

#### Pour les comptes « Email-Pro » :

- Retrouvez l'ensemble de nos guides d'aide à la configuration dans les sections `Configuration sur ordinateur` et `Configuration sur smartphone` de [notre documentation sur l'offre Email-Pro](web-cloud-email-collaborative-solutions-email-pro1.).

#### Pour les comptes e-mail « Exchange » :

- Retrouvez l'ensemble de nos guides d'aide à la configuration dans les sections `Configuration Exchange sur ordinateur` et `Configuration Exchange sur smartphone` de [notre documentation sur l'offre Exchange](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Etape 9 : remplacer les serveurs DNS actifs de votre nom de domaine par ceux d'OVHcloud <a name="step9"></a>

La zone DNS pré-configurée lors de l'[étape 2](hosting_migrating_to_ovh_#step2.) n'est pas encore appliquée à votre nom de domaine.
...
Transférez ensuite votre nom de domaine à l'aide de notre guide sur le [transfert d'un nom de domaine chez OVHcloud](transfer_incoming_generic_domain1.).
...
[Généralités sur les e-mails mutualisés](email_generalities1.).

[Généralités sur les serveurs DNS](dns_server_general_information1.).

[Créer une adresse e-mail mutualisée](email_creation1.).

[Importation d’une base de données MySQL](sql_importing_mysql_database1.).

[Gestion d’une base de données depuis un hébergement mutualisé](sql_create_database1.).
