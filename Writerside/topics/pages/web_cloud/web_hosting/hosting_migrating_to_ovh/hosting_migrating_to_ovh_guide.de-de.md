...
> 
> Diese Anleitung soll Sie bei allgemeinen Aufgaben bestmöglich unterstützen. Dennoch empfehlen wir Ihnen, falls Sie Hilfe brauchen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren oder Ihre Fragen in der OVHcloud Community zu stellen. Leider können wir Ihnen für administrative Aufgaben keine weitergehende technische Unterstützung anbieten. Weitere Informationen finden Sie am [Ende dieser Anleitung](hosting_migrating_to_ovh_#go-further.).
>
...
- [Schritt 1: Webhosting und E-Mail-Accounts bei OVHcloud bestellen](hosting_migrating_to_ovh_#step1.)
- [Schritt 2: Eine DNS-Zone für Ihren Domainnamen bei OVHcloud erstellen und vorkonfigurieren](hosting_migrating_to_ovh_#step2.)
- [Schritt 3: Vollständiges Backup Ihrer Website erstellen](hosting_migrating_to_ovh_#step3.)
- [Schritt 4: Backup Ihrer Website in Ihr OVHcloud Hosting importieren](hosting_migrating_to_ovh_#step4.)
- [Schritt 5: Ihre E-Mail-Accounts bei OVHcloud neu erstellen](hosting_migrating_to_ovh_#step5.)
- [Schritt 6: OVHcloud E-Mail-Server in der aktiven DNS-Zone Ihres Domainnamens hinterlegen](hosting_migrating_to_ovh_#step6.)
...
- Hosting [Basic](hosting-personal-offer.) mit **10 "MX Plan E-Mail-Accounts**
- Hosting [Pro](hosting-professional-offer.) mit **100 "MX Plan E-Mail-Accounts** (für gewerbliche Webseiten)
- Hosting [Performance](hosting-performance-offer.) mit **1000 "MX Plan E-Mail-Accounts** (skalierbare dedizierte Ressourcen)
- Hosting [Cloud Web](hosting-cloud-web-offer.) mit **200 E-Mail-Accounts** (für Anwendungsentwickler)
...
Wenn Sie bereits Kunde sind, kann die Bestellung auch über Ihr [OVHcloud Kundencenter](manager.) ausgeführt werden. Wenn Sie eingeloggt sind, klicken Sie auf den Bereich `Web Cloud`{.action}. Klicken Sie dann links auf den Button `Bestellen`{.action} und wählen Sie `Hosting-Pakete`{.action}. Folgen Sie den Bestellschritten **ohne den Transfer Ihrer Domain zu beantragen**.
...
> [!primary]
> OVHcloud bietet neben MX Plan weitere E-Mail-Angebote an. Sie können beispielsweise [E-Mail Pro](email-pro.)-Accounts und/oder [Exchange](emails-hosted-exchange.)-Accounts mit E-Mail-Accounts kombinieren.
>

### Schritt 2: Eine DNS-Zone für Ihren Domainnamen bei OVHcloud erstellen und vorkonfigurieren <a name="step2"></a>

Wenn Ihr Webhosting eingerichtet ist, loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und erstellen Sie eine DNS-Zone für Ihren Domainnamen. Verwenden Sie dabei nicht "**www**". Weitere Informationen finden Sie in unserer Anleitung zur [Erstellung einer DNS-Zone bei OVHcloud](dns_zone_create1.).

Sobald die DNS-Zone eingerichtet ist, kann sie zur Verwendung mit dem Webhosting vorbereitet werden. Verwenden Sie dazu unsere Anleitung zum [Bearbeiten einer OVHcloud DNS-Zone](dns_zone_edit1.). Fügen Sie folgende Einträge hinzu, sofern diese nicht existieren:
...
- Ihr Domainname ohne "www" als Eintragstyp "A", mit der IP-Adresse Ihres OVHcloud Hostings als Ziel. Die zu verwendende IP-Adresse ist gelistet im [Verzeichnis von IP-Adressen für die Webhosting Cluster](clusters_and_shared_hosting_IP1.).
...
Um das Backup des FTP-Speicherplatzes Ihrer Website zu importieren, [verbinden Sie sich mit dem FTP-Speicherplatz Ihres OVHcloud Hostings](ftp_connection1.) und laden Sie das Backup in den "www"-Wurzelordner hoch (gegebenenfalls in einen anderen Wurzelordner, den Sie zuvor erstellt haben).

Wir empfehlen, das Programm [FileZilla](ftp_filezilla_user_guide1.) zu verwenden, um das FTP-Backup auf Ihr Hosting zu übertragen.
...
[Erstellen Sie eine neue Datenbank](sql_create_database1.) für Ihr Hosting und [importieren Sie dann Ihr Backup](sql_importing_mysql_database1.).
...
> [!success]
>
> Wenn Sie ein Content Management System (CMS) wie WordPress, Joomla!, Drupal oder PrestaShop verwenden, finden Sie die Informationen zur Datenbank in deren Konfigurationsdateien. Sie finden Details hierzu in **Schritt 2** der Anleitung "[Änderung des Passworts einer Datenbank](sql_change_password1.)".
>

Deklarieren und authorisieren Sie Ihren externen Domainnamen auf Ihrem OVHcloud Webhosting mithilfe unserer Anleitung "[Mehrere Websites auf einem Webhosting einrichten](multisites_configure_multisite1.)". Geben Sie den Namen des Ordners, den Sie zu Beginn von [Schritt 4](hosting_migrating_to_ovh_#step4.) ausgewählt haben, als Wurzelverzeichnis an. Zur Erinnerung: Es handelt sich um den Ordner im FTP-Speicherplatz, in den Sie Ihre Webseiten-Dateien abgelegt haben.
...
Erstellen Sie neue E-Mail-Accounts und verwenden Sie dabei die gleichen E-Mail-Adressen, die Sie aktuell bei Ihrem E-Mail-Provider haben. Sie finden mehr Informationen in unserer Anleitung zur [Erstellung von E-Mail-Accounts mit MX Plan](email_creation1.).
...
- [Email-Pro](first_config1.)
- [Exchange](exchange_starting_hosted1.)
...
Wenn die Migration abgeschlossen ist, loggen Sie sich über [OVHcloud Webmail](email.) in Ihren OVHcloud E-Mail-Account ein, um zu überprüfen, ob alle Ihre E-Mails im neuen Account enthalten sind.
...
- Alle Konfigurationseinstellungen finden Sie in unserer Anleitung "[Erste Schritte mit MX Plan](email_generalities#2-e-mail-client-verwenden.)". Sie finden dort auch Links zu den Konfigurationsanleitungen für die wichtigsten E-Mail-Clients.

#### Für E-Mail Pro E-Mail-Accounts:

- Sie finden alle unsere Anleitungen zur Konfigurationsunterstützung unter `Konfiguration des E-Mail-Clients` auf der [Seite für E-Mail Pro](web-cloud-email-collaborative-solutions-email-pro1.).

#### Für Exchange E-Mail-Accounts:

- Sie finden alle unsere Anleitungen zur Konfigurationsunterstützung in den Abschnitten `Konfiguration des Exchange E-Mail-Clients`und `Konfiguration von Exchange auf kompatiblen Smartphones/Tablets` auf der [Seite für Exchange](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Schritt 9: Die aktiven DNS-Server Ihres Domainnamens mit OVHcloud DNS-Servern ersetzen <a name="step9"></a>

Die in [Schritt 2](hosting_migrating_to_ovh_#step2.) vorkonfigurierte DNS-Zone wird noch nicht auf Ihren Domainnamen angewendet.
...
Transferieren Sie anschließend Ihren Domainnamen mithilfe unserer Anleitung zum [Transfer eines Domainnamens zu OVHcloud](transfer_incoming_generic_domain1.).
...
[DNS-Server ändern](dns_server_general_information1.)

[E-Mail-Accounts erstellen](email_creation1.)

[Backup in eine Webhosting-Datenbank importieren](sql_importing_mysql_database1.)

[Verwaltung einer Webhosting-Datenbank](sql_create_database1.)
