...
>
> This guide is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](partner.) or reach out to the OVHcloud community if you encounter any difficulties. We will not be able to assist you. You can find more information in the [Go further](hosting_migrating_to_ovh_#go-further.) section of this guide.
>
...
- [Step 1: Order the hosting plan and email accounts from OVHcloud](hosting_migrating_to_ovh_#step1.)
- [Step 2: Create and preconfigure a DNS zone for your domain name at OVHcloud](hosting_migrating_to_ovh_#step2.)
- [Step 3: Retrieve a full backup of your website](hosting_migrating_to_ovh_#step3.)
- [Step 4: Import your website backup to your OVHcloud hosting plan](hosting_migrating_to_ovh_#step4.)
- [Step 5: Recreate your email accounts at OVHcloud](hosting_migrating_to_ovh_#step5.)
- [Step 6: Declare the OVHcloud email servers in your domain name’s active DNS zone](hosting_migrating_to_ovh_#step6.)
...
- Hosting [Personal](hosting-personal-offer.) with **10 MX Plan email accounts**
- Hosting [Pro](hosting-professional-offer.) with **100 MX Plan email accounts** (for business)
- Hosting [Performance](hosting-performance-offer.) with **1000 MX Plan email accounts** (scalable dedicated resources)
- Hosting [Cloud Web](hosting-cloud-web-offer.) with **200 MX Plan email accounts** (for application developers)
...
As an OVHcloud customer you can also place an order from your [OVHcloud Control Panel](manager.). Once you have logged in, click on the `Web Cloud`{.action} section, then click on the `Order`{.action} button in the top left-hand corner and choose `Hosting plans`{.action}. Continue with the steps for the **order process without requesting the transfer of your domain name**.
...
> [!primary]
>
> OVHcloud offers other email services in addition to the MX Plan solution. For example, you can combine email accounts of the [Email Pro offer](email-pro.) and the [Exchange offer](emails-hosted-exchange.) with MX Plan email accounts.
>

### Step 2: Create and preconfigure a DNS zone for your domain name at OVHcloud <a name="step2"></a>

After your hosting service is installed, log in to your [OVHcloud Control Panel](manager.) to create a DNS zone for your domain name. Do not use "**www**" when doing this. You can refer to our guide on [Creating a DNS zone at OVHcloud](dns_zone_create1.).

Once you have created a DNS zone, you can start configuring it with the help of our guide: [Editing an OVHcloud DNS zone](dns_zone_edit1.). Add the following records if they do not exist:
...
- Your domain name without "www" as record type "A", with the IP address of your OVHcloud hosting as target. To retrieve the correct IP address, please refer to our guide listing the [IP addresses of the shared hosting clusters](clusters_and_shared_hosting_IP1.).
...
To import the backup of your old service provider’s FTP storage space, [log in to the FTP storage space of your OVHcloud hosting](ftp_connection1.) and upload the backup to the root folder ‘www’ (or another root folder you have already created).

We recommend that you use [FileZilla](ftp_filezilla_user_guide1.) to upload your FTP backup to your hosting.
...
[Create a new database](sql_create_database1.) for your hosting, then [import your backup into your new database](sql_importing_mysql_database1.).
...
> [!success]
>
> If you are using a Content Management System (CMS) like WordPress, Joomla!, Drupal or PrestaShop, you can find the information in their configuration files. See **Step 2** of the guide “[Changing a database password](sql_change_password1.)”.
>

Declare and authorise your external domain name on your OVHcloud web hosting plan by referring to our guide on [Managing multiple websites on an OVHcloud web hosting plan](multisites_configure_multisite1.). Enter the name of the folder you chose at the beginning of [step 4](hosting_migrating_to_ovh_#step4.) as the Multisite "Root folder". As a reminder, this is the folder in which you have placed your files in your FTP storage space.
...
Create new email accounts and name them according to your current email addresses hosted by your email provider. Use our guide on [Creating MX Plan email addresses](email_creation1.).
...
- [For Email Pro](first_config1.)
- [For Exchange](exchange_starting_hosted1.)
...
**Option 1**: Use our [OVH Mail Migrator (OMM)](https://omm.ovh.net/){.external} tool, which allows you to copy the content of email accounts at your old service provider to your new OVHcloud accounts. You can refer to our guide [Migrating email accounts using OVH Mail Migrator](migration_omm1.).
...
Once the migration is complete, log in to your OVHcloud email account using [OVHcloud webmail](email.) and check that all your emails are in the new account.
...
- You can find all of the configuration settings in our guide “[Getting started with the MX Plan solution](email_generalities#2-using-the-software-of-your-choice.)”. You will also find links to custom configuration guides for major email software.

#### For Email Pro accounts:

- You can find all of our configuration guides in the section `Email client configuration` of [our documentation on the Email Pro offer](web-cloud-email-collaborative-solutions-email-pro1.).

#### For Exchange email accounts:

- You can find all of our configuration guides in the `Exchange configuration on computer` and `Exchange configuration on smartphone` sections of [our Exchange documentation](web-cloud-email-collaborative-solutions-microsoft-exchange1.).

### Step 9: Replace your domain name’s active DNS servers with those of OVHcloud <a name="step9"></a>

The DNS zone preconfigured in [step 2](hosting_migrating_to_ovh_#step2.) has not yet been applied to your domain name.
...
Then transfer your domain name using our guide on [transferring a domain name to OVHcloud](transfer_incoming_generic_domain1.).
...
[General information on DNS servers](dns_server_general_information1.)

[Creating an email address with an MX Plan solution](email_creation1.)

[Importing a MySQL database](sql_importing_mysql_database1.)
