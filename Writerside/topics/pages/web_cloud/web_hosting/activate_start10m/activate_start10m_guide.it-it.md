...
Con [Hosting gratuito 100M](domains-free-hosting.){.external}, OVHcloud ti offre un hosting Web da 100 MB e un account email con 5 GB di storage. Questa guida ti mostra come attivare l’hosting gratuito 100M sul tuo [dominio](domains.){.external}.
...
> Per creare un sito Web composto da più pagine e che necessita di database, come un CMS (WordPress, Joomla!, PrestaShop, Drupal, ecc...), ti consigliamo di ordinare direttamente uno [dei nostri hosting Web](hosting.) dal nostro sito o dal tuo [Spazio Cliente OVHcloud](manager.){.external}.
>
...
- Disporre di un [dominio](domains.){.external} nel tuo [Spazio Cliente OVHcloud](manager.){.external}, scollegato da un hosting Web e senza [MX Plan](email_generalities1.) associato.
- Avere accesso allo [Spazio Cliente OVHcloud](manager.){.external}.

## Procedura

Accedi allo [Spazio Cliente OVHcloud](manager.){.external}, clicca su `Domini`{.action} nel menu a sinistra e seleziona il dominio interessato.
...
>
> Se la tua zona DNS non è gestita dallo [Spazio Cliente OVHcloud](manager.){.external}, è necessario effettuare manualmente le modifiche nella tua zona DNS esterna.
>
> Per maggiori informazioni, consulta la nostra guida su [modificare una zona DNS OVHcloud](dns_zone_edit1.).
>
...
>
> Per maggiori informazioni, consulta le nostre [offerte di hosting](hosting.).
>
...
Una volta confermato l'ordine, riceverai un'email con le informazioni di [connessione FTP](ftp_connection1.){.external} al tuo hosting gratuito 100M.

Consulta la guida di [creazione di un account MX Plan](email_creation1.){.external} per usufruire dell'indirizzo email incluso con il tuo hosting gratuito 100M.

## Per saperne di più

[Accedere allo spazio di storage di un hosting Web](ftp_connection1.){.external}

[Creare un indirizzo email con la soluzione MX Plan](email_creation1.){.external}
