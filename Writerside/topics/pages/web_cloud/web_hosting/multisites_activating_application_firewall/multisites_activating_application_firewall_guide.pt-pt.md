...
- Dispor de, pelo menos, um [domínio](domains.){.external} associado ao alojamento.
- Ter acesso à [Área de Cliente OVHcloud](manager.){.external}.

## Instruções

Ligue-se à sua [Área de Cliente OVHcloud](manager.){.external} e selecione `Web Cloud`{.action}. Na barra à esquerda, clique em `Alojamentos`{.action} e, a seguir, no alojamento correspondente.
...
![gestão em curso](firewall-planned.png){.thumbnail}
