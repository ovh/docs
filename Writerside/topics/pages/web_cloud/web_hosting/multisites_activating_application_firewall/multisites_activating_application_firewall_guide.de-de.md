...
- Sie haben ein [Webhosting](hosting.) in Ihrem OVHcloud Account.
- Sie verfügen über mindestens einen mit dem Hosting verbundenen [Domainnamen](domains.){.external}.
- Sie haben Zugriff auf Ihr [OVHcloud Kundencenter](manager.)

## In der praktischen Anwendung

Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und wählen Sie `Web Cloud`{.action} aus. Klicken Sie auf den Bereich `Hosting-Pakete`{.action} und dann auf das betreffende Hosting.
...
![Multisite](modify-a-domain.png){.thumbnail}
...
![Multisite](firewall-planned.png){.thumbnail}
...
![Multisite](firewall-enabled.png){.thumbnail}
