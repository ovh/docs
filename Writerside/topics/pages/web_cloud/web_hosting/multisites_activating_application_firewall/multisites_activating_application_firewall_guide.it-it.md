...
- Disporre di almeno un [dominio](domains.){.external} associato all'hosting
- Avere accesso allo [Spazio Cliente OVHcloud](manager.){.external}

## Procedura

Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e seleziona `Web Cloud`{.action}. Clicca su `Hosting`{.action} e poi su quello corrispondente.
...
![gestione in corso](firewall-planned.png){.thumbnail}
