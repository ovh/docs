...
- Posiadanie co najmniej jednej [domeny](domains.){.external} przypisanej do hostingu
- Dostęp do [Panelu klienta OVHcloud](manager.){.external}.

## W praktyce

Zaloguj się do swojego [panelu klienta OVHcloud](manager.){.external} i wybierz `Web Cloud`{.action}. Kliknij sekcję `Hosting`{.action}, a następnie nazwę odpowiedniego hostingu.
...
![zapalenie skóry](modify-a-domain.png){.thumbnail}
...
![modyfydomain](modify-a-domain-enable-firewall-step-1.png){.thumbnail}

### Sprawdź stan zadania aktywacji

![zarządzanie w trakcie](firewall-planned.png){.thumbnail}
...
![gerageenabled](firewall-enabled.png){.thumbnail}
