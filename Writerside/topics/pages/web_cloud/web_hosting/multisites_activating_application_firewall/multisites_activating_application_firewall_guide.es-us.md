...
- Tener al menos un [dominio](domains.){.external} asociado al alojamiento.
- Haber iniciado sesión en el [área de cliente de OVHcloud](manager.){.external}.

## Procedimiento

Conéctese al [área de cliente de OVHcloud](manager.){.external} y seleccione `Web Cloud`{.action}. Haga clic en `Alojamientos`{.action} y seleccione el alojamiento correspondiente.
...
![gestión en curso](firewall-planned.png){.thumbnail}
