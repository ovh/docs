...
utilizzando due file di configurazione (HTTP) Apache da inserire nello [spazio FTP](ftp_connection1.) del tuo hosting Web: 

- "**.htaccess**": per maggiori informazioni sulle funzionalità di questo file, consulta il nostro tutorial sulle ["Operazioni realizzabili con un file ".htaccess"](htaccess_what_else_can_you_do1.).
...
- Disporre di una [offerta di hosting Web](hosting.)
- Avere accesso allo [Spazio Cliente OVHcloud](manager.)
- Disporre delle credenziali di accesso a [spazio FTP del tuo hosting](ftp_connection1.)
...
>
> Se utilizzi un CMS Wordpress, OVHcloud mette a disposizione anche una guida su come [utilizzare il file htaccess con WordPress](htaccess_how_to_protect_wordpress1.).
>
...
>
> Per farlo, contatta la nostra [Community di utenti](https://community.ovh.com/en/) o i nostri [partner OVHcloud](partner.).
>
...
> 
> Per modificare la versione di PHP sui tuoi siti Web, consulta la nostra guida "[Hosting Web : ambiente, versione PHP, .ovhconfig](configure_your_web_hosting1.)".
...
Accedi a [lo spazio di storage FTP](ftp_connection1.) del tuo hosting Web. Apri la ["cartella di root"](multisites_configure_multisite1.) verso cui punta il tuo dominio.
...
Se disponi di un hosting [Pro](hosting-professional-offer.) o [Performance](hosting-performance-offer.), collegati in [SSH](ssh_on_webhosting1.) al tuo hosting Web. Scendete nella "**directory**" dove si trova il vostro script "crypt.php".
...
> [!warning]
>
> Per motivi di sicurezza, si raccomanda l'utilizzo di SSH. Tuttavia, se disponi di un'offerta **Starter** o [Personale](hosting-personal-offer.) sulle quali l'SSH non è disponibile, puoi anche eseguire il file "crypt.php" tramite il tuo browser Web.
>
...
- `your_ftp_login`: il login FTP che utilizzi per accedere al tuo spazio di archiviazione FTP. Per recuperare il tuo login FTP, consulta la nostra guida sulla [connessione al tuo spazio FTP](ftp_connection1.).
...
[Tutorial - Operazioni realizzabili con un file ".htaccess"](htaccess_what_else_can_you_do1.)
