...
Isto utilizando dois ficheiros de configuração (HTTP) Apache que pretende colocar no [espaço FTP](ftp_connection1.) do seu alojamento Web: 

- "**.htaccess**": para mais informações sobre as funcionalidades deste ficheiro, consulte o nosso tutorial sobre as ["Operações possíveis com um ficheiro ".htaccess"](htaccess_what_else_can_you_do1.).
...
> 
> Este manual fornece as instruções necessárias para realizar as operações mais habituais. No entanto, se encontrar dificuldades, recomendamos que recorra a um [fornecedor especializado](partner.) e/ou que contacte o editor do serviço. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção ["Quer saber mais?"](htaccess_protect_directory_by_password_#go-further.) deste manual.
>
...
- Ter um [serviço de alojamento web](hosting.).
- Ter acesso à [Área de Cliente OVHcloud](manager.).
- Dispor de credenciais de acesso ao [espaço FTP do seu alojamento](ftp_connection1.).
...
>
> Se utilizar um CMS Wordpress, a OVHcloud também disponibiliza um tutorial sobre como [utilizar o ficheiro htaccess com WordPress](htaccess_how_to_protect_wordpress1.).
>
...
>
> Para isso, contacte a nossa [comunidade de utilizadores](https://community.ovh.com/en/) ou os nossos [parceiros OVHcloud](partner.).
>
...
> 
> Para alterar a versão de PHP dos seus websites no seu alojamento web, consulte consulte o guia "[Alojamento web : ambiente, versão PHP, .ovhconfig](configure_your_web_hosting1.)".
>
...
Ligue-se ao [espaço de armazenamento FTP](ftp_connection1.) do seu alojamento web. Abra a ["pasta raiz"](multisites_configure_multisite1.) para a qual aponta o seu domínio.
...
Se dispõe de um alojamento [Pro](hosting-professional-offer.) ou [Performance](hosting-performance-offer.), aceda em [SSH](ssh_on_webhosting1.) ao seu alojamento web. Desça à "**pasta raiz**" onde está o script "crypt.php".
...
> [!warning]
>
> Por razões de segurança, recomenda-se a utilização de SSH. No entanto, se dispõe de uma oferta **Starter** ou [Perso](hosting-personal-offer.) em que o SSH está indisponível, também pode executar o ficheiro "crypt.php" através do seu browser.
>
...
- `your_ftp_login`: o login FTP que utiliza para se ligar ao seu espaço de armazenamento FTP. Para obter o seu login FTP, consulte o nosso guia sobre a [ligação ao seu espaço FTP](ftp_connection1.).
...
[Tutorial - Operações realizáveis com um ficheiro ".htaccess"](htaccess_what_else_can_you_do1.)
