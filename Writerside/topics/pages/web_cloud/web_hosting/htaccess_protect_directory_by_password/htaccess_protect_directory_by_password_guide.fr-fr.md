...
Ceci en utilisant deux fichiers de configuration (HTTP) Apache à placer dans [l'espace FTP](ftp_connection1.) de votre hébergement Web : 

- « **.htaccess** » : pour plus d'informations sur les fonctionnalités de ce fichier, consultez notre tutoriel sur les [« Opérations réalisables avec un fichier « .htaccess » »](htaccess_what_else_can_you_do1.).
...
- Disposer des identifiants de connexion à [l'espace FTP de votre hébergement](ftp_connection1.).
...
>
> Si vous utilisez un CMS Wordpress, OVHcloud met également à disposition un tutoriel sur comment [utiliser le fichier htaccess avec WordPress](htaccess_how_to_protect_wordpress1.).
>
...
>
> Pour cela, contactez notre [communauté d'utilisateurs](https://community.ovh.com) ou nos [partenaires OVHcloud](partner.).
>
...
> 
> Pour modifier la version de PHP de vos sites web sur votre hébergement web, consultez notre guide « [Hébergement web : environnement, version PHP, « .ovhconfig »](configure_your_web_hosting1.) ».
>
...
Connectez-vous à [l'espace de stockage FTP](ftp_connection1.) de votre hébergement web. Ouvrez le [« dossier racine »](multisites_configure_multisite1.) vers lequel pointe votre nom de domaine.
...
Si vous disposez d'un hébergement [Pro](hosting-professional-offer.) ou [Performance](hosting-performance-offer.), connectez-vous ensuite en [SSH](ssh_on_webhosting1.) à votre hébergement web. Descendez dans le « **dossier racine** » où se trouve votre script « crypt.php ».
...
> [!warning]
>
> Pour des raisons de sécurité, l'utilisation du SSH est recommandée. Toutefois, si vous disposez d'une offre **Starter** ou [Perso](hosting-personal-offer.) sur lesquelles le SSH est indisponible, vous pouvez aussi exécuter le fichier « crypt.php » via votre navigateur Web.
>
...
- `your_ftp_login` : le login FTP que vous utilisez pour vous connecter à votre espace de stockage FTP. Pour récupérer votre login FTP, consultez notre guide sur la [connexion à votre espace FTP](ftp_connection1.).
...
[Se connecter à l'espace FTP de son hébergement Web](ftp_connection1.)

[Tutoriel - Opérations réalisables avec un fichier « .htaccess »](htaccess_what_else_can_you_do1.)
...
[Réécrire l'URL d'accès à mon site grâce au mod_rewrite via le fichier .htaccess](htaccess_url_rewriting_using_mod_rewrite1.)
