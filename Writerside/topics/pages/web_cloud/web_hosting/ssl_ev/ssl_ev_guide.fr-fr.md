...
OVHcloud propose plusieurs types de certificats SSL sur nos offres d'[hébergement mutualisé OVHcloud](hosting.). Ils sont présentés dans notre guide « [Gérer un certificat SSL sur son hébergement web](ssl_on_webhosting1.) ». Les certificats SSL sont incontournables pour la sécurité de votre site web.
...
> [!warning]
>
> Les certificats SSL EV ne sont pas disponibles pour tout le monde. Vérifiez si vous êtes éligible à sa souscription **avant** de commander ce dernier, en vous aidant des éléments indiqués dans les [prérequis](ssl_ev_#requirements.) de ce guide.
>
...
- Commander ou disposer d'un [hébergement mutualisé OVHcloud](hosting.) sur lequel aucun certificat SSL n'est déjà installé.
- Commander ou disposer d'un [nom de domaine](domains.) et disposer des droits exclusifs sur son utilisation. Le nom de domaine ne doit pas déjà être lié à un certificat SSL.
...
Consultez notre guide sur comment [gérer un certificat SSL sur son hébergement web](ssl_on_webhosting1.) et sélectionnez le **Certificat SSL EV** une fois arrivé dans le tunnel de commande.
...
Si vous n'avez pas encore commandé votre nom de domaine et l'hébergement associé, rendez-vous sur notre [page d'accueil OVHcloud](website.), saisissez un nom de domaine dans le **formulaire de recherche prévu à cet effet** puis cliquez sur `Rechercher`{.action} pour démarrer votre commande.
...
Dans le but de s'assurer que les échanges par e-mail avec Sectigo se passent correctement, vérifiez également la validité de l'adresse e-mail renseignée dans le formulaire lors de votre commande du SSL EV, ainsi que l'adresse e-mail de contact associée à votre [espace client OVHcloud](manager.).
...
>
> Nous vous recommandons aussi de **ne pas** activer ou laisser actif le [pare-feu applicatif](multisites_activating_application_firewall1.), disponible avec nos hébergements web, pendant toute la durée de l'installation de votre certificat SSL EV.
>
...
Il ne vous restera plus qu'à [passer votre site en HTTPS](ssl-activate-https-website1.) pour utiliser pleinement votre certificat SSL EV.
...
[Passer votre site en HTTPS](ssl-activate-https-website1.)
