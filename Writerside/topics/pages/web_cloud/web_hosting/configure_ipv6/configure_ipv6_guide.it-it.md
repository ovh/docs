...
- Avere un [dominio](domains.){.external} nello Spazio Cliente OVHcloud
- Disporre di una [offerta di hosting Web](hosting.){.external}
...
Accedi al tuo [Spazio Cliente OVHcloud](manager.){.external}. Nella sezione `Web Cloud`{.action}, clicca su `Hosting`{.action}, seleziona il nome dell'hosting e clicca sulla scheda `Informazioni generali`{.action}.
...
Per consentire al browser di trovare l'indirizzo IPv6 con il tuo dominio, modifica la zona DNS attiva del tuo dominio. Per creare un record DNS di tipo **AAAA**, consulta la guida "[Modificare una zona DNS OVHcloud](dns_zone_edit#modifica-la-zona-dns-ovhcloud-del-dominio.)".
...
[Modifica una zona DNS OVHcloud](dns_zone_edit#modifica-la-zona-dns-ovhcloud-del-dominio.)
