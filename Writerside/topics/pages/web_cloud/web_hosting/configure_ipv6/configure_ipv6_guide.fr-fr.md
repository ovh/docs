...
Connectez-vous à votre [espace client OVHcloud](manager.){.external}. Dans la partie `Web Cloud`{.action}, cliquez sur `Hébergements`{.action}, choisissez le nom de l'hébergement concerné puis positionnez-vous sur l'onglet `Informations générales`{.action}.
...
Afin que votre navigateur trouve l'adresse IPv6 avec votre nom de domaine, modifiez la zone DNS active de votre domaine. Utilisez notre guide « [Éditer une zone DNS OVHcloud](dns_zone_edit#editer-la-zone-dns-ovhcloud-de-votre-nom-de-domaine.) » pour créer une entrée DNS de type **AAAA**.
...
[Éditer une zone DNS OVHcloud](dns_zone_edit#editer-la-zone-dns-ovhcloud-de-votre-nom-de-domaine.)
