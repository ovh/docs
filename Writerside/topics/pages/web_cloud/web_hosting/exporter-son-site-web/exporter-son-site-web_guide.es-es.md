...
Esta guía explica los pasos que debe seguir para exportar todos los elementos de su sitio web al formato estándar desde un [alojamiento web de OVHcloud](hosting.){.external}.
...
- **Cliente FTP o SFTP** : deberá instalar en su ordenador un programa compatible, como [FileZilla ](ftp_filezilla_user_guide1.). Si necesita ayuda, le recomendamos que se ponga en contacto con el editor del programa instalado, ya que OVHcloud no lo ha creado.

- **Acceso SSH**: Para interactuar con su espacio de almacenamiento, deberá ejecutar comandos desde un terminal. Este tipo de acceso requiere conocimientos técnicos avanzados y un [plan de hosting de OVHcloud](hosting.){.external} específico. Para más información, consulte nuestra guía ["Utilizar el acceso SSH de un alojamiento web"](ssh_on_webhosting1.){.external}. 
...
[Exportar una copia de seguridad de la base de datos de un alojamiento web](sql_database_export1.){.external}.
...
[Guardar y exportar una base de datos en un servidor de bases de datos](save-export-on-database-server1.){.external}.
...
[Conectarse al espacio de almacenamiento de un alojamiento web](ftp_connection1.){.external}.

[Cambiar la contraseña de un usuario FTP](ftp_change_password1.){.external}

[Utilizar FileZilla con su alojamiento](ftp_filezilla_user_guide1.){.external}.

[Utilizar el acceso SSH de un alojamiento web](ssh_on_webhosting1.){.external}. 
