...
Pour configurer votre hébergement, connectez-vous tout d'abord à votre [espace client OVHcloud](manager.). Depuis la rubrique `Hébergements`, vous pourrez notamment gérer vos certificats SSL, la version PHP, l'option CDN, le multisite, les bases de données, etc.

**Trucs et Astuces** : Pour vous aider à configurer votre hébergement, nous vous invitons à prendre connaissance de la rubrique « *Premiers pas* » que vous retrouverez [ici](web-cloud-hosting1.).

### Comment gérer mes mots de passe ?

Pour gérer vos mots de passe, vous devez tout d'abord vous connecter à votre [espace client OVHcloud](manager.). En cas d'oubli de votre identifiant ou de votre mot de passe, cliquez sur `Identifiant ou mot de passe oublié`{.action} sous la fenêtre de connexion. Un e-mail vous sera envoyé avec une procédure de réinitialisation.

Vous pouvez également consulter le guide « [Modifier le mot de passe de votre compte](manage-ovh-password1.) ».

Une fois connecté à votre espace client :

- Pour modifier le mot de passe de votre espace FTP, suivez les instructions de [ce guide](ftp_change_password1.).
- Pour modifier le mot de passe de votre base de données, suivez les instructions de [ce guide](sql_change_password1.).
- Pour modifier le mot de passe de votre adresse e-mail MX Plan, suivez les instructions de [ce guide](email_change_password1.).

### Comment mettre en ligne mon site ? 

Pour mettre en ligne votre site, vous devez disposer d'un [nom de domaine](domains.) qui correspondra à l'adresse Web depuis laquelle votre site sera accessible (exemple : *mondomaine.com*). Vous aurez également besoin d'un [hébergement](hosting.) sur lequel installer votre site.

Pour suivre les étapes nécessaires à la construction de votre site, consultez cette [page](hosting-website.) puis suivez les instructions du guide « [Mettre en ligne un site Internet sur son hébergement web](hosting_how_to_get_my_website_online1.) ».

**Trucs et Astuces** : Pour vous aider à créer votre site, OVHcloud vous permet d'installer sur votre hébergement un logiciel d'assistance à la création de sites (WordPress, PrestaShop, Joomla! et Drupal), grâce la fonctionnalité [Modules en 1 clic](cms_install_1_click_modules1.).

### Comment transférer mon site et mes e-mails sur les serveurs OVHcloud ? 

Consultez le guide « [Migrer son site et ses e-mails vers OVHcloud](hosting_migrating_to_ovh1.) ».

### Comment héberger plusieurs sites web sur un même hébergement mutualisé ?

Consultez le guide « [Partager son hébergement entre plusieurs sites](multisites_configure_multisite1.) ».

### Comment changer de formule d'hébergement ?

Afin de commander la formule d'hébergement Web la plus adaptée à vos besoins, vous pouvez consultez nos offres sur [cette page](hosting.).

Une fois votre choix fait, suivez les instructions de notre documentation [« Faire évoluer son offre d’hébergement web »](how_to_upgrade_web_hosting_offer1.).
...
Pour cela, connectez-vous à votre [espace client OVHcloud](manager.). Rendez-vous dans la partie `Web Cloud`{.action} puis sur l'hébergement concerné à gauche dans l'onglet `Hébergements`{.action}. 
...
Retrouvez plus d'informations dans notre guide sur la ["Configuration d'une offre Web Cloud Databases"](configure-database-server#modify-ram-web-cloud-db.)
...
> [!warning]
>
> Si vous rencontrez une anomalie non-répertoriée dans cette FAQ, consultez les pages « Diagnostic » de [notre documentation](web-cloud-hosting1.).
>

### Que faire si mon site web dysfonctionne ? 

Plusieurs raisons peuvent expliquer le dysfonctionnement de votre site. Pour en identifier la cause, commencez par vérifier qu'aucun de vos abonnements n'a besoin d'être **renouvelé** en vous connectant à votre [espace client OVHcloud](manager.).
...
Ce fichier est automatiquement désactivé lors de la création de votre [module en 1 clic](cms_install_1_click_modules1.).

Si vous avez choisi [d'installer votre site manuellement](cms_manual_installation1.), [connectez-vous à votre espace FTP](ftp_connection1.) afin de le renommer en **index.html.old**.
...
Enfin, relancez son installation sur le nom de domaine souhaité, en utilisant la fonctionnalité [Module en 1 clic](cms_manage_1_click_module1.).
...
Si votre site comporte une base de données, consultez notre guide « [Restaurer une sauvegarde de votre base de données](sql_importing_mysql_database#restaurer-une-sauvegarde-depuis-lespace-client.) ».
...
Cette anomalie indique une configuration erronée au niveau des [serveurs DNS](dns_server_general_information1.) ou de la [zone DNS](dns_zone_edit1.) associés à votre nom de domaine.
...
Rendez-vous dans la partie `Domaines`{.action} de votre [espace client OVHcloud](manager.). Cliquez sur le nom de domaine concerné puis sur l'onglet `Serveurs DNS`{.action}.
...
- Si les éléments sont identiques, remplacez la cible `213.186.33.5` par le code à quatre nombres noté dans l'onglet `Informations générales` sous la mention `IPv4` (pour plus de détails sur les manipulations à effectuer, suivez les instructions de [ce guide](dns_zone_edit#en-pratique.)).

- Si les éléments ne sont pas identiques, mais que les `Serveurs DNS` indiqués dans l'onglet du même nom apparaissent dans [cette liste](clusters_and_shared_hosting_IP1.), effectuez une réinitialisation en suivant les instructions de [ce guide](dns_server_general_information#reinitialiser-les-serveurs-dns.).

- Si les éléments ne sont pas identiques et que les `Serveurs DNS` indiqués dans l'onglet du même nom n'apparaissent pas dans [cette liste](clusters_and_shared_hosting_IP1.), contactez votre Webmaster ou recherchez un [prestataire spécialisé](partner.) via la page des [partenaires OVHcloud](partner.).
...
Si les restaurations ne vous permettent pas de rétablir l'accès à votre site, contactez votre Webmaster ou recherchez un [prestataire spécialisé](partner.) sur le site des [partenaires OVHcloud](partner.).
...
Si vous avez activé [l'option CDN](cdn_how_to_use_cdn1.) de votre hébergement, désactivez le mode *Maintenance* sur votre site WordPress ou PrestaShop.

Si vous n'avez pas activé cette option ni utilisé le mode *Maintenance*, contactez votre Webmaster ou recherchez un [prestataire spécialisé](partner.) sur le site des [partenaires OVHcloud](partner.).
...
Ce message indique que le type de requête HTTP que vous tentez de faire sur votre site est interdit pour un temps limité. Dans cette situation, [examinez les logs](logs_and_statistics1.) de votre site afin de déterminer quelles requêtes ont provoqué ce blocage.

Pour vous aider à corriger ces anomalies, contactez votre Webmaster ou l'un de nos [partenaires](partner.).
...
Dans cette situation, [examinez les logs](logs_and_statistics1.) de votre site, afin de déterminer quelles requêtes ont provoqué ce blocage.<br>
Vérifiez également que votre poste informatique n'est pas infecté par un virus.<br>
Vous pouvez enfin contacter l'un de [nos partenaires](partner.), afin qu'il vérifie le code informatique de votre site.
...
[FAQ - E-mails mutualisés MX Plan](faq-emails1.)
