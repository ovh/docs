...
![ftp login](images_tab-pro.png){.thumbnail}
...
> [!primary]
>
> Je nach Ihrem [OVHcloud Webhosting-Angebot](hosting.){.external} werden einige der oben beschriebenen Informationen (insbesondere bezüglich SSH) möglicherweise nicht angezeigt.
>

Wenn Sie das Passwort eines FTP- oder SSH-Benutzers nicht mehr kennen, lesen Sie unsere Anleitung "[Passwort eines FTP-Benutzers ändern](ftp_change_password1.)".

![ftp login](change-password.png){.thumbnail}
...
- [2. Verbindung über einen FTP-Client](#ftpsoftware.): Ermöglicht den Zugriff auf Ihren FTP-Speicherplatz über eine Client-Software (z.B. [FileZilla](ftp_filezilla_user_guide1.) oder [Cyberduck](ftp_cyberduck_user_guide_on_mac1.)). 
Installieren Sie zunächst das gewählte FTP-Programm auf Ihrem Computer.

- [3. Verbindung über einen SSH Zugang](ftp_connection_#ssh.): Ermöglicht den Zugriff auf Ihren FTP-Speicherplatz über einen SSH-Zugang. Für die Nutzung dieses Zugriffstyps sind fortgeschrittene Kenntnisse sowie ein [OVHcloud Webhosting](hosting.){.external} **Pro** oder **Performance** erforderlich.

#### 1. Via "FTP Explorer" verbinden <a name="ftpexplorer"></a>

Um den "FTP-Explorer" zu verwenden, loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.){.external} ein und gehen Sie in den Bereich `Web Cloud`{.action}.
...
![ftp login](ftp-explorer.png){.thumbnail}
...
![ftp login](login-interface.png){.thumbnail}

#### 2. Via FTP-Client verbinden <a name="ftpsoftware"></a>

Nachdem Sie zuvor das FTP-Programm Ihrer Wahl auf Ihrem Computer installiert haben (z.B. [FileZilla](ftp_filezilla_user_guide1.) oder [Cyberduck](ftp_cyberduck_user_guide_on_mac1.), starten Sie es. 
...
Weitere Informationen zu SSH finden Sie in unserer Anleitung zur [Nutzung von SSH mit Ihrem OVHcloud Shared Hosting](ssh_on_webhosting1.).

Ein SSH-Client ist nativ auf *macOS*, *Linux* und *Windows 10* installiert. Eine ältere Windows-Umgebung erfordert die Installation von Software wie [PuTTY](ssh_using_putty_on_windows1.) oder das Hinzufügen der Funktion "*OpenSSH*". 
...
Wenn nötig lesen Sie unsere Anleitung "[SSH-Zugang Ihres Webhostings verwenden](ssh_on_webhosting1.)".

![ftp login](terminal-ssh-connected.png){.thumbnail}

## Weiterführende Informationen

[Passwort eines FTP-Benutzers ändern](ftp_change_password1.)

[SSH-Zugang Ihres Webhostings verwenden](ssh_on_webhosting1.)

[Verwendung von PuTTY für Windows](ssh_using_putty_on_windows1.)

[Tutorial - FileZilla mit Ihrem OVHcloud Hosting nutzen](ftp_filezilla_user_guide1.)

[Tutorial - Cyberduck mit Ihrem OVHcloud Hosting nutzen](ftp_cyberduck_user_guide_on_mac1.)
