...
> [!primary]
>
> Dane te otrzymasz w wiadomości e-mail potwierdzającej instalację hostingu podczas jego rejestracji. Są one dostępne w [Panelu klienta OVHcloud](manager.){.external}.
> 
> **Jeśli dysponujesz już tymi elementami**, przejdź bezpośrednio do etapu 2  [niniejszego przewodnika](#ftp_storage_access.).
> 

Jeśli nie posiadasz tych elementów, zaloguj się do [Panelu klienta OVHcloud](manager.){.external} i przejdź do sekcji `Web Cloud`{.action}. W kolumnie po lewej stronie kliknij pozycję 'Hosting{.action}. Wybierz odpowiedni hosting i przejdź do zakładki `FTP - SSH`{.action}. 
...
> [!primary]
>
> W zależności od oferty [hosting OVHcloud](hosting.){.external}, którą posiadasz, niektóre z informacji opisanych powyżej (zwłaszcza dotyczących SSH) mogą się nie pojawić.
>

Jeśli nie znasz hasła użytkownika FTP lub SSH, zapoznaj się z naszym przewodnikiem "[Zmiana hasła do konta FTP](ftp_change_password1.)".
...
- [2. Połączenie przez program FTP](#ftpsoftware.): umożliwia dostęp do Twojej przestrzeni dyskowej FTP przy użyciu programu (takiego jak [FileZilla](ftp_filezilla_user_guide1.) lub [Cyberduck](ftp_cyberduck_user_guide_on_mac1.)). 
Najpierw zainstaluj wybrany program/klient FTP na komputerze.

- [3. Połączenie przez dostęp SSH](ftp_connection_#ssh.): umożliwia dostęp do Twojej przestrzeni dyskowej FTP przez SSH. Do tego dostępu konieczne są zaawansowane umiejętności techniczne oraz oferta[hostingu OVHcloud](hosting.){.external} **Pro** lub **Performance**.

#### 1. Logowanie przez FTP Explorer <a name="ftpexplorer"></a>

Aby zalogować się do Twojej przestrzeni dyskowej FTP za pomocą "FTP Explorer", zaloguj się do [Panelu klienta OVHcloud](manager.){.external} i przejdź do sekcji `Web Cloud`{.action}.
...
Po zainstalowaniu na komputerze odpowiedniego oprogramowania FTP (takiego jak [FileZilla](ftp_filezilla_user_guide1.) lub [Cyberduck](ftp_cyberduck_user_guide_on_mac1.), uruchom je. 
...
Więcej informacji na temat SSH znajdziesz w naszym przewodniku [korzystanie z SSH na hostingu OVHcloud](ssh_on_webhosting1.)

Narzędzie to jest zainstalowane domyślnie na *macOS*, *Linux* i *Windows 10*. Starsze środowisko Windows będzie wymagało instalacji oprogramowania, takiego jak [PuTTY](ssh_using_putty_on_windows1.) lub dodania funkcji "*OpenSSH*". 
...
Skorzystaj z naszej dokumentacji "[Korzystanie z połączenia SSH na hostingu](ssh_on_webhosting1.)", jeśli to konieczne.

![ftpconnect](terminal-ssh-connected.png){.thumbnail}

## Sprawdź również

[Zmień hasło użytkownika FTP](ftp_change_password1.){.external}.

[Korzystanie z połączenia SSH na hostingu](ssh_on_webhosting1.){.external}.

[Użyj PuTTY do logowania przez SSH](ssh_using_putty_on_windows1.)

[Korzystaj z programu FileZilla na Twoim hostingu](ftp_filezilla_user_guide1.)

[Korzystaj z Cyberduck na Twoim hostingu](ftp_cyberduck_user_guide_on_mac1.)
