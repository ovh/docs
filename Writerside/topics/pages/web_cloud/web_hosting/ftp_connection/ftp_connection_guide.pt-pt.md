...
> [!primary]
>
> Estes elementos foram-lhe enviados por e-mail após a subscrição do seu alojamento web. Estão disponíveis a partir do seu [Área de Cliente OVHcloud](manager.){.external}.
>
> **Se já dispõe destes elementos**, consulte o etapa 2 "[Aceder ao espaço de armazenamento](#ftp_storage_access.)" deste manual.
>

Se não dispõe destes elementos, aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}. Clique na secção `Alojamentos`{.action} na coluna da esquerda. Escolha o nome do alojamento web em causa e aceda ao separador `FTP - SSH`{.action}. 
...
> [!primary]
>
> Dependendo da oferta de [alojamento web OVHcloud](hosting.){.external} que possui, algumas das informações descritas acima (nomeadamente sobre o SSH) podem não aparecer.
>

Se já não sabe a palavra-passe de um utilizador FTP ou SSH, consulte o nosso guia "[Modificar a palavra-passe de um utilizador FTP](ftp_change_password1.)".
...
- [2. Ligação através de um software FTP](#ftpsoftware.): permite aceder ao seu espaço de armazenamento FTP através de um software (como [FileZilla](ftp_filezilla_user_guide1.) ou [Cyberduck](ftp_cyberduck_user_guide_on_mac1.). 
Deverá instalar previamente o software/cliente FTP escolhido no seu computador.

- [3. Ligação via acesso SSH](ftp_connection_#ssh.): permite aceder ao seu espaço de armazenamento FTP através de um acesso SSH. Este tipo de acesso requer conhecimentos técnicos avançados, bem como uma oferta de [alojamento web da OVHcloud](hosting.){.external} **Pro* ou **Performance**.

#### 1. Ligação através de FTP Explorer <a name="ftpexplorer"></a>

Para se ligar ao espaço de armazenamento FTP através do "FTP Explorer", aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}.
...
Depois de instalar o software FTP à sua escolha no seu computador (como [FileZilla](ftp_filezilla_user_guide1.) ou [Cyberduck](ftp_cyberduck_user_guide_on_mac1.), inicie-o. 
...
Encontre mais informações sobre o SSH no nosso guia sobre a [utilização do SSH com o seu alojamento partilhado OVHcloud](ssh_on_webhosting1.)

Esta ferramenta é instalada de forma padrão em *macOS*, *Linux* e *Windows 10*. Um ambiente Windows mais antigo exigirá a instalação de um software como [PuTTY](ssh_using_putty_on_windows1.) ou a adição da funcionalidade "*OpenSSH*". 
...
Para mais informações, consulte o manual "[Utilizar uma ligação SSH num alojamento web](ssh_on_webhosting1.)".

![ftpconnect](terminal-ssh-connected.png){.thumbnail}

## Quer saber mais?

[Modificar a palavra-passe de um utilizador FTP](ftp_change_password1.){.external}.

[Utilizar uma ligação SSH num alojamento web](ssh_on_webhosting1.){.external}.

[Utilizar PuTTY para se ligar em SSH](ssh_using_putty_on_windows1.)

[Utilize o FileZilla com o seu alojamento web](ftp_filezilla_user_guide1.)

[Utilize o Cyberduck com o seu alojamento web](ftp_cyberduck_user_guide_on_mac1.)
