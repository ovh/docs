...
- Posiadanie [domeny](domains.){.external}i możliwość zarządzania nią w Panelu klienta
- Posiadanie [hostingu WWW](hosting.){.external}.
- Dostęp do [Panelu klienta OVHcloud](manager.){.external}.

> [!warning]
>
> Jeśli chcesz aktywować opcję "adres(y) e-mail zawarty(e) w Twoim hostingu" dla zewnętrznej domeny (niezarejestrowanej w OVHcloud), **której administrator**, musisz utworzyć (co najmniej) strefę DNS w OVHcloud dla Twojej zewnętrznej domeny. W razie potrzeby skorzystaj z naszego przewodnika "[Tworzenie strefy DNS w OVHcloud](dns_zone_create1.)". Dzięki temu Twoja domena będzie mogła pojawić się na liście domen dostępnych po aktywacji opcji.
>
> Dodatkowo, jeśli Twoja domena jest już zarejestrowana w OVHcloud lub jeśli jedna strefa DNS już istnieje dla domeny [konto OVHcloud](manager.) niż własne, zapoznaj się z naszą dokumentacją "[Zarządzanie kontaktami dla usług OVHcloud](managing_contacts1.)", aby uzyskać informacje na temat zarządzania domeną.
>
...
Zaloguj się do [Panelu klienta](manager.){.external}, kliknij `Hosting`{.action}, po czym wybierz odpowiedni hosting.
...
> [!primary]
> Jeśli chcesz korzystać z większej liczby kont e-mail zawartych w ofercie, zachęcamy do zamówienia wyższej [oferty hostingu www](hosting.){.external} na naszej stronie handlowej.
>
