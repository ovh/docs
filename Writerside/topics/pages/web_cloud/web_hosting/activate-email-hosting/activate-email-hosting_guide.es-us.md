...
- Disponer de un [dominio](domains.){.external} en su área de cliente de OVHcloud.
...
> [!warning]
>
> Si quiere activar la opción "dirección(s) de correo incluida(s) con su alojamiento web" para un dominio externo (no registrado en OVHcloud) **del que deba ser obligatoriamente administrador**, deberá crear (al menos) una zona DNS en OVHcloud para su dominio externo. Si lo necesita, consulte nuestra guía "[Crear una zona DNS en OVHcloud](dns_zone_create1.)". Al activar esta opción, su dominio aparecerá en la lista de dominios disponibles.
>
> Por otro lado, si su dominio ya está registrado en OVHcloud o si existe una zona DNS para él en otra [cuenta de OVHcloud](manager.) que el suyo, consulte nuestra documentación "[Gestionar los contactos de sus servicios](managing_contacts1.)" para consultar su gestión.
>
...
Conéctese al [área de cliente de OVHcloud](manager.){.external}, haga clic en `Alojamientos`{.action} y seleccione el alojamiento correspondiente.
...
![Activar el correo](enable-email-included-webhosting.png){.thumbnail}
...
![Activar el correo](order-activate-email-included-webhosting-step-1.png){.thumbnail}

> [!primary]
> Si desea disfrutar de más cuentas de correo incluidas, puede contratar un [plan de hosting](hosting.){.external} superior desde nuestra página comercial.
>
...
![Desvincular el correo](detach-email-included-webhosting.png){.thumbnail}
...
![Dar de baja el correo](cancel-email-included-webhosting.png){.thumbnail}
