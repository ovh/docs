...
- Disposer d'une [offre d'hébergement web OVHcloud](hosting.) incluant un accès SSH. Cet accès permet d'installer en ligne de commande une ou plusieurs solutions alternatives à celles proposées par défaut dans nos offres d'hébergements web.
- Être familiarisé à la saisie en ligne de commande.
- Être en capacité de transférer des fichiers en FTP avec un client comme [FileZilla](ftp_filezilla_user_guide1.).
- Paramétrer votre zone DNS pour faire pointer votre nom de domaine (ou sous-domaine) vers votre hébergement web mutualisé. Cela s'avère notamment utile si vous souhaitez héberger plusieurs sites en [multisites](multisites_configure_multisite1.) sur votre hébergement web mutualisé.
...
Les [hébergements web mutualisés](hosting.) permettent de déclarer des domaines ou sous-domaines en multisites. Un domaine ou un sous-domaine est nécessaire pour déployer votre site web réalisé avec **Cecil**.

Pour vous aider à déclarer un domaine ou un sous-domaine en multisite sur votre hébergement, consultez notre page « [Partager son hébergement entre plusieurs sites](multisites_configure_multisite1.) ».
...
![Installation Cecil](static_website_installation_cecil01.png){.thumbnail}
...
![Installation Cecil](static_website_installation_cecil02.png){.thumbnail}

Vous pouvez maintenant voir le résultat en vous rendant sur votre nom de domaine:

![Installation Cecil](static_website_installation_cecil03.png){.thumbnail}
...
Si votre nom de domaine ou sous-domaine est hébergé chez OVHcloud, consultez nos guides relatifs à la [configuration DNS](dns_zone_edit1.) et à la mise en place d'un [multisites sur votre hébergement web](multisites_configure_multisite1.).
...
![Fichier de configuration YAML](static_website_installation_cecil04.png){.thumbnail}
...
![Installation Cecil](static_website_installation_cecil05.png){.thumbnail}
...
![Installation Cecil](static_website_installation_cecil06.png){.thumbnail}

Vous pouvez le visualiser sur votre serveur en tapant l'URL de votre site, suivi de `/mypage/`:

![Résultat navigateur](static_website_installation_cecil07.png){.thumbnail}

### Personnaliser les fichiers de votre site

#### Modification sur le serveur

L'édition des fichiers _Markdown_ peut se faire directement sur le serveur d'hébergement web. Sur votre offre d'[hébergement Performance](hosting-performance-offer.), votre accès SSH pour permet d'utiliser indifféremment [GNU nano](https://nano-editor.org/){.external}, [vi](https://ex-vi.sourceforge.net/){.external} ou [vim](https://www.vim.org/){.external}.
...
![Édition du fichier dans GNU nano](static_website_installation_cecil08.png){.thumbnail}

Ajoutez quelques lignes en respectant la syntaxe _Markdown_:

![Ajout de contenu dans le fichier](static_website_installation_cecil09.png){.thumbnail}
...
![Page mise à jour](static_website_installation_cecil10.png){.thumbnail}
...
![Téléchargement avec FileZilla](cecil-folder.png){.thumbnail}

Vous pouvez maintenant éditer les fichiers dans votre I.D.E.:

![Affichage dans Visual Studio Code](static_website_installation_cecil12.png){.thumbnail}
...
[Ajouter du contenu dynamique dans une page web statique générée avec Cecil](static_website_installation_cecil_api_call1.)
...
Notre [guide sur l'utilisation de FileZilla](ftp_filezilla_user_guide1.)
