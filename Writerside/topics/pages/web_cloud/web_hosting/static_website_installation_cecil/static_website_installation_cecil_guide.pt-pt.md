...
- Ter um [plano de alojamento web da OVHcloud](hosting.) que inclui um acesso SSH. Este acesso permite instalar em linha de comandos uma ou várias soluções alternativas às que são propostas por predefinição nas nossas ofertas de alojamento web.
- estar familiarizado com a introdução em linha de comandos.
- Ter capacidade para transferir ficheiros em FTP com um cliente como [FileZilla](ftp_filezilla_user_guide1.).
- Parametrizar a sua zona DNS para apontar o seu nome de domínio (ou subdomínio) para o seu alojamento web partilhado. Isto é particularmente útil se deseja alojar vários sites em [multi-sites](multisites_configure_multisite1.) no seu alojamento web partilhado.
...
Os [alojamentos web partilhados](hosting.) permitem declarar domínios ou subdomínios em multi-sites. Um domínio ou um subdomínio é necessário para implementar o seu website realizado com **Cecil**.

Para o ajudar a declarar um domínio ou um subdomínio em multi-site no seu alojamento, consulte a nossa página "[Partilhar o alojamento entre vários sites](multisites_configure_multisite1.)".
...
![Instalação Cecil](static_website_installation_cecil01.png){.thumbnail}
...
![Instalação Cecil](static_website_installation_cecil02.png){.thumbnail}

Agora pode ver o resultado ao aceder ao seu domínio:

![Instalação Cecil](static_website_installation_cecil03.png){.thumbnail}
...
Se o seu domínio ou subdomínio estiver alojado na OVHcloud, consulte os nossos guias relativos à [configuração DNS](dns_zone_edit1.) e à implementação de um [multisite no seu alojamento web](multisites_configure_multisite1.).
...
![Ficheiro de configuração YAML](static_website_installation_cecil04.png){.thumbnail}
...
![Instalação Cecil](static_website_installation_cecil05.png){.thumbnail}
...
![Instalação Cecil](static_website_installation_cecil06.png){.thumbnail}

Pode visualizá-lo no seu servidor introduzindo o URL do seu site, seguido de `/mypage/`:

![Resultado browser](static_website_installation_cecil07.png){.thumbnail}

### Personalizar os ficheiros do seu site

#### Modificação no servidor

A edição dos ficheiros _Markdown_ pode ser feita diretamente no servidor de alojamento web. Na sua oferta de [alojamento Performance](hosting-performance-offer.), o seu acesso SSH permite utilizar indiferentemente [GNU nano](https://nano-editor.org/){.external}, [vi](https://ex-vi.sourceforge.net/){.external} ou [vim](https://www.vim.org/){.external}.
...
![Edição do ficheiro em GNU nano](static_website_installation_cecil08.png){.thumbnail}

Adicione algumas linhas respeitando a syntaxe _Markdown_:

![Adicionar conteúdo ao ficheiro](static_website_installation_cecil09.png){.thumbnail}
...
![Página atualizada](static_website_installation_cecil10.png){.thumbnail}
...
![Descarregamento com FileZilla](cecil-folder.png){.thumbnail}

Pode agora editar os ficheiros no seu I.D.E.:

![Visual Studio Code](static_website_installation_cecil12.png){.thumbnail}
...
[Adicionar conteúdo dinâmico a uma página web estática gerada com Cecil](static_website_installation_cecil_api_call1.)
...
Nosso [guia sobre a utilização do FileZilla](ftp_filezilla_user_guide1.)
