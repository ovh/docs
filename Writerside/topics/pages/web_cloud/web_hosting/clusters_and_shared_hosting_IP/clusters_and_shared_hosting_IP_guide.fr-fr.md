...
> C'est à dire qu'elles n'ont pas besoin de géolocalisation ([Plus d'informations ici](hosting-options-cdn.)).
> 
...
Pour connaître le cluster d'hébergement web sur lequel se trouve votre service, connectez-vous à l'[espace client OVHcloud](manager.) et sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action}, puis choisissez l'hébergement Web concerné. Ensuite, accédez à l'onglet `FTP - SSH`{.action}.
...
[Mettre en place un multisite sur votre hébergement Web](multisites_configure_multisite1.)

[Activer le HTTPS avec votre certificat SSL sur votre hébergement Web](ssl-activate-https-website1.)

[Optimiser les performances de votre site Web](optimise_your_website_performance1.)
