...
> Isso significa que não precisam de geolocalização ([Mais informações aqui](hosting-options-cdn.)).
> 

## Requisitos

- Ter um serviço de [alojamento web OVHcloud](hosting.){.external}.
- Ter acesso à [Área de Cliente OVHcloud](manager.){.external}.

## Instruções

Para conhecer o cluster de alojamento web no qual se encontra o seu serviço, aceda à [Área de Cliente OVHcloud](manager.) e selecione a `Web Cloud`{.action}. Clique em `Alojamentos`{.action} e escolha o alojamento web em causa. A seguir, aceda ao separador `FTP - SSH`{.action}.
...
[Implementar um multi-site no seu alojamento Web](multisites_configure_multisite1.)

[Ativar o HTTPS com o seu certificado SSL no seu alojamento Web](ssl-activate-https-website1.)

[Otimizar as performances do seu website](optimise_your_website_performance1.)
