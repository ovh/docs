...
> ovvero non hanno bisogno di geolocalizzazione ([Maggiori informazioni qui](hosting-options-cdn.)).
> 
...
Per conoscere il cluster di hosting Web su cui si trova il tuo servizio, accedi allo [Spazio Cliente OVHcloud](manager.) e seleziona `Web Cloud`{.action}. Seleziona il tuo servizio nella sezione `Hosting`{.action}. In seguito accedi alla scheda `FTP - SSH`{.action}.
...
[Installare un multisito sul tuo hosting Web](multisites_configure_multisite1.) 

[Attiva l'HTTPS con il tuo certificato SSL sul tuo hosting Web](ssl-activate-https-website1.)

[Ottimizza le performance del tuo sito Web](optimise_your_website_performance1.)
