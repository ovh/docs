...
> Esto quiere decir que no necesitan geolocalización ([Más información aquí](hosting-options-cdn.)).
> 

## Requisitos

- Haber iniciado sesión en el [área de cliente de OVHcloud](manager.){.external}.
- Tener contratado un [plan de alojamiento web](hosting.){.external}.

## Procedimiento

Para conocer el cluster de alojamiento web en el que se encuentra su servicio, conéctese al [área de cliente de OVHcloud](manager.) y seleccione `Web Cloud`{.action}. Haga clic en `Alojamientos`{.action} y seleccione el alojamiento correspondiente. A continuación, abra la pestaña `FTP - SSH`{.action}.
...
[Crear un multisitio en un alojamiento web](multisites_configure_multisite1.)

[Activar el HTTPS con el certificado SSL en un alojamiento web](ssl-activate-https-website1.)
