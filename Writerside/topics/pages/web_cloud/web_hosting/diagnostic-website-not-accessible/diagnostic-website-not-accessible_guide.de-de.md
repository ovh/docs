...
Im Browser können mehrere Fehlermeldungen auftreten, wenn Ihre Website nicht erreichbar ist. Die folgenden Beispiele zeigen eine fehlerhafte [DNS-Konfiguration](dns_server_general_information#dns-konzept-verstehen.) oder eine angebrochene Domain an (wenn Ihre Website keine der hier beschriebenen Fehlermeldungen zeigt, lesen Sie den Abschnitt [Weiterführende Informationen](diagnostic-website-not-accessible_#go-further.)):

|Browser|Betreffende Fehlermeldung|
|-|---|
|Chrome:<br>"Die Website ist nicht erreichbar"|![cantbereached_chrome](cant-be-reached-chrome.png){.thumbnail}|
|Firefox:<br>"Seite wurde nicht gefunden"|![cantbereached_firefox](cant-be-reached-firefox.png){.thumbnail}|
|Edge:<br>"Hmmm...diese Seite ist leider nicht erreichbar"|![cantbereached_edge](cant-be-reached-edge.png){.thumbnail}|
|Safari:<br>"Safari kann keine Verbindung zum Server aufbauen"|![cantbereached_safari](cant-be-reached-safari.png){.thumbnail}|
...
> 
> Diese Anleitung soll Sie bei allgemeinen Aufgaben bestmöglich unterstützen. Dennoch empfehlen wir Ihnen, falls Sie Hilfe brauchen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren und/oder Ihre Fragen in der OVHcloud Community zu stellen. Leider können wir Ihnen für administrative Aufgaben keine weitergehende technische Unterstützung anbieten. Weitere Informationen finden Sie am [Ende dieser Anleitung](diagnostic-website-not-accessible_#go-further.).
>
...
>
> Daher empfehlen wir Ihnen dringend, die [automatische Verlängerung](how_to_use_automatic_renewal#in-der-praktischen-anwendung.) für alle Ihre OVHcloud Abonnements zu aktivieren.
>

Um die Gültigkeit des Abonnements für Ihre Domain zu überprüfen, klicken Sie oben rechts in Ihrem [OVHcloud Kundencenter](manager.) auf Ihren Namen, um das Kontextmenü anzuzeigen, und dann auf `Produkte und Dienstleistungen`{.action}.
...
Um die Gültigkeit Ihrer [DNS Server](dns_server_general_information1.) zu überprüfen, klicken Sie in Ihrem [OVHcloud Kundencenter](manager.) auf `Domainnamen`{.action} und dann auf die Domain Ihrer Website.
...
Wenn die Servernamen mit den Zielen der `NS`-Einträge in der `DNS-Zone`{.action} identisch sind, gehen Sie zu [Schritt 3](diagnostic-website-not-accessible_#step3.).
...
Ändern Sie die DNS Server entsprechend den Anweisungen in dieser [Anleitung](dns_server_general_information#dns-server-andern.), damit diese mit den Zielen der Einträge vom Typ `NS` in der `DNS-Zone`{.action} identisch sind.
...
Wenn Ihre Website auf einem unserer [Hosting-Angebote](hosting.) gehostet wird, wählen Sie aus `Hosting-Pakete`{.action} den entsprechenden Dienst aus.
...
Tragen Sie diese dann in die [DNS-Zone](dns_zone_edit#bearbeiten-der-ovhcloud-dns-zone-ihrer-domain.) Ihrer Domain ein, indem Sie einen oder mehrere Einträge vom Typ `A` bearbeiten oder erstellen.
