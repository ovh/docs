...
With an OVHcloud Web Hosting plan, you can get an [SSL certificate](hosting-options-ssl.). This allows one or more of your websites to have a secure connection by being accessible in *HTTPS*. To do this, you will need to follow several steps before your websites can use this secure connection.

**This guide explains how to switch your website to HTTPS after activating an SSL certificate.**

## Requirements

- An [SSL certificate](hosting-options-ssl.){.external} set up on your [OVHcloud Web Hosting plan](hosting.){.external}
- At least one website set up and accessible via your OVHcloud Web Hosting plan
- Access to the [OVHcloud Control Panel](manager.){.external}, part `Web Cloud`{.action}
...
>
> This guide is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist service provider](partner.) if you encounter any difficulties. We will not be able to assist you. You can find more information in the [Go further](ssl-activate-https-website_#go-further.) section of this guide.
>
...
To activate an SSL certificate on your Web Hosting plan, or check that an SSL certificate has already been set up for your website, please read our guide on "[Managing an SSL certificate on a Web Hosting plan](ssl_on_webhosting1.)".
...
The information below is generic. We recommend contacting a [specialist provider](partner.) if you experience any difficulties.
...
As soon as your Web Hosting plan has an active SSL certificate, and the [multisite](multisites_configure_multisite1.) concerned has an active SSL connection, and your website is ready to switch to *HTTPS*, you can activate it.

> [!warning]
>
> Before you take any steps, we recommend that you retrieve a full backup of your website. This backup must contain not only the files on the [FTP storage space](ftp_save_and_backup1.), but also those from the [database](sql_database_export1.) if the website uses one.
>
> You can do this directly from the files that make up your website. Do not hesitate to contact a [specialist provider](partner.) if you encounter any difficulties.
>
...
**Warning**, for [Cloud Web](hosting-cloud-web-offer.) hosting plans, the script to use is as follows:
...
[Managing an SSL certificate on a Web Hosting plan](ssl_on_webhosting1.)
