...
> 
> Colocamos à sua disposição este tutorial para o acompanhar o melhor possível em tarefas comuns. No entanto, se precisar de ajuda, recomendamos que recorra a um [fornecedor especializado](partner.) ou [editor do CMS WordPress](https://wordpress.com/support/){.external}. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção [Quer saber mais?](htaccess_how_to_protect_wordpress_#go-further.) deste tutorial.
>
...
- Ter capacidade para utilizar um cliente FTP como [FileZilla](https://filezilla-project.org/). Pode consultar o nosso guia "[Utilizar o FileZilla](ftp_filezilla_user_guide1.)".
...
>
> Se não utiliza um CMS, a OVHcloud disponibiliza também um tutorial para a [proteção de um diretório ou da interface de administração do seu website através de ficheiros .htaccess](htaccess_protect_directory_by_password1.).
>
> Para qualquer questão relativa à criação, utilização ou programação do seu website, contacte a nossa [comunidade de utilizadores](https://community.ovh.com/en/) ou os nossos [parceiros OVHcloud](partner.). As equipas de suporte da OVHcloud não poderão ajudá-lo nestas questões.
>
...
> [!warning]
>
> Antes de seguir os passos abaixo, é necessário reencaminhar o protocolo HTTP para HTTPS. Para isso, siga as instruções do nosso manual ["Ativar o HTTPS num website com certificado SSL"](ssl-activate-https-website#1-ativar-o-certificado-ssl-no-alojamento.).
...
Para mais informações, consulte o nosso guia sobre a [restrição do acesso por IP através do ficheiro .htaccess](htaccess_how_to_block_a_specific_ip_address_from_accessing_your_website1.).
