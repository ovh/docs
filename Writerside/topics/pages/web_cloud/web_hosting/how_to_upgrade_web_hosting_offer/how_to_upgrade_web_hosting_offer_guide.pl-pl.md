...
[Panel klienta OVHcloud](manager.) pozwala na zwiększenie wydajności Twoich [ofert hostingu www](hosting.). Możesz również:

- wydajniejszego hostingu;
- więcej przestrzeni dyskowej FTP;
- dodatkowych baz danych; 
- dodatkowych adresów e-mail;
- dodatkowe funkcje, takie jak [listy mailingowe](feature_mailing_list1.) (od [oferta Pro](hosting-professional-offer.)) lub [usługa Web Cloud Databases](databases.){.external} (zawarte w [ofertach Performance](https://www.ovhcloud.com/pl/web/hosting/performance-hosting/performance-offer/).
...
- Posiadanie co najmniej kontaktu "[Administrator](managing_contacts1.)" dla usług, dla których chcesz zmienić subskrypcję.
...
Aby zmienić abonament, przejdź do [Panelu klienta OVHcloud](manager.) w części `Web Cloud`{.action}. Kliknij opcję `Hosting`{.action} i wybierz odpowiedni hosting.
...
Sprawdź, czy nowa oferta zawiera wystarczającą liczbę [baz danych](hosting-options-startsql.). Sprawdź również, czy ich rozmiary są wystarczające.

W przeciwnym razie usuń nieużywane bazy danych i w razie potrzeby zmniejsz ilość danych w nich zawartych. Ilość ta nie może przekroczyć maksymalnego rozmiaru baz danych w nowej ofercie. Jeśli potrzebujesz wsparcia w zakresie wymaganych działań, skontaktuj się z [partnerami OVHcloud](partner.).

Po usunięciu danych z Twoich baz, ponownie oblicz wykorzystany limit. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie kliknij opcję `Hosting`{.action}, a następnie wybierz odpowiedni hosting. Na stronie, która się wyświetli przejdź do zakładki `Bazy danych`{.action}, następnie kliknij przycisk`...`{.action} po prawej stronie odpowiedniej bazy danych, a następnie `Przelicz rozmiar bazy`{.action}.
...
Jeśli korzystasz z oferty [Web Cloud Databases](starting_with_clouddb1.) zawartej w Twoim hostingu [Performance](hosting-performance-offer.) i chcesz przenieść swój hosting na ofertę [Pro](hosting-professional-offer.), musisz najpierw oddzielić ofertę Web Cloud Databases od hostingu.
W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie kliknij opcję `Hosting`{.action}, a następnie wybierz odpowiedni hosting. Na stronie, która się wyświetli pozostań w zakładce `Informacje ogólne`{.action}. W kolumnie środkowej `Konfiguracja` kliknij przycisk `...`{.action} po prawej stronie pozycji `Web Cloud Databases`{.action}, a następnie `Odłącz`{.action}.
...
1. Wykonaj kopię zapasową danych, postępując zgodnie z instrukcjami zawartymi w tym [przewodniku](save-export-on-database-server1.).<br>
2. Usuń serwer Web Cloud Databases za pomocą [Panelu klienta OVHcloud](manager.). W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.), kliknij Twoją nazwę w prawym górnym rogu, a następnie ikonę `Produkty i usługi`{.action}. Następnie kliknij przycisk `...`{.action} po prawej stronie wiersza oferty Web Cloud Databases/Private SQL . `Usuń mój hosting Private SQL`{.action}.

#### 3 - Przestrzeń dyskowa FTP

Upewnij się, czy nowa oferta zawiera wystarczającą [przestrzeń dyskową FTP](ftp_connection1.), aby umożliwić import plików z Twojego aktualnego hostingu.

Aby sprawdzić rozmiar przestrzeni dyskowej FTP użytej na Twoim hostingu, zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie kliknij `Hosting`{.action}, a następnie wybierz odpowiedni hosting. Na stronie `Informacje ogólne`{.action}, która się wyświetla, znajdź limit w sekcji `Przestrzeń dyskowa`.

![ftp](how_to_upgrade_web_hosting_offer_images_find-disk-space.png){.thumbnail}

#### 4 - Adresy e-mail

Upewnij się, że w nowym pakiecie jest dostępna wystarczająca liczba dostępnych kont e-mail. W przeciwnym razie usuń nieużywane adresy e-mail po wykonaniu kopii zapasowej [kopia zapasowa](manual_email_migration1.) ich zawartości, jeśli jest to konieczne.

Jeśli chcesz zachować tę samą liczbę kont e-mail i **przed zmianą hostingu na niższą ofertę**, możesz również zamówić nową usługę poczty elektronicznej **MX Plan**. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W kolumnie po lewej stronie kliknij `E-maile`{.action}, po czym wybierz odpowiednią ofertę e-mail. Na stronie, która się wyświetla w ramce `Abonament`{.action} i po prawej stronie wzmianki `Usługa`{.action} kliknij przycisk `...`{.action} następnie `Zmień ofertę`{.action}.

![mxplan](images_change-solution.png){.thumbnail}

>[!primary]
>
> Jeśli przycisk `...`{.action} jest niedostępny w Twojej ofercie e-mail, możesz odłączyć ofertę e-mail od hostingu. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.) w części `Web Cloud`{.action}. W lewej kolumnie kliknij `Hosting`{.action}, a następnie wybierz odpowiedni hosting. Na stronie `Informacje ogólne`{.action}, która się wyświetla i w ramce `Konfiguracja`{.action}, kliknij przycisk `...`{.action} z prawej strony wzmianki `Adresy e-mail`{.action}, a następnie `Odłącz opcję e-mail`{.action}.
>

#### 5 - Listy mailingowe

Funkcja [Mailing Lists](feature_mailing_list1.) jest opcjonalna dla hostingu [Perso](hosting-personal-offer.).

Aby zmienić pakiet hostingowy na ofertę [Perso](hosting-personal-offer.), należy najpierw usunąć listy mailingowe lub zamówić ofertę poczty elektronicznej zawierającą tę funkcję (**MX Plan 100** lub **MX Plan Full**) w [Panelu klienta OVHcloud](manager.).

W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W kolumnie po lewej stronie kliknij `E-maile`{.action}, po czym wybierz odpowiednią ofertę e-mail. Na stronie, która się wyświetla w ramce `Abonament`{.action} i po prawej stronie wzmianki `Usługa`{.action} kliknij przycisk `...`{.action} następnie `Zmień ofertę`{.action}.
...
> **Włącz opcję Boost**
>>
>> Aby aktywować opcję Boost, zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie kliknij `Hosting`{.action}, a następnie wybierz odpowiedni hosting. W ramce `Informacje ogólne` na stronie, która się wyświetli kliknij przycisk`...`{.action} po prawej stronie `Boost`, a następnie `Skorzystaj z opcji Boost`{.action}.<br><br>
>> ![boost](boost-my-hosting-plan.png){.thumbnail}<br>
>>
> **Wyłącz opcję Boost**
>>
>> Aby wyłączyć opcję Boost, zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie kliknij `Hosting`{.action}, a następnie wybierz odpowiedni hosting. Na stronie, która się wyświetli przejdź do zakładki `Więcej` i kliknij `Skorzystaj z opcji Boost`{.action}.<br>
...
> **Przykład:**<br>
>
> Zamówiłeś abonament [Perso](hosting-personal-offer.) od 1 stycznia 2022.
>
> 31 października 2022 zmienisz abonament z tej oferty **Perso** na ofertę [Pro](hosting-professional-offer.).<br>
>
...
[Sprawdzanie statystyk i logów strony hostowanej w ramach oferty hostingowej](logs_and_statistics1.)
