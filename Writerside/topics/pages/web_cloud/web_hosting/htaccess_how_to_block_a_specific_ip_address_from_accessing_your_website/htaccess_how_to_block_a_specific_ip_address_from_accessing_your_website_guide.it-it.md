...
Puoi creare diversi file ".htaccess" in [lo spazio FTP](ftp_connection1.) del tuo hosting ma **un solo** per directory o sottodirectory per evitare conflitti tra diversi file ".htaccess".
...
>
> Per modificare (o creare) queste directory, accedi allo spazio FTP del tuo hosting. Se necessario, consulta la guida [Accedi al tuo spazio di storage](ftp_connection1.).
>
...
In caso di errore, accedi allo [spazio FTP](ftp_connection1.) dell'hosting per correggere la situazione. 
...
- [Proteggere l'interfaccia di gestione del tuo sito tramite il ".htaccess"](htaccess_protect_directory_by_password1.).
