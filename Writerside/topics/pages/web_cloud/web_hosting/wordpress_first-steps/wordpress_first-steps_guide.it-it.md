...
> 
> Mettiamo a tua disposizione questo tutorial per supportarti nelle operazioni più frequenti. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a [fornitore specializzato](partner.) o [amministratore del CMS WordPress](https://wordpress.com/support/){.external}. OVH non potrà fornirti alcuna assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](wordpress_first-steps_#go-further.) di questo tutorial.
>
...
- Aver [installato Wordpress](cms_install_1_click_modules1.) sul tuo hosting Web
- Avere accesso allo[Spazio Cliente OVHcloud](manager.){.external}

## Procedura

Se non lo hai già fatto e prima di continuare, [aggiungi un certificato SSL](ssl-activate-https-website#step-1-attiva-il-certificato-ssl-sullhosting.) sul dominio associato al tuo sito Web.
...
![WordPress - Appearance/Themes](dashboard-themes.png){.thumbnail}

Scegli un tema tra quelli proposti e clicca su `Attiva`{.action}:

![WordPress - Appearance/Themes](themes.png){.thumbnail}
...
![WordPress - Gutenberg](post-editor.png){.thumbnail}
...
![WordPress - Categorie](categories.png){.thumbnail}
...
![WordPress - Categorie filled](categories-2.png){.thumbnail}

Una volta inserite le informazioni, clicca su `Aggiungi una nuova categoria`{.action}:

![WordPress - Categorie added](categories-3.png){.thumbnail}
...
![WordPress - Sub-estina](categories-4.png){.thumbnail}
...
![WordPress - Set new categto an an existing post](posts.png){.thumbnail}
...
![WordPress - GB to page](pages.png){.thumbnail}
...
![WordPress - Pagine, Gutenberg page builder](pages-editor.png){.thumbnail}

Crea il contenuto della tua pagina e pubblicalo:

![WordPress - Pagine, content](post-editor-4.png){.thumbnail}
...
![WordPress - Settings/Permalinks](dashboard-users-permalinks.png){.thumbnail}
