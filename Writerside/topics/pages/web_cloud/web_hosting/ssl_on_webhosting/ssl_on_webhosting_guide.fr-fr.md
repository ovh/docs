...
- Avoir accès à votre [espace client OVHcloud](manager.){.external}, partie « Web ».
...
Selon le [certificat SSL](hosting-options-ssl.){.external} que vous souhaitez commander, vous pouvez activer une connexion SSL sécurisée sur un ou plusieurs de vos multisites. Pour ce faire, connectez-vous à votre [espace client OVHcloud](manager.){.external} et sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action}, puis choisissez l'hébergement web concerné. Positionnez-vous ensuite sur l'onglet `Multisite`{.action}.
...
|État|Description|
|---|---|
|Activé|Un certificat SSL a déjà été activé pour ce multisite. Cependant, si votre site web n'est pas disponible en HTTPS, reportez-vous aux instructions répertoriées dans notre guide « [Passer son site en HTTPS grâce au SSL](ssl-activate-https-website1.){.external} ».|
...
>
> Pour plus de détails, n'hésitez pas à consulter nos guides [sur la configuration d'une entrée multisite](multisites_configure_multisite1.) ou sur [la configuration d'une zone DNS](dns_zone_edit#les-enregistrements-dns.).

Votre hébergement web OVHcloud vous permet d'activer un [certificat SSL selon plusieurs solutions](hosting-options-ssl.){.external} :

- un certificat SSL gratuit Let's Encrypt [inclus avec une offre d'hébergement web compatible](hosting-options-ssl.){.external} ;
- un certificat SSL payant [en option avec un hébergement web compatible](hosting-options-ssl.){.external} ;
- l'importation d'un certificat SSL commandé auprès d'un autre fournisseur.

Pour activer votre certificat, connectez-vous à votre [espace client OVHcloud](manager.){.external} et sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action} puis sélectionnez l'hébergement concerné. Cliquez sur l'onglet `Informations générales`{.action}. Sous l'onglet « Certificat SSL », la mention « Non » devrait apparaître, indiquant qu'aucun certificat SSL n'a été configuré ni installé sur votre hébergement web.
...
Dans la fenêtre qui apparaît, sélectionnez le certificat SSL que vous souhaitez commander. Selon votre [offre d'hébergement web](hosting.){.external} et sa configuration, il se peut qu'aucune des solutions répertoriées ci-dessous ne soit disponible. Une fois que vous avez sélectionné une option, cliquez sur le bouton `Suivant`{.action}.
...
> [!primary]
>
> Cette opération s'applique uniquement aux certificats SSL gratuit Let's Encrypt [inclus avec une offre d'hébergement web compatible](hosting-options-ssl.) permettant d'activer une connexion SSL sécurisée pour plusieurs multisites.
>
...
Pour récupérer l'adresse IP de votre hébergement web, consultez notre guide « [Liste des adresses IP des clusters et hebergements web](clusters_and_shared_hosting_IP1.) ».
Pour éditer la zone DNS active de votre nom de domaine, consultez notre guide « [Éditer une zone DNS OVHcloud](dns_zone_edit1.) ».
...
Pour corriger cela, consultez notre tutoriel « [Réécrire l'URL d'accès à mon site grâce au mod_rewrite via le fichier .htaccess](htaccess_url_rewriting_using_mod_rewrite1.) » ou faites appel à un [prestataire spécialisé](partner.) si vous épprouvez des difficultés.
...
Si besoin, consultez notre tutoriel « [Hébergement web : passer son site web en HTTPS](ssl-activate-https-website1.) » ou faites appel à un [prestataire spécialisé](partner.) si vous épprouvez des difficultés.
...
Si besoin, consultez notre guide « [Utiliser un certificat SSL EV pour votre site Web](ssl_ev1.) » pour résoudre cette situation.
