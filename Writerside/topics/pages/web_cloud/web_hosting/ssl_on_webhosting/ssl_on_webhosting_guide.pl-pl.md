...
- Dostęp do [Panelu klienta OVHcloud](manager.){.external}, sekcja `Web Cloud`{.action}
...
W zależności od [certyfikatu SSL](hosting-options-ssl.){.external}, który chcesz zamówić, możesz aktywować bezpieczne połączenie SSL na jednej lub kilku stronach podpiętych w opcji MultiSite. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.){.external} i wybierz `Web Cloud`{.action}. Kliknij `Hosting`{.action}, po czym wybierz odpowiedni hosting. Przejdź następnie do zakładki `MultiSite`{.action}.
...
>
> Aby uzyskać więcej informacji, zapoznaj się z naszymi przewodnikami [dotyczącymi konfiguracji wpisu w opcji MultiSite](multisites_configure_multisite1.) lub [konfiguracji strefy DNS](dns_zone_edit1.).

Twój hosting umożliwia uruchomienie [certyfikatu SSL w zależności od potrzeb](hosting-options-ssl.){.external}:

- bezpłatny certyfikat SSL Let's Encrypt [zawarty w ofercie kompatybilnego hostingu](hosting-options-ssl.){.external};
- płatny certyfikat SSL [w opcji z kompatybilnym hostingiem](hosting-options-ssl.){.external};
- import certyfikatu SSL uzyskanego we własnym zakresie.

Przed rozpoczęciem operacji zaloguj się do [Panelu klienta](manager.){.external}i wybierz `Web Cloud`{.action}. Kliknij `Hosting`{.action}, po czym wybierz nazwę odpowiedniego hostingu. Następnie przejdź do sekcji `Informacje ogólne`{.action}. W zakładce „Certyfikat SSL” powinna pojawić się informacja „Nie” wskazująca, że na Twoim hostingu nie został zainstalowany żaden certyfikat SSL.
...
W oknie, które się pojawi wybierz certyfikat SSL, który chcesz zamówić. Nie wszystkie rozwiązania wymienione powyżej mogą być dostępne. Zależy to od wykupionego przez Ciebie [pakietu hostingowego](hosting.){.external} lub konfiguracji. Po wybraniu opcji kliknij `Dalej`{.action}.
...
> [!primary]
>
> Operacja ta dotyczy wyłącznie certyfikatów SSL Let's Encrypt [zawartych w ofercie kompatybilnego hostingu](hosting-options-ssl.) umożliwiających aktywację bezpiecznego połączenia SSL dla kilku stron podpiętych w opcji MultiSite.
>
...
Zaloguj się do [Panelu klienta](manager.){.external} i wybierz `Web Cloud`{.action}. Kliknij `Hosting`{.action}, po czym wybierz odpowiedni hosting. Następnie przejdź do sekcji `Informacje ogólne`{.action}. Kliknij trzy kropki obok napisu „Certyfikat SSL”, a następnie kliknij `Wygeneruj ponownie certyfikat SSL`{.action}.
...
Gdy będziesz gotowy do usunięcia certyfikatu SSL zaloguj się do [Panelu klienta](manager.){.external} i wybierz `Web Cloud`{.action}. Kliknij `Hosting`{.action}, po czym wybierz nazwę odpowiedniego hostingu. Następnie przejdź do sekcji`Informacje ogólne`{.action}. Kliknij trzy kropki obok napisu „Certyfikat SSL”, a następnie kliknij `Usuń certyfikat SSL`{.action}.
...
Aby sprawdzić adres IP hostingu, zapoznaj się z naszym przewodnikiem "[Lista adresów IP klastrów i hostingów WWW](clusters_and_shared_hosting_IP1.)".
Aby zmodyfikować aktywną strefę DNS Twojej domeny, zapoznaj się z naszym przewodnikiem "[Edycja strefy DNS OVHcloud](dns_zone_edit1.)".
...
Aby to naprawić, zapoznaj się z naszym tutorialem "[przepisz adres URL dostępu do mojej strony za pomocą mod_rewrite w pliku .htaccess](htaccess_url_rewriting_using_mod_rewrite1.)" lub w przypadku trudności skorzystaj z usług [wyspecjalizowanego dostawcy](partner.).
...
W razie potrzeby sprawdź tutorial "[Hosting WWW: przełącz stronę WWW na HTTPS](ssl-activate-https-website1.)" lub skorzystaj z pomocy [wyspecjalizowanego usługodawcy](partner.) w przypadku trudności.
...
Jeśli potrzebujesz, zapoznaj się z naszym przewodnikiem "[Korzystanie z certyfikatu SSL EV dla Twojej strony WWW](ssl_ev1.)", aby rozwiązać ten problem.
