...
> [!primary]
>
> Este guia trata principalmente dos e-mails enviados a partir de scripts situados no seu [alojamento web da OVHcloud](hosting.) com a ajuda da função "mail()" de PHP.
>
> Se pretender gerir os endereços de e-mail incluídos na sua oferta MX Plan ou na sua oferta de[alojamento web OVHcloud](hosting.){.external}, consulte a nossa documentação sobre os [E-mails partilhados - MX Plan](web-cloud-email-collaborative-solutions-mx-plan1.).
>
...
O acompanhamento e a gestão dos e-mails automatizados do seu alojamento web OVHcloud efetuam-se a partir do seu [Área de Cliente OVHcloud](manager.){.external}. Uma vez conectado, aceda à secção `Web Cloud`{.action}, clique em `Alojamentos`{.action} e escolha o alojamento em causa na lista. A seguir, clique no separador `Plus`{.action} e depois em `Scripts emails`{.action}.

![hosting](go-to-email-scripts-tab.png){.thumbnail}

A página que aparece permite-lhe seguir e gerir os e-mails automatizados enviados a partir do seu [alojamento web da OVHcloud](hosting.).

### Apresentação da parte "Scripts emails"

![hosting](mail_function_script_records_images_tab.png){.thumbnail}
...
- desativou a funcionalidade a partir do seu [Área de Cliente OVHcloud](manager.).
...
![hosting](email-script-disabled.png){.thumbnail}
...
- Analise os [logs do seu alojamento](logs_and_statistics1.) para identificar as falhas de segurança e os ficheiros infetados.
...
> 
> Colocamos à sua disposição a parte que vai seguir para o acompanhar melhor nas suas tarefas comuns. No entanto, se encontrar dificuldades, recomendamos que recorra a um [fornecedor especializado](partner.). Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção [mail_function_script_records_#go-further.](#go-further) deste manual.
>
...
> [!warning]
> 
> Os e-mails emitidos com um script utilizando uma configuração SMTP não poderão ser geridos e acompanhados a partir do seu [Área de Cliente OVHcloud](manager.).
> 
...
[Consultar os logs do seu alojamento](logs_and_statistics1.)

[Corrigir a página 403 Forbidden que aparece no seu site](diagnostic_403_forbidden1.)

[Restaurar o espaço de armazenamento FTP do seu alojamento](ftp_save_and_backup1.)
