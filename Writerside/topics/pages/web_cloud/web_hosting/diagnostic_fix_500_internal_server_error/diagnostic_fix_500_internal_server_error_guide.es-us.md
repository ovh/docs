...
>
> Esta guía le ayudará a realizar las operaciones más habituales. No obstante, si tiene alguna duda le recomendamos que contacte con un proveedor de servicios especializado o con el editor del servicio. Nosotros no podremos asistirle al respecto. Para más información, consulte el apartado [Más información](diagnostic_fix_500_internal_server_error_#go-further.) de esta guía.
>
...
Un sitio web está formado por un **código fuente** (los archivos en .php, por ejemplo, visibles durante una conexión a su alojamiento en [FTP](ftp_connection1.)), al que se añade a menudo una **base de datos**.
...
- Consulte esta [guía](ftp_filezilla_user_guide1.) para obtener una copia de su código fuente.
- Si su sitio web utiliza una base de datos, consulte también este [documento](sql_database_export1.) para obtener una copia de la misma.

En caso de error 500, es totalmente posible realizar una [restauración](diagnostic_fix_500_internal_server_error_#restore.) del sitio web. Sin embargo, es preferible realizar un diagnóstico a fondo para determinar el origen exacto del error.

### Comprobar los logs de su alojamiento

En primer lugar, consulte esta [guía](logs_and_statistics1.) para buscar la causa del error 500 en los logs de su alojamiento.

### Poner un sitio web en modo de desarrollo

Para identificar los posibles errores de PHP, ponga el alojamiento en modo `desarrollo` utilizando estas [indicaciones](configure_your_web_hosting#2-comprobar-la-configuracion-del-alojamiento-web.).
...
Para comprobarlo, [conéctese por FTP](ftp_connection1.) al alojamiento.

Cambie el nombre del archivo a `.htaccess.old` y permanezca en su sitio web.

Si este último vuelve a estar accesible, entonces el `.htaccess` está en cuestión. Por lo tanto, deberá modificarse. Si lo desea, contacte con uno de nuestros [partners](partner.) al respecto.
...
Para acceder a estos archivos, conéctese por FTP al alojamiento siguiendo la [guía](ftp_connection1.).

La guía "[Uso de FileZilla con el alojamiento](ftp_filezilla_user_guide#permisos-de-los-archivos-y-carpetas.)" le ayudará a comprobar lo siguiente :
...
Si usted o su desarrollador desea tener acceso a estos detalles, puede conectarse a su sitio web a través de una [conexión SSH](ssh_on_webhosting1.) a través de la fórmula de alojamiento [Pro](hosting-professional-offer.) o superior.

### Comprobar el estado de la base de datos

Para más información sobre los errores que puedan estar relacionados con la base de datos de su sitio web, consulte nuestra guía ["Resolver los errores más frecuentes asociados a las bases de datos"](diagnosis_database_errors1.).
...
Si su sitio web incluye una base de datos, consulte nuestra guía [Importar una copia de seguridad en la base de datos de un alojamiento web](sql_importing_mysql_database#restaurar-una-copia-de-seguridad-desde-el-area-de-cliente.) para restaurarla a un estado anterior.

Por último, si se ha producido un error al actualizar la versión PHP del alojamiento, consulte nuestra guía "[Cambiar la versión de PHP de un alojamiento web](configure_your_web_hosting1.)" para volver a la configuración anterior.

## Más información <a name="go-further"></a>

[¿Qué hacer si mi sitio web no está accesible?](diagnostic-website-not-accessible1.)

[¿Qué hacer en caso de error « La conexión no es privada »?](diagnostic-not-secured1.)

[¿Qué hacer en caso de página del « Index of » ?](diagnostic-index-of1.)

[¿Qué hacer en caso de página 403 forbidden?](diagnostic_403_forbidden1.)

[Resolver los errores más frecuentes asociados a las bases de datos](diagnosis_database_errors1.)

[Mi sitio es lento. ¿Qué hacer?](diagnostic_slownesses1.)
