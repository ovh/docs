...
>
> Questa guida ti aiuta a realizzare le operazioni più ricorrenti. Per questo, ti suggeriamo di rivolgerti a un [provider specializzato](partner.) o di contattare l’amministratore del servizio nel caso in cui dovessi incontrare delle difficoltà. Non saremo effettivamente in grado di fornirti assistenza. Per maggiori informazioni consulta la sezione [“Per saperne di più”](cron_tasks_#go-further.) di questa guida. 
>

## Prerequisiti

- Disporre di una soluzione di [hosting Web](hosting.) attiva
- Avere accesso allo [Spazio Cliente OVHcloud](manager.)

## Procedura

Accedi allo [Spazio Cliente OVHcloud](manager.). Clicca sulla scheda `Web Cloud`{.action} e poi su `Hosting`{.action}.
...
![cron di conferma](add-scheduling-step-3.png){.thumbnail}
...
Nei log del tuo hosting Web, accessibili dal tuo [Spazio Cliente OVHcloud](manager.){.external}, visualizzerai la categoria di log intitolata "CRON".

Consulta la nostra guida ["Consulta le statistiche e i log del tuo sito su un'offerta condivisa"](logs_and_statistics1.) per maggiori dettagli.
...
[Configurare il file .ovhconfig di un hosting Web](configure_your_web_hosting1.)

[Utilizza l'accesso SSH di un hosting Web](ssh_on_webhosting1.)
