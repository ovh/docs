...
Declare su nuevo dominio con ayuda de nuestra documentación sobre el[adición de un multisitio a su alojamiento compartido](multisites_configure_multisite1.). Declare también su subdominio en `wwx` si desea, por ejemplo, que `www.NewDomain.tld` muestre también su sitio web además de `NewDomain.tld`.
...
- Compruebe que su nuevo dominio apunta correctamente a la dirección IP correcta de su alojamiento compartido. Para obtener la dirección IP, conéctese a su [área de cliente de OVHcloud](manager.), acceda a la sección `Web cloud`{.action}, haga clic en `Alojamientos`{.action}, seleccione su alojamiento y recupere **l IPv4** en la pestaña `Información general`{.action}.

> [!warning]
>
> Si activa las opciones **IP del país** o **CDN** con su nuevo dominio, utilice la dirección IP correcta con nuestra documentación que recoge [todas las direcciones IP de nuestros alojamientos compartidos](multisites_configure_multisite1.).
>
...
> **Cuando llegue al [paso 3](How_to_change_the_domain_name_of_an_existing_website_#step3.) de esta guía**, deberá eliminar su certificado SSL de pago para contratar otro para su nuevo dominio.<br>
...
![multisitios](all-domain-same-config-enable.png){.thumbnail}
...
Si su sitio web no utiliza bases de datos ni reescribe URL para su sitio web, este último ya debe mostrarse correctamente con su nuevo dominio. En ese caso, vaya directamente a [el paso 3](How_to_change_the_domain_name_of_an_existing_website_#step3.) de esta guía. En caso contrario, vaya al paso 2.
...
> [!warning]
>
> Atención, las operaciones descritas en el paso 2 son extremadamente sensibles y pueden tener consecuencias graves para su sitio web si no se efectúan con precaución. Si tiene alguna duda, no intente nada y contacte con un [proveedor especializado](partner.).
>
> Antes de realizar cualquier acción, le recomendamos que obtenga una [copia de seguridad de su espacio de almacenamiento FTP](ftp_save_and_backup1.) y una [copia de seguridad de su base de datos](sql_database_export1.). Para restaurar el sitio web en caso de manipulación incorrecta.
>
...
Tenga en cuenta que en estos CMS también es posible realizar sus modificaciones directamente [en la base de datos](sql_create_database1.). Deberá cambiar la URL de acceso al sitio web en la tabla que se indica a tal efecto.
...
Para reescribir sus URL con su nuevo dominio, [conéctese a la base de datos de su sitio web](sql_create_database1.) y, a continuación, sustituya su antiguo dominio por el nuevo en la tabla correspondiente. 
...
Para evitar el "*Duplicate-content*" y cuando su nuevo dominio esté plenamente operativo con su sitio web, deberá eliminar la declaración en multisitio para su antiguo dominio utilizando la guía sobre la gestión de [multisitios en su alojamiento compartido](multisites_configure_multisite1.).

> [!warning]
>
> No olvide contratar su certificado SSL *Sectigo EV*, *Sectigo DV* o *Custom* como se especifica en [el paso 1](How_to_change_the_domain_name_of_an_existing_website_#step1.).
>

Una vez que el antiguo dominio haya sido eliminado de la pestaña multisitio, y si está registrado en OVHcloud, podrá redirigirlo utilizando una [redirección visible permanente 301](redirect_domain_name1.). De este modo, sus visitas podrán ser automáticamente redirigidas hacia su sitio web visualizando su nuevo dominio en la barra de direcciones/URL de su navegador.

## Más información <a name="go-further"></a>

[Lista de las IP de nuestros alojamientos compartidos](multisites_configure_multisite1.)

[Gestionar un certificado SSL en un alojamiento web](ssl_on_webhosting1.)

[Habilitar HTTPS en un sitio web con certificado SSL](ssl-activate-https-website1.)

[Redirigir un dominio](redirect_domain_name1.)
