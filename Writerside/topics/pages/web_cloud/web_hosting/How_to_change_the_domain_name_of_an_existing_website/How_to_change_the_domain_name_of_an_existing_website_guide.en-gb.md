...
>
> This tutorial is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](partner.) or reach out to the OVHcloud community if you encounter any difficulties. We will not be able to assist you. You can find more information in the [Go further](How_to_change_the_domain_name_of_an_existing_website_#go-further.) section of this guide.
>
...
> Be careful about the changes you make, and contact a [specialised provider](partner.) for SEO if necessary.
>
...
Declare your new domain name using our guide on [adding a multisite on your web hosting](multisites_configure_multisite1.). Also declare the subdomain as `www` if, for example, you want to have `www.NewDomain.tld` also display your website in addition to `NewDomain.tld`.
...
- Check that your new domain points to the IP address of your hosting plan. To retrieve the IP address, log in to your [OVHcloud Control Panel](manager.), go to the `Web Cloud`{.action} section, click on `Hosting plans`{.action}, select your service, then retrieve **IPv4** in the `General information`{.action} tab.

> [!warning]
>
> If you enable the **country IP** or **CDN** options with your new domain, be sure to use the appropriate IP address listed in our documentation [IP address list for Web Hosting clusters](clusters_and_shared_hosting_IP1.).
>
...
> **Once you have reached [step 3](How_to_change_the_domain_name_of_an_existing_website_#step3.) in this guide**, you will need to delete your paid SSL certificate in order to subscribe to another one for your new domain.<br>
...
If your website does not use databases and/or you do not rewrite URLs for your website, your website must already display correctly with your new domain. In this case, go directly to [step 3](How_to_change_the_domain_name_of_an_existing_website_#step3.) of this guide. If not, continue to step 2 below.
...
> [!warning]
>
> Warning, the operations described in step 2 are extremely sensitive and can have serious consequences for your website if they are not carried out with caution. If you have any doubts, do not try anything and contact a [specialist provider](partner.).
>
> Before you take any action, we advise retrieving a [backup of your FTP storage space](ftp_save_and_backup1.) and a [backup of your database](sql_database_export1.). This will allow you to restore your website in the event of an error.
>
...
- Drupal: The publisher of this software does not offer any documentation to date for changing the access domain for your website. Please contact the publisher directly on this subject. For more information, see the official page [drupal.org](https://drupal.org){.external}.
...
Please note that for these CMSs, you can also edit the domain names directly [in the corresponding database](sql_create_database1.). You will need to change the URL for accessing your website in the table used for your CMS.
...
To rewrite your URLs with your new domain, [log in to your website database](sql_create_database1.) then replace your old domain with the new one in the corresponding table. 
...
To avoid duplicate content, you will need to delete the multisite entry for your old domain name. Once your new domain name is fully operational with your site, use our guide on [managing multisites on your hosting](multisites_configure_multisite1.) to remove it.

> [!warning]
>
> Don't forget to take care of your SSL certificate *Sectigo EV*, *Sectigo DV* or *Custom* as specified in [step 1](How_to_change_the_domain_name_of_an_existing_website_#step1.).
>

Once your old domain has been removed from the multisites tab, and if it is registered with OVHcloud, you can redirect it using a [permanent visible redirection (301)](redirect_domain_name1.). This will allow your visitors to be automatically redirected to your site when accessing your old domain name/URL in the address bar of their browser.
...
[Redirecting a domain name](redirect_domain_name1.)
