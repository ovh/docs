...
> [!warning]
>
> Si vous activez les options **IP du pays** ou **CDN** avec votre nouveau domaine, utilisez bien la bonne adresse IP à l'aide de notre documentation recensant [l'ensemble des adresses IP de nos hébergements mutualisés](multisites_configure_multisite1.).
>
...
Notez que pour ces CMS, il est également possible d'effectuer ses modifications directement [en base de données](sql_create_database1.). Vous devrez changer l'URL d'accès à votre site dans la table prévue à cet effet.
...
Pour réécrire vos URLs avec votre nouveau domaine, [connectez-vous à la base de données de votre site](sql_create_database1.) puis remplacez votre ancien domaine par le nouveau dans la table correspondante. 
...
[Liste des IPs de nos hébergements mutualisés](multisites_configure_multisite1.)
