...
>
> This tutorial is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](partner.) or the [WordPress support](https://wordpress.com/support/){.external} if you encounter any difficulties. We will not be able to assist you. You can find more information in the [Go further](how_to_backup_your_wordpress_#go-further.) section of this guide.
>
...
- An [OVHcloud web hosting plan](hosting.) with [WordPress installed](cms_install_1_click_modules1.)

## Instructions

You can perform a backup in two ways: **manually** or by **using a plugin**.

OVHcloud provides an [automatic data backup service](ftp_save_and_backup1.) (non-contractual) and makes these backups available. However, it is your responsibility to set up your own restoration policy and to determine restoration points at times you deem appropriate.
...
Retrieving your data is done via an FTP client like FileZilla. See our tutorial "[Using FileZilla with your OVHcloud hosting](ftp_filezilla_user_guide1.)" for more information.
...
![View of WordPress files and directories at the root](wordpress-into-www.png){.thumbnail}
...
> [!success]
>
> Please refer to our guide on [exporting a database](sql_database_export1.).

![PHPMyAdmin access - Home](images_pma-main-page.png){.thumbnail}

Click `Export`{.action} at the top of the page:

![PHPMyAdmin access - Export](pma-export.png){.thumbnail}
...
![PHPMyAdmin Access - Export - Download](dowload-successfull.png){.thumbnail}
...
![Administrator login access on wp-admin](images_login-interface.png){.thumbnail}

On the home page, go to the `Plugins`{.action} section, then click `Add New`{.action}:

![Add extension](plugins-add-new.png){.thumbnail}

Upload the plugin by clicking the `Browse`{.action} button:

![Upload extension](plugins-add-new-updraftplus.png){.thumbnail}

Click `Install Now`{.action}:

![Install extension](plugins-browse-updraftplus.png){.thumbnail}

After installation, you are prompted to activate the plugin:

![Installation confirmation](plugins-activate-updraftplus.png){.thumbnail}

Once you have enabled it, it will appear in the `Plugins` list:

![List of installed extensions](plugins-list-updraftplus.png){.thumbnail}
...
![Page UpdraftPlus Settings](updraftplus-settings-2.png){.thumbnail}

Choose `Email` backup.

Backups will be sent to the email address of the administrator account (the account you are using):

![Backup by email address](email-setting.png){.thumbnail}
...
![Page UpdraftPlus Backup/Restore](updraftplus-backup-now.png){.thumbnail}

In the window that pops up, click again on `Backup Now`{.action}:

![Page UpdraftPlus modal Backup/Restore](updraftplus-perform-backup.png){.thumbnail}
...
- [More information on backups for your web hosting plan](hosting_technical_specificities#automatic-backup-information.)
