...
> 
> Nous mettons à votre disposition ce tutoriel afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) ou à [l'éditeur du CMS WordPress](https://wordpress.com/fr/support/){.external} si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](how_to_backup_your_wordpress_#go-further.) de ce tutoriel.
>
...
La récupération se fait via un client FTP comme FileZilla. Consultez notre guide « [Utiliser FileZilla avec votre hébergement OVHcloud](ftp_filezilla_user_guide1.) » pour plus d'informations.
...
![Vue des fichiers et répertoires WordPress à la racine](wordpress-into-www.png){.thumbnail}
...
> [!success]
>
> N'hésitez pas à consulter notre guide sur [l'export d'une base de données](sql_database_export1.).

![Accès PHPMyAdmin - Accueil](images_pma-main-page.png){.thumbnail}

Cliquez sur `Exporter`{.action} en haut de la page :

![Accès PHPMyAdmin - Exporter](pma-export.png){.thumbnail}
...
![Accès PHPMyAdmin - Exporter - Téléchargement](dowload-successfull.png){.thumbnail}
...
![Accès connexion administrateur sur wp-admin](images_login-interface.png){.thumbnail}

Sur la page d'accueil, rendez-vous dans la section `Extensions`{.action} puis cliquez sur `Ajouter`{.action} :

![Ajouter une extension](plugins-add-new.png){.thumbnail}

Téléversez l'extension en cliquant sur le bouton `Parcourir`{.action} :

![Uploader l'extension](plugins-add-new-updraftplus.png){.thumbnail}

Cliquez sur `Installer maintenant`{.action} :

![Installer l'extension](plugins-browse-updraftplus.png){.thumbnail}

Après l'installation, vous êtes invité à activer l'extension :

![Confirmation d'installation](plugins-activate-updraftplus.png){.thumbnail}

Une fois activée, celle-ci apparaitra dans la liste des extensions :

![Liste des extensions installées](plugins-list-updraftplus.png){.thumbnail}
...
![Page UpdraftPlus Backup/Restore](updraftplus-settings.png){.thumbnail}

Définissez la sauvegarde quotidienne de vos fichiers et de votre base de données :

![Page UpdraftPlus Réglages](updraftplus-settings-2.png){.thumbnail}
...
![Sauvegarde par adresse mail](email-setting.png){.thumbnail}
...
![Page UpdraftPlus Sauvegarder/Restaurer](updraftplus-backup-now.png){.thumbnail}

Dans la fenêtre qui s'affiche, cliquez à nouveau sur `Sauvegarder`{.action} :

![Page UpdraftPlus modale Sauvegarder/Restaurer](updraftplus-perform-backup.png){.thumbnail}
...
- [Plus d'informations sur les sauvegardes de votre hébergement web](hosting_technical_specificities#informations-sur-les-sauvegardes-automatiques.)
