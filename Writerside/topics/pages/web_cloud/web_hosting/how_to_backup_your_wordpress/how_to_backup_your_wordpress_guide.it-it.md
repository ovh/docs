...
OVHcloud fornisce un [servizio (non contrattuale) di backup automatico dei dati](ftp_save_and_backup1.) e la messa a disposizione di questi backup. E' tuttavia vostro compito attuare la vostra politica di ripristino e individuare i punti di ripristino nei momenti che riterrete opportuni.
...
Il recupero viene effettuato tramite un client FTP come FileZilla. Per maggiori informazioni, consulta la guida "[Utilizzare FileZilla con il tuo hosting OVHcloud](ftp_filezilla_user_guide1.)".
...
![Visualizza file e directory WordPress alla radice](wordpress-into-www.png){.thumbnail}
...
> [!success]
>
> Consulta la nostra guida su [esportare un database](sql_database_export1.).

![Accesso PHPMyAdmin - Home](images_pma-main-page.png){.thumbnail}

Clicca su `Esportare`{.action} in alto a pagina:

![Accesso PHPMyAdmin - Esporta](pma-export.png){.thumbnail}
...
![Accesso PHPMyAdmin - Esporta - Scaricamento](dowload-successfull.png){.thumbnail}
...
![Accesso amministratore su wp-admin](images_login-interface.png){.thumbnail}

Accedi alla sezione `Plugin`{.action} e clicca su `Aggiungi nuovo`{.action}

![Aggiungi un'estensione](plugins-add-new.png){.thumbnail}

Seleziona l'estensione cliccando su `Percorrere`{.action}:

![Carica l'estensione](plugins-add-new-updraftplus.png){.thumbnail}

Clicca su `Installa adesso`{.action}:

![Installare l'estensione](plugins-browse-updraftplus.png){.thumbnail}

Dopo l'installazione, ti verrà chiesto di attivare l'estensione:

![Conferma di installazione](plugins-activate-updraftplus.png){.thumbnail}

Una volta attivata, comparirà nella lista delle estensioni:

![Elenco delle estensioni installate](plugins-list-updraftplus.png){.thumbnail}
...
![Pagina UpdraftPlus Backup/Restore](updraftplus-settings.png){.thumbnail}

Definisci il backup giornaliero dei tuoi file e del tuo database:

![Pagina UpdraftPlus](updraftplus-settings-2.png){.thumbnail}
...
![Backup via email](email-setting.png){.thumbnail}
...
![Pagina UpdraftPlus Salvare/Ripristinare](updraftplus-backup-now.png){.thumbnail}

Nella nuova finestra, clicca su `Esegui il backup adesso`{.action}:

![Pagina UpdraftPlus modale Salva/Ripristina](updraftplus-perform-backup.png){.thumbnail}
...
- [Maggiori informazioni sui backup del tuo hosting Web](hosting_technical_specificities#informazioni-sui-backup-automatici.)
