...
> [!primary]
>
> Jeśli chcesz zdalnie zarządzać swoją witryną internetową bez użycia Visual Studio Code, możesz zainstalować klienta FTP FileZilla. Zapoznaj się z naszym przewodnikiem "[Korzystanie z FileZilla na Twoim hostingu OVHcloud](ftp_filezilla_user_guide1.)". Jeśli chcesz połączyć się ze stroną WWW przez SSH, zapoznaj się z naszą dokumentacją "[Korzystanie z dostępu SSH do hostingu WWW](ssh_on_webhosting1.)".
>
...
> 
> Oddajemy w Twoje ręce tutorial, którego celem jest pomoc w jak najbardziej optymalnym wykonywaniu bieżących zadań. Niemniej jednak w przypadku trudności zalecamy skorzystanie z pomocy [wyspecjalizowanego dostawcy](partner.) lub [Visual Studio Code](https://code.visualstudio.com/){.external} IDE. Niestety firma OVH nie jest w stanie udzielić Ci wsparcia w tym zakresie. Więcej informacji znajduje się w sekcji ["Sprawdź również"](handle_sftp_connexion_vscode_#go-further.) tego tutoriala.
>
...
[Korzystanie z FileZilla na Twoim hostingu OVHcloud](ftp_filezilla_user_guide1.)

[Korzystanie z dostępu do hostingu WWW przez SSH](ssh_on_webhosting1.). Pamiętaj, że aby korzystać z SSH, musisz mieć wykupiony [hosting Pro lub Performance](hosting.).
