...
>
> Se precisar de um serviço com uma taxa de disponibilidade SLA superior a 99%, sugerimos que considere a utilização de um [Servidor Privado Virtual (VPS)](vps.) ou de um [Servidor Dedicado](bare-metal.).
>
...
- Dispor de um site alojado numa das nossas ofertas de [alojamento partilhado OVHcloud](hosting.)
...
> 
> Este manual fornece as instruções necessárias para realizar as operações mais habituais. No entanto, se encontrar dificuldades, recomendamos que recorra a um [fornecedor especializado](partner.). De facto, não poderemos fornecer-lhe assistência **a partir do momento em que a infraestrutura em que o seu alojamento partilhado está presente não esteja em causa**. Para mais informações, aceda à secção ["Quer saber mais?"](diagnostic_slownesses_#go-further.) deste manual.
>
...
Para conhecer o cluster e filer onde se encontra o alojamento partilhado, aceda ao seu [Área de Cliente OVHcloud](manager.), vá à secção `Web Cloud`{.action}, clique em `Alojamentos`{.action} e escolha o alojamento web em causa. No separador `Information gerais`{.action}, localize o `datacenter` do seu alojamento partilhado bem como o `filer` onde está.

![Recuperar Filer](find-filer.png){.thumbnail}

A seguir, clique no separador `Multisites`{.action} para obter o número do cluster onde está o seu alojamento partilhado.

![Recuperar o cluster](find-cluster.png){.thumbnail}
...
Coloque na raiz do seu website no [espaço de armazenamento FTP do seu alojamento partilhado](ftp_connection1.) um ficheiro que denominará `phpinfo.php`.
...
>
> As manipulações num ficheiro "**.htaccess**" podem ter consequências na visualização do seu site. Recorra a um [fornecedor especializado](partner.) se sentir dificuldades em realizar o seguinte.
>
...
Aceda à sua base de dados seguindo *** o etapa 3** do nosso manual sobre a [criação de uma base de dados partilhada](sql_create_database1.).

Se utiliza uma base de dados numa oferta **Web Cloud Databases**, consulte o nosso guia sobre [a ligação à sua base de dados presente numa oferta Web Cloud Databases](connecting-to-database-on-database-server1.).

Se a ligação for bem-sucedida, poderá aceder à seguinte interface:

![PHPMyAdmin](pma-main-page.png){.thumbnail}

> [!warning]
>
> Se encontrar um erro, consulte o nosso manual sobre os [erros comuns encontrados com uma base de dados](diagnosis_database_errors1.). De seguida, altere a sua situação usando o guia acima para tentar novamente aceder à base de dados.
>
...
> Isto significa que a lentidão que encontra provém diretamente dos scripts que compõem o seu website. Pode passar diretamente para [etapa 2](diagnostic_slownesses_#step2.) para seguir os conselhos de otimização para resolver a sua situação.
...
> [!warning]
>
> Se encontrar dificuldades para realizar as ações que se seguem, pode contactar um dos nossos [prestadores especializados](partner.). A OVHcloud não prestará assistência ao desenvolvimento e/ou otimização do conteúdo do seu website.
>
...
Verifique o motor PHP, a versão PHP e o ambiente de execução utilizados no seu alojamento web, através do nosso guia sobre a [configuração do seu alojamento web](configure_your_web_hosting1.).
...
Para comparar as versões de PHP disponíveis em função do ambiente de execução utilizado, consulte ***a etapa 2** do guia sobre a [configuração da versão PHP no seu alojamento](configure_your_web_hosting1.).
...
Para analisar as ligações de saída do seu alojamento, consulte os logs **OUT** deste último. Pode consultar o nosso manual sobre [a consulta dos logs do seu alojamento](logs_and_statistics1.).
...
Para realizar esta ação, consulte os logs **WEB** do seu alojamento web através do nosso manual sobre [como consultar os logs do seu alojamento](logs_and_statistics1.).
...
>
>![Análise de rede Firefox](F12.png){.thumbnail}
>
...
> [!primary]
>
> A OVHcloud oferece várias [ofertas CDN](hosting-options.). Se pretender utilizá-los ou ativar um para o seu alojamento web, aceda à [Área de Cliente OVHcloud](manager.) e consulte o nosso guia sobre [utilização do CDN OVHcloud](cdn_how_to_use_cdn1.).
>
...
> [!warning]
>
> As ações que realiza na sua base de dados podem ter consequências irreversíveis se não forem realizadas de forma metódica e correta. Recorra a um [fornecedor especializado](partner.) se não tem a certeza das ações a realizar. 
>
...
Consulte o nosso guia sobre a [otimização das performances para o seu website](optimise_your_website_performance1.).
...
Pode considerar uma [oferta de alojamento partilhado](hosting.) superior ou uma infraestrutura dedicada como um [Servidor Privado Virtual (VPS)](vps.) ou um [Servidor Dedicado](bare-metal.). 
