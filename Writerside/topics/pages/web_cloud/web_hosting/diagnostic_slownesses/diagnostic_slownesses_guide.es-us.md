...
>
> Si necesita un servicio con una tasa de disponibilidad SLA superior al 99%, le recomendamos que considere la posibilidad de utilizar un [Servidor Privado Virtual (VPS)](vps.) o un [Servidor Dedicado](bare-metal.).
>
...
> 
> Ponemos a su disposición esta guía para ayudarle a realizar las tareas más habituales. No obstante, si tiene alguna duda, le recomendamos que contacte con un [proveedor especializado](partner.). Nosotros no podremos asistirle. **siempre que la infraestructura en la que está alojado su plan de hosting no sea válida**. Más información en la sección ["Más información"](diagnostic_slownesses_#go-further.) de esta guía.
>
...
![Obtener cluster](find-cluster.png){.thumbnail}
...
Ponga en la raíz de su sitio web en el[espacio de almacenamiento FTP de su alojamiento compartido](ftp_connection1.) un archivo al que denominará "phpinfo.php".
...
>
> Las operaciones en un archivo "**.htaccess**" pueden afectar a la visualización de su sitio web. Si tiene dificultades para realizar lo siguiente, contacte con un [proveedor especializado](partner.).
>
...
Conéctese a su base de datos siguiendo **el etapa 3** de nuestra guía sobre la [creación de una base de datos compartida](sql_create_database1.).
...
> [!warning]
>
> Si se produce un error, consulte nuestra documentación sobre los [errores comunes encontrados con una base de datos](diagnosis_database_errors1.). A continuación, utilice la guía para corregir su situación e intente volver a conectarse a la base de datos.
>
...
> Esto significa que la lentitud con la que se encuentra proviene directamente de los scripts que componen su sitio web. Puede ir directamente a [el etapa 2](diagnostic_slownesses_#step2.) para seguir los consejos de optimización para resolver su situación.
...
> [!warning]
>
> Si tiene dificultades para realizar las siguientes acciones, puede contactar con uno de nuestros [proveedores especializados](partner.). OVHcloud no ayudará en el desarrollo o la optimización del contenido de su sitio web.
>
...
Compruebe el motor PHP, la versión PHP y el entorno de ejecución utilizados en su alojamiento web, con ayuda de nuestra guía sobre la [configuración de su alojamiento web](configure_your_web_hosting1.).
...
Para comparar las versiones de PHP disponibles en función del entorno de ejecución utilizado, consulte **el etapa 2** de la guía sobre la [configuración de la versión PHP en su alojamiento](configure_your_web_hosting1.).
...
Para analizar las conexiones salientes del alojamiento, consulte los logs **OUT** del mismo. Si necesita ayuda, consulte nuestra guía sobre [la consulta de los logs de su alojamiento](logs_and_statistics1.).
...
Para ello, consulte los logs **WEB** de su alojamiento web con nuestra guía sobre [cómo consultar los logs de su alojamiento](logs_and_statistics1.).
...
>
>![Análisis de red Firefox](F12.png){.thumbnail}
>
...
> [!primary]
>
> OVHcloud ofrece varios [productos CDN](hosting-options.). Si desea utilizar o activar una para su alojamiento web, conéctese a su [área de cliente de OVHcloud](manager.) y consulte nuestra guía sobre el uso de la CDN de OVHcloud](/pages/web_cloud/web_hosting/cdn_how_to_use_cdn).
>
...
> [!warning]
>
> Las acciones que realice en la base de datos pueden tener consecuencias irreversibles si no se realizan de forma ordenada y correcta. Contacte con un [proveedor especializado](partner.) si no está seguro de las acciones a realizar. 
>
...
Consulte nuestra guía sobre la [optimización del rendimiento de su sitio web](optimise_your_website_performance1.).
...
Puede considerar un [plan de hosting](hosting.) superior o una infraestructura dedicada como un [Servidor Privado Virtual (VPS)](vps.) o un [Servidor Dedicado](bare-metal.). 
