...
>
> Jeśli potrzebujesz usługi ze współczynnikiem dostępności SLA większym niż 99%, zalecamy rozważenie korzystania z [Prywatnego Serwera Wirtualnego (VPS)](vps.) lub [Serwera dedykowanego](bare-metal.).
>
...
- Posiadanie strony internetowej zainstalowanej w ramach jednej z naszych ofert[hosting OVHcloud](hosting.)
...
> 
> Oddajemy w Twojej ręce niniejszy przewodnik, którego celem jest pomoc w jak najlepszym wykonywaniu bieżących zadań. W przypadku trudności zalecamy skorzystanie z pomocy [wyspecjalizowanego usługodawcy](partner.). Niestety nie będziemy w stanie udzielić Ci wsparcia **w przypadku, gdy infrastruktura, na której hostowana jest Twoja oferta hostingu współdzielonego, nie jest istotna**. Więcej informacji znajduje się w sekcji ["Sprawdź również"](diagnostic_slownesses_#go-further.) niniejszego przewodnika.
>
...
Aby dowiedzieć się, gdzie znajduje się Twój hosting, zaloguj się do [Panelu klienta OVHcloud](manager.), przejdź do sekcji `Web Cloud`{.action}, kliknij przycisk `Hosting`{.action} i wybierz odpowiedni hosting. W zakładce `Informacje ogólne`{.action} znajdziesz `centrum` danych hostingu współdzielonego oraz `filer`, w którym znajduje się ono.

![Pobierz Filer](find-filer.png){.thumbnail}

Następnie kliknij zakładkę `MultiSite`{.action}, aby pobrać numer klastra, na którym znajduje się Twój hosting.

![Pobranie klastra](find-cluster.png){.thumbnail}
...
Umieść w katalogu głównym Twojej strony WWW[przestrzeń dyskowa FTP Twojego hostingu](ftp_connection1.) plik, który nazwiesz `phpinfo.php`.
...
>
> Operacje na pliku "**.htaccess**" mogą mieć wpływ na wyświetlanie strony. W przypadku trudności z realizacją tego zadania, należy skontaktować się z [wyspecjalizowanym dostawcą](partner.).
>
...
Zaloguj się do bazy danych, postępując zgodnie z **etap 3** w przewodniku [dotyczącym tworzenia wspólnej bazy danych](sql_create_database1.).

Jeśli korzystasz z bazy danych w ofercie **Web Cloud Databases**, zapoznaj się z naszym przewodnikiem [dotyczącym połączenia z bazą danych w usłudze Web Cloud Databases](connecting-to-database-on-database-server1.).
...
> [!warning]
>
> Jeśli wystąpi błąd, zapoznaj się z naszą dokumentacją dotyczącą [błędów występujących w bazie danych](diagnosis_database_errors1.). Następnie skorzystaj z powyższego przewodnika, aby poprawić Twoją sytuację, po czym spróbuj ponownie zalogować się do bazy danych.
>
...
> Oznacza to, że powolne działania pochodzą bezpośrednio ze skryptów tworzących Twoją stronę WWW. Możesz przejść bezpośrednio do [etap 2](diagnostic_slownesses_#step2.) i postępować zgodnie z instrukcjami optymalizacyjnymi, aby rozwiązać Twoją sytuację.
...
> [!warning]
>
> Jeśli masz trudności z wykonaniem kolejnych działań, możesz skontaktować się z naszym [wyspecjalizowanym usługodawcą](partner.). OVHcloud nie będzie wspierać rozwoju i/lub optymalizacji zawartości Twojej strony WWW.
>
...
Sprawdź framework PHP, wersję PHP i środowisko wykonawcze używane na Twoim hostingu. Zapoznaj się z naszym przewodnikiem dotyczącym [konfiguracji Twojego hostingu](configure_your_web_hosting1.).
...
Aby porównać dostępne wersje PHP w zależności od używanego środowiska uruchomieniowego, sprawdź **etap 2** w przewodniku dotyczącym [konfiguracji wersji PHP na Twoim hostingu](configure_your_web_hosting1.).
...
Aby przeanalizować połączenia wychodzące z Twojego hostingu, sprawdź logi **OUT** tego hostingu. Zapoznaj się z naszą dokumentacją dotyczącą [sprawdzanie logów hostingu](logs_and_statistics1.).
...
Aby przeprowadzić tę operację, zapoznaj się z logami **WEB** Twojego hostingu w dokumentacji dotyczącej hostingu [sprawdź logi](logs_and_statistics1.).
...
>
>![Analiza sieci Firefox](F12.png){.thumbnail}
>
...
> [!primary]
>
> OVHcloud oferuje kilka [oferty CDN](hosting-options.). Jeśli chcesz korzystać z tej usługi lub ją włączyć dla Twojego hostingu, zaloguj się do [Panelu klienta OVHcloud](manager.), a następnie zapoznaj się z naszym przewodnikiem dotyczącym [korzystania z usługi CDN OVHcloud](cdn_how_to_use_cdn1.).
>
...
> [!warning]
>
> Operacje, które wykonujesz w bazie danych mogą mieć nieodwracalne konsekwencje, jeśli nie są przeprowadzane metodycznie i prawidłowo. Skontaktuj się z [wyspecjalizowanym dostawcą](partner.), jeśli nie masz pewności co do działań, jakie należy przeprowadzić. 
>
...
Zapoznaj się z naszym przewodnikiem dotyczącym [optymalizacji wydajności dla Twojej strony internetowej](optimise_your_website_performance1.).
...
Możesz rozważyć ofertę [hostingu współdzielonego](hosting.) lub infrastrukturę dedykowaną, taką jak [Serwer Wirtualny Prywatny (VPS)](vps.) lub [Serwer dedykowany](bare-metal.). 
