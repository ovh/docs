...
>
> Wenn Sie einen Dienst mit einer Verfügbarkeitsgarantie von mehr als 99% benötigen, empfehlen wir die Verwendung eines [Virtual Private Server (VPS)](vps.) oder eines [Dedicated Server](bare-metal.).
>
...
- Sie hosten eine Website auf einem [OVHcloud Webhosting](hosting.).
...
> 
> Diese Anleitung soll Sie bei allgemeinen Aufgaben bestmöglich unterstützen. Dennoch empfehlen wir Ihnen, falls Sie Hilfe brauchen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren oder Ihre Fragen in der OVHcloud Community zu stellen. Leider können wir Ihnen für administrative Aufgaben keine weitergehende technische Unterstützung anbieten, sofern die **Ausfälle nicht auf Ebene der Hosting-Infrastruktur verursacht werden**. Weitere Informationen finden Sie am [Ende dieser Anleitung](diagnostic_slownesses_#go-further.).
>
...
Legen Sie im Wurzelverzeichnis Ihrer Website auf dem [FTP-Speicherplatz Ihres Webhostings](ftp_connection1.) eine Datei mit dem Namen **phpinfo.php** an.
...
>
> Änderungen an einer **.htaccess** können Auswirkungen auf die korrekte Anzeige Ihrer Website haben. Wenden Sie sich an [spezialisierter Dienstleister](partner.), wenn Sie Schwierigkeiten haben, die folgenden Maßnahmen durchzuführen.
>
...
Loggen Sie sich in Ihrer Datenbank ein, indem Sie **Schritt 3** unserer [Anleitung zu Webhosting-Datenbanken](sql_create_database1.) befolgen.

Wenn Sie eine Datenbank mit **Web Cloud Databases** nutzen, lesen Sie unsere Anleitung zur [Datenbankverbindung mit Web Cloud Databases](connecting-to-database-on-database-server1.).
...
> [!warning]
>
> Falls Sie einen Fehler feststellen, lesen Sie unsere [Anleitung zur Behebung von Datenbankfehlern](diagnosis_database_errors1.). Loggen Sie sich nach der Fehlerbehebung erneut ein.
>
...
> Das bedeutet, dass die Verlangsamung direkt mit den Skripten Ihrer Website zusammnhängt. Sie können zu [Schritt 2](diagnostic_slownesses_#step2.) übergehen, um den Optimierungsratschlägen zur Verbesserung dieser Situation zu folgen.
...
> [!warning]
>
> Wenn Sie Schwierigkeiten haben, die folgenden Aktionen durchzuführen, können Sie einen [sachverständigen Partner](partner.) kontaktieren. OVHcloud kann keine Unterstützung bei der Entwicklung oder Optimierung Ihrer Website anbieten.
>
...
Überprüfen Sie die auf Ihrem Webhosting verwendete PHP-Engine, PHP-Version und Ausführungsumgebung. Lesen Sie hierzu unsere Anleitung zur [Konfiguration Ihres Webhostings](configure_your_web_hosting1.).
...
Um die verfügbaren PHP-Versionen je nach Ausführungsumgebung zu vergleichen, lesen Sie **Schritt 2** der Anleitung zur [Konfiguration der PHP-Version auf Ihrem Hosting](configure_your_web_hosting1.).
...
Um die ausgehenden Verbindungen Ihres Hostings zu analysieren, analysieren Sie dessen Logs vom Typ **OUT**. Weitere Informationen finden Sie in unserer Dokumentation zur [Verwendung von Hosting-Logs](logs_and_statistics1.).
...
Lesen Sie hierzu die Logs vom Typ **WEB** Ihres Webhostings unter Verwendung unserer Anleitung: [Die Logs Ihres Webhostings einsehen](logs_and_statistics1.).
...
>
>![Firefox Netzwerk-Analyse](F12.png){.thumbnail}
>
...
> [!primary]
>
> OVHcloud bietet mehrere [CDN-Optionen](hosting-options.) an. Wenn Sie CDN für Ihr Webhosting aktivieren möchten, loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und folgen Sie unserer Anleitung zur [Verwendung des OVHcloud CDN](cdn_how_to_use_cdn1.).
>
...
> [!warning]
>
> Änderungen in Ihrer Datenbank können irreversible Folgen haben, wenn sie nicht systematisch und korrekt durchgeführt werden. Wenden Sie sich an einen [spezialisierten Dienstleister](partner.), falls Sie sich der geeigneten Maßnahmen nicht sicher sind. 
>
...
Lesen Sie auch unsere Anleitung zur [Optimierung der Performance Ihrer Website](optimise_your_website_performance1.).
...
Ziehen Sie in diesem Fall ein leistungsfähigeres [Webhosting](hosting.) oder eine dedizierte Infrastruktur in Betracht, wie einen [Virtual Private Server (VPS)](vps.) oder einen [Dedicated Server](bare-metal.).
