...
>
> In caso di necessità di un servizio con un tasso di disponibilità SLA superiore al 99%, ti consigliamo di considerare l'utilizzo di un [Server Privato Virtuale (VPS)](vps.) o di un [Server Dedicato](bare-metal.).
>
...
- Disporre di un sito ospitato su una delle nostre offerte di [hosting condiviso OVHcloud](hosting.)
...
> 
> Questa guida ti aiuta a eseguire le operazioni necessarie. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a uno [specialista del settore](partner.). OVHcloud non sarà infatti in grado di fornirti assistenza **se l'infrastruttura in cui è presente la tua offerta di hosting condiviso non è interessata**. Per maggiori informazioni consulta la sezione ["Per saperne di più"](diagnostic_slownesses_#go-further.) di questa guida.
>
...
Accedi al tuo [Spazio Cliente OVHcloud](manager.), clicca su `Web Cloud`{.action}, seleziona il cluster e il file in cui si trova il tuo hosting condiviso, clicca su `Hosting`{.action} e seleziona l'hosting Web corrispondente. Nella scheda `Informazioni generali`{.action}, clicca su `datacenter` del tuo hosting condiviso e `filer` dov'è.

![Recupera Filer](find-filer.png){.thumbnail}

Clicca sulla scheda `Multisito`{.action} per recuperare il numero del cluster in cui si trova il tuo hosting condiviso.

![Recuperare cluster](find-cluster.png){.thumbnail}
...
Nella root del tuo sito Web accedi allo [spazio di storage FTP del tuo hosting condiviso](ftp_connection1.), un file che potrai chiamare `phpinfo.php`.
...
>
> Le operazioni su un file "**.htaccess**" possono avere conseguenze sulla visualizzazione del tuo sito. In caso di difficoltà o dubbi, rivolgiti a un [fornitore specializzato](partner.).
>
...
Accedi al tuo database seguendo **lo step 3** della nostra guida sulla [creazione di un database condiviso](sql_create_database1.).

Se utilizzi un database su un'offerta **Web Cloud Databases**, consulta la nostra guida su [la connessione al tuo database presente su un'offerta Web Cloud Databases](connecting-to-database-on-database-server1.).
...
> [!warning]
>
> In caso di errore, consulta la nostra guida sugli [errori ricorrenti riscontrati nel database](diagnosis_database_errors1.). Per correggere la posizione del tuo database, utilizza la guida qui sotto.
>
...
> [!warning]
>
> In caso di difficoltà nell'esecuzione delle azioni successive, contatta uno dei nostri [specialisti del settore](partner.). OVHcloud non fornirà assistenza nello sviluppo e/o nell'ottimizzazione del contenuto del tuo sito Web.
>
...
Per verificare il motore PHP, la versione PHP e l'ambiente di esecuzione utilizzati sull'hosting Web, consulta la guida sulla [configurazione del tuo hosting Web](configure_your_web_hosting1.).
...
Per confrontare le versioni di PHP disponibili in base all'ambiente di esecuzione utilizzato, consulta **lo Step 2** della guida sulla [configurazione della versione PHP sul tuo hosting](configure_your_web_hosting1.).
...
Per analizzare le connessioni in uscita dal tuo hosting, consulta i log **OUT** di quest'ultimo. Consulta la nostra guida su [consulta i log del tuo hosting](logs_and_statistics1.).
...
Per effettuare questa azione, consulta i log **WEB** del tuo hosting web utilizzando la nostra guida su [Come consultare i log del tuo hosting](logs_and_statistics1.).
...
>
>![Analisi di rete Firefox](F12.png){.thumbnail}
>
...
> [!primary]
>
> OVHcloud propone diverse [offerte CDN](hosting-options.). Per utilizzarne o attivarne uno per il tuo hosting Web, accedi al tuo [Spazio Cliente OVHcloud](manager.) e consulta la nostra guida su [l'utilizzo della CDN OVHcloud](cdn_how_to_use_cdn1.).
>
...
> [!warning]
>
> Le azioni eseguite nei database possono avere conseguenze irreversibili se non eseguite in modo corretto e metodico. Se non sei sicuro delle azioni da realizzare, rivolgiti a un [provider specializzato](partner.). 
>
...
Consulta la nostra guida sull'[ottimizzazione delle performance per il tuo sito Internet](optimise_your_website_performance1.).
...
È possibile scegliere una [offerta di hosting condiviso](hosting.) superiore o un'infrastruttura dedicata come [Server Privato Virtuale (VPS)](vps.) o un [Server Dedicato](bare-metal.). 
