...
>
> If you need a service with a guaranteed availability rate higher than 99%, we recommend that you consider using a [Virtual Private Server (VPS)](vps.) or a [Dedicated Server](bare-metal.).
>
...
- A website hosted on an [OVHcloud web hosting plan](hosting.)
...
>
> This guide is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](partner.) or reach out to the OVHcloud community if you encounter any difficulties. We will not be able to assist you with issues **not caused by the relevant hosting infrastructure itself**. You can find more information in the [Go further](diagnostic_slownesses_#go-further.) section of this guide.
>
...
To find out the cluster and filer of your hosting, log in to your [OVHcloud Control Panel](manager.), go to the `Web Cloud`{.action} section, click on `Hosting plans`{.action} then select the web hosting plan concerned. In the `General information`{.action} tab you can see the `Datacenter` and `Filer` of your web hosting.

![Retrieve Filer](find-filer.png){.thumbnail}

Then click on the `Multisite`{.action} tab to retrieve the cluster number where your shared hosting is located.

![Recover cluster](find-cluster.png){.thumbnail}
...
Place a file called **phpinfo.php** at the root of your website in the [FTP storage space on your shared hosting](ftp_connection1.).
...
>
> Changes to a **.htaccess** file may affect your website’s display. Contact a [specialist provider](partner.) if you have difficulty in doing the following.
>
...
Log in to your database by following **step 3** in our guide on [web hosting databases](sql_create_database1.).

If you are using a database of a **Web Cloud Databases** solution, please refer to our guide on [Web Cloud Databases databases](connecting-to-database-on-database-server1.).
...
> [!warning]
>
> If you encounter an error, please refer to our documentation on [common errors encountered with a database](diagnosis_database_errors1.). Then use the guide above to resolve the issue, and try connecting to your database again.
>
...
> This means that the delays you encounter come directly from the scripts that make up your website. You can skip to [step 2](diagnostic_slownesses_#step2.) to follow the optimisation tips to resolve the delays.
...
> [!warning]
>
> If you have any difficulties taking the following actions, you can contact a [specialised service provider](partner.). OVHcloud cannot assist with developing or optimising the content on your website.
>
...
Check the PHP engine, PHP version and runtime environment used on your Web Hosting plan, using our guide to [configuring your Web Hosting plan](configure_your_web_hosting1.).
...
To compare the available PHP versions depending on the runtime environment you are using, see **Step 2** in the guide on [configuring the PHP version on your web hosting](configure_your_web_hosting1.).
...
To analyse outgoing connections to your hosting plan, please read its **OUT** logs. You can use our documentation on [web hosting logs](logs_and_statistics1.).
...
To do this, please refer to your web hosting plan’s **WEB** logs using our guide on [how to view your hosting plan’s logs](logs_and_statistics1.).
...
>
>![Firefox Network Scan](F12.png){.thumbnail}
>
...
> [!primary]
>
> OVHcloud offers several [CDN options](hosting-options.). If you would like to use or activate one for your web hosting plan, log in to your [OVHcloud Control Panel](manager.) then read our guide on [Using the OVHcloud CDN](cdn_how_to_use_cdn1.).
>
...
> [!warning]
>
> The actions you carry out in your database can have irreversible consequences if they are not carried out methodically and correctly. Contact a [specialist provider](partner.) if you are unsure about what you need to do. 
>
...
See our guide on [optimising performance for your website](optimise_your_website_performance1.).
...
You might consider a more capable [shared hosting offer](hosting.) or a dedicated infrastructure such as a [Virtual Private Server (VPS)](vps.) or [Dedicated Server](bare-metal.). 
