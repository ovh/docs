...
- Disposer d'une [offre d'hébergement web OVHcloud](hosting.) avec une base de données mutualisée OVHcloud associée.
  
## En pratique
...
> 
> Nous mettons à votre disposition ce tutoriel afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](sql_overquota_database_#go-further.) de ce guide.
>

Lorsque votre base de données mutualisée OVHcloud arrive à saturation (**overquota**), nos robots vous avertissent par e-mail sur l'adresse e-mail du [contact « Administrateur »](managing_contacts1.) de la base de données. 
...
Récupérez les informations d'accès à votre base de données directement dans le fichier de configuration de votre site web. Réalisez cette action à l'aide de l'**étape 1** de notre guide sur [le changement du mot de passe d'une base de données](sql_change_password1.).

Connectez-vous à votre [espace client OVHcloud](manager.) et sélectionnez `Web Cloud`{.action} dans la barre de navigation en haut de l’écran. Cliquez sur `Hébergements`{.action} puis choisissez l’hébergement web associé à votre base de données mutualisée OVHcloud. Positionnez-vous enfin sur l'onglet `Bases de données`{.action}.
...
>
> Assurez-vous des manipulations que vous effectuez. Si vous éprouvez la moindre difficulté, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.). En effet, OVHcloud ne pourra pas vous fournir une assistance sur le contenu de votre base de données.
>
...
Il est possible de dupliquer le contenu de votre base de données OVHcloud directement vers une autre de vos bases de données OVHcloud grâce à une fonctionnalité présente dans votre [espace client OVHcloud](manager.). Pour cela, consultez notre guide « [Dupliquer le contenu d'une base de données dans une autre](copy_database1.) ».

Dans le cas d'une migration vers une base de données externe aux offres [Start SQL](hosting-options-startsql.) et [Web Cloud Databases](databases.), vous pouvez déplacer manuellement le contenu de votre ancienne base de données vers une nouvelle à l'aide de nos guides:

- [Exporter votre base de données existante](sql_database_export1.)
- [Premiers pas avec l'offre Web Cloud Databasess](starting_with_clouddb1.)
- [Importer votre ancienne base de données dans votre offre Web Cloud Databasess](restore-import-on-database-server1.)
...
> [!alert]
>
> La suite de ce guide vous décrit comment supprimer des données présentes dans votre base de données. Assurez-vous de ce que vous faites ou contactez un [prestataire spécialisé](partner.) en cas de doute.
>
...
[Dupliquer le contenu d'une base de données dans une autre](copy_database1.)
