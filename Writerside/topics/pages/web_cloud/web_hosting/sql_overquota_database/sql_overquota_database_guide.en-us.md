...
- An [OVHcloud web hosting plan](hosting.) with an associated OVHcloud shared database
  
## Instructions
...
> 
> If you experience any difficulties following the steps in this tutorial, we recommend contacting a [specialist provider](partner.) or reach out to the OVHcloud community. We will not be able to assist you. You can find more information in the [Go further](sql_overquota_database_#go-further.) section of this guide.
>

When your OVHcloud shared database reaches a certain saturation level (**overquota**), our robots will send a notification to the email address of the database’s [administrator contact](managing_contacts1.). 
...
Retrieve your database access data directly from your website’s configuration file. Perform this action using **step 1** in our guide to [changing a database password](sql_change_password1.).

Log in to your [OVHcloud Control Panel](manager.) and select `Web Cloud`{.action} in the top navigation bar. Click `Hosting plans`{.action} , then choose the web hosting plan associated to your OVHcloud shared database. Go to the `Databases`{.action} tab.
...
>
> Be sure about each command you execute on the database. If you experience any difficulties, we recommend contacting a [specialist provider](partner.). OVHcloud cannot assist you with the content of your database.
>
...
You can duplicate the content of your OVHcloud database directly to another of your OVHcloud databases, using a feature in your [OVHcloud Control Panel](manager.). To do this, please refer to our guide "[Duplicating the contents of one database to another](copy_database1.)".

If you are migrating to a database outside of the [Start SQL](hosting-options-startsql.) and [Web Cloud Databases](databases.) solutions, you can manually move the content from your old database to a new one using our guides:

- [Export your existing database](sql_database_export1.)
- [First steps with Web Cloud Databasess](starting_with_clouddb1.)
- [Import your old database into your Web Cloud Databasess solution](restore-import-on-database-server1.)
...
> [!alert]
>
> The rest of this tutorial will show you how to delete data stored in your database. Be sure of the actions you need to take or contact a [specialist provider](partner.) if in doubt.
>
...
[Duplicating the contents of one database to another](copy_database1.)
