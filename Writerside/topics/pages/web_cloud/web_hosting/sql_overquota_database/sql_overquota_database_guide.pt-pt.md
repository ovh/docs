...
- Ter um [serviço de alojamento web da OVHcloud](hosting.) com uma base de dados partilhada da OVHcloud associada.
...
> 
> Colocamos à sua disposição este tutorial para o acompanhar o melhor possível em tarefas comuns. No entanto, se encontrar dificuldades, recomendamos que recorra a um [fornecedor especializado](partner.). Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção ["Quer saber mais?"](sql_overquota_database_#go-further.) deste manual.
>

Quando a sua base de dados partilhada da OVHcloud chega a saturação (**overquota**), os nossos robôs avisam-no por e-mail no endereço de e-mail do [contacto "Administrador"](managing_contacts1.) da base de dados. 
...
Recupere as informações de acesso à sua base de dados diretamente no ficheiro de configuração do seu website. Realize esta ação através da **etapa 1** do nosso guia sobre [a alteração da palavra-passe de uma base de dados](sql_change_password1.).

Aceda à [Área de Cliente OVHcloud](manager.) e selecione `Web Cloud`{.action} na barra de navegação no topo do ecrã. Clique em `Alojamentos`{.action} e escolha o alojamento web associado à sua base de dados partilhada da OVHcloud. Por fim, clique no separador `Bases de dados`{.action}.
...
>
> Certifique-se de que as suas operações são efetuadas. Se tiver alguma dificuldade, recomendamos que recorra a um [fornecedor especializado](partner.). A OVHcloud não lhe poderá fornecer assistência relativamente ao conteúdo da sua base de dados.
>
...
É possível duplicar o conteúdo da sua base de dados OVHcloud diretamente para outra das suas bases de dados OVHcloud graças a uma funcionalidade presente na sua [Área de Cliente OVHcloud](manager.). Para isso, consulte o guia "[Duplicar o conteúdo de uma base de dados noutra](copy_database1.)".

No caso de uma migração para uma base de dados externa às ofertas [Start SQL](hosting-options-startsql.) e [Web Cloud Databases](databases.), pode migrar manualmente o conteúdo da sua antiga base de dados para uma nova através dos nossos guias:

- [Exportar a sua base de dados existente](sql_database_export1.)
- [Primeiros passos com a oferta Web Cloud Databasess](starting_with_clouddb1.)
- [Importar a sua antiga base de dados para o serviço Web Cloud Databasess](restore-import-on-database-server1.)
...
> [!alert]
>
> O resto deste manual explica-lhe como eliminar dados presentes na sua base de dados. Em caso de dúvida, certifique-se de que está a fazer ou a contactar um [fornecedor especializado](partner.).
>
...
[Duplicar o conteúdo de uma base de dados noutra](copy_database1.)
