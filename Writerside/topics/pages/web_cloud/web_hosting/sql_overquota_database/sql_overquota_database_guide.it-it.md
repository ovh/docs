...
- Disporre di una [offerta di hosting Web OVHcloud](hosting.) con un database condiviso OVHcloud associato
...
> 
> Mettiamo a tua disposizione questo tutorial per supportarti nelle operazioni più frequenti. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a uno [specialista del settore](partner.). OVH non potrà fornirti alcuna assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](sql_overquota_database_#go-further.) di questa guida.
>

Quando il database condiviso OVHcloud raggiunge la saturazione (**overquota**), i nostri robot ti avvertono via email all'indirizzo del [contatto "amministratore"](managing_contacts1.) del database. 
...
Recupera le informazioni di accesso al database direttamente nel file di configurazione del tuo sito Web. Per effettuare questa operazione, utilizza lo step 1** della nostra guida su [modificare la password di un database](sql_change_password1.).

Accedi allo [Spazio Cliente OVHcloud](manager.) e seleziona `Web Cloud`{.action} nella barra di navigazione in alto a sinistra. Clicca su `Hosting`{.action} e seleziona l'hosting Web associato al database condiviso OVHcloud. e clicca sulla scheda `Database`{.action}.

Visualizzi una tabella con tutti i database disponibili nella parte inferiore dello schermo.

![phpMyAdmin Access](sql_overquota_database_images_go-to-phpmyadmin.png){.thumbnail}
...
![phpMyAdmin Go Login](images_pma-interface-login.png){.thumbnail}
...
>
> Assicurati di aver effettuato le operazioni necessarie. In caso di difficoltà o dubbi, ti consigliamo di rivolgerti a un [provider specializzato](partner.). OVHcloud non potrà fornirti assistenza sul contenuto del database.
>

Una volta connesso, visualizzi questa pagina:

![phpMyAdmin Login](sql_overquota_database_images_pma-main-page.png){.thumbnail}

Nella colonna di sinistra clicca su `"Nome del database"`{.action} e poi su `Taille`{.action} in alto a destra.

![phpMyAdmin Tables](pma-check-size.png){.thumbnail}
...
È possibile duplicare il contenuto del database OVHcloud direttamente verso un altro database grazie a una funzionalità disponibile nello [Spazio Cliente OVHcloud](manager.). Per farlo, consulta la nostra guida "[Duplicare il contenuto di un database in un altro](copy_database1.)".

In caso di migrazione verso un database esterno alle offerte [Start SQL](hosting-options-startsql.) e [Web Cloud Databases](databases.), è possibile spostare manualmente il contenuto del vecchio database verso un nuovo database utilizzando le nostre guide:

- [Esporta il tuo database esistente](sql_database_export1.)
- [Iniziare a utilizzare l'offerta Web Cloud Databasess](starting_with_clouddb1.)
- [Importare il tuo database precedente nella tua offerta Web Cloud Databases](restore-import-on-database-server1.)
...
> [!alert]
>
> Questa guida ti mostra come eliminare i dati presenti nel tuo database. In caso di dubbi, assicurati di aver consultato uno [specialista del settore](partner.).
>
...
![phpMyAdmin SQL request](pma-sql-menu.png){.thumbnail}
...
[Duplicare il contenuto di un database in un altro](copy_database1.)
