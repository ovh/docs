...
- A configuração [Multisite](multisites_configure_multisite1.) do seu nome de domínio não está corretamente configurada para o seu diretório alvo
...
>
> Este manual fornece as instruções necessárias para realizar as operações mais habituais. No entanto, se encontrar dificuldades, recomendamos que recorra a um [prestador de serviços especializado](partner.) e/ou que contacte o editor do serviço. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção [Quer saber mais](diagnostic-index-of_#go-further.)?
>
...
O seu nome de domínio é declarado para aceder a um diretório alvo (um "`Pasta raiz`") no servidor [FTP](ftp_connection1.) do seu alojamento web partilhado. Isto no separador [Multisite](multisites_configure_multisite1.) do seu alojamento web presente no seu [Área de Cliente OVHcloud](manager.).
...
> [!primary]
>
> Para associar temporariamente o seu domínio a um `Pasta raiz` que não contém ficheiro **index.php** ou **index.html**, pode proibir a apresentação da lista de pastas do seu site seguindo este [tutorial](htaccess_what_else_can_you_do1.). Pode também proteger o acesso às suas pastas com uma [palavra-passe](htaccess_protect_directory_by_password1.).

>
> Se precisar de ajuda, recomendamos que recorra a um [prestador de serviços especializado](partner.). As nossas equipas de suporte não poderão fornecer-lhe assistência relativamente a qualquer alteração da programação interna do seu site.

### Resolver o caso mais comum de uma página "Index of"

Importou os ficheiros do seu site **mydomain.ovh** para a pasta `www` do seu alojamento por [FTP](ftp_connection1.). Se o domínio não estiver associado a esta pasta, consulte a coluna `Pasta raiz` do `Multisite`{.action}.
...
> [!primary]
>
> A utilização do diretório `www` como `Pasta raiz` não é, em caso algum, obrigatória. Pode instalar o seu site noutra pasta do seu [servidor FTP](ftp_connection1.).
>
