...
- Konfiguracja [MultiSite](multisites_configure_multisite1.) Twojej domeny nie została poprawnie skonfigurowana w katalogu docelowym
- Folder docelowy, do którego Twoja nazwa domeny nie zawiera plików **"index.html"** lub **"index.php"**

![indeks](index-of.png){.thumbnail}
...
- Zalogowanie do [Panelu klienta OVHcloud](manager.)

## W praktyce

### Zrozumieć pochodzenie strony "Index of"

Twoja domena jest zadeklarowana, aby uzyskać dostęp do katalogu docelowego ("`Katalog główny`") na serwerze [FTP](ftp_connection1.) na hostingu. Jest to w zakładce [MultiSite](multisites_configure_multisite1.) Twojego hostingu w [Panelu klienta OVHcloud](manager.).
...
> [!primary]
>
> Aby tymczasowo powiązać domenę z `Katalogiem głównym` niezawierającym pliku **index.php** lub **index.html**, możesz zablokować wyświetlanie listy katalogów na swojej stronie internetowej zgodnie z tym [tutorial](htaccess_what_else_can_you_do#blokada-listowania-zawartosci-katalogu.). Możesz również chronić dostęp do Twoich katalogów za pomocą [hasła](htaccess_protect_directory_by_password1.).
>
> W przypadku trudności z wdrożeniem tej konfiguracji zalecamy skorzystanie z [pomocy specjalisty](partner.). Zespół pomocy technicznej OVHcloud nie będzie w stanie udzielić wsparcia w przypadku jakichkolwiek zmian w wewnętrznym programowaniu Twojej strony WWW.

### Rozwiąż najczęstszy przypadek na stronie "Index of"

Przeprowadziłeś pliki strony **mydomain.ovh** do katalogu `www` hostingu przez [FTP](ftp_connection1.). Poza tym Twoja domena nie jest powiązana z tym folderem w kolumnie `Katalog główny` Twojej strony w opcji `MultiSite`{.action}.
...
> [!primary]
>
> Używanie katalogu `www` jako `Katalogu główny` nie jest w żadnym wypadku obowiązkowe. Możesz zainstalować Twoją stronę WWW w innym katalogu Twojego [serwera FTP](ftp_connection1.).
>
...
[Instalacja kilku stron WWW na jednym hostingu](multisites_configure_multisite1.)
