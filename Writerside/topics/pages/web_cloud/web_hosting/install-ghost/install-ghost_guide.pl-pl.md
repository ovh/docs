...
[Hosting Cloud Web](hosting-cloud-web-offer.){.external} umożliwia użycie Node.js jako frameworka dla Twoich stron WWW oraz instalację i hosting silnika Ghost lub innej aplikacji stworzonej na potrzeby Node.js.
...
- Hosting [Cloud Web](hosting-cloud-web-offer.){.external} od OVHcloud
- Node.js aktywowany jako framework
- Domenę dodaną w opcji MultiSite oraz Node.js wskazany jako jej framework.
- Dostęp do [Panelu klienta](manager.){.external} > sekcja `Web Cloud`{.action}
-Co powinieneś umieć:
...
Aby uzyskać dostęp do frameworków Twojego hostingu Cloud Web, zaloguj się do [Panelu klienta](manager.){.external}. Po zalogowaniu kliknij `Hosting`{.action} na pasku usług po lewej stronie, następnie wybierz odpowiedni hosting Cloud Web. Teraz przejdź do zakładki `Frameworki`{.action}.
...
Po uzupełnieniu informacji, kliknij przycisk `Zatwierdź`{.action}. Jeśli potrzebujesz więcej informacji o zarządzaniu frameworkami, skorzystaj z naszego przewodnika [Zarządzanie frameworkami Cloud Web](manage-runtime1.){.external}.
...
Następnie przekieruj plik `server.js` (określony [podczas etapu 1](install-ghost_#etape-1-activer-nodejs-comme-moteur-dexecution.)) do pliku `index.js` silnika Ghost:
...
Aby ponownie uruchomić *daemon* Node.js, wróć do [Panelu klienta](manager.){.external}. Przejdź do zakładki `MultiSite`{.action}, po prawej stronie nazwy wybranej domeny kliknij na przycisk `...`{.action} po prawej stronie, po czym kliknij `Uruchom ponownie`{.action}.
