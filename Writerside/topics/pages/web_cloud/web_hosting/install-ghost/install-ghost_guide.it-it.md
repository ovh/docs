...
Gli [hosting Cloud Web OVHcloud](hosting-cloud-web-offer.){.external} permettono di utilizzare Node.js come motore di esecuzione per i siti Web consentendo quindi l’installazione e l’hosting di Ghost o qualsiasi altra applicazione sviluppata per Node.js.
...
- Disporre di un [hosting Cloud Web](hosting-cloud-web-offer.){.external} attivo
...
Accedi allo [Spazio Cliente OVHcloud](manager.){.external}, seleziona il tuo servizio Cloud Web nella sezione `Hosting`{.action} nel menu a sinistra  e clicca sulla scheda `Programma di esecuzione`{.action}.
...
Una volta inserite tutte le informazioni, clicca su `Conferma`{.action}. Per maggiori informazioni sui motori di esecuzione, consulta la nostra guida [Gestire i motori di esecuzione di un hosting Cloud Web](manage-runtime1.){.external}.
...
Per riavviare il daemon Node.js, torna allo [Spazio Cliente OVHcloud](manager.){.external}. Apri la scheda `Multisito`{.action}, clicca sul pulsante `...`{.action} a destra in corrispondenza del tuo dominio e seleziona `Riavvia`{.action}.
...
[Migrare un sito e un servizio di posta in OVHcloud](hosting_migrating_to_ovh1.){.external} (in inglese)

[Pubblicare un sito Web](hosting_how_to_get_my_website_online1.){.external} (in inglese)

[Installare i moduli in 1 click OVHcloud](cms_install_1_click_modules1.){.external}

[Ospitare più siti su uno stesso hosting](multisites_configure_multisite1.){.external} (in inglese)
