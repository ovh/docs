...
To restart the Node.js daemon, go back to the [OVHcloud Control Panel](manager.){.external}. Go to the `Multisite`{.action} tab, click on `...`{.action} button to the right of the domain name concerned, then click `Restart`{.action}.
