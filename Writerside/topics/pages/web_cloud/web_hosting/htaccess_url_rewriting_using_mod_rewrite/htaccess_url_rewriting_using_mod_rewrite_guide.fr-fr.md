...
> 
> Ce tutoriel est mis à votre disposition afin de vous aider au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](htaccess_url_rewriting_using_mod_rewrite_#go-further.) de ce guide.
>
...
Pour éditer (ou créer) des répertoires, connectez-vous à l'espace FTP de votre hébergement. Au besoin, aidez-vous du guide « [Accéder à mon espace de stockage](ftp_connection1.) ».
...
Consultez notre guide sur la [configuration d'un multisite sur un hébergement mutualisé](multisites_configure_multisite1.) si vous souhaitez plus d'informations sur le sujet.
...
Si vous ne disposez pas d'un certificat SSL, consultez notre guide sur la [gestion d'un certificat SSL sur un hébergement mutualisé OVHcloud](ssl_on_webhosting1.).
...
[Bloquer l'accès à mon site web pour certaines adresses IP via un fichier .htaccess](htaccess_how_to_block_a_specific_ip_address_from_accessing_your_website1.)
