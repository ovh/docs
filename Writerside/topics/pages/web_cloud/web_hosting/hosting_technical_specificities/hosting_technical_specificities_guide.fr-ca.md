...
- Disposer d'une [offre d'hébergement web](hosting.){.external} compatible.
...
>
> Nous mettons à votre disposition ce guide afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) et/ou de contacter l'éditeur du service si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section « [Aller plus loin](hosting_technical_specificities_#go-further.) » de ce guide.
>
...
- Si vous rencontrez l'erreur d'accès « Échec de l'authentification de connexion 530 » lors de la connexion à votre espace de stockage FTP: Assurez-vous que les informations d'accès à votre espace FTP sont correctes. Pour cela, connectez-vous à votre [espace client OVHcloud](manager.) puis rendez-vous dans la partie `Web Cloud`{.action}. Dans la colonne de gauche, cliquez sur `Hébergements`{.action} puis sélectionnez l'hébergement web concerné. Sur la page qui s'affiche, cliquez sur l'onglet `FTP - SSH`{.action}.
...
Retrouvez plus d'informations sur le sujet dans notre guide « [Se connecter à l'espace de stockage FTP de son hébergement Web](ftp_connection1.) ».
...
- [Suivre et gérer les e-mails automatisés de son hébergement web](mail_function_script_records1.);
...
Sur les offres d'hébergement Web (bases de données partagées), il existe une limite de 30 connexions simultanées par base de données (cette limite passe à 200 si vous utilisez une offre [Web Cloud Databases](databases.)). Consulter le [détail de nos offres d'hébergement web](hosting.) pour connaître les options disponibles dans chaque offre d'hébergement Web.

Vous pouvez également commander des offres [Web Cloud Databases](databases.) supplémentaires, celles-ci disposent d'options de personnalisation :
...
Pour en savoir plus, consultez les détails de nos [offres d'hébergement web](hosting.) et notre guide « [Premiers pas avec votre offre Web Cloud Databases](starting_with_clouddb1.) ».
...
Seuls les serveurs de bases de données [Web Cloud Databases](databases.) permettent à des serveurs externes de s'y connecter. Ceci en ayant préalablement autorisé l'adresse IP de votre serveur externe sur votre serveur de bases de données. Au besoin, consultez notre guide « [Premiers pas avec votre offre Web Cloud Databases](starting_with_clouddb1.) ».
...
Pour plus d'informations sur la gestion des bases de données et sur la connection à l'interface *phpMyAdmin*, reportez-vous au guide « [Créer une base de données sur son hébergement web](sql_create_database1.) ».

### PHP

Nous vous recommandons de consulter nos [offres d'hébergement web](hosting-programming-language.) pour vous assurer que l'offre d'hébergement Web que vous souhaitez commander convient à vos besoins.
...
>
> Retrouvez plus de détails sur ce sujet dans notre guide « [Hébergement web : environnement, version PHP, « .ovhconfig »](configure_your_web_hosting1.) »
>
...
Découvrez comment [se connecter à l’espace de stockage FTP de votre hébergement web](ftp_connection1.) ou [restaurer l’espace de stockage FTP de son hébergement web](ftp_save_and_backup1.) sur nos documentations.
...
Pour les bases de données mutualisées (comprises dans votre offre d'hébergement Web) ou les serveurs de base de données (Web Cloud Databases), proposés sur Gravelines (GRA), en France et Beauharnois (BHS), au Canada, la sauvegarde des bases est faite tous les jours. Ces sauvegardes sont accessibles (via [l'espace client OVHcloud](manager.){.external} ou via les [API OVHcloud](https://api.ovh.com/)). Les sauvegardes sont également stockées sur une autre infrastructure. Ces données sont répliquées dans un centre de données situé à Strasbourg (SBG). La politique de rétention des sauvegardes est de 30 jours.

Découvrez comment [Récupérer la sauvegarde de la base de données d’un hébergement web](sql_database_export1.) sur notre documentation.
...
[Se connecter à l’espace de stockage FTP de son hébergement web](ftp_connection1.)

[Passer son site internet en HTTPS grâce au SSL](ssl-activate-https-website1.)

[Optimisation des performances de votre site](optimise_your_website_performance1.)

[Restaurer l’espace de stockage FTP de son hébergement web](ftp_save_and_backup1.)
