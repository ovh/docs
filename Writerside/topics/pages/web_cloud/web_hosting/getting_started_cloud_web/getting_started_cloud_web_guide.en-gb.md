...
For our new [Cloud Web](hosting-cloud-web-offer.) solution, we have combined 20 years of experience in web hosting with the power of our Public Cloud. Like our standard hosting plans, your websites are hosted on a service that is managed 24/7. However, [Cloud Web](hosting-cloud-web-offer.) offers an even wider range of features, and higher performance levels for hardware such as SSD disks.
...
- a [domain name](domains.) for your website
...
OVHcloud [Cloud Web](hosting-cloud-web-offer.) hosting offers a wider range of configuration options than standard web hosting plans, so that you can tailor your plan to suit your project. To ensure that your project is a success, it is important to have a clear idea of your objective. To do this, you should:
...
- **Ensure that your project is technically compatible with Cloud Web hosting.** Need a particular runtime software type or SQL? If you have not already done so, check that what you need is available with your [Cloud Web](hosting-cloud-web-offer.) hosting plan.
...
With [Cloud Web](hosting-cloud-web-offer.), you can choose from a range of different coding languages to build your project. If you would like to use a language other than PHP, which is the default selection, you will need to select a runtime software application that corresponds to your coding language.
...
To access the runtime software applications for your [Cloud Web](hosting-cloud-web-offer.) hosting plan, log in to the [OVHcloud Control Panel](manager.){.external}, click `Hosting plans`{.action} in the services bar on the left-hand side, then select the name of the Cloud Web hosting plan concerned. Next, go to the `Runtime software`{.action} tab.
...
Depending on which [Cloud Web](hosting-cloud-web-offer.) hosting plan you have chosen, you can also add additional runtime software applications by clicking the `Actions`{.action} button, then `Add a runtime software application`{.action}. Please note that the maximum number of runtime software applications will vary, depending on which [Cloud Web](hosting-cloud-web-offer.) solution you have ordered.
...
If you would like to deploy your project several times in different environments (e.g. development, test and production), you will need to provide variables in order for your code to run properly. To do this, [Cloud Web](hosting-cloud-web-offer.) prompts you to define environment variables that can be accessed by the code for your website or web application.
...
Repeat this step if you would like to add several domain names to your Cloud Web hosting plan. For more information on adding a domain name as a multisite, please read the following guide: [Hosting multiple websites on your Web Hosting plan](multisites_configure_multisite1.){.external}.
...
There is no clear-cut, single approach to take since every project is different, but our guides to [Publishing a website on your Web Hosting space](hosting_how_to_get_my_website_online1.){.external} and [Migrating your website and emails to OVHcloud](hosting_migrating_to_ovh1.){.external} should be able to help you establish how to proceed.
...
|---|---|---|
|A|For the website|In the [OVHcloud Control Panel](manager.){.external}, go to the `Hosting`{.action} section for the hosting plan concerned. Copy the IP address that appears next to "IPv4" in the `General information`{.action} tab.|
|MX|For emails|In the [OVHcloud Control Panel](manager.){.external}, go to the `Emails`{.action} section for the domain name concerned. Copy the information that appears next to “MX records” in the `General information`{.action} tab.|

#### 2. Check and/or modify the DNS records.

Now that you are more familiar with the OVHcloud DNS records for your [Cloud Web](hosting-cloud-web-offer.) hosting plan and your OVHcloud email solution, you can check if they have been configured properly, and edit them if required. These two changes must be made with the service provider managing your domain name’s DNS configuration (DNS zone).
...
> 
> - If your domain name is registered with OVHcloud, you can check if the domain name is using our DNS configuration. To do this, go to the [Control Panel](manager.){.external}, select the domain name concerned, and go to the `DNS servers`{.action} tab.
>
...
|---|---|
|OVHcloud|In the [OVHcloud Control Panel](manager.){.external}, go to the `Domains`{.action} section for the domain name concerned. Then in the DNS zone tab, check and edit the information as required. If you need any further assistance, you can read our guide to [Editing an OVHcloud DNS zone](dns_zone_edit1.){.external}.|
...
[Migrating your website and emails to OVHcloud](hosting_migrating_to_ovh1.){.external}

[Publishing a website on your Web Hosting space](hosting_how_to_get_my_website_online1.){.external}

[Setting up your website with 1-click modules](cms_install_1_click_modules1.){.external}

[Hosting multiple websites on your Web Hosting plan](multisites_configure_multisite1.){.external}
