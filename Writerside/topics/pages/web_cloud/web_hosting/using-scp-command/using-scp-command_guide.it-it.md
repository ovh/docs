...
- Disporre di una soluzione di [hosting Web](hosting.) con accesso in SSH
...
Per recuperare gli accessi SSH del tuo hosting Web, consulta la nostra guida "[Utilizzare l’accesso SSH del tuo hosting Web](ssh_on_webhosting1.)".
...
- `ssh.cluster0XX.hosting.ovh.net`: sostituisci gli `XX` con il numero del cluster in cui si trova il tuo hosting Web. In caso di necessità, consulta la nostra guida "[Utilizzare l’accesso SSH del proprio hosting Web](ssh_on_webhosting1.)" per recuperare questa informazione.
- `/homez.XXX/FTP-main-login/`: modifica i `XXX` con il numero del *filer* e il `FTP-main-login` con i parametri recuperati allo [step 2](using-scp-command_#step2.) di questa guida.
...
Per verificare che alcuni contenuti presenti in locale sul tuo computer siano stati copiati sul tuo hosting Web, [accedi allo spazio di storage FTP del tuo hosting Web](ftp_connection1.) e clicca sulla cartella di destinazione in cui i contenuti devono essere copiati.
...
[Utilizzare l’accesso SSH del proprio hosting Web](ssh_on_webhosting1.)

[Accedere allo spazio di storage FTP dell’hosting Web](ftp_connection1.)
