...
- Tener contratado un [plan de hosting de OVHcloud](hosting.){.external} compatible.
- Tener uno o más [dominios](domains.){.external}.
- Poder modificar la configuración de sus dominios (la [zona DNS](dns_zone_edit#entender-el-concepto-de-dns.)).
...
> [!primary]
>
> La mayoría de los [planes de hosting de OVHcloud](hosting.){.external} incluyen una opción de creación de direcciones de correo personalizadas con su nombre de dominio.
...
> No dude en consultar nuestra [guide](activate-email-hosting1.) para más detalles sobre la activación de esta opción.
>

### 1\. Acceder a la gestión del multisitio <a name="multisite-menu"></a>

En primer lugar, conéctese al [área de cliente de OVHcloud](manager.){.external} y seleccione `Web Cloud`{.action}. haga clic en `Alojamientos`{.action} y seleccione el alojamiento correspondiente. A continuación, abra la pestaña `Multisitio`{.action}.
...
![Multisitio](multisites_configure_multisite_images_tab.png){.thumbnail}
...
![acciones](actions-menu.png){.thumbnail}
...
![Multisitio](add-a-domain-or-sub-domain-ovh-step-1.png){.thumbnail}
...
A continuación, deberá personalizar la adición del dominio o subdominio. Según el [plan de hosting de OVHcloud](hosting.){.external} contratado, algunas opciones podrían no estar disponibles.
...
![Multisitio](add-a-domain-or-sub-domain-ovh-step-2.png){.thumbnail}
...
|SSL|Permite disfrutar de una conexión segura (https://) en el dominio seleccionado. Más información en nuestra página sobre [SSL](hosting-options-ssl.){.external}. Si activa conjuntamente las opciones SSL y CDN (Content Delivery Network), podrá disfrutar también del protocolo **HTTP2** (activo por defecto en nuestro datacenter de Gravelines).|
|Activar la CDN|Permite activar el servicio CDN (almacenamiento en caché de los elementos estáticos del sitio web, como las imágenes) en el dominio seleccionado. Más información en nuestra página sobre [CDN](hosting-options-cdn.){.external}. Si activa conjuntamente las opciones SSL y CDN, podrá disfrutar también del protocolo **HTTP2** (activo por defecto en nuestro datacenter de Gravelines).|
|IP del país|Permite disfrutar de una dirección IP geolocalizada (a elegir entre diversos países) en el dominio seleccionado. Más información en nuestra página sobre las [IP](hosting-options.){.external}.|
|Activar el firewall|Permite activar un firewall (análisis de las peticiones) en el dominio seleccionado. Más información en nuestra página sobre [Mod Security](hosting-options.){.external}.|
|Logs separados|Permite activar un nuevo espacio de logs en el dominio seleccionado. Deberá elegir un dominio de la lista, que determinará el nombre de acceso al nuevo espacio. Más información en [nuestra página sobre estadísticas completas](hosting-traffic-analysis.){.external}.|
...
![Multisitio](add-a-domain-or-sub-domain-ovh-step-3.png){.thumbnail}
...
- **Para una configuración DNS manual**, desmarque la casilla `Configuración automática (recomendado)`{.action}. En este caso, se mostrarán los parámetros que debe modificar.  Si desea realizar esta configuración, consulte nuestra guía ["Editar una zona DNS de OVHcloud"](dns_zone_edit1.){.external}.
...
>
> ![Multisitio](multisites_configure_multisite_images_find-ipv4-and-ipv6.png){.thumbnail}
>

 Una vez seleccionado el dominio que quiera asociar al alojamiento, deberá personalizar su información. Tenga en cuenta que algunas opciones incluidas en su [plan de hosting de OVHcloud](hosting.){.external} no pueden activarse durante este proceso. Es necesario añadir el dominio en primer lugar y posteriormente activar dichas opciones modificando la configuración del multisitio.
...
|Carpeta raíz| Defina la carpeta, en su espacio de almacenamiento, hacia la que apunta el dominio . Los archivos del sitio web deberán publicarse en este espacio. Por ejemplo, para blog.domain.tld, la carpeta raíz podría ser "blog". Si la carpeta no existe, se creará automáticamente.|
|Activar IPv6|Permite activar el protocolo IPv6 en el dominio indicado. Más información en nuestra página sobre las [IP](hosting-options.){.external}.|

Una vez introducidos los datos, haga clic en el botón `Siguiente`{.action}. Compruebe que la información que se muestra en el resumen es correcta.

![Multisitio](add-a-domain-or-sub-domain-external-step-2.png){.thumbnail}
...
![Multisitio](add-a-domain-or-sub-domain-external-step-3.png){.thumbnail}
...
Si necesita ayuda, puede disfrutar de una infraestructura de sitio web lista para usar gracias a los módulos en 1 clic de OVHcloud. Si utiliza estos módulos, el sitio web se instalará automáticamente en la carpeta raíz configurada anteriormente. Para más información, consulte nuestra guía [Instalar un sitio web con un módulo en un clic](cms_install_1_click_modules1.){.external}. 

Si, por el contrario, quiere instalar manualmente su sitio web, deberá subir los archivos a la carpeta raíz adecuada de su espacio de almacenamiento. Para más información, consulte nuestra guía [Publicar un sitio web en internet](hosting_how_to_get_my_website_online1.){.external}.
...
>
> Recuerde que cuantos más sitios web aloje en su hosting, mayor será la demanda de recursos asignados. [En la página de nuestros planes de hosting](hosting.){.external} puede consultar el número de sitios web recomendados que puede alojar en su espacio.
>

## Más información

[Instalar un sitio web con un módulo en un clic](cms_install_1_click_modules1.){.external}

[Editar una zona DNS de OVHcloud](dns_zone_edit1.){.external}

[Publicar un sitio web en internet](hosting_how_to_get_my_website_online1.){.external}
