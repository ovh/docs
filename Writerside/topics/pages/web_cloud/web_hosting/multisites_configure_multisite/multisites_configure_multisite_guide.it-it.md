...
- Disporre di una soluzione [di hosting Web OVHcloud](hosting.){.external} compatibile
- Disporre di uno o più [domini](domains.){.external}
- Poter modificare la configurazione dei tuoi domini (la [zona DNS](dns_zone_edit#capire-il-concetto-di-dns.))
...
> [!primary]
>
> La maggior parte delle soluzioni di [hosting Web OVHcloud](hosting.){.external} includono un’opzione per la creazione di indirizzi email personalizzati con il dominio.
...
> Per maggiori informazioni sull’attivazione di questa opzione, consulta la nostra [guida](activate-email-hosting1.).
>

### Step 1: accedi alla gestione del multisito <a name="multisite-menu"></a>

Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e seleziona `Web Cloud`{.action}. clicca su `Hosting`{.action}, seleziona il tuo servizio e clicca sulla scheda `Multisito`{.action}.
...
![multisito](multisites_configure_multisite_images_tab.png){.thumbnail}
...
![azioni](actions-menu.png){.thumbnail}

- **Aggiungi un dominio registrato in OVHcloud**:

Nello Spazio Cliente OVHcloud vengono mostrati solo i domini per i quali sei [contatto tecnico o amministratore](managing_contacts1.). Selezionane uno dalla lista e clicca su `Seguente`{.action}. Prosegui seguendo lo [Step 2.1: aggiungere un dominio registrato in OVHcloud](#add-ovhcloud-domain.).
...
![multisito](add-a-domain-or-sub-domain-ovh-step-1.png){.thumbnail}
...
A questo punto è necessario personalizzare l'aggiunta del dominio o sottodominio. In base al piano di [hosting Web](hosting.){.external} attivato, alcune opzioni potrebbero non essere disponibili.
...
![multisito](add-a-domain-or-sub-domain-ovh-step-2.png){.thumbnail}
...
|SSL|Permette di usufruire di una connessione sicura (https://) sul dominio selezionato.  Per maggiori informazioni, accedi alla pagina relativa ai [certificati SSL](hosting-options-ssl.){.external}. Attivando contemporaneamente le opzioni SSL e CDN (Content Delivery Network), potrai usufruire anche del protocollo **HTTP2**.|
|Attiva la CDN|Permette di attivare la CDN sul dominio selezionato memorizzando in cache gli elementi statici di un sito, ad esempio le immagini. Per maggiori informazioni, accedi alla pagina relativa alla [CDN](hosting-options-cdn.){.external}.  Attivando contemporaneamente le opzioni SSL e CDN (Content Delivery Network), potrai usufruire anche del protocollo **HTTP2**.|
|IP del Paese|Permette di usufruire di un indirizzo IP geolocalizzato (a scelta tra diversi Paesi) per il dominio selezionato. Per maggiori informazioni, accedi alla pagina relativa agli [IP](hosting-options.){.external}. |
|Attiva il firewall|Permette di attivare un firewall (analisi del traffico) sul dominio selezionato.  Per maggiori informazioni, accedi alla pagina relativa a [Mod Security](hosting-options.){.external}.|
|Log separati|Permette di attivare un nuovo spazio di log sul dominio selezionato. Dovrai scegliere, tra quelli disponibili nella lista, il dominio che determinerà il nome di accesso al nuovo spazio. Per maggiori informazioni, accedi alla [pagina relativa alle statistiche dettagliate](hosting-traffic-analysis.){.external}.|
...
![multisito](add-a-domain-or-sub-domain-ovh-step-3.png){.thumbnail}
...
- **Per una configurazione DNS manuale**, deseleziona la casella `Configurazione automatica (consigliata)`{.action} e recupera i parametri da modificare. Per eseguire questa operazione, consulta la guida [Modificare una zona DNS di OVHcloud](dns_zone_edit1.){.external}.
...
> |---|---|---|
> |TXT|Nella scheda `Multisito`{.action}, clicca sul pulsante `Configura il token ovhcontrol`{.action} in basso a sinistra.|Consente a OVHcloud di assicurarsi che l'aggiunta di ciascun dominio esterno sia legittima. Provvedi a creare il record TXT con il sottodominio ovhcontrol (ad esempio, ovhcontrol.domain.tld) nella zona DNS che è l'autorità per il dominio da aggiungere.<br></br>Per aggiungere `blog.domain.tld` è necessario creare il record per il sottodominio `ovhcontrol.domain.tld` e non `ovhcontrol.blog.domain.tld`.<br></br>Per recuperarla, consulta i [server DNS](dns_server_general_information#capire-il-concetto-di-dns.) a cui è associato il tuo dominio. Dovrai convalidare solo il dominio principale, non tutti i sottodomini.|
>
> ![multisito](find-token.png){.thumbnail}
>
> |Campo|Dove trovare l’informazione|Azione da realizzare|
> |---|---|---|
...
>
> ![multisito](multisites_configure_multisite_images_find-ipv4-and-ipv6.png){.thumbnail}
>

 Una volta selezionato il dominio che vuoi associare all’hosting, è necessario personalizzarne i parametri. Tieni presente che alcune delle opzioni incluse nel tuo piano di [hosting Web](hosting.){.external} non potranno essere attivate durante questo processo: sarà prima necessario aggiungere il dominio e modificare i parametri del multisito.
...
|Cartella di root| Definisci la cartella, sul tuo spazio di archiviazione, verso cui il dominio punta. È in questo spazio che i file del sito dovranno essere messi online. Ad esempio, per blog.domain.tld, la cartella di root potrebbe essere "blog". Se la cartella non esiste, verrà creata automaticamente.|
|Attiva l’IPv6|Permette di attivare il protocollo IPv6 sul dominio selezionato.  Per maggiori informazioni, accedi alla pagina relativa agli [IP](hosting-options.){.external}. |

Una volta inseriti tutti i dati, clicca su `Seguente`{.action} e verifica la correttezza delle informazioni inserite.

![multisito](add-a-domain-or-sub-domain-external-step-2.png){.thumbnail}
...
![multisito](add-a-domain-or-sub-domain-external-step-3.png){.thumbnail}
...
[Modificare una zona DNS OVHcloud](dns_zone_edit1.){.external}

[Mettere online un sito Internet su un hosting Web](hosting_how_to_get_my_website_online1.){.external}
