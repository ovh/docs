...
> 
> Nous mettons à votre disposition ce tutoriel afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [« Aller plus loin »](cms_what_to_do_if_your_site_is_hacked_#go-further.) de ce guide.
>

## Prérequis

- Disposer d'une [offre d'hébergement Web Cloud](hosting.) avec votre site Web hébergé dessus.
...
- [Changer le mot de passe d'accès à votre identifiant client OVHcloud](manage-ovh-password1.).
- [Sécuriser l'accès à votre espace client OVHcloud avec une double authentification](secure-ovhcloud-account-with-2fa1.).
- [Modifier le mot de passe d'accès à l'espace de stockage FTP de votre hébergement Web](ftp_change_password1.).
- [Changer le mot de passe d'accès à votre base de données](sql_change_password1.).

Nous vous recommandons également d'utiliser un [gestionnaire de mots de passe](manage-ovh-password#utiliser-un-gestionnaire-de-mots-de-passe.).
...
> [!warning]
>
> Si vous éprouvez des difficultés pour effectuer les actions décrites ci-dessous, faites appel à un [prestataire spécialisé](partner.) en cyber sécurité.
>

Utilisez notre guide sur [les statistiques et logs de votre hébergement Web](logs_and_statistics1.) pour rechercher les éléments malveillants injectés dans votre site web. Vous trouverez les informations dans les logs « web ». 
...
>
> Si vous êtes absolument sûr qu'il s'agit d'une IP malveillante, vous pouvez lui bloquer l'accès à votre hébergement en suivant notre documentation sur les [restrictions d'accès via le fichier « .htaccess »](htaccess_how_to_block_a_specific_ip_address_from_accessing_your_website1.).
> 
...
- [Restaurer l'espace de stockage FTP de votre hébergement Web](ftp_save_and_backup1.)
- [Récupérer la sauvegarde SQL de votre base de données](sql_database_export1.)
- [Importer la sauvegarde SQL dans votre base de données](sql_importing_mysql_database1.)
...
>
> Si votre hébergement est en état « désactivé », les robots de restauration automatique disponibles depuis votre [espace client OVHcloud](manager.) seront désactivés également.
...
>
> Pour réactiver l'hébergement Web, suivez les instructions de l'étape 4 de ce [guide](diagnostic_403_forbidden1.).
>
...
Vous devrez supprimer manuellement les fichiers et les codes malveillants préalablement détectés lors de [l'étape 2](cms_what_to_do_if_your_site_is_hacked_#step2.) de ce guide puis corriger les failles de sécurité de votre site.

Pour vous connecter à l'espace de stockage de votre hébergement, consultez [notre guide](ftp_connection1.) sur le sujet.
...
Retrouvez plus de détails sur les droits « CHMOD » dans la section « Informations utiles » de notre [tutoriel sur l'utilisation du client FTP Filezilla](ftp_filezilla_user_guide#useful-information.).
...
Sécurisez vos formulaires de contact à minima à l'aide d'un système de type « Captcha », afin d'éviter que des robots malveillants n'émettent du SPAM par ce biais. Si la fonction « mail() » de PHP a également été bloquée sur votre hébergement, consultez [notre guide](mail_function_script_records1.) sur ce sujet pour résoudre ce blocage.

Consultez également notre guide sur [comment sécuriser votre site Web](secure_your_website1.), afin de diminuer au maximum le risque qu'un nouveau piratage se produise.

## Aller plus loin <a name="go-further"></a>

[Se connecter à l'espace de stockage de son hébergement Web](ftp_connection1.)

[Modifier la configuration de son hébergement Web](configure_your_web_hosting1.)

[Activer le pare-feu applicatif](multisites_activating_application_firewall1.)

[Optimiser les performances de son site](optimise_your_website_performance1.)
