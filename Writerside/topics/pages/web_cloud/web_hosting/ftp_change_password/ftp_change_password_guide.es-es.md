...
Este acceso permite, entre otras cosas, [publicar el sitio web](hosting_how_to_get_my_website_online1.).
...
Conéctese al [área de cliente de OVHcloud](manager.), acceda a la sección `Web Cloud`{.action}, haga clic en `Alojamientos`{.action} y seleccione el alojamiento correspondiente. Seleccione la pestaña `FTP-SSH`{.action}.
...
Según el plan de [hosting de OVHcloud](hosting.), la modificación de la contraseña del usuario FTP a través de la pestaña `FTP-SSH`{.action} se realizará por dos caminos diferentes:
...
Para acceder a su espacio de almacenamiento FTP, consulte nuestra guía ["Conectarse al espacio de almacenamiento de un alojamiento web"](ftp_connection1.).
...
[Publicar el sitio web](hosting_how_to_get_my_website_online1.)
