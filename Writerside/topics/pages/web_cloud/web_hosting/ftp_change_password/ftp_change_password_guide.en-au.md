...
This access allows you to [put your website online](hosting_how_to_get_my_website_online1.).
...
Log in to the [OVHcloud Control Panel](manager.). Go to the `Web Cloud`{.action} section, click on `Hosting plans`{.action}, then select the service concerned. Click on the `FTP-SSH`{.action} tab.
...
Depending on which OVHcloud [Web Hosting plan](hosting.) you have, there are two different paths for changing your FTP user password user via the `FTP-SSH`{.action} tab:
...
To access your FTP storage space, please refer to our guide ["Logging in to your Web Hosting plan’s storage space"](ftp_connection1.).
...
[Put your website online](hosting_how_to_get_my_website_online1.)
