...
Su base de datos es un elemento central en la construcción de su sitio web dinámico. Durante el ciclo de vida de su sitio web, puede que necesite copiar el contenido de su base de datos en otra de sus bases de datos [start SQL](hosting-options-startsql.) o [Web Cloud Databases](databases.).
...
- Tener contratadas bases de datos [start SQL](hosting-options-startsql.) y/o [Web Cloud Databases](databases.). Es necesario crear previamente las dos bases de datos para poder utilizar la herramienta de duplicación.
- Estar conectado al [área de cliente de OVHcloud](manager.)
- Disponer de los permisos necesarios para todos los servicios de bases de datos afectados. Para más información, consulte nuestra guía [Gestionar los contactos de sus servicios](managing_contacts1.).
...
- una base de datos [Start SQL](hosting-options-startsql.) (incluida en algunos de nuestros [alojamientos web](hosting.) o [contratada por separado](hosting-options-startsql.);
- de una base de datos alojada en un servidor [Web Cloud Databases](databases.) (incluida con nuestros [planes de hosting Performance](hosting-performance-offer.) o [contratada por separado](databases.). 

En función de su situación, la ruta de acceso a la base de datos de origen es diferente.

#### Base de datos Start SQL

En su [área de cliente de OVHcloud](manager.), seleccione `Web Cloud`{.action} en el menú superior. En la columna izquierda, abra la pestaña `Alojamientos`{.action} y haga clic en el alojamiento web en el que se encuentra la base de datos de origen cuyo contenido desea copiar.

![Lista de alojamientos](web-hosting-selection.png){.thumbnail}

Al hacer clic en la pestaña de `Bases de datos`{.action}, aparecerá la lista de sus bases de datos Start SQL.

![Lista de BDD Start SQL](sharedsql-dashboard-db-list.png){.thumbnail}

#### Web Cloud Databases

En su [área de cliente de OVHcloud](manager.), seleccione `Web Cloud`{.action} en el menú superior. En la columna izquierda, abra la pestaña `Web Cloud Databases`{.action} y seleccione el servidor Web Cloud Databases en el que se encuentra la base de datos de origen cuyo contenido desea copiar.

![Lista de servidores WCD](wcdb-server-selection.png){.thumbnail}
...
![Lista de bases de datos WCD](wcdb-dashboard-db-list.png){.thumbnail}
...
![CTA_copie_BDD](copy-db-tool.png){.thumbnail}

Se abrirá una ventana en la que podrá identificar la base de datos de destino.

![Interfaz de copia BDD](copy-db-tool-step-1.png){.thumbnail}
...
![Lista de bases de datos WCD](copy-db-tool-link-to-buy-db.png){.thumbnail}

Puede elegir entre contratar un servicio "[start SQL](hosting-options-startsql.)" o un servidor de bases de datos "[Web Cloud Databases](databases.)".

> [!primary]
>
> La nueva base de datos no está activada de forma predeterminada cuando la adquiere. No olvide activarlo. Para ello, conéctese a su [área de cliente de OVHcloud](manager.) y acceda a la sección `Web Cloud`{.action}.
> 
> - Para una base de datos Shared SQL, siga nuestra guía "[Crear una base de datos en un alojamiento web](sql_create_database1.)";
> - Para una base de datos que estará presente en un servidor Web Cloud Databases, siga nuestra guía "[Crear una base de datos en un servidor Web Cloud Databases](create-db-and-user-on-db-server1.)".
>
...
![Mensaje de confirmación copiar BDD](copy-db-tool-step-2.png){.thumbnail}
...
![Mensaje de éxito de BDD](copy-db-tool-copied-successfull.png){.thumbnail}
...
![Tareas en curso](copy-db-tool-ongoing-tasks.png){.thumbnail}
...
Para más información, o si utiliza otro CMS, consulte nuestra guía [Cambiar la contraseña de la base de datos de un alojamiento web](sql_change_password1.).
...
- Configurar una nueva base de datos en su servidor [Web Cloud Databases](databases.);
- Contratar un servicio "[start SQL](hosting-options-startsql.)" o un servidor de bases de datos "[Web Cloud Databases](databases.)"
...
- Contratar una nueva base de datos [start SQL](hosting-options-startsql.) con más espacio.
- Si tiene un servidor [Web Cloud Databases](databases.), cambie a una solución Web Cloud Databases con más espacio de almacenamiento.
...
[Realizar una copia de seguridad de una base de datos y exportarla a un servidor de bases de datos](save-export-on-database-server1.)

[Restaurar e importar una base de datos en el servidor de bases de datos](restore-import-on-database-server1.)

[Obtener la copia de seguridad de la base de datos de un alojamiento web](sql_database_export1.)

[Importar una copia de seguridad en la base de datos de un alojamiento web](sql_importing_mysql_database1.)
