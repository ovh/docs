...
- Posiadanie ofert baz danych [start SQL](hosting-options-startsql.) i/lub [Web Cloud Databases](databases.). Obie bazy danych muszą zostać wcześniej utworzone, aby można było korzystać z narzędzia do powielania.
- Dostęp do [panelu klienta OVHcloud](manager.)
- Wystarczające uprawnienia do wszystkich odpowiednich usług bazodanowych. Więcej informacji znajdziesz w naszym przewodniku [Zarządzanie kontaktami dla usług OVHcloud](managing_contacts1.).
...
- bazy danych [Start SQL](hosting-options-startsql.) (zawartej w niektórych naszych [hostingach www](hosting.) lub [zamówionej oddzielnie](hosting-options-startsql.));
- bazy danych zainstalowanej na serwerze [Web Cloud Databases](databases.) (zawartej w ofercie [Hosting Performance](hosting-performance-offer.) lub [zamówionej oddzielnie](databases.)). 

W zależności od Twojego przypadku ścieżka dostępu do źródłowej bazy danych jest inna.

#### Baza danych Start SQL

[Panel klienta OVHcloud](manager.) wybierz `Web Cloud`{.action} w menu na górze interfejsu. W lewej kolumnie przejdź do zakładki `Hosting`{.action}, następnie kliknij hosting www, na którym znajduje się źródłowa baza danych, której zawartość ma zostać skopiowana.

![Lista hostingów](web-hosting-selection.png){.thumbnail}

Po kliknięciu na zakładkę `Bazy danych`{.action} wyświetli się lista Twoich baz danych Start SQL.

![Lista baz danych Start SQL](sharedsql-dashboard-db-list.png){.thumbnail}

#### Web Cloud Databases

[Panel klienta OVHcloud](manager.) wybierz `Web Cloud`{.action} w menu na górze interfejsu. W kolumnie z lewej strony przejdź do zakładki `Web Cloud Databases`{.action}, następnie wybierz serwer Web Cloud Databases, na którym znajduje się Twoja źródłowa baza danych, której zawartość chcesz skopiować.

![Lista serwerów WCD](wcdb-server-selection.png){.thumbnail}
...
![Lista baz danych WCD](wcdb-dashboard-db-list.png){.thumbnail}
...
![Interfejs kopiowania bazy danych](copy-db-tool-step-1.png){.thumbnail}
...
![Lista baz danych WCD](copy-db-tool-link-to-buy-db.png){.thumbnail}

Możesz wybrać zakup usługi "[start SQL](hosting-options-startsql.)" lub serwera baz danych "[Web Cloud Databases](databases.)".

> [!primary]
>
> Gdy kupujesz nową bazę danych, nie jest ona włączona domyślnie. Nie zapomnij go aktywować. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.), a następnie przejdź do sekcji `Web Cloud`{.action}.
> 
> - W przypadku bazy danych "Shared SQL": zapoznaj się z przewodnikiem "[Tworzenie bazy danych na hostingu WWW OVHcloud](sql_create_database1.)";
> - W przypadku bazy danych, która będzie dostępna na serwerze "Web Cloud Databases": zapoznaj się z naszym przewodnikiem "[Tworzenie bazy danych na serwerze Web Cloud Databases](create-db-and-user-on-db-server1.)".
>
...
![Wiadomość potwierdzająca skopiuj bazy danych](copy-db-tool-step-2.png){.thumbnail}
...
![Wiadomość o sukcesie bazy danych](copy-db-tool-copied-successfull.png){.thumbnail}
...
![Zadania w trakcie](copy-db-tool-ongoing-tasks.png){.thumbnail}
...
Aby uzyskać więcej informacji lub skorzystać z innego CMS-a, zapoznaj się z naszym przewodnikiem [Zmiana hasła do bazy danych hostingu WWW](sql_change_password1.).
...
- Skonfiguruj nową bazę danych na swoim serwerze [Web Cloud Databases](databases.);
- Zamów ofertę "[start SQL](hosting-options-startsql.)" lub serwer baz danych "[Web Cloud Databases](databases.)"
...
- Zamów nową bazę danych [start SQL](hosting-options-startsql.) dysponującą większą przestrzenią.
- Jeśli posiadasz serwer [Web Cloud Databases](databases.), zmień ofertę Web Cloud Databases dysponującą większą przestrzenią dyskową.
...
[Zapisz i wyeksportuj bazę danych na serwer baz danych](save-export-on-database-server1.)

[Przywróć i zaimportuj bazę danych na serwer baz danych](restore-import-on-database-server1.)

[Tworzenie i pobieranie kopii zapasowej bazy danych na hostingu WWW](sql_database_export1.)

[Importuj kopię zapasową do bazy danych hostingu WWW](sql_importing_mysql_database1.)
