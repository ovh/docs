...
In case you do not have this information, log in to the [OVHcloud Control Panel](manager.){.external}, go to the Web Cloud section, then click on `Hosting`{.action}. Select the name of the web hosting plan, and click on the `FTP - SSH`{.action} tab. The information associated with your storage space will then appear:

![hosting](ftp_filezilla_user_guide_images_tab-pro.png){.thumbnail}

> [!warning]
>
> Some OVHcloud solutions do not use port 22 for SFTP and/or SSH connections. so make good use of the ports displayed in your [OVHcloud Control Panel](manager.){.external}
>
...
Go to the Web Cloud section of your [OVHcloud Control Panel](manager.){.external}, then click on `Hosting plans`{.action}. Select the name of the web hosting plan and click on the `FTP - SSH`{.action} tab.
...
> [!success]
> A guide is available for [changing the FTP](ftp_change_password1.) password on shared hosting plans.
...
>
> You must then [secure your solutions](diagnostic_403_forbidden1.) while correcting the security vulnerabilities mentioned in the block notification received via email.
>
...
Consult our documentation for [resolving recurring errors when using FTP](ftp_recurring_ftp_problems1.) software.

Generally, find all our [of our guides related to shared](web-cloud-hosting1.).
