...
Sie können damit Dateien bzw. Ihre Websiten online stellen, indem Sie sich im [FTP-Bereich Ihres Hostings einloggen](ftp_connection1.).
...
> 
> Diese Anleitung soll Sie bei allgemeinen Aufgaben bestmöglich unterstützen. Dennoch empfehlen wir Ihnen, falls Sie Hilfe brauchen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren oder Ihre Fragen in der OVHcloud Community zu stellen. Leider können wir Ihnen für administrative Aufgaben keine weitergehende technische Unterstützung anbieten. Weitere Informationen finden Sie am [Ende dieser Anleitung](ftp_filezilla_user_guide_#go-further.).
>
...
Falls diese Informationen nicht vorliegen, loggen Sie sich in Ihrem [OVHcloud Kundencenter](manager.){.external} ein und klicken Sie dann auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus und gehen Sie dann auf den Tab `FTP - SSH`{.action}. Die Informationen zu Ihrem Speicherplatz werden dort angezeigt.

![Hosting](ftp_filezilla_user_guide_images_tab-pro.png){.thumbnail}

> [!warning]
>
> Einige OVHcloud Angebote verwenden Port 22 nicht für SFTP- und/oder SSH-Verbindungen. Im Zweifel verwenden Sie die Ports, die in Ihrem [OVHcloud Kundencenter](manager.){.external} zu finden sind.
>
...
Das Protokoll **SFTP** (**S**ecure **F**ile **T**ransfer **P**rotocol) ist nahezu identisch mit **FTP**, verwendet aber (wie SSH) standardmäßig Port 22 statt Port 21. Wenn Sie ein Cloud Web Hosting-Angebot verwenden, verwenden Sie den Port, der in Ihrem [OVHcloud Kundencenter](manager.){.external} angezeigt wird. Port 22 ist aus Sicherheitsgründen für Cloud Web Hostings deaktiviert und kann daher nicht für Verbindungen über SSH oder SFTP verwendet werden.
...
Gehen Sie in Ihrem [OVHcloud Kundencenter](manager.){.external} in den Bereich "Web Cloud" und klicken Sie dann auf `Hosting-Pakete`{.action}. Wählen Sie das betreffende Hosting aus und gehen Sie dann auf den Tab `FTP - SSH`{.action}.

Überprüfen Sie anschließend, ob **SFTP** in der Tabelle unten aktiv ist.

![SFTP Start Angebot aktivieren](sftp-enabled-pro.png){.thumbnail}

Falls **SFTP** nicht aktiviert ist:

- Klicken Sie auf den Button `...`{.action} rechts neben der Tabelle und dann auf `Bearbeiten`{.action}.

![SFTP 1 Aktivierung](edit-login.png){.thumbnail}
...
![SFTP 2 Aktivierung](modify-user-step-1-connexion-protocols.png){.thumbnail}
...
Überprüfen Sie Ihre Zugangsdaten, um sicherzustellen, dass diese korrekt sind. Nötigenfalls können Sie das Passwort des FTP-Zugangs für Ihr Hosting direkt im [OVHcloud Kundencenter](manager.){.external} ändern.

> [!success]
> Sie finden genauere Informationen dazu in der [Anleitung zur Änderung des FTP-Passworts](ftp_change_password1.).
...
Überprüfen Sie diesen anhand des in Ihrem [OVHcloud Kundencenter](manager.){.external} angezeigten Hostnamens.
...
>
> [Sichern Sie dann Ihre Dienste](diagnostic_403_forbidden1.) und korrigieren Sie dabei die Sicherheitslücken, die in der per E-Mail erhaltenen Blockierungsbenachrichtigung erwähnt werden.
>
...
Sie finden [hier unsere Anleitung zur Behebung von Fehlern bei der Verwendung eines FTP-Programms](ftp_recurring_ftp_problems1.).

Im Allgemeinen finden Sie [hier alle unsere Hilfen zu Webhostings](web-cloud-hosting1.).
