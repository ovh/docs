...
Permite publicar ficheiros ou o seu website [acedendo ao espaço FTP](ftp_connection1.) do seu alojamento.
...
> 
> Este tutorial está disponível para o acompanhar nas suas operações mais habituais. No entanto, se encontrar dificuldades, recomendamos que recorra a um [prestador de serviços especializado](partner.) e/ou que contacte o editor do software. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção ["Quer saber mais?"](ftp_filezilla_user_guide_#go-further.) deste tutorial.
> 
...
![alojamento](images_main-interface.png){.thumbnail}
...
![alojamento](quick-connect.png){.thumbnail}
...
Se não possuir estas informações, aceda à [Área de Cliente OVHcloud](manager.){.external} na secção "Web Cloud" e clique em `Alojamentos`{.action}. De seguida, selecione o nome do alojamento correspondente e aceda ao separador `FTP - SSH`{.action}. Aparecerá a informação relativa ao seu espaço de armazenamento:

![alojamento](ftp_filezilla_user_guide_images_tab-pro.png){.thumbnail}

> [!warning]
>
> Algumas ofertas da OVHcloud não utilizam a porta 22 para as ligações em SFTP e/ou SSH. Utilize as portas que aparecem na sua [Área de Cliente OVHcloud](manager.){.external}
>

Quando tudo estiver corretamente introduzido na caixa **1** da imagem abaixo, clique em `Ligação rápida`{.action}.

![alojamento](quick-connect-successfull.png){.thumbnail}
...
O **SFTP** (para **S**ecure **F**ile **T**ransfer **P**rotocol) é um protocolo semelhante ao **FTP**. Como o SSH, utiliza a porta 22 predefinida em vez da porta 21. Se utiliza uma oferta de alojamento Cloud Web, deve utilizar a porta apresentada na [Área de Cliente OVHcloud](manager.){.external}. A porta 22 é desativada por segurança em SSH e SFTP para os alojamentos Cloud Web.
...
Aceda à [Área de Cliente OVHcloud](manager.){.external}, na secção "Web Cloud", e clique em `Alojamentos`{.action}. De seguida, selecione o nome do alojamento correspondente e aceda ao separador `FTP - SSH`{.action}.

De seguida, verifique se o **SFTP** está ativo na tabela.

![Ativação SFTP oferta start](sftp-enabled-pro.png){.thumbnail}

Se não estiver ativo:

- Clique no botão `...`{.action} à direita da tabela e, a seguir, em `Editar`{.action}.

![Ativação SFTP 1](edit-login.png){.thumbnail}
...
![Ativação SFTP 2](modify-user-step-1-connexion-protocols.png){.thumbnail}
...
![alojamento](unknown-host-key-message.png){.thumbnail}

### Erros de ligação

A mensagem apresentada abaixo indica um erro de identificação aquando da ligação em FTP ou SFTP ao alojamento partilhado:

![alojamento](authentification-failed-could-not-connect-server.png){.thumbnail}

Este tipo de mensagem é gerada por um erro no binário Login/Palavra-passe.

Verifique os seus dados de acesso para garantir que não há erros. Se necessário, pode alterar a palavra-passe do acesso FTP do seu alojamento diretamente na [Área de Cliente OVHcloud](manager.){.external}.

> [!success]
> Está disponível um guia sobre a [alteração da palavra-passe FTP](ftp_change_password1.) nas ofertas partilhadas.

No caso abaixo, o erro é gerado por um nome de host incorreto:

![alojamento](connection-timed-out-after-20s.png){.thumbnail}

Verifique este último em relação ao nome do host declarado na sua [Área de Cliente OVHcloud](manager.){.external}.
...
![alojamento](drag-drop-en.png){.thumbnail}
...
![alojamento](waiting-list-view.png){.thumbnail}
...
![alojamento](contextual-menu-server.png){.thumbnail}
...
![alojamento](change-file-attributes.png){.thumbnail}
...
>
> Deverá [proteger as suas soluções](diagnostic_403_forbidden1.) corrigindo as falhas de segurança evocadas na notificação de bloqueio recebida por e-mail.
>
...
![alojamento](images_site-chmod-705-command.png){.thumbnail}
...
![alojamento](transfert-binary-files.png){.thumbnail}

### Comparação de dossiers

![alojamento](comparison-tool.png){.thumbnail}
...
A seguir, encontrará o link para o nosso manual para [corrigir erros recorrentes durante a utilização de um programa FTP](ftp_recurring_ftp_problems1.).

De forma mais geral, consulte [o conjunto dos nossos guias relativos aos alojamentos partilhados](web-cloud-hosting1.).
