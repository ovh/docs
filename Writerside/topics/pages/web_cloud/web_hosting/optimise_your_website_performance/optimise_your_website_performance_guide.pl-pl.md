...
- [plan hostingu WWW OVHcloud](hosting.){.external}
- e-mail potwierdzający skonfigurowanie planu hostingu WWW
- [nazwa domeny](domains.){.external} umożliwiająca dostęp do witryny internetowej
...
Na [stronie produktu](hosting.){.external} przestawiamy oferty hostingu WWW zalecane do wskazanych powyżej systemów CMS.

Więcej informacji na temat wyboru właściwego planu znajdziesz na [tej stronie](hosting-best-web.){.external}.
...
Informacje o aktualizacji wersji PHP zawiera [ten przewodnik](configure_your_web_hosting1.).

Aby zmienić wersję PHP-FPM na _stabilną_ lub uzyskać szczegółowe informacje o bardziej zaawansowanych opcjach hostingu WWW, zapoznaj się z [tym przewodnikiem](configure_your_web_hosting1.).
...
Logi, statystyki i wykresy są dostępne bezpośrednio w [Panelu klienta OVHcloud](manager.){.external}.
...
![logi](optimise_your_website_performance_images_tab.png){.thumbnail}
...
Aby mieć pewność, że korzystasz z oferty hostingu dopasowanej do potrzeb Twojego systemu CMS, zapoznaj się z porównaniem usług na [stronie produktu](hosting.){.external}.
...
**Dostęp do bazy danych za pośrednictwem narzędzia phpMyAdmin:** Aby uzyskać dostęp do bazy danych za pomocą narzędzia phpMyAdmin, skorzystaj z[Panelu klienta OVHcloud](manager.){.external}:
...
[Zmiana konfiguracji hostingu WWW](configure_your_web_hosting1.)

[Zarządzanie bazą danych w hostingu WWW](sql_create_database1.){.external}
