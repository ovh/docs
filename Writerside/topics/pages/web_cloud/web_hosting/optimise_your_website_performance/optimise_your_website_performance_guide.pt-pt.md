...
- um [plano de Alojamento Web OVHcloud](hosting.){.external}
- um e-mail a confirmar a configuração do seu plano de Alojamento Web
- um [nome de domínio](domains.){.external} que pode ser utilizado para aceder ao seu website
- acesso à [Área de Cliente OVHcloud](manager.){.external}
...
Fornecemos recomendações sobre a oferta de alojamento web a utilizar para os CMS acima indicados na [página de produto](hosting.){.external}.

Encontrará mais informações acerca do plano a escolher [nesta página](hosting-best-web.){.external}.
...
Pode obter informações sobre como atualizar o PHP [neste guia](configure_your_web_hosting1.).

Para mudar para a utilização do PHP-FPM na versão _estável_ ou para obter detalhes sobre opções mais avançadas do seu alojamento web, consulte [este guia](configure_your_web_hosting1.).
...
Pode aceder aos seus logs, estatísticas e gráficos diretamente a partir da [Área de Cliente OVHcloud](manager.){.external}.
...
![gráficos](images_infrastructure-statistics-graph.png){.thumbnail}
...
Para garantir a adaptação da sua oferta de alojamento às necessidades do seu CMS, pode encontrar uma comparação dos nossos serviços na [página de produto](hosting.){.external}.
...
**Como aceder à base de dados via phpMyAdmin:** Para aceder à sua base de dados via phpMyAdmin, siga o procedimento disponibilizado na [Área de Cliente OVHcloud](manager.){.external}:
...
[Alterar a configuração de um plano de alojamento web](configure_your_web_hosting1.)

[Gestão de uma base de dados num pacote de alojamento web](sql_create_database1.){.external}

[Web Cloud Databases - primeira utilização](starting_with_clouddb1.){.external}
