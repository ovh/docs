...
- Un [plan de hospedaje web de OVHcloud](hosting.){.external}
- Un mensaje de correo electrónico que confirme que su plan de hospedaje web ha sido contratado
- Un [nombre de dominio](domains.){.external} que pueda utilizarse para acceder a su sitio web
- Haber iniciado sesión en el [área de cliente de OVHcloud para tener acceso al panel de control](manager.){.external}
...
Proporcionamos recomendaciones sobre qué oferta de hospedaje web utilizar para los sistemas de gestión de contenidos antes mencionados en [la página del producto](hosting.){.external}.

Puede encontrar más información sobre qué plan elegir en [esta página](hosting-best-web.){.external}.
...
Averigüe cómo actualizar el lenguaje PHP en [esta guía](configure_your_web_hosting1.).

Para cambiar al uso de PHP-FPM en la versión _estable_ o para obtener información sobre opciones más avanzadas de su hospedaje web, consulte [esta guía](configure_your_web_hosting1.).
...
Puede acceder a sus registros, estadísticas y gráficos directamente desde el panel de control, iniciando sesión en el [área de cliente de OVHcloud](manager.){.external}.
...
Para garantizar que nuestra oferta de hospedaje se adapte a las necesidades de su sistema de gestión de contenidos, puede encontrar una comparativa de nuestros servicios en [la página del producto](hosting.){.external}.
...
**Cómo acceder a la base de datos mediante phpMyAdmin:** A continuación, se detalla el procedimiento para acceder a su base de datos mediante phpMyAdmin, disponible en el panel de control una vez iniciada sesión en el [área de cliente de OVHcloud](manager.){.external}:
...
[Modificar la configuración de un plan de hospedaje web](configure_your_web_hosting1.)

[Gestionar una base de datos en un paquete de hospedaje web](sql_create_database1.){.external}

[Primeros pasos con el servicio Cloud Databases](starting_with_clouddb1.){.external}
