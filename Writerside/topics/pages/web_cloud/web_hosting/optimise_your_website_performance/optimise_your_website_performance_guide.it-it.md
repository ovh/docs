...
- Una [soluzione Web Hosting OVHcloud](hosting.){.external}
- Una email conferma che la tua soluzione di Web Hosting è stato configurato
- Un [dominio](domains.){.external}per accedere al tuo sito
- avere accesso allo [Spazio Cliente](manager.){.external}OVHcloud
...
Ti forniamo alcuni consigli su quale soluzione Hosting Web utilizzare per i CMS sopraelencati in questa [pagina](hosting.){.external}.

Per maggiori informazioni clicca [qui](hosting-best-web.){.external}.
...
Per configurare una nuova versione di PHP, consulta [questa guida](configure_your_web_hosting1.).

Per passare all’uso di PHP-FPM nella versione _stabile_o per ottenere informazioni sulle opzioni più avanzate del tuo Web hosting, consulta [questa guida](configure_your_web_hosting1.).
...
Puoi accedere ai tuoi log, alle statistiche e ai grafici direttamente dallo [Spazio Cliente OVHcloud](manager.)){.external}.
...
Per essere sicuro che il tuo piano di hosting sia adatto alle esigenze del tuo CMS, puoi confrontare i nostri servizi in [questa pagina](hosting.){.external}.
...
**Come accedere al database tramite phpMyAdmin:** Per accedere al database tramite phpMyAdmin, segui la procedura dallo [Spazio Cliente OVHcloud](manager.){.external}:
...
[Modificare la configurazione di un hosting Web](configure_your_web_hosting1.)

[Gestisci un database dal tuo hosting condiviso](sql_create_database1.){.external}
