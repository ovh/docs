...
- Utilizzare una versione recente di PHP e un ambiente di esecuzione compatibile sul tuo hosting Web OVHcloud. Scopri lo stato delle diverse versioni disponibili in questa [pagina](https://webhosting-infos.hosting.ovh.net/). Se necessario, consulta la nostra [guida](configure_your_web_hosting1.) sull’argomento per modificare rapidamente questa configurazione.
- La directory (cartella di root) in cui sarà installato il tuo "modulo in 1 click" deve essere vuota o al momento inesistente.
- Il dominio (con sottodominio se necessario) che verrà utilizzato per il tuo sito web deve essere dichiarato come [Multisito](multisites_configure_multisite1.) sul tuo hosting Web OVHcloud.

## Procedura

> [!primary]
>
> In caso di difficoltà durante l’esecuzione di uno degli step descritti qui sotto, consulta la nostra documentazione specifica sugli [errori più frequenti associati ai "moduli in 1 click"](diagnostic_errors_module1clic1.). 
>
...
Utilizzando questa soluzione, dovrai scegliere tra i 4 CMS citati sopra. Se hai già scelto OVH, prosegui nella lettura degli step di questa guida. In caso contrario, consulta la nostra pagina [comparativa dei CMS](hosting-cms-comparison.) per effettuare la tua scelta.

Per installare un CMS non disponibile tramite i nostri "moduli in 1 click", è possibile installarlo manualmente sul tuo hosting. Questo CMS è compatibile con le nostre soluzioni di [hosting Web OVHcloud](hosting.).

![Logo dei CMS](cms-logos.png){.thumbnail}

### Step 2 - accedi alla gestione dei "moduli in 1 click"

Accedi al tuo [Spazio Cliente OVHcloud](manager.) e seleziona `Web Cloud`{.action}. Clicca su `Hosting`{.action}, seleziona l'offerta di hosting su cui vuoi installare un "modulo in 1 click" e clicca sulla scheda `CMS in 1 click`{.action}.
...
![Accesso alla sezione Moduli in 1 click](cms_install_1_click_modules_images_tab.png){.thumbnail}
...
![Scelta del modulo](add-a-module-select-module-and-domain.png){.thumbnail}

Se il tuo dominio non è nella lista, clicca sulla scheda `Multisito`{.action} per aggiungerlo. Consulta la nostra guida [Come condividere un hosting Web tra più siti](multisites_configure_multisite1.){.external} in caso di necessità.
...
>
> Se necessario, consulta la nostra guida "[Come condividere il tuo hosting Web tra più siti](multisites_configure_multisite1.)" per modificare la directory di destinazione del tuo dominio.
>
...
![Installazione base di un modulo](add-a-module-quick-mod-step-1-b.png){.thumbnail}
...
> [!primary]
>
> L’installazione e la ricezione dell’email possono richiedere fino a 15 minuti a partire dal momento in cui hai cliccato sul pulsante `Installa`{.action} nel tuo [Spazio Cliente OVHcloud](manager.).
>
...
![Installazione avanzata di un modulo](add-a-module-advanced-mod-step-1.png){.thumbnail}
...
> 
> Per recuperare le credenziali di accesso al tuo database incluso nel tuo hosting Web, consulta [questa guida](sql_create_database1.).
>
> Per recuperare le credenziali di accesso al database creato su un'istanza Web Cloud Databases, consulta [questa guida](starting_with_clouddb1.).
>

![Database per l'installazione avanzata](add-a-module-advanced-mod-step-3.png){.thumbnail}
...
- Il database non è ancora stato creato sul tuo hosting Web: [Crea il tuo database incluso con il tuo hosting](sql_create_database1.), torna al menu a tendina `Seleziona il database`{.action} e inserisci le informazioni richieste.
- Il database è [creato sulla tua istanza Web Cloud Databases](create-db-and-user-on-db-server1.): nel menu a tendina `Seleziona il database`{.action}, seleziona `Database esterno al tuo hosting Web`{.action} e inserisci le informazioni richieste. L'istanza e l'hosting Web devono essere ospitati nello stesso datacenter.
...
- *Nome del database*: questo nome è stato definito durante la creazione del database nello[Spazio Cliente OVHcloud](manager.).

- *Porta*: inserisci sistematicamente il numero **3306** (porta di default) per un database incluso con il tuo hosting Web. Per creare un database su un'istanza Web Cloud, consulta [questa guida](starting_with_clouddb1.).

- *Username*: è identico al nome del database se utilizzi un database incluso con il tuo hosting Web.
Per i database creati su un'offerta Web Cloud Database, consulta le informazioni indicate in [questa guida](starting_with_clouddb1.).
...
- *dominio:* dominio con cui vuoi installare il tuo CMS. Se necessario, consulta la guida su [Come condividere un hosting Web tra più siti](multisites_configure_multisite1.).
...
![Configurazione del modulo per installazione avanzata](add-a-module-advanced-mod-step-2.png){.thumbnail}

##### Conferma l'installazione

Verifica le informazioni visualizzate e clicca su `Conferma`{.action} se tutto è in ordine:

![Conferma dell’installazione in modalità avanzata](add-a-module-advanced-mod-step-4.png){.thumbnail}
...
[Scegli il CMS per il tuo sito Web](hosting-cms-comparison.){.external}.
...
[Gestire il vostro CMS](cms_manage_1_click_module1.).

[Disinstallare il tuo CMS](cms_manage_1_click_module#step-3-elimina-il-tuo-modulo.).

Per mantenere il pieno controllo sull’installazione del CMS è possibile [installare manualmente un CMS sull’hosting Web OVHcloud](cms_manual_installation1.).
