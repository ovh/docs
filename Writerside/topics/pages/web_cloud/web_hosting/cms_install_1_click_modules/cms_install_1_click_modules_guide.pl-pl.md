...
- Posiadanie [hostingu OVHcloud](hosting.) zawierającej co najmniej jedną bazę danych.
- Dostęp do [Panelu klienta OVHcloud](manager.){.external}.
- Użycie najnowszej wersji PHP i kompatybilnego środowiska wykonawczego na Twoim hostingu OVHcloud. Sprawdź stan poszczególnych wersji dostępnych na tej [stronie](https://webhosting-infos.hosting.ovh.net/). W razie potrzeby sprawdź [przewodnik](configure_your_web_hosting1.), aby szybko zmienić tę konfigurację.
...
- Domena (wraz z subdomeną, jeśli jest potrzebna), która zostanie użyta na Twojej stronie WWW powinna zostać zadeklarowana jako [MultiSite](multisites_configure_multisite1.) na Twoim hostingu OVHcloud.

## W praktyce

> [!primary]
>
> Jeśli masz trudności z wykonaniem jednego z opisanych poniżej kroków, zapoznaj się z naszą dokumentacją dotyczącą [najczęstszych błędów związanych z "modułami za 1 kliknięciem"](diagnostic_errors_module1clic1.). 
>
...
Używając tego rozwiązania powinieneś wybrać spośród 4 systemów CMS wymienionych powyżej. Jeśli już dokonałeś wyboru, przejdź do kolejnych etapów niniejszego przewodnika. Jeśli tak się nie stanie, skorzystaj z naszych [porównań CMS](hosting-cms-comparison.), aby dokonać wyboru.

Jeśli chcesz zainstalować niedostępny CMS za pomocą naszych "modułów za 1 kliknięciem", możesz zainstalować go ręcznie na Twoim hostingu. Pod warunkiem, że ten CMS jest kompatybilny z naszą ofertą [hostingiem www OVHcloud](hosting.).

![Dostępne w OVHcloud CMS](cms-logos.png){.thumbnail}

### Etap 2 - dostęp do zarządzania modułami za pomocą 1 kliknięcia

Zaloguj się do [Panelu klienta OVHcloud](manager.) i wybierz `Web Cloud`{.action}. Kliknij przycisk `Hosting`{.action}, wybierz ofertę hostingu, na której chcesz zainstalować "moduł za 1 kliknięciem", następnie kliknij zakładkę `Moduły CMS`{.action}.
...
![Dostęp do sekcji Moduły OVHcloud](cms_install_1_click_modules_images_tab.png){.thumbnail}
...
![Wybór modułu](add-a-module-select-module-and-domain.png){.thumbnail}

Jeśli Twoja domena nie jest na liście, przejdź do zakładki `MultiSite`{.action}, aby ją dodać. Zapoznaj się z naszym przewodnikiem [Jak rozdzielić hosting WWW na kilka stron](multisites_configure_multisite1.), jeśli potrzebujesz.
...
>
> W razie potrzeby sprawdź przewodnik "[Jak rozdzielić swój hosting WWW na kilka stron](multisites_configure_multisite1.)", aby zmienić katalog docelowy Twojej domeny.
>
...
![Prosta instalacja modułu](add-a-module-quick-mod-step-1-b.png){.thumbnail}
...
> [!primary]
>
> Instalacja i odbiór e-maili mogą potrwać do 15 minut od momentu kliknięcia przycisku `Instaluj`{.action} w [Panelu klienta OVHcloud](manager.).
>
...
![Instalacja w trybie zaawansowanym](add-a-module-advanced-mod-step-1.png){.thumbnail}
...
> 
> Dane do logowania do bazy danych zawartej w pakiecie hostingowym znajdziesz w [przewodniku](sql_create_database1.).
>
> Dane do logowania do bazy danych utworzonej na instancji Web Cloud Databases znajdują się w [przewodniku](starting_with_clouddb1.).
>

![Baza danych do instalacji zaawansowanej](add-a-module-advanced-mod-step-3.png){.thumbnail}
...
- Baza danych nie została jeszcze utworzona na Twoim hostingu: [utwórz bazę danych zawartą na hostingu](sql_create_database1.), następnie powróć do rozwijanego menu `Wybierz bazę danych`{.action} i uzupełnij wymagane informacje.
- Baza danych jest [utworzona na instancji Web Cloud Databases](create-db-and-user-on-db-server1.): w rozwijanym menu `Wybierz bazę danych`{.action}, wybierz opcję `Baza danych poza hostingiem`{.action} i uzupełnij wymagane informacje. Instancja i hosting muszą być hostowane w tym samym centrum danych (datacenter).
...
- *Nazwa bazy*: nazwa ta została zdefiniowana podczas tworzenia bazy danych w [Panelu klienta OVHcloud](manager.).

- *Port*: systematycznie wpisz numer **3306** (port domyślny) dla bazy danych zawartej w Twoim hostingu. W przypadku bazy danych na instancji Cloud Databases, zapoznaj się z [tym przewodnikiem](starting_with_clouddb1.).

- *Nazwa użytkownika*: nazwa bazy danych jest identyczna, jeśli korzystasz z bazy danych zawartej w Twoim hostingu.
W przypadku baz danych utworzonych w ramach usługi Web Cloud Databases, zapoznaj się z informacjami podanymi w [tym przewodniku](starting_with_clouddb1.).
...
- *domena:* domena, z którą chcesz zainstalować CMS-a. Jeśli potrzebujesz pomocy, zapoznaj się z naszym przewodnikiem [Jak rozdzielić swój hosting na kilka stron](multisites_configure_multisite1.).
...
![Konfiguracja modułu dla instalacji zaawansowanej](add-a-module-advanced-mod-step-2.png){.thumbnail}

##### Potwierdź instalację

Sprawdź informacje, które się wyświetlają i kliknij na `Zatwierdź`{.action}, jeśli wszystko jest w porządku:

![Zatwierdzanie instalacji w trybie zaawansowanym](add-a-module-advanced-mod-step-4.png){.thumbnail}
...
[Porównanie rozwiązań CMS](hosting-cms-comparison.){.external}.

[Jak rozdzielić hosting WWW na wiele stron](multisites_configure_multisite1.).

[Zarządzanie bazą danych na hostingu](sql_create_database1.).

[Zarządzanie CMS](cms_manage_1_click_module1.).

[Odinstaluj CMS](cms_manage_1_click_module#etap-3-usun-modul.).

Jeśli chcesz zachować pełną kontrolę nad instalacją Twojego CMS-a, możesz [zainstalować ręcznie CMS na Twoim hostingu OVHcloud](cms_manual_installation1.).
