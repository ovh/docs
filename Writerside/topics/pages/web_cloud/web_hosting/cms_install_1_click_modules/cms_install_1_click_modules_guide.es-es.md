...
- Tener contratado un [plan de hosting](hosting.) que incluya al menos una base de datos. 
- Estar conectado al [área de cliente de OVHcloud](manager.).
- Utilizar una versión reciente de PHP y un entorno de ejecución compatible en su alojamiento web de OVHcloud. Descubra el estado de las distintas versiones disponibles en esta [página](https://webhosting-infos.hosting.ovh.net/). Si lo necesita, consulte nuestra [documentación](configure_your_web_hosting1.) para cambiar rápidamente esta configuración.
- El directorio (directorio raíz) en el que se instalará el módulo en 1 clic debe estar vacío o no existe actualmente.
- El dominio (y el subdominio si así lo desea) que vaya a utilizar para su sitio web debe estar declarado como [Multisitio](multisites_configure_multisite1.) en su alojamiento web de OVHcloud.

## Procedimiento

> [!primary]
>
> Si experimenta alguna dificultad al seguir uno de los pasos que se describen a continuación, consulte nuestra documentación específica sobre los [errores más frecuentes relacionados con los módulos en un clic](diagnostic_errors_module1clic1.). 
>
...
Si utiliza esta solución, deberá elegir entre los 4 CMS anteriores. Si ya ha elegido la opción, siga leyendo esta guía. En caso contrario, consulte nuestra [comparativa de los CMS](hosting-cms-comparison.) para elegir.
...
![Logotipos de los CMS](cms-logos.png){.thumbnail}

### Etapa 2 - acceder a la gestión de "módulos en 1 clic"

Conéctese al [área de cliente de OVHcloud](manager.) y seleccione `Web Cloud`{.action}. Haga clic en `Alojamientos`{.action}, seleccione el alojamiento en el que desea instalar un "módulo en 1 clic" y, seguidamente, haga clic en la pestaña `Módulos en 1 clic`{.action}.
...
![Acceso a la sección Módulos en un clic](cms_install_1_click_modules_images_tab.png){.thumbnail}
...
![Elección del módulo](add-a-module-select-module-and-domain.png){.thumbnail}

Si su dominio no está en la lista, acceda a la pestaña `Multisitio`{.action} para añadirlo. Consulte nuestra guía [Cómo compartir un alojamiento web con varios sitios web](multisites_configure_multisite1.){.external} si es necesario.
...
>
> Si lo necesita, consulte nuestra guía "[Cómo compartir un alojamiento web con varios sitios web](multisites_configure_multisite1.)" para modificar el directorio de destino de su dominio.
>
...
![Instalación rápida de un módulo](add-a-module-quick-mod-step-1-b.png){.thumbnail}
...
> [!primary]
>
> La instalación y recepción del mensaje de correo electrónico puede tardar hasta 15 minutos a partir del momento en que haga clic en el botón `Instalar`{.action} en su [área de cliente de OVHcloud](manager.).
>
...
![Instalación avanzada de un módulo](add-a-module-advanced-mod-step-1.png){.thumbnail}
...
> 
> Para obtener las claves de conexión a su base de datos incluida con su alojamiento web, consulte [esta guía](sql_create_database1.).
>
> Para obtener las claves de conexión a la base de datos creada en una instancia de Web Cloud Databases, consulte [esta guía](starting_with_clouddb1.).
>

![Base de datos para instalación avanzada](add-a-module-advanced-mod-step-3.png){.thumbnail}
...
- La base de datos todavía no está creada en su alojamiento web: [cree su base de datos incluida con su alojamiento](sql_create_database1.), vuelva al menú desplegable : `Seleccione la base de datos`{.action} y complete la información solicitada.
- La base de datos está [creada en su instancia Web Cloud Databases](create-db-and-user-on-db-server1.): en el menú desplegable. `Seleccione la base de datos`{.action} y elija la opción `Base de datos externa al alojamiento web`{.action} e introduzca la información solicitada. El datacenter en el que se alojan el servicio y el alojamiento web es el mismo.
...
- *Nombre de la BD* : este nombre se ha establecido al crear la base de datos en el [área de cliente de OVHcloud](manager.).

- *Puerto* : sitúe sistemáticamente el número **3306** (puerto por defecto) para una base de datos incluida con su alojamiento web. Para una base de datos de una instancia de Web Cloud Databases, consulte [esta guía](starting_with_clouddb1.).

- *Nombre de usuario* : es idéntico al nombre de la base de datos si utiliza una base de datos incluida con su alojamiento web.
Para las bases de datos creadas en un plan de hosting Cloud Databases, consulte la información que se indica en [esta guía](starting_with_clouddb1.).
...
- *dominio :* nombre de dominio con el que quiere instalar su CMS. Si lo necesita, consulte nuestra guía [Cómo alojar varios sitios web en un mismo hosting](multisites_configure_multisite1.).
...
![Configuración del módulo para instalación avanzada](add-a-module-advanced-mod-step-2.png){.thumbnail}

##### Confirmar la instalación

Compruebe que la información mostrada es correcta y haga clic en `Aceptar`{.action} si todo está en orden:

![Validación de la instalación en modo avanzado](add-a-module-advanced-mod-step-4.png){.thumbnail}
...
[Seleccione un CMS para crear un sitio web](hosting-cms-comparison.){.external}.

[Cómo compartir un alojamiento web entre varios sitios web](multisites_configure_multisite1.).

[Gestión de una base de datos desde un alojamiento compartido](sql_create_database1.).

Descubra nuestras [ofertas Web Cloud Databases](databases.){.external}.

[Gestionar su CMS](cms_manage_1_click_module1.).

[Desinstalar su CMS](cms_manage_1_click_module#etapa-3-eliminar-el-modulo.).

Si quiere conservar un control total sobre la instalación de su CMS, puede [instalar manualmente un CMS en su alojamiento web de OVHcloud](cms_manual_installation1.).
