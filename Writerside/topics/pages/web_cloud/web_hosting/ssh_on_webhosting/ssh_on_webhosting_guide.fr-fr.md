...
- Disposer d'une offre d'[hébergement web OVHcloud](hosting.){.external} bénéficiant d'un accès SSH.
- Être connecté à votre [espace client OVHcloud](manager.){.external}, partie `Web Cloud`{.action}.

> [!warning]
> 
> L'accès SSH à un hébergement web OVHcloud est disponible à partir de [l'offre Pro](hosting-compare.).

## En pratique

### Étape 1 : s'assurer que l'accès SSH est actif

Connectez-vous à votre [espace client OVHcloud](manager.){.external} dans la partie `Web Cloud`{.action}, puis cliquez sur `Hébergements`{.action}. Sélectionnez l'hébergement concerné, cliquez sur l'onglet `FTP - SSH`{.action}. Les informations liées à votre espace de stockage apparaissent. 
...
Selon la version de PHP que vous souhaitez utiliser, il se peut que l'environnement d'exécution nécessite d'être modifié pour des raisons de compatibilité. Reportez-vous à notre documentation « [Hébergement web : environnement, version PHP, « .ovhconfig »](configure_your_web_hosting1.) » pour en apprendre plus. 
...
>
> Retrouvez plus d'information sur l'utilisation de la commande `scp` avec nos hébergements web OVHcloud dans notre guide « [Hébergement Web - Copier des fichiers avec la commande SCP](using-scp-command1.) »
>

## Aller plus loin

[Hébergement web : environnement, version PHP, « .ovhconfig »](configure_your_web_hosting1.).
