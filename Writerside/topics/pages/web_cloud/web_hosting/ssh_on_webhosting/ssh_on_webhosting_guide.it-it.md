...
- Disporre di una soluzione [hosting Web OVHcloud](hosting.) con accesso SSH
- Disporre delle informazioni necessarie per l’accesso in SSH allo spazio di storage
- Avere accesso allo [Spazio Cliente OVHcloud](manager.), sezione `Web Cloud`{.action}

> [!warning]
> 
> L'accesso SSH a un hosting Web OVHcloud è possibile a partire da [l'offerta Pro](hosting-compare.).

## Procedura

### Step 1: verifica che l’accesso SSH sia attivo <a name="sshcheck"></a>

Per prima cosa, accedi allo [Spazio Cliente OVHcloud](manager.) nella sezione `Web Cloud`{.action}, quindi clicca su `hosting`{.action}. Seleziona il nome dell’hosting interessato e clicca sulla scheda `FTP - SSH`{.action}. Visualizzi le informazioni associate allo spazio di storage. 
...
A seconda della versione di PHP che vuoi utilizzare, potrebbe essere necessario modificare l’ambiente di esecuzione per motivi di compatibilità. Consulta la nostra documentazione "[Hosting Web : ambiente, versione PHP, .ovhconfig](configure_your_web_hosting1.)".
...
> - due server remoti
>
> Per maggiori informazioni sull’utilizzo del comando `scp` con i nostri hosting Web OVHcloud, consulta la nostra guida "[Hosting Web - Copiare file con il comando SCP](using-scp-command1.)"
>

## Per saperne di più

[Hosting Web : ambiente, versione PHP, .ovhconfig](configure_your_web_hosting1.)
