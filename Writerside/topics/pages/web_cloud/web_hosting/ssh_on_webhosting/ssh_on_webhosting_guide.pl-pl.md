...
- Posiadanie [hostingu WWW OVHcloud](hosting.) z dostępem przez protokół SSH.
- Posiadanie informacji umożliwiających logowanie się do przestrzeni dyskowej za pomocą SSH.
- Mieć dostęp do [panelu klienta OVHcloud](manager.), sekcja `Web Cloud`{.action}.

> [!warning]
> 
> Dostęp SSH do hostingu OVHcloud jest możliwy od [oferta Pro](hosting-compare.).

## W praktyce

### Etap 1: upewnij się, że dostęp przez SSH jest aktywny <a name="sshcheck"></a>

Rozpocznij od zalogowania się do [panelu klienta OVHcloud](manager.) w sekcji `Web Cloud`{.action} i kliknij `Hostingi`{.action} na pasku usług po lewej stronie. Wybierz odpowiedni hosting i przejdź do zakładki `FTP - SSH`{.action}. Wyświetlą się wówczas dane dotyczące Twojej przestrzeni dyskowej. 
...
W zależności od wersji PHP, której chcesz używać, środowisko uruchomieniowe może wymagać modyfikacji, aby zagwarantować kompatybilność. Sprawdź naszą dokumentację [Hosting WWW : środowisko, wersja PHP, .ovhconfig](configure_your_web_hosting1.).
...
> - dwa zdalne serwery
>
> Więcej informacji na temat korzystania z komendy `scp` z naszym hostingiem OVHcloud znajdziesz w przewodniku "[Hosting WWW - Kopiowanie plików za pomocą komendy SCP](using-scp-command1.)"
>

## Sprawdź również

[Hosting WWW : środowisko, wersja PHP, .ovhconfig](configure_your_web_hosting1.).
