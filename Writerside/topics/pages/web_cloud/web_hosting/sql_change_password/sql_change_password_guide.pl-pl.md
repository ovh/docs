...
Połączenie z tą bazą danych jest możliwe za pomocą **pliku konfiguracyjnego** zawartego [w przestrzeni dyskowej plików](ftp_connection1.) Twojego hostingu. Zawiera on informacje umożliwiające Twojej stronie WWW identyfikację z jej **serwerem bazy danych**.

Zmiana hasła do bazy danych musi zatem zawsze być wykonywana:

- W [pliku konfiguracyjnym](cms_manage_1_click_module#etap-1-zidentyfikowac-baze-danych-powiazana-z-modulem.) Twojej strony WWW za [pomocą przestrzeni FTP Twojego hostingu](ftp_connection1.);

- **I** w serwerze zawierającym bazę danych w [Panelu klienta OVHcloud](manager.).

Dopóki zmiana nie zostanie dokonana **w tych dwóch miejscach**, Twoja strona WWW będzie wyświetlać "[błąd w logowaniu do bazy danych](diagnosis_database_errors#blad-podczas-logowania-do-bazy-danych.)".

Jeśli chcesz zmienić hasło do bazy danych, wykonaj **wszystkie operacje** opisane w tym przewodniku. W przypadku wątpliwości dotyczących sposobu postępowania skontaktuj się z webmasterem lub skontaktuj się z [wyspecjalizowanym](partner.) dostawcą usług hostingowych.

Zmiana hasła do bazy danych składa się z czterech etapów:

- [Etap 1: zidentyfikować plik konfiguracyjny Twojej strony](sql_change_password_#step1.);
- [Etap 2: zidentyfikować bazę danych Twojej strony](sql_change_password_#step2.);
- [Etap 3: zmienić hasło do bazy danych Twojej strony WWW w pliku konfiguracyjnym](sql_change_password_#step3.);
- [Etap 4: zmienić hasło do bazy danych na serwerze baz danych](sql_change_password_#step4.).
...
>
> Oddajemy w Twoje ręce niniejszy przewodnik, którego celem jest pomoc w wykonywaniu bieżących zadań. W przypadku trudności zalecamy skorzystanie z pomocy wyspecjalizowanego webmastera lub kontakt z producentem oprogramowania. Niestety firma OVHcloud nie będzie mogła udzielić wsparcia w tym zakresie. Więcej informacji znajduje się w sekcji [Sprawdź](sql_change_password_#go-further.) ten przewodnik.
>

## Wymagania początkowe

- Posiadanie [hostingu WWW OVHcloud](hosting.)
- Dostęp do [Panelu klienta OVHcloud](manager.).
- Korzystanie z [bazy danych powiązanej z hostingiem](hosting-options-startsql.) lub z [Web Cloud Databases](https://www.ovh.pl/cloud/cloud-databases/).
- Posiadanie danych dostępowych FTP umożliwiających zalogowanie się do [przestrzeni dyskowej Twojego hostingu](ftp_connection1.).

## W praktyce

### Etap 1: zidentyfikować plik konfiguracyjny Twojej strony <a name="step1"></a>

W [Panelu klienta OVHcloud](manager.) kliknij pozycję `Web Cloud`{.action}, a następnie wybierz `Hosting`{.action}, a następnie nazwę odpowiedniego hostingu. Następnie przejdź do zakładki `MultiSite`{.action}. Zidentyfikuj nazwę `Katalog główny` Twojej strony (katalogu, w którym znajdują się pliki i katalogi).
...
> [!primary]
>
> Jeśli chcesz zmienić hasło do Twojej przestrzeni FTP, zapoznaj się z tym [przewodnikiem](ftp_change_password1.).
>
> Jeśli chcesz zalogować się inną metodą, zapoznaj się z tym [przewodnikiem](ftp_connection1.).
>
...
Aby określić, który przypadek dotyczy Twojej strony WWW, w pliku konfiguracyjnym opisanym w [Etap 1](sql_change_password_#step1.), zapisz nazwę bazy danych:
...
Następnie wróć do [Panelu klienta OVHcloud](manager.) w sekcji `Web Cloud`{.action}:
...
Jeśli w tej części Twojego Panelu klienta znalazłeś nazwę bazy znajdującej się w pliku konfiguracyjnym, przejdź do [Etap 3](sql_change_password_#step3.).
...
W ten sam sposób, jak w [Etap 1](sql_change_password_#step1.), powróć do przestrzeni dyskowej plików Twojego hostingu, a następnie otwórz w edycji plik konfiguracyjny Twojej strony.
...
- Jeśli Twoja baza danych znajduje się w części [Panelu klienta OVHcloud](manager.) poświęconej [hostingowi](hosting.), postępuj zgodnie z tymi [instrukcjami](#case1.).

- Jeśli Twoja baza danych znajduje się w części [Panelu klienta OVHcloud](manager.) poświęconej ofertom [Web Cloud Databases](web-cloud-clouddb1.), postępuj zgodnie z tymi [instrukcjami](#case2.).
...
W oknie, które się wyświetla wprowadź nowe hasło do Twojej bazy danych (zdefiniowane w [Etap 3](sql_change_password_#step3.)), potwierdź je, następnie kliknij przycisk `Zatwierdź`{.action}.
...
Aby zmienić hasło do bazy danych, kliknij trzy kropki po prawej stronie `nazwy użytkownika`{.action} zidentyfikowanej w [Etap 2](sql_change_password_#step2.), a następnie `Zmień hasło`{.action}.
...
[Przewodnik dotyczący korzystania z programu FileZilla](ftp_filezilla_user_guide1.)
