...
- Dans le [fichier de configuration](cms_manage_1_click_module#etape-1-identifier-la-base-de-donnees-lie-a-votre-module.) de votre site via [l'espace FTP de votre hébergement](ftp_connection1.);
...
- Utiliser une [base de données associée à votre offre d'hébergement mutualisé](hosting-options-startsql.) ou à un [serveur Web Cloud Databases](databases.).
- Être en possession des identifiants FTP permettant de se connecter à l'[espace de stockage](ftp_connection1.) de votre hébergement.
...
Afin de déterminer quel cas s'applique à votre site, dans le fichier de configuration identifié à [l'étape 1](sql_change_password_#step1.), commencez par noter le nom de sa base de données :
...
Si vous avez retrouvé dans cette partie de votre espace client le nom de la base indiqué dans le fichier de configuration, passez à [l'étape 3](sql_change_password_#step3.).

Dans le cas contraire, la base de données de votre site est liée à une offre [Web Cloud Databases](databases.).
...
Dans l'onglet `Informations générales`{.action}, identifiez, dans votre offre [Web Cloud Databases](databases.), celle qui contient, sous la mention `Nom d'hôte`{.action}, le nom du serveur de votre base de données trouvé précédemment.
...
- Si votre base de données se trouve dans la partie de votre [espace client OVHcloud](manager.) dédiée à votre [hébergement web](hosting.), suivez ces [instructions](#case1.).
- Si votre base de données se trouve dans la partie de votre [espace client OVHcloud](manager.) dédiée à votre offre [Web Cloud Databases](web-cloud-clouddb1.), suivez ces [instructions](#case2.).
...
Pour modifier le mot de passe de votre base de données sur le serveur, cliquez sur les trois points à droite du `Nom d'utilisateur`{.action} identifié à [l'étape 2](sql_change_password_#step2.) puis sur `Changer le mot de passe`{.action}.
