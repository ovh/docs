...
La connexion à cette base de données est rendue possible par un **fichier de configuration** contenu dans [l'espace de stockage de fichiers](ftp_connection1.) de votre hébergement. Il contient les informations permettant à votre site de « s'identifier » auprès de son **serveur de base de données**.

Le changement de mot de passe d'une base de données doit donc toujours être effectué :

- Dans le [fichier de configuration](cms_manage_1_click_module#etape-1-identifier-la-base-de-donnees-liee-a-votre-module.) de votre site via [l'espace FTP de votre hébergement](ftp_connection1.);

- **Et** dans le serveur contenant sa base de données via votre [espace client OVHcloud](manager.).

Tant que ce changement n'aura pas été effectué **dans ces deux endroits**, votre site affichera une « [erreur de connexion à la base de données](diagnosis_database_errors#erreur-lors-de-la-connexion-a-la-base-de-donnees.) ».

Il est donc impératif, si vous souhaitez changer le mot de passe de votre base de données, que vous réalisiez **l'ensemble des opérations** indiquées dans ce guide. En cas de doutes sur les manipulations à réaliser, contactez votre webmaster ou faites appel à un [prestataire spécialisé](partner.).

La modification du mot de passe de la base de données de votre site se fait en quatre étapes :

- [Étape 1 : identifier le fichier de configuration de votre site](sql_change_password_#step1.);
- [Étape 2 : identifier la base de données de votre site](sql_change_password_#step2.);
- [Étape 3 : modifier le mot de passe de la base de données de votre site dans son fichier de configuration](sql_change_password_#step3.);
- [Étape 4 : modifier le mot de passe de la base de données de votre site sur le serveur de bases de données](sql_change_password_#step4.).
...
>
> Nous mettons à votre disposition ce guide afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) et/ou de contacter l'éditeur du service si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [Aller plus loin](sql_change_password_#go-further.) de ce guide.
>
...
- Utiliser une [base de données associée à votre offre d'hébergement mutualisé](hosting-options-startsql.).
- Être en possession des identifiants FTP permettant de se connecter à l'[espace de stockage de votre hébergement](ftp_connection1.).

## En pratique

### Étape 1 : identifier le fichier de configuration de votre site <a name="step1"></a>

Dans votre [espace client OVHcloud](manager.), cliquez sur `Web Cloud`{.action} puis sur `Hébergements`{.action} et enfin sur l'hébergement concerné. Rendez-vous ensuite dans l'onglet `Multisite`{.action}. Identifiez le nom du `Dossier racine` de votre site (le répertoire dans lequel se trouvent ses fichiers et dossiers).
...
> [!primary]
>
> Si vous souhaitez modifier le mot de passe de votre espace FTP, consultez ce [guide](ftp_change_password1.).
>
> Si vous souhaitez vous y connecter par une autre méthode, consultez ce [guide](ftp_connection1.).
>
...
Dans le fichier de configuration identifié à [l'étape 1](sql_change_password_#step1.), commencez par noter le nom de sa base de données :
...
Retournez ensuite dans votre [espace client OVHcloud](manager.) toujours dans la partie `Web Cloud`{.action} :
...
> [!primary]
>
> Pour plus d'informations sur les bonnes pratiques en matière de gestion de mots de passe, suivez les instructions de ce [guide](manage-ovh-password1.).
>
...
De la même manière qu'à [l'étape 1](sql_change_password_#step1.), retournez dans l'espace de stockage de fichiers de votre hébergement puis ouvrez en édition le fichier de configuration de votre site.
...
Dans la fenêtre qui s'affiche, renseignez le nouveau mot de passe de votre base de données (défini à [l'étape 3](sql_change_password_#step3.)), confirmez-le puis cliquez sur le bouton `Valider`{.action}.

![database-password-step3](change-password-window.png){.thumbnail}

## Aller plus loin <a name="go-further"></a>

[Utilisation logiciel FileZilla avec votre hébergement](ftp_filezilla_user_guide1.)
