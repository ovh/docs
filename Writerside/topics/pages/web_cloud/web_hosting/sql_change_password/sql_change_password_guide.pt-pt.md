...
A ligação a esta base de dados é possível graças a um **ficheiro de configuração** contido no [espaço de armazenamento de ficheiros](ftp_connection1.) do seu alojamento. Contém as informações que permitem ao seu site "identificar-se" junto do seu **servidor de base de dados**.

Por isso, a alteração da palavra-passe de uma base de dados deve ser sempre efetuada:

- No [ficheiro de configuração](cms_manage_1_click_module#etapa-1-identificar-a-base-de-dados-do-modulo.) do seu site através do [espaço FTP do seu alojamento](ftp_connection1.);

- **E** no servidor que contém a base de dados através da [Área de Cliente OVHcloud](manager.).

Enquanto esta alteração não for efetuada **nestes dois locais**, o seu site irá apresentar um "[erro de ligação à base de dados](diagnosis_database_errors#error-establishing-a-database-connection.)".

Assim, para alterar a palavra-passe da base de dados, é imperativo que realize **o conjunto das operações** indicadas neste guia. Se tiver dúvidas sobre as operações a realizar, contacte o seu webmaster ou um [fornecedor especializado](partner.).

A alteração da palavra-passe da base de dados do seu site faz-se em quatro etapas:

- [Etapa 1: identificar o ficheiro de configuração do seu site](sql_change_password_#step1.);
- [Etapa 2: identificar a base de dados do seu site](sql_change_password_#step2.);
- [Etapa 3: alterar a palavra-passe da base de dados do seu site no seu ficheiro de configuração](sql_change_password_#step3.);
- [Etapa 4: alterar a palavra-passe da base de dados do seu site para o servidor de bases de dados](sql_change_password_#step4.).
...
>
> Este manual fornece as instruções necessárias para realizar as operações mais habituais. No entanto, se encontrar dificuldades, recomendamos que recorra a um [prestador de serviços especializado](partner.) e/ou que contacte o editor do serviço. Não poderemos proporcionar-lhe assistência técnica. Para mais informações, aceda à secção [Quer saber mais](sql_change_password_#go-further.)?
>

## Requisitos

- Ter um [plano de alojamento web OVHcloud](hosting.).
- Ter acesso à [Área de Cliente OVHcloud](manager.).
- Utilizar uma [base de dados associada à sua oferta de alojamento partilhado](hosting-options-startsql.) ou a um [Web Cloud Databases](https://www.ovh.pt/cloud/cloud-databases/).
- Ter acesso às credenciais FTP para aceder ao [espaço de armazenamento do alojamento](ftp_connection1.).

## Instruções

### Etapa 1: identificar o ficheiro de configuração do seu site <a name="step1"></a>

Na sua [Área de Cliente OVHcloud](manager.), clique em `Web Cloud`{.action} e em `Alojamentos`{.action} e, por fim, no alojamento em causa. De seguida, aceda ao separador `Multisite`{.action}. Identifique o nome da `Pasta raiz` do seu site (o diretório no qual se encontram os seus ficheiros e pastas).
...
> [!primary]
>
> Se pretender alterar a palavra-passe do seu espaço FTP, consulte este [guia](ftp_change_password1.).
>
> Para aceder através de outro método, consulte este [guia](ftp_connection1.).
>
...
Para determinar o caso aplicável ao seu website, no ficheiro de configuração identificado na [Etapa 1](sql_change_password_#step1.), comece por anotar o nome da sua base de dados:
...
De seguida, volte à sua [Área de Cliente OVHcloud](manager.) na secção `Web Cloud`{.action}:
...
Se localizar o nome da base de dados indicado no ficheiro de configuração nesta parte da Área de Cliente, consulte o [Etapa 3](sql_change_password_#step3.).
...
Da mesma forma que na [Etapa 1](sql_change_password_#step1.), volte ao espaço de armazenamento de ficheiros do seu alojamento e abra para edição o ficheiro de configuração do seu site.
...
- Se a base de dados estiver na parte da [Área de Cliente OVHcloud](manager.) dedicada ao seu [alojamento web](hosting.), siga as [instruções](#case1.).

- Se a sua base de dados estiver na parte da sua [Área de Cliente OVHcloud](manager.) dedicada às suas ofertas [Web Cloud Databases](web-cloud-clouddb1.), siga as [instruções](#case2.).
...
Na nova janela, introduza a nova palavra-passe da base de dados (definida no [Etapa 3](sql_change_password_#step3.)), confirme que está correta e clique no botão `Validar`{.action}.
...
Para alterar a palavra-passe da base de dados no servidor, clique nos três pontos à direita do `Nome de utilizador`{.action} identificado na [Etapa 2](sql_change_password_#step2.) e, a seguir, em `Alterar palavra-passe`{.action}.
...
[Partilhado: Guia de utilização do FileZilla](ftp_filezilla_user_guide1.)
