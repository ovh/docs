...
>
> Questa guida ti aiuta a eseguire le operazioni necessarie. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a un [provider specializzato](partner.) e/o di contattare la nostra [Community di utenti](https://community.ovh.com/en/). OVHcloud non potrà fornirti alcuna assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](diagnostic_403_forbidden_#go-further.) di questa guida.
>

## Prerequisiti

- Disporre di una [soluzione di hosting Web](hosting.) OVHcloud
- Disporre delle [credenziali di accesso](ftp_connection1.) allo spazio di storage FTP dell'hosting
...
Se la pagina **"403 forbidden"** è apparsa in seguito a una modifica del tuo sito Web, [ripristina totalmente o in parte lo spazio di storage FTP del tuo hosting](ftp_save_and_backup1.) a una data precedente.

Se i backup disponibili non ti permettono di ripristinare l'accesso al tuo sito Web, contatta uno dei [provider specializzati](partner.).
...
- Verifica che sia installato un antivirus, aggiornalo e esegui una scansione completa. Se non ne hai uno, consulta [uno specialista](partner.) prima di installare il tuo servizio.

- Modificate l'insieme delle vostre password locali, in particolare quelle dei vostri indirizzi email, rispettando i **best practice** specificati in [questa guida](manage-ovh-password1.).

- Modifica le password di tutti i tuoi servizi OVHcloud, in particolare quelli del tuo [database](sql_change_password1.) e dell'accesso al tuo [spazio di storage FTP](ftp_change_password1.).

> [!warning]
>
> Prima di modificare la password del database del tuo sito Web dallo [Spazio Cliente OVHcloud](manager.), aggiorna il file di configurazione del tuo sito Web per connettersi al database con la nuova password.
>
...
>
> In caso di dubbi sulle operazioni da effettuare, contatta i [partner OVHcloud](partner.).
>
...
>
> In caso di disattivazione tramite restrizione di accesso FTP in "**CHMOD 000**", contatta obbligatoriamente i nostri [team di supporto](support.) per fare il punto sulla situazione prima di proseguire gli step descritti in questa guida. 
>
...
Per ripristinare totalmente o in parte il tuo spazio di storage FTP, consulta la [nostra guida](ftp_save_and_backup1.) sull'argomento.
...
> Per identificare queste falle di sicurezza, analizza i ["log web"](logs_and_statistics1.) del tuo hosting web o rivolgiti a un [fornitore specializzato](partner.) per effettuare un audit di sicurezza dei tuoi siti Web.
>

#### Caso 2: il tuo hosting è stato disattivato più di due settimane fa

Se il tuo hosting è stato chiuso più di due settimane fa, contatta un [provider specializzato](partner.) per effettuare un audit di sicurezza dei tuoi siti Web. 

> [!success]
>
> Per maggiori informazioni sui [step 2 e 3](#step-2.), consulta la nostra guida su [le azioni da effettuare in caso di hacking del tuo sito Web](cms_what_to_do_if_your_site_is_hacked1.).
>
...
> [!primary]
>
> Per installare il software **FileZilla** per gestire i file del tuo sito Web, segui le istruzioni di questo [tutorial](ftp_filezilla_user_guide1.).
>

Apri il software FileZilla e poi [collegati al tuo spazio di storage FTP](ftp_connection1.). Clicca su `Server`{.action} nella barra del menu e poi su `Effettua un ordine FTP`{.action} (la denominazione può essere diversa in base alla versione di FileZilla utilizzata):
...
Nello [Spazio Cliente OVHcloud](manager.), accedi alla sezione `Web Cloud`{.action}, poi `Hosting`{.action} e clicca sulla scheda `FTP-SSH`{.action} dell'hosting in questione.

Clicca su `FTP Explorer`{.action} e accedi al tuo [spazio di storage FTP](ftp_connection1.).
...
[Consigli in caso di hacking del tuo sito WordPress](cms_what_to_do_if_your_site_is_hacked1.)

[Attivazione del firewall applicativo](multisites_activating_application_firewall1.)

[Modificare la versione di PHP su un hosting Web](configure_your_web_hosting1.)
