...
> 
> Diese Anleitung soll Sie bei allgemeinen Aufgaben bestmöglich unterstützen. Dennoch empfehlen wir Ihnen, falls Sie Hilfe brauchen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren oder Ihre Fragen in der [OVHcloud Community](https://community.ovh.com/en/) zu stellen. Leider können wir Ihnen für administrative Aufgaben keine weitergehende technische Unterstützung anbieten. Weitere Informationen finden Sie am [Ende dieser Anleitung](diagnostic_403_forbidden_#go-further.).
>

## Voraussetzungen

- Sie haben ein [OVHcloud Webhosting](hosting.).
- Sie verfügen über die [Login-Daten](ftp_connection1.) für den FTP-Speicherplatz Ihres Hostings.
...
Wenn die Seite "**403 forbidden**" nach einer Änderung an Ihrer Website erscheint, [setzen Sie den FTP-Speicherplatz Ihres Hostings ganz oder teilweise zu einem früheren Zeitpunkt zurück](ftp_save_and_backup1.).

Wenn Sie mit den verfügbaren Backups den Zugang zu Ihrer Website nicht wiederherstellen können, kontaktieren Sie einen [spezialisierten Dienstleister](partner.).
...
- Überprüfen Sie, ob ein Antivirus-Dienst installiert ist, aktualisieren Sie ihn und starten Sie einen vollständigen Scan. Wenn Sie keine solche Software besitzen, konsultieren Sie vor der Installation einen [spezialisierten Dienstleister](partner.)

- Ändern Sie alle Ihre persönlichen Passwörter, insbesondere Ihrer E-Mail-Accounts, und halten Sie sich dabei an die **Best Practices** in [dieser Anleitung](manage-ovh-password1.).

- Ändern Sie die Passwörter für alle Ihre OVHcloud Dienste, insbesondere für die [Datenbank](sql_change_password1.) und den [FTP Speicherplatz](ftp_change_password1.).

> [!warning]
>
> Bevor Sie Datenbankpasswörter Ihrer Website über Ihr [OVHcloud Kundencenter](manager.) ändern, aktualisieren Sie die Konfigurationsdatei Ihrer Website, damit sie weiterhin auf die Datenbank zugreifen kann.
>
...
>
> Sollten Sie Hilfe bei den durchzuführenden Maßnahmen benötigen, wenden Sie sich an die [OVHcloud Partner](partner.).
>
...
>
> Wenn sie "**CHMOD 000**" zurücksetzen möchten, kontaktieren Sie unsere [Support-Teams](support.), um den Status Ihrer Website überprüfen zu lassen, bevor Sie mit den in dieser Anleitung beschriebenen Schritten fortfahren. 
>
...
Um Ihren FTP-Speicherplatz ganz oder teilweise zurückzusetzen, lesen Sie [unsere Anleitung](ftp_save_and_backup1.).
...
>
> Um diese Sicherheitslücken zu identifizieren, analysieren Sie die [Web-Logs](logs_and_statistics1.) Ihres Hostings oder wenden Sie sich an einen einen [spezialisierten Dienstleister](partner.), um eine Sicherheitsüberprüfung Ihrer Webseiten durchzuführen.
>

#### Fall 2: Ihr Hosting wurde vor mehr als zwei Wochen deaktiviert

Wenn Ihr Hosting vor mehr als zwei Wochen gesperrt wurde, kontaktieren Sie einen [spezialisierten Dienstleister](partner.), um eine Sicherheitsüberprüfung Ihrer Webseiten durchzuführen.

> [!success]
>
> Wenn Sie weitere Informationen zu den [Schritten 2 und 3](#step-2.) benötigen, lesen Sie unsere Hilfe zu [Aktionen wenn Ihre Website von einem Hack betroffen ist](cms_what_to_do_if_your_site_is_hacked1.).
>
...
> [!primary]
>
> Wenn Sie die Anwendung **FileZilla** installieren möchten, um die Dateien Ihrer Website zu bearbeiten, folgen Sie den Anweisungen in [diesem Tutorial](ftp_filezilla_user_guide1.).
>

Öffnen Sie FileZilla und [verbinden Sie sich mit Ihrem FTP-Speicherplatz](ftp_connection1.). Klicken Sie anschließend im Menü auf `Server`{.action} und dann auf `Benutzerdefinierten Befehl eingeben`{.action}. (Je nach der Version von FileZilla kann die Bezeichnung variieren.)
...
Gehen Sie in Ihrem [OVHcloud Kundencenter](manager.) in den Bereich `Web Cloud`{.action} und öffnen Sie `Hosting-Pakete`{.action} im linken Menü. Wechseln Sie zum Tab `FTP-SSH`{.action} des betroffenen Hostings.

Klicken Sie auf den Button `FTP-Explorer`{.action} und [loggen Sie sich in Ihren FTP-Speicherplatz ein](ftp_connection1.). 
...
[Aktivieren der Web Application Firewall](multisites_activating_application_firewall1.)

[PHP-Version Ihres Webhostings ändern](configure_your_web_hosting1.)
