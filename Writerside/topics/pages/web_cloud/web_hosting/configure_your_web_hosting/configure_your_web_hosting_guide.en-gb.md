...
The [OVHcloud Web Hosting](hosting.){.external} offers allow you to host your websites, provided they are compatible with the [configuration of our shared infrastructure](https://webhosting-infos.hosting.ovh.net){.external}.
...
- In your [OVHcloud Control Panel](manager.)
...
In short, changing your Web Hosting plan’s configuration from the [OVHcloud Control Panel](manager.), or changing the values in the .ovhconfig file is the same as doing the same.
...
- An [OVHcloud web hosting plan](hosting.){.external}, with the exception of a [Cloud Web hosting plan](hosting-cloud-web-offer.)
- Access to your web hosting plan in the [OVHcloud Control Panel](manager.), or via [FTP login](ftp_connection1.)
...
> [!warning]
>
> Modifying at least one of these elements may affect the display or the proper functioning of your website. **Please ensure that your website is compatible with the changes you want to make to your web hosting plan’s configuration**. Contact a [specialist provider](partner.) if you have any doubts or encounter any difficulties.
>
...
There are several versions of the PHP programming language. Version upgrades provide various patches and add or remove features. OVHcloud offers the latest major versions of PHP, a list of which can be found [here](hosting-programming-language.).
...
- **In the OVHcloud Control Panel**: Log in to the [OVHcloud Control Panel](manager.){.external}, then go to the `Web Cloud`{.action} section. In the left-hand column, click `Hosting plans`{.action}, then select the Web Hosting plan concerned. In the `General information`{.action} tab, find the version below *Global PHP version*. 
...
Put it online by placing it in your [FTP storage space](ftp_connection1.), then call it by accessing its full address/URL.

> [!warning]
>
> You can no longer modify the PHP version via a .htaccess file on the latest [OVHcloud Web Hosting](hosting.) solutions.<br>
...
Please refer to our guide “[Activating the application firewall](multisites_activating_application_firewall1.)” for more details on this topic.
...
> [!warning]
>
> As a reminder, changing at least one of these elements may affect the display or the proper functioning of your website. **Please ensure that your website is compatible with the changes you want to make to your web hosting plan’s configuration.** If you have any doubts or encounter any difficulties, please contact a [specialist provider](partner.).
>

#### 2.1 - Access Web Hosting plan configuration management

Log in to your [OVHcloud Control Panel](manager.){.external}, then go to the `Web Cloud`{.action} section. In the left-hand column, click `Hosting plans`{.action}, then select the Web Hosting plan concerned. Make sure you are positioned on the tab
...
>
> If the `Global PHP version`{.action} option does not appear in your [OVHcloud Control Panel](manager.), check that the *.ovhconfig* file exists in the FTP root of your OVHcloud web hosting plan.
>
...
Log in to your [OVHcloud Control Panel](manager.){.external}, then go to the `Web Cloud`{.action} section. In the left-hand column, click `Hosting plans`{.action}, then select the web hosting plan concerned. Go to the `FTP - SSH`{.action} tab. Here, you will find the information you need to log in.

For the FTP user password, please refer to our guide on [Modifying an FTP user password](ftp_change_password1.) if necessary.

![ovhconfig](tab-perso.png){.thumbnail}

#### 3.2 - Create or open the .ovhconfig file

Once you have logged in to your [FTP storage space](ftp_connection1.), you can view all of the files currently hosted on it. Inside the root directory of your hosting (which is indicated by a `/`) you should find the .ovhconfig file.
...
|app.engine.version|Allows you to define [the PHP version](#php-versions.) used by the hosting plan among [the ones OVHcloud offers](hosting-programming-language.){.external}. Enter the version of your choice (appropriate for the runtime environment you have chosen to use).|
|http.firewall|Enables or disables the [firewall of your OVHcloud web hosting plan](hosting-options.){.external}. Enter **security** to enable it, or **none** to disable it.|
...
Once you have modified the .ovhconfig file, upload it to your [FTP storage space](ftp_connection1.). Connect to your [FTP storage space](ftp_connection1.) and go to the root of your [FTP storage space](ftp_connection1.) (which is indicated by a `/`). Upload the .ovhconfig file you have just edited to your [FTP storage space](ftp_connection1.). If the file already exists, replace it.
...
Create a .ovhconfig file containing the PHP version you want for the *multisites* concerned. Use the instructions in the section [3.3 - Modify the content of the .ovhconfig file](#update-ovhconfig.) of this guide, if required. When you upload the .ovhconfig file to your [FTP storage space](ftp_connection1.), do so in the root folder where the files that make up the relevant "*multisites*" website are located. You can find the root folder of your *multisites* in your [OVHcloud Control Panel](manager.) in the `Multisite`{.action} tab of the Web Hosting plan concerned.

Refer to our guide on [Configuring a multisite on your web hosting plan](multisites_configure_multisite1.), if required.

> [!warning]
>
> **You cannot specify a second [runtime environment](#runtime-environment.), a second [runtime mode](#runtime-mod.) and/or a second [PHP runtime environment](#php-runtime.)** on the same web hosting plan. Only those entered in the .ovhconfig file located in the **root folder** of your [FTP storage space](ftp_connection1.) will be taken into account.
>
...
[Log in to your Web Hosting plan’s storage space](ftp_connection1.)
