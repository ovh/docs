...
Les offres d'[hébergement web OVHcloud](hosting.){.external} permettent d’héberger le site web que vous souhaitez, tant que celui-ci est compatible avec la [configuration de nos infrastructures mutualisées](https://webhosting-infos.hosting.ovh.net){.external}.
...
- depuis votre [espace client OVHcloud](manager.) ;
...
En résumé, modifier la configuration de votre hébergement web depuis l'[espace client OVHcloud](manager.) ou modifier les valeurs présentes dans le fichier « .ovhconfig » revient à réaliser la même opération.
...
- Disposer d’une offre d’[hébergement web OVHcloud](hosting.){.external}, à l'exception d'une [offre d'hébergement Cloud Web](hosting-cloud-web-offer.).
- Avoir accès à votre offre d’hébergement web depuis l’[espace client OVHcloud](manager.) ou connaître les informations permettant de se connecter à l'[espace de stockage FTP](ftp_connection1.).
...
Plusieurs versions du langage de programmation PHP existent. Les évolutions de versions apportent des correctifs divers ainsi que l'ajout ou l'arrêt de fonctionnalités. OVHcloud propose les dernières versions majeures de PHP dont vous pouvez retrouver la liste [ici](hosting-programming-language.).
...
- **Via l'espace client OVHcloud**. Connectez-vous à [l'espace client OVHcloud](manager.){.external} puis rendez-vous dans la partie `Web Cloud`{.action}. Dans la colonne de gauche, cliquez sur `Hébergements`{.action} puis choisissez l'hébergement web concerné. Dans l'onglet `Informations générales`{.action}, repérez la version en dessous de *Version PHP globale*. 
...
Mettez-le ensuite en ligne sur votre [espace de stockage FTP](ftp_connection1.), puis appelez-le en accédant à son adresse/URL complète.

> [!warning]
>
> La modification de la version de PHP via un fichier « .htaccess » n'est plus possible sur les dernières offres d'[hébergement web OVHcloud](hosting.).<br>
...
Consultez notre guide « [Activation du pare-feu applicatif](multisites_activating_application_firewall1.) » pour plus de détails sur le sujet.
...
> [!warning]
>
> Pour rappel, modifier au moins l'un de ces éléments peut avoir des conséquences sur l'affichage ou le bon fonctionnement de votre site web. **Assurez-vous au préalable que votre site web est compatible avec les changements que vous souhaitez effectuer dans la configuration de votre hébergement web.** Contactez un [prestataire spécialisé](partner.) en cas de doute ou si vous rencontrez des difficultés.
>

#### 2.1 - Accéder à la gestion de la configuration de l'hébergement web

Connectez-vous à votre [espace client OVHcloud](manager.){.external} puis rendez-vous dans la partie `Web Cloud`{.action}. Dans la colonne de gauche, cliquez sur `Hébergements`{.action} puis choisissez l'hébergement web concerné. Assurez-vous d'être bien positionné sur l'onglet
...
>
> Si l'option `Version PHP Globale`{.action} n'apparaît pas dans votre [espace client OVHcloud](manager.), vérifiez que fichier *.ovhconfig* existe bien à la racine FTP de votre hébergement mutualisé OVHcloud.
>
...
Pour cela, connectez-vous à votre [espace client OVHcloud](manager.){.external} puis rendez-vous dans la partie `Web Cloud`{.action}. Dans la colonne de gauche, cliquez sur `Hébergements`{.action} puis sélectionnez l'hébergement concerné. Positionnez-vous enfin sur l'onglet `FTP - SSH`{.action}. Vous y retrouverez les informations vous permettant de vous connecter. 

Concernant le mot de passe de l'utilisateur FTP, reportez-vous aux instructions décrites dans notre documentation [« Modifier le mot de passe d’un utilisateur FTP »](ftp_change_password1.) si nécessaire.

![ovhconfig](tab-perso.png){.thumbnail}

#### 3.2 - Récupérer ou créer le fichier « .ovhconfig »

Une fois connecté à votre [espace de stockage FTP](ftp_connection1.), vous visualisez l'ensemble des fichiers actuellement hébergés sur ce dernier. Restez positionné sur la racine de votre hébergement (que l'on peut symboliser par un « / »). Vous devriez y trouver le fichier « .ovhconfig ».
...
|app.engine.version|Permet de définir [la version de PHP](#php-versions.) utilisée par l'hébergement parmi [celles qu'OVHcloud propose](hosting-programming-language.){.external}. Renseignez la version de votre choix (en adéquation avec l'environnement d'exécution que vous avez choisi d'utiliser).|
|http.firewall|Permet d'activer ou de désactiver le [firewall fourni avec les hébergements web OVHcloud](hosting-options.){.external}. Renseignez **security** pour l'activer ou **none** pour le désactiver.|
...
Une fois le fichier « .ovhconfig » modifié, téléchargez-le sur votre [espace de stockage FTP](ftp_connection1.). Pour cela, reconnectez-vous à votre [espace de stockage FTP](ftp_connection1.) et positionnez-vous à la racine votre [espace de stockage FTP](ftp_connection1.) (que l'on peut symboliser par un « / »). Chargez le fichier « .ovhconfig » que vous venez de modifier dans votre [espace de stockage FTP](ftp_connection1.). Si le fichier existe déjà, remplacez-le.
