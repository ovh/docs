...
Le offerte di [hosting Web OVHcloud](hosting.){.external} permettono di ospitare il sito Web che si desidera, purché sia compatibile con la [configurazione delle nostre infrastrutture condivise](https://webhosting-infos.hosting.ovh.net){.external}.
...
- dallo [Spazio Cliente OVHcloud](manager.);
...
Per riassumere, modificare la configurazione dell’hosting Web dallo [Spazio Cliente OVHcloud](manager.) o modificare i valori presenti nel file ".ovhconfig" equivale a effettuare la stessa operazione.
...
- Disporre di una soluzione di [hosting Web OVHcloud](hosting.){.external}, ad eccezione di una [soluzione di hosting Cloud Web](hosting-cloud-web-offer.).
- Avere accesso al piano di hosting Web dallo [Spazio Cliente OVHcloud](manager.) o conoscere le informazioni di accesso allo [spazio di storage FTP](ftp_connection1.).
...
Esistono diverse versioni del linguaggio di programmazione PHP. Le modifiche apportate alle versioni apportano patch di vario tipo e aggiungono o rimuovono alcune funzionalità. OVHcloud propone le ultime versioni principali di PHP, disponibili nella lista [qui](hosting-programming-language.).
...
- **Dallo Spazio Cliente OVHcloud**. Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e seleziona `Web Cloud`{.action}. Nella colonna di sinistra, clicca su `Hosting`{.action} e seleziona l’hosting Web interessato. Nella scheda `Informazioni generali`{.action}, individua la versione sotto *Versione PHP*. 
...
Accedi allo [spazio di storage FTP](ftp_connection1.) e chiama il tuo dominio accedendo al suo indirizzo/URL completo.

> [!warning]
>
> La modifica della versione di PHP tramite un file ".htaccess" non è più possibile sulle ultime offerte di [hosting Web OVHcloud](hosting.).<br>
...
Per maggiori informazioni, consulta la nostra guida "[Attivazione del firewall applicativo](multisites_activating_application_firewall1.)".
...
Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e clicca su `Web Cloud`{.action}. Nella colonna di sinistra, clicca su `Hosting`{.action} e seleziona l’hosting Web interessato. Assicurati di essere posizionato sulla scheda
...
>
> Se l’opzione `Versione PHP Globale`{.action} non compare nel tuo [Spazio Cliente OVHcloud](manager.), verifica che il file *.ovhconfig* esista effettivamente nella root FTP del tuo hosting condiviso OVHcloud.
>
...
Per farlo, accedi al tuo [Spazio Cliente OVHcloud](manager.){.external} e clicca sulla sezione `Web Cloud`{.action}. Nella colonna di sinistra, clicca su `Hosting`{.action} e seleziona l’hosting interessato. e clicca sulla scheda `FTP - SSH`{.action}. In questa interfaccia sono disponibili anche le informazioni necessarie per effettuare l’accesso. 

Per conoscere la password associata a un utente FTP, consulta le istruzioni fornite nella nostra guida ["Modificare la password di un utente FTP"](ftp_change_password1.).

![ovhconfig](tab-perso.png){.thumbnail}

#### 3.2 - Recuperare o creare il file ".ovhconfig"

Una volta effettuato l’accesso allo [spazio di storage FTP](ftp_connection1.), visualizzi tutti i file attualmente ospitati su questo spazio. Rimani posizionato sulla root dell’hosting (simbolo "/"). dovrebbe essere presente il file ".ovhconfig".
...
|app.engine.version|Permette di definire [la versione di PHP](#php-versions.) utilizzata dall’hosting tra [quelle proposte da OVHcloud](hosting-programming-language.){.external}. Inserisci la versione scelta (in base all’ambiente di esecuzione che hai scelto di utilizzare).|
|http.firewall|Permette di attivare o disattivare il [firewall incluso negli hosting Web OVHcloud](hosting-options.){.external}. Inserisci **security** per attivarlo o **none** per disattivarlo.|
...
Una volta modificato il file ".ovhconfig", caricalo sul tuo [spazio di storage FTP](ftp_connection1.). Riconnettiti allo [spazio di storage FTP](ftp_connection1.) e posizionati alla root del tuo [spazio di storage FTP](ftp_connection1.) (simbolo "/"). Carica il file ".ovhconfig" modificato nello [spazio di storage FTP](ftp_connection1.). Se il file esiste già, sostituiscilo.
...
Crea un file ".ovhconfig" contenente la versione di PHP desiderata per i *multisiti* interessati. Se necessario, guida alle operazioni descritte nella sezione "[3.3 - Modificare il contenuto del file ".ovhconfig"](#update-ovhconfig.)" di questa guida. Quando carichi il file ".ovhconfig" sul tuo [spazio di storage FTP](ftp_connection1.), accedi alla cartella root in cui si trovano i file che compongono il sito Web "*multisiti*" interessato. Consulta la cartella root dei tuoi *multisiti* nel tuo [Spazio Cliente OVHcloud](manager.) nella scheda `Multisito`{.action} dell’hosting interessato.

Se necessario, consulta la nostra guida "[Configurare un multisito su un hosting Web](multisites_configure_multisite1.)".

> [!warning]
>
> **Non è possibile specificare un secondo [ambiente di esecuzione](#runtime-environment.), un secondo [modo di esecuzione](#runtime-mod.) e/o un secondo [motore di esecuzione PHP](#php-runtime.)** sullo stesso hosting Web. Saranno presi in considerazione solo quelli inseriti nel file ".ovhconfig" che si trova nella root del tuo [spazio di storage FTP](ftp_connection1.).
>
...
[Accedere allo spazio di storage dell’hosting Web](ftp_connection1.)
