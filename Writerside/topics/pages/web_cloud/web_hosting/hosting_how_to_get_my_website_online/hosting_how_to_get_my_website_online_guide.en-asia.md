...
If you want to create a new database with OVHcloud, log in to the [OVHcloud Control Panel](manager.){.external}, then click on `Hosting`{.action} in the services bar on the left. Select the name of the hosting plan concerned, and click on the `Database`{.action} tab.
