...
- You have at least one [domain name](domains.).
- You have an active DNS zone for your domain name. If you need further guidance, please refer to our guide on "[Creating an OVHcloud DNS zone](dns_zone_create1.)".
- You have access to the [OVHcloud Control Panel](manager.).
- You have sufficient rights to all of the services concerned. You can find more information in our guide [Managing contacts for your services](managing_contacts1.).
  
## Instructions

### Definition of a subdomain

A [domain name](domains.) can be associated with several types of services (email, website, etc.).
...
- *dns1.example.com* and *dns2.example.com* to customize your DNS servers using [GLUE records](glue_registry1.)
...
[Domain names](domains.) all need a **DNS zone** to work which is a domain name’s configuration file. It consists of DNS **records** that map the domain name to various services and functions.

For more details on DNS zones, please read our guides “[Creating an OVHcloud DNS zone](dns_zone_create1.)” and “[Editing an OVHcloud DNS zone](dns_zone_edit1.)”.
...
>
> 1: To identify the location of the active DNS zone for a domain name registered with OVHcloud, you can use our guide “[Modifying the DNS servers for an OVHcloud domain name](dns_server_general_information1.)”.
>
> 2: If your domain name is not registered with OVHcloud, contact your domain name’s current *registrar* to find out where the active DNS zone is hosted. Please note that you can use our guide on [Transferring a domain name to OVHcloud](transfer_incoming_generic_domain1.) if you would like to perform this action.
>
...
To add your subdomains to your domain name’s active DNS zone, please read our guide on "[Editing an OVHcloud DNS zone](dns_zone_edit1.)".
...
As with a domain name, and to authorize a subdomain to display the contents of a *target* folder on a web hosting plan, log in to your [OVHcloud Control Panel](manager.){.external} and select `Web Cloud`{.action}. Click on `Hosting plans`{.action} in the left-hand column, select the plan concerned where your website is located, then select the `Multisite`{.action} tab.

This is where you authorize your subdomain to access your web hosting plan, where your website is located.

For more details on setting up a domain or subdomain on a web hosting plan, please read our guide on [Hosting multiple websites on your web hosting plan](multisites_configure_multisite1.). The procedure is the same whether you are using a domain name or a subdomain.
...
- [Getting started with the Hosted Exchange service](exchange_starting_hosted1.)
- [Adding a domain name on an email platform](exchange_adding_domain1.)
- [Adding a CNAME record to validate your domain on your email solution](exchange_dns_cname1.)
