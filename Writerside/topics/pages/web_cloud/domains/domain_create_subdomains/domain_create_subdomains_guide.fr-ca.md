...
- Disposer d'au moins un [nom de domaine](domains.).
- Disposer d'une zone DNS active pour votre nom de domaine. Si besoin, consultez notre guide « [Créer une zone DNS OVHcloud](dns_zone_create1.) ».
- Être connecté à votre [espace client OVHcloud](manager.).
- Disposer des droits suffisants sur l’ensemble des services concernés. Retrouvez plus d'informations sur notre guide « [Gérer les contacts de ses services](managing_contacts1.) ».

## En pratique

### Définition d'un sous-domaine

Un [nom de domaine](domains.) peut être associé à plusieurs types de services (e-mail, site web, etc.). Néanmoins, un nom de domaine ne peut être associé qu'à un seul site web à la fois.
...
- *dns1.example.com* et *dns2.example.com* pour personnaliser vos serveurs DNS à l'aide des [GLUE records](glue_registry1.) ;
...
Les [noms de domaine](domains.) ont tous besoin d'une **zone DNS** pour fonctionner. La zone DNS se compose d’informations techniques, appelées « enregistrements DNS ». Elle est, en quelque sorte, comme un centre d'aiguillage.

Pour plus de détails sur les zones DNS, consultez notre guide « [Créer une zone DNS OVHcloud](dns_zone_create1.) » et « [Editer une zone DNS OVHcloud](dns_zone_edit1.) ».
...
>
> 1 : Pour identifier l'endroit où se trouve la zone DNS active d'un nom de domaine enregistré chez OVHcloud, vous pouvez vous aider de notre guide « [Modifier les serveurs DNS d'un nom de domaine OVHcloud](dns_server_general_information1.) ».
>
> 2 : Si votre nom de domaine n'est pas enregistré chez OVHcloud, rapprochez-vous du *bureau d'enregistrement* actuel de votre nom de domaine afin de connaître l'endroit où est hébergée sa zone DNS active. Sachez que vous pouvez utiliser notre guide « [Transférer son nom de domaine vers OVHcloud](transfer_incoming_generic_domain1.) » si vous souhaitez réaliser cette action.
>
...
Pour ajouter vos sous-domaines dans la zone DNS active de votre nom de domaine, consultez notre guide « [Editer une zone DNS OVHcloud](dns_zone_edit1.) ».
...
Tout comme pour un nom de domaine, pour autoriser un sous-domaine à afficher le contenu d'un dossier « cible » présent sur un hébergement web, connectez-vous à l'[espace client OVHcloud](manager.){.external} et sélectionnez `Web Cloud`{.action}. Cliquez sur `Hébergements`{.action} dans la colonne de gauche, sélectionnez l'offre concernée où se trouve votre site web puis choisissez l'onglet `Multisite`{.action}.

C'est ici que vous autorisez l'accès de votre sous-domaine à votre hébergement web où se trouve votre site web.

Pour plus de détails sur la configuration d'un domaine ou d'un sous-domaine sur un hébergement web, consultez notre guide « [Partager son hébergement entre plusieurs sites](multisites_configure_multisite1.) ». Qu'il s'agisse d'un nom de domaine ou d'un sous-domaine, la procédure est la même.
...
- [Premiers pas avec le service Hosted Exchange](exchange_starting_hosted1.)
- [Ajouter un nom de domaine sur une plateforme e-mail](exchange_adding_domain1.)
- [Ajouter un enregistrement CNAME pour valider votre domaine sur votre offre e-mail](exchange_dns_cname1.)

## Aller plus loin <a name="go-further"></a>

[Créer une zone DNS OVHcloud](dns_zone_create1.)
