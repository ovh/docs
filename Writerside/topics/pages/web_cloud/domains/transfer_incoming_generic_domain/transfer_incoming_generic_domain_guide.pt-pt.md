...
> [!success]
>
> Para conhecer as condições tarifárias para a transferência de um nome de domínio em função da sua extensão, insira o domínio que deseja transferir na nossa página [www.ovhcloud.com/pt/domains/tld/](domains-tld.) e siga as instruções.
>
...
|---|---|---|---|---|
|[1](transfer_incoming_generic_domain_#step1.)|[Verificar a informação relativa ao domínio](transfer_incoming_generic_domain_#step1.)|O administrador do domínio|O agente registador atual|Depende das suas ações|
|[2](transfer_incoming_generic_domain_#step2.)|[Desbloquear o domínio e obter o código de transferência](transfer_incoming_generic_domain_#step2.)|O administrador do domínio, com a autorização do proprietário|O agente registador atual|Depende das suas ações|
...
|[4](transfer_incoming_generic_domain_#step4.)|[Validação da transferência](transfer_incoming_generic_domain_#step4.)|O agente registador atual|Através de um pedido enviado pela entidade que gere a extensão do domínio|5 dias, no máximo|
...
**Para começar, é importante verificar se as informações relativas ao nome de domínio estão atualizadas.** Desde a criação do RGPD, os dados visíveis no ["Whois"](domains-whois.) tornaram-se muito limitados. Assim, recomendamos que verifique as informações relativas ao seu nome de domínio no seu agente registador atual.
...
> [!primary]
>
> Se não sabe qual o agente de registo responsável pelo seu domínio, as linhas "Registar", que aparecerão no resultado da pesquisa da ferramenta [Whois](domains-whois.){.external}, podem fornecer-lhe informações sobre a sua identidade.
>
...
Depois de desbloquear o seu nome de domínio e obter o código, pode encomendar a sua transferência para a OVHcloud a partir do [nosso site](domains.){.external}. Introduza o seu nome de domínio e siga os passos da encomenda.
...
Também pode concluir a sua encomenda com um [alojamento web](hosting.){.external} e outras soluções da OVHcloud. Isto pode interessar-lhe se deseja também migrar os seus serviços para a OVHcloud. O nosso manual intitulado "[Como migrar um site para a OVHcloud](hosting_migrating_to_ovh1.){.external}?" irá fornecer-lhe instruções sobre como realizar este procedimento.
...
- Se continuar sem efetuar esta operação, o domínio será fornecido com uma nova zona DNS nos servidores DNS da OVHcloud. A [alteração manual da zona DNS](dns_zone_edit1.) pode ser necessária.
...
O processo de transferência pode ser reiniciado na [Área de Cliente OVHcloud](manager.){.external}. Selecione `Web Cloud`{.action} e aceda à secção `Nomes de domínio`{.action} e clique em `Operações em curso`{.action}.
...
Uma vez terminado o processo de transferência, pode gerir o seu domínio a partir da [Área de Cliente OVHcloud](manager.){.external}. Para isso, selecione a `Web Cloud`{.action}, clique em `Nomes de domínio`{.action} e, a seguir, clique no domínio em causa.
...
>
> Para isso, aceda à [Área de Cliente OVHcloud](manager.) e aceda à secção `Web Cloud`{.action}. Na coluna da esquerda, clique em `Nomes de domínio`{.action} e selecione o domínio em causa. Na página que aparece e imediatamente abaixo do seu nome de domínio, na parte superior da página, encontrará a data de renovação prevista com o **mês** e **ano** de expiração.
>
...
[Migração do seu website e dos seus e-mails para a OVHcloud](hosting_migrating_to_ovh1.){.external}
