...
> [!success]
>
> Per conoscere le condizioni tariffarie per il trasferimento di un dominio in base alla sua estensione, inserisci il dominio che vuoi trasferire sulla nostra pagina [www.ovhcloud.com/it/domains/tld/](domains-tld.) e segui gli step di questa guida.
>
...
|---|---|---|---|---|
|[1](transfer_incoming_generic_domain_#step1.)|[Verifica delle informazioni associate al dominio](transfer_incoming_generic_domain_#step1.)|L'amministratore del dominio|Il Registrar attuale|In base alle azioni effettuate|
|[2](transfer_incoming_generic_domain_#step2.)|[Sblocco del dominio e recupero del codice di trasferimento](transfer_incoming_generic_domain_#step2.)|L'amministratore del dominio, con l'autorizzazione del proprietario|Il Registrar attuale|In base alle azioni effettuate|
...
|[4](transfer_incoming_generic_domain_#step4.)|[Conferma del trasferimento](transfer_incoming_generic_domain_#step4.)|Il Registrar attuale|Tramite una richiesta da parte del Registro che gestisce l’estensione del dominio|Massimo cinque giorni|
...
**Come prima cosa, è importante accertarsi che i dati associati al dominio siano aggiornati.** Dall'entrata in vigore del GDPR, i dati visibili nel ["Whois"](domains-whois.) sono diventati molto limitati. Puoi consultare le informazioni relative al tuo dominio presso il tuo attuale Registrar.
...
> [!primary]
>
> Se non sai quale Registrar è responsabile del tuo dominio, la riga "Registrar", che compare nel risultato della ricerca del [tool Whois](domains-whois.){.external}, può fornirti informazioni sulla sua identità.
>
...
Una volta sbloccato il dominio e ottenuto il codice, è possibile ordinarne il trasferimento in OVHcloud dal [nostro sito](domains.). Inserisci il nome del tuo dominio e segui la procedura d’ordine.
...
È inoltre possibile completare l'ordine con un [hosting Web](hosting.){.external} e altre soluzioni OVHcloud. Per effettuare questa operazione è necessario migrare i servizi verso OVHcloud. Questa guida, intitolata "[Migrare un sito e un servizio di posta in OVHcloud](hosting_migrating_to_ovh1.)", fornisce alcune istruzioni su come procedere.
...
- Se continui senza effettuare questa operazione, il dominio verrà fornito sui server DNS OVHcloud con una nuova zona DNS. Una [modifica manuale della zona DNS](dns_zone_edit1.) può rendersi necessaria.
...
Una volta confermato l'ordine, riceverai un buono d'ordine. La procedura di trasferimento inizierà solo dopo aver ricevuto il pagamento. Una volta completata l'operazione, è possibile seguire lo stato di avanzamento del processo dallo [Spazio Cliente OVHcloud](manager.){.external}. Per monitorare lo stato di avanzamento dell'operazione, clicca su `Domini`{.action} e poi `Operazioni in corso`{.action}.
...
Il trasferimento può essere riavviato dallo [Spazio Cliente OVHcloud](manager.){.external}. Seleziona `Web Cloud`{.action} e accedi alla sezione `Domini`{.action}, poi clicca su `Operazioni in corso`{.action}.
...
Una volta completata la procedura, è possibile gestire il dominio dallo [Spazio Cliente OVHcloud](manager.){.external}. Seleziona `Web Cloud`{.action}, clicca su `Domini`{.action} nel menu a sinistra e poi clicca sul dominio interessato.
...
[Migrazione del tuo sito Web e delle tue email verso OVHcloud](hosting_migrating_to_ovh1.)
