...
W [Panelu klienta OVHcloud](manager.) przejdź do sekcji `Web Cloud`{.action}, a następnie nazwy `Domeny`{.action}. Kliknij `Operacje w toku`{.action} nad listą domen.

W tabeli możesz sprawdzić operacje związane z domenami w Twoim Panelu klienta.

![domena](domain-name-operations-header.png){.thumbnail}
...
![domena](domain-name-operations-error-creating-domain-name-with-registry.png){.thumbnail}
...
>
> - Sprawdź, czy jesteś na bieżąco w [płatnościach](invoice_management#pay-bills.) i [odnowieniach](how_to_use_automatic_renewal#renewal-management.) Twoich domen.
...
![domena](contacts-update-provide-us-with-the-documents-required.png){.thumbnail}
...
![domena](operation-data-provide-us-with-the-documents-required.png){.thumbnail}
...
![domena](domain-name-operations-complete-nic-admin-es-tld.png){.thumbnail}
...
![domena](operation-data-complete-nic-admin-es-tld.png){.thumbnail}
...
![domena](domain-name-operations-auth-code-missing.png){.thumbnail}

Kliknij przycisk `...`{.action} po prawej stronie operacji.<br>
Pojawi się poniższe okno, wprowadź kod transferu (**authInfo**) i uruchom ponownie operację.

![domena](operation-data-auth-code-missing.png){.thumbnail}
...
![domena](domain-name-operations-dns-update-unable-to-retrieve-dns-ip.png){.thumbnail}

W sekcji `Domeny`{.action} wybierz odpowiednią domenę, a następnie kliknij zakładkę `Serwery DNS`{.action}. W tej zakładce [zmień serwery DNS](dns_server_general_information1.). 
...
![domena](domain-name-operations-dns-update-error-occured-updating-domain.png){.thumbnail}
...
![domena](domain-name-operations-renewal-internal-error.png){.thumbnail}
