...
In your [OVHcloud Control Panel](manager.), go to the `Web Cloud`{.action} section, then `Domain names`{.action}. Click on `Ongoing operations`{.action} above the list of domain names.
...
>
> - Verify that you are up to date with [payments](invoice_management#pay-bills.) and domain name [renewals](how_to_use_automatic_renewal#renewal-management.).
...
In the `Domain names`{.action} section, select the domain name concerned, then click the `DNS servers`{.action} tab. In this tab, [edit your DNS servers](dns_server_general_information1.). 
...
[Transferring a domain name to OVHcloud](transfer_incoming_generic_domain1.)
