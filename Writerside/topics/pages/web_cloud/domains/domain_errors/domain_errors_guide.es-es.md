...
Acceda a la sección `Web Cloud`{.action} y seleccione `Dominios`{.action} en el [área de cliente de OVHcloud](manager.) . Haga clic en `Operaciones en curso`{.action} sobre la lista de dominios.
...
>
> - Compruebe que está actualizado en los [pagos](invoice_management#pay-bills.) y [renovaciones](how_to_use_automatic_renewal#renewal-management.) de sus dominios.
...
En la sección `Dominios`{.action}, seleccione el dominio correspondiente y abra la pestaña `Servidores DNS`{.action}. Desde esta pestaña, [modifique sus servidores DNS](dns_server_general_information1.). 
...
[Transferir un dominio a otro agente registrador](transfer_outgoing_domain1.)

[Cambiar los servidores DNS de un dominio de OVHcloud](dns_server_general_information1.)
