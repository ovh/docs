...
Para mais informações sobre estes temas, consulte a nossa documentação sobre [servidores DNS da OVHcloud](dns_server_general_information1.) e sobre [a edição de uma zona DNS da OVHcloud](dns_zone_edit1.).
...
> Recomendamos que siga os passos indicados abaixo ou, em caso de dúvida, que recorra a um [fornecedor especializado](partner.).
>
...
Se precisar de mais informações, consulte o nosso tutorial sobre a ferramenta [Zonemaster](dns_zonemaster1.)
...
Aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}. Na coluna da esquerda, clique em `Nomes de domínio`{.action} e escolha o domínio que vai utilizar para personalizar os nomes dos servidores DNS. 
...
Na imagem acima, ao retomar o exemplo do [etapa 1](glue_registry_#step1.), o « GLUE » que se deseja adicionar aqui (a partir do nome de domínio *domain.tld*) é **dns1.domain.tld**. 
...
- **O seu domínio utiliza uma zona DNS ativa na OVHcloud**: aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}. Na coluna da esquerda, clique em `Nomes de domínio`{.action} e selecione o domínio que utilizou para criar os « GLUE » durante a [etapa 2](glue_registry_#step2.). Aceda ao separador `Zona DNS`{.action} e clique em `Adicionar uma entrada`{.action}. Selecione a entrada do tipo *A* ou *AAAA* em função do tipo de IP associado que pretende adicionar. Siga os passos indicados para introduzir o *subdomínio* e o endereço *IPv4* (A) ou *IPv6* (AAAA). Se necessário, consulte as instruções descritas na nossa documentação « [Editar uma zona DNS da OVHcloud](dns_zone_edit1.) ».
...
Para isso, aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}. Na coluna da esquerda, clique em ‘Nomes de domínio‘ {.action} e selecione *o nome do domínio para o qual pretende personalizar os servidores DNS*.
...
Conclua os passos e, se necessário, consulte as instruções descritas na nossa documentação « [Alterar os servidores DNS de um nome de domínio OVHcloud](dns_server_general_information1.) ».
...
- **O seu domínio utiliza uma zona DNS ativa na OVHcloud**: aceda à [Área de Cliente OVHcloud](manager.){.external} e aceda à secção `Web Cloud`{.action}. Na coluna da esquerda, clique em `Nomes de domínios`{.action} e selecione o domínio para o qual personalizou os servidores DNS. Aceda ao separador `Zona DNS`{.action} e clique em `Editar em modo de texto`{.action}. 
...
> Contacte um [fornecedor especializado](partner.) em caso de dúvida.
>
...
A alteração não será imediatamente visível na [Área de Cliente OVHcloud](manager.){.external}. Aguarde cerca de 20 minutos e aceda à Área de Cliente OVHcloud para verificar se as alterações foram aplicadas corretamente.
...
Se necessário, consulte as instruções descritas na nossa documentação « [Editar uma zona DNS da OVHcloud](dns_zone_edit1.) ».
...
[Generalidades sobre os servidores DNS da OVHcloud](dns_server_general_information1.)
