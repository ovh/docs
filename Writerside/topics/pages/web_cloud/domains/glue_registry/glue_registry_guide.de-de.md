...
Weitere Informationen zu diesen Themen finden Sie in unserer [Dokumentation zu den OVHcloud DNS-Servern](dns_server_general_information1.) und der [Bearbeitung einer OVHcloud DNS-Zone](dns_zone_edit1.).
...
- Sie verfügen über einen bei OVHcloud registrierten [Domainnamen](domains.).
- Sie haben Zugriff auf Ihr [OVHcloud Kundencenter](manager.) mit den erforderlichen Berechtigungen zum Verwalten des Domainnamens (Domainadministrator).
...
> Folgen Sie genau den nachstehenden Anweisungen oder wenden Sie sich im Zweifelsfall an einen [spezialisierten Dienstleister](partner.).
>
...
Weitere Informationen finden Sie in unserem Tutorial zum Tool [Zonemaster](dns_zonemaster1.).
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.){.external} ein und gehen Sie in den Bereich `Web Cloud`{.action}. Klicken Sie in der linken Spalte auf `Domainnamen`{.action} und wählen Sie dann den Domainnamen aus, den Sie anpassen möchten.
...
In der Abbildung oben, die dem Beispiel aus [Schritt 1](glue_registry_#step1.) entspricht, ist der hinzuzufügende *Glue*-Eintrag **dns1.domain.tld** (für *domain.tld*). 
...
- **Ihr Domainname verwendet eine aktive DNS-Zone bei OVHcloud**: Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.){.external} ein und gehen Sie in den Bereich `Web Cloud`{.action}. Klicken Sie in der linken Spalte auf `Domainnamen`{.action} und wählen Sie dann den Domainnamen aus, mit dem Sie die *Glue*-Einträge im [Schritt 2](glue_registry_#step2.) erstellt haben. Gehen Sie auf den Tab `DNS-Zone`{.action} und klicken Sie dann auf `Eintrag hinzufügen`{.action}. Wählen Sie den Eintragstyp *A* oder *AAAA* aus, je nachdem, welchen IP-Typ Sie hinzufügen möchten. Folgen Sie den Anweisungen, indem Sie die *Subdomain* und die *IPv4* (A) oder *IPv6* (AAAA) eingeben und bis zur Bestätigung fortfahren. Falls erforderlich, folgen Sie den Anweisungen in unserer Dokumentation "[OVHcloud DNS-Zone bearbeiten](dns_zone_edit1.)".
...
Loggen Sie sich hierzu in Ihr [OVHcloud Kundencenter](manager.){.external} ein und gehen Sie in den Bereich `Web Cloud`{.action}. Klicken Sie in der linken Spalte auf `Domainnamen`{.action} und wählen Sie *den Domainnamn aus, für die Sie die DNS-Server anpassen möchten*.
...
Beenden Sie die Schritte und folgen Sie bei Bedarf den Anweisungen in unserer [Dokumentation zum Ändern der OVHcloud DNS-Server](dns_server_general_information1.)".
...
> Im Zweifelsfall wenden Sie sich an einen [spezialisierten Dienstleister](partner.).
>
...
Die Änderung wird nicht sofort im [OVHcloud Kundencenter](manager.){.external} sichtbar. Warten Sie etwa 20 Minuten und loggen Sie sich dann erneut in Ihrem OVHcloud Kundencenter ein, um zu sehen, ob Ihre Änderungen korrekt übernommen wurden.
...
Falls erforderlich, folgen Sie den Anweisungen in unserer Dokumentation "[OVHcloud DNS-Zone bearbeiten](dns_zone_edit1.)".
...
[Allgemeine Informationen zu den OVHcloud DNS-Servern](dns_server_general_information1.)
