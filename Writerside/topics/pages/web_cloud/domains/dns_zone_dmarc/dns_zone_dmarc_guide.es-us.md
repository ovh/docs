...
El registro **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**onformance (DMARC) es un mecanismo de seguridad del correo electrónico. Se basa en los resultados de las comprobaciones [SPF](dns_zone_spf1.) y [DKIM](dns_zone_dkim1.).
...
> 
> Esta guía le ayudará a realizar las operaciones más habituales. No obstante, si necesita ayuda, le recomendamos que contacte con un [proveedor especializado](partner.). Nosotros no podremos asistirle al respecto. Para más información, consulte la sección [«Más información»](dns_zone_dmarc_#go-further.) de este tutorial.
>

## Requisitos

- Tener acceso a la gestión del dominio (asociado a la solución de correo) desde el [área de cliente de OVHcloud](manager.).
- Uno de los mecanismos de autenticación, [SPF](dns_zone_spf1.) y/o [DKIM](dns_zone_dkim1.), debe configurarse en la zona DNS del dominio de la solución de correo.
...
DMARC consulta los mecanismos de autenticación [SPF](dns_zone_spf1.) y [DKIM](dns_zone_dkim1.) para comprobar los mensajes de correo electrónico entrantes.<br>
...
> [!warning]
>
> La configuración del parámetro `p=` puede afectar considerablemente a la entrega de los mensajes de correo electrónico del dominio. Es recomendable configurar `p=none` y realizar un análisis de los informes de fallos durante varias semanas para corregir cualquier anomalía. Pasar a `p=quarantine` o `p=reject` requiere un control total de los parámetros de seguridad del correo electrónico, relativos al [SPF](dns_zone_spf1.) y al [DKIM](dns_zone_dkim1.). El uso del factor `pct=`, que se muestra a continuación, permite realizar una transición gradual.
...
Puede añadir el registro TXT a su zona DNS desde el [área de cliente de OVHcloud](manager.) y accediendo a la sección `Web Cloud`{.action}. Haga clic en `Dominios`{.action} y seleccione el dominio correspondiente. A continuación, abra la pestaña `Zona DNS`{.action}.
...
>
> La configuración del parámetro `p=` puede afectar considerablemente a la entrega de los mensajes de correo electrónico del dominio. Se recomienda configurar `p=none` y realizar un análisis de los informes de errores durante varias semanas para corregir cualquier anomalía. Pasar a `p=quarantine` o `p=reject` requiere un control total de los parámetros de seguridad del correo electrónico, relativos al [SPF](dns_zone_spf1.) y al [DKIM](dns_zone_dkim1.). El uso del factor `pct=`, que se muestra a continuación, permite realizar una transición gradual.
...
En este segundo ejemplo, se ha utilizado un [registro TXT](dns_zone_dmarc_#txt-record.) para utilizar etiquetas que no están disponibles a través del [registro DMARC](#dmarc-record.) simplificado.
...
Para servicios especializados (posicionamiento web, desarrollo...), póngase en contacto con los [partners de OVHcloud](partner.).

Si necesita ayuda sobre el uso y la configuración de sus soluciones de OVHcloud, puede consultar nuestras diferentes [ofertas de soporte](support.).
