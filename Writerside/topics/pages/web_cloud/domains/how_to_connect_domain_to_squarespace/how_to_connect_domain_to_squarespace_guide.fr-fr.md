...
>
> - OVHcloud met à votre disposition des services dont la configuration, la gestion et la responsabilité vous incombent. Il vous revient de ce fait d'en assurer le bon fonctionnement.<br><br> Nous mettons à votre disposition ce guide afin de vous accompagner au mieux sur des tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](partner.) et/ou de contacter l'éditeur du service si vous éprouvez des difficultés. En effet, nous ne serons pas en mesure de vous fournir une assistance. Plus d'informations dans la section [Aller plus loin](how_to_connect_domain_to_squarespace_#go-further.) de ce guide.
>

## Prérequis

- Être connecté à votre [espace client OVHcloud](manager.){.external}.
- Disposer d'un [nom de domaine](domains.){.external} enregistré chez OVHcloud.
- Disposer des [autorisations appropriées pour gérer](managing_contacts1.) le nom de domaine depuis votre [espace client OVHcloud](manager.){.external}.
- Disposer d’un hébergement chez SquareSpace.
- Avoir accès à la gestion de cet hébergement chez SquareSpace.

## En pratique

Avant de suivre les deux étapes de ce guide, nous vous conseillons de vous familiariser avec la configuration d'une zone DNS à l'aide de notre guide « [Éditer une zone DNS OVHcloud](dns_zone_edit1.) ».
...
Connectez-vous à votre [espace client OVHcloud](manager.){.external} dans la section `Web Cloud`{.action}. Cliquez sur `Noms de domaine`{.action}, puis choisissez le nom de domaine concerné. Positionnez-vous enfin sur l'onglet `Zone DNS`{.action}.

Le tableau qui s'affiche liste l'ensemble des enregistrements DNS du nom de domaine sélectionné.

![dnszone](how_to_connect_domain_to_squarespace_images_tab.png){.thumbnail}
...
>> ![dnszone](images_filter-a.png){.thumbnail}<br>
...
>> ![dnszone](images_filter-aaaa.png){.thumbnail}<br>
...
>> ![dnszone](images_filter-txt.png){.thumbnail}<br>
...
>> ![dnszone](images_filter-cname.png){.thumbnail}
...
>> ![cname-entry](add-an-entry-to-the-dns-zone-cname-squarespace.png){.thumbnail}
...
Si vous utilisez une offre e-mail OVHcloud ou que vous prévoyez de souscrire à l'une de [nos offres e-mail](emails.), préparez votre zone DNS en conséquence. Consultez notre guide sur la « [Configuration d’un enregistrement MX](dns_zone_mx1.) ».

## Aller plus loin <a name="go-further"></a>

[Modifier les serveurs DNS d’un nom de domaine OVHcloud](dns_server_general_information1.)

[Créer une zone DNS OVHcloud pour un nom de domaine](dns_zone_create1.)

[Editer une zone DNS OVHcloud](dns_zone_edit1.)

Pour modifier la gestion de votre nom de domaine vers un autre compte client OVHcloud, suivez le guide « [Gérer les contacts de ses services](managing_contacts1.) ».
