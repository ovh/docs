...
Zapoznaj się z naszą dokumentacją dotyczącą [wpisów DNS i edycji strefy DNS](dns_zone_edit1.) dostępną w [Panelu klienta OVHcloud](manager.), jeśli chcesz dowiedzieć się więcej na ten temat.
...
- Strefa DNS dla Twojej domeny w [Panelu klienta OVHcloud](manager.)
- Dostęp do [panelu klienta OVHcloud](manager.)
- Dostęp do interfejsu zarządzania domeną

## W praktyce

Aby uzyskać dostęp do tej funkcji, zaloguj się do [Panelu klienta OVHcloud](manager.) i przejdź do sekcji `Web Cloud`{.action} w górnej części interfejsu. W kolumnie po lewej stronie przejdź do zakładki `Domeny`{.action}, następnie wybierz domenę powiązaną ze strefą DNS, którą chcesz zarządzać.
...
![Wyświetl strefę DNS](visualize-dns-eyes.png){.thumbnail}

Wyświetlą się dane odpowiedniej strefy DNS.

![Szczegóły strefy DNS](details-dns-zone.png){.thumbnail}
...
![Pobierz strefę DNS](download-dns-zone.png){.thumbnail}
...
![Przywróć strefę DNS](restore-dns-zone.png){.thumbnail}

Pojawi się następujące okno.

![Potwierdzenie przywrócenia strefy DNS](confirmation-restore-dns-zone.png){.thumbnail}
...
![Porównanie dwóch stref DNS](compare-two-dns-zone.png){.thumbnail}
...
![Porównanie dwóch stref DNS](compare-dns-zone-details.png){.thumbnail}
...
[Utworzenie strefy DNS w OVHcloud](dns_zone_create1.)
