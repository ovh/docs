...
- **Owner**: physical or legal person owning the domain name, sometimes called the **registrant**. They are constrained by [eligibility rules](api_domain_rules1.). They are legally responsible of the domain name.
...
It works in the same way as any other OVHcloud service. More details explaining the workflow, as well as how to do this, can be found on [this page](managing_contacts1.).
...
> [!primary]
>
> The update payload of a contact (PUT) must always satisfy [eligibility rules](api_domain_rules1.).
...
For the other extensions, the [eligibility rules](api_domain_rules1.) API will return each field status.
...
The following steps are described with more details in the [domain name order](api_domain_order1.) documentation page.
...
More details about task management can be seen [on this page](api_domain_tasks1.).
