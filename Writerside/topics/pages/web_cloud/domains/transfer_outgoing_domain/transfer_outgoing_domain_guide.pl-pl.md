...
>
> Aby przenieść zarządzanie domeną na inne konto klienta OVHcloud, należy **zmienić kontakt**. Procedurę opisano w [tym przewodniku](managing_contacts1.).
>
...
- Posiadanie [domeny zarejestrowanej](domains.) w OVHcloud
...
> [!primary]
>
> Jeśli jesteś **właścicielem** domeny, ale zarządzanie nią w Panelu klienta OVHcloud jest niedostępne, zarówno poprzez własny dostęp, jak i poprzez kontakt administracyjny domeny, zapoznaj się z [tym przewodnikiem](managing_contacts#przypadek-wlasciciela-domeny.).
>
...
> - Lista [TLD dostępnych w OVHcloud](domains-tld.);
...
Zaloguj się do [Panelu client OVHcloud](manager.) wybierz `Web Cloud`{.action}. Kliknij `Domeny`{.action}, po czym wybierz odpowiednią nazwę domeny.
...
![ochrona włączona](protection-against-domain-name-transfert-enabled.png){.thumbnail}
...
>
> Ponadto, jeśli pojawi się komunikat: "**AUTH/INFO code : Authcode is not managed by OVHcloud, contact the registry to claim it**", co oznacza, że kod transferu Twojej domeny nie będzie możliwy do pobrania za pośrednictwem [Panelu klienta OVHcloud](manager.).  
> 
> W obu przypadkach sprawdź, czy jesteś kontaktem **administrator** domeny korzystając z naszego przewodnika do [zarządzanie kontaktami](managing_contacts1.), a następnie sprawdź, czy rozszerzenie Twojej domeny pozwala na odblokowanie w [Panelu klienta OVHcloud](manager.).
> 
...
![dezaktywacja](protection-against-domain-name-transfert-deactivating.png){.thumbnail}
...
[Transfer domeny.co.uk wychodzący](transfer_outgoing_couk1.)
