...
- Disposer d'un [nom de domaine](domains.) enregistré chez OVHcloud
...
> [!primary]
>
> Si vous êtes le **propriétaire** du domaine mais que sa gestion dans l'espace client OVHcloud est indisponible, que ce soit via votre propre accès ou via le contact administratif du domaine, veuillez consulter [ce guide](managing_contacts1.) avant de continuer.
>
...
> - la [liste des TLD disponibles chez OVHcloud](domains-tld.);
...
Connectez-vous à votre [espace client OVHcloud](manager.) et sélectionnez `Web Cloud`{.action}. Cliquez sur `Noms de domaine`{.action}, puis choisissez le nom de domaine concerné.
...
![protection activée](protection-against-domain-name-transfert-enabled.png){.thumbnail}
...
>
> De plus, si vous rencontrez le message : "**AUTH/INFO code : Authcode is not managed by OVHcloud, contact the registry to claim it**", cela signifie que le code de transfert de votre nom de domaine n'est pas récupérable via votre [espace client OVHcloud](manager.).  
> 
> Dans les deux cas, vérifiez que vous êtes bien le contact **administrateur** du domaine à l'aide de notre guide sur la [gestion des contacts](managing_contacts1.) puis vérifiez que l'extension de votre domaine permet un déverrouillage depuis l'[espace client OVHcloud](manager.).
> 
...
![désactivation protection](protection-against-domain-name-transfert-deactivating.png){.thumbnail}
...
[Transférer un nom de domaine .co.uk](transfer_outgoing_couk1.)
