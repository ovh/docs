...
Se necessario, consulta la nostra documentazione su [i record DNS e la modifica di una zona DNS](dns_zone_edit1.) dal tuo [Spazio Cliente OVHcloud](manager.).
...
Per maggiori informazioni sui **server DNS**, consulta la nostra [guida](dns_server_general_information1.) sull'argomento.
...
Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e clicca su `Web cloud`{.action}. Nella colonna di sinistra, clicca su `Ordina`{.action} e seleziona `Zona DNS`{.action}.
...
Una volta completata la verifica, scegli se attivare il numero minimo di record per la zona DNS che vuoi creare. Questa scelta non è definitiva, perché in seguito sarà possibile [modificare i record della zona DNS](dns_zone_edit1.).

![dnszonecreate](adding-a-dns-zone-step-2.png){.thumbnail}

|Attivare i record minimi?|Dettaglio|
|---|---|
|Sì|Seleziona questa scelta se vuoi personalizzare in autonomia la zona DNS in seguito.</br>![minimale-entra](dashboard-minimal-entries.png){.thumbnail}|
|No|Seleziona questa opzione se intendi utilizzare servizi OVHcloud come [hosting web](hosting.){.external}, con la zona preconfigurata a tal fine.</br>![no-minimale-entra](dashboard-ovh-full-entries.png){.thumbnail}|
...
Per modificare questa zona DNS, consulta la nostra guida "[Modifica una zona DNS in OVHcloud](dns_zone_edit1.)".
...
Accedi allo [Spazio Cliente OVHcloud](manager.){.external} e clicca su `Web cloud`{.action}. Nella colonna di sinistra, clicca su `Domini`{.action} e seleziona la zona DNS interessata. 
...
>
> Se sei il contatto *Amministratore* del dominio, puoi modificare direttamente i **server DNS** utilizzando la nostra [guida](dns_server_general_information1.) sull'argomento.
>
...
[Modifica una zona DNS OVHcloud](dns_zone_edit1.)
