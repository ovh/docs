...
Para más información sobre los **servidores DNS**, consulte nuestra [guía](dns_server_general_information1.) sobre el asunto.
...
Conéctese a su [área de cliente de OVHcloud](manager.){.external} y acceda al apartado `Web cloud`{.action}. En la columna izquierda, haga clic en `Contratar`{.action} y, seguidamente, en el recuadro `Zona DNS`{.action}.
...
Una vez realizada la comprobación, seleccione si quiere activar los registros mínimos para la zona DNS que va a crear. Esta elección no es definitiva, ya que siempre podrá [editar los registros de la zona DNS](dns_zone_edit1.) después.

![dnszonecreate](adding-a-dns-zone-step-2.png){.thumbnail}

|Activar los registros mínimos ?|Detalles|
|---|---|
|Sí|Seleccione esta opción si desea personalizar usted mismo la zona DNS más adelante.</br>![mínimo-dns-entries](dashboard-minimal-entries.png){.thumbnail}|
|No|Seleccione esta opción si tiene previsto utilizar servicios de OVHcloud como un [alojamiento web](hosting.){.external}, ya que la zona está preconfigurada a tal efecto.</br>![no-mínimo-dns-entries](dashboard-ovh-full-entries.png){.thumbnail}|
...
Para editar esta zona DNS, consulte nuestra guía [Editar una zona DNS en OVHcloud](dns_zone_edit1.).
...
Para ello, conéctese al [área de cliente de OVHcloud](manager.){.external} y acceda al apartado `Web cloud`{.action}. En la columna izquierda, haga clic en `Dominios`{.action} y seleccione la zona DNS correspondiente. 
...
>
> Si es el contacto *Administrador* de este último, podrá en este caso cambiar directamente los **servidores DNS** con ayuda de nuestra [guía](dns_server_general_information1.) sobre el asunto.
>
