...
For more details on **DNS servers**, see [our guide](dns_server_general_information1.) on this topic.
...
Log in to your [OVHcloud Control Panel](manager.){.external} then go to the `Web Cloud`{.action} section. In the left-hand column, click on `Order`{.action} then on the `DNS zone`{.action} icon.
...
Once the verification is complete, choose whether to enable the minimum records for the DNS zone you are going to create. This choice is not definitive, as you can always [edit DNS zone records](dns_zone_edit1.) later.

![dnszonecreate](adding-a-dns-zone-step-2.png){.thumbnail}

|Enable minimal entries?|Details|
|---|---|
|Yes|Select this option if you would like to customise the DNS zone yourself at a later stage.</br>![minimal-dns-entries](dashboard-minimal-entries.png){.thumbnail}|
|No|Select this option if you plan to use OVHcloud services as a [web hosting](hosting.){.external}, with the zone preconfigured for this purpose.</br>![no-minimum-dns-entries](dashboard-ovh-full-entries.png){.thumbnail}|
...
To edit this DNS zone, please read our guide on [Editing a DNS zone at OVHcloud](dns_zone_edit1.).
...
Log in to your [OVHcloud Control Panel](manager.){.external} then go to the `Web cloud`{.action} section. In the left-hand column, click on `Domain names`{.action} then select the relevant DNS zone. It is named after your domain name and has a globe-shaped logo labelled *DNS* on its left.
...
>
> If you are the *Administrator* contact of the latter, you can directly change the **DNS servers** using [our guide](dns_server_general_information1.) on the subject.
>
