...
- Access to the [OVHcloud Control Panel](manager.) with the necessary permissions to manage the domain name
...
- **From the [OVHcloud Control Panel](manager.)**, where a configuration assistant can be used to set your redirection.
...
> Be careful about the changes you apply, or contact a [specialised provider](partner.) for SEO if necessary.
>
> Warning: A redirection created from the [OVHcloud Control Panel](manager.) does not redirect an URL as `https://` to another domain or URL.
> To create this type of redirection, you will have to apply [a URL rewrite](htaccess_url_rewriting_using_mod_rewrite1.), for example by editing a ".htaccess" file. 
>

### Redirecting a domain name from the Control Panel

Log in to the [OVHcloud Control Panel](manager.){.external}, go to the `Web Cloud`{.action} section, select the domain to redirect from `Domain names`{.action}, then click on the `Redirection`{.action} tab.
...
![Introduction to the redirection menu](add-a-redirection.png){.thumbnail}

Three redirection options are available from the [OVHcloud Control Panel](manager.), and each of them is made up of **5 successive steps**. 
...
>
> For more information on DNS records, see our [DNS zone documentation](dns_zone_edit1.).
>
...
>
> This guide is designed to help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](partner.) if you encounter any difficulties. We will not be able to assist you. You can find more information in the [Go further](redirect_domain_name_#go-further.) section of this guide.
>
...
A faulty .htaccess file can make your website inaccessible. If in doubt, contact a [specialist provider](partner.).

You can find all of our documentation on .htaccess in the [Go further](redirect_domain_name_#go-further.) section of this guide.
...
[URL rewriting using mod_rewrite](htaccess_url_rewriting_using_mod_rewrite1.).

[Using .htaccess for advanced purposes](htaccess_what_else_can_you_do1.).
