...
- **Z poziomu[Panelu klienta OVHcloud](manager.)**, gdzie asystent konfiguracji pozwala na ustawienie przekierowania.
...
> Bądź czujny nad operacjami, które zamierzasz wykonać lub skontaktuj się z [wyspecjalizowanym dostawcą](partner.) w celu pozycjonowania strony, jeśli to konieczne.
>
> Uwaga: przekierowanie utworzone w [Panelu klienta OVHcloud](manager.) nie pozwala na przekierowanie adresu URL na `https://` na inną domenę lub adres URL. 
> Aby utworzyć ten rodzaj przekierowania, należy obowiązkowo przejść przez [wpisanie adresu URL](htaccess_url_rewriting_using_mod_rewrite1.), na przykład przez plik ".htaccess".

### Przekieruj domenę w Panelu klienta

Zaloguj się do [Panelu klienta OVHcloud](manager.){.external}, przejdź do sekcji `Web Cloud`{.action}, wybierz domenę, którą chcesz przekierować do sekcji `Domeny`{.action}, następnie kliknij zakładkę `Przekierowanie`{.action}.
...
![Prezentacja przekierowania menu](add-a-redirection.png){.thumbnail}

Trzy opcje przekierowania są dostępne w [Panelu klienta OVHcloud](manager.). Każda z nich składa się z **5 kolejnych etapów**. 
...
>
> Więcej informacji na temat wpisów DNS znajdziesz w dokumentacji dotyczącej [rekordy DNS](dns_zone_edit1.).
>
...
>>
>> ![Etap 1](add-a-redirection-step-1.png){.thumbnail}
>>
...
>>
>> ![Etap 2](add-a-redirection-step-2-to-a-web-adress.png){.thumbnail}
>>
...
>>
>> ![Etap 3](add-a-redirection-step-3-a-visible-redirection.png){.thumbnail}
>>
...
>>
>> ![Etap 4](add-a-redirection-step-4-permanent.png){.thumbnail}
>>
...
>>
>> ![Etap 5](add-a-redirection-step-5-permanent.png){.thumbnail}
>>
...
>>
>> ![Etap 4](add-a-redirection-step-4-temporary.png){.thumbnail}
>>
>> Kliknij na `Dalej`{.action}, aby przejść do etapu 5.
>>
> **Etap 5**
...
>> W tym ostatnim etapie upewnij się, że wyświetlane informacje są poprawne.
>>
>> ![Etap 5](add-a-redirection-step-5-temporary.png){.thumbnail}
>>
>> Kliknij na `Potwierdź`{.action}, aby zatwierdzić Twoją konfigurację.
>> 
>> > [!primary]
...
>>
>> ![Etap 3](add-a-redirection-step-3-with-an-invisible-redirection.png){.thumbnail}
>>
>> Kliknij na `Dalej`{.action}, aby przejść do etapu 4.
>>
> **Etap 4**
...
>>
>> ![Etap 4](add-a-redirection-step-4-iframe.png){.thumbnail}
>>
...
>> W tym ostatnim etapie upewnij się, że wyświetlane informacje są poprawne.
>>
>> ![Etap 5](add-a-redirection-step-5-iframe.png){.thumbnail}
>>
>> Kliknij na `Potwierdź`{.action}, aby zatwierdzić Twoją konfigurację.
>> 
>> > [!primary]
...
Manipulowanie plikiem ".htaccess" może spowodować niedostępność Twojej strony WWW. W przypadku wątpliwości należy skontaktować się z [wyspecjalizowanym dostawcą](partner.).

Pełna dokumentacja dotycząca ".htaccess" znajduje się w sekcji ["Sprawdź również"](redirect_domain_name_#go-further.) niniejszego przewodnika.
...
[Zablokować dostęp do mojej strony dla niektórych adresów IP przez plik ".htaccess" ](htaccess_how_to_block_a_specific_ip_address_from_accessing_your_website1.).

[Chroń interfejs administracyjny swojej strony za pomocą ".htaccess" ](htaccess_protect_directory_by_password1.).

[Stworzenie adresów URL za pomocą mod_rewrite](htaccess_url_rewriting_using_mod_rewrite1.)

[Wykonywanie innych operacji za pomocą pliku ".htaccess" ](htaccess_what_else_can_you_do1.).

[Jak edytować strefę DNS?](dns_zone_edit1.)
