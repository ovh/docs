...
Para más información sobre los **servidores DNS** y sus modificaciones, consulte nuestra guía [Cambiar los servidores DNS de un dominio de OVHcloud](dns_server_general_information1.) .
...
> 
> - Si el dominio está registrado en OVHcloud, compruebe que utiliza nuestra configuración. Para ello, acceda al [área de cliente de OVHcloud](manager.){.external}, en la pestaña `Servidores DNS`{.action} del dominio correspondiente.
> 
...
> [!primary]
>
> A diferencia del nombre de dominio, para una zona DNS no existe el concepto de propietario, sino de gestión de contactos para una zona DNS de OVHcloud. Si desea cambiar la gestión de su zona DNS a otra cuenta de OVHcloud, siga nuestra guía [Gestionar contactos de servicio](managing_contacts1.).
...
![Zona DNS](dns_zone_edit_images_tab.png){.thumbnail}
...
![Zona DNS](add-an-entry.png){.thumbnail}
...
![Zona DNS](modify-record.png){.thumbnail}
...
![Zona DNS](delete-record.png){.thumbnail}
...
![Zona DNS](reset-my-dns-zone.png){.thumbnail}
...
[Añadir un registro SPF a la configuración del dominio](dns_zone_spf1.){.external}
