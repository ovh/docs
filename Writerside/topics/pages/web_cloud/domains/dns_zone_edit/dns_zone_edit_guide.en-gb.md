...
You can find information about **DNS servers** and how to change them in our guide on [Editing DNS servers for an OVHcloud](dns_server_general_information1.) domain name.

![DNS](images_dns-server.png){.thumbnail}
...
![DNS](dns-resolution.gif){.thumbnail}
...
![DNS](dns-zone-mydomain-ovh.png){.thumbnail}

**This guide explains how to edit your OVHcloud DNS zone via the OVHcloud Control Panel.**

## Requirements

- Appropriate rights to manage the domain name from your [OVHcloud Control Panel](manager.)
...
> 
> - If your domain name is registered with OVHcloud, you can check if it is using our configuration. To do this, go to your [Control Panel](manager.), and open the `DNS servers`{.action} tab of the domain name in question.
>
...
> [!primary]
>
> Unlike domain names, a DNS zone has no "owner", but is managed at OVHcloud through the contact management. If you wish to assign the management of your DNS zone to another OVHcloud customer account, follow our guide [Managing contacts for your services](managing_contacts1.).

Log in to the [OVHcloud Control Panel](manager.) and go to the `Web Cloud`{.action} section. Click `Domain names`{.action}, then choose the domain name concerned. Go to the `DNS zone`{.action} tab.
...
![dnszone](dns_zone_edit_images_tab.png){.thumbnail}
...
- **NS** (**N**ame **S**erver): Define the DNS servers associated with your DNS zone. For example, if the NS records in your DNS zone display the servers *dns19.ovh.net* and *ns19.ovh.net*, you will need to use them in the `DNS servers`{.action} tab in your OVHcloud Control Pane. Please refer to our guide on [Editing DNS servers for an OVHcloud domain name](dns_server_general_information1.) for more information.
...
- **SPF** (**S**ender **P**olicy **F**ramework): Avoids potential identity theft on email addresses using your domain name (spoofing). For example, the record `v=spf1 include:mx.ovh.com ~all` indicates that only outgoing servers linked to your OVHcloud mail service can be considered legitimate by the incoming server. You can enter this record as a TXT record, or via our automatic configuration system. For further information, please refer to our guide on [Adding an SPF record to your domain name’s configuration](dns_zone_spf1.).
...
![dnszone](add-an-entry.png){.thumbnail}
...
![dnszone](modify-record.png){.thumbnail}
...
![dnszone](delete-record.png){.thumbnail}
...
![dnszone](reset-my-dns-zone.png){.thumbnail}
...
[Editing the DNS servers for an OVHcloud domain name](dns_server_general_information1.)

[Adding a SPF record to your domain name’s configuration](dns_zone_spf1.)

[Securing your domain name with DNSSEC](dns_dnssec1.)
