...
- Dostęp do interfejsu zarządzania strefą DNS danej domeny w [Panelu klienta](manager.).
...
- Posiadanie konta e-mail MX Plan (zawartego w pakiecie [hostingowym](hosting.), [bezpłatnym hostingu 100M](domains-free-hosting.) lub w ofercie MX Plan zamówionej oddzielnie), jednej z naszych [ofert e-mail OVHcloud](emails.) lub zewnętrznej usługi e-mail.
...
>
> - Jeśli Twoja domena jest zarejestrowana w OVHcloud, możesz sprawdzić, czy używa ona konfiguracji OVHcloud w Twoim [Panelu klienta](manager.). Po wybraniu domeny w zakładce `Informacje ogólne`{.action} w sekcji `Serwery DNS`{.action}, jeśli pozycja `Aktywne` znajduje się pod adresem "**Serwery DNS**", korzystasz z serwerów DNS OVHcloud.
>
...
> W przypadku wątpliwości zalecamy skorzystanie z pomocy wyspecjalizowanego [usługodawcy](partner.).

### Wartości konfiguracji MX OVHcloud <a name="mxovhcloud"></a>

Zapoznaj się z przedstawioną poniżej konfiguracją MX OVHcloud przewidzianą dla rozwiązań MX Plan (występującą samodzielnie lub włączoną do oferty [hostingu WWW OVHcloud](hosting.)), [E-mail Pro](email-pro.) oraz [Exchange](emails.). Na serwerach poczty elektronicznej OVH zainstalowane jest oprogramowanie antyspamowe i antywirusowe.
...
Aby utworzyć lub zmodyfikować rekordy MX w konfiguracji OVHcloud Twojej domeny, zaloguj się do [Panelu klienta OVHcloud](manager.). Przejdź do sekcji `Nazwy domen`{.action}, kliknij odpowiednią domenę, następnie przejdź do zakładki `Strefa DNS`{.action}.
...
[Informacje na temat serwerów DNS.](dns_server_general_information1.)

[Edycja strefy DNS OVHcloud](dns_zone_edit1.)

[Konfiguracja rekordu SPF w domenie](dns_zone_spf1.)
