...
- Disposer d'un accès à la gestion de la zone DNS du nom de domaine concerné depuis l'[espace client OVHcloud](manager.).
...
- Disposer d'une offre MX Plan (incluse dans l'offre d’[hébergement web](hosting.), l'[hébergement gratuit 100M](domains-free-hosting.) ou l'offre MX Plan commandée séparément), une de nos [offres e-mail OVHcloud](emails.), ou un service e-mail externe.
...
>
> - Si votre nom de domaine est déposé chez OVHcloud, vous pouvez vérifier si ce dernier utilise notre configuration OVHcloud dans votre [espace client](manager.). Une fois positionné sur le domaine concerné, depuis l'onglet `informations générales`{.action}, dans la partie `Serveurs DNS`{.action}, si la mention `Actif` est présente sous « **serveurs DNS** », vous utilisez bien les serveurs DNS OVHcloud.
>
...
> En cas de doute, nous vous conseillons de faire appel à un [prestataire spécialisé](partner.).

### Valeurs de la configuration MX OVHcloud <a name="mxovhcloud"></a>

Retrouvez ci-dessous la configuration MX OVHcloud à utiliser pour nos solutions MX Plan (seule ou incluse dans une offre d’[hébergement web OVHcloud](hosting.)), [E-mail Pro](email-pro.) et [Exchange](emails.). Nos serveurs e-mail disposent d'un antispam et d'un antivirus intégré.
...
Pour créer ou modifier les enregistrements MX dans la configuration OVHcloud de votre nom de domaine, connectez-vous à votre [espace client OVHcloud](manager.). Rendez-vous dans la section `Noms de domaine`{.action}, cliquez sur le domaine concerné puis rendez-vous dans l'onglet `Zone DNS`{.action}.
...
[Généralités sur les serveurs DNS](dns_server_general_information1.)

[Éditer une zone DNS OVHcloud](dns_zone_edit1.)

[Configurer un enregistrement SPF sur son nom de domaine](dns_zone_spf1.)
