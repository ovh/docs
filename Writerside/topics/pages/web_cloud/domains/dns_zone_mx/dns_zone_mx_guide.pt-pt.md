...
- Ter acesso à gestão da zona DNS do domínio na [Área de Cliente OVHcloud](manager.).
...
- Dispor de uma oferta MX Plan (incluída na oferta de [alojamento web](hosting.), no [alojamento gratuito 100M](domains-free-hosting.) ou na oferta MX Plan encomendada separadamente), uma das nossas [ofertas de e-mail OVHcloud](emails.), ou um serviço de e-mail externo.
...
> Em caso de dúvida, sugerimos que recorra a um [fornecedor especializado](partner.).

### Valores da configuração MX da OVHcloud <a name="mxovhcloud"></a>

Consulte abaixo a configuração MX da OVHcloud que deve utilizar nas soluções MX Plan (só ou incluída numa oferta de [alojamento web da OVHcloud](hosting.)), [E-mail Pro](email-pro.) e [Exchange](emails.). Os nossos servidores de e-mail dispõem de um antisspam e antivírus integrado.
...
Para criar ou alterar os registos MX na configuração da OVHcloud do seu domínio, aceda à [Área de Cliente OVHcloud](manager.). Aceda à secção `Nomes de domínio`{.action}, clique no domínio que pretende e aceda ao separador `Zona DNS`{.action}.
...
[Alterar os servidores DNS de um nome de domínio OVH.](dns_server_general_information1.)

[Editar uma zona DNS da OVHcloud](dns_zone_edit1.)

[Configurar um registo SPF no domínio](dns_zone_spf1.)
...
Se pretender beneficiar de uma assistência ao uso e à configuração das suas soluções OVHcloud, sugerimos que consulte as nossas diferentes [ofertas de suporte](support.).
