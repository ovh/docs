...
- Tener acceso a la gestión de la zona DNS del dominio desde el [área de cliente de OVHcloud](manager.).
...
- Tener una solución MX Plan (incluida en el plan de [alojamiento web](hosting.), el [alojamiento gratuito 100M](domains-free-hosting.) o la solución MX Plan contratada por separado), una de nuestras [soluciones de correo de OVHcloud](emails.) o un servicio de correo externo.
...
>
> - Si el dominio está registrado en OVHcloud, compruebe que utiliza nuestra configuración en el [área de cliente de OVHcloud](manager.). Una vez que se haya registrado el dominio en cuestión, abra la pestaña `Información General`{.action}, en la sección `Servidores DNS`{.action}. Si la mención `Activo` aparece en «**Servidores DNS**», podrá utilizar los servidores DNS de OVHcloud.
>
> ![Correo electrónico](dns-servers-enabled.png){.thumbnail}
...
![Correo electrónico](mx-dns-resolution.png){.thumbnail}
...
> En caso de duda, le recomendamos que contacte con un [proveedor especializado](partner.).

### Valores de la configuración MX de OVHcloud <a name="mxovhcloud"></a>

A continuación ofrecemos la configuración MX de OVHcloud que deberá utilizar para nuestros MX Plan (solo o incluido en un plan de [hosting de OVHcloud](hosting.)), [Email Pro](email-pro.) y [Exchange](emails.). Nuestros servidores de correo disponen de antispam y antivirus integrado.
...
Para crear o modificar los registros MX en la configuración de su dominio en OVHcloud, conéctese al [área de cliente de OVHcloud](manager.). Acceda a la sección `Nombres de dominio`{.action}, haga clic en el dominio correspondiente y, seguidamente, abra la pestaña `Zona DNS`{.action}.
...
![Registro MX en la zona DNS](mx-entries-research.png){.thumbnail}
...
![Registro MX en la zona DNS](modify-a-dns-zone-record-mx-step-1.png){.thumbnail}
...
[Configurar un registro SPF en un dominio](dns_zone_spf1.)

[Configurar un registro DKIM](dns_zone_dkim1.)
