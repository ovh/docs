...
- Sie haben Zugriff auf Ihr [OVHcloud Kundencenter](manager.) mit den erforderlichen Berechtigungen zum Verwalten des Domainnamens.
- Der Domainname verwendet die OVHcloud Konfiguration (die OVHcloud DNS-Server).
- Sie verfügen über einen MX Plan (enthalten in einem [Webhosting](hosting.) oder [Kostenloses Hosting 100M](domains-free-hosting.) oder separat bestellt), einen unserer [OVHcloud E-Mail-Dienste](emails.) oder einen externen E-Mail-Dienst.
...
>
> ![E-Mail](dns-servers-enabled.png){.thumbnail}
...
![E-Mail](mx-dns-resolution.png){.thumbnail}
...
> Im Zweifelsfall empfehlen wir Ihnen, einen [spezialisierten Dienstleister](partner.) zu kontaktieren.

### Werte der OVHcloud MX-Konfiguration <a name="mxovhcloud"></a>

Nachfolgend finden Sie die Konfiguration für OVHcloud MX Plan (Standalone oder in einem [OVHcloud Webhosting](hosting.) enthalten), [E-Mail Pro](email-pro.) und [Exchange](emails.). Unsere E-Mail-Server verfügen über integrierte Antispam- und Antivirensoftware.
...
Um MX-Einträge in der OVHcloud Konfiguration Ihres Domainnamens zu erstellen oder zu bearbeiten, loggen Sie sich in Ihrem [OVHcloud Kundencenter](manager.) ein. Gehen Sie in den Bereich `Domainnamen`{.action}, klicken Sie auf den Domainnamen und anschließend in den Tab `DNS-Zone`{.action}.
...
[Allgemeine Informationen zu DNS-Servern](dns_server_general_information1.)
