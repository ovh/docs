...
- Disposer d'un accès à la gestion du nom de domaine concerné depuis l'[espace client OVHcloud](manager.) ou auprès de votre prestataire de domaine s'il est enregistré en dehors d'OVHcloud.
- Être connecté à votre [espace client OVHcloud](manager.).
- Avoir souscrit une de nos offres [Exchange](emails.), [E-mail Pro](email-pro.) ou une offre e-mail hors OVHcloud disposant du DKIM.
...
>
> Si votre nom de domaine est déposé chez OVHcloud, vous pouvez vérifier si ce dernier utilise notre configuration OVHcloud dans votre [espace client](manager.) depuis l'onglet `Zone DNS`{.action}, une fois le domaine concerné sélectionné.
>
...
Depuis la plateforme e-mail, le DKIM va utiliser le hachage pour créer une signature à partir de certains éléments de [l'en-tête de l'e-mail](diagnostic_headers1.) et du corps de l'e-mail (contenu de l'e-mail).
...
Pour que le destinataire puisse vérifier la signature DKIM de l'expéditeur, il aura besoin des paramètres DKIM et surtout de la clé publique pour la déchiffrer. La [zone DNS](dns_zone_edit1.) d'un nom de domaine est publique, c'est pourquoi un enregistrement DNS est ajouté pour transmettre la clé publique et les paramètres DKIM au destinataire.
...
La configuration automatique du DKIM est accessible pour les offres e-mail [Exchange](emails.) et [E-mail Pro](email-pro.).
...
> **Exchange**
>>
>> Depuis votre [espace client OVHcloud](manager.), dans l'onglet `Web Cloud`{.action}, cliquez sur `Microsoft`{.action}, puis sur `Exchange`{.action}. Cliquez sur le nom du service Exchange concerné. Enfin, allez dans l'onglet `Domaines associés`{.action}.
>>
...
> **E-mail Pro**
>>
>> Depuis votre [espace client OVHcloud](manager.), dans l'onglet `Web Cloud`{.action}, cliquez sur `E-mails Pro`{.action}, puis sur le nom du service E-mail Pro concerné. Enfin, allez dans l'onglet `Domaines associés`{.action}.
>>
>> À droite du nom de domaine concerné, vous pouvez observer que la pastille `DKIM` est grise.
>>
>> ![email](dkim-auto01.png){.thumbnail}
...
> **Exchange**
>>
>> Depuis votre [espace client OVHcloud](manager.), dans l'onglet `Web Cloud`{.action}, cliquez sur `Microsoft`{.action}, puis sur `Exchange`{.action}. Cliquez enfin sur le nom du service Exchange concerné. Par défaut, le nom de votre plateforme correspond à sa référence ou celle-ci sera visible sous le nom que vous lui avez attribué (voir l'image ci-dessous).
>>
>> ![email](dns-dkim-platform-exchange.png){.thumbnail}
>>
> **E-mail Pro**
>>
>> Depuis votre [espace client OVHcloud](manager.), dans l'onglet `Web Cloud`{.action}, cliquez sur `E-mails Pro`{.action}, puis sur le nom du service E-mail Pro concerné. Par défaut, le nom de votre plateforme correspond à sa référence ou celle-ci sera visible sous le nom que vous lui avez attribué (voir l'image ci-dessous).
>>
...
> Appuyez-vous sur notre guide [« Découvrez comment utiliser les API OVHcloud »](first-steps1.) si vous n'avez jamais utilisé les API.
...
>> Depuis [l'espace client OVHcloud](manager.) où est hébergé le nom de domaine de votre plateforme Exchange, dans l'onglet `Web Cloud`{.action}, cliquez sur `Noms de domaine`{.action} dans la colonne de gauche et sélectionnez le nom de domaine concerné.<br>
...
>>
> **4.Configurer l'enregistrement DNS**
>> Depuis [l'espace client OVHcloud](manager.) où est hébergé le nom de domaine de votre plateforme E-mail Pro, dans l'onglet `Web Cloud`{.action}, cliquez sur `Noms de domaine`{.action} dans la colonne de gauche et sélectionnez le nom de domaine concerné.<br>
>> Dirigez-vous vers l'onglet `Zone DNS`{.action} puis cliquez sur `Ajouter une entrée`{.action} dans la fenêtre qui s'affiche. Choisissez `CNAME` puis complétez selon les valeurs que vous avez relevées.<br>
>>
>> Si on prend les valeurs de l'exemple à l'étape "**3.Récupérer l'enregistrement DNS**":
>>
...
Depuis [l'espace client OVHcloud](manager.), cliquez sur l'onglet `Web Cloud`{.action} puis sur `Noms de domaine`{.action} dans la colonne de gauche et sélectionnez le nom de domaine concerné.
...
Pour récupérer l'en-tête d'un e-mail, consulter notre guide « [Récupérer l'en-tête d'un e-mail](diagnostic_headers1.) ».
...
> **E-mail Pro**
>>
>> Depuis votre [espace client OVHcloud](manager.), dans l'onglet `Web Cloud`{.action}, cliquez sur `E-mails Pro`{.action} puis sur le nom du service E-mail Pro concerné.<br><br> Dans la rubrique `Domaines associés`{.action}, vérifiez la couleur de l'icône `DKIM` à droite du nom de domaine concerné (voir l'image ci-dessous).
>>
>> ![email](red-dkim.png){.thumbnail}
>>
