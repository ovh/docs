...
- Access to manage the domain name concerned in the [OVHcloud Control Panel](manager.), or via your DNS service provider if it is registered outside of OVHcloud
- Access to the [OVHcloud Control Panel](manager.)
- An OVHcloud [Exchange](emails.), OVHcloud [Email Pro](email-pro.) or external email solution with DKIM
...
>
> If your domain name is registered with OVHcloud, you can check if it is using the OVHcloud configuration in your [Control Panel](manager.). To do this, go to the `DNS Zone`{.action} tab, and select the domain concerned.
>
...
From the email platform, DKIM will use hashing to create a signature from certain elements of [the email header](diagnostic_headers1.) and email body (email content).
...
In order for a recipient to verify the sender's DKIM signature, they will need the DKIM parameters and especially the public key to decrypt it. A domain name’s [DNS zone](dns_zone_edit1.) is public, which is why a DNS record is added to transmit the public key and DKIM settings to the recipient.
...
For this rotation principle to work, we're going to use something called **DKIM selectors**. A DKIM selector includes a private/public key pair. It is visible as a character string in the DKIM signature of an email after the argument `s=`. This signature is visible in [the email header](diagnostic_headers1.).
...
The automatic configuration of DKIM is accessible for the email solutions [Exchange](emails.) and [Email Pro](email-pro.).
...
> **Exchange**
>>
>> From your [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Microsoft`{.action}, then `Exchange`{.action}. Click on the name of the Exchange service concerned. Finally, go to the `Associated domains`{.action} tab.
>>
...
> **Email Pro**
>>
>> From your [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Email Pro`{.action}, then on the name of the Email Pro service concerned. Finally, go to the `Associated domains`{.action} tab.
>>
>> To the right of the domain name concerned, you can see that the `DKIM` box is gray.
>>
>>![email](dkim-auto01.png){.thumbnail}
...
> **Exchange**
>>
>> In the [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Microsoft`{.action}, then `Exchange`{.action}. Next, click on the name of the Exchange service concerned. By default, your platform name will match its reference number, or it will be visible under the name you have given it (see image below).
>>
>> ![email](dns-dkim-platform-exchange.png){.thumbnail}
>>
> **Email Pro**
>>
>> In the [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Email Pro`{.action}, then click the name of the Email Pro service concerned. By default, your platform name will match its reference number, or it will be visible under the name you have given it (see image below).
>>
...
> Visit our guide ["First Steps with the OVHcloud APIs"](first-steps1.) if you have never used APIs.
...
>> > [!primary]
>> >
>> > If your domain name is managed in the same OVHcloud customer account as your platform, and you have ticked `autoEnableDKIM` and `configureDkim`, go directly to the section [**The different states of DKIM**](#dkim-status.) below to track DKIM activation.
>>
...
>> Log in to the [OVHcloud Control Panel](manager.). In the `Web Cloud`{.action} section, click on `Domain names`{.action} in the left column and select the relevant domain name.<br>
...
>> > [!primary]
>> >
>> > If your domain name is managed in the same OVHcloud customer account as your platform, and you have ticked `autoEnableDKIM` and `configureDkim`, go directly to the [**The different states of DKIM**](#dkim-status.) section below to track DKIM activation.
>>
> **3. Retrieve the DNS record**
>> You must manually configure your domain name’s DNS zone **in the following cases**:
>>
...
>>
> **4. Configure the DNS record**
>> Log in to the [OVHcloud Control Panel](manager.). In the `Web Cloud`{.action} section, click on `Domain names`{.action} in the left column and select the relevant domain name of your Email Pro platform.<br>
>> Go to the `DNS Zone`{.action} tab, then click on `Add an entry`{.action}. In the window that appears, choose `CNAME`, then enter the information according to the values retrieved previously.<br>
>>
>> If we take the values of the example in step **3. Retrieve the DNS record**:
>>
...
Log in to the [OVHcloud Control Panel](manager.). In the `Web Cloud`{.action} section, click on `Domain names`{.action} in the left column and select the domain name concerned.
...
To retrieve the header of an email, please read our guide on [Retrieving email headers](diagnostic_headers1.).
...
> **Exchange**
>>
>> From your [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Microsoft`{.action} then on `Exchange`{.action}. Finally, click on the name of the Exchange service concerned.<br><br> In the `Associated domains`{.action} section, check the colour of the `DKIM` icon to the right of the domain name concerned (see the image below).
>>
>>![email](red-dkim.png){.thumbnail}
>>
> **Email Pro**
>>
>> From your [OVHcloud Control Panel](manager.), in the `Web Cloud`{.action} tab, click `Email Pro`{.action}, then on the name of the Email Pro service concerned.<br><br> In the `Associated domains`{.action} section, check the colour of the `DKIM` icon to the right of the domain name concerned (see the image below).
>>
>>![email](red-dkim.png){.thumbnail}
