...
- Sie haben über das [OVHcloud Kundencenter](manager.) Zugriff auf die Konfiguration des betreffenden Domainnamens oder entsprechenden Verwaltungszugriff bei Ihrem DNS-Anbieter, wenn der Domainname nicht über OVHcloud registriert ist.
- Sie haben Zugriff auf Ihr [OVHcloud Kundencenter](manager.).
- Sie nutzen OVHcloud [Exchange](emails.), OVHcloud [E-Mail Pro](email-pro.) oder ein externes E-Mail-Angebot mit DKIM.
...
>
> Wenn Ihr Domainname bei OVHcloud registriert ist, können Sie im [Kundencenter](manager.) überprüfen, ob OVHcloud DNS-Server genutzt werden, indem Sie den Domainnamen auswählen und dann den Tab `DNS-Zone`{.action} öffnen.
>
...
Für E-Mail-Dienste nutzt DKIM das Hashing-Verfahren, um eine Signatur aus einigen Elementen des [E-Mail-Headers](diagnostic_headers1.) sowie des Inhalts der E-Mail (Body) zu erstellen.
...
Damit der Empfänger die DKIM-Signatur des Absenders überprüfen kann, benötigt er die DKIM-Parameter, insbesondere den öffentlichen Schlüssel, um die Signatur zu entschlüsseln. Die [DNS-Zone](dns_zone_edit1.) eines Domainnamens ist öffentlich verfügbar, deshalb wird, um den öffentlichen Schlüssel und die DKIM-Parameter an den Empfänger weiterzuleiten, ein DNS-Eintrag verwendet.
...
![E-Mail](dns-dkim-send.gif){.thumbnail}
...
![E-Mail](dns-dkim-receive.gif){.thumbnail}

### DKIM automatisch für OVHcloud Exchange oder E-Mail Pro konfigurieren <a name="auto-dkim"></a>

Die automatische Konfiguration des DKIM ist für die E-Mail-Angebote [Exchange](emails.) und [E-Mail Pro](email-pro.) verfügbar.
...
> **Exchange**
>>
>> Klicken Sie in Ihrem [OVHcloud Kundencenter](manager.) im Tab `Web Cloud`{.action} auf `Microsoft`{.action} und dann auf `Exchange`{.action}. Klicken Sie dann auf den Namen des betreffenden Dienstes. Der Dienstname ist identisch mit der Referenz oder wird unter dem individuellen Namen angezeigt, den Sie diesem Dienst gegeben haben (siehe Abbildung).
>>
>> ![E-Mail](dns-dkim-platform-exchange.png){.thumbnail}
>>
> **E-Mail Pro**
>>
>> Klicken Sie in Ihrem [OVHcloud Kundencenter](manager.) im Tab `Web Cloud`{.action} auf `E-Mail Pro`{.action} und anschließendauf den Namen des betreffenden Dienstes. Der Dienstname ist identisch mit der Referenz oder wird unter dem individuellen Namen angezeigt, den Sie diesem Dienst gegeben haben (siehe Abbildung).
>>
>> ![E-Mail](dns-dkim-platform-emailpro.png){.thumbnail}
...
![E-Mail](dns-dkim-domain.png){.thumbnail}
...
> Lesen Sie unsere Anleitung zur [Verwendung der API](first-steps1.), wenn Sie damit noch nicht vertraut sind.
...
![E-Mail](dns-dkim-api01.png){.thumbnail}
...
>> Klicken Sie in Ihrem [OVHcloud Kundencenter](manager.) im Bereich `Web Cloud`{.action} in der linken Spalte auf `Domainnamen`{.action} und wählen Sie den entsprechenden Domainnamen aus.<br>
...
>>
>> ![E-Mail](dns-dkim-api02.png){.thumbnail} <br>
>> 
...
Klicken Sie in Ihrem [OVHcloud Kundencenter](manager.) im Bereich `Web Cloud`{.action} in der linken Spalte auf `Domainnamen`{.action} und wählen Sie den entsprechenden Domainnamen aus.
...
![E-Mail](dns-dkim-add.png){.thumbnail}
...
Um den Header einer E-Mail einzusehen, lesen Sie unsere Anleitung "[E-Mail-Header extrahieren](diagnostic_headers1.)".
