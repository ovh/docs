...
- Posiadanie dostępu do interfejsu zarządzania domeną w [Panelu klienta OVHcloud](manager.) lub u operatora domeny, jeśli jest zarejestrowany poza OVHcloud.
- Dostęp do [Panelu klienta OVHcloud](manager.).
- Wykupienie jednej z naszych ofert [Exchange](emails.), [E-mail Pro](email-pro.) lub usługi e-mail poza OVHcloud z DKIM
...
>
> Jeśli Twoja domena jest zarejestrowana w OVHcloud, możesz sprawdzić, czy używa ona konfiguracji OVHcloud w [Panelu klienta](manager.) w zakładce `Strefa DNS`{.action}, po wybraniu domeny.
>
...
W ramach platformy e-mail DKIM będzie używać hakowania do tworzenia podpisu [z określonych elementów nagłówka e-maila](diagnostic_headers1.) i treści e-maila (zawartość e-maila).
...
Aby zasada rotacji działała, użyjemy tzw. **selektorów DKIM**. Wybieracz DKIM zawiera parę klucza prywatnego/klucza publicznego. Jest on widoczny w postaci łańcucha znaków w podpisze DKIM w wiadomości e-mail przez argument `s=`. Podpis ten jest widoczny [w nagłówku wiadomości e-mail](diagnostic_headers1.).
...
Automatyczna konfiguracja DKIM jest dostępna dla ofert e-mail [Exchange](emails-hosted-exchange.) i [E-mail Pro](email-pro.).
...
> **Exchange**
>>
>> W [Panelu klienta OVHcloud](manager.) w zakładce `Web Cloud`{.action} kliknij `Microsoft`{.action}, a następnie `Exchange`{.action}. Następnie kliknij nazwę odpowiedniej usługi Exchange. Domyślnie nazwa Twojej platformy odpowiada jej nazwie lub będzie widoczna pod przypisaną jej nazwą (patrz obrazek poniżej).
>>
>> ![email](dns-dkim-platform-exchange.png){.thumbnail}
>>
> **E-mail Pro**
>>
>> W [Panelu klienta OVHcloud](manager.) w zakładce `Web Cloud`{.action} kliknij `E-mail Pro`{.action}, a następnie nazwę odpowiedniej usługi E-mail Pro. Domyślnie nazwa Twojej platformy odpowiada jej nazwie lub będzie widoczna pod przypisaną jej nazwą (patrz obrazek poniżej).
>>
...
> Skorzystaj z naszego przewodnika ["Dowiedz się, jak korzystać z API OVHcloud"](first-steps1.), jeśli nigdy wcześniej nie korzystałeś z API.
...
>> Z [strefy klienta OVHcloud](manager.), gdzie nazwa Twojej domeny Exchange, w zakładce `Web Cloud`{.action} kliknij `Nazwy domen`{.action} w lewej kolumnie i wybierz odpowiednią nazwę domeny.<br>
...
>>
> **4.Konfiguracja rekordu DNS**
>> Z [strefy klienta OVHcloud](manager.), gdzie nazwa Twojej domeny Platforma E-mail Pro, w zakładce `Web Cloud`{.action} kliknij `Nazwy domen`{.action} w lewej kolumnie i wybierz odpowiednią nazwę domeny.<br>
>> Przejdź do zakładki `Strefa DNS`{.action}, następnie kliknij `Dodaj rekord`{.action} w oknie, które się wyświetli. Wybierz `CNAME` i uzupełnij zgodnie z podanymi przez Ciebie wartościami.<br>
>>
>> Jeśli weźmiemy wartości z przykładu na etapie "**3.Pobierz rekord DNS**":
>>
...
W [Panelu klienta OVHcloud](manager.) kliknij zakładkę `Web Cloud`{.action}, następnie `Domeny`{.action} w kolumnie z lewej strony i wybierz odpowiednią nazwę domeny.
...
Aby pobrać nagłówek wiadomości e-mail, zapoznaj się z naszym przewodnikiem "[Pobierz nagłówek wiadomości](diagnostic_headers1.)".
