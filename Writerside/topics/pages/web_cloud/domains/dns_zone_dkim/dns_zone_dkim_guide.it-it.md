...
- Avere accesso alla gestione del dominio dallo [Spazio Cliente OVHcloud](manager.) o dal provider del tuo dominio registrato al di fuori di OVHcloud
- Avere accesso allo [Spazio Cliente OVHcloud](manager.)
- Aver sottoscritto una delle nostre soluzioni [Exchange](emails.), [Email Pro](email-pro.) o una soluzione email extra OVHcloud con DKIM
...
>
> Se il tuo dominio è registrato presso OVHcloud, verifica che utilizzi la configurazione OVHcloud nel tuo [Spazio Cliente](manager.) utilizzando la scheda `Zona DNS`{.action}.
>
...
Dalla piattaforma email, il DKIM utilizzerà l'hash per creare una firma a partire da alcuni elementi [dell'intestazione dell'email](diagnostic_headers1.) e del corpo dell'email (contenuto dell'email).
...
Affinché il destinatario possa verificare la firma DKIM del mittente, avrà bisogno dei parametri DKIM e soprattutto della chiave pubblica per decifrarla. La [zona DNS](dns_zone_edit1.) di un dominio è pubblica, motivo per cui è stato aggiunto un record DNS per trasmettere la chiave pubblica e i parametri DKIM al destinatario.
...
La configurazione automatica di DKIM è accessibile per le offerte di posta elettronica [Exchange](emails.) e [Email Pro](email-pro.).
...
> **Exchange**
>>
>> Dallo [Spazio Cliente OVHcloud](manager.), nella scheda `Web Cloud`{.action}, clicca su `Microsoft`{.action} e poi su `Exchange`{.action}. Infine clicca sul nome del servizio Exchange interessato. Di default, il nome della tua piattaforma corrisponde al suo riferimento o sarà visibile con il nome che le hai assegnato (vedi l'immagine qui sotto).
>>
>> ![email](dns-dkim-platform-exchange.png){.thumbnail}
>>
> **Email Pro**
>>
>> Dallo [Spazio Cliente OVHcloud](manager.), nella scheda `Web Cloud`{.action}, clicca su `Email Pro`{.action} e poi sul nome del servizio Email Pro interessato. Di default, il nome della tua piattaforma corrisponde al suo riferimento o sarà visibile con il nome che le hai assegnato (vedi l'immagine qui sotto).
>>
...
> Clicca sulla nostra guida ["Scopri come utilizzare le API OVHcloud"](first-steps1.) se non hai mai utilizzato le API.
...
>> Da [l'area clienti OVHcloud](manager.) dove il nome del dominio del tuo Piattaforma Exchange, nella scheda `Web Cloud`{.action}, fai clic su `Nomi di dominio`{.action} nella colonna di sinistra e seleziona il nome di dominio pertinente.<br>
...
>>
> **4.Configura il record DNS**
>> Da [l'area clienti OVHcloud](manager.) dove il nome del dominio del tuo Piattaforma E-mail Pro, nella scheda `Web Cloud`{.action}, fai clic su `Nomi di dominio`{.action} nella colonna di sinistra e seleziona il nome di dominio pertinente.<br>
>> Clicca sulla scheda `Zona DNS`{.action} e poi su `Aggiungi un record`{.action} nella finestra che appare. Scegli `CNAME` e completa in base ai valori rilevati.<br>
>>
>> Se prendiamo i valori dell'esempio nello step "**3.Recupera il record DNS**":
>>
...
Dallo [Spazio Cliente OVHcloud](manager.), clicca sulla scheda `Web Cloud`{.action} e poi su `Domini`{.action} nella colonna di sinistra.
...
Per recuperare l'intestazione di un'email, consulta la guida [Recuperare l'intestazione di un'email](diagnostic_headers1.).
