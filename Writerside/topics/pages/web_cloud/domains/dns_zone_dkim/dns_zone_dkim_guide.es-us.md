...
- Tener acceso a la gestión del dominio desde el [área de cliente de OVHcloud](manager.) o desde su proveedor de servicios si está registrado fuera de OVHcloud.
- Haber iniciado sesión en el [área de cliente de OVHcloud](manager.).
- Haber contratado una de nuestras soluciones [Exchange](emails.).
...
>
> Si el dominio está registrado en OVHcloud, puede comprobar si utiliza nuestra configuración de OVHcloud desde el [área de cliente](manager.), en la pestaña `Zona DNS`{.action}, después de seleccionar el dominio correspondiente.
>
...
Desde la plataforma de correo, DKIM utilizará el hackeo para crear una firma a partir de algunos elementos de [la cabecera del email](diagnostic_headers1.) y del cuerpo del email (contenido del email).
...
Para que el destinatario pueda verificar la firma DKIM del remitente, necesitará los parámetros DKIM y, sobre todo, la clave pública para descifrarla. La [zona DNS](dns_zone_edit1.) de un dominio es pública, por lo que se añade un registro DNS para transmitir la clave pública y los parámetros DKIM al destinatario.
...
Para que funcione este principio de rotación, usaremos lo que se llama **selector DKIM**. Un selector DKIM incluye un par de claves públicas o privadas. Es visible como una cadena de caracteres en la firma DKIM de un correo electrónico con el argumento `s=`. Puede consultar esta firma en [la cabecera del email](diagnostic_headers1.).
...
![Correo electrónico](dns-dkim-send.gif){.thumbnail}
...
![Correo electrónico](dns-dkim-receive.gif){.thumbnail}

### Configurar el DKIM automáticamente para una solución de correo Exchange de OVHcloud <a name="auto-dkim"></a>

La configuración automática del DKIM está disponible para los servicios de correo [Exchange](emails.).
...
Desde su [área de cliente de OVHcloud](manager.), en la pestaña `Web Cloud`{.action}, haga clic en `Microsoft`{.action} y seleccione `Exchange`{.action}. Haga clic en el nombre del servicio Exchange correspondiente. Por último, abra la pestaña `Dominios asociados`{.action}.
...
![Correo electrónico](dns-dkim-domain.png){.thumbnail}
...
> Si nunca ha utilizado la API, puede consultar nuestra guía ["Cómo utilizar las API de OVHcloud"](first-steps1.).
...
![Correo electrónico](dns-dkim-api01.png){.thumbnail}
...
>> Desde [el área de clientes de OVHcloud](manager.) donde el nombre de dominio de su Exchange, en la pestaña `Web Cloud`{.action}, haga clic en `Domain name`{.action} en la columna de la izquierda y seleccione el nombre de dominio correspondiente.<br>
...
>>
>> ![Correo electrónico](dns-dkim-api02.png){.thumbnail} <br>
>> 
...
En [el área de cliente de OVHcloud](manager.), abra la pestaña `Web Cloud`{.action} y haga clic en `Dominios`{.action} en la columna izquierda y seleccione el dominio correspondiente.
...
![Correo electrónico](dns-dkim-add.png){.thumbnail}
...
Para consultar la cabecera de un mensaje de correo electrónico, consulte nuestra guía [Obtener la cabecera de un email](diagnostic_headers1.).
