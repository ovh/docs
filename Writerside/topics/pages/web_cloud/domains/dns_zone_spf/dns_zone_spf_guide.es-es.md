...
- Tener acceso a la gestión del dominio desde el [área de cliente de OVHcloud](manager.){.external}.
...
>
> Si el dominio está registrado en OVHcloud, puede comprobar si utiliza nuestra configuración de OVHcloud desde el [área de cliente](manager.){.external}, en la pestaña `Servidores DNS`{.action}, seleccionando el dominio correspondiente.
>
...
Si el dominio utiliza la configuración de OVHcloud, puede comprobar si ya hay un registro SPF configurado para este último. Para ello, conéctese al [área de cliente de OVHcloud](manager.){.external}. En la sección `Dominios`{.action}, seleccione el dominio correspondiente. Haga clic en la pestaña `Zona DNS`{.action}.
...
![Dominio](spf_records_check_OVH_configuration.png){.thumbnail}

### Configurar el registro SPF

Para añadir un registro SPF a la configuración de OVHcloud de su dominio, conéctese al [área de cliente de OVHcloud](manager.){.external}.Haga clic en `Dominios`{.action} y seleccione el dominio correspondiente. A continuación, abra la pestaña `Zona DNS`{.action}.
...
- [Añadir un registro SPF](#spfrecordovhcloud.)**y utilizar la configuración de OVHcloud**: para los usuarios que solo posean los productos de correo electrónico de OVHcloud en su dominio (excepto [Private Exchange](emails-hosted-exchange.){.external} y Exchange Provider).
...
- Solo MX Plan o incluido en un plan de [hosting de OVHcloud](hosting.){.external} .
- [Email Pro](email-pro.).
...
Para modificar el SPF en la configuración de OVHcloud de su dominio, conéctese al [área de cliente de OVHcloud](manager.){.external}.Haga clic en `Dominios`{.action} y seleccione el dominio correspondiente. A continuación, abra la pestaña `Zona DNS`{.action}.
...
- Solo MX Plan o incluido en un plan de [hosting de OVHcloud](hosting.).
- [Email Pro](email-pro.).
- [Hosted Exchange](emails-hosted-exchange.)
...
[Cambiar los servidores DNS de un dominio de OVHcloud](dns_server_general_information1.){.external}.
