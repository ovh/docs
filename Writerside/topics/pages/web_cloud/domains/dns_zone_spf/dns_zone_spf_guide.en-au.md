...
>
> ![domain](spf_records_diagram.png){.thumbnail}
...
- You have access to manage the domain name from your [OVHcloud Control Panel](manager.).
...
>
> If your domain name is registered with OVHcloud, you can check if it is using the OVHcloud configuration in your [Control Panel](manager.). Click on the `DNS servers`{.action} tab of your domain name.
>
...
If your domain uses OVHcloud DNS servers, you can check if an SPF record is already configured for it. Log in to the [OVHcloud Control Panel](manager.) and switch to `Web Cloud`{.action} in the top navigation bar. In the `Domain names`{.action} section, select the domain name concerned. Click on the `DNS zone`{.action} tab.
...
![domain](spf_records_check_OVH_configuration_ca.png){.thumbnail}

### Configuring a new SPF record

Log in to the [OVHcloud Control Panel](manager.) and switch to `Web Cloud`{.action} in the top navigation bar. In the `Domain names`{.action} section, select the domain name concerned. Click on the `DNS zone`{.action} tab.

To add an SPF record, click on `Add an entry`{.action} in the right-hand menu.

![domain](spf_records_add_entry_step1.png){.thumbnail}
...
- [Add an OVHcloud SPF record](#spfrecordovhcloud.) **and use the OVHcloud configuration**: For users who only have OVHcloud email offers on their domain name (excluding [Private Exchange](emails-hosted-exchange.){.external}).
...
![domain](spf_records_add_entry.png){.thumbnail}
...
- MX Plan standalone or included in an OVHcloud [web hosting plan](hosting.){.external}
...
![domain](spf_records_add_entry_step2_ca.png){.thumbnail}
...
![domain](spf_records_add_entry_personnalize_step1_ca.png){.thumbnail}
...
![domain](spf_records_add_TXT_entry_ca.png){.thumbnail}
...
- MX Plan standalone or included in an OVHcloud [web hosting plan](hosting.)
- [Hosted Exchange](emails-hosted-exchange.)
...
>
> ![domain](spf_records_ip.png){.thumbnail}
