...
>
> ![domena](spf_records_diagram.png){.thumbnail}
...
>
> Jeśli Twoja domena jest zarejestrowana w OVHcloud, możesz sprawdzić w [Panelu klienta](manager.){.external}, czy używa ona konfiguracji OVHcloud w zakładce `Serwery DNS`{.action}.
>
...
Jeśli Twoja domena używa konfiguracji OVHcloud, możesz sprawdzić, czy rekord SPF jest już skonfigurowany dla tej domeny. W tym celu zaloguj się do [Panelu klienta OVHcloud](manager.){.external}. W sekcji `Domeny`{.action} wybierz odpowiednią domenę. Kliknij zakładkę `Strefa DNS`{.action}.
...
![domena](spf_records_check_OVH_configuration.png){.thumbnail}

### Konfiguracja rekordu SPF

Aby dodać rekord SPF w konfiguracji OVHcloud Twojej domeny, zaloguj się do [Panelu klienta OVHcloud](manager.){.external}. Kliknij `Domeny`{.action}, następnie wybierz domenę i przejdź do zakładki `Strefa DNS`{.action}.

Aby dodać rekord SPF, kliknij przycisk `Dodaj rekord`{.action}.

![domena](spf_records_add_entry_step1.png){.thumbnail}

W oknie, które się wyświetli, pojawi się kilka rekordów DNS. Jeśli chcesz dodać SPF, masz dwie możliwości:

- [Dodaj rekord SPF OVHcloud](#spfrecordovhcloud.)**i użyj konfiguracji OVHcloud**: dla użytkowników posiadających tylko oferty e-mail OVHcloud dla ich domeny (poza [Private Exchange](emails-hosted-exchange.){.external}).
...
![domena](spf_records_add_entry.png){.thumbnail}
...
- MX Plan w monoterapii lub w cenie [hostingu OVHcloud](hosting.){.external}.
...
![domena](spf_records_add_entry_step2.png){.thumbnail}
...
![domena](spf_records_add_entry_personnalize_step1.png){.thumbnail}
...
![domena](spf_records_add_TXT_entry.png){.thumbnail}

### Zmień rekord SPF

Aby zmienić SPF w konfiguracji OVHcloud Twojej domeny, zaloguj się do [Panelu klienta OVHcloud](manager.){.external}. Na pasku usług, w sekcji `Domeny`{.action}, kliknij wybraną domenę, następnie przejdź do zakładki `Strefa DNS`{.action}.
...
- Usługa MX Plan w monoterapii lub w [cenie hostingu](hosting.).
- [E-mail Pro](email-pro.).
- [Hosted Exchange](emails-hosted-exchange.).
...
>
> ![domena](spf_records_ip.png){.thumbnail}
>
...
>
> ![domena](spf_records_ip2.png){.thumbnail}
>
...
>
> ![domena](spf_records_ip3.png){.thumbnail}
>
...
[Zmiana serwerów DNS domeny OVHcloud](dns_server_general_information1.){.external}.
