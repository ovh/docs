...
O processo de mudança de agente de registo (*registar*) para os domínios de primeiro nível (*top-level domain*, ou **TLD**) do indicativo de país **UK** (**.uk**) difere do que é detalhado no nosso [guia de transferência dos TLD genéricos](transfer_outgoing_domain1.). As seguintes instruções dizem respeito às seguintes extensões:
...
- Ter um [domínio .uk](domains.) registado na OVHcloud
- Ter acesso à [Área de Cliente OVHcloud](manager.) e ter os direitos necessários para gerir o domínio (ser administrador do domínio)
...
![transferência de saída](outgoing-transfer-tag.jpg){.thumbnail}

Na nova janela, introduza a TAG do novo agente de registo e clique em `Confirmar`{.action}.

![transferência de saída](outgoing-transfer-tag-confirmation.jpg){.thumbnail}
