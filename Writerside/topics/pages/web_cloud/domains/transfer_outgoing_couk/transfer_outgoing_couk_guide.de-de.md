...
Der Transferprozess für Top Level Domains (TLDs) des Ländercodes **UK** (**.uk**) unterscheidet sich von dem, der in unserer [Anleitung zu generischen TLDs](transfer_outgoing_domain1.) erklärt wird. Die nachfolgenden Anweisungen betreffen diese Endungen:
...
- Sie verfügen über einen bei OVHcloud registrierten [.uk-Domainnamen](domains.).
- Sie haben Zugriff auf Ihr [OVHcloud Kundencenter](manager.) mit den erforderlichen Berechtigungen zum Verwalten der Domain (Domainadministrator).
...
>
> Wenn Sie der **Inhaber** der Domain sind, deren Verwaltung Ihnen im OVHcloud Kundencenter jedoch nicht möglich ist, weder über Ihren eigenen Zugang noch den Administrator-Kontakt der Domain, konsultieren Sie bitte [diese Anleitung](managing_contacts#sonderfall-bei-domaininhabern.), bevor Sie fortfahren.
>
...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](manager.) ein und wählen Sie im Bereich `Web Cloud`{.action} Ihren Domainnamen unter `Domainnamen`{.action} aus.
...
![ausgehender Transfer](outgoing-transfer-tag.jpg){.thumbnail}

Geben Sie im neuen Fenster den TAG Ihres neuen Registrars ein und klicken Sie dann auf `Bestätigen`{.action}.

![ausgehender Transfer](outgoing-transfer-tag-confirmation.jpg){.thumbnail}
...
[Einen Domainnamen zu einem anderen Registrar transferieren](transfer_outgoing_domain1.)
