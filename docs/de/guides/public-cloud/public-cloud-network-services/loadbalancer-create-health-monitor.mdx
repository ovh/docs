...
- A previously configured [Load Balancer](/guides/public-cloud/public-cloud-network-services/getting-started-load-balancer) within your OVHcloud project
...
- **Monitoring**: Use OVHcloud monitoring tools to track the performance and health status of your Load Balancer and adjust the configuration as needed. See [Public Cloud Load Balancer monitoring with Prometheus](/guides/public-cloud/public-cloud-network-services/loadbalancer-monitoring-prometheus).
...
<strong>2.</strong> Navigate to the `Public Cloud` section and select your project.<br />
<strong>3.</strong> Click on `Load Balancer` in the left menu and select the Load Balancer you want to configure.<br />
<strong>4.</strong> Go to the `Pools` tab, then click on the Pool for which you want to create a Health Monitor.

![Schema 1](/images/guides/public-cloud/public-cloud-network-services/technical-resources-03-lbaas-create-health-monitor/healthM1.png)

<strong>5.</strong> Click on the `Health Monitors` tab within the Pool details, then on `Add a Health Monitor`.

![Schema 1](/images/guides/public-cloud/public-cloud-network-services/technical-resources-03-lbaas-create-health-monitor/healthM2.png)
...
<strong>6.</strong> Once all the information is filled in, click on `Add` to activate your Health Monitor.

![Schema 1](/images/guides/public-cloud/public-cloud-network-services/technical-resources-03-lbaas-create-health-monitor/healthM3.png)
...
Choose the Load Balancer you want to configure and click on the `Health Monitors` tab.

Click on `Create Health Monitor` and fill in the required fields such as `type`, `check interval`, `maximum retry count`, and `timeout`.

Confirm the creation by clicking on `Create`.
