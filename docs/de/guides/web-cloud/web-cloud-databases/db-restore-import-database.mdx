...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](/links/manager) ein und klicken Sie im Bereich `Web Cloud` auf `Web Cloud Databases`. Wählen Sie den Datenbanknamen aus und gehen Sie dann auf den Tab `Datenbanken`.
...
Klicken Sie auf `...` rechts neben der Datenbank und dann auf `Die Sicherungen anzeigen`.

Die Liste der verfügbaren Backups erscheint. Klicken Sie auf den Button `...` rechts neben dem ausgewählten Backup und dann auf `Sicherung wiederherstellen`.
...
Klicken Sie auf den Button `...`. rechts neben der Datenbank und dann auf `Datei importieren`.
...
Klicken Sie anschließend auf **Neue Datei importieren** und dann auf `Weiter`.

Geben Sie einen Namen für Ihre importierte Datei ein, klicken Sie auf `Durchsuchen`, um diese auszuwählen, bestätigen mit `Absenden` und klicken dann auf `Weiter`.
...
Haken Sie, wenn Sie möchten, **Aktuelle Datenbank leeren** und **E-Mail am Ende des Imports senden** vor dem Import an, um über den Abschluss der Operation auf der E-Mail-Adresse Ihres OVHcloud Kunden-Accounts informiert zu werden, und klicken Sie dann auf `Bestätigen`.
...
Wählen Sie dann die Datei im Drop-down-Menü aus und klicken Sie auf `Weiter`.
...
Um Ihre Datenbank direkt über phpMyAdmin zu importieren, müssen Sie sich zuerst mit ihr verbinden. Folgen Sie hierzu den Schritten im Abschnitt ["Mit einer MySQL oder MariaDB Datenbank verbinden"](/guides/web-cloud/web-cloud-databases/db-connecting-database-server#mit-einer-mysql-oder-mariadb-datenbank-verbinden).

Wenn Sie in phpMyAdmin eingeloggt sind, wählen Sie die Datenbank aus, indem Sie auf ihren Namen klicken.

Klicken Sie anschließend auf den Tab `Importieren`.

Wählen Sie Ihre Backup-Datei aus, indem Sie `Durchsuchen` klicken. (Die Datei darf 100 MB nicht überschreiten.)
...
Lassen Sie die Standardoptionen aktiviert und klicken Sie auf `Ausführen`, um den Import zu starten.
...
Diese Aktion ist nur über [SSH](/guides/web-cloud/web-hosting/ssh-on-webhosting) von einem [OVHcloud Webhosting](/links/web/hosting) aus möglich.
...
> [!warning]
>
> - Um zu vermeiden, dass jemand auf diese Datei mit sensiblen Daten zugreift, können Sie sie [entsprechend absichern](/guides/web-cloud/web-hosting/htaccess-protect-directory-by-password).
...
Diese Aktion ist nur über [SSH](/guides/web-cloud/web-hosting/ssh-on-webhosting) von einem [OVHcloud Webhosting](/links/web/hosting) aus möglich (ab Version "stable").
