...
Loggen Sie sich in Ihr [OVHcloud Kundencenter](/links/manager) ein und klicken Sie im Bereich `Web Cloud` auf `Web Cloud Databases`. Wählen Sie den Datenbanknamen aus und wechseln Sie zum Tab `Datenbanken`.
...
Klicken Sie auf `...` rechts neben der Datenbank und dann auf `Jetzt sichern`.
...
Die Liste der verfügbaren Backups erscheint. Klicken Sie auf `...` rechts neben dem ausgewählten Backup und dann auf `Sicherung herunterladen`, um die Datei anzufordern.
...
Wenn Sie in phpMyAdmin eingeloggt sind, klicken Sie auf den Namen der Datenbank, die Sie exportieren möchten, und anschließend auf den Tab `Exportieren`.
...
[Datenbank auf Ihrem Datenbankserver wiederherstellen und importieren](/guides/web-cloud/web-cloud-databases/db-restore-import-database)
