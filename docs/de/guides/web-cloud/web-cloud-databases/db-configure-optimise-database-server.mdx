...
Gehen Sie In Ihrem [OVHcloud Kundencenter](/links/manager) in den Bereich `Web Cloud Databases` und klicken Sie anschließend auf die entsprechende SQL-Instanz. Vergewissern Sie sich, dass Sie sich im Tab `Allgemeine Informationen` befinden.
...
![Allgemeine Informationen](/images/assets/screens/control-panel/product-selection/web-cloud/web-cloud-databases/general-information/general-information.png)
...
Gehen Sie In Ihrem [OVHcloud Kundencenter](/links/manager) in den Bereich `Web Cloud Databases` und klicken Sie anschließend auf die entsprechende Web Cloud Databases Instanz.

Klicken Sie hierzu im Tab `Autorisierte IPs` auf den Button `IP-Adresse / Maske hinzufügen`.
...
#### Ihren Web Cloud Databases Dienst ändern <a id="modify-ram-web-cloud-db"></a>
...
>
> Um einen Web Cloud Databases Dienst vom Webhosting **Performance** abzutrennen, lesen Sie unsere Anleitung "[Web Cloud Databases von einem Webhosting abtrennen](/guides/web-cloud/web-cloud-databases/db-detach-from-web-hosting)".
>
...
Um Ihren Web Cloud Databases Dienst zu bearbeiten, melden Sie sich in Ihrem [OVHcloud Kundencenter](/links/manager) an. Klicken Sie auf `Web Cloud` und dann auf `Web Cloud Databases`. Wählen Sie den Namen Ihres Datenbankservers aus.

In der Standardansicht **Allgemeine Informationen** klicken Sie auf `...` rechts neben "RAM" und dann auf `RAM Menge ändern`, um zur Bestellung geleitet zu werden.
...
Wählen Sie die gewünschte RAM-Größe aus und klicken Sie auf `Weiter`. Danach können Sie die Laufzeit wählen.
...
Loggen Sie sich in Ihrem OVHcloud Kundencenter ein Klicken Sie auf `Web Cloud` und dann auf `Web Cloud Databases`. und wählen Sie den Namen Ihres Web Cloud Databases Servers aus. 

#### MySQL und MariaDB Instanzen

- Gehen Sie zum Tab `Konfiguration`.

Im Kasten **Allgemeine Konfiguration von MySQL** finden Sie die derzeit für Ihre Datenbank festgelegte Konfiguration. Sie können hier direkt Änderungen durchführen und dann auf `Anwenden` klicken.
...
Im Feld **Allgemeine Konfiguration von PostgreSQL** finden Sie die derzeit für Ihre Datenbank festgelegte Konfiguration. Sie können diese direkt bearbeiten und dann auf `Anwenden` klicken.
...
Außerdem können Sie Erweiterungen für Ihre Datenbanken aktivieren. Klicken Sie hierzu auf den Tab `Datenbanken` und dann auf das Tabellensymbol der betreffenden Datenbank in der Spalte **Erweiterungen**.
...
Loggen Sie sich in Ihrem OVHcloud Kundencenter ein Klicken Sie auf `Web Cloud` und dann auf `Web Cloud Databases`. Wählen Sie den Namen Ihres Datenbankservers aus.
