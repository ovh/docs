...
Sie können den TXT-Eintrag über das [OVHcloud Kundencenter](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.de/&ovhSubsidiary=de) im Bereich <code className="action">Web Cloud</code> zu Ihrer DNS-Zone hinzufügen. Klicken Sie auf <code className="action">Domainnamen</code> und wählen Sie die betreffende Domain aus. Gehen Sie dann auf den Tab <code className="action">DNS-Zone</code>.
