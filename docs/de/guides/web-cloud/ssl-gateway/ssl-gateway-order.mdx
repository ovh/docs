...
Wählen Sie Ihr Angebot aus und klicken Sie auf `Jetzt bestellen`.

![order ssl gateway](/images/assets/screens/website/order/configure-my-ssl-gateway.png)

Geben Sie auf der neuen Seite im Formular `Domainnamen suchen` den betreffende Domainnamen oder Subdomain ein und klicken Sie dann auf das Lupensymbol rechts.
...
Falls gewünscht und bei der Bestellung verfügbar, aktivieren Sie die Option `Ich verwalte die DNS-Zone dieser Domain und autorisiere OVHcloud, den erforderlichen DNS-Eintrag automatisch zu ändern`. Die Ihrem Domainnamen oder Ihrer Subdomain zugeordnete DNS Zone wird dann automatisch mit der IP-Adresse des SSL Gateways aktualisiert.
...
:::details Fall 1: Ihre DNS Zone wird von den Shared DNS Servern von OVHcloud verwaltet.
...
:::details Fall 2: Ihre DNS Zone wird nicht von den Shared DNS Servern von OVHcloud verwaltet.
