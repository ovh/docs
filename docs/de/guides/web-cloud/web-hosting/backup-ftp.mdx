...
Um den Link zur Wiederherstellung zu erstellen, loggen Sie sich in Ihrem [OVHcloud Kundencenter](/links/manager) ein. Gehen Sie in den Bereich `Web Cloud`, klicken Sie auf `Hosting-Pakete` und dann auf das betreffende Hosting. 

Wählen Sie den Tab `FTP - SSH` aus und klicken Sie rechts auf `Backup erstellen`.

![backupftpcw](/images/assets/screens/control-panel/product-selection/web-cloud/cloud-web/ftp-ssh/generate-a-backup.png)

Wählen Sie im neu angezeigten Fenster eines der verfügbaren Backups aus und klicken Sie dann auf `Weiter`.
...
Klicken Sie auf `Bestätigen`, um Ihre Anfrage zu bestätigen.
