...
- `Administrator username`: log in to your [OVHcloud Control Panel](/links/manager), then go to the `Web Cloud` section. Select the web hosting plan concerned, and click the `1-click modules` tab. In the table that pops up, identify the row that corresponds to your 1-click module. Your admin name is in the `Login` column.
