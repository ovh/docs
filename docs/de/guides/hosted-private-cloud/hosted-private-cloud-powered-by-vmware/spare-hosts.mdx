...
Standardmäßig ist der Dienst HA ([High Availability](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/vmware-ha-high-availability/)) von VMware für Ihren Cluster aktiviert. Wenn Sie Ihn aktiviert lassen, werden Ihre virtuellen Maschinen automatisch neu gestartet. Ist der Dienst DRS (Distributed Ressources Scheduler) aktiviert und im Modus „fully automated“, so findet die Lastverteilung zwischen den Hosts Ihres Clusters ebenfalls automatisch statt.
...
Wir empfehlen Ihnen, uns den Original-Host zurückzugeben, damit wir ihn nach dieser Störung einer Reihe von Tests unterziehen (und so zukünftige Störungen vermeiden) können. Sie können dann den Ersatz-Host behalten. Dazu können Sie die Anleitung [Host-Server löschen](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/delete-host/) zu Rate ziehen.
