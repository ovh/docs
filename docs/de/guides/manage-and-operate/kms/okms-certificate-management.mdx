...
- An [OVHcloud KMS ordered](/guides/manage-and-operate/kms/kms-quick-start)
...
For that, log in to the [OVHcloud Control Panel](/links/manager) and access the administration console `Identity, Security & Operations`. Click on `Key Management Service`, then on `Generate an access certificate`.
