...
> [!info]
>
> Se non hai ancora creato progetti Public Cloud, inizia con la nostra [guida sulla creazione di progetti](/guides/public-cloud/public-cloud-cross-functional/create-a-public-cloud-project).
>
> **I dettagli tecnici** importanti relativi al Public Cloud di OVHcloud sono disponibili su [questa pagina](/guides/public-cloud/public-cloud-cross-functional/compute-essential-information).
>
...
Il [protocollo SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) permette una comunicazione client-server crittografata. Una **coppia di chiavi SSH** è composta da una chiave pubblica e da una chiave privata.
...
La maggior parte dei sistemi operativi desktop contemporanei include nativamente il client **OpenSSH** accessibile tramite l'applicazione da riga di comando del sistema (`cmd`, `Powershell`, `Terminal`, ecc...). Se non hai familiarità con l'utilizzo delle chiavi SSH come metodo di autenticazione, puoi utilizzare le istruzioni di [questa guida](/guides/public-cloud/compute/compute-creating-ssh-keys#create-ssh-key) per creare la tua coppia di chiavi.

Se si utilizza un altro software, consultare la relativa documentazione utente. Le istruzioni per la soluzione open source `PuTTY` sono disponibili in [questa guida](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows).

<a id="import-ssh"></a>

### Step 2: Importa le chiavi SSH

Le chiavi SSH pubbliche possono essere archiviate nella sezione `Public Cloud` dello [Spazio Cliente OVHcloud](/links/manager). Non è obbligatorio, ma rende più pratico il processo di creazione dell'istanza.

> [!info]
>
> Le chiavi SSH salvate ti permettono di creare le tue istanze più velocemente nello Spazio Cliente. Per modificare le coppie di chiavi e aggiungere utenti dopo aver creato l'istanza, consulta la guida su [chiavi SSH supplementari](/guides/public-cloud/compute/configuring-additional-ssh-keys).
>
...
Apri `Chiavi SSH` nel menu a sinistra sotto **Impostazioni**. Clicca sul pulsante `Aggiungi una chiave SSH`.

![ssh keys](/images/assets/screens/control-panel/product-selection/public-cloud/cp-pci-sshkeys.png)

Nella nuova finestra, immettere un nome per la chiave. Compila il campo `Chiave` con la stringa di chiave pubblica, ad esempio quella creata allo [step 1](#create-ssh). Conferma cliccando su `Aggiungi`.
...
- Se l'istanza deve essere connessa a una nuova rete privata (OVHcloud [vRack](/links/network/vrack)), **crea la tua vRack** prima di continuare. Per maggiori dettagli consulta la [guida sulla vRack Public Cloud](/guides/public-cloud/public-cloud-network-services/public-cloud-network-services-vrack).
...
Per maggiori informazioni, consulta le nostre guide disponibili nella sezione [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts). La [guida ai concetti](/guides/public-cloud/public-cloud-network-services/concepts) fornisce un'introduzione al Public Cloud Networking.
...
Nella pagina "**Home**", clicca su `Crea un'istanza`.

![istanza creazione](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation01.png)
...
![istanza model](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation02.png)

Nella sezione `Discovery`, proponiamo modelli di istanze con risorse condivise a prezzi vantaggiosi. Ideali per testare il Public Cloud in generale o un'applicazione Web ad esempio.

I modelli di istanza di tipo `Metal` forniscono risorse fisiche dedicate.

> [!info]
>
> Le risorse Public Cloud inizialmente saranno limitate per motivi di controllo dei costi e di sicurezza. È possibile verificare queste quote cliccando su `Quota e Region` nella barra di navigazione a sinistra sotto **Impostazioni**. Per ulteriori informazioni, consultare [la documentazione dedicata](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota).
>
...
Le Local Zones sono un'estensione delle **Region** che avvicinano i servizi OVHcloud a siti specifici, offrendo latenza ridotta e performance migliorate per le applicazioni. Per maggiori informazioni, consulta la [pagina Web delle Local Zones](/links/public-cloud/local-zones) e la [documentazione delle capacità delle Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).
...
Consulta anche le informazioni della [pagina Web des Local Zones](/links/public-cloud/local-zones) e della [documentazione delle capacità des Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).

![selezione della regione](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation03.png)
...
<Tabs>
<Tab label="Utilizza chiave archiviata">
...
<Tab label="Inserisci direttamente la chiave">

Per aggiungere una chiave pubblica incollando la stringa di chiave, clicca sul pulsante `Aggiungi una chiave`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation06.png)<br />
Immettere un nome per la chiave e la stringa di chiave nei rispettivi campi. Clicca su `Seguente`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation07.png)<br />
Prima di cliccare su `Avanti`, è possibile scegliere di utilizzare il pulsante `Aggiungi chiave` per salvare la chiave nello Spazio Cliente OVHcloud (per maggiori informazioni, vedere lo [step 2](#import-ssh)).
...
![istanza select](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation08.png)

Questo step offre diverse opzioni di configurazione. Fare clic sulle schede seguenti per visualizzare i dettagli:

<Tabs>
<Tab label="1: Numero di istanze da creare">

È possibile creare più istanze in base alle selezioni effettuate durante le fasi di creazione, ma [i limiti di quota delle risorse](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota) si applicheranno.<br />
  </Tab>
<Tab label="2: Istanza flessibile">
...
<Tab label="3: Nome dell'istanza">

Inserisci un nome completo per la tua istanza. Il riferimento aziendale del modello di istanza è il valore predefinito.<br />
  </Tab>
<Tab label="4: Script di post-installazione">

Potete aggiungere [il vostro script](/guides/public-cloud/compute/launching-script-when-creating-instance) a questo campo.<br />
  </Tab>
<Tab label="5: Backup automatico delle istanze">

È possibile attivare [backup automatizzati](/guides/public-cloud/compute/save-an-instance) selezionando questa opzione. Ti preghiamo di prendere visione delle informazioni tariffarie e dei dettagli complementari.
...
Se clicchi su `Crea una nuova rete privata`, il processo di creazione dell'istanza verrà interrotto e dovrà essere riavviato dall'inizio.<br />
  </Tab>
...
Se clicchi su `Crea una nuova rete privata`, il processo di creazione dell'istanza verrà interrotto e dovrà essere riavviato dall'inizio.
...
![modalità di fatturazione](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation14.png)
...
La fatturazione oraria è la scelta migliore se non si è certi della durata del periodo di utilizzo. Se si decide di mantenere l'istanza per un utilizzo a lungo termine, è comunque possibile [passare a un abbonamento mensile](/guides/account-and-service-management/managing-billing-payments-and-services/changing-hourly-monthly-billing).<br />
...
Una volta terminata la configurazione dell'istanza, clicca sul pulsante `Crea un'istanza`. La consegna del servizio potrebbe richiedere alcuni minuti.
...
> [!info]
>
> Se hai installato un **OS con applicazione**, consulta la nostra [guida sui primi passi con le applicazioni](/guides/public-cloud/compute/apps-first-steps) oltre alla documentazione ufficiale dell'editor dell'OS.
>
...
![spazio cliente](/images/assets/screens/control-panel/product-selection/public-cloud/select-project.png)

Seleziona `Istanze` nella barra di navigazione a sinistra sotto **Compute**. La tua istanza è pronta quando lo stato è impostato su `Attivato` nella tabella. Se l'istanza è stata creata di recente e ha uno stato diverso, clicca sul pulsante "Aggiorna" accanto al filtro di ricerca.

![pagina istanze](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-connect01.png)

Clicca sul nome dell'istanza in questa tabella per aprire il `Dashboard`, in cui puoi trovare tutte le informazioni relative all'istanza. Per maggiori informazioni sulle funzioni disponibili su questa pagina, consulta la nostra guida su [gestire le istanze nello Spazio Cliente](/guides/public-cloud/compute/first-steps-with-public-cloud-instance).

Un **utente con diritti elevati (*sudo*) viene creato automaticamente** sull'istanza. Il nome utente riflette l'immagine installata, ad esempio "ubuntu", "debian", "fedora", ecc. È possibile verificarlo sul lato destro del `Dashboard` nella sezione **Reti**.

![pagina istanze](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-connect02.png)
...
>
> Questa guida non copre la rete privata per le istanze. Consulta la nostra documentazione [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts) a questo proposito.
>
...
> [!info]
>
> Se ricevi messaggi di errore relativi alle **chiavi SSH**, verifica che il tuo dispositivo locale disponga di una chiave SSH privata configurata correttamente utilizzando le informazioni di [questa guida](/guides/public-cloud/compute/compute-creating-ssh-keys#create-ssh-key).<br />
> In caso di difficoltà, è possibile sostituire la coppia di chiavi utilizzando [questa guida](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
> Se hai creato un'istanza senza chiave SSH, tramite l'[API OVHcloud](/guides/manage-and-operate/api/first-steps) o l'[interfaccia OpenStack Horizon](/guides/public-cloud/compute/create-instance-in-horizon), puoi aggiungere una chiave SSH alla tua istanza soltanto tramite il [modo rescue](/guides/public-cloud/compute/put-an-instance-in-rescue-mode) seguendo le istruzioni descritte in [questa guida](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
...
[In base alla configurazione](#create-ssh), sarà necessario immettere una frase segreta che protegga la chiave privata o specificare il percorso del file della chiave. Per maggiori informazioni, consulta la nostra [guida alle chiavi SSH](/guides/public-cloud/compute/compute-creating-ssh-keys#multiplekeys).
...
Dopo aver verificato che l'istanza Windows sia [installata](#verify-status), apri la scheda `Console VNC` nel tuo [Spazio Cliente OVHcloud](/links/manager).
...
<Tabs>
<Tab label="1. Impostazioni internazionali">

Configura il tuo **paese**, la tua **lingua Windows** preferita e il tuo **layout di tastiera**. Clicca sul pulsante `Avanti` in basso a destra.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-locale.png)<br />
  </Tab>
<Tab label="2. Password amministratore">

Definisci una password per il tuo account Windows `Administrator` e confermala, poi clicca su `Finish`.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-admin.png)<br />
  </Tab>
<Tab label="3. Schermata di login">

Windows applicherà le impostazioni e visualizzerà la schermata di accesso. Clicca sul pulsante `Send CtrlAltDel` in alto a destra per accedere.<br /><br />
...
Inserisci l'indirizzo IPv4 dell'istanza, l'identificativo e la password. In genere viene visualizzato un messaggio di avviso che richiede di confermare la connessione a causa di un certificato sconosciuto. Clicca su `Sì` per accedere.
...
Se compare un avviso di certificato, clicca su `Yes`. Immettere il nome utente e la password per Windows e fare clic su `OK` per stabilire la connessione.<br /><br />
![linux remote](/images/guides/public-cloud/compute/public-cloud-first-steps/24-rem-connect03.png)<br />
  </Tab>
<Tab label="3. Impostazioni">

Potete trovare elementi utili nella barra degli strumenti a sinistra. Ad esempio, clicca sull'icona `Toggle dynamic resolution update` per migliorare la risoluzione della finestra.<br /><br />
...
Seleziona `Istanze` nella barra di navigazione a sinistra sotto **Compute**. Clicca sul nome dell'istanza e apri la scheda `Console VNC`.

![console vnc](/images/assets/screens/control-panel/product-selection/public-cloud/cp-pci-vnc-login.png)

<Tabs>
<Tab label="Istanza con sistema operativo GNU/Linux installato">
...
> [!info]
>
> Durante la configurazione degli account utente e dei livelli di autorizzazione su un'istanza, ti consigliamo di utilizzare le informazioni contenute nella nostra [guida sull'account utente](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
>
...
Per una spiegazione dettagliata di questi passaggi, consulta la nostra [guida dedicata](/guides/public-cloud/compute/configuring-additional-ssh-keys).
...
[Come iniziare con OpenStack](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment)
