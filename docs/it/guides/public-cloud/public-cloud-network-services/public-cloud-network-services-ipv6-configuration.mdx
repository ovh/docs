...
> [!info]
> 
> Al momento, le offerte Floating IP e Gateway non supportano l'IPv6. L'IPv6 è utilizzabile solo con le istanze in [modalità pubblica](/guides/public-cloud/public-cloud-network-services/concepts#publicmode).
>
...
Accedi al tuo Spazio Cliente, vai alla sezione `Public Cloud`, seleziona il tuo progetto Public Cloud e clicca su `Istanze` nella barra di navigazione a sinistra. Poi clicca su `...` accanto all'istanza corrispondente e clicca su `Dettagli dell'istanza`.
...
Vai alla sezione `Connessione di rete` di Windows.
...
Clicca con il tasto destro sulla scheda di rete per accedere a `Proprietà`.
...
Clicca su `Internet Protocol Version 6 (TCP/IPv6)` e poi sul pulsante `Proprietà`.
...
Una volta terminata, spunta la casella `Conferma le impostazioni all'uscita` e clicca sul pulsante `OK` per confermare le modifiche.
