...
First of all, we will need to set up vRack Private Network for our Public Cloud. To do it, we follow the [Configuring vRack for Public Cloud](/guides/public-cloud/public-cloud-network-services/getting-started-09-creating-vrack-with-openstack/) guide. 
...
Now we need to download the `openrc.sh` configuration file, as explained in the [Setting OpenStack environment variables](/guides/public-cloud/public-cloud-cross-functional/loading-openstack-environment-variables/) guide. 
...
Following the steps on the [Setting OpenStack environment variables](/guides/public-cloud/public-cloud-cross-functional/loading-openstack-environment-variables/) guide to be sure that the Openstack CLI is working on our workstation.
...
Then we create a Kubernetes cluster in GRA5 region, attached to`priv_net_01`, as explained in the [Create a cluster](/guides/public-cloud/containers-orchestration/managed-kubernetes/creating-a-cluster/) guide. 
...
Don't forget to grab your `kubeconfig` and configure `kubectl` to use it, as explained in the [Configuring kubectl on an OVHcloud Managed Kubernetes cluster](/guides/public-cloud/containers-orchestration/managed-kubernetes/configuring-kubectl-on-an-ovh-managed-kubernetes-cluster/) guide.

### Setting up a PCI attached to `priv_net_02`

Now we can create a new Public Cloud instance, also in GRA5 region, and attach it to `priv_net_02` by following the [Integrating an instance into vRack](/guides/public-cloud/public-cloud-network-services/getting-started-07-creating-vrack#step-3-integrating-an-instance-into-vrack/) guide.
