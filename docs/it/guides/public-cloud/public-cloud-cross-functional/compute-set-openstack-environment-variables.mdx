...
- Aver preparato l'ambiente di sviluppo per utilizzare l'API OpenStack. A questo proposito, consulta questa guida: [Preparare l'ambiente per utilizzare l'API OpenStack](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment)
...
Accedi al tuo [Spazio Cliente OVHcloud](/links/manager), accedi alla sezione `Public Cloud` e seleziona il tuo progetto Public Cloud in alto a sinistra.
<br /> Nel menu di sinistra, sotto la rubrica **Impostazioni**, clicca su `Utenti e ruoli`. Poi clicca sui `...` a destra del tuo utente e seleziona `Scarica il file RC di Openstack`.
