...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HA3.png)
...
If HA is not enabled in your cluster, go to the `Configure` tab of your cluster and then to the `vSphere Availability` tab available in `Services`.

Click `Edit` and slide the cursor to enable the HA feature.
...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HA.png)
...
Default restart management on the cluster is also possible. You can refine this by virtual machine in the tab `VM Overrides`.
...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HAparam1.PNG)
...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HAparam2.PNG)
...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HAparam3.PNG)
...
![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HAparam4.PNG)

##### VM Monitoring

VM monitoring is available after [VMware tools](/guides/bare-metal-cloud/managed-bare-metal/vmware-tools-install) is installed, in case of non-response via the **tools** (heartbeat signals) the virtual machine will be automatically restarted. Advanced configuration is possible for this feature (for example, reboot interval).

![](/images/guides/bare-metal-cloud/managed-bare-metal/vmware-ha-high-availability/HAparam5.PNG)
...
In the `configuration` section and then in the `VM/Host Rules` tab, you can create a rule of type "VMs".
