...
Questa chiamata API consente di recuperare l'elenco di tutte le vRack create. Se i riferimenti mostrati non ti permettono di identificare la vRack, recupera l’informazione dallo [Spazio Cliente OVHcloud](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.it/&ovhSubsidiary=it): nella sezione <code className="action">Bare Metal Cloud</code> è sufficiente cliccare su <code className="action">Network</code>, poi su <code className="action">vRack</code> nella colonna a sinistra per visualizzare tutte le vRack attive.
