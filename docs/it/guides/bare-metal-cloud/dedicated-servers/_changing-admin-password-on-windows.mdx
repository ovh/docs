...
Nello [Spazio Cliente OVHcloud](/links/manager), clicca sulla scheda `IPMI` per aprire una sessione KVM.
...
In questa interfaccia è possibile modificare il file *SAM* per cancellare la password dell'utente admin. Il percorso di accesso predefinito della directory *WINDOWS* è precompilato. Apri il file per visualizzare la lista degli utenti cliccando su `Apri`.

L'utente interessato sarà "admin" o "Administrator", secondo la versione di Windows. Se sono presenti entrambi, seleziona "admin". Clicca su `Modifica la password`.

![ntpwedit](/images/guides/bare-metal-cloud/dedicated-servers/changing-admin-password-on-windows/adminpw_win_10.png)

Nella finestra che appare, lasciate che i campi siano vuoti e cliccate su `OK`. e infine clicca su `Salva le modifiche` e poi su `Via`.
...
Nella finestra KVM, seleziona l'opzione di arresto `Riavvia` utilizzando il pulsante Windows "Avvia" in basso a sinistra.
...
[Utilizzare l'IPMI sui server dedicati](/guides/bare-metal-cloud/dedicated-servers/dedicated-servers-ipmi)
