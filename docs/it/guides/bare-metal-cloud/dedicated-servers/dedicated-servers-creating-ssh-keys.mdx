...
Se si conferma con `Entrata` senza immettere un nome, verrà utilizzato il nome file standard (in questo esempio, `id_rsa`).
...
Se si conferma con il tasto `Invio` senza immettere un nome, verrà utilizzato il nome file standard (in questo esempio, `id_rsa`).
...
Quando si lavora da riga di comando **Windows**, utilizzare il pulsante destro del mouse per **incollare** il contenuto degli Appunti nella finestra della riga di comando. Per **copiare** una stringa dalla finestra della riga di comando, evidenziarla e premere `Invio`. Queste funzioni sono disponibili anche facendo clic con il pulsante destro del mouse sulla barra dei menu della finestra della riga di comando.
