...
- **Se hai già configurato un account:** clicca su `Strumenti` nella barra del menu in alto e poi su `Account`. Nella finestra che appare, clicca su `+` e poi su `Nuovo account`.
...
Inserisci il tuo indirizzo email e clicca su `Avanti`. Come provider, seleziona IMAP/POP e compila i campi richiesti.
...
Una volta inseriti tutti i dati, clicca su `Avanti`. Se le informazioni inserite sono corrette, la connessione all'account andrà a buon fine. 
