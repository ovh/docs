...
Per generare il link di recupero, accedi al tuo [Spazio Cliente OVHcloud](/links/manager), accedi alla sezione `Web Cloud`, clicca su `Hosting` e poi sul Cloud Web corrispondente. 

Seleziona la scheda `FTP - SSH` e clicca sul pulsante `Genera un backup` a destra.

![backupftpcw](/images/assets/screens/control-panel/product-selection/web-cloud/cloud-web/ftp-ssh/generate-a-backup.png)

Nella nuova finestra, seleziona uno dei backup disponibili e clicca su `Avanti`.
...
Clicca su `Conferma` per confermare la tua richiesta.
