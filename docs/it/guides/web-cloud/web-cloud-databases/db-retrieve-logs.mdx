...
2. Clicca sulla scheda `Web Cloud`.
3. Nella colonna di sinistra, clicca sul menu `Web Cloud Databases`.
4. Seleziona l'istanza Web Cloud Databases interessata.
5. Clicca sulla scheda `Log`.
...
> [!info]
>
> Come precisato in precedenza, i log sono disponibili qui solo in tempo reale. I log verranno visualizzati solo se generati nel momento in cui ti trovi sulla scheda `Log`. 
>
> Se esci dalla scheda `Log` e poi ci ritorni sopra in seguito, la cronologia che appariva in precedenza sarà scomparsa.
>
...
>
> Per verificare questo, recupera l'indirizzo IP pubblico del tuo punto di accesso a Internet e consulta la sezione **Autorizza un indirizzo IP** di [questa guida](/guides/web-cloud/web-cloud-databases/db-getting-started).
>
...
5. Nella nuova pagina, rimani sulla scheda `Informazioni generali` e clicca sul riquadro intitolato `Informazioni di login`.
6. Sotto la voce `SFTP` sono disponibili tutte le informazioni necessarie per accedere in SFTP.

> [!info]
>
> Se non conosci la `Password del server`, clicca sul pulsante `...` a destra per modificarla.
>
...
Per FileZilla, vai in alto a sinistra nel menu `File` e clicca su `Site Manager`.

Clicca su `New site` e inserisci i parametri rilevati precedentemente.
...
### Iscrivi i log della tua soluzione Web Cloud Databases a Logs Data Platform <a id="wcdb-ldp"></a>
...
Per maggiori informazioni su Logs Data Platform, consulta la nostra guida d'[introduzione a Logs Data Platform](/guides/manage-and-operate/observability/logs-data-platform/introduction) (EN).
...
4. Seleziona l'istanza Web Cloud Databases interessata.
5. Clicca sulla scheda `Log`.
6. A destra del riquadro in cui vengono visualizzati i log in tempo reale, clicca sul pulsante `Abbonati`.
...
#### Caso n°1 - Iscriversi a un flusso già esistente sulla tua soluzione Logs Data Platform <a id="wcdb-ldp-case1"></a>

Se il flusso esiste già, viene visualizzato come una riga nella tabella a piè di pagina.
In questo caso, per iscrivere la tua soluzione Web Cloud Databases a questo flusso esistente, è sufficiente cliccare sul pulsante `Abbonati` a destra della linea corrispondente al flusso in questione.
...
Se il flusso in questione non esiste ancora, clicca sul pulsante `Aggiungere un flusso di dati`.
...
Se necessario, consulta le nostre guide "[Introduzione a Logs Data Platform](/guides/manage-and-operate/observability/logs-data-platform/introduction)" (EN) e "[Iniziare rapidamente con Logs Data Platform](/guides/manage-and-operate/observability/logs-data-platform/getting-started-quick-start)" (EN) per eseguire queste azioni.

Una volta inseriti i moduli e le informazioni, clicca sul pulsante `Salva`.
...
Per effettuare questa operazione e, come spiegato [in precedenza](#wcdb-ldp), torna alla scheda `Log` della tua soluzione Web Cloud Databases per iscriverti a questo nuovo flusso di dati, quindi segui questa volta il [Caso n°1](#wcdb-ldp-case1) descritto sopra.

## Per saperne di più <a id="go-further"></a>

[Iniziare a utilizzare il Web Cloud Databases](/guides/web-cloud/web-cloud-databases/db-getting-started)
