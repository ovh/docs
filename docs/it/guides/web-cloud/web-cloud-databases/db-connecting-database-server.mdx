...
Accedi allo [Spazio Cliente OVHcloud](/links/manager) e seleziona `Web Cloud`. Clicca sulla scheda `Web Cloud Databases` nella colonna di sinistra e seleziona il nome del tuo database server.
...
- **Server:** inserire il *Nome host* del database server seguito dal *numero di porta*. Il *numero di porta* deve essere separato dal *nome host* da un "**spazio**" o da "**:**". Ad esempio, se il *Nome host* è **aaXXXXX-XXX.eu.clouddb.ovh.net** e il *numero di porta* è **12345**, è necessario inserire **aaXXXXX-XXX.eu.clouddb.ovh.net:12345** o **aaXXXXX-XXX.eu.clouddb.ovh.net 12345**. Per recuperare il *Nome host* e il *numero di porta* del tuo server Web Cloud Databases, accedi allo [Spazio Cliente OVHcloud](/links/manager) e accedi alla sezione `Web Cloud`. Clicca sulla scheda `Web Cloud Databases` nella colonna di sinistra e seleziona il nome del tuo database server. Nella pagina `Informazioni generali` che appare, troverai il *Nome host* e il *numero di porta* nel riquadro `Informazioni di login`.

- **Utente:** Inserisci il *nome utente* del tuo database server. Per recuperare il *nome utente* del tuo database, accedi al tuo [Spazio Cliente OVHcloud](/links/manager), clicca su `Web Cloud`. Clicca sulla scheda `Web Cloud Databases` nella colonna di sinistra e seleziona il nome del tuo database server. Clicca sulla scheda `Utenti e diritti`. Troverai una tabella con tutti gli utenti creati sulla tua soluzione Web Cloud Databases.

- **Password:** Inserisci la *password* associata al *nome utente* pertinente. Se non ricordi più la *password* associata al tuo *nome utente*, accedi allo [Spazio Cliente OVHcloud](/links/manager) e clicca su `Web Cloud`. Clicca sulla scheda `Web Cloud Databases` nella colonna di sinistra e seleziona il nome del tuo database server. Clicca sulla scheda `Utenti e diritti`. Clicca sul pulsante `...` a destra di *l'utente* interessato per `Modifica la password`.
...
> [!warning]
>
> Se utilizzi un'offerta "Web Cloud Databases"/"SQL Privato", non dimenticare di autorizzare il tuo IP utilizzando la guida sulla [configurazione del tuo server database](/guides/web-cloud/web-cloud-databases/db-configure-optimise-database-server).
>
...
- Lanciate SQuirreL SQL e cliccate su `Aliases`, poi su `+`

![launch SQuirreL SQL](/images/assets/screens/other/web-tools/squirrel/aliases.png)

- Completa i campi qui sotto e conferma con il pulsante `OK`:
    - **Name**: Scegli un nome
...
- Conferma di nuovo con il pulsante `Connect`
