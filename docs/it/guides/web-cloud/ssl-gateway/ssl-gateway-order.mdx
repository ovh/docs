...
Scegli la tua offerta e clicca su `Ordina adesso`.

![order ssl gateway](/images/assets/screens/website/order/configure-my-ssl-gateway.png)

Nella nuova pagina, inserisci nel form `Cerca il tuo dominio` il dominio o sottodominio interessato e clicca sull'icona a forma di lente di ingrandimento sulla destra.
...
Se lo desideri e se disponibile al momento dell'ordine, spunta la casella `Gestisco la zona DNS di questo dominio e autorizzo OVHcloud a modificare automaticamente il record DNS richiesto`. La zona DNS associata al dominio o sottodominio verrà aggiornata automaticamente con l'indirizzo IP dell'SSL Gateway.
...
Verifica che tutte le tue scelte siano corrette nella pagina dell'ordine e clicca su `Continua`.

Per terminare, lasciati guidare fino alla convalida / pagamento del tuo buono d'ordine.

### Configurazione della zona DNS

Una volta convalidato l'ordine, se non hai selezionato la casella `Gestisco la zona DNS di questo dominio e autorizzo OVHcloud a modificare automaticamente il record DNS richiesto`, riceverai un'email in cui ti viene chiesto di far puntare il tuo dominio o sottodominio all'infrastruttura OVHcloud entro 3 giorni.
...
:::details Caso 1: la zona DNS è gestita dai server DNS condivisi OVHcloud.
...
:::details Caso 2: la zona DNS non è gestita dai server DNS condivisi OVHcloud.
