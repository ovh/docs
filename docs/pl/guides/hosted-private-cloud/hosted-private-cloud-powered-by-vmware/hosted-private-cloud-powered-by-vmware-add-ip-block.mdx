...
Aby zamówić dodatkowy blok IP dla Twojej usługi **Private Cloud**, przejdź do Panelu klienta OVHcloud. W sekcji `Serwer` kliknij rubrykę `IP` w lewej kolumnie, a następnie wybierz opcję `Zamów dodatkowe IP`. Teraz wybierz Twoją usługę **Private Cloud** w rozwijanym menu, aby móc przejść do następnego etapu.
...
Możesz śledzić przenoszenie bloku IP z poziomu Twojego [Panelu klienta OVHcloud](/links/manager) w sekcji `Serwer`, a następnie `Private Cloud`. Kliknij Twoją usługę Hosted Private Cloud, a następnie kartę `Operacje`.
...
Następnie IP pojawi się w `Parking adresów IP`.
