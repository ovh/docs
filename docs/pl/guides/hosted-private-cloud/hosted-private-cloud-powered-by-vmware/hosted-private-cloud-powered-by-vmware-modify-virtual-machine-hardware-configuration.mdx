...
Wszystkie opisane poniżej zmiany należy wprowadzać z poziomu Private Cloud w vSphere po kliknięciu prawym przyciskiem maszyny wirtualnej i wybraniu opcji `Zmień ustawienia`.
...
Po zaznaczeniu pola `Dodawanie CPU bez przerwy w pracy` możesz zmieniać te ustawienia po uruchomieniu maszyny wirtualnej.
...
Ten interfejs może być przydatny w przypadku niewłaściwego działania sieci. Możesz się upewnić, że *port ID* odpowiada wprowadzonemu w zakładce `Networking` oraz `Ports` dla danej karty.
