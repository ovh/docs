...
W zależności od serwera KMS możesz się do niego zalogować za pomocą przeglądarki. Kliknij opcję <code className="action">View Certificate</code>, a następnie <code className="action">Thumbprint</code>.
...
W Panelu klienta przejdź do sekcji `Bare Metal Cloud`. Kliknij <code className="action">Managed Bare Metal</code> na pasku usług po lewej stronie, po czym wybierz odpowiednią usługę Managed Bare Metal.

Na stronie głównej usługi kliknij opcję <code className="action">Bezpieczeństwo</code>.
...
Niżej na stronie znajduje się sekcja **Virtual Machine Encryption Key Management Servers**. Kliknij przycisk <code className="action">Dodaj nowy serwer KMS</code>.
...
Potwierdź uwzględnienie tej dokumentacji, a następnie kliknij <code className="action">Dalej</code>. 
...
Rozpocznij operację, logując się do usługi Managed Bare Metal przy użyciu klienta Web vSphere. Następnie przejrzyj listę zasobów i wybierz vCenter. Kliknij opcję “Zarządzaj”, a następnie “Key Management Servers”. Kliknij <code className="action">Dodaj KMS</code>, a w asystencie, który się pojawi, podaj informacje o KMS i kliknij <code className="action">OK</code>.
Kliknij <code className="action">Trust</code>, aby zatwierdzić certyfikat.
...
W vCenter wybierz uprzednio dodany serwer KMS. W sekcji “Wszystkie opcje” kliknij <code className="action">Ustal zaufane połączenie z KMS</code>.
...
Utwórz maszynę wirtualną. Po utworzeniu kliknij ją prawym przyciskiem myszy. Następnie kliknij <code className="action">VM Policies</code>, a potem <code className="action">Edit VM Storage Policies</code>.
...
Edytuj maszynę wirtualną i przejdź do <code className="action">VM Options</code>.
...
<img className="thumbnail" alt="Zaszyfrowany vMotion" src="/images/bare-metal-cloud/managed-bare-metal/vm-encrypt/vm-encrypt_06.png" loading="lazy" />
