...
Alternatively, you can have a job already `RUNNING` that is connected to that object store container and is writing metric logs inside it (example: `my_tf_metrics@GRA:/runs:RW:cache`). In that last case, don't forget the `cache` parameter indicating that the volume is cached and sharable among jobs. More information about volumes configuration in jobs can be found [here](/guides/public-cloud/ai-machine-learning/cli-12-howto-run-job-cli#attaching-volumes/), information about volume caching can be found [here](/guides/public-cloud/ai-machine-learning/gi-02-concepts-data#capabilities/).
