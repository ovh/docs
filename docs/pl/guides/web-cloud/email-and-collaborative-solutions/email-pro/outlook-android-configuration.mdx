...
### Dodaj konto <a id="add-account"></a>
...
>
> Znajdziesz tę cyfrę w [Panelu klienta OVHcloud](/links/manager), przejdź do sekcji `Web Cloud`, a następnie `E-mail Pro`. Nazwa serwera jest widoczna w ramce **Połączenie** w zakładce `Informacje ogólne`.

- **Podczas pierwszego uruchomienia aplikacji** : wyświetli się asystent konfiguracji, po czym kliknij `Dodaj konto`.
...
- **Jeśli konto zostało już skonfigurowane**:
    ...
    - Następnie naciśnij przycisk `+` w lewym pionowym pasku.
    - Kliknij `Dodaj konto`.
...
Wpisz swój adres e-mail i naciśnij przycisk `Kontynuuj`.
...
> [!warning]
>
> Jeśli okno wyboru protokołu nie wyświetla się, kliknij przycisk `?` w prawym górnym rogu ekranu, następnie wybierz `Zmień dostawcę konta`. Następnie wybierz `IMAP`(zalecane) lub `POP3`.<br />
...
W kolejnym oknie zaznacz `Ustawienia zaawansowane` i uzupełnij następujące informacje:
...
### Zmień istniejące ustawienia <a id="modify-settings"></a>
...
- Kliknij `Usuń konto`.
- Naciśnij przycisk `Usuń` na pytanie "Czy chcesz usunąć konto?".
...
### Przypomnienie parametrów POP, IMAP i SMTP <a id="popimap-settings"></a>
...
## Przejdź dalej <a id="go-further"></a>
