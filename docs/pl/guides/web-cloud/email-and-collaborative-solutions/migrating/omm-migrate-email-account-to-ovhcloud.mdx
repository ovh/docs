...
### Tworzenie projektu migracji <a id="create-project"></a>
...
Kliknij `New migration`, aby rozpocząć tworzenie projektu.
...
Kliknij `Create my project`, aby rozpocząć tworzenie projektu.
...
- Kliknij `New migration` w lewym górnym rogu tabeli.
...
- **Importing files**: Możliwe jest przeniesienie zawartości plików PST, ICS, CSV i XML Rules przez OMM do konta poczty e-mail docelowego. Gdy ta funkcja zostanie wybrana, po prostu przeciągnij i upuść swój dokument do wyznaczonej strefy lub przejdź do terminala za pomocą przycisku `Browse your files`.
...
- `Migrate and add another account`, jeśli chcesz dodać kolejną migrację do listy projektu.
- `Migrate my account`, aby rozpocząć skonfigurowaną migrację i wrócić do strony głównej projektu.

### Migrowanie za pomocą połączenia z kontem klienta OVHcloud <a id="sso-migration"></a>
...
- Kliknij `Login`, aby wyświetlić okno logowania do Panelu klienta OVHcloud.
...
- Wprowadź identyfikator lub adres e-mail powiązany z kontem OVHcloud, wprowadź odpowiednie hasło i kliknij `Login`;
- lub kliknij `Continue`, jeśli był już zidentyfikowany.
...
- Pojawia się nowe okno. Pozwala ono na udzielenie OMM uprawnień do dostępu do funkcji Twojej strefy klienta, a także na wyświetlenie ofert i kont pocztowych. Domyślnie ważność (Validity) tych uprawnień wynosi 24 godziny (1 dzień). Ustaw okres ważności, który Ci odpowiada, a następnie kliknij `Authorize`.
...
> [!warning]
>
> Możesz wylogować bieżące połączenia z Panelu klienta OVHcloud, klikając `Log out`, a następnie pod spodem przy wzmiance `OVHcloud account ID`, kliknij `Log out of the account`.
>
> ![omm](/images/guides/web-cloud/email-and-collaborative-solutions/migrating/migration-omm/omm-migration-sso-05.png)

### Śledzenie projektu migracji <a id="follow-migration"></a>
...
- Z [strony głównej OMM](/links/web/omm) kliknij `Track a migration`. Następnie wprowadź swoje `Project ID` i swoje `Project password`.

Kliknij `Connect to project`, aby uzyskać dostęp do strony głównej projektu i śledzić swoje migracje.
...
Kliknij przycisk `⋮` po prawej stronie wiersza migracji, aby wyświetlić opcje:

- `See more details`: Zostaniesz przekierowany do strony umożliwiającej śledzenie postępu migracji i odczyt raportu po zakończeniu migracji.
- `Cancel migration`: Pozwala to na anulowanie trwającej migracji. Przemieszczone elementy zostaną nadal na koncie docelowym.
- `Delete migration data (GDPR)`: Ta opcja uruchamia usunięcie wszystkich danych związanych z migracją konta. Nadal zachowasz informacje dotyczące zdarzeń, które miały miejsce podczas migracji.
...
> [!info]
> 
> Jeśli podczas migracji wystąpi błąd, link do logu błędu zostanie wyświetlony w oknie `See more details`.
