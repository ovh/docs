...
Aby uzyskać dostęp do frameworków Twojego hostingu Cloud Web, zaloguj się do [Panelu klienta](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.pl/&ovhSubsidiary=pl). Po zalogowaniu kliknij <code className="action">Hosting</code> na pasku usług po lewej stronie, następnie wybierz odpowiedni hosting Cloud Web. Teraz przejdź do zakładki <code className="action">Frameworki</code>.
...
Po uzupełnieniu informacji, kliknij przycisk <code className="action">Zatwierdź</code>. Jeśli potrzebujesz więcej informacji o zarządzaniu frameworkami, skorzystaj z naszego przewodnika [Zarządzanie frameworkami Cloud Web](/guides/web-cloud/web-hosting/manage-runtime/).
