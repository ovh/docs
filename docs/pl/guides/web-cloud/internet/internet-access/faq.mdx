...
Une fois connecté, rendez-vous dans l'univers `Telecom` puis `Accès Internet`. Vous y retrouverez votre offre.
...
Si la liaison ne remonte pas, vous avez la possibilité d'utiliser notre assistant de dépannage. Pour y accéder, connectez-vous, depuis un autre accès à Internet ou un smartphone, à votre [espace client OVHcloud](/links/manager) dans la section `Telecom`. Une fois positionné sur l'accès Internet concerné, cliquez sur `Lancer le diagnostic`. 
...
### Comment passer d'un dégroupage partiel à un dégroupage total ? <a id="degroupage"></a>
