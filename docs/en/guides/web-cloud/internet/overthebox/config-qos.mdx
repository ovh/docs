...
![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step2-tc-1-selectInterface-2024.png)

- Rendez-vous sur l'onglet `Traffic Control`.
- Modifiez le paramètre `Traffic Control` en `Static` à l'aide du menu déroulant.
- Dans le paramètre `Download bandwidth`, renseignez le débit descendant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 10Mbps, la valeur est `10000` Kbps.
- Dans le paramètre `Upload bandwidth`, renseignez le débit montant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 1,2Mbps, la valeur est `1200` Kbps.
- Confirmez vos changements avec le bouton `Save`.
- Appliquez vos changements avec le bouton `Save & Apply`.

![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step2-tc-2-configureInterface-2024.png)
...
- Rendez-vous dans l'onglet `OverTheBox > QoS Settings`.
- L'écran de configuration listant toutes les règles QoS s'affiche.
- Cliquez sur `Add`.

> [!info]
>
> Les règles sont testées dans l'ordre du tableau, utilisez le bouton `≡` pour modifier la position d'une règle.
>

![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step3-tc-1-addRule-2024.png)

- Rendez-vous sur l'onglet `Traffic Control`.
- Modifiez le paramètre `Direction` pour sélectionner le sens de la règle, montant ou descendant. Pour notre exemple, nous souhaitons que le débit montant émanant du **NAS** soit prioritaire, nous renseignons donc `upload`.
- Modifiez le paramètre `Protocol` pour sélectionner le protocole sur lequel s'applique la règle. Pour notre exemple, nous souhaitons n'importe quel protocole, nous renseignons donc `all`.
- Modifiez le paramètre `Source Host` pour sélectionner la source du trafic. Pour notre exemple, le **NAS** est l'hôte source, nous renseignons donc son adresse IP `192.168.100.80`.
- Modifiez le paramètre `Destination Host` pour sélectionner la destination du trafic. Pour notre exemple, nous souhaitons n'importe quelle destination, nous renseignons donc `all`.
- Modifiez le paramètre `Class` pour sélectionner la priorité du trafic. Pour notre exemple, nous souhaitons que le trafic soit prioritaire, nous renseignons donc `High priority`.
- Modifiez le paramètre `Comment` pour nommer la règle. Pour notre exemple, nous nommons la règle `NAS`.
- Confirmez vos changements avec le bouton `Save`.
- Appliquez vos changements avec le bouton `Save & Apply`.

![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step3-tc-2-configureRule-2024.png)
...
Votre **OverTheBox** contient un service qui s'appelle `otb-auto-sqm` qui est capable de lancer des tâches en arrière-plan afin de configurer automatiquement SQM sur toutes les interfaces WAN disponibles.
...
- Rendez-vous dans l'onglet `Network > SQM QoS`.
- L'écran de configuration listant la configuration **SQM** s'affiche.
- Cliquez sur `Add`.

![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step2-sqm-1-addConfig-2024.png)

- Cochez la case du paramètre `Enable this SQM instance` pour activer la configuration **SQM**.
- Modifiez le paramètre `Interface name` pour sélectionner l'interface sur laquelle **SQM** s'exécute. Pour notre exemple, notre lien **ADSL** est l'interface `eth2`, nous renseignons donc `eth2`.
- Dans le paramètre `Download speed (ingress)`, renseignez le débit descendant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 25Mbps, la valeur est `25000` Kbps.
- Dans le paramètre `Upload speed (egress)`, renseignez le débit montant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 3Mbps, la valeur est `3000` Kbps.
- Appliquez vos changements avec le bouton `Save & Apply`.

![overthebox](/images/guides/web-cloud/internet/overthebox/config-qos/step2-sqm-2-configure-2024.png)
...
> [!warning]
>
> Cette section concerne la page `OverTheBox > QoS Settings` disponible uniquement sur les versions v0.9 et antérieures.
>
