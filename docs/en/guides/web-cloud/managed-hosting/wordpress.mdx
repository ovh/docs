...
Log in to the [OVHcloud Control Panel](/links/manager), go to the `Web Cloud` section, and click on `Managed hosting for WordPress`.

#### Create a website

To create a website, click the `Create` button. The following page appears:
...
- `WordPress admin language`: select the default language for the administration interface.
- `PHP version`: choose the desired PHP version for your WordPress environment. Check the available PHP versions on [this page](/guides/web-cloud/web-hosting/web-hosting-main-info).
- `Admin email`: enter the administrator's email address for the WordPress site.
- `Admin password`: set the password associated with this administrator account.

Click on `Continue` to validate the information.

You are redirected to the `My sites` tab. At the top of the screen, a confirmation message appears indicating that your website creation request has been processed.
...
- `Domain name` column: corresponds to the technical domain name of your website. It serves as the staging URL (pre-production), allowing you to access your website before associating it with your own domain name.
- `Status` column: indicates the progress of the website creation. The status initially appears as `In progress`, while the infrastructure is deployed and WordPress is installed. Once your website is operational, the status changes to `Enabled`, indicating that your WordPress site is now available online via its technical domain name.
...
You can also track the progress of your website creation by clicking on the `Tasks` tab.
...
To import a WordPress website, click on the `Import` button. The following screen appears:
...
- `Website administration URL`: enter the address used to access the administration interface of your current WordPress website, for example `https://www.mywebsite.com/wp-admin`.
- `Admin account`: enter the WordPress administrator username.
- `Admin password`: enter the WordPress administrator password.
...
Once the required fields are filled in, click on `Continue` to start the import process.
...
Once all items are selected, click on `Start import`.
...
To delete a website, go to the `My sites` tab and identify the row in the table corresponding to your website. Click on the `⋮` button and then on `Delete`.
...
A confirmation message appears. Click on `Delete` to confirm the deletion of your website.
...
You are redirected to the `My sites` tab. A confirmation message appears at the top of the screen, indicating that the deletion process of your website is in progress.
...
To delete multiple websites at the same time, go to the `My sites` tab. Identify the rows in the table corresponding to the websites you want to delete and check the boxes on the left. Once the websites are selected, click on the `Delete X websites` button.
...
A confirmation message appears. Click on `Delete` to confirm the deletion of your websites.
...
You are redirected to the `My sites` tab. A confirmation message appears at the top of the screen, indicating that the deletion process of your websites is in progress.
...
To manage all your websites, go to the `My sites` tab and click on `Manage websites`. This button automatically opens the fleet manager, integrated into Managed Hosting for WordPress. This space allows you to manage multiple WordPress websites simultaneously, from a unified interface.
