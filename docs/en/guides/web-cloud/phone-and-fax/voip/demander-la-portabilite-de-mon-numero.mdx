...
![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porta-request.png)
...
#### Porter un numéro isolé <a id="numero-isole"></a>

Cochez la case `Un numéro isolé` et renseignez le numéro isolé que vous souhaitez porter au format international (par exemple, « 0033100000000 » pour un numéro français). Indiquez ensuite le *RIO* du numéro concerné, sélectionnez la date souhaitée de la portabilité, puis cliquez sur `Suivant`.

![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step2-2021.png)

Dans la page suivante, renseignez avec précision les informations personnelles du titulaire de la ligne ainsi que l'adresse complète d'installation de cette dernière. Précisez également si vous souhaitez voir apparaître votre numéro dans l'annuaire. Une fois ces données complétées, cliquez sur le bouton `Suivant`.

![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step3-2021.png)
...
Vous pouvez également configurer une redirection, effective une fois le numéro porté, des appels réceptionnés sur ce numéro vers une de vos lignes. Utilisez le bouton `Choisir une ligne` pour la définir.
...
![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step4-2021.png)

La dernière étape consiste à finaliser votre demande de portabilité. Prenez connaissance des contrats liés à la portabilité et validez leur acceptation. Cliquez ensuite sur `Commander` et validez le bon de commande qui vous sera présenté. 

Dès lors, pour suivre l'avancement de votre demande, reportez-vous aux informations de l'étape 3 « [Suivre une demande de portabilité](/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero#etape-3-suivre-une-demande-de-portabilite) » de cette documentation. 

#### Porter une tranche de numéros <a id="tranche-numeros"></a>

Pour porter tout ou partie des numéros d'une tranche, cochez la case `Un ou plusieurs numéros appartenant à une tranche de numéros (SDA)`. 
...
- **porter l'ensemble de la tranche de numéros** : pour cela, cochez `Porter votre numéro principal et l'ensemble des numéros` ;

- **porter certains numéros de la tranche, dont celui de tête de ligne** : pour cela, cochez `Porter votre numéro principal et une partie des numéros` puis renseignez les numéros additionnels de la tranche que vous souhaitez porter. Utilisez alors la zone de texte en dessous de `Numéro SDA à ajouter` ainsi que le bouton `Ajouter`.

Sélectionnez la date souhaitée de la portabilité, puis cliquez sur `Suivant`.

![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step5-2021.png)
...
Verifiez bien que l'adresse et le nom du titulaire correspondent. En cas d'erreur, utilisez le bouton `Modification d'informations` prévu à cet effet.

Si les informations sont valides, prenez connaissance des contrats liés à la portabilité et validez leur acceptation. Finalisez votre demande de portabilité en cliquant sur `Commander` puis en validant le bon de commande qui vous sera présenté.
...
Pour cela, sélectionnez votre groupe de téléphonie dans la barre de services à gauche puis cliquez sur `Consulter mes portabilités en cours`. Cliquez alors sur `Attacher mon mandat de portabilité signé`, sous la demande de portabilité concernée, pour importer le mandat au format PDF.

> [!info]
>
> Si le bouton `Attacher mon mandat de portabilité signé` n'est pas disponible ou que vous n'avez pas reçu de mandat de portabilité, vérifiez que les informations concernant le titulaire de la ligne sont bien exactes. Le cas échéant, utilisez le bouton `Modification d'informations` pour les corriger.
>

Avant la validation manuelle du mandat par nos services techniques, vous pouvez, en cas d'erreur, supprimer le mandat actuel pour en attacher un autre. Pour cela, cliquez sur `Liste des documents envoyés` puis sur le bouton `...` à droite du mandat actuel et enfin sur `Supprimer`. 

![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step7-2.png)

Pour suivre l'avancement de votre demande de portabilité, reportez-vous aux informations décrites dans l'étape 3 « [Suivre une demande de portabilité](/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero#etape-3-suivre-une-demande-de-portabilite) » de cette documentation.

#### Porter un numéro spécial de Services à Valeur Ajoutée (SVA) <a id="numero-special"></a>
...
Une fois le numéro spécial à porter renseigné (au format international) dans le champ prévu à cet effet, cliquez sur le bouton `Vérifier l'identité`. Vous serez alors redirigé vers la procédure de vérification d'identité.
...
1. Positionnez-vous sur l'onglet `Tableau de bord` et cliquez sur `Consulter mes portabilités en cours`.

![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step8-2021.png)
...
![numberport](/images/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero/porting-number-step9-2021.png)

Si, durant le processus, vous souhaitez annuler votre demande de portabilité, vous pouvez le faire, au plus tard jusqu'à 72 heures avant la date de portabilité, grâce au bouton `Annuler la portabilité ?`.
...
### Étape 4 : préparer la configuration des numéros portés <a id="preparer-configuration-numero"></a>
