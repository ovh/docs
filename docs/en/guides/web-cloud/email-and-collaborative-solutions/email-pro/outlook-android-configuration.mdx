...
>
> Find this number in your [OVHcloud Control Panel](/links/manager), in the `Web Cloud` section, then `Email Pro`. The server name is visible in the **Connection** box in the `General information` tab.

- **When you start the application for the first time**: A configuration wizard will appear. Tap `Add account`.
...
- **If an account has already been set up**:
    ...
    - Then tap the `+`' button in the left-hand vertical bar.
    - Tap `Add Account`.
...
> [!warning]
>
> If the protocol selection window does not appear, tap the `?` button in the top right-hand corner of the screen, then choose `Change account provider`. Then select `IMAP`(recommended) or `POP3`.<br />
...
In the next window, tick `Advanced settings` and enter the following information:
...
### Modify existing settings <a id="modify-settings"></a>
...
- Tap `Delete account`.
- Tap `Delete` at the question "Do you want to delete the account?".
...
### Reminder of POP, IMAP and SMTP settings <a id="popimap-settings"></a>
