...
> 3. In the `MICROSOFT` section, click on `Exchange`.
> 4. Select the relevant platform.
> 5. The server name is visible in the **Connection** section of the `General Information` tab.
...
When Thunderbird detects an OVHcloud domain name, an automatic configuration related to the MX Plan offer is proposed. Click on `MODIFY THE CONFIGURATION`.
...
1\. Click on `Test` to verify the entered settings.
...
- To modify settings related to the **reception** of your emails, click on `Server Settings` in the left column under your email address.
...
- To modify settings related to the **sending** of your emails, click on `Outgoing Server (SMTP)` at the bottom of the left column.
- Click on the relevant email address in the list, then click on `Edit`.
