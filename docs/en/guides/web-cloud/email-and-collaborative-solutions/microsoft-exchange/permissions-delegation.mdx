...
The table that appears will list all of the accounts linked to your service. Click on the three dots to the right of the account you want to delegate permissions for, then on `Manage delegations`.
...
In the configuration window, select the permissions you want to delegate. You will need to link them to one or more accounts that you wish to grant the permissions to. Then press `Next`.
...
Take a few moments to check the change summary. If the information in the summary is correct, click `Confirm`. Within a few minutes, the delegation will be created on our servers.
...
Log in via the [OVHcloud webmail](/links/web/email) using the credentials for the email account that has been delegated permissions. Once you are logged in, right-click on the name of the account in the menu on the left-hand side, then select `Add shared folder`.

In the window that pops up, enter the name for the account that holds the delegated permissions, then click `Add`. The account will then appear in the left-hand menu, and you can view its content.
...
In the Outlook application, click `File` in the menu bar at the top of your screen, then `Account Settings`. In the drop-down menu, click again on `Account Settings`. In the window that pops up, select the account with the delegated permission, then click `Change`.
...
Next, click on `More Settings`. In the new window, go to the `Advanced` tab, then click `Add`. Enter the name of the account that will use the delegated permission, then confirm the addition. The account will then appear in the left-hand menu on your application, and you can view its content.
...
Log in via the [OVHcloud webmail](/links/web/email) using the credentials for the email account that has been delegated permissions. Once you have logged in, start editing a new email by clicking `+ New`.

In the zone that appears, click on the three dots, then `Show From`. Then click on the `From` button, and select the email address you want to appear as the sender (i.e. the address you have delegated that permission for). If the address is not listed, delete the one that is already displayed, and enter the address you want to appear.
...
In your Outlook application, start editing a new email. Ensure that the `From` button is visible in the editing window. If it is not visible, go to the `Options` tab, then click `From`.

Then click on the `From` button, and select the email address you want to appear as the sender (i.e. the address you have delegated permissions for). If it is not displayed, click `Other Email Address`, enter the email address you would like to appear as the sender, then confirm.
...
In the zone that appears, click on the three dots, then `Show From`. Then click on the `From` button, and select the email address you want to appear as the sender (i.e. the address you have delegated permissions for). If the address is not listed, delete the one that is already displayed, and enter the address you want to appear.
