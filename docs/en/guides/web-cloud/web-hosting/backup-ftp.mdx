...
To generate the recovery link, log in to the [OVHcloud Control Panel](/links/manager), go to the `Web Cloud` section, click `Hosting plans`, then click on the Cloud Web concerned. 

Select the `FTP - SSH` tab and click the `Generate a backup` button on the right.

![backupftpcw](/images/assets/screens/control-panel/product-selection/web-cloud/cloud-web/ftp-ssh/generate-a-backup.png)

In the window that opens, select one of the available backups, then click `Next`.
...
Click `Confirm` to confirm your request.
