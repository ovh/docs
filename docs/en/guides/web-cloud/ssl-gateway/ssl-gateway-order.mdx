...
> [!warning]
>
> SSL Gateways are not compatible with [OVHcloud web hosting](/links/web/hosting) offers. If you would like to use an SSL certificate for this type of solution, please read our guide on [Managing SSL certificates on a web hosting plan](/guides/web-cloud/web-hosting/ssl-on-webhosting).
>
...
Choose your solution, then click `Order now`.

![order ssl gateway](/images/assets/screens/website/order/configure-my-ssl-gateway.png)

On the new page, enter the domain name or subdomain concerned in the `Search for your domain name` form, then click on the magnifying glass icon on the right.
...
If you wish, and if it is available during the order, tick the `I manage the DNS zone of this domain and authorize OVHcloud to automatically modify the required DNS record`. The DNS zone associated with your domain name or subdomain will then be automatically updated with the SSL Gateway IP address.
...
Check that all of your choices are correct on the order page, then click `Continue`.

Finally, please follow the steps until you pay for your purchase order.

### Configuration of your DNS zone

Once your purchase order has been validated, and if you have not ticked the option `I manage the DNS zone of this domain and authorize OVHcloud to automatically modify the required DNS record`, you will be sent an email asking you to point your domain name or subdomain to the OVHcloud infrastructure within 3 days.
...
:::details Case 1: Your DNS zone is managed by OVHcloud shared DNS servers.
...
:::details Case 2: Your DNS zone is not managed by OVHcloud shared DNS servers.
