...
>
> You can find more information in our [guide on how to create a Public Cloud instance](/guides/public-cloud/compute/compute-getting-started).
>
...
`Ed25519` is considered the most secure but `RSA` is a valid alternative. Both methods are compatible with the [OVHcloud Control Panel](/guides/public-cloud/compute/compute-getting-started).
...
If you confirm with `Enter` without entering a name, the standard file name will be used (`id_rsa` in this example).
...
Copy this key string to [add it to a new instance or to import it into the OVHcloud Control Panel](/guides/public-cloud/compute/compute-getting-started).
...
When working from a **Windows** command line, you can use a right-click to **paste** the content of your clipboard into the command line window. To **copy** a string from the command line window, highlight it, then press `Enter`. You can also find these functions via a right-click on the menu bar of the command line window.
...
[How to create a Public Cloud instance](/guides/public-cloud/compute/compute-getting-started)
