...
However, if you perform too many actions within a short period of time, some OpenStack actions will fall in error due to too many API Calls. The current limit is 60 token creations per minutes and per OpenStack user. The authentication endpoint will return HTTP 429 errors beyond this limit. See our [Public Cloud API Rate Limits](/guides/public-cloud/public-cloud-cross-functional/api-rate-limits/) guide for more information.
...
<img className="thumbnail" alt="download openRC file" src="/images/public-cloud/public-cloud-cross-functional/using-openstack-tokens-for-multiple-calls/openrc.png" loading="lazy" />
