...
- [Data](/guides/public-cloud/ai-machine-learning/gi-02-concepts-data/) can be attached to an app to serve as input (e.g. model weights).
- If you do not customize your resource request, the default requested is 1 GPU (Tesla V100S). Memory is not customizable.
- [Scaling](/guides/public-cloud/ai-machine-learning/deploy-guide-04-scaling-strategies/) for applications depends on the chosen configuration. It can be **static** or **automatic**, and is based on a trigger threshold according to the metric chosen by the user.
- [Billing](/guides/public-cloud/ai-machine-learning/deploy-guide-06-billing-concept/) for **apps** is minute-based and applies during the `SCALING` and `RUNNING` states of the application. Each commenced minute is billed completely.
- You can read further on app limitations [here](/guides/public-cloud/ai-machine-learning/deploy-guide-01-capabilities/).
...
:::info
Only the `RUNNING` and `SCALING` time of the app **is billed**. For more information about apps billing, refer to this [documentation](/guides/public-cloud/ai-machine-learning/deploy-guide-06-billing-concept/).
...
- You can check the [OVHcloud documentation on how to submit an app](/guides/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/).
