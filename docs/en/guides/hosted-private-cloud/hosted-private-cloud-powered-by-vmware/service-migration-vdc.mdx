...
- Access to the [OVHcloud Control Panel](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.co.uk/&ovhSubsidiary=GB) (<code className="action">VMware</code> in the <code className="action">Hosted Private Cloud</code> section)
...
For connections to the VMware platform, you can choose to block access to vSphere by default. Please refer to our guide on the [vCenter access policy](/guides/bare-metal-cloud/managed-bare-metal/vcenter-modify-access-policy/) for details.
...
To do this, please refer to our guides on [Changing user rights](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/manager-ovh-private-cloud/), [Changing the User Password](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/changement-du-mot-de-passe-utilisateur/) and [Associating an email with a vSphere user](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/vsphere-edit-user/).
...
As part of a migration process, by default, the new vDC will be linked to the same vRack as the source vDC. Please consult our guide to [Using Private Cloud within a vRack](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/using-private-cloud-in-vrack/).
...
Setting up a new vDC involves reconfiguring VMware High Availability (HA), including boot order and priority. Please consult our guide about [VMware HA configuration](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/vmware-ha-high-availability/).
...
For more information, consult OVHcloud's guide [How to create a V(x)LAN within a vRack](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/creation-vlan#vlan-vrack/) and VMware's documentation on [how to edit general distributed port group settings](https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.networking.doc/GUID-FCA2AE5E-83D7-4FEE-8DFF-540BDB559363.html).
...
- However, if promiscuous mode needs to be enabled on the “VM Network” portgroup in the new vDC, follow the instructions from [this guide](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/vmware-carp/).
...
As a first step, please read our documentation on [getting started with NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-01-first-steps/).
...
For more information, read our documentation on [Managing the distributed firewall in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-05-manage-distributed-firewall/).
...
To create T1 Gateways, follow our documentation on [Adding a New Tier-1 Gateway](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-10-add-new-tier1-gateway/). This documentation also guides you on how to create segments. Before creating segments, it is necessary to inventory the VXLANs used in the source vDC and create a segment for each VXLAN used in your infrastructure.

Afterwards, you can connect them to the provided T0 in NSX.

For more information, read our documentation on [segment management in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-02-segment-management/).
...
To recreate DHCP and associate them with your segments and T1 Gateways, refer to our guide on [configuring DHCP in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-03-configure-dhcp-onsegment/).

<a name="dns"></a>
##### Step 4.8.3.3 DNS

To recreate DNS and associate them with your T1 Gateways, please refer to our guide on [configuring the DNS forwarder in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-04-configure-dns-forwarder/).

<a name="nat"></a>
##### Step 4.8.3.4 NAT Rules

To recreate your NAT rules and associate them with the T1 Gateways, please refer to our guide on [Setting up NAT for port redirections with NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-07-configure-nat-redirection/).

<a name="lb"></a>
##### Step 4.8.3.5 NSX Load Balancing

To recreate your Load Balancers, follow the instructions in our guide on [configuring Load Balancing in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-09-configure-loadbalancing/).

<a name="fwgw"></a>
##### Step 4.8.3.6 T0 T1 Gateway Firewall

To recreate the firewall rules associated with your previous edges, please refer to our guide on [Gateway Firewall Management in NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-06-manage-gateway-firewall/).
...
To recreate your IPSec sessions, please refer to our guide on [setting up an IPsec Tunnel with NSX](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/nsx-12-configure-ipsec/).
...
Next, in the [OVHcloud Control Panel](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.co.uk/&ovhSubsidiary=GB), follow the instructions in our [Move an Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip/) guide to move the initial NSX-V block to the PCC service you are migrating, but specify the VIP IP of the T0 as the "next hop", as shown in the example below:

<img className="thumbnail" alt="NSX IP Migration" src="/images/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/service-migration-vdc/MoveIPNextHop.png" loading="lazy" />
...
The next step depends on the current configuration per [Virtual Protection Group](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/zerto-virtual-replication-as-a-service/):
...
To "move" Private Gateway to destination vDC, you must first disable it by following the steps in [Disable the private gateway](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/private-gateway#disablegw/).
...
Then enable it again by following the steps in [Enable the private gateway](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/private-gateway#enablegw/) and choose the datacentreId of the new vDC.
