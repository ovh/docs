...
For instructions for cross platforms set up, please see our guide on [using Zerto between OVHcloud and a third party platform](/guides/hosted-private-cloud/hosted-private-cloud-powered-by-vmware/zerto-virtual-replication-customer-to-ovh).
...
From `Actions`, select `Create VPG`
...
The last screen summarizes the settings for the new VPG. If everything is OK, click `DONE`.
...
From the interface, click on the bottom right button `FAILOVER` (the toggle is by default on **TEST**).
