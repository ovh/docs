...
| --- | --- | --- | --- | --- |
| **OCI API and compatibility** | Native OCI (Open Container Initiative) support for artifacts, images, Helm charts, Cosign signatures, and more. | OCI, Helm, Cosign (signatures), JSON | **Inbound**: Direct push of artifacts via standard tools (docker, helm, oras, cosign, etc.) or OCI API.<br/>**Outbound**: Pull/export of artifacts via the same tools or API to any other OCI/Harbor/Artifact Registry compatible registry. | [Migrate Helm Chart from Chartmuseum to OCI](/guides/public-cloud/containers-orchestration/managed-private-registry/migrate-helm-charts-from-chartmuseum-to-oci/)|
| **Import/Export Artifacts** | Upload and download artifacts (push/pull) via CLI/API standard Harbor/OCI | OCI, Helm, JSON | **Inbound**:Import via docker push, helm push, oras push, etc.<br/>**Outbound**: Export via docker pull, helm pull, oras pull, then push to the target. | [Artifact Import/Export](/guides/public-cloud/containers-orchestration/managed-private-registry/migrate-helm-charts-from-chartmuseum-to-oci/) |
