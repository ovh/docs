...
Se confirmar com `Enter` sem introduzir um nome, será utilizado o nome de ficheiro padrão (`id_rsa` neste exemplo).
...
Ao trabalhar com base numa linha de comandos **Windows**, utilize o botão direito do rato para **colar** o conteúdo da área de transferência na janela da linha de comandos. Para **copiar** um canal a partir da janela da linha de comandos, realce-o e prima `Enter`. Pode igualmente consultar estas funções através de um clique com o botão direito do rato na barra de menu da janela da linha de comandos.
