...
Para gerar o link de recuperação, aceda à [Área de Cliente OVHcloud](/links/manager), aceda à secção `Web Cloud`, clique em `Alojamentos` e, a seguir, no Cloud Web em causa. 

Selecione o separador `FTP - SSH` e clique no botão `Gerar um backup` à direita.

![backupftpcw](/images/assets/screens/control-panel/product-selection/web-cloud/cloud-web/ftp-ssh/generate-a-backup.png)

Na nova janela, selecione um dos backups disponíveis e clique em `Seguinte`.
...
Clique em `Confirmar` para validar o seu pedido.
