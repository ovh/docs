...
Clique no separador `Bases de dados` e em `Adicionar uma base de dados`.
...
- **Permissões** (apenas se a opção `Criar um utilizador` foi selecionada): trata-se das permissões atribuídas ao utilizador da base de dados. Para uma utilização normal, selecione `Administrador`. As permissões podem ser redefinidas a qualquer momento.
...
Clique no separador `Utilizadores e permissões` e, a seguir, em `Adicionar um utilizador`.
...
Depois de introduzir um «nome de utilizador» e uma «palavra-passe», clique em `Validar`. 
...
Para gerir as permissões de cada utilizador, aceda à [Área de Cliente OVHcloud](/links/manager). Clique no separador `Web Cloud` e, a seguir, em `Web Cloud Databases`. Selecione o nome do seu servidor de bases de dados. Clique no separador `Utilizadores e permissões`.

Clique no botão `...` à direita do utilizador em questão e, a seguir, em `Gerir as permissões`.
...
Para eliminar uma base de dados, aceda ao separador `Bases de dados` e clique no botão `...` à direita da base em causa. Por fim, clique em `Eliminar a base`.
