...
Aceda à [Área de Cliente OVHcloud](/links/manager) e aceda à secção `Web Cloud`. Clique no separador `Web Cloud Databases` na coluna à esquerda e selecione o nome do servidor de bases de dados.
...
![sql-privado](/images/assets/screens/control-panel/product-selection/web-cloud/web-cloud-databases/general-information/database-administration.png)

Será conduzido à página de conexão do phpMyAdmin.

![sql-privado](/images/assets/screens/other/web-tools/phpmyadmin/pma-login-web-cloud-db.png)

Introduza os seguintes elementos para se ligar à sua base de dados:

- **Servidor:** Indique o *nome do host* do seu servidor de bases de dados, seguido do *número da porta*. O *número da porta* deve ser separado do *nome do host* por um "**espaço**" ou por "**:**". Por exemplo, se o *nome do host* for **aaXXXXX-XXX.eu.clouddb.ovh.net** e o *número da porta* for **12345**, deverá introduzir-se **aaXXXXXXX-XXX.eu.clouddb.ovh.net:12345** ou **aaXXXXX-XXX.eu.clouddb.ovh.net:12345**. Para encontrar o *nome do host* e o *número da porta* do seu servidor Web Cloud Databases, aceda à [Área de Cliente OVHcloud](/links/manager) e aceda à secção `Web Cloud`. Clique no separador `Web Cloud Databases` na coluna à esquerda e selecione o nome do servidor de bases de dados. Na página `Informações gerais`, aparecerá o *nome do host* e o *número da porta* na caixa `Informações da ligação`.

- **Utilizador:** indique o *nome de utilizador* do servidor de bases de dados. Para encontrar o *nome de utilizador* da sua base de dados, aceda à [Área de Cliente OVHcloud](/links/manager) e aceda à secção `Web Cloud`. Clique no separador `Web Cloud Databases` na coluna à esquerda e selecione o nome do servidor de bases de dados. Na página que se abrir, clique no separador `Utilizadores e permissões`. Encontrará uma tabela com o conjunto dos utilizadores criados no seu serviço Web Cloud Databases.

- **Palavra-passe:** indique a *palavra-passe* associada ao *nome de utilizador* em questão. Se já não se lembra da *palavra-passe* associada ao seu *nome de utilizador*, aceda à [Área de Cliente OVHcloud](/links/manager) e aceda à secção `Web Cloud`. Clique no separador `Web Cloud Databases` na coluna à esquerda e selecione o nome do servidor de bases de dados. Na página que se abrir, clique no separador `Utilizadores e permissões`. Clique no botão `...` situado à direita de *o utilizador* em causa para `Alterar palavra-passe`.
...
![sql-privado](/images/assets/screens/other/web-tools/phpmyadmin/pma-main-page-web-cloud-db.png)
...
> [!warning]
>
> Se utiliza uma oferta "Web Cloud Databases"/"SQL Privado", não se esqueça de autorizar o seu IP com a ajuda do guia sobre a [configuração do seu servidor de base de dados](/guides/web-cloud/web-cloud-databases/db-configure-optimise-database-server).
>
...
- Lance o SQuirreL. Clique em `Aliases` e, a seguir, em `+`.

![lançar SQuirreL SQL](/images/assets/screens/other/web-tools/squirrel/aliases.png)

- Preencha os campos abaixo e valide clicando no botão `OK`:
...
![config conexao](/images/assets/screens/other/web-tools/squirrel/add-alias.png)

- Torne a validar clicando no botão `Connect`.

![valid conexao](/images/assets/screens/other/web-tools/squirrel/connect-to-mysql.png)

Já está conectado à base de dados:

![config conexao](/images/assets/screens/other/web-tools/squirrel/general-dashboard.png)
