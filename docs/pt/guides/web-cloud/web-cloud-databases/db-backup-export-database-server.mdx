...
Clique no botão `...` à direita da base de dados e, a seguir, em `Fazer backup agora`.
...
Quando surgir a lista dos backups, clique no botão `...` à direita do backup escolhido e em `Descarregar o backup` para o obter.
...
Para exportar uma base de dados diretamente a partir do phpMyAdmin, antes de mais tem de se conectar a ela. Se necessário, consulte o guia [«Conexão a uma base de dados do servidor de bases de dados»](/guides/web-cloud/web-cloud-databases/db-connecting-database-server).

Depois de se conectar ao phpMyAdmin, clique no nome da base de dados que deseja exportar e, de seguida, clique no separador `Exportar` ao alto.
...
> [!warning]
>
> - De modo a evitar acessos indesejados a este ficheiro com dados sensíveis, proteja-o recorrendo ao guia [Using .htaccess to password protect a directory on your website](/guides/web-cloud/web-hosting/htaccess-protect-directory-by-password).
