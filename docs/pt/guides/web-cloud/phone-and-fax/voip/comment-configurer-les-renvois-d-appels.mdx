...
- Disposer d'un accès à la gestion de la ligne SIP concernée depuis votre [espace client OVHcloud](/links/manager), partie `Télécom`.
...
### Activer le verrouillage des appels sortants <a id="verrouillage"></a>
...
1. Positionnez-vous sur l'onglet `Gestion des appels` et cliquez sur `Verrouillage des appels sortants`.

![gerer-appels-verrouillage](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-locking-step1.png)

Sur la page qui s'affiche, cochez la case `Activer le verrouillage` et renseignez un code à 4 chiffres. Celui-ci vous permettra d'émettre tout de même des appels sortants depuis la ligne que vous verrouillez.

Vous avez la possibilité d'appliquer ce verrouillage à plusieurs lignes grâce au bouton `Appliquer à plusieurs lignes`. Cliquez finalement sur le bouton `Valider` pour appliquer la nouvelle configuration.

![gerer-appels-verrouillage](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-locking-step2.png)

### Activer le renvoi d'appel <a id="renvoi"></a>
...
1. Cliquez sur `VoIP & Fax` puis sur le groupe de facturation contenant votre ligne SIP.
1. Cliquez sur l'onglet `Services` puis sur la ligne SIP concernée.
1. Positionnez-vous sur l'onglet `Gestion des appels` et cliquez sur `Renvoi d'appel`.

![gerer-appels-renvoi](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-fowarding-step1.png)
...
Vous avez la possibilité d'appliquer vos choix de configuration à plusieurs lignes, grâce au bouton `Appliquer à plusieurs lignes`. Cliquez enfin sur le bouton `Valider` pour terminer l'opération.
...
![gerer-appels-renvoi](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-fowarding-step2.png)
...
### Activer le filtrage d'appels <a id="filtrage"></a>

1. Connectez-vous à votre [espace client OVHcloud](/links/manager) et cliquez sur `Télécom`.
1. Cliquez sur `VoIP & Fax` puis sur le groupe de facturation contenant votre ligne SIP.
1. Cliquez sur l'onglet `Services` puis sur la ligne SIP concernée.
1. Positionnez-vous sur l'onglet `Gestion des appels` et cliquez sur `Filtrage d'appels`.

![gerer-appels-filtrage](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-filtering-step1.png)
...
![gerer-appels-filtrage](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-filtering-step2.png)
...
|Configurations|Description|
|---|---|  
|Ajout en mode manuel|Vous devrez remplir les informations demandées dans la partie « Configurer de nouveaux filtrages » selon ce que vous souhaitez mettre en place, puis cliquer sur le bouton `Ajouter`. Ce mode vous permet également de filtrer les « services OVHcloud » (tels que le service de messagerie).|
|Utiliser un assistant|L'assistant vous permet de créer des filtres sur des types de numéros (comme les numéros spéciaux). Ce filtre sera ajouté à la liste que vous avez sélectionnée lors de la complétion de l'assistant. Pour accéder à celui-ci, cliquez sur `Assistant`.|
|Importer un CSV|Vous devrez sélectionner sur votre propre machine le fichier CSV contenant les filtres que vous souhaitez importer. Pour accéder à l'import, cliquez sur `Import CSV`.|

Enfin, dans la partie « Configuration actuelle », vous pouvez visualiser les filtres en place sur vos listes mais aussi les supprimer. Pour cela, sélectionnez un ou plusieurs filtres puis cliquez sur le bouton `Supprimer le(s) filtre(s) sélectionné(s)`.
...
![gerer-appels-filtrage](/images/guides/web-cloud/phone-and-fax/voip/comment-configurer-les-renvois-d-appels/manage-calls-filtering-step3.png)
