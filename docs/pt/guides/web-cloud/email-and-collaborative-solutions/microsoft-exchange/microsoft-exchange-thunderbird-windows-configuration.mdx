...
> 2. Acesse a seção `Web Cloud`.
> 3. Na seção `MICROSOFT`, clique em `Exchange`.
> 4. Selecione a plataforma desejada.
> 5. O nome do servidor está visível no quadro **Ligação** da aba `Informações gerais`.
...
Clique em `Continuar` para concluir as configurações.
...
Quando o Thunderbird detectar um domínio OVHcloud, uma configuração automática relativa à oferta MX Plan será proposta. Clique em `MODIFICAR A CONFIGURAÇÃO`.
...
2. Clique em `Continuar` para confirmar estas configurações.
...
Insira a senha associada ao endereço de e-mail, em seguida clique em `Continuar` para finalizar a configuração.
...
- Para modificar as configurações relacionadas à **recepção** de seus e-mails, clique em `Configurações do servidor` na coluna à esquerda sob seu endereço de e-mail.
...
- Para modificar as configurações relacionadas ao **envio** de seus e-mails, clique em `Servidor de saída (SMTP)` no final da coluna à esquerda.
- Clique no endereço de e-mail desejado na lista, em seguida clique em `Editar`.
