...
### Adicionar a conta <a id="add-account"></a>
...
>
> Encontre este algarismo na sua área de cliente OVHcloud(/links/manager), na rubrica `Web Cloud` e depois `E-mail Pro`. O nome do servidor está visível na tabela **Ligação** do separador 'Informações gerais`.
...
- **Se uma conta já tiver sido parametrizada**:
    ...
    - De seguida, prima o botão `+` na barra vertical à esquerda.
    - Prima `Adicionar uma conta`.
...
Introduza o seu endereço de e-mail e prima `Continuar`.

![Outlook e-mail pro](/images/guides/web-cloud/email-and-collaborative-solutions/email-pro/outlook-android-configuration/outlook-app-android-add-step01.png)
...
[!warning]
> 
> Se a janela de escolha do protocolo não aparecer, prima o botão `?` no canto superior direito do ecrã e escolha `Mudar de fornecedor de conta`. Selecione `IMAP`(recomendado) ou `POP3`.<br />
...
Na seguinte janela, selecione `Configurações avançadas` e introduza as seguintes informações:
...
### Alterar os parâmetros existentes <a id="modify-settings"></a>
...
- Prima `Eliminar conta`.
- Toque em `Eliminar` à pergunta "Deseja eliminar a conta?".
...
### Lembrete dos parâmetros POP, IMAP e SMTP <a id="popimap-settings"></a>
