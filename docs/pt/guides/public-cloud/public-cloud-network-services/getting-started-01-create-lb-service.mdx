...
- Understanding the [Load Balancer concepts](/guides/public-cloud/public-cloud-network-services/concepts-03-loadbalancer/)
- Understanding the [Public Cloud Networking concepts](/guides/public-cloud/public-cloud-network-services/concepts-01-public-cloud-networking-concepts/)
- A Load Balancer requires a subnet, read [this guide](/guides/public-cloud/public-cloud-network-services/getting-started-09-creating-vrack-with-openstack/) for more information
- (Optional): This guide explains the load balancer configuration through Graphical Interface & Command Line Interface. If you want to use the latter, then install the [OpenStack Command Line environment](/guides/public-cloud/public-cloud-cross-functional/prepare-the-environment-for-using-the-openstack-api/)
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    ...
    **Step 1: Region choice**
    ...
    <img className="thumbnail" alt="Region choice" src="/images/public-cloud/public-cloud-network-services/getting-started-01-create-lb-service/region.png" loading="lazy" />
    ...
    **Step 2: Size choice**
    ...
    <img className="thumbnail" alt="Size choice" src="/images/public-cloud/public-cloud-network-services/getting-started-01-create-lb-service/size.png" loading="lazy" />
...
At this stage, you need to know if your Load Balancer will be receiving public traffic or not (for more details, read our [Public Cloud Networking concepts](/guides/public-cloud/public-cloud-network-services/concepts-01-public-cloud-networking-concepts/) page).
    ...
    **Step 4: Select the private network and the subnet where the Load Balancer will be spawned**
    ...
    <img className="thumbnail" alt="Network choice" src="/images/public-cloud/public-cloud-network-services/getting-started-01-create-lb-service/private_network.png" loading="lazy" />

The interface will inform you if the private network / subnet is not compliant with the prerequisites (see [Public Cloud Networking concepts](/guides/public-cloud/public-cloud-network-services/concepts-03-loadbalancer#network-prerequisites/)).
    
    **Step 5 (optional): Define the listener(s) and the members**
    
    <img className="thumbnail" alt="Listener choice" src="/images/public-cloud/public-cloud-network-services/getting-started-01-create-lb-service/listener.png" loading="lazy" />
    
    - First, choose the listener protocol & the port according to the traffic you will receive. Note that a specific listener called `Prometheus` is available to monitor your Load Balancer. In that case, it is not possible to add members. For more information on this listener, read [this page](/guides/public-cloud/public-cloud-network-services/technical-resources-02-octavia-monitoring-prometheus/).
    - Then, choose the Health Monitor type. Note that since some health monitor types are not compatible with some protocols, the user interface filters those types so that you can only choose compatible items. For more information on the health monitor compatibility, read [this page](/guides/public-cloud/public-cloud-network-services/concepts-01-public-cloud-networking-concepts/).
...
You can update the name according to your choice and click on <code className="action">Create a Load Balancer</code>.
    ...
    You will be redirected to the Load Balancer listing page. Among the attributes that are displayed, the `Operating status` and `Provisioning status` provide information on the state of your load balancer. Find more information on the "[Load Balancer concepts](/guides/public-cloud/public-cloud-network-services/concepts-03-loadbalancer#operating-provisioning-status/)" page.
  </Tab>
  <Tab label="Via the OpenStack CLI">
    ...
    **Configuring your private network**
    ...
    You can now attach your instances to the new network. We recommend following our guide to [integrate an instance into vRack](/guides/public-cloud/public-cloud-network-services/getting-started-07-creating-vrack#instance-integration/).
