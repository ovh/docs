...
No menu à esquerda, aceda à secção **Network** e abra a parte `IP públicos`. Clique no separador `Additional IP`.

Neste exemplo, o endereço Additional IP rodado para "Instância_A" será migrado para "Instância_B".

Clique em `...` na linha do Additional IP e selecione `Alterar a instância associada`.
