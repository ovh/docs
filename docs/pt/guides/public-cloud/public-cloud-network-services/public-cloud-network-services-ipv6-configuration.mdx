...
> [!info]
> 
> Atualmente, as ofertas Floating IP e Gateway não tomam a cargo o IPv6. O IPv6 só pode ser utilizado com as instâncias em [modo público](/guides/public-cloud/public-cloud-network-services/concepts#publicmode).
>
...
Aceda à Área de Cliente, clique no menu da secção `Public Cloud`, selecione o seu projeto Public Cloud e clique em `Instâncias` no menu à esquerda. Depois clique no `...` ao lado da instância correspendente e clique em `Detalhes da instância`.
...
Aceda à secção `Ligações de rede` do seu Windows.
...
A seguir, clique em `Internet Protocol Version 6 (TCP/IPv6)` e, a seguir, no botão `Propriedades`.
...
Uma vez terminado, selecione a opção `Validar os parâmetros à saída` e clique no botão `OK` para validar as suas alterações.
