...
>
> **Os detalhes técnicos** importantes relativos ao Public Cloud da OVHcloud estão disponíveis em [esta página](/guides/public-cloud/public-cloud-cross-functional/compute-essential-information).
>
...
Pode armazenar as suas chaves SSH públicas na secção `Public Cloud` da sua [Área de Cliente OVHcloud](/links/manager). Não é obrigatório, mas torna o processo de criação de instâncias mais prático.
...
Abra o `Chaves SSH` no menu à esquerda em **Parâmetros**. Clique no botão `Adicionar chave SSH`.

![ssh keys](/images/assets/screens/control-panel/product-selection/public-cloud/cp-pci-sshkeys.png)

Na nova janela, introduza um nome para a chave. Preencha o campo `Chave` com a sua cadeia de chave pública, por exemplo, a criada na [etapa 1](#create-ssh). Confirme clicando em `Adicionar`.
...
- Se a instância estiver ligada a uma nova rede privada (OVHcloud [vRack](/links/network/vrack)), **crie o seu vRack** antes de continuar. Consulte os detalhes no [manual sobre o serviço vRack Public Cloud](/guides/public-cloud/public-cloud-network-services/public-cloud-network-services-vrack).
...
Para mais informações, consulte os nossos manuais na secção [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts). O [guia de conceitos](/guides/public-cloud/public-cloud-network-services/concepts) fornece uma introdução ao Public Cloud Networking.
...
Na página **Página Inicial**, clique em `Criar uma instância`.
...
Na secção `Discovery`, propomos-lhe modelos de instâncias com recursos partilhados a preços vantajosos. Elas são ideais para testar o Public Cloud em geral ou uma aplicação web por exemplo.

Os modelos de instâncias do tipo `Metal` fornecem recursos físicos dedicados.

> [!info]
>
> O total dos seus recursos Public Cloud será inicialmente limitado por razões de controlo de custos e de segurança. Pode verificar estas quotas clicando em `Limite e regiões` na barra de navegação à esquerda em **Parâmetros**. Consulte [a documentação dedicada](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota) para mais informações.
>
...
As Local Zones são uma extensão das **regiões** que aproximam os serviços da OVHcloud de sites específicos, oferecendo uma latência reduzida e performances melhoradas para as aplicações. Pode encontrar mais informações na [página Web das Local Zones](/links/public-cloud/local-zones) e na [documentação das capacidades das Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).
...
Consulte também a [página Web das Locais Zones](/links/public-cloud/local-zones) e a [documentação das capacidades das Locais Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).

![seleção de região](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation03.png)
...
<Tab label="Introduza uma chave">

Para adicionar uma chave pública colando a cadeia de chaves, clique no botão `Adicionar chave`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation06.png)<br />
Introduza um nome para a chave e cadeia de chaves nos respetivos campos. A seguir, clique em `Seguinte`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation07.png)<br />
Antes de clicar em `Seguinte`, pode optar por utilizar o botão `Adicionar chave` para armazenar esta chave na Área de Cliente OVHcloud (ver [etapa 2](#import-ssh) para mais informações).
...
<Tabs>
<Tab label="1: Número de instâncias a criar">

Pode criar várias instâncias em função das seleções efetuadas nas etapas de criação, mas [os limites de quota de recursos](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota) aplicar-se-ão.<br />
  </Tab>
<Tab label="2: Instância flexível">
...
<Tab label="3: Nome da instância">

Introduza um nome completo para a sua instância. A referência comercial do modelo de instância é o valor predefinido.<br />
  </Tab>
<Tab label="4: Script de pós-instalação">

É possível adicionar [o seu script](/guides/public-cloud/compute/key-concepts) neste campo.<br />
  </Tab>
<Tab label="5: Backup automático das instâncias">

Pode ativar o serviço [Backups automatizados](/guides/public-cloud/compute/save-an-instance) selecionando esta opção. Queira consultar as informações tarifárias e os pormenores complementares.
...
Atenção: se clicar em `Criar uma nova rede privada`, o processo de criação da instância será interrompido e deverá ser reiniciado desde o início.<br />
  </Tab>
...
Atenção: se clicar em `Criar uma nova rede privada`, o processo de criação da instância será interrompido e deverá ser reiniciado desde o início.
...
![Modo de faturação](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation14.png)
...
A faturação à hora é a melhor escolha se não definiu claramente a duração do período de utilização. Se decidir conservar a instância para uma utilização a longo prazo, poderá sempre [passar para uma subscrição mensal](/guides/account-and-service-management/managing-billing-payments-and-services/changing-hourly-monthly-billing).<br />
...
Uma vez terminada a configuração da instância, clique no botão `Criar uma instância`. O serviço poderá demorar alguns minutos a ser entregue.
...
> [!info]
>
> Se instalou um SO **OS com aplicação**, consulte o nosso [guia de primeiros passos com as aplicações](/guides/public-cloud/compute/apps-first-steps) bem como a documentação oficial do editor do SO.
>
...
Selecione `Instâncias` na barra de navegação à esquerda em **Compute**. A sua instância está pronta quando o estado está definido em `Ativado` na tabela. Se a instância tiver sido criada recentemente e tiver um estado diferente, clique no botão "Atualizar" junto do filtro de pesquisa.

![page instâncias](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-connect01.png)

Clique no nome da instância neste quadro para abrir o `Dashboard` no qual pode encontrar todas as informações relativas à instância. Para saber mais sobre as funções disponíveis nesta página, consulte o guia [Gestão das instâncias na Área de Cliente](/guides/public-cloud/compute/first-steps-with-public-cloud-instance).

Um **utilizador com permissões elevadas (*sudo*) é automaticamente criado** na instância. O nome de utilizador reflete a imagem instalada, por exemplo "ubuntu", "debian", "fedora", etc. Pode verificá-lo no lado direito do `Dashboard` na secção **Redes**.

![page instâncias](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-connect02.png)
...
>
> Este manual não cobre a rede privada para as instâncias. Consulte a nossa documentação [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts) sobre este assumpto.
>
...
> [!info]
>
> Se receber mensagens de erro sobre as suas **chaves SSH**, verifique se o seu dispositivo local dispõe de uma chave SSH privada corretamente configurada utilizando as informações de [este guia](/guides/public-cloud/compute/compute-creating-ssh-keys#create-ssh-key).<br />
> Se continuar a ter problemas, pode substituir o par de chaves utilizando [este manual](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
> Se criou uma instância sem chave SSH através da [API OVHcloud](/guides/manage-and-operate/api/first-steps) ou da [interface OpenStack Horizon](/guides/public-cloud/compute/create-instance-in-horizon), só pode adicionar uma chave SSH à sua instância através do [modo rescue](/guides/public-cloud/compute/put-an-instance-in-rescue-mode) seguindo as instruções descritas em [este manual](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
...
[Em função da sua configuração](#create-ssh), deverá introduzir uma frase secreta que proteja a sua chave privada ou especificar o caminho de acesso ao seu ficheiro de chave. Consulte o nosso [guia das chaves SSH](/guides/public-cloud/compute/compute-creating-ssh-keys#multiplekeys) para informações detalhadas sobre este assumpto.

Se estiver a utilizar outro software cliente SSH, consulte a documentação do utilizador. Está disponível um exemplo de utilização da solução open source `PuTTY` em [este manual](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows).
...
Depois de verificar que a instância Windows está [instalada](#verify-status), abra o separador `Consola VNC` no seu [Área de Cliente OVHcloud](/links/manager).
...
Configure o seu **país/região**, o **idioma preferido do Windows** e a sua **configuração do teclado**. A seguir, clique no botão `Next` no canto inferior direito.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-locale.png)<br />
  </Tab>
<Tab label="2. Palavra-passe de administrador">

Defina uma palavra-passe para a sua conta Windows `Administrator` e confirme-a e clique em `Finish`.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-admin.png)<br />
  </Tab>
<Tab label="3. Ecrã de ligação">

O Windows aplicará as suas configurações e, em seguida, exibirá a tela de login. Clique no botão `Send CtrlAltDel` no canto superior direito para iniciar sessão.<br /><br />
...
Introduza o endereço IPv4 da sua instância, depois o seu identificador e a sua passphrase. Normalmente, é apresentada uma mensagem a avisar-lhe para confirmar a ligação devido a um certificado desconhecido. Clique em `Sim` para iniciar sessão.
...
Se aparecer uma mensagem de aviso de certificado, clique em `Yes`. Introduza o nome de utilizador e a palavra-passe para o Windows e clique em `OK` para iniciar sessão.<br /><br />
![linux remote](/images/guides/public-cloud/compute/public-cloud-first-steps/24-rem-connect03.png)<br />
  </Tab>
<Tab label="3. Parâmetros">

Pode encontrar elementos úteis na barra de ferramentas à esquerda. Por exemplo, clique no ícone `Toggle dynamic resolution update` para melhorar a resolução da janela.<br /><br />
...
Selecione `Instâncias` na barra de navegação à esquerda em **Compute**. Clique no nome da instância e abra o separador `Consola VNC`.

![consola vnc](/images/assets/screens/control-panel/product-selection/public-cloud/cp-pci-vnc-login.png)


<Tabs>
<Tab label="Instance com um SO GNU/Linux instalado">
...
Consulte o nosso [manual dedicado](/guides/public-cloud/compute/configuring-additional-ssh-keys) para uma explicação pormenorizada destas etapas.
...
[Como iniciar com OpenStack](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment)
