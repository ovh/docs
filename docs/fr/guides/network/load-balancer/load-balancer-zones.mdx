...
![Fonctionnement avec plusieurs zones et plusieurs fermes](/images/guides/network/load-balancer/zones/multi_zones_multi_backends.png)
...
![Différence entre les déploiements multi-régions et multi-AZ](/images/guides/network/load-balancer/zones/multi-az.png)
...
Vous pouvez commander une zone supplémentaire depuis l'[espace client OVHcloud](/links/manager). Dans la section `Network`, sous `Services réseau` cliquez sur `Load Balancer`.

Sélectionnez votre Load Balancer, puis dans l'onglet `Accueil`, section `Configuration`, cliquez sur le bouton `...` sur la droite de "Zones de disponibilité" et choisissez `Ajouter`.

![Ajout d'une zone Load Balancer depuis le manager](/images/guides/network/load-balancer/zones/add_Zone_IPLB.png)

Ensuite, sélectionnez la (ou les) zone(s) que vous souhaitez commander et cliquez sur `Ajouter`.

![Sélection d'une zone Load Balancer depuis le manager](/images/guides/network/load-balancer/zones/Select_Zone_IPLB.png)

Une commande sera générée, que vous devrez régler.

![Paiement de la commande de zone Load Balancer depuis le manager](/images/guides/network/load-balancer/zones/Paybill_Zone_IPLB.png)
...
Sélectionnez le Load Balancer que vous souhaitez modifier, puis créez un nouveau frontend, ou modifiez-en un existant, via l'onglet `Frontends`.

Dans le champ `Datacenter`, choisissez la zone que vous souhaitez associer à votre frontend. 
...
![Sélection de la zone](/images/guides/network/load-balancer/zones/Select-Datacenter.png)

Une fois le frontend configuré, cliquez sur `Ajouter` ou `Modifier`, selon que vous configurez un nouveau frontend ou modifiez un existant.

N'oubliez pas de déployer la configuration. Pour cela, cliquez sur `Appliquer la configuration` dans la bannière rappelant que la configuration n'est pas appliquée.

![Application d'une configuration Load Balancer](/images/guides/network/load-balancer/zones/apply-configuration.PNG)
