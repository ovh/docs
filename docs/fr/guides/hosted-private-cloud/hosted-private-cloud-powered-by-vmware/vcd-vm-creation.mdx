...
Pour créer votre machine virtuelle, accédez d'abord au datacenter où vous prévoyez de la déployer. Ensuite, naviguez vers la section `Calculer` > `Machines Virtuelles` > `Nouvelle VM`.
...
En ce qui concerne la section `Stratégies de stockage`, vous ne trouverez qu'une seule possibilité : la politique de stockage par défaut de vSAN.
...
Pour conclure, veuillez examiner toutes les informations fournies pour vous assurer de leur exactitude. Cliquez ensuite sur `OK` pour lancer le déploiement de la machine virtuelle. Une fois le déploiement terminé, vous verrez la nouvelle VM répertoriée dans le centre de données virtuel sélectionné.
