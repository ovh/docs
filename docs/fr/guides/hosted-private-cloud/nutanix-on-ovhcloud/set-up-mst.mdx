...
Connectez-vous à Prism Central, rendez-vous dans le centre d'administration, puis cliquez sur `Enable Marketplace`.
...
Depuis `Infrastructure`, rendez-vous dans `Prism Central Settings`, cliquez sur `Network Controller` puis sur `Enable`.
...
1. Dans **Nutanix Apps**, cliquez sur `Get` pour **Multicloud Snapshot Technology**.

![11 get mst](/images/guides/hosted-private-cloud/nutanix-on-ovhcloud/81-multi-snapshot-technologie/mst11.png)

Sur la page d'introduction, cliquez sur `Deploy`.
...
- **MST Instance Size** : Sélectionnez `Small` (ou une autre taille si disponible).
...
- **Object Store Provider** : Sélectionnez `Nutanix Object Store`.
...
4. Protéger les VM :
    ...
    Selectionnez les VM à protéger puis dans le menu `action` → `data protection`, cliquez sur `Protect`.
