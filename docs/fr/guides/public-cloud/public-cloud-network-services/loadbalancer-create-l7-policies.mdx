...
- Comprendre les [concepts du OVHcloud Load Balancer](/guides/public-cloud/public-cloud-network-services/load-balancer-concepts).
- Un [projet Public Cloud OVHcloud](/guides/public-cloud/public-cloud-cross-functional/create-a-public-cloud-project).
- Un Load Balancer [déjà configuré](/guides/public-cloud/public-cloud-network-services/getting-started-load-balancer) dans votre projet Public Cloud.
...
- **CLI OpenStack :** pour la gestion via la ligne de commande. Retrouvez plus d'informations pour préparer votre environnement API OpenStack sur [ce guide](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment).
- **Interface Horizon :** offre une gestion graphique via OpenStack pour les utilisateurs familiarisés avec cette plateforme. Retrouvez plus de détails supplémentaires sur [cette page](/guides/public-cloud/public-cloud-cross-functional/introducing-horizon).
...
Cliquez sur l'onglet `Public Cloud`. Sélectionnez le projet Public Cloud souhaité si vous en avez plusieurs.

Sélectionnez `Load Balancer`, vous verrez apparaître la liste de vos Load Balancers. Cliquez sur celui que vous souhaitez configurer.

#### Etape 3 - Gérer les Politiques L7

Après avoir sélectionné votre Load Balancer, vous accéderez à son interface de gestion. Cliquez sur l'onglet `Listeners` pour afficher la liste des listeners.

Pour accéder aux options de gestion des politiques L7 d'un listener spécifique, cliquez sur le bouton `...` situé à droite du listener que vous souhaitez configurer. 
...
Sur la page des politiques L7 de votre listener, localisez la politique pour laquelle vous souhaitez ajouter des règles. Cliquez sur le bouton `...` à côté de la politique concernée et sélectionnez `Gérer les L7 rules`.
...
- Pour vous connecter avec l'authentification unique OVHcloud : utilisez le lien `Horizon` dans le menu de gauche sous « Interfaces de gestion » après avoir ouvert votre projet `Public Cloud` dans l'[espace client OVHcloud](/links/manager).

- Pour vous connecter avec un utilisateur OpenStack spécifique : ouvrez la [page de connexion à Horizon](https://horizon.cloud.ovh.net/auth/login/) et entrez les [identifiants de l'utilisateur OpenStack](/guides/public-cloud/public-cloud-cross-functional/create-and-delete-a-user) précédemment créées, puis cliquez sur `Se connecter`.

Dans le tableau de bord d'Horizon, sélectionnez votre `Projet`.<br />
Dans le menu de gauche, cliquez sur l'onglet `Network` et sélectionnez `Load Balancers`.<br />
