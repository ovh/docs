...
>
> Retrouvez plus d'informations dans notre [guide sur la création d'une instance Public Cloud](/guides/public-cloud/compute/compute-getting-started).
...
`Ed25519` est considéré comme le plus sûr mais `RSA` est une alternative valable. Les deux méthodes sont compatibles avec l'[espace client OVHcloud](/guides/public-cloud/compute/compute-getting-started).
...
Si vous confirmez avec le bouton `Entrée` sans saisir de nom, le nom de fichier standard sera utilisé (`id_rsa` dans cet exemple).
...
Copiez cette chaîne de clé pour [l'ajouter à une nouvelle instance ou l'importer dans votre espace client](/guides/public-cloud/compute/compute-getting-started).
...
Si vous confirmez avec la touche `Entrée` sans saisir de nom, le nom de fichier standard sera utilisé (`id_rsa` dans cet exemple).
...
Vous pouvez ouvrir le fichier de clés à l'aide d'un éditeur de texte (Notepad, Notepad++, etc.). Depuis l'explorateur de fichiers Windows, faites un clic droit sur le fichier et sélectionnez `Ouvrir avec`.
...
:::info Utilisation du Presse-papiers
...
[Comment créer une instance Public Cloud et s'y connecter](/guides/public-cloud/compute/compute-getting-started)
