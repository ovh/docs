...
For this tutorial, we will load test a spam classifier API from the [AI Deploy app portfolio](/guides/public-cloud/ai-machine-learning/deploy-guide-05-app-portfolio/).
This API takes sentences (emails) as input text, and outputs a spam probability score. 

You can deploy this API easily from the [OVHcloud control panel](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.com/fr/&ovhSubsidiary=fr) or [OVHcloud CLI](/guides/public-cloud/ai-machine-learning/cli-10-howto-install-cli/). A good strategy is to deploy with autoscaling, with minimum and maximum replicas. This way we will monitor the growth of used replicas.
...
:::info
To be able to connect to your AI Deploy app, you have to [create a token](/guides/public-cloud/ai-machine-learning/cli-13-howto-app-token-cli/) bearer for your OVHcloud AI user.
...
- We put the token to access our API, generated via the OVHcloud Control Panel or `ovhai` CLI. We specify it in the header of the request. If you want to know more about generation and use of tokens, you can follow [this tutorial](/guides/public-cloud/ai-machine-learning/deploy-guide-03-tokens/).
