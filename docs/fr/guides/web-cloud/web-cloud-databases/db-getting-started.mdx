...
Depuis l'onglet `Informations générales`, dans le cadre `Configuration`, cliquez sur le bouton `...` à droite de **Web Cloud Databases**. Cliquez enfin sur `Activer` pour lancer le processus d'activation.

![Informations générales](/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/general-information/web-cloud-databases-enable.png)

</Tab>
<Tab label="Etape 4">

Enfin, suivez les instructions fournies afin de déterminer le type et la version de votre serveur Web Cloud Databases. Il sera ensuite accessible depuis la colonne de gauche dans `Web Cloud Databases`.
...
Dans la barre de services à gauche de votre [espace client OVHcloud](/links/manager), rendez-vous dans la section `Web Cloud Databases`, puis sur l'instance concernée. Assurez-vous d'être bien positionné sur l'onglet `Informations générales`.
...
Pour créer votre première base de données sur votre instance Web Cloud Databases, cliquez sur l'onglet `Bases de données` puis sur le bouton `Ajouter une base de données`.
...
Si votre projet ne nécessite pas un utilisateur additionnel, vous pouvez passer à la manipulation suivante. Dans le cas contraire, pour créer un utilisateur sur votre instance Web Cloud Databases, cliquez sur l'onglet `Utilisateurs et droits` puis sur le bouton `Ajouter un utilisateur`.
...
Sur la fenêtre qui s'affiche, complétez maintenant les informations demandées puis cliquez sur `Valider`.
...
Depuis l'onglet `Bases de donées`, cliquez sur le pictogramme en forme de roue dentée puis sur `Importer un fichier`. Sur la fenêtre qui s'affiche, cochez la case `Importer un nouveau fichier` puis cliquez sur `Suivant`.
...
![Web Cloud Databases](/images/assets/screens/control-panel/product-selection/web-cloud/web-cloud-databases/databases/database-import-new-file-step-1.png)
...
Afin que l'accès à votre instance Web Cloud Databases fonctionne, il est obligatoire d'indiquer les IP ou plages d'IP pouvant se connecter à vos bases de données. Pour cela, cliquez sur l'onglet `IP autorisées` puis sur le bouton `Ajouter une adresse IP/masque`.
...
Sur la fenêtre qui s'affiche, indiquez l'adresse IP ou le masque que vous désirez autoriser dans `IP/masque` puis ajoutez une description si vous le souhaitez. Décidez ensuite si vous voulez donner accès uniquement aux bases de données ou au SFTP. Enfin, cliquez sur `Valider`.
...
### Autoriser la connexion à un hébergement web OVHcloud <a id="trustip"></a>
...
Pour cela, cliquez sur l'onglet `IP autorisées` puis sur le bouton `Accès aux hébergements web OVHcloud`.
...
|Information|Description|
|---|---|
|Nom de la base de données|Il s'agit du nom que vous avez défini lors de la création de la base de données. Vous pouvez retrouver toutes vos bases de données créées sur votre instance Web Cloud Databases depuis l'onglet `Bases de données`.|
|Nom d'utilisateur|Il s'agit du nom d'utilisateur que vous avez défini lors de la création de la base de données ou d'un éventuel utilisateur additionnel que vous auriez ajouté. Vous pouvez retrouver tous les utilisateurs créés sur votre instance Web Cloud Databases depuis l'onglet `Utilisateurs et droits`.|
...
|Port du serveur|Il s'agit du port de connexion à votre instance Web Cloud Databases pour que votre site puisse se connecter à votre base de données. Cette information est accessible dans votre espace client, depuis le cadre **Informations de connexion** de l'onglet `Informations générales`.|

> [!warning]
>
> Le champ `port` peut ne pas être proposé dans la configuration de votre site. Vous devrez ajouter ce champ après le nom d'hôte de votre serveur en les séparant par un *:* .<br /><br />
...
Pour accéder aux logs de votre solution Web Cloud Databases, consultez notre guide « [Web Cloud Databases - Comment récupérer les logs ?](/guides/web-cloud/web-cloud-databases/db-retrieve-logs) ».

## Aller plus loin

[Créer vos bases de données et vos utilisateurs sur votre serveur de bases de données](/guides/web-cloud/web-cloud-databases/db-create-databases-users)

[Se connecter à la base de données de votre serveur de bases de données](/guides/web-cloud/web-cloud-databases/db-connecting-database-server)

[Sauvegarder et exporter une base de données sur votre serveur de bases de données](/guides/web-cloud/web-cloud-databases/db-backup-export-database-server)

[Restaurer et importer une base de données sur votre serveur de bases de données](/guides/web-cloud/web-cloud-databases/db-restore-import-database)

[Configurer votre serveur de bases de données](/guides/web-cloud/web-cloud-databases/db-configure-optimise-database-server)
