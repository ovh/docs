...
Sur la page qui s'affiche, cliquez sur l'onglet `IPs autorisées`.
...
Sur la page qui apparaît, cliquez sur le bouton `Ajouter une adresse IP / masque` situé au-dessus du tableau.
...
> [!tip]
>
> Si vous souhaitez modifier une adresse IP ou une plage d'adresses IP déjà autorisée, cliquez directement dans le tableau sur le bouton `...` situé à droite de la ligne correspondant à l'adresse IP ou à la plage d'adresses IP à modifier, puis sur `Éditer la whitelist`.
...
- `IP / masque *` : Saisissez ici l'adresse IP (par exemple : `203.0.113.44`) ou la plage d'adresses IP (par exemple : `203.0.113.0/24` représentant toutes les adresses IP de `203.0.113.0` à `203.0.113.255`) que vous souhaitez autoriser sur votre solution Web Cloud Databases.
- `Description` (facultatif) : Vous pouvez, par exemple, y ajouter des informations sur le rôle de l'adresse IP ou de la plage d'adresses IP concernée.
- `Bases de données` : Cochez cette case pour que l'adresse IP ou la plage d'adresses IP soit autorisée à accéder aux bases de données présentes sur votre solution Web Cloud Databases.
- `SFTP` : Cochez cette case pour que l'adresse IP ou la plage d'adresses IP soit autorisée à accéder aux logs de votre solution Web Cloud Databases.

> [!warning]
>
> Il est fortement déconseillé de cocher la case `Bases de données` pour autoriser la plage d'adresses IP `0.0.0.0/0` à accéder à vos bases de données.
>
...
Sur la page qui apparaît, décochez la case précédant la mention `Autoriser les hébergements web OVHcloud à accéder à la base de données`.
