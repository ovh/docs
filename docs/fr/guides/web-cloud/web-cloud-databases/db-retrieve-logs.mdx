...
3. Dans la colonne de gauche, cliquez sur le menu `Web Cloud Databases`.
4. Sélectionnez l'instance Web Cloud Databases concernée.
5. Sur la page qui s'affiche, cliquez sur l'onglet `Logs`.
...
> [!info]
>
> Comme précisé ci-dessus, les logs ne sont disponibles ici qu'en temps réel. Cela signifie que ces logs n'apparaîtront que s'ils sont générés au moment où vous vous trouvez sur l'onglet `Logs`. 
>
...
>
> Pour vérifier cela, récupérez l'adresse IP publique de votre point d'accès à Internet puis consultez la section **Autoriser une adresse IP** de [ce guide](/guides/web-cloud/web-cloud-databases/db-getting-started).
>
...
5. Sur la page qui s'affiche, restez sur l'onglet `Informations générales` puis positionnez-vous au niveau de l'encadré intitulé `Informations de connexion`.
6. En dessous de la mention `SFTP`, vous retrouverez l'ensemble des informations nécessaires pour vous connecter en SFTP.

> [!info]
>
> Si vous ne connaissez pas le `Mot de passe du serveur`, cliquez sur le bouton `...` à droite pour le modifier.
>
...
Pour FileZilla, rendez-vous en haut à gauche dans le menu `Fichier`, puis cliquez sur `Gestionnaire de sites`.

Cliquez sur `Nouveau site` puis saisissez les paramètres relevés précédemment.
...
### Abonner les logs de votre solution Web Cloud Databases à Logs Data Platform <a id="wcdb-ldp"></a>
...
4. Sélectionnez l'instance Web Cloud Databases concernée.
5. Sur la page qui s'affiche, cliquez sur l'onglet `Logs`.
6. Sur la droite de l'encadré où s'affichent vos logs en temps réel, cliquez sur le bouton `S'abonner`.
...
#### Cas n°1 - S'abonner à un flux déjà existant sur votre solution Logs Data Platform <a id="wcdb-ldp-case1"></a>

Si le flux concerné existe déjà, celui-ci apparaît sous la forme d'une ligne dans le tableau situé en bas de page.
Dans ce cas précis et pour abonner votre solution Web Cloud Databases à ce flux existant, il vous suffit de cliquer sur le bouton `S'abonner` situé à droite de la ligne correspondant au flux concerné.
...
Si le flux concerné n'existe pas encore, cliquez sur le bouton `Ajouter un flux de données`.
...
Pour effectuer cela et, comme expliqué [précédemment](#wcdb-ldp), retournez dans l'onglet `Logs` de votre solution Web Cloud Databases pour vous abonner à ce nouveau flux de données, puis suivez cette fois-ci le [Cas n°1](#wcdb-ldp-case1) décrit plus haut.

## Aller plus loin <a id="go-further"></a>

[Premiers pas avec votre Web Cloud Databases](/guides/web-cloud/web-cloud-databases/db-getting-started)
