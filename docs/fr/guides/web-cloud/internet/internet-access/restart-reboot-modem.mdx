...
- **Depuis l'[espace client](https://manager.eu.ovhcloud.com/#/telecom/pack)** : Positionnez-vous sur l'onglet <code className="action">Mon modem</code> de l'accès à Internet concerné, puis utilisez le bouton <code className="action">Réinitialiser le modem</code>. Cette solution est commune à tous les modèles de box. Si nécessaire, reportez-vous à notre guide « [Configurer sa box à partir de l'espace client OVHcloud](/guides/web-cloud/internet/internet-access/configuration-du-modem-a-partir-de-votre-espace-client#22-configuration-generale/) » ;
