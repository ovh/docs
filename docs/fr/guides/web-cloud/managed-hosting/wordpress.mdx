...
Connectez-vous à [l'espace client OVHcloud](/links/manager), rendez-vous dans la partie `Web Cloud` puis cliquez sur `Managed hosting for WordPress`.

#### Créer un site web

Pour créer un site web, cliquez sur le bouton `Créer`. La page ci-dessous s'affiche :

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_create_site.png)

Remplissez les champs du formulaire afin de définir les identifiants de votre site web WordPress :

- `Langue de l'admin WordPress` : sélectionnez la langue par défaut de l'interface d'administration.
- `Version PHP` : choisissez la version PHP souhaitée pour votre environnement WordPress. Consultez les versions disponibles de PHP sur [cette page](/guides/web-cloud/web-hosting/web-hosting-main-info).
- `E-mail de l'admin` : saisissez l'adresse e-mail de l'administrateur du site WordPress.
- `Mot de passe admin` : définissez le mot de passe associé à ce compte administrateur.

Cliquez sur `Continuer` pour valider les informations.

Vous êtes redirigé vers l'onglet `Mes sites`. En haut de l'écran, un message de confirmation s'affiche vous indiquant que la requête de votre création de site web a été prise en compte.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_banner_creation_site.png)

Dans le tableau, une nouvelle ligne concernant votre nouveau site web apparaît :

- Colonne `Nom de domaine` : correspond au nom de domaine technique de votre site web. Il sert d'URL de staging (pré-production), permettant d'accéder à votre site web avant de l'associer à votre propre nom de domaine.
- Colonne `Statut` : indique l'état d'avancement de la création du site web. Le statut apparaît d'abord comme `En cours`, le temps que l'infrastructure soit déployée et que WordPress soit installé. Une fois votre site web opérationnel, le statut passe à `Activé`, indiquant que votre WordPress est désormais disponible en ligne via son nom de domaine technique.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_tab_new_line.png)

Vous pouvez également suivre la progression de la création de votre site web en cliquant sur l'onglet `Tâches`.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_task_new_line.png)
...
Pour importer un site web WordPress, cliquez sur le bouton `Importer`. L'écran ci-dessous s'affiche :

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_import_site.png)
...
- `URL d'administration du site` : indiquez l'adresse utilisée pour vous connecter à l'interface d'administration de votre site web WordPress actuel, par exemple `https://www.monsite.com/wp-admin`.
- `Compte admin` : saisissez le nom d'utilisateur administrateur WordPress.
- `Mot de passe admin` : indiquez le mot de passe de l'utilisateur administrateur WordPress.
...
Une fois les champs obligatoires remplis, cliquez sur `Continuer` pour lancer le processus d'import.
...
Une fois tous les éléments sélectionnés, cliquez sur `Lancer l'import`.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_import_site_selection.png)

#### Supprimer un site web

##### Supprimer un seul site web

Pour supprimer un site web, dirigez-vous dans l'onglet `Mes sites` et identifiez la ligne du tableau correspondant à votre site web. Cliquez sur le bouton `⋮` puis sur `Supprimer`.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_site_button.png)

Un message de confirmation s'affiche. Cliquez sur `Supprimer` pour confirmer la suppression de votre site web.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_site_confirmation.png)

Vous êtes redirigé vers l'onglet `Mes sites`. En haut de l'écran, un message de confirmation s'affiche vous indiquant que le processus de suppression de votre site web est en cours.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_site_banner_validation.png)
...
Pour supprimer plusieurs sites web en même temps, dirigez-vous dans l'onglet `Mes sites`. Identifiez les lignes du tableau correspondant aux sites web que vous souhaitez supprimer et cochez les cases situées à gauche. Une fois les sites web sélectionnés, cliquez sur le bouton `Supprimer mes X sites`.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_several_sites.png)

Un message de confirmation s'affiche. Cliquez sur `Supprimer` pour confirmer la suppression de vos sites web.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_several_sites_confirmation.png)

Vous êtes redirigé vers l'onglet `Mes sites`. En haut de l'écran, un message de confirmation s'affiche vous indiquant que le processus de suppression de vos sites web est en cours.

![managed_wordpress](/images/guides/web-cloud/managed-hosting/01-managed-wordpress-getting-started/managed_wordpress_delete_several_site_banner_validation.png)

Lorsque vos sites web sont définitivement supprimés, leurs lignes correspondantes dans le tableau disparaissent.

#### Gérer ses sites web

Pour gérer tous vos sites web, dirigez-vous dans l'onglet `Mes sites` puis cliquez sur `Gérer mes sites`. Ce bouton ouvre automatiquement le gestionnaire de flotte, intégré au Managed Hosting for WordPress. Cet espace vous permet de gérer plusieurs sites web WordPress simultanément, depuis une interface unifiée.
