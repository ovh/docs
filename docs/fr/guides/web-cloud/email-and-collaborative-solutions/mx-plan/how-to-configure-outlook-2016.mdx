...
La fenêtre est divisée en deux parties, **Courrier entrant** et **Courrier sortant**. Cliquez sur la partie que vous souhaitez modifier.
    ...
    Retrouvez ce chiffre dans votre [espace client OVHcloud](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.com/fr/&ovhSubsidiary=fr), dans la rubrique <code className="action">Web Cloud</code> puis <code className="action">Email Pro</code>. Le nom du serveur est visible dans le cadre **Connexion** de l'onglet <code className="action">Informations Générales</code>.
