...
- **Lors du premier démarrage de l'application** : un assistant de configuration s'affiche, appuyez sur `Ajouter un compte`.
...
- **Si un compte a déjà été paramétré** :
    ...
    - Appuyez ensuite sur le bouton `+` dans la barre verticale de gauche.
...
> [!warning]
>
> Si la fenêtre de choix du protocole n'apparait pas, appuyez sur le bouton `?` dans le coin supérieur droit de l'écran, puis choisissez `Changer de fournisseur de compte`. Sélectionnez alors `IMAP`(recommandé) ou `POP3`.<br />
...
Dans la fenêtre suivante, cochez `Paramètres avancés` et complétez les informations suivantes :
...
- Appuyez sur `Supprimer` à la question « Voulez-vous supprimer le compte ? ».
...
### Rappel des paramètres POP, IMAP et SMTP <a id="popimap-settings"></a>
