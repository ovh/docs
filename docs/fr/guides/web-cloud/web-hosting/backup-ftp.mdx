...
Pour générer le lien de récupération, connectez-vous sur votre [espace client OVHcloud](/links/manager), rendez-vous dans la partie `Web Cloud`, cliquez sur `Hébergements` puis sur le Cloud Web concerné. 

Sélectionnez l'onglet `FTP - SSH` et cliquez sur le bouton `Générer une sauvegarde` à droite.

![backupftpcw](/images/assets/screens/control-panel/product-selection/web-cloud/cloud-web/ftp-ssh/generate-a-backup.png)

Dans la fenêtre qui s'ouvre, sélectionnez l'une des sauvegardes disponibles puis cliquez sur `Suivant`.
...
Cliquez sur `Confirmer` pour valider votre demande.
