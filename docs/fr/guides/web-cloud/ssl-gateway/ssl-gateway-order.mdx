...
Choisissez votre offre, puis cliquez sur `Commander maintenant`.

![order ssl gateway](/images/assets/screens/website/order/configure-my-ssl-gateway.png)

Sur la nouvelle page qui s'affiche, indiquez dans le formulaire `Rechercher votre nom de domaine` le nom de domaine ou le sous-domaine concerné, puis cliquez sur l'icône en forme de loupe sur la droite.
...
Si vous le souhaitez et si celle-ci est disponible lors de la commande, cochez la case `Je gère la zone DNS de ce domaine et autorise OVHcloud à modifier automatiquement l'enregistrement DNS requis`. La zone DNS associée à votre nom de domaine ou votre sous-domaine sera alors automatiquement mise à jour avec l'adresse IP du SSL Gateway.
...
Vérifiez que l'ensemble de vos choix sont corrects sur la page de commande, puis cliquez sur `Continuer`.

Pour terminer, laissez-vous guider jusqu'à la validation / paiement de votre bon de commande.

### Configuration de votre zone DNS

Une fois votre bon de commande validé et si vous n'avez pas coché la case `Je gère la zone DNS de ce domaine et autorise OVHcloud à modifier automatiquement l'enregistrement DNS requis`, un e-mail vous sera envoyé afin de vous demander de faire pointer votre nom de domaine ou votre sous-domaine vers l'infrastructure OVHcloud dans un délai de 3 jours.
...
:::details Cas 1 : votre zone DNS est gérée par les serveurs DNS mutualisés OVHcloud.
...
:::details Cas 2 : votre zone DNS n'est pas gérée par les serveurs DNS mutualisés OVHcloud.
