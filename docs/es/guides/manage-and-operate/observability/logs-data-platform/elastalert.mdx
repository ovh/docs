...
2. In the Alias panel, click the `Add an alias` button.
3. Choose a name and define a description for your alias.
4. Save the entry by clicking the `Save` button.
5. Once the alias has been created. Use the **...** menu on the right side and select the `Attach content to the alias` option.
