...
<iframe className="video" width="560" height="315" src="https://www.youtube-nocookie.com/embed/uulkyuwgOsk?si=h3d6-3ESXwv2YQwc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
>
> **Los detalles técnicos** importantes relativos al Public Cloud de OVHcloud están disponibles en [esta página](/guides/public-cloud/public-cloud-cross-functional/compute-essential-information).
>
...
El [protocolo SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) permite una comunicación cliente-servidor encriptada. Un **par de claves SSH** se compone de una clave pública y una clave privada.
...
Puede almacenar sus llaves SSH públicas en la sección `Public Cloud` de su [área de cliente de OVHcloud](/links/manager). No es obligatorio, pero hace que el proceso de creación de instancias sea más práctico.

> [!info]
>
> Las llaves SSH almacenadas le permiten crear sus instancias más rápidamente en su área de cliente. Para cambiar los pares de claves y añadir usuarios una vez creada la instancia, consulte la guía sobre [las claves SSH adicionales](/guides/public-cloud/compute/configuring-additional-ssh-keys).
>
...
Conéctese al [área de cliente de OVHcloud](/links/manager), acceda a la sección `Public Cloud` y seleccione el proyecto de Public Cloud correspondiente.

![control panel](/images/assets/screens/control-panel/product-selection/public-cloud/select-project.png)

Abra `Claves SSH` en el menú de la izquierda debajo de **Ajustes**. Haga clic en el botón `Añadir una clave SSH`.

![ssh keys](/images/assets/screens/control-panel/product-selection/public-cloud/cp-pci-sshkeys.png)

En la nueva ventana, introduzca un nombre para la clave. Rellene el campo `Clave` con su cadena de clave pública, por ejemplo, la creada en el [paso 1](#create-ssh). Confirme haciendo clic en `Añadir`.
...
- Si la instancia debe estar conectada a una nueva red privada (OVHcloud [vRack](/links/network/vrack)), **cree su vRack** antes de continuar. Para más información, consulte la [guía sobre el vRack de Public Cloud](/guides/public-cloud/public-cloud-network-services/public-cloud-network-services-vrack).
...
Para más información, consulte nuestras guías en la sección [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts). La [guía de conceptos](/guides/public-cloud/public-cloud-network-services/concepts) proporciona una introducción a Public Cloud Networking.
...
En la página **Inicio**, haga clic en `Crear una instancia`.
...
En la sección `Discovery` de la guía, encontrará modelos de instancias con recursos compartidos a precios muy asequibles. Son ideales para probar el Public Cloud en general o una aplicación web, por ejemplo.

Los modelos de instancia de tipo `Metal` proporcionan recursos físicos dedicados.

> [!info]
>
> El total de recursos de Public Cloud se limitará inicialmente por motivos de control de costes y seguridad. Puede comprobar estas cuotas haciendo clic en `Cuotas y regiones` en la barra de navegación de la izquierda debajo de **Ajustes**. Consulte [la documentación dedicada](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota) para obtener más información.
>
...
Las Local Zones son una extensión de las **regiones** que acercan los servicios de OVHcloud a sitios específicos, ofreciendo una latencia reducida y un rendimiento mejorado para las aplicaciones. Puede encontrar más información en la [página web de Local Zones](/links/public-cloud/local-zones) y en la [documentación de capacidades de Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).
...
Consulte también la información de la [página web de Local Zones](/links/public-cloud/local-zones) y la [documentación de capacidades de Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations).

![selección de región](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation03.png)
...
<Tab label="Introducir directamente una llave">

Para añadir una clave pública pegando la cadena de clave, haga clic en el botón `Añadir una clave`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation06.png)<br />
Escriba un nombre para la clave y la cadena de clave en los campos respectivos. Haga clic en `Siguiente`.<br /><br />
![key selection](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation07.png)<br />
Antes de hacer clic en `Siguiente`, puede utilizar el botón `Añadir una llave` para almacenar la llave en su área de cliente de OVHcloud (consulte el [paso 2](#import-ssh) para más información).
...
<Tabs>
<Tab label="1: Número de instancias que quiere crear">

Puede crear varias instancias en función de las selecciones realizadas durante las fases de creación, pero se aplicarán [los límites de cuota de recursos](/guides/public-cloud/public-cloud-cross-functional/increasing-public-cloud-quota).<br />
  </Tab>
<Tab label="3: Nombre de la instancia">
...
<Tab label="4: Script de post-instalación">

Puede añadir [su script](/guides/public-cloud/compute/key-concepts) a este campo.<br />
  </Tab>
<Tab label="5: Backup automático de instancias">

Puede activar [las copias de seguridad automatizadas](/guides/public-cloud/compute/save-an-instance) seleccionando esta opción. Consulte la información sobre precios y otros detalles.
...
Tenga en cuenta que si hace clic en `Crear una nueva red privada`, el proceso de creación de instancias se interrumpirá y deberá reiniciarse desde el principio.<br />
  </Tab>
<Tab label="Modo público">
...
Tenga en cuenta que si hace clic en `Crear una nueva red privada`, el proceso de creación de instancias se interrumpirá y deberá reiniciarse desde el principio.
...
![modo de facturación](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-creation14.png)
...
La facturación por horas es la mejor opción si no ha determinado claramente la duración del período de uso. Si decide conservar la instancia para su uso a largo plazo, podrá [cambiar a una suscripción mensual](/guides/account-and-service-management/managing-billing-payments-and-services/changing-hourly-monthly-billing).<br />
...
- [Facturación del Public Cloud](/guides/public-cloud/public-cloud-cross-functional/analyze-billing)
- [FAQ sobre la facturación mensual](/guides/public-cloud/compute/faq-change-of-monthly-billing-method)

Una vez que haya configurado su instancia, haga clic en el botón `Crear una instancia`. La entrega del servicio puede tardar unos minutos.
...
> [!info]
>
> Si ha instalado un **OS con aplicación**, consulte nuestra [guía Primeros pasos con las aplicaciones](/guides/public-cloud/compute/apps-first-steps) y la documentación oficial del editor del SO.
>
...
![área de cliente](/images/assets/screens/control-panel/product-selection/public-cloud/select-project.png)

Seleccione `Instancias` en la barra de navegación izquierda debajo de **Compute**. La instancia está lista cuando el estado se establece en `Activado` en la tabla. Si la instancia se ha creado recientemente y tiene un estado diferente, haga clic en el botón "Actualizar" situado junto al filtro de búsqueda.

![page instances](/images/guides/public-cloud/compute/public-cloud-first-steps/24-instance-connect01.png)

Haga clic en el nombre de la instancia en este panel para abrir el `Dashboard`, donde puede encontrar toda la información relativa a la instancia. Para más información sobre las funciones disponibles en esta página, consulte nuestra guía sobre [la gestión de las instancias en el área de cliente](/guides/public-cloud/compute/first-steps-with-public-cloud-instance).

Se creará automáticamente un **usuario con derechos elevados (*sudo*) en la instancia**. El nombre de usuario refleja la imagen instalada, por ejemplo "ubuntu", "debian", "fedora", etc. Puede comprobarlo en el lado derecho del `Dashboard` en la sección **Redes**.
...
>
> Esta guía no cubre la red privada para las instancias. Para más información, consulte nuestra documentación [Public Cloud Network Services](/guides/public-cloud/public-cloud-network-services/concepts).
>
...
> [!info]
>
> Si recibe mensajes de error relativos a sus **llaves SSH**, compruebe que su dispositivo local dispone de una llave SSH privada correctamente configurada utilizando la información de [esta guía](/guides/public-cloud/compute/compute-creating-ssh-keys#create-ssh-key).<br />
> Si sigue teniendo problemas, puede reemplazar el par de claves con [esta guía](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
> Si ha creado una instancia sin llave SSH, a través de la [API de OVHcloud](/guides/manage-and-operate/api/first-steps) o de la [interfaz OpenStack Horizon](/guides/public-cloud/compute/create-instance-in-horizon), solo podrá añadir una llave SSH a su instancia a través del [modo de rescate](/guides/public-cloud/compute/put-an-instance-in-rescue-mode) siguiendo las instrucciones descritas en [esta guía](/guides/public-cloud/compute/compute-replacing-lost-ssh-key-pair).
>
...
[En función de su configuración](#create-ssh), tendrá que escribir una frase de contraseña que proteja su clave privada o especificar la ruta de acceso a su archivo de clave. Para más información, consulte nuestra [guía de claves SSH](/guides/public-cloud/compute/compute-creating-ssh-keys#multiplekeys).

Si utiliza otro cliente SSH, consulte la documentación del usuario de SSH. Puede consultar un ejemplo de uso de la solución open source `PuTTY` en [esta guía](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows).
...
Una vez que haya comprobado que la instancia Windows está [instalada](#verify-status), abra la pestaña `Consola VNC` en su [área de cliente de OVHcloud](/links/manager).
...
Configure su **país/región**, el **idioma preferido de Windows** y su **distribución de teclado**. Haga clic en el botón `Siguiente` situado en la esquina inferior derecha.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-locale.png)<br />
  </Tab>
<Tab label="2. Contraseña de administrador">

Establezca una contraseña para su cuenta Windows `Administrator` y confírmela, y haga clic en `Finalizar`.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-admin.png)<br />
  </Tab>
<Tab label="3. Pantalla de conexión">

Windows aplicará la configuración y mostrará la pantalla de inicio de sesión. Haga clic en el botón `Send CtrlAltDel` en la esquina superior derecha para conectarse.<br /><br />
![VNC](/images/assets/screens/other/windows/windows-vnc.png)<br />
  </Tab>
<Tab label="4. Usuario administrador">
...
Introduzca la dirección IPv4 de su instancia, su identificador y su contraseña. Normalmente, aparece un mensaje de advertencia solicitándole que confirme la conexión debido a un certificado desconocido. Haga clic en `Sí` para conectarse.
...
Si aparece un mensaje de advertencia de certificado, haga clic en `Yes`. Escriba el nombre de usuario y la contraseña de Windows y haga clic en `OK` para conectarse.<br /><br />
![linux remote](/images/guides/public-cloud/compute/public-cloud-first-steps/24-rem-connect03.png)<br />
  </Tab>
<Tab label="3. Parámetros">

Puede encontrar elementos útiles en la barra de herramientas de la izquierda. Por ejemplo, haga clic en el icono `Toggle dynamic resolution update` para mejorar la resolución de la ventana.<br /><br />
...
Seleccione `Instancias` en la barra de navegación izquierda debajo de **Compute**. Haga clic en el nombre de la instancia y abra la pestaña `Consola VNC`.
...
[Cómo empezar con OpenStack](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment)
