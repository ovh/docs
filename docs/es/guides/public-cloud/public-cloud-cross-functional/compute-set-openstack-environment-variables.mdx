...
- Haber preparado el entorno para utilizar OpenStack. Para ello, consulte la siguiente guía: [«Preparar el entorno para utilizar la API de OpenStack»](/guides/public-cloud/public-cloud-cross-functional/compute-prepare-openstack-api-environment).
...
Conéctese a su [área de cliente de OVHcloud](/links/manager), acceda a la sección `Public Cloud` y seleccione su proyecto de Public Cloud.

En el menú de la izquierda, haga clic en `Usuarios y roles` en **Ajustes**. A continuación, haga clic en `«...»` a la derecha de su usuario y seleccione `«Descargar el archivo RC de OpenStack»`.
