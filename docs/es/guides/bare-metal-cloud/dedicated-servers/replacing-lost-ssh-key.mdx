...
Si [utiliza llaves SSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys-dedicated/) para conectarse a su servidor, la pérdida de su llave SSH privada podría significar la pérdida total de acceso a su servidor.
...
Cree un nuevo par de llaves SSH en su puesto de trabajo, tal y como se describe en la primera parte de la guía ["Crear claves SSH"](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys-dedicated/).
...
Cambie al modo de arranque "normal" y reinicie el servidor desde el [Panel de configuración de OVHcloud](https://www.ovh.com/auth/?onsuccess=https://manager.eu.ovhcloud.com/&from=https://www.ovh.es/&ovhSubsidiary=es). Consulte la guía "[Activar y utilizar el modo de rescate](#step2)" si es necesario.
