...
En el [área de cliente de OVHcloud](/links/manager), acceda a la pestaña `IPMI` para abrir una sesión de KVM.
...
Haga clic en `Aceptar` y estará conectado.
...
En esta interfaz puede manipular el archivo *SAM* para borrar la contraseña del usuario admin. La ruta de acceso predeterminada del directorio *WINDOWS* se autocompletará. Abra el archivo para ver la lista de usuarios haciendo clic en `Abrir`.

El usuario en cuestión será "admin" o "Administrator", según la versión de Windows. Si ambos están presentes, seleccione "admin". Haga clic en `Cambiar la contraseña`.

![ntpwedit](/images/guides/bare-metal-cloud/dedicated-servers/changing-admin-password-on-windows/adminpw_win_10.png)

En la nueva ventana, deje los campos vacíos y haga clic en `Aceptar`. Finalice haciendo clic en `Guardar los cambios` y luego en `Salir`.
...
Al volver a la ventana de KVM, seleccione la opción de apagado `Reiniciar` con el botón Windows "Iniciar" en la parte inferior izquierda.
...
[Utilizar IPMI en un servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/dedicated-servers-ipmi)
