...
### Añadir la cuenta <a id="add-account"></a>
...
>
> Encontrará esta cifra en su [área de cliente de OVHcloud](/links/manager), en la sección `Web Cloud`, y en la columna izquierda, `E-mail Pro`. El nombre del servidor puede verse en el recuadro **Conexión** de la pestaña `Información general`.

- **Cuando inicie la aplicación por primera vez**, aparecerá un asistente de configuración y pulse `Añadir cuenta`.
...
- **Si ya tiene una cuenta configurada**:
    ...
    - A continuación, pulse el botón `+` en la barra vertical izquierda.
    - Pulsa `Añadir cuenta`.
...
Introduzca su dirección de correo electrónico y pulse `Continuar`.
...
> [!warning]
>
> Si no aparece la ventana de selección de protocolo, pulse el botón `?` en la esquina superior derecha de la pantalla y seleccione `Cambiar proveedor de cuenta`. Seleccione `IMAP`(recomendado) o `POP3`.<br />
...
En la siguiente ventana, marque `Parámetros avanzados` e introduzca la siguiente información:
...
### Cambiar la configuración existente <a id="modify-settings"></a>
...
- Pulsa `Eliminar la cuenta`.
- Pulsa `Eliminar` en la pregunta «¿Quieres eliminar la cuenta?».
...
### Aviso de los parámetros POP, IMAP y SMTP <a id="popimap-settings"></a>
