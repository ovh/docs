...
[KMIP](https://www.oasis-open.org/committees/tc_home.php?wg_abbrev=kmip) - Key Management Interoperability Protocol - is a protocol design to standardise the communications with a KMS.
So any products supporting KMIP (such as VMware vSphere, Veeam, Nutanix, etc.) can natively be interfaced with a KMIP compatible KMS such as the OVHcloud KMS

It's bring an easy connection, and a reversible configuration

### Connection of a KMIP compatible product with the OVHcloud KMS

The configuration depends of the product to integrate, but does not need any specific configuration on the OVHcloud KMS other than generated an access certificate.
Software editor usualy have dedicated guide to this purpose

### Direct use

It's also possible to use the KMIP API directly

Authentication on this procotol is done with a client certificate, in the same way than for the REST API. It need to open a TLS channel with a valide access certificate.

Then it's possible to exchange KMIP message such as define in the standard. Depending of the technology stack, we recommend the folloing library :
...
The OVHcloud KMS cover a part of 1.0 to 1.4 version of the KMIP standard.
