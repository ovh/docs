...
> [Shipping logs to Logs Data platform with Filebeat](/pages/manage_and_operate/observability/ingestion_filebeat)
>
...
To keep thing brief, this extract has only the parts relevant to the access log file. [The syslog-ng tutorial](/pages/manage_and_operate/observability/ingestion_syslog_ng) covers the configuration for any syslog file (like the error log file). This configuration is only valid for syslog-ng 3.8+.


### Apache logs format

If you want to use your own log format and include some useful information here is a cheat sheet for you (Note that the labels follows [the field naming conventions](/pages/manage_and_operate/observability/getting_started_field_naming_convention)).
...
The complete procedure of its installation is described [on this page](/pages/manage_and_operate/observability/alerting_stream) in the Apache logs use case.
