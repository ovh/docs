...
You can get mode information on Kafka from the [official Kafka website](https://kafka.apache.org/intro){.external).
...
Please visit the [capabilities page](https://docs.ovh.com/gb/en/publiccloud/databases/) of your selected database type for detailed information on hardware resources and other properties of the database installation.
...
[OpenSearch General information](images/kafka_cluster_ready_to_configure.png){.thumbnail}

### Step 1 (mandatory): Set up a user

Switch to the tab `Users`{.action}. An admin user is preconfigured during the service installation. You can add more users by clicking on the button `Add user`{.action}.
...
Passwords can be reset for the admin user or changed for others afterward in the `Users`{.action} tab.
...
Copy the IP address shown on this website and keep them for later.

Your Apache Kafka service is now fully accessible!

Optionally, you can configure access control lists (ACL) for granular permissions and create something called topics, shown below.
...
For this first example let's push the "test-message-key" and its "test-message-content" to the topic "my-topic".

```bash
echo test-message-content | kcat -F kafkacat.conf -P -t my-topic -k test-message-key
```

*Note*: depending of the installed binary, the CLI command can be either **kcat** or **kafkacat**.
...
Congratulations, you now have an up and running Apacke Kafka cluster, fully managed and secured. You are able to push and retrieve data easily via CLI.
...
[Kafka capabilities](https://docs.ovh.com/gb/en/publiccloud/databases/kafka/capabilities/)
