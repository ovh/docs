...
The primary node, as known as the master node, acts as the leader of your cluster. It can read and write in your database. There is only one primary node per PostgreSQL cluster.
...
- Data resiliency: your data gets replicated to another physical resource, carrying "hot data".
...
- Performance: for read-only operations, you can connect to replica nodes, allowing you to reduce the load on primary server.

Having at least two replica nodes node is that even during recovery from (single-node) failures, there are always two copies of the data on two different nodes.
...
The impact of different failure scenarios depends of the selected service plan, since the amount of nodes are related to them. Browse the right section according to your offer.
...
**RTO** is the **Recovery Time Objective**, meaning how long it takes to go back to normal situation.
...
| Datacenter outage (backups in same DC)     | **RPO**:depends of manual backups made by customer. **RTO**: multiple hours/days (time to restore your backup) | **RPO**: depends of manual backups made by customer. **RTO**: depends of customer actions            |
...
Also, some write operations data may have been lost since we restore from the last backup and latest backuped PostgreSQL Write Ahead Logs (WAL) files. Typically, this time window is limited to either five minutes of time or one WAL file.
...
> [!primary]
>
> You can verify your backups physical location in your control panel, directly in the *Backups* tab of your service.
>
...
We plan to provide backups storage in another location during 2022 calendar year.
...
In both cases the Recovery Time Objective will vary, depending of multiples parameters such as amount of data to restore, network congestion and latency or node performances. If can go from few hours up to few days.


### Scenarios for Highly available Business and Enterprise service plans

Business plan provides a primary and a replica node, Enterprise plan starts with a primary and two replica nodes.
...
if both PostgreSQL replica nodes have a failure, the same process happens but twice.

Although OVHcloud cannot commit to a set duration of the whole process, you can expect zero downtime since the primary is up and few hours to get the new replacement nodes up and ready (the time to restore the data).
...
The duration of recovery process depends of the amount of data stored, the region, the network bandwidth. OVHcloud cannot guarantee a maximum recovery time, but you can expect a few hours.
...
> [!primary]
>
> Public Cloud Databases for PostgreSQL does not support multi-AZ so far: all the nodes are in the same datacenter.
> You can verify your backups physical location in your control pane, directly in the *Backups* tab of your service.
>
...
In both cases the Recovery Time Objective will vary, depending of multiples parameters such as amount of data to restore, network congestion and latency or node performances. It can go from few hours up to few days.
