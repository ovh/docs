...
credentials. The only thing you need to do is give them the url shown when doing `ovhai notebook get <notebook-id>`.
...
Wait a few seconds for the notebook to be in `RUNNING` state.
...
## Share a notebook with people out of your public cloud project

In order to allow people out of your public cloud project to access your notebook, we need to generate some credentials
...
We just created a token named `my-token`, that will allow to access any notebook that have a label `subject=image-recognition`.
...
If you're already logged in your browser, then it will display the notebook. Otherwise, you will get a page like this:

![image](images/ovh_login_page.png)

You can try in a private navigation window to get the login page and test the token.
Clicking on the `Login with token` leads to this page:

![image](images/ovh_login_token_page.png)
