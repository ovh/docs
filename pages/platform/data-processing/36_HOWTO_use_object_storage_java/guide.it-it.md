...
You have to upload it in an Object Storage container. (see [Getting started with the Swift API](https://docs.ovh.com/it/storage/object-storage/pcs/getting-started-with-the-swift-api/){.external} to read more about how to create an OVHcloud Object Storage container). In this example, we created a container named 'textfile" and uploaded the novel.txt object into that container. 
- OVHcloud Object Storage "secret key" and "access key" for the container that you have uploaded the 'novel.txt' text file (textfile container in this example). (See [How to create EC2 credentials](https://docs.ovh.com/it/storage/object-storage/pcs/getting-started-with-the-swift-s3-api/){.external} for more details).
