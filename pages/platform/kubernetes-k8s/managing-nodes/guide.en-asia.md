...
In this guide we explain how to do some basic operations with nodes and node pools using the [OVH Cloud Manager](https://ca.ovh.com/auth/?action=gotomanager): adding nodes to an existing node pool, creating a new node pool...

## Editing nodes to an existing node pool

### Step 1 - Editing nodes to an existing node pool using the OVH Cloud Manager

Access our administration UI for your OVHcloud Managed Kubernetes clusters by clicking on the *Platforms and services* menu in the [OVH Cloud Manager](https://ca.ovh.com/auth/?action=gotomanager)
