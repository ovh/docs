...
Thanks to the Node pool's labels propagation to Nodes, we will:

- create a Managed Kubernetes cluster
- create a node pool
- create another node pool, with a different configuration and autoscaling activated
- taint a node
- cordon a node
- drain a node
...
Follow the cluster creation step by step guide...


« we also recently manage labels on the nodepool CRD that we auto apply to the nodes of that nodepool »

A node is a virtual (VM) or physical machine

Nos applications, nos workloads vont tourner dans des Pods qui eux meme tournent dans des Nodes.
Chaque node …
Mais parfois, le scheduler de kubernetes ne peut pas deployer un Pod sur Node, pour diverses raisons :
* Node is not ready
* Nos is unreachable
* Out of disk
* Network unaivalable
...
On peut effectuer des operations sur un Node.

### Cordon a Node

Make the node unschedulable

TODO: insérer illustration de la video

Cela signifie que ce node la ne peut plus accueillir de pods tant qu’il est marqué comme Unschedulable.


### drain a node

It means: evict all pods from a Node & mark the node as unschedulable
...
Multiple taints an be seated in the same Node.

TODO: insérer illustration

* Avec la taint NoSchedule, pods that don’t tolerate the taint can’t be schedule on the node.
* Avec la taint PreferNoSchedule, Kubernetes avoid scheduled Pods that don’t tolerate the taint.
* Avec la taint NoExecute, Pods are evicted from the Node if they are already running, else they can’t be scheduling.

$ kukectl get node

Éjecter tous les pods d’un node - drain a node
$ Kubectl drain my-node 
$ kuebctl drain my-node —ignore-daemonsets

Cordon a node (and mark as unschedulable)
...
Display taint infos for all Nodes
...
Unpause a node
$ Kubectl taint node y-node my-key:NoSchedule-

Create a pod that can be scheduled on a Node ho have the taint specialkey=specialvalue:NoSchedule
