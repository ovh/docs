...
By default the time parameter are rendered with an **rfc3339** string. To get a time value, you will need to specify `epoch` as query parameter with the wanted unit. The supported one on the Metrics Data Platform are: `ms`, `us` and `ns`.
