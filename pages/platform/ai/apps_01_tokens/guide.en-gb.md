...
From the **AI Apps** page you access the tokens management page by clicking of the `Tokens` tab.
...
A Read-only token will only grant you the right to query the deployed AI App while en Operator token would also allow you to manage the AI App itself.
...
Additional information about the use of a token to manage an AI Training resource can be found [here](https://docs.ovh.com/gb/en/ai-training/app-token-cli/#use-the-app-token)

## Using a token to query an AI App

With the token we generated in the previous step we will now query the AI App, for this demonstration we simply deployed a simple Hello World app that always responds `Hello, World!`.
...
To use the token to access this AI App you can click on `Login with token`. Fill in your token in the dedicated field and click `Connect`.

You know land on the exposed AI App service:
