...
This guide covers the submission of [**apps**](/pages/platform/ai/deploy_guide_02_getting_started) through the **ovhai** CLI.
...
If you need any help while submitting a new job, run `ovhai app run --help`:
...
Options:
  ...
  -o, --output <OUTPUT>                                                                                         Command output format [possible values: json, yaml]
      ...
      --partner <PARTNER_ID>                                                                                    Partner providing the image you want to use. This option is mandatory if you want to use a partner image. You can list images provided by partners by executing `ovhai capabilities preset ls`
      --no-color                                                                                                Remove colors from output
  -h, --help                                                                                                    Print help (see more with '--help')
```

Pour le environnement faut que la variable ne soit pas définit dans le code Python, sinon ça la remodif automatiquement. A voir si on peut pas la déclarer uniquement si elle n'existe pas. 
Sinon pour le fonctionnement général:  on met une valeur par défaut dans le dockerfile, et on peut la changer dans les paramètres de notre app

Soit on met la valeur d'initialisation par defaut comme ça 
if not os.environ["DEFAULTVAR"]:

    os.environ['DEFAULTVAR'] = 'default'
    
soit dans le docker file, soit les 2 (peutêtre mieux pour eviter la non intialisation) 

J'ai vu que si je mettais une variable environnement dans python, mais pas déclaré en tant que ENV dans le Dockerfile, je pouvais quand même la modif avec le param --env 

Si on fait les 2 ça fonctionne 

Si je l'intiilise pas dans python, mais que je la mets dans Docker, ça marche 

Si  je l'intiilise pas dans python, et que je la mets pas dans Docker, ça marche

----- 

Les deux paramètre font la même chose => ils set les variables d'environnement à l'intérieur de ton container.

Le paramètre —env est plus prio que celui du docker file, si les deux sont spécifié c'est celui de la ligne de commande qui sera pris en compte.
