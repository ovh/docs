...
You can upload your data to the cloud either by using the [OVHcloud Control Panel (UI)](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr) or with the [OVHcloud AI CLI](https://docs.ovh.com/fr/publiccloud/ai/cli/install-client/).
...
Then, select the `Object Storage`{.action} section (in the Storage category) and [create a new object container](https://docs.ovh.com/fr/storage/object-storage/pcs/create-container/) by clicking `Storage`{.action} > `Object Storage`{.action} > `Create an object container`{.action}.
...
- Do you want to use Streamlit to create an audio classification app? [Learn how to do it](https://docs.ovh.com/fr/publiccloud/ai/deploy/tuto-streamlit-sounds-classification/).
