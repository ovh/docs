...
```bash
ovhai token create token-operator-ai --role operator
```
...
Id:         98a35849-4d6b-4f99-a9ee-d56cbc1fd7b3
Created At: 04-07-23 12:54:08
Updated At: 04-07-23 12:54:08
Spec:
  Name:           token-operator-ai
  Role:           Operator
  Label Selector: 
Status:
  Value:   dGhpcyBpcyBhIHdvbmRlcmZ1bCB0b2tlbiBmb3IgYWkgdHV0b3JpYWxzICE=
  Version: 1
```

The token is now created. Don't forget to save the token to use it later.
For more information on the AI token please read the [Manage access tokens](/pages/platform/ai/cli_13_howto_app_token_cli/) documentation.
-->
### Train your model
...
<!--Before launching the training command, you must sign in using the previous created token:
```bash
ovhai login --token <generated token>
```
Output should be like this:
```bash
$ ovhai login --token dGhpcyBpcyBhIHdvbmRlcmZ1bCB0b2tlbiBmb3IgYWkgdHV0b3JpYWxzICE=
```

Next,--> You can launch the training specifying more ore less GPU depending on the speed you want for you training.
