...
Tout d'abord, redémarrez votre instance à l'aide du système d'exploitation de l'instance ou de l'[espace client OVHcloud](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/fr/&ovhSubsidiary=qc). Si vous ne parvenez toujours pas à établir une connexion entre le réseau public et votre IP fail-over et que vous suspectez un problème réseau, vous devez redémarrer l'instance en [mode rescue](../passer-une-instance-en-mode-rescue/). Vous pouvez ensuite configurer l'adresse IP fail-over directement sur l'instance.
