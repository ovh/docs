...
- You have ordered the Kibana pack Option or one of the Index Option.
...
To create an Elasticsearch index, you first need to order one with your active LDP (Logs Data Platform) account. This step can be done in the [OVH manager](https://www.ovh.com/manager/){.external}. Navigate to the **Plan and options** page accessible through the *...* (three dots) menu at the top right of the main page of the LDP Manager.

![Plan and options menu](images/options_menu.png){.thumbnail}

On this page, you have the status and usage of your current plan and you can order new options by clicking `+ Add options`{.action} at the Options subsection. You can then order an **index** with different sizes and number of **shards**. A **shard** is the main component of **indices**. Multiple shards means more volume, more parallelism in your requests and thus more performance. We offer several choices of number of shards.
...
Logs Data Platform Elasticsearch indices are compatible with the [Elasticsearch REST API](https://www.elastic.co/guide/en/elasticsearch/reference/2.4/docs.html){.external}. Therefore you can use simple http requests to index and search your data. The api is accessible behind a secured https endpoint with mandatory authentication. We recommend that you use [tokens](https://docs.ovh.com/gb/en/logs-data-platform/tokens-logs-data-platform/){.externa} to authenticate yourself. You can retrieve the endpoint of the API at the **About** page of your cluster. Here is a simple example to index a document with curl with an index on the cluster `graX.logs.ovh.com`.


```shell-session
$ curl -u <your-token-value>:token -XPUT 'https://graX.logs.ovh.com:9200/logs-<username>-i-<suffix>/test/1' -d '{ "user" : "Oles", "company" : "OVH", "message" : "Hello World !", "post_date" : "1999-11-02T23:01:00" }'
```
...
```shell-session
$ curl -XGET -u <your-token-value>:token 'https://graX.logs.ovh.com:9200/logs-<username>-i-<suffix>/test/1'
{"_index":"logs-<username>-i-<suffix>","_type":"test","_id":"1","_version":1,"found":true,"_source":{ "user" : "Oles", "company" : "OVH", "message" : "Hello World !", "post_date" : "1999-11-02T23:01:00" }}
```

To issue a simple search you can either use the [Query DSL](https://www.elastic.co/guide/en/elasticsearch/reference/2.4/query-dsl.html){.external} or a [URI search.](https://www.elastic.co/guide/en/elasticsearch/reference/2.4/search-uri-request.html){.external}. Here is a simple example with an URI search:

```shell-session
$ curl -XGET -u <your-token-value>:token 'https://graX.logs.ovh.com:9200/logs-<username>-i-<suffix>/_search?q=user:Oles'
{"took":32,"timed_out":false,"_shards":{"total":1,"successful":1,"failed":0},"hits":{"total":1,"max_score":3.140066,"hits":[{"_index":"logs-<username>-i-<suffix>","_type":"test","_id":"1","_score":3.140066,"_source":{ "user" : "Oles", "company" : "OVH", "message" : "Hello World !", "post_date" : "1999-11-02T23:01:00" 
```
...
```json
{ "index" : { "_index" : "logs-<username>-i-<suffix>", "_type" : "users" } }
...
```shell-session
$ curl -u <your-token-value>:token -XGET 'https://graX.logs.ovh.com:9200/logs-<username>-i-<suffix>/users/_search?pretty=true'
```
...
If you don't know how to create a Logstash collector, please refer to the [Logstash guide](https://docs.ovh.com/gb/en/logs-data-platform/){.external}. Edit the configuration of Logstash. For this example we will use a SSL TCP input with the GELF codec. Here is the input configuration.
...
- **username**: This is the username to authenticate yourself against the API. Again, we recommend that you use [tokens](https://docs.ovh.com/gb/en/logs-data-platform/){.external} for that.
...
- **fields**: This is where the magic happens. The field of the document found will be added to the event. The field of the document is on the left and the new (or updated) field of the event is on the right. Be sure to follow the [field naming conventions](https://docs.ovh.com/gb/en/logs-data-platform/){.external}.
