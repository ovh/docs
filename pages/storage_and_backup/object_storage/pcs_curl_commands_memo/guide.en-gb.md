...
export OS_STORAGE_URL="https://storage.<region>.cloud.ovh.net/v1/AUTH_<project_id>"
...
```bash
curl -i "${OS_STORAGE_URL}/<container_name>" -X PUT -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```
...
```bash
curl -i "${OS_STORAGE_URL}/<container_name>" -X HEAD -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```

### View object information

```bash
curl -i "${OS_STORAGE_URL}/<container_name>/<object_name>" -X HEAD -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```
...
```bash
curl "${OS_STORAGE_URL}/<container_name>" -X GET -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```

### Upload an object smaller than 5GB

```bash
curl -i "${OS_STORAGE_URL}/<container_name>/<object_name>" -X PUT -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "Content-Type: application/octet-stream" -d "@<object_path>"
```
...
- **path** (Required)
  e.g.: `<container_name>_segments/<object_name>_01`
- **etag** (Optional)
  Segment MD5 checksum. This value is available in the segment's `Etag` metadata via: `curl -i -X HEAD "$OS_STORAGE_URL/<container_name>/<object_name>" -H "X-Auth-Token: $OS_AUTH_TOKEN"`
- **size_bytes** (Optional)
  Segment size. This value is available in the segment's `Content-Length` metadata via: `curl -i -X HEAD "$OS_STORAGE_URL/<container_name>/<object_name>" -H "X-Auth-Token: $OS_AUTH_TOKEN"`
...
[
  {
    "path":"<container_name>_segments/<object_name>_00",
    "etag":"11a3e229084349bc25d97e29393ced1d",
    "size_bytes":"10485760"
  },
  {
    "path":"<container_name>_segments/<object_name>_01",
    "etag":"6ccef1b25ea58fb8be3ca1a1a744ea53",
    "size_bytes":"10485760"
  },
  {
    "path":"<container_name>_segments/<object_name>_02",
...
```bash
curl "${OS_STORAGE_URL}/<container_name>/<object_name>" -X GET -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -o <output_file>
```

### Delete an empty container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X DELETE -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```
...
```bash
OBJECTS=$(curl -s "${OS_STORAGE_URL}/<container_name>" -X GET -H "X-Auth-Token: ${OS_AUTH_TOKEN}")
for obj in $OBJECTS
do
  curl "${OS_STORAGE_URL}/<container_name>/$obj" -X DELETE -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
done
curl "${OS_STORAGE_URL}/<container_name>" -X DELETE -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```
...
```bash
curl "${OS_STORAGE_URL}/<container_name>/<object_name>" -X DELETE -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```

### Delete an object larger than 5 GB

```bash
curl "${OS_STORAGE_URL}/<container_name>/<object_name>?multipart-manifest=delete" -X DELETE -H "X-Auth-Token: ${OS_AUTH_TOKEN}"
```
...
```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Container-Meta-Access-Control-Allow-Origin:http://example.com"
```

### Add metadata to an object

```bash
curl "${OS_STORAGE_URL}/<container_name>/<object_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Object-Meta-my-custom-key:value"
```

### Delete a metadata from a container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Remove-Container-Meta-Access-Control-Allow-Origin"
```

### Delete a metadata from an object

```bash
curl "${OS_STORAGE_URL}/<container_name>/<object_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Remove-Object-My-Custom-Key"
```

### Set Read ACL on a container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Container-Read:${OS_TENANT_ID}:*"
```

### Set write ACL on a container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Container-Write:${OS_TENANT_ID}:*"
```

### Delete the read ACL on a container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Remove-Container-Read:x"
```

### Delete write ACL on a container

```bash
curl "${OS_STORAGE_URL}/<container_name>" -X POST -H "X-Auth-Token: ${OS_AUTH_TOKEN}" -H "X-Remove-Container-Write:x"
```
