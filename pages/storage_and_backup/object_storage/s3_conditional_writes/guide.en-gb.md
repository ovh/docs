...
**This guide explains how to use conditional writes to control whether a PUT request should succeed based on the current state of the object in your OVHcloud Object Storage bucket.**
...
Conditional writes allow you to attach preconditions to a PUT request. OVHcloud Object Storage evaluates the condition against the current state of the target object before processing the write. If the condition is not met, the request is rejected with an HTTP `412 Precondition Failed` response — without modifying the object.

The supported condition is the `If-None-Match` HTTP header:

| Header | Value | Effect |
|--------|-------|--------|
| `If-None-Match` | `*` | Write succeeds **only if the object does not already exist** |

This enables **write-if-not-exists** (also called *first-write-wins*) semantics: the first client to PUT the object wins; subsequent attempts to overwrite it are rejected.

> [!primary]
>
> Conditional writes operate at the object level and are evaluated atomically. They do not require versioning to be enabled.
>

### Use cases

- **Preventing duplicate uploads**: ensure that an object is only written once, even if multiple processes attempt to upload the same key simultaneously.
- **Distributed coordination**: use conditional writes as a lightweight locking primitive — the successful writer "claims" the key.
- **Idempotent initialisation**: safely initialise a configuration or state object without risk of overwriting an already-existing version.

### How conditional writes work

When you include the `If-None-Match: *` header in a PUT request:

1. OVHcloud Object Storage checks whether an object already exists at the specified key.
2. **If the object does not exist** → the write proceeds normally and returns HTTP `200 OK`.
3. **If the object already exists** → the write is rejected and returns HTTP `412 Precondition Failed`. The existing object is left unchanged.

> [!warning]
>
> The `If-None-Match: *` condition does not take versioning into account. If versioning is enabled on the bucket, the condition checks for the existence of the *current* version of the object. A versioned delete marker does not count as an existing object.
>
...
> [!tabs]
> Via AWS CLI
>> The AWS CLI `s3api put-object` command supports the `--if-none-match` flag:
>>
>> ```sh
>> aws s3api put-object \
>>   --bucket <bucket_name> \
...
>> **Explanations:**
>>
>> - `--bucket <bucket_name>`: replace with the name of your target bucket.
>> - `--key <object_key>`: the key (path) of the object to write.
>> - `--body <local_file_path>`: local file to upload.
>> - `--if-none-match "*"`: the conditional write header — only write if the object does not exist.
>>
>> **Successful upload (object did not exist):**
>>
>> ```json
>> {
>>     "ETag": "\"d41d8cd98f00b204e9800998ecf8427e\""
...
>> ```
>>
>> **Rejected upload (object already exists):**
>>
>> ```
>> An error occurred (PreconditionFailed) when calling the PutObject operation:
>> At least one of the pre-conditions you specified did not hold
...
>>
> Via curl (S3 REST API)
>> You can use the `If-None-Match: *` header directly with a signed PUT request:
>>
>> ```sh
>> curl -X PUT \
>>   -H "If-None-Match: *" \
...
>> ```
>>
>> Replace `<region>`, `<access_key>`, `<secret_key>`, `<bucket_name>`, and `<object_key>` with your actual values.
>>
>> **Successful upload** — HTTP `200 OK`
>>
>> **Rejected upload** — HTTP `412 Precondition Failed`
>>

### Verifying that an object exists before writing

To check whether an object already exists without downloading it, use a HEAD request:

```sh
aws s3api head-object --bucket <bucket_name> --key <object_key>
```

- Returns object metadata if the object **exists**.
- Returns a `404 Not Found` error if the object **does not exist**.

Use this to inspect the state before attempting a conditional write, or to confirm the outcome of a write.

### Important considerations

- **Atomicity**: the condition evaluation and the write are atomic. There is no window between the check and the write where another client could intervene.
- **Multipart uploads**: conditional writes using `If-None-Match` are not supported for multipart uploads. Use regular single-part PUT operations for conditional writes.
- **Versioning interaction**: if versioning is enabled and the current version is a delete marker, the object is considered non-existent and the conditional write will succeed.
- **Cost**: a rejected PUT (`412 Precondition Failed`) does not incur storage costs, but may be counted as a request for billing purposes.
