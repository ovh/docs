...
Para encontrar os seus Vaults, faça login no seu [área de cliente OVHcloud](/links/manager), vá para `Bare Metal Cloud`{.action} e depois para `Backup Agent`{.action}. Clique no separador `Vaults`{.action}.
