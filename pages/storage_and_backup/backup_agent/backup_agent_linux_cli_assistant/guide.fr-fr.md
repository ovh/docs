...
### Récupérer les URL d'installation
...
Via la ligne de commande que nous vous fournissons, vous pouvez installer l'agent et l'assistant en même temps. Remplacez `URL_DU_SCRIPT` (à 2 reprises) et `URL_DU_PAQUET_AGENT` par les liens copiés précédemment.
...
Si vous lancez le script **sans argument** avec un terminal interactif (`sudo bash ovh-ba-install.sh`), un court **écran d'accueil** vérifie si **`ovhbackupagent`** est encore présent et si le Backup Agent est détecté, puis propose au besoin de **réinstaller uniquement** le raccourci.

## Exemple pas à pas : première installation

**Scénario** : nouveau serveur Linux, vous disposez des deux URL (script et paquet Linux) copiées depuis la fenêtre **Télécharger l'agent** de votre espace client.
...
2\. Remplacez `URL_DU_SCRIPT` (à 2 reprises) et `URL_DU_PAQUET_AGENT` dans la commande ci-dessous par vos URL, puis exécutez-la.
...
## Assistant de désinstallation — Uninstall Wizard (touche **U**)
