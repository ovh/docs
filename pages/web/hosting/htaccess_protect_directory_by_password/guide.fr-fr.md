...
Ce tutoriel explique comment mettre en place une authentification « utilisateur/mot de passe » pour accéder via un navigateur Internet à tout ou partie de votre site web. 
...
- « **.htpasswd** » : En complément du présent tutoriel, consultez la [documentation officielle Apache](https://httpd.apache.org/docs/2.4/fr/programs/htpasswd.html) relative à ce fichier.
...
>
> Pour toutes questions relatives à la création, l'utilisation ou la programmation de votre site web, OVHcloud ne sera pas en mesure de vous apporter une assistance sur ces sujets.
>
...
- Configuration des règles dans le fichier « .htaccess » et suppressions du fichier « crypt.php ».
...
Ouvrez ou créez le dossier destiné à être protégé de votre site web. (Dans notre exemple ci-après il s'agira du dossier « admin »). Créez dans ce répertoire un fichier « .htpasswd » et un fichier « .htaccess ».
...
> [!warning]
>
> Pour des raisons de sécurité, l'utilisation du SSH est recommandée. Toutefois, si vous disposez d'une offre [Kimsufi Web](https://www.kimsufi.com/fr/hosting.xml) ou [Perso](https://www.ovhcloud.com/fr/web-hosting/personal-offer/) où le SSH est indisponible, vous pouvez aussi exécuter le fichier « crypt.php » via votre navigateur Web.
>
> Pour cela, saisissez l'URL suivante : `https://domain.tld/crypt.php` en modifiant `domain.tld` par votre propre nom de domaine. Ceci directement dans la barre d'URL de votre navigateur web.
>
...
Les valeurs `encrypted_password1`, `encrypted_password2` et `encrypted_password3` doivent, par exmple, ressembler au format de la ligne suivante :
...
Le fichier « .htpasswd » contient les mots de passe chiffrés respectifs à chacun des utilisateurs déclarés dans le fichier. Ce sont ces utilisateurs uniquement qui seront autorisés à se connecter au répertoire pour lequel vous souhaitez protéger l'accès.
...
Pour récupérer votre login FTP, consultez notre guide sur la [connexion à votre espace FTP](/pages/web/hosting/ftp_connection/).
...
>
> De plus, tant que vous ne vous êtes pas authentifié pour accéder aux fichiers concernés par la directive, ces derniers peuvent ne pas apparaître et ne peuvent donc pas être « listable » via une page « index of ».
>
