...
- You must have a [Web Hosting plan](https://www.ovhcloud.com/en-ca/web-hosting/) that contains at least one database.
- You must have [domain name](https://www.ovhcloud.com/en-ca/domains/)
- You need [WordPress installed](https://docs.ovh.com/ca/en/hosting/web_hosting_web_hosting_modules/) on your web hosting plan
- You must be logged in to [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/ca/en/&ovhSubsidiary=ca){.external}

## Instructions

If this is not already the case, [add an SSL certificate](https://docs.ovh.com/ca/en/hosting/activate-https-website-ssl/#step-1-activate-the-ssl-certificate-on-the-web-hosting-plan) on the domain name associated with your website before you continue.
...
Go to the admin interface access link sent by email when you installed the CMS. By default, the URL ends with 'wp-admin'. If you have not logged in to your admin interface, **WordPress** will automatically redirect you to your URL ending with `wp-login`:

![WordPress - Admin login](images/wordpress_first_steps_1.png){.thumbnail}

> [!primary]
> 
> On this homepage, you can change the default language of the **WordPress** interface. Go to the dropdown menu at the bottom of the page, select the language of your choice, then confirm by pressing the `Change`{.action} button. The language can be changed later.
> 

Enter the login (or "Administrator's name") that was provided to you by email and the "WordPress password" indicated in the same email. You will then land on your dashboard:

![WordPress - Dashboard](images/wordpress_first_steps_2.png){.thumbnail}

### Change the theme of your WordPress website

**WordPress themes** are sets of files that allow you to modify the presentation of your website without modifying its content. There are many themes available on the internet, free and paid, with different themes (websites, blogs, e-commerce, online press, ...).
...
You can view the result by going to your website with your domain name.
...
To add content, click the `+`{.action} sign and choose what you want to insert:

![WordPress - Gutenberg, add block](images/wordpress_first_steps_8.png){.thumbnail}

On the right of your page, you can find three links to do the following:
...
In our **example**, click `View`{.action}, then `Preview in New Tab`{.action}. Choose the type of device to render on (PC, tablet or smartphone):
...
Pages are to be distinguished from articles. They are mainly used to write content that will not evolve over time, such as legal notices, general conditions of use, etc.
...
> [!primary]
>
> By default, there is a page that is generated when you install **WordPress**. For readability, this page was removed from the example.
>

Click `Add`{.action}. You will find the publisher Gutenberg:
...
You can go back to your website's homepage, and you will see a link to your new page:

![WordPress - Home page with new page link](images/wordpress_first_steps_20.png){.thumbnail}

### Improve Permalinks

By default, links on your **WordPress** pages are written with a syntax of type `parameter=value`, where `value` is an integer that is not explicit. Modifying the writing of permalians allows URLs with a more explicit format. Your URLs will be easier to read and improve your website's SEO.

On the dashboard home page, go to `Settings`{.action} then `Permalians`{.action}:

![WordPress - Settings/Permalinks](images/wordpress_first_steps_21.png){.thumbnail}

You can choose between several types of permalians. Select the “Publication title”, then confirm at the bottom of the page:
