...
> 
> Questa guida ti aiuta a eseguire le operazioni necessarie. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a uno [specialista del settore ](https://partner.ovhcloud.com/fr/). OVHcloud non potrà fornirti alcuna assistenza. Per maggiori informazioni consulta la sezione ["Per saperne di più"](#go-further) di questa guida.
>
...
- L'hosting [Personale](https://www.ovhcloud.com/fr/web-hosting/personal-offer/) con **10 indirizzi email** "MX Plan"
- Hosting [Pro](https://www.ovhcloud.com/fr/web-hosting/professional-offer/) con **100 indirizzi email** "MX Plan"
- Hosting [Performance](https://www.ovhcloud.com/fr/web-hosting/performance-offer/) con **1000 indirizzi email** "MX Plan". Questa offerta è disponibile in 4 "sottolivello".
- L'hosting [Cloud Web](https://www.ovhcloud.com/fr/web-hosting/cloud-web-offer/) con **200 indirizzi email** "MX Plan". Questa offerta è utilizzata dagli sviluppatori di applicazioni.
...
> [!primary]
>
> OVHcloud propone altre offerte email in aggiunta al servizio "MX Plan". È possibile, ad esempio, combinare a degli indirizzi email "MX Plan" degli indirizzi ["Email-Pro"](https://www.ovhcloud.com/fr/emails/email-pro/) e/o degli account ["Exchange"](https://www.ovhcloud.com/fr/emails/hosted-exchange/).
>

### Step 2: creare e preconfigurare una zona DNS per il tuo dominio presso OVHcloud <a name="step2"></a>

Una volta creato il tuo hosting, accedi allo [Spazio Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr) e crea una zona DNS per il tuo dominio **senza le "www"**. Per maggiori informazioni, consulta la guida sulla [creazione di una zona DNS OVHcloud](https://docs.ovh.com/fr/domains/creer-une-zone-dns-pour-un-domaine-externe/).

Una volta creata la zona DNS, consulta la guida [Modificare una zona DNS OVHcloud](https://docs.ovh.com/fr/domains/editer-ma-zone-dns/). Se non sono presenti, inserisci le seguenti voci:
...
- Il dominio senza "www", verso l'indirizzo IP di destinazione di tipo "A" del tuo hosting OVHcloud Per recuperare l'indirizzo IP corretto, consulta la guida che elenca gli [indirizzi IP dei diversi cluster di hosting condivisi](https://docs.ovh.com/fr/hosting/liste-des-adresses-ip-des-clusters-et-hebergements-web/).
...
Per importare il backup dello spazio di storage FTP del tuo precedente provider, [accedi allo spazio di storage FTP del tuo hosting OVHcloud](https://docs.ovh.com/fr/hosting/connexion-espace-stockage-ftp-hebergement-web/) e trasferisci il backup nella cartella root "ww" (o in un'altra cartella root creata precedentemente).

Ti consigliamo di utilizzare il software [FileZilla](https://docs.ovh.com/fr/hosting/mutualise-guide-utilisation-filezilla/) per trasferire il tuo backup FTP sul tuo hosting.
...
Per il backup del tuo database, [crea un nuovo database](https://docs.ovh.com/fr/hosting/creer-base-de-donnees/) e [importa il backup nel tuo nuovo database](https://docs.ovh.com/fr/hosting/mutualise-guide-importation-dune-base-de-donnees-mysql/).

> [!primary]
>
> OVHcloud propone server di database CloudDB. Per utilizzare questa offerta con il tuo sito Web, consulta la nostra documentazione su questo prodotto nella nostra pagina dedicata <https://docs.ovh.com/fr/clouddb/>.
>
...
> [!success]
>
> Per collegare il tuo nuovo database se utilizzi un Content Management System (CMS) come WordPress, Joomla!, Drupal o PrestaShop, consulta le informazioni sui file di configurazione nello **Step 2** della guida ["Modificare la password di un database"](https://docs.ovh.com/fr/hosting/modifier-mot-de-passe-base-de-donnees/).
>

Dichiarare/autorizzare un dominio esterno su un hosting Web OVHcloud tramite la nostra guida ["Gestione dei multisiti di un hosting Web OVHcloud"](https://docs.ovh.com/fr/hosting/multisites-configurer-un-multisite-sur-mon-hebergement-web/). Dichiarare correttamente il "nome" della cartella di root scelta all'inizio dello [step 4](#step4). Ti ricordiamo che questa è la cartella in cui hai inserito i tuoi file nel tuo spazio di storage FTP.
...
Ricevi allo stesso modo gli indirizzi email presenti presso il tuo provider tramite la nostra guida sulla [creazione di indirizzi email "MX Plan"](https://docs.ovh.com/fr/emails/creation-dune-adresse-e-mail/).

Se hai scelto una soluzione Email Pro o Exchange, consulta la nostra documentazione per creare i tuoi indirizzi email:

- Per Email-Pro: <https://docs.ovh.com/fr/emails-pro/premiere-configuration/>
- Per "Exchange": <https://docs.ovh.com/fr/microsoft-collaborative-solutions/premiere-configuration-exchange/>
...
Una volta completata la migrazione, accedi al tuo indirizzo email OVHcloud utilizzando la [Webmail OVHcloud](https://www.ovhcloud.com/fr/mail/) per verificare che tutte le tue email siano presenti nel nuovo account.
...
- Consulta la guida Per maggiori informazioni sui parametri di configurazione [Gestisci le email MX Plan](https://docs.ovh.com/fr/emails/generalites-sur-les-emails-mutualises/#2-utiliser-le-logiciel-de-votre-choix). e i link alle guide personalizzate per i principali client di posta.

#### Per gli account "Email-Pro":

- Consulta le nostre guide all'aiuto alla configurazione nelle sezioni `Configurazione su computer` e `Configurazione su smartphone` de [la nostra guida sull'offerta Email-Pro](https://docs.ovh.com/fr/emails-pro/).

#### Per gli account email "Exchange":

- Tutte le nostre guide all'aiuto alla configurazione sono disponibili nelle sezioni `Configurazione Exchange su` e `Configurazione Exchange su smartphone` di [la nostra guida sull'offerta Exchange](https://docs.ovh.com/fr/microsoft-collaborative-solutions/).
...
In seguito, trasferisci il tuo dominio nella guida su [trasferisci un dominio in OVHcloud](https://docs.ovh.com/fr/domains/transferer-mon-domaine-generique/).
...
[Generalità sulle email condivise](https://docs.ovh.com/fr/emails/generalites-sur-les-emails-mutualises/).

[Generalità sui server DNS](https://docs.ovh.com/fr/domains/generalites-serveurs-dns/).

[Creare un indirizzo email condiviso](https://docs.ovh.com/fr/emails/creation-dune-adresse-e-mail/).

[Importa un database MySQL](https://docs.ovh.com/fr/hosting/mutualise-guide-importation-dune-base-de-donnees-mysql/).

[Gestire un database da un hosting condiviso](https://docs.ovh.com/fr/hosting/gestion-dune-base-de-donnees-depuis-un-hebergement-mutualise/).
