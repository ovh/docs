...
> 
> This tutorial will help you with common tasks. Nevertheless, we recommend contacting a [specialist provider](https://partner.ovhcloud.com/fr/) if you encounter any difficulties. We will not be able to assist you. You can find more information in the ["Go further"](#go-further) section of this guide.
>

## Requirements

- Have a [domain name](https://www.ovhcloud.com/fr/domains/).
- Have an [OVHcloud shared hosting](https://www.ovhcloud.com/fr/web-hosting/).
- Mog in to your [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr).
...
> Be careful about the changes you are going to make, or contact a [specialised provider](https://partner.ovhcloud.com/fr/) for SEO if necessary.
>
...
Declare your new domain name using our guide on [adding a multisite on your shared hosting](https://docs.ovh.com/fr/hosting/multisites-configurer-un-multisite-sur-mon-hebergement-web/). Also declare the sub-domain as `www` if, for example, you want to have `www.NewDomain.tld` also display your website in addition to `NewDomain.tld`.
...
- Check that your new domain points to the right IP address on your web hosting plan. To retrieve the IP address, log in to your [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr), go to the `Web cloud`{.action} section, click on `Hosting plan`{.action}, select your Hosting, then retrieve **IPv4** in the `General information`{.action} tab.

> [!warning]
>
> If you enable the **country IP** or **CDN** options with your new domain, use the correct IP address using our documentation listing [all shared hosting IP addresses](https://docs.ovh.com/fr/hosting/liste-des-adresses-ip-des-clusters-et-hebergements-web/).
>
...
> - [Activating HTTPS on your website with an SSL certificate](https://docs.ovh.com/fr/hosting/passer-site-internet-https-ssl/)
>
...
> [!warning]
>
> Warning, the operations described in step 2 are extremely sensitive and can have serious consequences for your website if they are not carried out with caution. If you have any doubts, do not try anything and contact a [specialist provider](https://partner.ovhcloud.com/fr/).
>
> Before you take any action, we advise retrieving a [backup of your FTP storage space](https://docs.ovh.com/fr/hosting/restauration-ftp-filezilla-espace-client/) and a [backup of your database](https://docs.ovh.com/fr/hosting/exportation-bases-donnees/). This will allow you to restore your website in the event of an error.
>
...
- PrestaShop: The publisher of this software does not offer any documentation to date for changing the access domain for your website. Please contact the publisher directly on this subject. For more information, click [here](https://help-center.prestashop.com/fr){.external} to go to their official page.

Please note that for these CMSs, you can also edit them directly [in the database](https://docs.ovh.com/fr/hosting/creer-base-de-donnees/#acceder-a-linterface-phpmyadmin). You will need to change the URL for accessing your website in the table provided.
...
To rewrite your URLs with your new domain, [log in to your website database](https://docs.ovh.com/fr/hosting/creer-base-de-donnees/#acceder-a-linterface-phpmyadmin) then replace your old domain with the new one in the corresponding table. 
...
To avoid "*Duplicate-content*" and when your new domain is fully operational with your site, you will need to delete the multisite declaration for your old domain using the guide on managing [multisites on your shared hosting](https://docs.ovh.com/fr/hosting/multisites-configurer-un-multisite-sur-mon-hebergement-web/).
