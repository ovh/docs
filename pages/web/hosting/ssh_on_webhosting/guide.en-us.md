...
- an [OVHcloud web hosting plan](https://www.ovh.com/world/web-hosting/){.external} with SSH access
...
If SSH access is disabled, click `...`{.action} to the right of the user concerned, then `Modify`{.action}. In the window that pops up, enable SSH access, then confirm the modification. If you cannot enable it, ensure that your [OVHcloud web hosting plan](https://www.ovh.com/world/web-hosting/){.external} offers SSH access.
