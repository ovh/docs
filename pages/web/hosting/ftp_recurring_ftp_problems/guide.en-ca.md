...
Choose `SFTP`{.action} or `Enabled`{.action} in the `SSH connection`{.action} section (if you have a [Pro](https://www.ovhcloud.com/en-ca/web-hosting/professional-offer/) or [Performance](https://www.ovhcloud.com/en-ca/web-hosting/performance-offer/) hosting plan), click on `Next`{.action}, then on `Confirm`{.action}.
