...
Start by backing up the data that makes up your (FTP **AND** database files), following the instructions in this [guide](../log-in-to-storage-ftp-web-hosting/) for FTP files and [this one](/pages/web/hosting/sql_database_export) for the database. Import them on your desktop or on an external device, such as a NAS server or an USB key.
