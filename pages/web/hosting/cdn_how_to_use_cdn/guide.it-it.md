...
Le regole figurano in un elenco. Per modificare una regola, clicca sui tre puntini in corrispondenza della `regola`{.action} in questione e seleziona `Modifica la regola`{.action}. o eliminarla cliccando su `Elimina la regola`{.action}.
...
> [!warning]
>
> Per l'installazione di un [modulo in 1 click OVHcloud](../modules-en-1-clic/), il WAF deve essere disattivato per evitare che l'installazione del modulo sia bloccata.
...
- **Purga avanzata**: Personalizza lo spurgo scegliendo gli elementi della cache da svuotare: tutto il sito, una cartella, un'URI, un'estensione di file o tramite un'espressione regolare personalizzata. 
	...
	Nella scheda Multisito, clicca sui tre puntini in corrispondenza del record multisito `e seleziona`{.action} `Purger CDN`{.action}. 
...
Le regole figurano in un elenco. Per modificare una regola, clicca sui tre puntini in corrispondenza della `regola`{.action} in questione e seleziona `Modifica la regola`{.action}. Per eliminarla, clicca su `Elimina la regola`{.action}.
