...
- a database created as part of an [OVH Web Hosting plan](https://www.ovhcloud.com/en-ie/web-hosting/){.external}
...
- **Carry out the import from an SSH command line**: this method involves using the SSH protocol to log in to your storage space, then using commands to interact with the storage space. More advanced knowledge and a specific [OVH Hosting Plan](https://www.ovhcloud.com/en-ie/web-hosting/){.external} are required to use this type of access.
...
First of all, log in to your [OVH Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.ie/&ovhSubsidiary=ie){.external}, click `Web Hosting`{.action}, then select the plan concerned. Next, go to the `Databases`{.action} tab.
