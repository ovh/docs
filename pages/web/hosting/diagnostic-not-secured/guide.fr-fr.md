...
|Dans l'onglet `Zone DNS`{.action}, un avertissement indique que votre domaine utilise d'autres serveurs DNS OVHcloud :<br><br>![warning_other_ovh_dns_srv](images/warning_other_ovh_dns_srv.png){.thumbnail}|Vous devez donc modifier les serveurs DNS de votre domaine, afin qu'ils correspondent à ceux inscrits dans les entrées de type `NS` de la zone. Pour effectuer cette opération, suivez les instructions de [ce guide](../../domains/generalites-serveurs-dns/#acceder-a-la-gestion-des-serveurs-dns-ovhcloud).|
|Dans l'onglet `Zone DNS`{.action}, un avertissement indique que votre domaine utilise des serveurs DNS extérieurs à OVHcloud :<br><br>![warning_other_ovh_dns_srv](images/warning_other_ovh_dns_srv.png){.thumbnail}|Contactez votre webmaster ou les [partenaires OVHcloud](https://partner.ovhcloud.com/fr/) à ce sujet.|
