...
Cette anomalie indique que le répertoire concerné ne contient pas de fichier **index.php** ou **index.html**. En terme de programmation, ce type de fichier constitue le « point d'entrée » de votre site.

Pour résoudre l'anomalie **Index of**, vous devrez donc relier votre domaine au `Dossier racine` qui contient ce fichier **index.php** ou **index.html**.

D'autres configurations erronées peuvent être à l'origine des pages **Index of**, mais l'une des plus courantes est la suivante : 
...
Cochez la case `Modifier également le sous domaine www.mydomain.ovh` et indiquez le répertoire contenant le fichier **index.php** ou **index.html** de votre site comme `Dossier racine`. Faites `Suivant` :
