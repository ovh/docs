...
### Trouver l'offre la plus adaptée à votre besoin
...
#### Que faire en cas de lenteurs ?

- Indiquer que le fait de passer sur une offre supérieure ne garantie pas d'arrêter les lenteurs si la problématique est liée au code. Mettre le lien vers le guide d'optimisation

#### Que faire en cas de dépassement de quota ?

- idem, un plugin défectueux peut être la cause du dépassement de quota en surchargeant la bdd de données inutiles, auquel cas l'upgrade ne servira à rien


### La facturation en cas de changement d'offre

- Préciser qu'on ne peut pas downgrad sur plusieurs formules différentes d'un seul coup
- Parler des proratas en cas de changements d'offre en cours d'abonnements
...
Sélectionnez ensuite la nouvelle formule de votre hébergement et la durée de votre nouvel abonnement. Validez les contrats correspondants puis cliquez sur `Valider`[action].



### Points de vigilances avant toute modification vers une offre inférieure

#### Nombre de sites

(Cas des Kismufi web (pas de multisite à proprement parler) et des start10m (pas de bdd et pas de downgrade possible))

#### Bases de données

- Surveiller le nombre de bases et leur quota

#### CloudDB

- cas des cloudDB offerts avec les perf
