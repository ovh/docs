...
Dans la barre de services à gauche de votre [espace client OVH](https://www.ovh.com/auth){.external}, rendez-vous dans la section `Bases de données`{.action}, puis sur l'instance SQL concernée. Assurez-vous d'être bien positionné sur l'onglet `Informations générales`{.action}.
