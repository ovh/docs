...
- **Carry out the backup from an SSH command line**: this method involves using the SSH protocol to log in to your storage space, then using commands to interact with it. More advanced knowledge and a specific [OVH Web Hosting plan](https://www.ovh.lt/svetainiu-talpinimas/){.external} are required to use this type of access.
