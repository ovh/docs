...
Dans votre [espace client OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr){.external}, rendez-vous dans la section `Bases de données`{.action}, puis sur l'instance SQL concernée. 
...
Après avoir récupéré le numéro du cluster sur lequel est situé votre hébergement, consultez la page [« Liste des adresses IP des clusters et hebergements web »](../liste-des-adresses-ip-des-clusters-et-hebergements-web/){.external}. Vous y trouverez l'adresse IP « **passerelle de sortie** » de chaque cluster.
...
- **sql_mode** : L'option **sql_mode** affecte la syntaxe SQL prise en charge, et les vérifications de validation des données effectués par MySQL/MariaDB.
