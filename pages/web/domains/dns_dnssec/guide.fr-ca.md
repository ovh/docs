...
Pour comprendre comment cette protection fonctionne, nous vous conseillons de consulter cette page : « [Comprendre le DNSSEC](https://www.ovh.com/fr/domaines/service_dnssec.xml){.external} ».
...
> [!primary]
>
> Pour vérifier si votre nom de domaine utilise la configuration DNS OVH : depuis votre [espace client OVH](https://www.ovh.com/auth/?action=gotomanager){.external}, cliquez sur l'onglet `Serveurs DNS`{.action}, une fois positionné sur le domaine concerné.
>

### Étape 1 : accéder à la gestion du nom de domaine

Pour débuter la manipulation, connectez-vous à votre [espace client OVH](https://www.ovh.com/auth/?action=gotomanager){.external}, et assurez-vous d'être positionné dans la partie « Web ». Cliquez alors sur `Hébergements`{.action} dans la barre de services à gauche, puis choisissez le nom de domaine concerné dans la liste.
