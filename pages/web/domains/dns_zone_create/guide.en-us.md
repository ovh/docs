...
Log in to the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=we){.external}. click `Web Cloud`{.action}, then click `Order`{.action}, then `DNS zone`{.action}.
...
If you would like to edit this DNS zone, click `Web Cloud`{.action} in the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=we), then click `Domain names`{.action} and choose the domain name concerned. Go to the `DNS zone`{.action} tab.
...
[Editing an OVHcloud DNS zone](https://www.ovhcloud.com/en/domains/web_hosting_how_to_edit_my_dns_zone/)
