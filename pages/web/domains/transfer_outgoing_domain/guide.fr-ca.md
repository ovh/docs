...
Pour lever la protection contre le transfert de votre nom de domaine OVHcloud, toujours connecté à votre [espace client](https://ca.ovh.com/auth/?action=gotomanager){.external}, cliquez sur `Web Cloud`{.action} dans la barre de services en haut. Rendez-vous dans la section `Domaines`{.action} située à gauche, puis cliquez sur le nom de domaine concerné. Depuis l'onglet `Informations générales`{.action}, décochez la case `Protection contre le transfert`{.action}, puis cliquez sur `Valider`{.action}.
