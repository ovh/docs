...
The DKIM (**D**omain**K**eys **I**dentified **M**ail) record allows you to sign emails to avoid identity theft. This signature works on the principle of hash combined with asymmetric cryptography.

**Find out how DKIM works, and how to set it up for your email service.**

## Requirements

- You must have access to manage the domain name concerned from the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/&ovhSubsidiary=GB), or from your domain service provider if it is registered outside of OVHcloud.
- access to the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/&ovhSubsidiary=GB)
- You must have signed up to an [Exchange](https://www.ovhcloud.com/en-gb/emails/), [Email Pro](https://www.ovhcloud.com/en-gb/emails/email-pro/) or non-OVHcloud email solution with DKIM.

> [!warning]
>
> If your domain name does not use OVHcloud DNS servers, you will need to modify the DKIM using the interface given by the service provider that manages your domain name configuration.
>
...
- [How does DKIM work?](#how-dkim-work)
    ...
    - [How are hash and asymmetric encryption used for DKIM?](#encrypt-and-hash)
...
- [Configuring DKIM for an OVHcloud Email Exchange or Email Pro solution](#internal-dkim)
...
To fully understand why DKIM can secure your email exchanges, you need to understand how it works. DKIM uses "**hash**" and "**asymmetric encryption**" to create a secure signature. Your domain name’s **email platform** and **DNS zone** will help deliver DKIM information to your recipients.

#### Hash <a name="hash"></a>

The principle of a **hash function** is to generate a **signature** (also called a fingerprint) from an input data. Its purpose is to create a fixed sequence of characters at the output, regardless of the amount of input data. 
...
The hash function is useful when you want to check the integrity of a message. Two data that may look very close have a completely different hash value with an equal length of characters in output, regardless of the input length.
...
#### How are hash and asymmetric encryption used for DKIM? <a name="encrypt-and-hash"></a>

From the email platform, DKIM will use the hash to create a signature from certain elements of [the email header](pages/web/emails/diagnostic_headers) and email body (email content).
...
In order for the recipient to verify the sender's DKIM signature, he will need the DKIM parameters and especially the public key to decrypt it. A domain name’s [DNS zone](/pages/web/domains/dns_zone_edit) is public, which is why a DNS record is added to transmit the public key and DKIM settings to the recipient.

#### What is a DKIM selector? <a name="selector"></a>

When you enable DKIM, it works with a public/private key pair. You can assign several pairs of keys to your domain name, for example, as part of a rotation. Indeed, when you change the key pair, the old pair must remain active until all emails you sent with the old key fail to check the DKIM on the incoming server.

For this rotation principle to work, we're going to use something called **DKIM** selectors. A DKIM selector includes a private/public key pair. It is visible as a character string in the DKIM signature of an email by the argument `s=`. This signature is visible in [the email header](pages/web/emails/diagnostic_headers).
...
The value of the selector is here `s=ovhex123456-selector1`.
...
### Configuring DKIM for an OVHcloud Email Exchange or Email Pro solution <a name="internal-dkim"></a>
...
> Click on our guide on ["Find out how to use OVHcloud APIs"](/pages/account/api/first-steps) if you have never used APIs.

Go to the `/email/exchange` (Exchange offers) or `/email/pro` (Email Pro offer) APIs section, and type "dkim" in the `Filter` box to display only the APIs related to DKIM.
...
>> - `autoEnableDKIM`: the DKIM will be directly activated by ticking this box. **Do not tick this box if your domain name is not registered in the same OVHcloud customer account, or in another registrar**.
>> - `configureDkim`: the CNAME record will be automatically added to your domain name’s DNS zone, if it is managed in the same OVHcloud customer account as your Exchange platform. **Do not tick this box if your domain name is not registered in the same OVHcloud Control Panel or in another registrar**.
...
>> > [!primary]
>> >
>> > If your domain name is managed in the same control panel as your platform, and you have ticked `autoEnableDKIM` and `configureDkim`, go directly to the [**Different DKIM states**](dkim-status#) section below to track DKIM activation.
>>
...
>> - your Exchange platform is linked to a domain name that is managed in another registrar ;<br>
...
>> ```
>> Retrieve the `customerRecord` and `targetRecord` values in a text file. Proceed to the next step.
>>
>> > [!primary]
>> >
>> > It is possible that the `status:` or in `todo`, this will not affect your DNS zone’s configuration.
>>
> **4.Configure the DNS record**
>> From [the OVHcloud customer area](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/&ovhSubsidiary=GB) where the name of domain of your Exchange platform, in the `Web Cloud`{.action} tab, click on `Domain names`{.action} in the left column and select the relevant domain name.<br>
>> Go to the `DNS Zone`{.action} tab, then click `Add record`{.action} in the window that appears. Choose `CNAME`, then enter the information according to the values you have entered.<br>
>>
>> If we take the values of the example in step "**3.Retrieve the DNS record**":
>>
>> - `customerRecord: "ovhex123456-selector1._domainkey.mydomain.ovh"` is the subdomain of the CNAME record. We only keep `ovhex123456-selector1._domainkey` because the `.mydomain.ovh` is already prefilled. <br>
>> - `targetRecord: "ovhex123456-selector1._domainkey.1500.ab.dkim.mail.ovh.net"` is the target of the record. Add a full stop at the end to punctuate the value. Cela donne `ovhex123456-selector1._domainkey.1500.ab.dkim.mail.ovh.net.`<br>
>>
...
>> > [!warning]
>> >
>> > In the [**Different DKIM states**](#dkim-status) section of this guide, check that the value `status:` is `ready` before you can activate DKIM.
>>
...
>> > [!success]
>> >
>> > You have now made all the changes to enable DKIM. To ensure that the DKIM is enabled, please refer to the "[**Different DKIM states**](dkim-status#)" section of this guide to check that the `status:` value is well `inProduction`. If this is the case, your DKIM is now active.
>>
...
>> - `autoEnableDKIM`: the DKIM will be directly activated by ticking this box. **Do not tick this box if your domain name is not registered in the same OVHcloud customer account, or in another registrar**.
>> - `configureDkim`: the CNAME record will be automatically added to your domain name’s DNS zone, if it is managed in the same OVHcloud customer account as your Email Pro platform. **Do not tick this box if your domain name is not registered in the same OVHcloud Control Panel or in another registrar**.
...
>>
> **4.Configure the DNS record**
>> From [the OVHcloud customer panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/en-gb/&ovhSubsidiary=fr) where the name of domain of your E-mail Pro platform, in the `Web Cloud`{.action} tab, click on `Domain names`{.action} in the left column and select the relevant domain name.<br>
>> Go to the `DNS Zone`{.action} tab, then click `Add record`{.action} in the window that appears. Choose `CNAME`, then enter the information according to the values you have entered.<br>
>>
>> If we take the values of the example in step **3.Retrieve the DNS record**:
>>
>> - `customerRecord: "ovhemp123456-selector1._domainkey.mydomain.ovh"` is the subdomain of the CNAME record. We only keep `ovhemp123456-selector1._domainkey` because the `.mydomain.ovh` is already prefilled. <br>
>> - `targetRecord: "ovhemp123456-selector1._domainkey.1500.ab.dkim.mail.ovh.net"` is the target of the record. Add a full stop at the end to punctuate the value. Cela donne `ovhemp123456-selector1._domainkey.1500.ab.dkim.mail.ovh.net.`<br>
>>
>> ![email](images/dns-dkim-api02.png){.thumbnail} <br>
>> 
>> Once you have entered the values, click `Next`{.action}, then `Confirm`{.action}.
...
>> > [!success]
>> >
>> > You have now made all the changes to enable DKIM. To ensure that the DKIM is enabled, please refer to the "[**Different DKIM states**](dkim-status#)" section of this guide to check that the `status value:` is well in `Production`. If this is the case, your DKIM is now active.
>>
...
>> ```
> **Email Pro**
>> When you carry out operations on the DKIM of your Email Pro platform, use the API call below to check the current status of the DKIM.
>>
>> > [!api]
>> >
>> > @api {GET} /email/pro/{service}/domain/{domainName}/dkim/{selectorName}
...
#### Enable or change DKIM selector <a name="enable-switch"></a>
...
> [!warning]
>
> The DKIM selector must be `inProduction` status before it can be disabled.
...
- [a CNAME record](#cname-record): A record used for an OVHcloud email solution or a Microsoft email server.

#### DKIM record <a name="dkim-record"></a>

This record is named DKIM on the interface but it is actually a TXT record on output. The purpose of the DKIM record is to make it easier to read the various parameters of the DKIM by presenting them as independent boxes.
...
By specifying a specific value for "g=", you can limit the use of the key to a specific local portion of an email address or a specific range of email addresses by using wildcards (for example: "\*-group").

- **Algorithm (hash) (h=)**: directive specifies the hash algorithms used to sign email headers. Use this tag to define a list of hash algorithms that will be used to generate a DKIM signature for a given message.

- **Key type (k=)**: specifies the signature algorithm used to sign outgoing e-mail messages. It allows recipients to know how the message was signed and what method is used to verify its authenticity.<br>
...
These notes may be useful for documentation purposes or to help administrators understand or manage how DKIM works. The notes in n= are not interpreted by the programmes and do not affect the operation of DKIM.

- **Public key (base64) (p=)**: used to populate DKIM public key data, which is encoded in base64.<br>
...
- **Type of service (s=)**: The "s=" (Service Type) tag is optional and is not present by default. It allows you to specify the type or types of services to which this DKIM record applies.<br>
...
The service types currently defined are "*" (all service types), "email" (email).

- **Test mode (t=y)**: allows domain name owners to test DKIM setup without the risk of having messages rejected or marked as spam if DKIM signature verification fails.<br>
...
- **Subdomains (t=s)**: allows the use of the DKIM signature to be restricted to the domain name only (for example: @mydomain.ovh) or allow sending from the domain name and its subdomains (e.g.: @mydomain.ovh, @test.mydomain.ovh, @other.mydomain.ovh, etc.).

#### TXT record <a name="txt-record"></a>

This is the type of native record used to set the DKIM in your domain name’s DNS zone. You need to have a good command of its syntax to complete it.
...
This is precisely the type of record used to enable DKIM on a domain name declared for an OVHcloud Exchange solution.
...
To retrieve the header of an email, please read our guide on [Retrieving the header of an email](pages/web/emails/diagnostic_headers).

## Go further

Join our community of users on [https://community.ovh.com/en/](https://community.ovh.com).
