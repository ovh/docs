...
|4|Realizar a primeira etapa de validação da transferência|O proprietário do nome de domínio e o administrador informam o novo agente de registo do nome de domínio|Por e-mail|5 dias, no máximo|
...
Depois de desbloquear o seu nome de domínio e obter o código, pode encomendar a sua transferência para a OVHcloud a partir do [nosso site](https://www.ovh.pt/dominios/){.external}. Introduza o seu nome de domínio e siga os passos da encomenda.
...
Também pode concluir a sua encomenda com um [alojamento web](https://www.ovh.pt/alojamento-partilhado/){.external} e outras soluções da OVHcloud. Isto pode interessar-lhe se deseja também migrar os seus serviços para a OVHcloud. O nosso manual intitulado "[Como migrar um site para a OVHcloud](../../hosting/migrar-site-para-ovh/){.external}?" irá fornecer-lhe instruções sobre como realizar este procedimento.
...
Para efetuar a transferência do domínio, é necessário passar por duas etapas de validação. A primeira etapa pode ser realizada desde o início da transferência, mas pode demorar até cinco dias.

|Quem recebe os pedidos de validação?|Para onde é enviado o pedido de validação?|
|---|---|
|O proprietário do domínio|Para o endereço de e-mail do proprietário, presente no Whois (se não estiver ocultado). Caso contrário, será enviada para o endereço de e-mail do proprietário do nome de domínio, introduzido durante o processo de encomenda.|
|O administrador indicado durante o processo de encomenda na OVHcloud.|Para o endereço de e-mail indicado no perfil de administrador na OVHcloud|

As duas partes validam a transferência através de uma interface da OVHcloud. Para aceder, utilize a ligação fornecida nos e-mails enviados.

Nomes de domínio:

Dependendo das ações realizadas pelo proprietário do domínio e pelo administrador, podem ser obtidos vários resultados possíveis nesta etapa.
...
|Um dos contactos recusa o pedido|O processo de transferência é anulado assim que um dos contactos recusar a transferência.|

Em caso de cancelamento de uma transferência, certifique-se de que todas as partes envolvidas estão de acordo antes de tentar novamente. Se não tiver recebido nenhum e-mail, verifique que todos os endereços de e-mail estão atualizados.

Este processo pode ser executado mais tarde na [Área de Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.pt/&ovhSubsidiary=pt){.external}. Selecione `Web Cloud`{.action} e aceda à secção `Nomes de domínio`{.action} e clique em `Operações em curso`{.action}.

### Etapa 5 - Realizar a segunda etapa de validação da transferência

Uma vez a segunda etapa iniciada, o agente de registo do domínio atual (que ainda não é a OVHcloud) receberá um pedido de validação. Aqui também podem ocorrer várias situações.

|Cenários|Resultado|
|---|---|
|Validação do agente de registo atual.|A transferência é realizada num prazo de 24 horas|
