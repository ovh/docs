...
Now that your domain name is unlocked and you have your transfer code, you can request for your domain name to be transferred to OVHcloud. Simply order a domain name transfer on [our website](https://www.ovh.com/world/domains/domain_transfer.xml){.external}. Enter your domain name, and follow the order process.
...
Domains can also be ordered in addition to a [Web Hosting plan](https://www.ovh.com/world/web-hosting/). Our guide on [Migrating your website and emails to OVHcloud](../../hosting/migrating-website-to-ovh/){.external} provides useful information for an optimised approach.
...
The transfer process can be restarted from the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/world/&ovhSubsidiary=we){.external}: Switch to `Web Cloud`{.action} and go to the `Domain names`{.action} section on the left-hand side. Then select `Ongoing operations`{.action}.
