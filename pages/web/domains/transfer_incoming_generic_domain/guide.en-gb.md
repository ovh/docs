...
The transfer process can be restarted from the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/&ovhSubsidiary=GB){.external}. Select `Web Cloud`{.action} and go to the `Domain`{.action} Names section, then click `Ongoing operations`{.action}.
