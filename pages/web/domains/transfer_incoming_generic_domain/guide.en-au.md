...
The transfer process can be restarted from the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com.au/&ovhSubsidiary=au){.external}. Select `Web Cloud`{.action} and go to the `Domain`{.action} Names section, then click `Ongoing`{.action} operations.
