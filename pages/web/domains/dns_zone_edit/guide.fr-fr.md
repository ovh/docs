...
Si nous reprenons l'exemple plus haut, lorsque vous tapez *mydomain.ovh*, les **serveurs DNS** associés à ce nom de domaine seront intérogés. Ces derniers contiennent la **zone DNS** du nom de domaine *mydomain.ovh* ou est renseigné l'adresse IP de l'hébergement de *mydomain.ovh*. Ainsi votre navigateur est en mesure d'afficher le site internet *mydomain.ovh* contenu sur l'hébergement. On appele cela une résolution DNS.
...
> [!warning]
>
> Ne changez pas les enregistrements NS de votre zone DNS, en mode textuel, avec des serveurs DNS externe à OVHcloud, car cette zone DNS ne fonctionne **uniquement** qu'avec des serveurs DNS OVHcloud.
>
...
**SSHFP** : Utiliser pour renseigner l'empreinte d'une clé publique SSH.

**TLSA** : Utiliser pour renseigner l'empreinte d'un certificat SSL/TLS.
...
Pour les utilisateurs avertis uniquement. Depuis l'onglet `Zone DNS`{.action}, cliquez sur `Modifier en mode textuel`{.action}, puis suivez les étapes qui s'affiche;
...
>
>Vous devez alors avoir comme cible *mydomain.ovh.* et nom pas *mydomain.ovh* sans le **.** à la fin.
