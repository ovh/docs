...
Aceda à Área de [Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager){.external} e dirija-se à secção `Web Cloud`{.action}. Selecione `Microsoft`{.action} na coluna à esquerda e, a seguir, `Exchange`{.action}. Clique no nome do serviço Hosted Exchange em questão.
