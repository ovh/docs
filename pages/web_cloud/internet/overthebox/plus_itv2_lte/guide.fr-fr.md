...
- Ou un autre modèle **OverTheBox** fournie par OVHcloud ou une [installation depuis le projet Open Source](/pages/web_cloud/internet/overthebox/advanced_installer_limage_overthebox_sur_votre_materiel), ainsi qu'une clé LTE Huawei E3372 ou un téléphone.
...
> - Vous décharger en touchant un objet métallique relié à la terre afin de limiter les risques de décharge électrostatique.
> [Plus d'informations](https://fr.wikihow.com/%C3%A9viter-d%27endommager-un-ordinateur-par-une-d%C3%A9charge-%C3%A9lectrostatique)
> - Vérifiez également que le boitier est éteint et que son alimentation est **débranché**.
> - Munissez vous d'un d'un tournevis cruciforme.
>
...
Une fois la carte SIM insérée, vous devez refermer le boitier en répétant les mêmes étapes dans l'ordre inverse et veiller à ce que toutes les vis soit utilisées sans trop serrer.

- Rebranchez la carte 4G en alignant ses encoches avec les crans du connecteur PCIe, puis insérez-la dans le connecteur en observant un angle de 30 degrés. Ne pas forcer pour reinsérer le carte 4G.  Insérer la vis dans le support prévu dans la carte mère pour sécuriser la carte 4G. Ne serrez pas trop la vis.
- Remettez la façade extérieur et sécuriser la avec les **6 vis** précédemment retiré.
...
Une fois les antennes installées, brancher l'alimentation du boitier et allumez le. Attendez environ une minute après le bip sonore.
...
L'interface 4G est configuré automatiquement, cependant certains opérateur requièrent une configuration spécifique de l'APN ou l'utilisation d'un code PIN particulier pour utiliser la carte SIM.

Depuis le menu déroulant sélectionnez `Network > Interfaces`{.action}.
...
> [!success]
>
> La configuration est terminé, vérifiez le bon fonctionnement de votre lien en suivant la section
> [Vérifier le fonctionnement du lien 4G](./#aller-plus-loin)
>
...
> [!alert]
>
> Attention les clés Brovî E3372-**325** ne sont pas nativement compatibles avec OverTheBox
>
...
> [!warning]
>
> Si l'interface n'est pas entouré d'un halos vert, cela signifie que le lien n'est pas fonctionnel.
> - Dans le cas de l'**OTB V3 LTE**, vérifiez la configuration des APN ainsi que le PIN de la carte SIM.
> - Pour les autre modèle, vérifiez le status de la clé USB.
>
...
Les liens mobiles sont souvent associés à des quotas de données, si vous souhaitez limiter l'utilisation du lien pour que l’agrégation se fasse seulement en cas de soucis sur le lien principal, il est possible de modifier le comportement de l’agrégation. Changer le comportement est également recommandé en cas de différence importante entre le débit de l'interface 4G et le lien principal (par exemple dans une configuration avec un lien fibre à 1Gbps et un lien 4G à 15Mbps).
...
- **Backup**: L’agrégation est passive, le lien n'est utilisé que lorsqu'aucune autre interface est disponible. Ce paramètre doit être sélectionné si vous souhaitez utiliser le lien qu'en cas de panne du lien principal (en _fail-over_).

Les options suivante sont également possible, mais ne sont **pas recommandé**:
- **Master**: Similaire au mode **Enabled**. L'interface est considéré comme principale. Ce paramètre n'est utilisable que sur une seule interface. Ce mode n'est pas recommandé
...
### Configurer manuellement un interface

Pour plus de détails sur la configuration des interfaces, reporter vous sur le guide « [Comment configurer une interface réseau ?](/pages/web_cloud/internet/overthebox/advanced_creer_une_interface_modem_manuellement) »
