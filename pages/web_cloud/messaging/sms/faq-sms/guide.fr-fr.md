...
Le solde de crédits est consultable en temps réel depuis l'espace client ou via l'API.
...
[Accédez à la section SMS](/links/control-panel/telecom-sms) de votre espace client OVHcloud, sélectionnez votre compte SMS puis l'onglet `Message et campagne`{.action} > `SMS envoyés`{.action} ou `SMS reçus`{.action}. Vous pouvez filtrer l'historique par date, statut de livraison, numéro de destinataire ou expéditeur. L'historique est conservé pendant 6 mois. L'historique peut également être exporté au format CSV pour analyse.
...
OVHcloud permet l'envoi de SMS depuis votre messagerie e-mail. Envoyez un e-mail à l'adresse `numéro_destinataire@email2sms.ovh.net` (ex : `0033612345678@email2sms.ovh.net`). Le corps de l'e-mail constitue le contenu du SMS. L'adresse e-mail expéditrice doit au préalable être autorisée dans les paramètres de votre compte SMS (`Options`{.action} > `Email2SMS`{.action}). L'objet de l'e-mail doit contenir vos identifiants au format : `compte:login:password`. Cette méthode est particulièrement utile pour des alertes automatisées depuis des systèmes qui ne supportent que l'envoi d'e-mails (serveurs de monitoring, applications métier).
...
Le suivi de la campagne est disponible dans `Message et campagne`{.action} > `SMS envoyés`{.action}.
...
[Accédez à la section SMS](/links/control-panel/telecom-sms) de votre espace client OVHcloud, sélectionnez votre compte SMS, puis accédez à `Contacts`{.action} > `Listes de destinataires`{.action}. Vous pouvez :
...
Les carnets d'adresses SMS offrent une gestion plus riche des contacts (nom, prénom, numéro). [Accédez à la section SMS](/links/control-panel/telecom-sms), sélectionnez votre compte SMS, puis accédez à `Contacts`{.action} > `Carnets d'adresses`{.action}. Vous pouvez :
...
2. **Consultez l'historique d'envoi :** dans `Message et campagne`{.action} > `SMS envoyés`{.action}, vérifiez le statut de chaque SMS. Un code PTT indique la raison de l'échec.
...
- Le destinataire peut **répondre** au SMS, et les réponses sont consultables dans l'espace client ou récupérables via l'API.
- Il peut être utilisé pour les envois contenant des URL.
...
4. La réponse est consultable dans votre espace client (onglet `SMS reçus`{.action}).
...
1. **Créez des clés API** depuis https://api.ovh.com/createToken/ en autorisant les endpoints `/sms/*`.
...
L'API retourne un identifiant de job permettant de suivre le statut d'envoi. Des librairies sont également disponibles en Python, Node.js, Java et C#. La documentation complète de l'API SMS est disponible sur https://api.ovh.com/console/#/sms.
...
La documentation interactive complète est disponible sur https://api.ovh.com/console/#/sms. L'authentification se fait via des clés API (Application Key, Application Secret, Consumer Key).
...
Le protocole SMPP (Short Message Peer-to-Peer) est un protocole industriel pour l'envoi et la réception de SMS en volume. [Accédez à la section SMS](/links/control-panel/telecom-sms) de votre espace client OVHcloud, sélectionnez votre compte, puis accédez à `Gestion SMPP`{.action}. Vous obtiendrez les paramètres de connexion : adresse du serveur SMPP, port, system_id et password. Le protocole SMPP offre une connexion persistante, un débit d'envoi plus élevé, et un support natif des DLR en mode push.
...
- **Email2SMS :** envoi par e-mail, utile pour les systèmes qui ne supportent que l'envoi d'e-mails (alertes de monitoring, ERP).
...
- **Via l'espace client :** pas de limite explicite, mais les campagnes volumineuses sont étagées dans le temps par la plateforme.
...
Chaque requête HLR coûte **0,1 crédit SMS**. Le résultat indique : le statut du numéro (actif/inactif), l'opérateur d'enregistrement, la joignabilité, le statut de portabilité, et le statut d'itinérance. Les requêtes HLR sont accessibles depuis l'espace client (`SMS`{.action} > `HLR`{.action}) ou via l'API.
...
Le service SMS OVHcloud permet l'envoi vers la grande majorité des destinations internationales. Cependant :

- **Destinations bloquées :** certaines destinations à haut risque de fraude peuvent être bloquées par défaut.
- **Tarification variable :** le coût en crédits varie selon la destination. Les tarifs sont consultables dans votre espace client ou sur la [page tarifaire OVHcloud](/links/telecom/sms-prices).
...
Avant de lancer une campagne internationale, vérifiez la tarification et la disponibilité de la destination dans votre espace client.
