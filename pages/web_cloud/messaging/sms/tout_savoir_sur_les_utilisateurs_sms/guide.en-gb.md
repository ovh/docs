...
Click on the `API users`{.action} tab. To add a user, click `Add a user account`{.action}.
...
You can manage quotas in the `API users`{.action} tab. Click the `Actions`{.action} button (ellipsis icon) next to the user concerned, then `Quota`{.action}.
...
To configure a limit for a user, stay in the same menu, click the `Actions`{.action} button (ellipsis icon), then `Limit`{.action}.
...
To enable it, click the `Actions`{.action} button (ellipsis icon) to the right of the user, then `Restrictions`{.action}.
...
To set up custom tracking for SMS delivery (Delivery reporting or DLR), you can specify a callback URL by clicking the `Actions`{.action} button (ellipsis icon) to the right of a user, then `Callback`{.action}.
