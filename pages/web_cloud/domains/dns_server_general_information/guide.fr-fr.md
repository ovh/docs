...
|---|---|---|
|Résolveur DNS (DNS resolver ou DNS recursive)|Premier serveur qui reçoit la requête DNS émise par un client (navigateur internet, logiciel de messagerie, etc.). Cette étape est représentée par l'étape **1** du schéma ci-dessus. Ce serveur fait la passerelle entre le client et le reste du réseau DNS. Il interroge les trois autres types de serveur DNS jusqu'à ce qu'il récupère l'adresse IP, demandée par la requête DNS, auprès du serveur DNS de référence.Le client envoi la requête DNS pour connaître l'enregistrement DNS de type A pour connaître l'adresse IP du nom de domaine *domain.tld*|Le navigateur internet envoi une requête DNS pour connaître l'adresse IP du nom de domaine *domain.tld*. Ceci afin de connaître le serveur où est hébergé le site web associé au nom de domaine *domain.tld*.|
