...
>>
>> **3 - Modification:** Modify each remaining "A" record for the domain name alone by clicking the `...`{.action} button, then clicking `Edit record`{.action}. Replace the target with one of the 4 SquareSpace IPv4 addresses (a different address per record):
>>
>> - `198.185.159.144`
>> - `198.185.159.145`
>> - `198.49.23.144`
...
>>
>> **3 - Modification:** If a "CNAME" record exists for the "www" subdomain, click the `...`{.action} button, then click `Edit record`{.action}. Replace only the **Target** with `ext-cust.squarespace.com.`. Click `Next`{.action} and confirm.
>>
