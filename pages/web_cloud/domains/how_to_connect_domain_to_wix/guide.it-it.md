...
>>
>> **3 - Modifica:** se esiste un record "A" per il nome di dominio, clicca sul pulsante `...`{.action} e poi su `Modifica il record`{.action}. Lascia il campo **Sottodominio** vuoto e sostituisci la destinazione con l'indirizzo IPv4 recuperato dalla tua interfaccia Wix. Clicca su `Continua`{.action} e conferma.
>>
>> Se non esiste alcun record "A", clicca su `Aggiungi un record`{.action} in alto a destra, seleziona il campo di puntamento `A`{.action}, lascia il campo **Sottodominio** vuoto e inserisci l'indirizzo IPv4 recuperato dalla tua interfaccia Wix nel campo **Destinazione**. Clicca su `Continua`{.action} e conferma.
>>
