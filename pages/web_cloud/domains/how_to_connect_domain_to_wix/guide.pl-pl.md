...
Jesteś abonentem nazwy domeny w OVHcloud i chcesz ją połączyć z hostingiem Wix. W tym przewodniku znajdziesz etapy przygotowania i konfiguracji strefy DNS OVHcloud, aby umożliwić konfigurację hostingu Wix.

**Dowiedz się, jak powiązać nazwę domeny OVHcloud z hostingiem Wix**

> [!warning]
>
> - Pomoc Wix nie ma dostępu do ustawień Twojej nazwy domeny OVHcloud, a zatem nie może udzielić Ci porad dotyczących informacji, które należy jej dostarczyć.
>
...
Jeśli korzystasz z hostingu Wix dla nazwy domeny OVHcloud, musisz najpierw przygotować hosting, postępując zgodnie z instrukcjami z **etapu 1** na [**tej stronie dokumentacji Wix**](https://support.wix.com/en/article/connecter-un-domaine-%C3%A0-wix-par-pointage-5727882).
...
> - Apri [**questa pagina della documentazione Wix**](https://support.wix.com/en/article/connected-un-domaine-%C3%A0-wix-par-pointage-5727882).
...
>>
>> - Kliknij przycisk `...`{.action} po prawej stronie wiersza tabeli, który odnosi się tylko do Twojej nazwy domeny, bez subdomeny (na przykład: `mydomain.ovh.`), a następnie kliknij `Zmień rekord`{.action}.
>> - Jeśli istnieje rekord dla subdomeny "www." (przykład: `www.mydomain.ovh.`), należy go usunąć, aby nie kolidował z rekordem CNAME, który wprowadzisz w etapie 3. Kliknij przycisk `...`{.action} po prawej stronie wiersza tabeli odnoszącego się tylko do Twojej nazwy domeny i subdomeny "www.", a następnie kliknij `Usuń rekord`{.action}.
...
>>
>> - Jeśli dla samej nazwy domeny (np. `mydomain.ovh.`) i dla jej subdomeny podano "www." (np. `www.mydomain.ovh.`) są widoczne rekordy "AAAA", należy je usunąć, aby nie kolidowały z rekordami "A" i "CNAME", które wprowadzisz w etapie 4. Kliknij przycisk `...`{.action} po prawej stronie wiersza tabeli odnoszącego się tylko do Twojej nazwy domeny i subdomeny "www.", a następnie kliknij `Usuń rekord`{.action}.<br>
> **Krok 3**
>> **Rekord TXT**<br><br>
