...
Please refer to our documentation on [DNS records and editing a DNS zone](/pages/web_cloud/domains/dns_zone_edit) in your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg) if you would like to learn more about this topic.
...
To access this feature, log in to your [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg) then go to the `Web Cloud`{.action} section on the top of the interface. In the left-hand column, go to the `Domain names`{.action} tab, then select the domain name linked to the DNS zone you want to manipulate.
