...
No separador `Informações gerais`{.action}, encontrará o cursor `Proteção contra a transferência` sob **Configuração**. Por predefinição, esta proteção está `Ativada`{.action}.
