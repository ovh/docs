...
>>
>> **4 - Addition:** If fewer than 2 "A" records existed, create the missing records. Click `Add an entry`{.action} at the top right, select the `A`{.action} pointer record, leave the **Subdomain** field blank and enter in the **Target** field each IPv4 address not yet assigned. Click `Next`{.action} and confirm.
>>
...
>>
>> **3 - Addition:** Create a TXT verification record. Click `Add an entry`{.action} at the top right, select the `TXT`{.action} pointer record, enter `_webflow` in the **Subdomain** field and in the **Target** field the value of type `one-time-verification=XXXXXXXX` found in the `Site settings > Publishing tab > Production`{.action} section of your Webflow account. Click `Next`{.action} and confirm.
>>
