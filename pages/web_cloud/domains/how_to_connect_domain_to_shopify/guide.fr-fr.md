...
>>
>> **3 - Modification :** si un enregistrement « CNAME » existe pour le sous-domaine « www », cliquez sur le bouton `...`{.action} puis sur `Modifier l'entrée`{.action}. Remplacez uniquement la **Cible** par `shops.myshopify.com.`. Cliquez sur `Suivant`{.action} et validez.
>>
>> Si aucun enregistrement « CNAME » n'existe pour le sous-domaine « www », cliquez sur `Ajouter une entrée`{.action} en haut à droite, sélectionnez le champ de pointage `CNAME`{.action}, saisissez `www` dans le champ **Sous-domaine** et `shops.myshopify.com.` dans le champ **Cible**. Cliquez sur `Suivant`{.action} et validez.
