...
>>
>> **3 - Modificação:** se existir um registo "A" para o nome de domínio apenas, clique no botão `...`{.action} e depois em `Alterar a entrada`{.action}. Deixe o campo **Subdomínio** vazio e substitua o destino pelo endereço IPv4 do Shopify: `23.227.38.65`. Clique em `Seguinte`{.action} e valide.
>>
>> Se não existir nenhum registo "A", clique em `Adicionar uma entrada`{.action} no canto superior direito, selecione o campo de apontamento `A`{.action}, deixe o campo **Subdomínio** vazio e introduza `23.227.38.65` no campo **Destino**. Clique em `Seguinte`{.action} e valide.
>>
...
>>
>> **3 - Modificação:** se existir um registo "AAAA" para o nome de domínio apenas, clique no botão `...`{.action} e depois em `Alterar a entrada`{.action}. Deixe o campo **Subdomínio** vazio e substitua o destino pelo endereço IPv6 do Shopify: `2620:0127:f00f:5::`. Clique em `Seguinte`{.action} e valide.
>>
>> Se não existir nenhum registo "AAAA", clique em `Adicionar uma entrada`{.action} no canto superior direito, selecione o campo de apontamento `AAAA`{.action}, deixe o campo **Subdomínio** vazio e introduza `2620:0127:f00f:5::` no campo **Destino**. Clique em `Seguinte`{.action} e valide.
>>
...
>>
>> **3 - Modificação:** se existir um registo "CNAME" para o subdomínio "www", clique no botão `...`{.action} e depois em `Alterar a entrada`{.action}. Substitua apenas o **Destino** por `shops.myshopify.com.`. Clique em `Seguinte`{.action} e valide.
>>
>> Se não existir nenhum registo "CNAME" para o subdomínio "www", clique em `Adicionar uma entrada`{.action} no canto superior direito, selecione o campo de apontamento `CNAME`{.action}, introduza `www` no campo **Subdomínio** e `shops.myshopify.com.` no campo **Destino**. Clique em `Seguinte`{.action} e valide.
