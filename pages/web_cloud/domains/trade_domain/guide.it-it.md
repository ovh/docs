...
excerpt: "Questa guida ti mostra come modificare l'intestatario di un dominio e aggiornare le informazioni associate a quest’ultimo"
updated: 2026-02-10
---

## Obiettivo

Per registrare un dominio, oltre alla [gestione dei contatti](/pages/account_and_service_management/account_information/managing_contacts), è necessario inserire i dati dell’interessario. In questo contesto, l'**intestatario** designa una persona, una società, un'organizzazione o un'associazione che detiene i diritti su questo dominio. **Il cambio di intestatario** (trasferimento di nome a dominio) si riferisce all’atto di modificare queste informazioni in modo legalmente vincolante, cioè il trasferimento dei diritti di utilizzo a una nuova persona o società. Ad esempio, questa procedura è obbligatoria quando un’azienda cambia ragione sociale.
...
>> >
>> > Se modifichi **solo** i dati dell'intestatario diversi da quelli indicati sopra, effettua le modifiche direttamente e poi clicca su `Confermare`{.action}. In questo caso specifico, non sarà necessario avviare una richiesta di modifica dell'intestatario. Per questa operazione non è necessario confermare le modifiche via email.
>> >
>> > Se necessario e per avviare la richiesta di modifica dell'intestatario, clicca sul link in fondo alla pagina e intitolato `Clicca qui per continuare`{.action}.
>>
...
>
> - Se l'intestatario del nome di dominio è stato modificato, il nome di dominio non potrà essere [trasferito verso un altro Registrar](/pages/web_cloud/domains/transfer_outgoing_domain) per un periodo di 60 giorni.
