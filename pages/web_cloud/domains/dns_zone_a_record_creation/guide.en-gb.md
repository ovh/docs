...
> **Step 3**
>>
>> In the window that opens, select the `A`{.action} record type.
>>
> **Step 4**
>>
>> Enter the IP address (e.g.: `203.0.113.0`) of the service your website is hosted on (web hosting plan, dedicated server, VPS, etc.) in the `Target *` field, then click `Next`{.action}.
>>
> **Step 5**
>>
...
> **Step 4**
>>
>> Enter the subdomain in the `Sub-domain` field (e.g.: `www` for the subdomain `www.domain.tld`), and the IP address (e.g.: `203.0.113.0`) of the service your website is hosted on (web hosting plan, dedicated server, VPS, etc.) in the `Target *` field. Click `Next`{.action}.
>>
> **Step 5**
>>
>> Check the summary, then click `Confirm`{.action}. It takes up to **24** hours for the change to propagate fully over the DNS network.
