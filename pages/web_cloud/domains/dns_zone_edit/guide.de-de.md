...
>> >
>> > Wenn Sie ein Let's Encrypt SSL-Zertifikat für Ihre Domain auf einem Shared Hosting von OVHcloud verwenden und einen CAA-Eintrag verwenden, verhindert dieser die erneute Erstellung des Let's Encrypt SSL-Zertifikats.
>>
> **NAPTR**
...
Durch das Zurücksetzen Ihrer DNS-Zone können Sie zu einer minimalen Konfiguration zurückkehren. Dabei werden die Standardeinträge von OVHcloud oder Ihrer Dienste verwendet. Sie können Ihren Domainnamen auch auf personalisierte E-Mail- und Webhosting-Dienste verweisen.
...
Klicken Sie im Tab `DNS Zone`{.action} auf `Meine DNS Zone zurücksetzen`{.action} und folgen Sie den 2 angezeigten Schritten.

![dnszone](images/reset-my-dns-zone.png){.thumbnail}

**Schritt 1**

Beantworten Sie die Frage `Möchten Sie die minimalen Einträge beim Zurücksetzen Ihrer DNS Zone aktivieren?`. Durch das Festlegen von Mindesteinträgen in einer DNS-Zone wird verhindert, dass eine Anfrage nach dem Domainnamen zu einem Fehler führt.
...
Unabhängig davon, welche Option Sie in Schritt 1 auswählen, müssen Sie bei der Abfrage Ihres Domainnamens eine Antwort festlegen, um eine fehlerhafte DNS-Antwort zu vermeiden.

Wählen Sie beide Optionen aus, indem Sie auf die folgenden Registerkarten klicken.
...
>> - `Benutzerdefiniert`: Legen Sie den IPv4-Wert ([A-Eintrag](#pointer-records)) des Webhostings fest, auf das verwiesen werden soll. <br><br>
>> ![dnszone](images/dns-zone-reset-01.png){.thumbnail}
>>
> **Adresse Ihres Mailservers**
>> - `Weiterleitung`: Ihr Domainname wird auf die Server für die E-Mail-Weiterleitung verweisen. Diese Wahl. Es ist besonders nützlich, wenn Sie keine E-Mail-Angebote haben, aber die E-Mails an eine oder mehrere E-Mail-Adressen außerhalb Ihrer Domain weiterleiten möchten.<br>
>> - `OVHcloud E-Mail-Server`: zu definieren, wenn Sie ein Webhosting-E-Mail-Angebot haben.<br>
>> - `Personalisiert`: Legen Sie die URL und die Priorität des E-Mail-Servers ([MX-Eintrag](#mail-records)) fest, auf den verwiesen werden soll.<br><br>
>> ![dnszone](images/dns-zone-reset-01.png){.thumbnail}
>>
