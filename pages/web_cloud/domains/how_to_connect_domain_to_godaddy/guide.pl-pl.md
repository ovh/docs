...
>>
>> **3 - Modyfikacja:** Jeśli istnieje rekord "A" dla samej nazwy domeny, kliknij przycisk `...`{.action}, a następnie kliknij `Zmień wpis`{.action}. Pozostaw pole **Subdomena** puste i zastąp cel adresem IPv4 z interfejsu GoDaddy. Kliknij `Dalej`{.action} i zatwierdź.
>>
>> Jeśli nie istnieje żaden rekord "A", kliknij `Dodaj wpis`{.action} w prawym górnym rogu, wybierz typ rekordu `A`{.action}, pozostaw pole **Subdomena** puste i wprowadź adres IPv4 z interfejsu GoDaddy w polu **Cel**. Kliknij `Dalej`{.action} i zatwierdź.
>>
