...
>>
>> **3 - Modification:** If an "A" record exists for the domain name alone, click the `...`{.action} button, then click `Edit record`{.action}. Leave the **Subdomain** field blank and replace the target with the IPv4 address from your GoDaddy interface. Click `Next`{.action} and confirm.
>>
...
>>
>> **3 - Modification:** If a "CNAME" record exists for the "www" subdomain, click the `...`{.action} button, then click `Edit record`{.action}. Replace only the **Target** with the value from your GoDaddy interface. Click `Next`{.action} and confirm.
>>
