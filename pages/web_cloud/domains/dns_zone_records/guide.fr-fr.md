...
Au préalable, nous vous recommandons de consultez nos guides « [Qu'est ce qu'un serveur DNS ?](pages/web_cloud/domains/dns_zone_general_information) » et « [Qu'est ce qu'une zone DNS ?](pages/web_cloud/domains/dns_zone_general_information) » dans cet ordre pour plus d'informations.
...
>> > 
>> > Pour reprendre l’exemple donné ci-dessus, vous ne pourrez pas créer un champ CNAME pour le domaine *domain.tld* dans la zone DNS que vous avez créé pour celui-ci.
>> > Vous pourrez cependant créer des champs CNAME avec tous les sous-domaines (exemples : *subdomain.domain.tld* ou *www.domain.tld*) du domaine *domain.tld* dans la zone DNS créée pour *domain.tld*.
>> >
