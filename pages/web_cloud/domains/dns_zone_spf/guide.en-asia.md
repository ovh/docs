...
If your name uses OVHcloud DNS servers, you can check if an SPF record is already configured for it.
...
- **An SPF record that corresponds to OVHcloud email solutions is displayed**: Your name currently uses the OVHcloud configuration. If you no longer wish to use it, you must modify it in the next step).
...
You have chosen the `SPF`{.action} record and would like to apply the OVHcloud configuration. This means that all of the OVHcloud outgoing email servers are included for the MX Plan solutions included in an OVHcloud [web hosting plan](/links/web/hosting)
...
Choose the `TXT`{.action} type record and enter the requested information. In the `Value` field, enter the full SPF string for your domain name.
