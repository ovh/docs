...
Rekord DMARC możesz dodać do strefy DNS w Panelu klienta OVHcloud. W tym celu zaloguj się do [Panelu klienta OVHcloud](/links/manager) i przejdź do sekcji `Web Cloud`{.action}. W lewej kolumnie wybierz w sekcji `Nazwy domen`{.action} odpowiednią nazwę domeny, następnie kliknij na zakładkę `Strefa DNS`{.action}, aby przejść do strefy DNS.
...
Możesz dodać rekord TXT do strefy DNS w [Panelu klienta OVHcloud](/links/manager) i przejść do sekcji `Web Cloud`{.action}. Kliknij `Domeny`{.action}, następnie nazwę wybranej nazwy domeny. Teraz przejdź do zakładki `Strefa DNS`{.action}.
