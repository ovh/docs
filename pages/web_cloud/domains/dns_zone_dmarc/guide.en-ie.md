...
>
> In addition, the **DMARC** record is currently incompatible with any [OVHcloud email solution](https://www.ovhcloud.com/en-ie/emails/). This tutorial is **only** for customers who have an active DNS zone at OVHcloud for their domain name associated with an **external email solution**.
>
...
You can add the TXT record to your DNS zone from the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.ie/&ovhSubsidiary=ie). Click the `Web Cloud`{.action} tab, go to `Domain names`{.action}, then choose the domain name concerned. Go to the `DNS Zone`{.action} tab.
