...
[Zonemaster](https://zonemaster.net/en/run-test) ist ein Tool, das aus der Zusammenarbeit zwischen der [AFNIC](https://www.afnic.fr/en/) (französische Registry) und der [Swedish Internet Foundation](https://internetstiftelsen.se/en/) (Schwedische Registry) hervorgegangen ist. So können Sie die DNS-Konfiguration (Domain Name System) einer Domainname analysieren und herausfinden, welche Elemente verbessert oder korrigiert werden können.
...
Um die aktuelle Konfiguration einer Domainname zu überprüfen, geben Sie Ihren Domainnamen ein und klicken Sie dann auf `Run`{.action}.
...
Um eine DNS-Konfiguration zu überprüfen, die für die betreffende Domainname erstellt, aber noch nicht angewandt wurde, aktivieren Sie das Feld `Options`{.action} und geben Sie folgende Angaben ein:

- **Nameservers**: Tragen Sie die der Domainname zugewiesenen DNS-Server ein. Klicken Sie auf `+`{.action}, um einen zusätzlichen Nameserver hinzufügen zu können. Die Eingabe einer IP-Adresse ist optional.
...
> [!primary]
>
> Wenn Sie eine Domainname angeben und auf die Buttons `Fetch NS from parent zone`{.action} und `Fetch DS from parent zone`{.action} klicken, erscheinen die der Domainname zugewiesenen DNS-Server sowie die Informationen zum DS-Eintrag (DNSSEC), wenn dieser konfiguriert ist.
...
[DNS-Server einer OVHcloud Domainname ändern](/pages/web_cloud/domains/dns_server_general_information)

[Bearbeiten der OVHcloud DNS-Zone](/pages/web_cloud/domains/dns_zone_edit).

[Domainname mit DNSSEC sichern](/pages/web_cloud/domains/dns_dnssec)
