...
* **The Web PaaS [Router cache](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/cache)**.  Every project includes a router instance, which includes optional HTTP caching.  It is reasonably configurable and obeys HTTP cache directives, but does not support push-based clearing.  If you are uncertain what caching tool to use, start with this one.  It is more than sufficient for the majority of use cases.
...
The solution is to specifically allow the cookies that should impact the cache and include only the application session cookie(s).  For the Router cache see [our documentation](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/cache#cookies).  For other cache systems consult their documentation.
