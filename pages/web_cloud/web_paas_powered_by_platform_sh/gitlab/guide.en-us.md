...
When you run `webpaas get <projectID>` or use the clone command shown in the "Git" dropdown in the management console to clone the project, you will actually be cloning from your remote integrated repository, so long as you have the [appropriate access to do so](/pages/web_cloud/web_paas_powered_by_platform_sh/administration-users#user-access-and-integrations). 
