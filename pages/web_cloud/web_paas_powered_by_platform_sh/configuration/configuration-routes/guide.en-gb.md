...
* `cache` controls [caching behavior of the route](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/cache).
* `ssi` controls whether Server Side Includes are enabled. For more information: see [SSI](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/ssi).
* `redirects` controls [redirect rules](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/redirects) associated with the route.
...
In this example, we will route both the apex domain and the www subdomain to an [application called "app"](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/name), the www subdomain being redirected to the apex domain using an HTTP 301 `Moved Permanently` response.
...
If your project involves only a single apex domain with one app or multiple apps under subdomains, it's generally best to use the `{default}` placeholder.  If you are running [multiple applications](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/multi-app) on different apex domains then you will need to use a static domain for all but one of them.
