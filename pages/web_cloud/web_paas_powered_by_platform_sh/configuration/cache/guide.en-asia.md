...
To set a cache lifetime for static resources configure that in your [.platform.app.yaml](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/web#locations) file. All static assets will have a Cache-Control header with a max age defaulting to 0 (which is the default for `expires` in the `.platform.app.yaml`).
