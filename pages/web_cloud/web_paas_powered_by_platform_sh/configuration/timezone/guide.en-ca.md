...
The `timezone` property sets the timezone for which the `spec` property of any [cron tasks](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/configuration-app/cron) defined by the application will be interpreted.
...
* PHP runtime - You can change the timezone by providing a [custom php.ini](/pages/web_cloud/web_paas_powered_by_platform_sh/languages/php/languages-php/ini) or by adding the following lines to your `.platform.app.yaml` file:
