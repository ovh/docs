...
When defining a [Worker](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration-app/workers) instance it is important to keep in mind what mount behavior is desired.  Unless the `mounts` block is defined within the `web` and `workers` sections separately, a top level `mounts` block will apply to both instances.  However, `local` mounts will be a separate storage area for each instance while `service` mounts will refer to the same file system.  For example:
