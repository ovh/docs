...
Set variables [in code](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/configuration-app/variables) using the `.platform.app.yaml` file.
...
The following example shows nested structures introduced in an [app configuration](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/configuration-app/variables):
...
To apply a setting to all environments or to vary them between different PHP containers in the same project, specify the variables in the `.platform.app.yaml` file for your application. See the [PHP configuration page](/pages/web_cloud/web_paas_powered_by_platform_sh/languages/php/languages-php/ini) for more information.
