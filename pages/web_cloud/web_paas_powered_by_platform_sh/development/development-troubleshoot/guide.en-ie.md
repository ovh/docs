...
If you're using [MariaDB 10+](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/mysql), you can use the SQL query `SHOW FULL PROCESSLIST \G` to list DB queries waiting for locks.  Find output like the following, and start debugging.
...
Everything will be read-only, except the writable [mounts](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/storage) you declare.  Writable mounts are there for your data: for file uploads, logs and temporary files. Not for your code.  In order to change code on Web PaaS you have to go through Git.
...
* Find the most visited pages and see if they can be cached and/or put behind a CDN.  You may refer to [how caching works](/pages/web_cloud/web_paas_powered_by_platform_sh/configuration/cache).
