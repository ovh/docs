...
New environments can be created in moments through a Git clone, via the [Web PaaS Command Line Interface](/pages/web_cloud/web_paas_powered_by_platform_sh/development/development-cli), or via the [Web PaaS web console](/pages/web_cloud/web_paas_powered_by_platform_sh/administration/administration-web), and will be an exact replica of their parent environments. This means new environments will have all of the data, as well as the services (like databases, network storage, queues, routing, etc.) instantly cloned. 
