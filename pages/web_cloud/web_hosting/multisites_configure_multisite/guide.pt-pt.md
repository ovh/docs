...
Pode hospedar vários sítios Web numa mesma oferta de alojamento Web, mesmo que os Nomes de domínio não estejam registados na OVHcloud.

Quer adicionar um novo sítio Web ao seu alojamento Web?

**Saiba como hospedar diferentes sítios Web na sua oferta de alojamento Web.**

> [!primary]
> Se já criou o sítio Web em questão no seu alojamento Web e pretende associar-lhe um novo nome de domínio ou subdomínio, consulte **directamente** [este guia](/pages/web_cloud/web_hosting/my_websites_add_domain_existing_website).
...
### 1 - Adicionar um sítio Web à sua oferta de alojamento Web

**Clique num dos títulos abaixo para ver as explicações.**

<a name="add-domain-ovhcloud"></a>

/// details | Adicionar um sítio Web com nome de domínio gerido a partir da sua área de cliente OVHcloud

Esta secção aplica-se apenas se o nome de domínio (e/ou a sua zona DNS activa) com o qual pretende criar o seu sítio Web se encontra **na sua área de cliente OVHcloud**.
...
>>
>> No campo **Nome do site - obrigatório**, insira o nome que pretende utilizar para o seu sítio Web. Este nome será visível apenas no separador `Meus sites`{.action} do seu alojamento Web.
>>
...
>> > [!warning]
>> >
>> > Por defeito, a **pasta raiz** do seu sítio Web é criada automaticamente ao adicionar o sítio Web ao seu alojamento Web. Esta mesma **pasta raiz** também é gerada no espaço de armazenamento do seu alojamento Web (acessível via FTP, SFTP ou SSH, consoante a sua oferta).
>> >
>> > Se pretender personalizar o nome da **pasta raiz**, nomeadamente se o conteúdo do seu sítio Web já estiver presente numa pasta específica do seu espaço de armazenamento, pode defini-lo ao activar o botão `Configuração avançada`{.action}.
>> >
>> > **O nome da pasta raiz só é personalizável no momento da adição do sítio Web ao seu alojamento Web.** Isto significa que, uma vez criado e adicionado o sítio Web ao seu alojamento Web, não será possível, por enquanto, **modificá-lo** posteriormente.
>>
...
>> >
>> > **Exceptuando a personalização da pasta raiz, todas estas funcionalidades podem ser activadas posteriormente, uma vez que o nome de domínio foi adicionado ao seu sítio Web.** Para isso, consulte directamente [este guia](/pages/web_cloud/web_hosting/multisites_modify_domain).
>>
>> Para personalizar o nome da pasta raiz que será associada ao seu sítio Web e que conterá os seus ficheiros, insira o nome pretendido no campo **pasta raiz**.
>>
>> Abaixo, encontra uma descrição das outras Opções.
>> Consoante a sua oferta de [alojamento Web](/links/web/hosting), alguns elementos entre as opções apresentadas abaixo não poderão ser seleccionados.
>>
...
>> |Opção|Descrição|
>> |---|---|
>> |IP do país|Permite beneficiar de um endereço IP geolocalizado (escolha entre uma lista de países) para o nome de domínio seleccionado.<br> Saiba mais com [esta página](/links/web/hosting-Opções).|
>> |Firewall|Permite activar um firewall (filtragem e análise das requisições) no nome de domínio seleccionado.<br> Saiba mais com [esta página](/links/web/hosting-Opções).|
>> |CDN|Permite activar o CDN (cache dos elementos estáticos do seu sítio Web, como as imagens) no nome de domínio seleccionado.<br> Saiba mais com [a nossa página CDN](/links/web/hosting-Opções-CDN).<br> Ao activar o SSL e o CDN, também beneficiará do protocolo **HTTP/2** (este protocolo está activado por defeito no nosso datacenter de Gravelines).|
>>
...
> **Etapa 7**
>>
>> A OVHcloud disponibiliza os módulos WordPress, Joomla!, PrestaShop e Drupal. Estes permitem dispor de uma estrutura de sítio Web pronta a utilizar, instalada automaticamente na pasta raiz configurada anteriormente. Para saber mais, consulte a nossa documentação « [“Módulos 1 clique: como instalar um CMS para criar um site”](/pages/web_cloud/web_hosting/cms_install_1_click_modules) ».
>>
...
>>
>> Ao contrário, se pretender instalar manualmente o seu sítio Web, recupere os seus ficheiros e carregue-os na pasta raiz apropriada no espaço de armazenamento do seu alojamento Web. Para saber mais, consulte a nossa documentação « [Publicar um site num alojamento web](/pages/web_cloud/web_hosting/hosting_how_to_get_my_website_online) ».
>>
> **Etapa 8**
>>
>> Verifique que todas as informações introduzidas anteriormente estão correctas, em seguida, clique em `Continuar`{.action} para finalizar a adição do seu nome de domínio ou subdomínio ao seu sítio Web.
>>
...
/// details | Adicionar um sítio Web com nome de domínio não gerido a partir da sua área de cliente OVHcloud

Esta secção aplica-se apenas se pretender adicionar um sítio Web com um nome de domínio que não está presente no seu conta OVHcloud. Pode tratar-se de um nome de domínio presente noutra conta OVHcloud ou registado por outro fornecedor.
...
>> > **O nome da pasta raiz só é personalizável no momento da adição do sítio Web ao seu alojamento Web.** Isto significa que, uma vez criado e adicionado o sítio Web ao seu alojamento Web, não será possível, por enquanto, **modificá-lo** posteriormente.
>>
>> Para personalizar o nome da pasta raiz que será associada ao seu sítio Web e que conterá os seus ficheiros, insira o nome pretendido no campo **Pasta raiz**. Se não pretender personalizá-lo, deixe o campo vazio.
>>
