...
> **Step 2**
>>
>> Click on the `Databases`{.action} tab. In the table that appears, click the `...`{.action} button to the right of the row corresponding to the database you want to delete, then `Delete a database`{.action}:
>>
...
/// details | Your 1-click module has a web address like "xxxxx.cluster0xx.hosting.ovh.net"
...
/// details | "Installation directory is not empty"
...
/// details | "Unable to connect to database" <a name="delete-the-module"></a>
...
> **Step 2**
>>
>> In the `1-click modules`{.action} tab, in the table that appears, click the `...`{.action} button to the right of the line corresponding to your domain name, then click `Delete the module`{.action}.
>>
...
/// details | The "Administrator" password for accessing the "administration interface" of your 1-click module does not work <a name="adminpassword"></a>
...
/// details | The table prefix in your database is already in use on your database
...
/// details | The domain name DNS record does not point to an OVHcloud web hosting plan
...
/// details | Your database must be in version "X", but it is currently in version "Y"
