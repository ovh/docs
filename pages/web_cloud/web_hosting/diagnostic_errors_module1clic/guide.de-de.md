...
Mit „[1-Klick-Module](/pages/web_cloud/web_hosting/cms_install_1_click_modules)“ können Sie Ihre Website schnell erstellen. Mit dieser Technologie können Sie Ihre Website mit den bekanntesten **C**ontent **M**anagement **S**ystem (**CMS**) wie *WordPress*, *Joomla!*, *Drupal* oder *PrestaShop* erstellen.
...
- Sie verfügen über ein kompatibles [Webhosting](https://www.ovhcloud.com/de/web-hosting/) Angebot.
...
- Sie haben die Funktion „[1-Klick-Modul](/pages/web_cloud/web_hosting/cms_install_1_click_modules)“ zum Erstellen einer neuen Website verwendet.
...
|Szenario|Lösung|
|---|---|
|Die Domain oder Subdomain, die mit der Website verknüpft ist, die Sie erstellen möchten, wird in der Tabelle auf der Registerkarte `Multisite`{.action} nicht angezeigt.|Fügen Sie Ihre Domain hinzu, indem Sie [diese Anweisungen](/pages/web_cloud/web_hosting/multisites_configure_multisite).|
|Der Domainname wurde ohne Aktion Ihrerseits aus der Multisite gelöscht.|Wenn Ihre Domain oder deren [DNS Zone](/pages/web_cloud/domains/dns_zone_edit) nicht über Ihren OVHcloud Account verwaltet werden, fügen Sie Ihre Domain über den Tab `Multisite`{.action} hinzu, indem Sie [diese Anleitung](/pages/web_cloud/web_hosting/multisites_configure_multisite) folgen hinzufügen.|
...
> [!primary]
>
> Denken Sie daran, unsere Datenbankangebote für einzelne Datenbanken [start SQL](https://www.ovhcloud.com/de/web-hosting/options/start-sql/) und unser Angebot [Web Cloud Databases](https://www.ovhcloud.com/de/web-cloud/databases/) im Voraus zu konsultieren.
>

#### Lösung Nr. 2: Ihr Webhosting Angebot ändern

> [!primary]
>
> Unsere [Webhosting-Angebote](https://www.ovhcloud.com/de/web-hosting/).
>
...
Klicken Sie auf der angezeigten Seite auf den Tab `Datenbanken`{.action}. Klicken Sie in der angezeigten Tabelle auf den Button `...`{.action} rechts neben der Zeile mit der Datenbank, die Sie löschen möchten, und klicken Sie dann auf `Datenbank löschen`{.action}:

![delete_a_database](images/delete_a_database.png){.thumbnail}

Anschließend können Sie ein neues „1-Klick-Modul“ installieren.

#### Lösung Nr. 4: Installation Ihres „1-Klick-Moduls“ auf einer bereits verwendeten Datenbank
...
Die Login-Daten Ihrer Datenbank finden Sie in unserer Anleitung [Website mit einem „1-Klick-Modul“ (CMS) installieren](/pages/web_cloud/web_hosting/cms_install_1_click_modules).
...
> [!primary]
>
> In dieser Situation können Sie die Daten einer einzigen Website mit einem [PHP-Skript oder einem SSH-Befehl](/pages/web_cloud/web_hosting/sql_database_export) sichern.
>
...
### Ihr „1-Klick-Modul“ wird auf einer Webadresse vom Typ „xxxx.cluster0xx.hosting.ovh.net“ angezeigt
...
Diese Nachricht bedeutet, dass der Ihrem Domainnamen zugeordnete **Wurzelverzeichnis** bereits eine oder mehrere Dateien oder Ordner enthält.
...
Sie können sich auch über das Protokoll [FTP](/pages/web_cloud/web_hosting/ftp_connection) mit Ihrem Webhosting verbinden und den Inhalt des Ordners löschen. Dies, nachdem Sie es lokal gesichert haben oder nachdem Sie den gesamten Inhalt in ein anderes FTP-Verzeichnis verschoben haben.
...
In dieser Situation [per FTP verbinden](/pages/web_cloud/web_hosting/ftp_connection), [lokales Backup](/pages/web_cloud/web_hosting/ftp_filezilla_user_guide) Ihre Daten auf Ihrem Webhosting, und löschen Sie alle Dateien, die für den Betrieb Ihrer Website nicht erforderlich sind.
...
Überprüfen Sie die Login-Daten Ihrer Datenbank. Weitere Informationen finden Sie in dieser [Anleitung](/pages/web_cloud/web_hosting/cms_install_1_click_modules).

Löschen Sie anschließend Ihr „1-Klick-Modul“. Loggen Sie sich hierzu in Ihr [OVHcloud Kundencenter](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.de/&ovhSubsidiary=de) ein und gehen Sie in den Bereich `Web Cloud`{.action}. Klicken Sie in der linken Spalte auf `Hosting-Pakete`{.action} und wählen Sie das betreffende Webhosting aus. Klicken Sie auf der angezeigten Seite auf den Tab `1-Klick-Module`{.action}. Klicken Sie in der angezeigten Tabelle auf den Button `...`{.action} rechts neben der Zeile für Ihren Domainnamen und dann auf `Das Modul löschen`{.action}.
...
>
> Achten Sie darauf, nur die gerade gestartete Installation zu entfernen. Stellen Sie hierzu sicher, dass es sich um das betreffende Verzeichnis in der Spalte `Path` (Pfad) handelt.
>
...
Diese Meldung erscheint nur bei der Installation eines „1-Klick-Moduls“ im **Erweiterten Modus**. Die Datenbank kann nicht mehr geändert werden, da die Datenmenge in der Datenbank den zulässigen Grenzwert überschreitet. In diesem Fall ist die Datenbank schreibgeschützt blockiert.

Installieren Sie in dieser Situation Ihr „1-Klick-Modul“ über den [Simple Mode](/pages/web_cloud/web_hosting/cms_install_1_click_modules) oder wählen Sie bei der Installation im Experten-Modus eine andere Datenbank. Bei Bedarf können Sie ein [Datenbank-Angebot](https://www.ovhcloud.com/de/web-hosting/options/start-sql/) bestellen.

Wenn Sie keine anderen Datenbanken haben und kein zusätzliches Angebot bestellen möchten, [importieren Sie eine Kopie Ihrer Datenbank lokal](/pages/web_cloud/web_hosting/sql_database_export), und löschen Sie dann die nicht benötigten Daten.
...
Wenn Sie auf den Link Ihrer Website in der Spalte `Pfad` klicken, öffnet sich im Tab `1-CKlick module` des Webhostings, das Ihre Website enthält, ein neuer Tab mit Ihrer Website. Der Ihrer Installation zugewiesene Domainname wird in der Adresse Ihres Internetbrowsers angezeigt. Wenn Ihre Domain zum Beispiel „domain.tld“ heißt, wird möglicherweise eine andere Domain oder eine OVHcloud-Standardseite angezeigt.
...
Sie teilt Ihnen per E-Mail mit, dass Sie bei dem Versuch, das „1-Klick-Modul“ zu installieren, ein Tabellenpräfix angegeben haben, das bereits für die zuvor ausgewählte Datenbank verwendet wurde, um Ihr „1-Klick-Modul“ zu installieren. Die Installation wird abgebrochen. 

Führen Sie die Installation mit einem anderen Tabellenpräfix oder einer anderen Datenbank erneut aus, um das Problem zu beheben.
...
### Ihre Datenbank muss in der „X“ Version sein, diese ist jedoch derzeit in der „Y“ Version
...
In derselben E-Mail finden Sie auch die Version, in der sich Ihre Datenbank befinden soll. Sie haben drei Möglichkeiten:

- Update des **D**ata**B**ase **M**anagement **S**ystem (DBMS wie MySQL, PostgreSQL, MariaDB usw.) in einer neueren Version.
- Installation einer neuen Datenbank für Ihr Webhosting. So stellen Sie sicher, dass das SGDB und die Version mit dem gewünschten „1-Klick-Modul“ kompatibel sind.
