...
To check the folder in which the website should be published, go to the `My sites`{.action} tab from your OVHcloud Control Panel. In the table that appears, for the desired website, look at the `Root folder`{.action} displayed. Then publish the website files in this folder.

You may find on your storage space a file named "index.html". This file may have been created by OVHcloud during the installation of your web hosting to display a default page on your website. If this is the case, do not forget to delete it when uploading your files.
...
Depending on the website used, this link must be created manually or via an interface generated by the website itself. It is carried out in several sub-steps, some of which may be optional.
