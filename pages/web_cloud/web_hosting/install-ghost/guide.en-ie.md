...
Download the [latest version of Ghost](https://ghost.org/), and unzip its contents.
...
To restart the Node.js daemon, go back to the [OVHcloud Control Panel](/links/manager). Go to the `Multisite`{.action} tab, click on the `...`{.action} button to the right of the domain name concerned, then click `Restart`{.action}.
