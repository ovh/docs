...
> [!success]
>
> Note the two target values with record type "NS". These values, of type `dnsXX.ovh.net` and `nsXX.ovh.net` (or `dns200.anycast.me` and `ns200.anycast.me`), correspond to the DNS servers associated with this DNS zone for your domain name. They will be used in [step 9](#step9) of this guide.
>
...
Replace the current DNS servers (of the original registrar) with the two DNS servers declared in the OVHcloud DNS zone (of type `dnsXX.ovh.net` and `nsXX.ovh.net` or `dns200.anycast.me` and `ns200.anycast.me`). You can do this in the management interface of the original registrar.
