...
- Administrator username (nom d'administrateur) : rendez-vous dans votre espace client OVHcloud, puis cliquez sur l'onglet `Modules en 1 clic`{.action}. Dans le tableau qui s'affiche, identifiez la ligne correspondant à votre module en un clic. Votre nom d'administrateur se trouve dans la colonne `Login`{.action}.
...
### Gérer les mises à jour logicielle depuis le dashboard MainWP
...
MainWP propose des fonctionnalités natives, telles que le gestion des mises à jour logicielles ou encore la gestion des clients. Cependant, MainWP devient encore plus intéressant et puissant grâce à ses nombreuses extensions, proposant ainsi un large éventail de fonctionnalités.

Il existe différentes [catégories d'extensions](https://mainwp.com/mainwp-extensions/), comme Administrative, Backups, Analytics, Security etc. Pour chacune des catégories, une liste d'extensions est disponible. Il existe trois différents types d'extensions :
...
Dans le champ `Enter your MainWP API Key`{.action}, collez la clé API que vous avez précédemment copié et cochez la case `Remember MainWP Main API Key`{.action}. Cliquez sur `Validate my MainWP Main API Key`{.action}. S'il n'y a aucune erreur, le message de confirmation suivant s'affiche.
