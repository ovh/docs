...
XXXXXXXXXXXX The CSR XXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXX
------ŻĄDANIE CERTYFIKATU------ 
"
...
>
> Inni dostawcy SSL dostarczają plik `ca_bundle.crt` w wielu częściach/plikach. Musisz połączyć zawartość tych plików, aby zmienić jeden plik `ca_bundle.crt` i tym samym postępować zgodnie z [krokiem 3](#step-3) niniejszego przewodnika.
>
...
- Status `Do wygenerowania` lub `Włączony` nie może być już obecny dla każdej domeny/subdomeny, której dotyczy zewnętrzny certyfikat SSL.

W razie potrzeby sprawdź przewodniki dotyczące "[Udostępnianie hostingu między kilkoma stronami](/pages/web_cloud/web_hosting/multisite_configure_multisite)" i "[Edycja strefy DNS OVHcloud](/pages/web_cloud/domains/dns_zone_edit)".
...
W oknie, które się pojawi wybierz z listy dostępnych opcji opcję `Import certyfikatu SSL`{.action}, następnie kliknij `Dalej`{.action}.
...
>
> Sprawdź przewodniki "[Udostępnianie hostingu między kilkoma stronami](/pages/web_cloud/web_hosting/multisites_configure_multisite)" i "[Edycja strefy DNS OVHcloud](/pages/web_cloud/domains/dns_zone_edit)", aby rozwiązać ten problem.
...
[Hosting WWW - Ustaw HTTPS](/pages/web_cloud/web_hosting/ssl-activate-https-website).

[Typowe błędy związane z zabezpieczaniem strony WWW za pomocą protokołu SSL](/pages/web_cloud/web_hosting/ssl_avoid_common_pitfalls_of_making_website_secure).
