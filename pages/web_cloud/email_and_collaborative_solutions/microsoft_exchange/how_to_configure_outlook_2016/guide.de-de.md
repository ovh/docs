...
>> - Setzen Sie den Haken neben `Ich möchte mein Account manuell einrichten`{.action} und klicken Sie anschließend auf `Verbinden`{.action}.
>>
>> ![exchange](images/config-outlook-legacy-exchange02.png){.thumbnail}
>>
>> - Wählen Sie aus der Liste der Accounttypen **Exchange** aus.
>> - Geben Sie im folgenden Fenster das Passwort Ihres E-Mail-Accounts ein, haken Sie die Speicheroption an, und klicken Sie dann auf `OK`{.action}.
>>
...
>> - Legen Sie dann die Aufbewahrungsfrist der Elemente in Ihrem Exchange Account fest, um diese **lokal auf Ihrem Computer** zu speichern. Klicken Sie auf `Weiter`{.action} und dann auf `Fertig stellen`{.action}.
>>
...
- Das Fenster mit den Accounteinstellungen wird angezeigt. Wählen Sie den betroffenen E-Mail-Account aus und klicken Sie auf `Löschen`{.action}.
