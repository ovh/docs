...
#### Étape 1 : Créer un projet de migration
...
#### Étape 2 : Se connecter au projet et créer la migration
...
- `Supprimer mes données de migration (RGPD)`{.action} : Déclenche la suppression de toutes les données relatives à la migration. Les informations sur les événements survenus durant la migration sont néanmoins conservées.
...
Dans votre espace client OVHcloud, accédez à votre service Zimbra et renommez l'adresse de votre compte e-mail Zimbra avec le nom du compte e-mail migré (si reprend l'exemple plus haut : `zimbra2@mydomain.ovh` en `john.smith@mydomain.ovh`).

### Conclusion <a name="conclusion"></a>

Votre compte e-mail est maintenant entièrement migré vers Zimbra. Vous pouvez désormais utiliser Zimbra pour gérer votre messagerie.
