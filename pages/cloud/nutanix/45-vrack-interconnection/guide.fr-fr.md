...
Avant d'interconnecter les deux clusters il faut s'assurer qu'ils utilisent des adresses IP différentes sauf pour la passerelle sur une même plage d'adresse IP , dans notre guide nouse allons utiliser le cette plage d'adresse `192.168.0.0/22`
...
Cette opération consiste à supprimer l'affectation du vRack du deuxième site et ensuite etendre le vRack du premier site au deuxième site. les modifications du **vRack** se font au travers de l'espace client OVHcloud. 
...
Allez dans le menu `Bare Metal Cloud` cliquez sur `le vRack`{.action} en dessous de l'option **vRack** dans **Network**.
...
> [!primary]
> 
> L'ajout de l'adresse publique n'est pas obligatoire mais elle sera disponible.
> 

![03 Add to vrack 03](images/03-addtovrack03.png){.thumbnail}

Le **vRack** qui était à l'origine uniquement utilisé par les serveurs du premier cluster est maintenant utilisé sur les deux sites et contient :

- Les serveurs physiques des deux clusters
- Les adresses IP publiques des deux clusters
- Le load balancer du premier site utilisé pour **Prism Central** du premier site.

### Changement des informations concernant le load balancer

Nous allons reconfigurer le **Load Balancer** du second site pour qu'il fonctionne avec le **vRack** commun au deux sites pour pouvoir accéder à **Prism Central** du deuxième site.
