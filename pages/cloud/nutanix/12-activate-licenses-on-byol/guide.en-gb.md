---
title: Add licences to your Nutanix BYOL cluster
...
Click on `Show all downloads in the top right`{.action}
...
#### Logging in to Nutanix portal 

Also in **Prism Central**, go to the licences page and click `Licences page`{.action}.
...
Now save the **Licence Summary File (LSF)**, which contains the licence on your computer. 
...
Return to **Prism Central** in licence management and click `Upload`{.action} to install the licence file.
...
Through **Prism Central** click on `the gear`{.action} in the top right to go to the parameters.

![05 generate report with licences 01](images/05-generate-report-with-licenses01.png){.thumbnail}

Select `Licencing`{.action} on the left and click `Update Licences`{.action}.
...
Through the **Nutanix** licence portal click `Manage Licences`{.action}.
...
Click the `scroll`{.action} bar to scroll to the bottom of the window.
