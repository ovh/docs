...
Conéctese al [área de cliente de OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.es/&ovhSubsidiary=es){.external}, acceda a la sección `Bare Metal Cloud`{.action} y seleccione el servidor de la lista de navegación de la izquierda en `Servidores privados virtuales`{.action}. 
