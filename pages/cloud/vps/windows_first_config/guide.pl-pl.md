...
Aby uzyskać dostęp do konsoli KVM Twojego serwera VPS, zapoznaj się z [przewodnikiem KVM VPS](/pages/cloud/vps/using_kvm_for_vps).
...
> [!primary]
> Aby zabezpieczyć system Windows za pomocą reguł firewalla, zapoznaj się z naszym przewodnikiem "[Konfiguracja firewalla w systemie Windows](/pages/cloud/vps/activate-port-firewall-soft-win)".
>

### Aktywacja logów startowych (boot log) Windows (opcjonalnie)

Zaloguj się do serwera przez sesję "zdalny pulpit" lub [KVM](/pages/cloud/vps/using_kvm_for_vps). Otwórz menu Start systemu Windows i kliknij `Uruchom`{.action}.
...
Aby uzyskać dostęp do zawartości tego pliku w trybie Rescue, postępuj zgodnie z instrukcjami zawartymi w [przewodniku dotyczącym trybu Rescue serwera VPS](/pages/cloud/vps/rescue).
