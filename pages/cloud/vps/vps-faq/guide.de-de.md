...
Wann ist ein Dedicated Server ratsam? Wenn Sie die Hardware selber verwalten und komplexere Infrastrukturen aufbauen möchten. Diese können z.B. ein privates Netzwerk (vRack) beinhalten. Oder aber wenn Sie außer Webservices komplexe Lösungen einsetzen möchten.
...
Der VPS ist eine Lösung, die auf Umgebungen der Produktion und deren Vorstufen ausgerichtet ist, die keine konstanten Leistungen benötigen.
Die Public Cloud von OVHcloud bietet eine Multi-Server-Infrastruktur mit hoher Verfügbarkeit der Maschinen. Mit dieser Lösung ist ebenfalls ein privates Netzwerk, das vRack, verfügbar.
...
- die erforderliche Ressourcenmenge (Prozessor, Speicher, Festplattenspeicher, Bandbreite...)
- das erforderliche Betriebssystem (Linux oder Windows)
- die technischen Voraussetzungen, die für das reibungslose Funktionieren der Anwendung von wesentlicher Bedeutung sind (z. B. benötigt eine Datenbank Lese-/Schreibgeschwindigkeit).
...
Wenn Sie garantierte Ressourcen ohne Kenntnisse in der Serveradministration nutzen möchten, empfehlen wir Ihnen unsere [Performance Webhosting](https://www.ovh.de/hosting/performance-hosting.xml).

### Wie kann ich mich mit meinem VPS verbinden?

Die Verbindung zu Ihrem VPS muss per SSH hergestellt werden. Hierfür verwenden Sie die IP-Adresse, den Benutzernamen und das Passwort, die Ihnen per E-Mail beim Eingang der Bestellung zugesandt werden.
Verbinden Sie sich mit dem Putty-Programm von einem Windows-Rechner aus. Die Verbindung kann direkt über einen Linux-Rechner mit dem Terminal hergestellt werden.

Alle Details finden Sie in der Anleitung ["Mit einem VPS starten"](../erste-schritte-mit-einem-vps/).
...
OVHcloud führt keine Backups der Daten durch, die auf Ihrem VPS gehostet werden. Es ist also an Ihnen, dafür zu sorgen.
Sie haben dafür folgende Optionen: das manuelle Backup (Snapshot) oder das automatische Backup.

### Wie kann ich meinen VPS schützen?

Standardmäßig ist der VPS "nackt"angegeben, auf dem keine Sicherheitskonfiguration vorhanden ist. Das ist also das Erste, was Sie bei der Rezeption machen müssen
Lesen Sie hierzu die Anleitung "[VPS sichern"](../vps-sicherheit/).

### Welche Bandbreite ist meinem VPS zugewiesen? Ist sie garantiert?

Die auf den Seiten unserer Angebote angezeigte Bandbreite ist garantiert. Dies ist die garantierte Mindestbandbreite, die Ihnen zugewiesen wird.

### Welches SLA wird auf meinem VPS angewendet?

Für alle VPS Reihen bietet OVHcloud ein SLA von 99,9%.

### Wie kann ich von einer anderen IP-Adresse als meiner Dienstleistung auf mein Storage-Backup zugreifen? <a name="backupstorage"></a>

Der Zugang zu Ihrem FTP-Backup kann über Ihr OVHcloud Kundencenter auf den damit verbundenen Dienst beschränkt werden.

Um weitere IP-Adressen verschiedener Dienste hinzufügen zu können, können Sie die OVHcloud API verwenden.
So können Sie Ihre Backups von einem anderen Standort aus abrufen.
...
- `serviceName `: den Namen Ihres VPS
- `cifs `: Falls erforderlich
- `ftp`: Falls erforderlich
- `ipBlock`: Geben Sie die IP-Adresse ein, auf die Zugriff in der Form `1.2.3.4/32` erfolgt
- `nfs`: Falls erforderlich
