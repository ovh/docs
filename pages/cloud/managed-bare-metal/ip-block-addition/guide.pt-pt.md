...
routes:
    canonical: 'https://docs.ovh.com/pt/private-cloud/adicionar-bloco-ip/'
...
> [!primary]
>
> Não hesite em consultar o nosso manual sobre o [plugin OVHcloud Network](../plugin-ovhcloud-network/){.external-link} para saber quais são os IP reservados do seu bloco, assim como a sua utilização.
>
...
Poderá acompanhar a transferência do bloco IP na sua [Área de Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager){.external} na secção `Bare Metal Cloud`{.action} > `Managed Bare Metal`{.action}. Clique no seu serviço Managed Bare Metal e, depois, no separador `Operações`{.action}.
