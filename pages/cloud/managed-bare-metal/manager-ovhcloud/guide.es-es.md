...
- Estar conectado al [área de cliente de OVHcloud](https://www.ovh.com/auth/?action=gotomanager){.external}, en la sección `Bare Metal Cloud`{.action}, y dentro, en `Managed Bare Metal`{.action}.
...
Una vez en la sección `Bare Metal Cloud`{.action} del [área de cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager){.external}, haga clic en `Managed Bare Metal`{.action} y aparecerá la información general de su Managed Bare Metal.
...
Puede consultar las tarifas [aquí](https://www.ovhcloud.com/es-es/managed-bare-metal/images-licenses/){.external}.
