...
This guide provides instructions for deployons the SLES 15 for SAP image on an OVHcloud dedicated server and preparing it for SAP HANA.

## Prerequisites

- Access to [OVHcloud manager](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/en/&ovhSubsidiary=en)
...
From the OVHcloud manager, you can launch the deployment of the SLES 15 for SAP image provided by OVHcloud. Click on `Install`{.action} button to launch it.

![install-manager](images/install-manager.png){.thumbnail}

Select `Install from an OVHcloud template`{.action} option.
...
If you does not custom the configuration of partitions, you will go to the last step.

You can set a hostname which will be displayed with the command `hostname`. You have also the possibility to [add your SSH key](https://docs.ovh.com/gb/en/dedicated/getting-started-dedicated-server/#adding-an-ssh-key-optional).

> [!primary]
> By default, a swap partition of 4 GB is created and follow the SAP recommendations which are explained in the [SAP Note 1999997 - FAQ: SAP HANA Memory](https://launchpad.support.sap.com/#/notes/1999997).

![sles-partitions](images/sles-partitions.png){.thumbnail}

Once these last paramaters configured, click on `Confirm`{.action} to launch the installation.
...
Once the SLES 15 for SAP installation done, you can [connect to your dedicated server](https://docs.ovh.com/gb/en/dedicated/getting-started-dedicated-server/#logging-on-to-your-server_1).

### Preparation of file systems

We will used the *Logical Volume* concept offering a lot of advantages within configuration of partitions.
...
<ol start="4">
  <li>Create logical volumes which will be partitions for the operating system.</li>
</ol>

Each logical volume will be shown as a directory for the SAP HANA installation.

We advise ou to follow this table to size your logical volumes.
...
Replace in each line, the caracter `<X>` by the size of your logical volume wanted in gigaoctet, for example 32.
...
<ol start="5">
  <li>Once these logical volumes created, it's necessary to format in one supported by SAP HANA.</li>
</ol>

In this guide, we use the XFS format. We recommend to take into consideration the [SAP Note 2972496 - SAP HANA Filesystem Types](https://launchpad.support.sap.com/#/notes/2972496) to discover all suported format by SAP HANA.
...
<ol start="8">
  <li>Add this content in the <code>/etc/fstab</code> file, replacing by values previously get.</li>
</ol>
...
The SAPtune package is available on the SUSE operating system, and allow applying system parameters depending of the SAP role hosted on the server. To discover more about this package, we invite you to go to the [SUSE page](https://documentation.suse.com/sles-sap/15-SP2/html/SLES-SAP-guide/cha-tune.html).

> [!primary]
> Reminder, you must have activate your SLES 15 for SAP license on your dedicated server before to continue this guide.
>
...
<ol start="2">
  <li>Depends on your futur SAP system, you have two possibilities.</li>
</ol>
&ensp;&thinsp;a. The first choice is `HANA` for running a SAP HANA database of a SAP Netweaver system.<br>
&ensp;&thinsp;b. The second chois is `S4HANA-DBSERVER` for running the SAP HANA database of a SAP S/4HANA system.

To set these parameters after choosed the solution, run this following command:
...
<ol start="3">
  <li>We recommend you to enable the SAPtune service during the startup. At each startup of your server, the SAPtune service will set up the recommended parameters for your operating system.</li>
</ol>
...
<ol start="4">
  <li>You can check if parameters have been successfully done with this following command:</li>
</ol>
...
> [!primary]
> If you want to learn more about the parameters for SLES 15 and SAP HANA, we recommend you these both SAP Notes.
>
