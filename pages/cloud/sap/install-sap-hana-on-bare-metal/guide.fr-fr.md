...
slug: serveur-dedie-sles15-pour-sap-hana
...
1 - À travers l'espace client OVHcloud, vous pouvez lancer le déploiement de l'image SLES 15 for SAP que OVHcloud met à disposition en cliquant sur le bouton `Install`{.action}.
![install-manager](images/install-manager.png){.thumbnail}

2 - Sélectionnez l'option `Installer à partir d'un template OVH`{.action}.<br>
![select-template](images/select-template.png){.thumbnail width="500" height="500"}

3 - La première étape consiste à renseigner des informations sur le système d'exploitation que vous souhaitez installer.<br>
&ensp;&thinsp;a. Dans la catégorie `Type de le système d'exploitation:`{.action}, sélectionnez `ERP`{.action}<br>
&ensp;&thinsp;b. Dans la catégorie `LINUX`{.action}, sélectionnez `SUSE Linux Enterprise Server 15 SP3 for SAP Applications - sles-sap15sp3`{.action}.

Nous vous recommandons de réaliser l'installation de le système d'exploitation sur la grappe de disque cible `2 X Disk SSD 480 GB, JBOD`{.action}.
...
4 - Vous pouvez indiquer un nom de serveur qui sera visible via la commande `hostname`. Vous avez également la possibilité d'ajouter votre clef SSH. Pour savoir comment ajouter une clef SSH, nous vous proposons de suivre ce [guide](https://docs.ovh.com/fr/dedicated/premiers-pas-serveur-dedie/#ajout-dune-cle-ssh-facultatif).

> [!primary]
> Une partition swap de 4 GB est par défaut créé et respecte les recommandations SAP qui sont indiquées dans la [SAP Note 1999997 - FAQ: SAP HANA Memory](https://launchpad.support.sap.com/#/notes/1999997).

![sles-partitions](images/sles-partitions.png){.thumbnail}

5 - Une fois ces derniers paramètres configurés, cliquez sur `Valider`{.action} pour lancer l'installation.

> [!warning]
> L'image SLES 15 for SAP que OVHcloud met à votre disponition est une image sans licence incluse. Suite au déploiement, vous devrez installer votre license SLES 15 for SAP via la commande: 
>
> `SUSEConnect -r <licence>`

6 - Une fois l'installation de l'image SLES 15 for SAP réalisée, vous pouvez vous connecter à votre serveur dédié. Pour savoir comment se connecter à votre serveur dédié, suivez les étapes dans ce [guide](https://docs.ovh.com/fr/dedicated/premiers-pas-serveur-dedie/#connexion-a-votre-serveur_1).
...
Pour vérifier, vous pouvez lancer la commande ci-dessous et vous obtiendrez ce résultat.
```bash
$ lsblk
...
2 - Créez un volume physique qui se base sur le RAID des disques de données avec la commande suivante :
```bash
$ pvcreate /dev/sda
```

3 - Créez un virtual group nommé `vg_hana` qui s'appuie sur le volume physique précédemment créé.
```bash
vgcreate vg_hana /dev/sda
```

4 - Créez les volumes logiques, qui représenteront les partitions pour le système d'exploitation.
...
Veuillez remplacer dans chaque ligne les caractères `<X>` par la taille souhaitée en gigaoctet de vos volumes logiques, exemple 32.
```bash
...
5 - Une fois les volumes logiques créés, il est nécessaire de les formater dans un format de système de fichiers supporté pour SAP HANA. Dans ce guide, nous utilisons le format de système de fichiers XFS. Nous vous recommandons de prendre connaissance de la [SAP Note 2972496 - SAP HANA Filesystem Types](https://launchpad.support.sap.com/#/notes/2972496) afin de découvrir les formats supportés pour SAP HANA.
```bash
...
6 - Créez les répertoires sur lesquels vont s'appuyer ces volumes logiques.
```bash
$ mkdir -p /hana/data /hana/log /hana/shared /usr/sap /hanabackup
```

7 - Afin de monter ces systèmes de fichiers sur le système d'exploitation, nous allons récupérer leur UUID afin de remplir le fichier `/etc/fstab`.
...
8 - Ajoutez ce contenu dans le fichier `/etc/fstab`, en remplaçant les caractères `<UUID>` par les valeurs précédemment obtenues.
```bash
...
9 - Vous pouvez maintenant exécuter la commande `mount -a` qui aura pour effet de monter sur votre système d'exploitation les volumes logiques. Ces derniers doivent être visible dans la sortie de la commande `df -h` avec les tailles que vous avez précédemment configurées.

### Application des paramètres avec SAPtune

SAPtune est un package disponible sur le système d'exploitation SUSE qui permet d'appliquer les paramètres systèmes en fonction du rôle SAP hébergé sur ce serveur. Pour en découvrir d'avantage sur ce package, nous vous invitons à vous rendre sur la [page de SUSE](https://documentation.suse.com/sles-sap/15-SP2/html/SLES-SAP-guide/cha-tune.html).
...
2 - En fonction de votre futur système SAP, deux possibilités s'offrent à vous.<br>
...
3 - Nous recommandons d'activer le service saptune au démarrage, l'application des paramètres systèmes recommandés sera ainsi réalisée à chaque démarrage de votre système d'exploitation.
```bash
$ saptune service enablestart
```

4 - Vous pouvez vérifier la bonne application des paramètres avec la commande suivante : 
```bash
saptune solution verify <HANA|S4HANA-DBSERVER>
```

> [!primary]
> Si vous souhaitez aller plus loin dans l'étude des paramètres pour SLES 15 et SAP HANA, nous vous recommandons ces deux SAP Notes.<br>
> [2578899 - SUSE Linux Enterprise Server 15: Installation Note](https://launchpad.support.sap.com/#/notes/2578899)<br>
