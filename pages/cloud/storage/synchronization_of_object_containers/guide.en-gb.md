...
> [!primary]
>
> If your container contains objects larger than 5Gb, your two containers must have the same name
>
...
In order to reverse the synchronization between two containers, you need to remove the `--sync-to` metadata from the source container, and redeclare it on the other container, which will then become the new source container.
...
In order to stop synchronization between two containers, you need to remove the `--sync-key` and `--sync-to` metadata
