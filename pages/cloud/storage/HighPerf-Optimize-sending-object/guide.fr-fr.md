...
Ce guide a pour objectif de vous montrer comment optimiser l'envoi de vos fichiers vers votre bucket.
...
Les commandes de transfert aws s3 sont multithreadées. À tout moment, plusieurs requêtes sont en cours. Par exemple, si vous téléchargez un répertoire via `aws s3 cp localdir s3://bucket/ --recursive`, l'AWS CLI pourrait télécharger les fichiers locaux localdir/file1, localdir/file2 et localdir/file3 en parallèle. La valeur `max_concurrent_requests` spécifie le nombre maximum de commandes de transfert autorisées à un moment donné.
