...
OVHcloud Cold Archive is  a class of storage designed for a long-term storage  
...
The hardware design is specifically build for this use case, to provide a trusted platform with best ratio resiliency/price
...
The service allow you to focus on building and deploying cloud applications while OVHcloud takes care of the service infrastructure and maintenance.

**This guide explains the concepts of the class of storage.**
...
During the lifecycle the data is placed at Object Storage level for a deposit or retrieval or stored in magnetic band for long duration archive
...
| --- | --- | --- | --- | --- |
| **`None`** | No Intelligent-Tiering configuration pushed on the bucket yet. | All | illimited | Standard |
| **`Archiving`** | Archiving in progress on tapes. | Listing | <48 hrs | Archive   |
| **`Archived`** | Objects archived on tapes only. | Listing | illimited | Archive  |
...
| **`Flushed`** | Bucket is empty and can safely be removed. | Listing (empty bucket) | NA | Archive  |

## Network, upload and retrieval performances  

Cold Archive  is a service based on Object Storage - S3 API. Permformance and limitations (number of conteners, account, maximum bandwidth per connection, number of request per second on bucket, maximum size per object / mpu / part) etc) are available [here](https://docs.ovh.com/gb/en/storage/object-storage/s3/limitations/)

To upload your data the maximum bandwidth is **1 Gbps per logical connection** and the number of connection that can be used in parallel is **unlimited.**

The time to upload depends in you internet connection. 

A few examples

- I have an internet connection of 1 Gbps, I upload an archive of 1TB, it will take 2,2 hours.
- I have an internet connection of 5 Gbps, I upload an archive of 1TB, it will take 26 minutes.
- I have an internet connection of 5 Gbps, I upload an archive of 100 TB, it  will take 1,9 days.

### Time to retrieve data 

The time it takes to access the data depends on the volume of data. For example, for a retrieval of several hundred TB, the average time is 48 hours. For a volume of a few TB, it can be between a few minutes and a few hours

## RSE, Certifications & Compliance

### Compliancy 

The service will be certified HDS and ISO 27001. You can follow these certification in our public roadmap page : <https://github.com/ovh/public-cloud-roadmap/projects/3?card_filter_query=label%3Astorage>
...
Immutable storage is often mandatory for legal reason and certification not to change or delete the data after it has been written.

Immutable storage is a mean to be protected against malware and attacks.
...
(180 days - number of days during which the service was used) x price of the storage class.
...
Operate your lifecycle and learn how to create buckets, archive, retrieve data, list metadata, following the guide [here](https://docs.ovh.com/gb/en/storage/object-storage/cold-archive/getting-started/).

All the Object Storage documentation is listed [here](https://docs.ovh.com/gb/en/storage/object-storage/).
