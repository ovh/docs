...
- You will need to have set up an account to interact with the OVHcloud API (see this [tutorial](https://docs.ovh.com/es/api/first-steps-with-ovh-api/)). From the application keys created, you will need to export the 4 environment variables :
...
- The *variable* block defines the region and s3 endpoint that are used to create the bucket. You can update it according to your needs : check this [page](https://docs.ovh.com/es/storage/object-storage/s3/location/) to know what region / endpoints are available.
