...
Yes, that interface is accessible through your [OVH customer control panel](https://ca.ovh.com/auth/){.external}, under `Cloud`{.action} and `Platforms and services`{.action}.
...
At any time you can also create manual snapshots, keep them with no time limit or delete them. This feature is available in your [OVH customer control panel](https://ca.ovh.com/auth/){.external} or through the[API](https://api.ovh.com/){.external}:
