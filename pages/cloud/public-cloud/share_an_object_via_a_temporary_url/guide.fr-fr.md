...
excerpt: 'Découvrez comment partager un objet avec une adresse temporaire'
...
- Dispsoer de Python installé sur votre poste.

## En pratique

### Comprendre le principe

L'adresse temporaire (ou *tempurl*) est une fonctionnalité qui vous permet de contrôler les fichiers que vous souhaitez partager, mais aussi combien de temps vous souhaitez que le lien soit disponible.

Cette fonction génère une adresse temporaire en utilisant les éléments suivants :
...
Dans un premier temps, vous devez générer une clé. Celle-ci sera utilisable pour tous les fichiers de votre projet. Une seule génération de clé est donc suffisante pour toutes vos adresses temporaires. 

> [!primary]
>
> Nous vous recommandons fortement de choisir une longue clé sécurisée, d'au minimum 20 caractères. Sachez cependant qu'il est possible de régénérer une nouvelle clé à tout moment.
> 

Pour générer votre clé, plusieurs solutions s’offrent à vous, telles que les commandes sha512sum ou sha256sum. Nous vous recommandons d’utiliser la méthode la plus adaptée à votre situation selon le niveau de chiffrement que vous souhaitez employer. Par exemple :

- date +%s | md5sum 
- date +%s | sha256sum
- date +%s | sha512sum

Une fois en possession de votre clé, vous pouvez configurer celle-ci sur votre projet à l'aide du client swift. Prenez soin de remplacer la chaine « 12345 » par votre clé) :
...
Maintenant que la clé est configurée sur le compte, vérifiez que le **header** à correctement été appliqué grâce à la commande suivante en utilisant le client Swift :
...
Les tâches suivantes peuvent être effectuées hors ligne. Nous allons générer l'adresse URL temporaire à l'aide d'une commande. Celle-ci devrait être personnalisé avec vos propres informations.
...
Dans l'exemple ci-dessus, cette adresse temporaire permet de télécharger le fichier **file** dans le conteneur **default**, pendant 60 secondes et cela sans authentification. Au-delà de ces 60 secondes, l'URL ne fonctionnera plus.
