...
Para encomendar um bloco IP adicional para o seu **Hosted Private Cloud**, aceda à sua Área de Cliente OVHcloud. Na secção `Server`, clique na opção `IP` no menu à esquerda e, a seguir, clique em `Encomendar IP adicionais`{.action}. De seguida, selecione o seu **Hosted Private Cloud** no menu pendente antes de passar para o seguinte passo.
...
> [!primary]
>
> Não hesite em consultar o nosso manual sobre o [plugin OVHcloud Network](../plugin-ovh-network/){.external-link} para saber quais são os IP reservados do seu bloco, assim como a sua utilização.
>
...
Poderá acompanhar a transferência do bloco IP na sua [Área de Cliente OVHcloud](https://www.ovh.com/auth/?action=gotomanager){.external} na secção `Server`{.action} > `Private Cloud`{.action}. Clique no seu serviço Hosted Private Cloud e, depois, no separador `Operações`{.action}.
