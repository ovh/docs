...
- Estar conectado al [área de cliente de OVHcloud](https://ca.ovh.com/auth/?action=gotomanager){.external}, en la sección `Servidor`{.action}, y dentro, en `Private Cloud`{.action}.
...
Una vez en la sección `Servidor`{.action} del [área de cliente OVHcloud](https://ca.ovh.com/auth/?action=gotomanager){.external}, haga clic en `Private Cloud`{.action} y aparecerá la información general de su Private Cloud.
