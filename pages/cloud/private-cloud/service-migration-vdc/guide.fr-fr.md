...
> Comment savoir si mes adaptateurs réseau sont VLAN ou VxLAN et compatible Essential? Dans vSphere, je vois par exemple et sans plus de détail: vxw-dvs-74-virtualwire-20-sid-....
>> Tout ce qui est %-virtualwire-% est du vxlan.
> Si j'ai plusieurs vm qui passe par le même edge nsx, il faudra faire la migration de l'ensemble des VM et du Edge en même temps au risque de ne plus avoir de liaison internet sur certaines VM dans la cas contraire?
>> Oui, il faudrait déplacer l'EDGE avec un redeploiement avant de déplacer les VMs. En fonction des cas, avec réseaux étendus ou pas, les 2 actions peuvent être séparées.
