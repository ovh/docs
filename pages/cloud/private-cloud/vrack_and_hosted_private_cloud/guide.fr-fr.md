...
Le produit vRack permet d'interconnecter plusieurs produits OVHcloud (https://www.ovh.com/world/solutions/vrack/){.external} et de les faire communiquer entre eux via 1 ou plusieurs vlan. Certaines configurations ne sont pas possibles.
...
* "VM Network", qui est un vRack sur un vlan unique, le vlan natif du vlan publique de Hosted Private Cloud. Ce vlan est utilisé dans Hosted Private Cloud pour router les IP Publiques. On le retrouve dans l'inventaire du vSphere en tant que PortGroup, dans la catégorie "Réseau", nommé "VM Network". Ce vRack est donc attaché au switch virtuel entièrement géré par OVH.

* "Datacenter vRack", ou "vRack vDC", ou "dvs-vrack", est le vRack qui permet d'avoir 4000 vlans. Ce vRack est attaché au switch virtuel géré par le client, avec ses propres carte réseau dédiées.
...
> [!primary]
>
> Note : pour que les VMs du vRack VM Network et les VMs du vRack vDC communiquent entre elles, les VMs du vRack vDC doivent être sur le vlan natif.
> 
