...
El sistema debe arrancarse en modo de rescate antes de poder cambiar la contraseña de administrador. Conéctese al [área de cliente de OVHcloud](https://ca.ovh.com/auth/?action=gotomanager), acceda a la sección `Bare Metal Cloud`{.action} y seleccione el servidor en la lista de navegación de la izquierda en `Servidores Dedicados`{.action}.
...
En primer lugar, sustituya el netboot por **Arrancar en el disco duro** en el [área de cliente de OVHcloud](https://ca.ovh.com/auth/?action=gotomanager) (ver [Etapa 1](./#1-reiniciar-el-servidor-en-modo-de-rescate_1)). 
...
En el [área de cliente de OVHcloud](https://ca.ovh.com/auth/?action=gotomanager), acceda a la pestaña `IPMI`{.action} para abrir una sesión de KVM.
