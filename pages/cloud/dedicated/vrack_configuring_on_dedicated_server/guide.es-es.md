...
Una vez que haya activado el vRack en su cuenta, acceda a la sección `Cloud`{.action} de la bare metal de su [Panel de configuración de OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.es/&ovhSubsidiary=es) y abra el menú `vRack`{.action}.
