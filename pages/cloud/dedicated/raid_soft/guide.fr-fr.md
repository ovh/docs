...
Le RAID (Redundant Array of Independent Disks) est un ensemble de techniques qui atténuent la perte de données sur un serveur en répliquant les données sur plusieurs disques.

Le niveau RAID par défaut pour les installations de serveur OVH est RAID 1, ce qui double l'espace occupé par vos données, réduisant ainsi de moitié l'espace disque utilisable.

**Ce guide va vous aider à configurer la baie RAID de votre serveur dans l'éventualité où elle doit être reconstruite en raison d'une corruption ou d'une panne de disque.**

## Prérequis

* Avoir un [serveur dédié](https://www.ovh.co.uk/dedicated_servers/){.external} avec une configuration Raid Logiciel
...
Cette commande nous montre que nous avons deux matrices RAID actuellement configurées, md2 étant la plus grande partition. La partition se compose de trois disques, appelés sda2, sdb2 et sdc2. Le \[UUU] signifie que tous les disques fonctionnent normalement. Un F indiquerait un disque défectueux.

Bien que cette commande affiche nos volumes RAID, elle ne nous dit pas la taille des partitions elles-mêmes. Nous pouvons obtenir cette information via la commande suivante :
...
Nous pouvons voir que `/dev/md1` se compose de 21 GB et `/dev/md2` contient 98,5 GB. Si nous devions exécuter la commande mount, nous pouvons également trouver la disposition du disque.
...
Comme les disques sont actuellement montés par défaut, pour retirer un disque du RAID, nous devons d'abord démonter le disque, puis simuler un échec et enfin le supprimer. Nous allons supprimer `/dev/sda2` du RAID avec la commande suivante :
...
Comme nous pouvons le voir, l'entrée de `/dev/md2` n'est plus montée. Cependant, le RAID est toujours actif, nous devons donc simuler un échec pour retirer le disque. Nous pouvons le faire avec la commande suivante:
...
La commande suivante vérifie que la partition a été supprimée :
...
Une fois le disque a été remplacé, nous devons copier la table de partition à partir d'un disque «sain» (dans cet exemple, sdb) à la nouvelle (sda) avec la commande suivante : 

```sh
sfdisk -d /dev/sdb | sfdisk /dev/sda 
```

Nous pouvons maintenant reconstruire la matrice RAID. L'extrait de code suivant montre comment nous pouvons reconstruire la disposition de la partition `/dev/md2` avec la table de partition sda copiée récemment : 
...
Nous pouvons vérifier les détails du RAID avec la commande suivante :
...
Le RAID a maintenant été reconstruit, mais nous avons encore besoin de monter la partition (`/dev/md2` dans cet exemple) avec la commande suivante : 
