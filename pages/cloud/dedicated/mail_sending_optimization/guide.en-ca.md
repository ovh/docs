...
To modify your reverse IP in the [OVH Control Panel](https://ca.ovh.com/auth/?action=gotomanager){.external}, go to the `Dedicated`{.action} section, then `IP`{.action}. Then select an IPv4 address, click the cogwheel on the right-hand side, then click `Modify the reverse`{.action}:
