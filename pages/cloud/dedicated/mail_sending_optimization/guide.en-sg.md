...
If your DNS Servers are managed by OVHcloud, please consult this [guide](https://docs.ovh.com/sg/en/domains/web_hosting_how_to_edit_my_dns_zone/#instructions).

Once done, you can add the PTR record (also known as the reverse) by performing the following steps:

Log into the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg){.external}, go to the `Bare Metal Cloud`{.action} section, then click on the `IP`{.action} section at the bottom of the column on the left side of the page. 
