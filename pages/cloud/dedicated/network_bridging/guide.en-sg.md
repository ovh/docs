...
- at least one [failover IP address](https://www.ovhcloud.com/en-sg/bare-metal/ip/) attached to the server
...
Log in to the [OVHcloud Control Panel](https://ca.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/sg/&ovhSubsidiary=sg) and switch to the `Bare Metal Cloud`{.action} section. Then click on `IP`{.action} in the menu on the left-hand side and locate your failover IP address in the table.
