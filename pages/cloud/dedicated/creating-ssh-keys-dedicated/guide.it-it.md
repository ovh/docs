...
- per un [server dedicato della gamma Eco](/pages/cloud/dedicated/getting-started-with-dedicated-server-eco);
- per un [VPS](/pages/cloud/vps/starting_with_a_vps).

Per maggiori informazioni, consulta la guida introduttiva del [protocollo SSH](/pages/cloud/dedicated/ssh_introduction).
...
Per saperne di più sulle connessioni SSH, consulta le guide di [primi passi](#getstarted)" e la nostra introduzione al [protocollo SSH](/pages/cloud/dedicated/ssh_introduction).

### Aggiungere chiavi SSH al tuo server <a name="addserverkey"></a>

[Accedi](/pages/cloud/dedicated/ssh_introduction) al server e assicurati di trovarti nella directory `$HOME` dell’utente. Se non esiste già, crea la cartella `.ssh`:
...
> [!primary]
>
> Tutte le chiavi registrate nella sezione `Dedicato` sono disponibili in preinstallazione su un server dedicato o un VPS. Per le chiavi SSH dei servizi Public Cloud, consulta la [presente guida](/pages/platform/public-cloud/public-cloud-first-steps).
>
...
[Introduzione al protocollo SSH](/pages/cloud/dedicated/ssh_introduction)

[Modalità Rescue su server dedicato](/pages/cloud/dedicated/rescue_mode)

[Modalità Rescue su VPS](/pages/cloud/vps/rescue)
