...
With [OVHcloud Dedicated Servers](https://www.ovhcloud.com/en-ca/bare-metal/), you can configure Partitions, [software RAID](https://docs.ovh.com/ca/en/dedicated/raid-soft/), LVM, ZFS, etc. during [OS installation](https://docs.ovh.com/ca/en/dedicated/getting-started-dedicated-server/) from the [OVHcloud API](https://ca.api.ovh.com/). as well as the [OVHcloud Control Panel](https://www.ovh.com/manager/#/dedicated/configuration). In this article, we will focus on the [OVHcloud API](https://ca.api.ovh.com/). This will give us more details about the engine that is running in the background in order to create the partitioning on the dedicated server from the input data passed on to the OVHcloud API.
...
- [software RAID](https://docs.ovh.com/ca/en/dedicated/raid-soft/) (SR),
...
>
> - lv: means that LVM layers will be added on top of the [software RAID](https://docs.ovh.com/ca/en/dedicated/raid-soft/) device (or the physical partition if RAID level is 0)
...
|Partition of type `t` with mountpoint `m` cannot fill the disk.|- You have chosen the `swap` partition to fill the disk (or partition with size 0 when defined within the [OVHcloud API](https://ca.api.ovh.com/console/)), we disallow this to avoid creating unnecessarily large `swap` partitions|- Set a fixed size for the `swap` partition|
...
In order to improve customer experience, reduce [OVHcloud support](https://help.ovhcloud.com/en-ca/) workload and to avoid introducing breaking changes for customer, some customer input is automatically fixed or changed by the backend. The following table gives an overview of what is currently auto-fixed/changed during the **pre-processing**:
...
[Configuring Software RAID](https://docs.ovh.com/ca/en/dedicated/raid-soft/)

[Managing Hardware RAID](https://docs.ovh.com/ca/en/dedicated/raid-hard/)
