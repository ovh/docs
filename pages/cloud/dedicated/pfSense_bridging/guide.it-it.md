...
- At least one [failover IP](https://www.ovhcloud.com/it/bare-metal/ip/) address attached to the server 
...
In the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.it/&ovhSubsidiary=it), go to the `Bare Metal Cloud`{.action} section and open the `IP`{.action} menu. 
