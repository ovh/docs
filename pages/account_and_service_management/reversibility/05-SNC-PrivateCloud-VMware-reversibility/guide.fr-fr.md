...
|---|---|
|**Migration entrante**:<br>- Souscrivez un projet Hosted Private Cloud SecNumCloud <br>-Commandez le nombre approprié d'hôtes et de datastores sur le projet pour obtenir une capacité comparable à celle de l'infrastructure d'origine.<br>-Migrez à l'aide d'un outil spécialisé (Veeam, API...) ou manuellement <br>
<br>-Utilisez la VPN Gateway de la zone SecNumCloud ou une solution VPN personnalisée (Ex : NSX ou solution tier en machine virtuelle) afin de garantir le chiffrement des données lors de la migration depuis un réseau extérieur.<br>
<br>-Activez ensuite le chiffrement des VM et les datastores du Cluster vSAN à l’aide de la brique logicielle vNKP ou votre propre KMS (compatible avec le protocole KMIP). <br>
<br>-Utilisez le SPN (Secure  Private Network) afin de connecter les services SecNumCloud à l’intérieur d’un site d’hébergement.<br>
<br>-Utilisez la solution SPN inter DC pour connecter vos infrastructures qualifiées hébergées dans d’autres sites d’hébergement couverts par la qualification SecNumCloud chez OVHcloud.<br>
