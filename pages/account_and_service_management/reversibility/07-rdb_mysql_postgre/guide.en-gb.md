...
| Manual Backup | Ability to generate a full on-demand database backup. | SQL, CSV, tar | **Incoming**: Restore from existing SQL dump. <br> **Outgoing**: SQL dump generated from OVHcloud instance, usable on any compatible MySQL environment. | [MySQL dump](https://dev.mysql.com/doc/refman/8.4/en/mysqldump.html){.external} |
...
| Infrastructure as Code | Automated deployment via OVHcloud-specific Terraform modules. | N/A | **Incoming**: Scripts need to be adapted for other providers. <br> **Outcoming**: Configuration rewrite required for Terraform. | [Terraform](https://registry.terraform.io/providers/ovh/ovh/latest/docs){.external} |
