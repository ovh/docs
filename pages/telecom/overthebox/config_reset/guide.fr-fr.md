...
slug: config-reset
excerpt: "Découvrez comment réinitialiser votre équipement OverTheBox afin de revenir sur la configuration par défaut"
...
Découvrez comment réinitialiser votre équipement OverTheBox afin de revenir sur la configuration par défaut
...
Plusieurs méthodes sont possible, le résultat final étant identique vous pouvez donc choisir celle qui vous convient le mieux.

### Méthode 1 : réinitialisation physique

- Appuyez de façon répétée, en moins de 2 seconde, 5 fois sur le bouton power on/off.
...
> [!success]
>
> Votre  OvertheBox  est maintenant réinitialisée, vous pouvez la réinstaller en suivant la procédure des guide :
>
...
> [!warning]
>
> Cette méthode n'est disponible que si le firmware de votre OverTheBox est en version 0.6 ou supérieur
>
...
Il est possible de réinstaller une nouvelle image depuis l'interface, cela restaurera complètement votre système.
...
Il est possible de réinstaller une nouvelle image depuis le CLI, cela restaurera complètement votre système.
...
- Sélectionnez la clé USB dans le champs **Device**
- Cliquez sur **"write"**
...
- Branchez la clé USB bootable sur un des ports USB au dos de l'**OverTheBox** (Ne pas brancher la clé sur le port USB de façade)

Dans le cas de l'OverTheBox Intel, la clé USB doit être branché sur un des port USB au dos de l'appareil.

![overthebox](images/reset-method6-3_OTBv1.png){.thumbnail}

Dans le cas d'une OverTheBox IT v1 la clé USB doit être branché sur un des ports USB noir :

![overthebox](images/reset-method6-3_OTBv2a.jpg){.thumbnail}

Dans le cas d'une OverTheBox Plus la clé USB doit être branché sur un des ports USB en façade :
