...
- Disposer d'un [numéro alias fourni par OVHcloud](https://www.ovhtelecom.fr/telephonie/numeros/){.external} ou d'un [numéro porté](../demander-la-portabilite-de-mon-numero).
...
Pour débuter la manipulation, connectez-vous à votre [espace client OVHcloud](https://www.ovh.com/auth?onsuccess=https%3A%2F%2Fwww.ovhtelecom.fr%2Fmanager&ovhSubsidiary=fr){.external} et assurez-vous de vous situer dans la partie `Télécom`{.action}. Cliquez sur `Téléphonie`{.action} dans la barre de services à gauche, sélectionnez votre groupe de téléphonie puis le numéro à configurer.
...
![configurationnumeroalias](images/alias-config1.png){.thumbnail}
