...
Esta ação pode ser feita diretamente através da [Área de Cliente OVHcloud](/links/manager). Quando estiver no espaço Managed Bare Metal, clique em `Segurança`{.action}. Aparecerá um quadro, no qual poderá ver os endereços IP autorizados ou recusados. Para acrescentar novos, clique à direita, em `Adicionar IP`{.action}:
...
Esta pode ser configurada na [Área de Cliente OVHcloud](/links/manager). Quando estiver no espaço Managed Bare Metal, escolha `Segurança`{.action}. A seguir, clique em `Alterar o prazo de expiração`{.action}, à direita.
