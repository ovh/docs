...
- Dostęp do [Panelu klienta OVHcloud](/links/manager) i sekcji `Bare Metal Cloud`{.action}, a następnie `Managed Bare Metal`{.action}
...
Zaloguj się do [Panelu klienta OVHcloud](/links/manager), przejdź do sekcji `Bare Metal Cloud`{.action} (1), kliknij `Managed Bare Metal`{.action} (2) i wybierz serwer z listy (3).
