...
* Disporre di un’[infrastruttura Managed Bare Metal](/links/websitemanaged-bare-metal/){.external} sul proprio account OVHcloud
...
OVHcloud conserva gli altri 23 snapshot orari (fino a H-24) su una rack di storage (datastore) a cui non hai accesso diretto. Tuttavia, è possibile richiedere il ripristino di uno Snapshot (al di là di H-1, quindi) per una determinata VM tramite una [richiesta di intervento **a pagamento** inviata al supporto tecnico](https://help.ovhcloud.com/csm?id=csm_get_help). Ripristinare lo Snapshot richiesto è possibile solo nello stesso datastore e questo ripristino non può essere garantito.
