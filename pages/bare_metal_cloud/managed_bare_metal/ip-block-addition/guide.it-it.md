...
È possibile seguire la migrazione del blocco IP dal tuo[Spazio Cliente OVHcloud](/links/manager) nella sezione`Bare Metal Cloud`{.action} > `Managed Bare Metal`{.action}. Clicca sul tuo servizio Managed Bare Metal, quindi sulla scheda `Operazioni`{.action}.
