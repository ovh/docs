...
- an [OVHcloud Storage Server](/links/websitebare-metal/storage/){.external}
- a production infrastructure ([VPS](/links/websitevps/){.external}, [Dedicated Servers](/links/bare-metal/bare-metal){.external}, [Public Cloud](/links/websitepublic-cloud/){.external}, etc.)
...
OVHcloud offers a range of [Storage Servers](/links/websitebare-metal/storage/){.external} with hardware configurations that contain several hard disks. In our example, we are using a software RAID (or softRAID) with four disks, which each have 6 TB capacity.
