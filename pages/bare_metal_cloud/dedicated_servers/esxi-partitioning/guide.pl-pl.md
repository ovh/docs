...
> Did you know?
>
> [OVHcloud's Hosted Private Cloud solution](https://www.ovhcloud.com/pl/hosted-private-cloud/vmware/) is based on ESXi with the partitioning layout `small`.
>
...
Log in to the [OVHcloud Control Panel](https://ovh.com/manager/auth/?action=gotomanager&from=https://www.ovh.pl/&ovhSubsidiary=pl). From the `General information`{.action} tab, click the `...`{.action} button next to the operating system and then click `Install`{.action}.
