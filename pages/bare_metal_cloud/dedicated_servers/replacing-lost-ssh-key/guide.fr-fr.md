...
Si vous [utilisez des clés SSH](/pages/bare_metal_cloud/dedicated_servers/creating-ssh-keys-dedicated) pour vous connecter à votre serveur dédié, la perte de votre clé SSH privée pourrait signifier la perte totale d'accès à votre serveur.
...
Suivez les étapes du guide sur le [mode rescue](/pages/bare_metal_cloud/dedicated_servers/rescue_mode) pour vous connecter à votre serveur et monter vos partitions.
...
Revenez au mode de démarrage « normal » et redémarrez le serveur dans votre [espace client OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.com/fr/&ovhSubsidiary=fr). Consultez le guide [« Activer et utiliser le mode rescue »](/pages/bare_metal_cloud/dedicated_servers/rescue_mode) si nécessaire.

Vous avez maintenant accès au serveur avec votre nouvelle paire de clés SSH.

## Aller plus loin

[Changer le mot de passe root sur un serveur dédié](/pages/bare_metal_cloud/dedicated_servers/changing_root_password_linux_ds)
