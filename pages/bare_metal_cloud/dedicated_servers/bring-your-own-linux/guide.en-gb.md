...
|Can't write qcow2 on disk|Impossible to write qcow2 image on disk.|Modify your image so that the `qemu-img convert -f "$imageType" -O raw $pathToImageFile "$device"` command works.|
