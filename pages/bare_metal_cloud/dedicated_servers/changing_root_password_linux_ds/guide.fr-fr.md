...
Avec la livraison d’un serveur dédié OVHcloud ou d’un VPS, vous bénéficiez d’un « **accès root** » à votre service. En termes simples, cela signifie que vous êtes l'administrateur du système et que vous disposez du niveau d'autorisation le plus élevé.

Même si le serveur n’est pas utilisé à des fins nécessitant l’administration d’utilisateurs réels, la gestion des **comptes d’utilisateurs** est un sujet lié à la sécurité qu’il ne faut pas sous-estimer. Ce guide fournit des conseils de base pour commencer avec les rubriques suivantes :

- Configuration des comptes utilisateurs système avec différents niveaux d'autorisation
...
> [!primary]
>
> Les instructions de ce guide sont basées sur un système d’exploitation de serveur Debian/Ubuntu et non exhaustives. Les exemples ci-dessous sont destinés à fournir un point de départ et à aider à prévenir les failles de sécurité facilement exploitables. Avec une connaissance de base de la gestion des comptes d’utilisateurs et des bonnes pratiques associées, vous pouvez aller plus loin avec les rubriques les plus pertinentes pour votre cas d’utilisation.
>
> Il est recommandé de consulter les **pages du manuel du système** pour chaque commande que vous utilisez. Vous pouvez le faire à partir de la ligne de commande en entrant `man` suivi d'un nom de commande, de fonction ou de fichier système.
>

### Gestion des comptes utilisateurs

Notez que les politiques de sécurité des serveurs peuvent être adaptées à différents cas d’utilisation et environnements utilisateur. Les étapes décrites ci-dessous offrent des explications de base sur la gestion des comptes d'utilisateurs en mettant l'accent sur la commodité et la sécurité et ne revendiquent pas une validité universelle.

Après une nouvelle installation de votre serveur (avec un template OVHcloud), vous débutez avec ces conditions :
...
Notez que les exemples de ligne de commande suivants continueront à utiliser « ubuntu » pour faire référence au `user account` préconfiguré.
...
Cela signifie que l'utilisateur avec lequel vous êtes actuellement connecté peut exécuter toutes les commandes en les faisant précéder de la commande `sudo` (`root privileges`). Vous trouverez des informations plus détaillées dans la [section guide correspondante ci-dessous](#sudouser).
...
> - `root` / `root user` / `root account` : compte d'utilisateur avec `root privileges` existant par défaut sur les systèmes GNU/Linux et utilisé à des fins spécifiques.
>
...
Même si vous n'avez pas besoin d'accorder l'accès à votre serveur à d'autres personnes, la création d'un compte d'utilisateur sans autorisations spéciales (aussi appelé `normal user` ou `regular user`) peut être utile à des fins de sécurité. Par exemple, il n'y a aucun risque d'endommager accidentellement le système en supprimant ou en modifiant les fichiers de configuration du serveur lors de l'exécution de commandes ou de processus à partir d'un compte d'utilisateur sans autorisations élevées.

Un autre exemple de bonne pratique consiste à créer un compte d’utilisateur dédié à une application hébergée sur votre serveur. Même si ce compte d'utilisateur est compromis par cette application, l'absence d'autorisations élevées empêchera des dommages plus importants.

Créer un nouveau compte d'utilisateur (remplacer `username` par le nom réel du compte d'utilisateur, par exemple le nom d'une application) :
...
Dans cette section, un nouveau compte d'utilisateur pour un `administrator` est créé et des autorisations élevées sont accordées sur le serveur (`root privileges`).

Créer un nouveau compte d'utilisateur (remplacer `username` par le nom réel du compte d'utilisateur) :
...
Le nouveau compte d'utilisateur est maintenant un `sudo user` et peut exécuter toutes les commandes. 
...
Ces configurations peuvent être trouvées dans `/etc/sudoers` et le répertoire `/etc/sudoers.d` respectivement.
...
Par exemple, pour modifier un mot de passe pour **n'importe quel compte d'utilisateur**, tapez `sudo passwd` suivi du `username` :
...
Cela verrouillera le compte (empêchera de se connecter par mot de passe) et le définira comme « *passwordless* », ce qui désactivera le compte.
...
En tant qu' `sudo user`, vous pouvez basculer vers n'importe quel autre compte d'utilisateur (sans avoir à connaître le mot de passe) :
...
Pour revenir à votre compte d'utilisateur précédent, basculez à nouveau ou utilisez `exit`.

#### Basculement vers le compte « root » (« root shell »)
...
Utilisez la commande suivante pour revenir au compte d'utilisateur précédent :

```bash
exit
```

**L'exécution de commandes en tant qu'`root user` n'est généralement pas nécessaire et peut même être contre-productive.**

Une idée fausse répandue est l'hypothèse que vous devez utiliser le véritable `root account` afin d'exécuter des commandes qui nécessitent des autorisations élevées (`root privileges`) sur un système.
