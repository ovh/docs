...
Once you receive the email with the unique link, click it to perform the initial configuration. If the link has already expired, connect to the server [via SSH](/pages/bare_metal_cloud/dedicated_servers/ssh_introduction) and run the command `sudo whmlogin` to generate a new link.
