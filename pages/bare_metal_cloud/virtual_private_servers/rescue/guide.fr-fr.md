...
Le mode rescue est un outil fourni par OVHcloud pour démarrer votre VPS dans un système d'exploitation temporaire. Vous pouvez alors accéder à votre système afin d'exécuter des tâches de diagnostic et de résoudre différents problèmes, par exemple :
...
Si vous rencontrez un problème avec votre système, effectuer des vérifications en mode rescue permet de déterminer s'il est lié à un logiciel installé sur le VPS ou s'il est causé par un problème plus profond. Avant de contacter nos équipes de support, nous vous recommandons de collecter les résultats des tests et d'exclure toute erreur logicielle en utilisant le mode rescue.
...
Si votre espace client est différent, reportez-vous à notre guide « [Gérer un ancien VPS](/pages/bare_metal_cloud/virtual_private_servers/vps_legacy_control_panel) ».
...
>
> Une façon de contourner ce problème est de « commenter » l'empreinte de votre VPS en ajoutant un `#` devant sa ligne dans le fichier `known_hosts`. N’oubliez pas d’annuler cette modification avant de repasser le netboot en mode « normal ».<br>Vous pouvez également supprimer la ligne du fichier. Votre client SSH ajoutera alors une nouvelle entrée d'empreinte pour le VPS lorsque la connexion sera établie à nouveau. Si vous avez besoin d'instructions plus détaillées, reportez-vous à notre guide « [Premiers pas avec SSH](/pages/bare_metal_cloud/dedicated_servers/ssh_introduction) ».
>

Pour la plupart des modifications que vous apportez à votre serveur via SSH en mode rescue, vous devrez monter la partition système.
...
En cas d'erreur lors du redémarrage d'un VPS, procédez comme suit :
...
- Si le KVM indique que le VPS est bloqué au démarrage ou ne parvient pas à trouver le disque, assurez-vous que [les journaux de démarrage sont activés](/pages/bare_metal_cloud/virtual_private_servers/bootlog_display_kvm). Relayez les logs pertinents à nos équipes support pour de plus amples investigations en [créant une demande de support](https://help.ovhcloud.com/csm?id=csm_get_help).
...
[Remplacement d’une paire de clés SSH](/pages/bare_metal_cloud/dedicated_servers/replacing-lost-ssh-key)
