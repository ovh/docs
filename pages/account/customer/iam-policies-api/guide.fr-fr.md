...
- Une ou plusieurs **identités** ciblées par cette politique. 
    - Il peut s'agir d'identifiants de compte, d'utilisateurs ou de groupes d'utilisateurs (comme ceux utilisés avec la [fédération d'identité](/pages/account/customer/ovhcloud-account-connect-saml-sso)). 
...
Si la fédération d'identité est activée via la [connexion SSO](/pages/account/customer/ovhcloud-account-connect-saml-sso), les politiques s'appliquent uniquement aux groupes d'utilisateurs, comme décrit dans la section précédente.
