...
excerpt: Connect ClickHouse® to Kafka® with Aiven integrations for seamless data exchange. Learn which data formats are supported for input and output to ensure smooth analytics workflows.
updated: 2026-01-15
---
...
- An [active Kafka cluster integrated via Aiven](https://aiven.io/docs/products/clickhouse/howto/data-service-integration#create-apache-kafka-integrations){.external} or a managed [Kafka cluster](/pages/public_cloud/data_analytics/analytics/kafka_create_cluster).
...
### Create the Apache Kafka Integration in Aiven
...
Follow the [official Aiven documentation to create the integration](https://aiven.io/docs/products/clickhouse/howto/data-service-integration#create-apache-kafka-integrations){.external}.
...
### Configure ClickHouse for External Connections

ClickHouse must accept connections from your Kafka cluster:

- Access your ClickHouse instance configuration (incoming connections guide).
...
### Create Kafka Engine Tables in ClickHouse
...
Track Kafka consumer lag to ensure no data is missed. Monitor ClickHouse ingestion metrics (rows/sec, disk usage). Tune Kafka batch sizes and ClickHouse buffer settings for optimal throughput.
...
[Official Aiven documentation to integrate Kafka](https://aiven.io/docs/products/clickhouse/howto/integrate-kafka){.external}
