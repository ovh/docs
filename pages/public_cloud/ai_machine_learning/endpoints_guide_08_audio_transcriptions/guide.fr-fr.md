...
| **chunking_strategy**        | No       | `string`/`server_vad object`/`null`   | -                                                                                     | null    | Controls how long audio is segmented before transcription. Keep it unset for single block behavior, or set it to improve throughput on long files. More details [here](#chunking-strategy).                                                                                                                                                     |
...
Setting `chunking_strategy` can significantly improve processing speed (often around x1.5), while keeping transcription quality very close to single-block mode. This parameter supports two values.
...
- `silence_duration_ms`: Duration of silence required to detect speech end (in milliseconds). Lower values make the model will react faster, but may increase sensitivity to short pauses.
