...
- **Monitoring**: Use OVHcloud monitoring tools to track the performance and health status of your Load Balancer and adjust the configuration as needed. See [Setting up Public Cloud Load Balancer Monitoring with Prometheus](https://help.ovhcloud.com/csm/en-public-cloud-network-loadbalancer-monitoring-prometheus?id=kb_article_view&sysparm_article=KB0061210).
...
2. **Selecting the Project**
   - Navigate to the "Public Cloud" section and select your project.

3. **Accessing the Load Balancer**
   - Click on "Load Balancer" in the left menu. Select the Load Balancer you want to configure.

4. **Accessing the Pools**
   - Go to the "Pools" tab, then click on the Pool for which you want to create a Health Monitor.

![Schema 1](images/healthM1.png)

5. **Configuring the Health Monitor**
   - Click on the "Health Monitors" tab within the Pool details, then on "Add a Health Monitor".
...
6. **Validation**
   - Once all the information is filled in, click on "Add" to activate your Health Monitor.
...
1. **Preparation**: Ensure the OpenStack CLI tool is installed and configured on your machine.
2. **Creating the Health Monitor**: Use the following command to create a Health Monitor:
...
3. **Verification**: Confirm the creation of the Health Monitor by listing the Health Monitors associated with your Load Balancer using:
...
1. **Login**: Log in to the Horizon interface of your OpenStack project. [Overview of Horizon](https://help.ovhcloud.com/csm/en-public-cloud-compute-horizon?id=kb_article_view&sysparm_article=KB0050895)
2. **Navigation**: In the left menu, under "Network", find and select "Load Balancers".
3. **Selecting the Load Balancer**: Choose the Load Balancer you want to configure and click on the "Health Monitors" tab.
4. **Adding a Health Monitor**: Click on "Create Health Monitor" and fill in the required fields such as type, check interval, maximum retry count, and timeout.
5. **Saving**: Confirm the creation by clicking on "Create".

#### Terraform

- **Configuring the Terraform File**: Create a `.tf` file and define your Health Monitor as a resource. For example:
...
```
Replace `<POOL_ID>` with the ID of your Pool. For more details on the available options for this resource, refer to the [official documentation for the `openstack_lb_monitor_v2` resource](https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs/resources/lb_monitor_v2) on the Terraform Registry.
