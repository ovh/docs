...
To interact with them, you will need to [generate an API key and Secret key in your Identity Access Manager](/pages/public_cloud/data_platform/getting-further/generate-api-key) from a **Data Platform Directory** account.
...
```
4. A few command examples (replacing `my-dataplant` with your [Project's subdomain](/pages/public_cloud/data_platform/product/project/config-ids#project-subdomain) and 'mybucket' with your desired bucket name):
...
rclone config file
```
4. Check your configuration if it matches the following below
```bash
