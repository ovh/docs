...
In this guide we will explore to create a Managed Kubernetes Service (MKS) cluster using our official OVHcloud Drivers

  how to use OVHcloud as a **Hosted Kubernetes provider**  to create a Managed Kubernetes Service (MKS) cluster.
...
You can create your [OVHcloud Managed Kubernetes Service](https://www.ovhcloud.com/en-gb/public-cloud/kubernetes/) clusters through the OVHcloud Control Panel, OVHcloud API, OVHcloud Terraform provider but managing several Kubernetes clusters can be complicated so let's discover how to create MKS cluster through Managed Rancher Service (MRS).
