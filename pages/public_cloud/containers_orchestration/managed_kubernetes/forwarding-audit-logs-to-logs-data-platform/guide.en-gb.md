...
You will have to define the target *Stream* of one of your LDP account you aim these logs to be forwarded to. The enablement of the forwarding will create a subscription for this stream id.
...
You can retrieve the API specification in OVH API Console. // add link
...
As per example below the POST request has a payload containing a streamId, which is the target data stream of your LDP account where you aim your Kubernetes cluster Audit logs will be forwarded.
```shell
...
You will get in response an operationid, so you can use it to retrieve the subscriptionid further management purposes using Logs Data Platform read operation endpoint:
```shell
{
