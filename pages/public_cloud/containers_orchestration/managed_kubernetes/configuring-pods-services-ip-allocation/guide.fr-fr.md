---
title: Personnaliser l'allocation IP sur un cluster OVHcloud Managed Kubernetes (offre Standard uniquement)
excerpt: "Découvrez comment configurer la politique d'allocation IP pour vos pods et services sur un cluster OVHcloud Managed Kubernetes avec l'offre Standard"
updated: 2026-03-13
---

## Objectif

**Ce guide détaille comment personnaliser les plages IP utilisées pour les pods et les services dans votre cluster OVHcloud Managed Kubernetes avec l'offre Standard.**
...
La personnalisation de la politique d'allocation IP des pods et services n'est pas possible sur les clusters avec l'offre Free.
...
| ------------------ | ----------------- | --------------------------------------------------------------------------- |
| `podsIpv4Cidr`     | `10.240.0.0/13`   | Il s'agit du sous-réseau utilisé pour adresser tous les pods du cluster     |
| `servicesIpv4Cidr` | `10.3.0.0/16`     | Il s'agit du sous-réseau utilisé pour adresser tous les services du cluster |

> [!primary]
>
> Vous trouverez plus d'informations sur la notation CIDR ici : [Classless Inter-Domain Routing](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing)
>

Gardez les règles suivantes à l'esprit pour ces paramètres :
...
> [!primary]
>
> Vous trouverez plus d'informations sur l'utilisation de l'API OVHcloud ici : [Premiers pas avec l'API OVHcloud](/pages/manage_and_operate/api/first-steps).
>
