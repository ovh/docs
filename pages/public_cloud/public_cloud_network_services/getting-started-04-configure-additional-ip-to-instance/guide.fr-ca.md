...
Le fichier de configuration principal se trouve dans `/etc/network/interfaces.d/`. Dans cet exemple, il est appelé « 55 ».
...
### Ubuntu 20.04 et versions ultérieures, Debian 12

Les fichiers de configuration réseau se trouvent dans le répertoire `/etc/netplan/`. Netplan ne prend pas en charge les interfaces virtuelles ou les alias Ethernet (par exemple ens3:0, ens3:1). Par conséquent, toutes les Additional IP sont configurées sur une seule interface réseau.
...
#### Etape 2 : créer et modifier le fichier de configuration
...
Ensuite, créez un fichier de configuration avec une extension .yaml pour configurer vos Additional IP. À titre d'exemple, notre fichier est nommé « 51-cloud-init.yaml ».
