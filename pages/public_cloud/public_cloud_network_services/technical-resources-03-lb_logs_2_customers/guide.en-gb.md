...
- **Logs Data Platform:** a fully managed and secured log management platform by OVHcloud. For more information, consult the [Logs Data Platform service page](https://www.ovhcloud.com/en-gb/logs-data-platform/).
...
The forwarded logs are generated by [HAproxy](https://en.wikipedia.org/wiki/HAProxy){.external} (the opensource component that is used to make the load balancing).
...
Note that the forwarding activation is free of charge, but you will be charged for the usage of the Logs Data Platform service as per the standard price plan. For LDP pricing, refer to the [Logs Data Platform product page](https://www.ovhcloud.com/en-gb/logs-data-platform/).
...
- `{loadBalancerId}`: This is the load balancer id, you can find it in the details page of your load balancer in the OVHCloud Control Panel or using the [dedicated API](https://eu.api.ovh.com/console-preview/?section=%2Fcloud&branch=v1#get-/cloud/project/-serviceName-/region/-regionName-/loadbalancing/loadbalancer)
...
Alternatively once the operation is finished, the subscriptions can be retrieved using the following call:
...
Once you have the `subscriptionId`, you can get the details using the following call:
