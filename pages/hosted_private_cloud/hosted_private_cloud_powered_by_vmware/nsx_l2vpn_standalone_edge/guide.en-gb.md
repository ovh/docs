...
> [!primary]
> We would also like to remind you that IPs are examples, and must be modified to suit your configuration.
...
In the `Segments` > `Add Segments` section of your NSX-T, create a segment. Name it and choose your gateway to match your VLAN. We will change its configuration later.

![Segment Creation](images/segment_creation.png){.thumbnail}

##### IPSec service creation

In the `VPN` > `VPN Services` > `Add Service` > `IPSec` section, create an IPsec service that will encrypt the L2VPN. In its configuration, use the same gateway as your previous segment. In addition, ensure that the **Admin Status** and **Session Sync** options are enabled.

![IPSec Creation](images/ipsec_service_creation.png){.thumbnail}

#### L2VPN service creation

In the `VPN` > `VPN Services` > `Add Service` > `L2 VPN Server` section, create a L2VPN server service. Here too, use the same gateway.

![L2VPN Server Creation](images/l2vpn_server_service_creation.png){.thumbnail}

#### Creating Local Endpoint

In the `VPN` > `Local Endpoints` > `Add Local Endpoint` section, create a Local Endpoint for the IPsec service. Use an IP in the IP block delivered with the Dedicated Cloud

![LocalEndpoint Creation](images/localendpoint_creation.png){.thumbnail}

Ensure that the **All IPSec Local Endpoints** option is enabled in the T1 Gateway you are using (`Tier-1 Gateways` > `Route Advertisement`).

![T1 Gateway Option](images/t1_gateway_options.png){.thumbnail}

#### Creation L2VPN server for LocalEndpoint

In the `VPN` > `VPN Services` > `Add Service` > `L2 VPN Server` section, create a L2VPN server service using the LocalEndpoint and the remote IP address of the L2VPN client that we will deploy later. The tunnel interface corresponds to a new IP address used for the tunnel.

![L2VPN Server for LocalEndpoint](images/l2vpn_server_for_localendpoint.png){.thumbnail}

Next, go back to configuring your segment in `Segments` > `Your Segment` > `L2VPN`. Select a VPN ID tunnel for this segment.
...
### Deploy NSX Standalone Edge on NSX-V PCC
...
> [!primary]
> If you are not sure how to deploy an OVF, you can refer to our guide [here](pages/bare_metal_cloud/managed_bare_metal/ovf_template/)
>
...
If you are using vxlan, NSX-v vxlan, you cannot. To do this, there is a call in the APIv6 that allows it, the call is the “enableCarp”. To enable Carp on the VMs on a specific network adapter, we need to enable promiscuous mode on the port. We'll use this call. Go to [our api](https://api.ovh.com/console/#/dedicatedCloud) and locate this call:
...
> [!primary]
> Before making the api call, click on your VM portgroup, click on your VM port
...
Wait a little while for the automation to run and you will see that your port's Promiscuous mode has changed to Accept.

![Promiscuous Policies Correct](images/promiscuos_policies_correct.png){.thumbnail}

#### Verifying the configuration

Then check if the L2VPN service is running, open the console, and log in.
...
The communication between the VMs is done well. Our configuration is functional.
