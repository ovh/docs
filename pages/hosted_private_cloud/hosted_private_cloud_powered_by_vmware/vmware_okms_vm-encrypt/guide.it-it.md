...
To access the OVHcloud KMS, go to the `Hosted Private Cloud`{.action} section. In the left-hand column, click `Identity, Security & Operation`{.action}, then `Key Management Service`{.action}.
...
Click [this link](/links/control-panel/privatecloud-vmware-vsphere) to access the `VMware vSphere`{.action} section, then select the service concerned. On the page that pops up, click on the `Security`{.action} tab.
...
>>
>> To enable vCenter to test your OVHcloud KMS server, click [this link](/links/control-panel/privatecloud-vmware-vsphere) to access the `VMware vSphere`{.action} section, then select the datacentre concerned.
>>
