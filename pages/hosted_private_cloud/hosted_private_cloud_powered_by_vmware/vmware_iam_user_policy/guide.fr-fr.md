...
1. `Identités et accès (IAM)` > `Politiques.`{.action}

Vous retrouverez vos politiques si vous en avez.

2. Cliquez sur : `...` `Gérer les identités associées.`{.action}

Si vous n'avez pas de politique définis suivez ce guide pour en créer une : [Comment ajouter un rôle IAM à une politique globale](pages/hosted_private_cloud/hosted_private_cloud_powered_by_vmware/vmware_iam_role_policy)
