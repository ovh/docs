...
|             ![vSphere Datastore Upload 01](images/datastore.png){.thumbnail}              | - Depuis `pcc-XXX-XXX-XXX-XXX.ovh.XX`, cliquez sur `Banques de données`{.action} |
...
Localisez l'élément à téléverser depuis votre ordinateur local (un fichier `XXX.iso` par exemple) et cliquez sur `Ouvrir`{.action} pour téléverser le fichier (ISO ou autre) à téléverser depuis votre machine connectée au client HTML vSphere.

Le fichier ISO se commence automatiquement sans validation dans le dossier (s'il y en a un) de votre banque de données sélectionnée.
...
Le téléversement ne commencera seulement qu'après la confirmation validée : 
