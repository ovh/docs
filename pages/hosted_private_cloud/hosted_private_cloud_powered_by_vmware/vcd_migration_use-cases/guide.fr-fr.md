...
| **Étapes**   | **Terminé** | **Avertissement** | **Cas d’usage**                               | **Prérequis**                                                                                                                                       | **Commentaires**                                                                                                                                                                                 | **Référence à la documentation externe**                                                                                                                                             |
|--------------|-------------|-------------------|-----------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Étape 1**  | ?           | ❌                 | **Multi-vDC**                                 | **Pas de multi-vDC (plusieurs datacenters, un seul possible).**                                                                                     | - Ne peut être migré que s'il n'a qu'un seul datacenter dans un vSphere géré sur OVHcloud. <br/> - Si ce n’est pas le cas, assurez-vous de migrer vos données dans le datacenter qui sera migré. | [Migration d'une infrastructure vers un nouveau vDC](/pages/hosted_private_cloud/hosted_private_cloud_powered by_vmware/service-migration-vdc).                                      |
...
| **Étape 3**  | ?           | ❎                 | **Affinité/anti-affinité DRS**                | **Aucune règle d’affinité/anti-affinité DRS.**                                                                                                      | - La migration de VMs à VMs peut être migrée vers VCD, mais de VM à hôtes ne peut pas être migrée.                                                                                               | [VMware DRS (Distributed resource Scheduler](/pages/bare_metal_cloud/managed_bare_metal/vmware_drs_distributed_resource_scheduler).                                                  |
...
| **Étape 5**  | ?           | ❎                 | **Clusters de datastores**                    | **Assurez-vous que toutes les règles de clustering sont supprimées dans vSphere sur OVHcloud.**                                                     | - Les règles de clustering devront être supprimés avant la migration car cette notion n'existe pas côté VCD.                                                                                     |                                                                                                                                                                                      |
| **Étape 6**  | ?           | ❎                 | **Sur-engagement mémoire**                    | **Scale up VCD.**                                                                                                                                   | - Prévoyez plus de ressources côté VCD ou redimensionnez/optimisez les VM avant la migration côté vSphere. <br/> - Parce que vous ne pourrez pas sur-engager (over-commit) des ressources.       | [Modification des ressources de la machine virtuelle](/pages/hosted_private_cloud/hosted_private_cloud_powered_by_vmware/modify_hardware_configuration_of_vm).                       |
...
| **Étape 8**  | ?           | ❎                 | **Options de sécurité (PCI-DSS, HDS, SNC)**   | **Ne pas avoir de vSphere certifiés PCI-DSS, HDS, SNC.**                                                                                            | - Ne peut pas être migré si le vSphere managé on OVHcloud est certifié PCI-DSS, HDS, SNC (vSphere avec options de sécurité).                                                                     |                                                                                                                                                                                      |
...
| **Étape 10** | ?           | ❌                 | **Zerto**                                     | **No Zerto.**                                                                                                                                       | - Si vous utilisez Zerto, vous ne pouvez pas migrer vers VCD pour le moment.                                                                                                                     | [Setting up Zerto Virtual Replication between two OVHcloud data centers](/pages/hosted_private_cloud/hosted_private_cloud_powered by_vmware/zerto_virtual_replication_as_a_service). |
| **Étape 11** | ?           | ❎                 | **Hosts / Zpool**                             | **Libérez des ressources (hôtes + Zpool) gratuitement et toutes les heures avant la migration. <br/> Ou convertissez-le en ressources mensuelles.** | - Libérez les ressources (hôtes + Zpool) "freespare" et "hourly" avant la migration. <br/> Ou convertissez-le en ressources "monthly".                                                           | [Informations de facturation du Hosted Private Cloud](/pages/account_and_service_management/manage_billing_payment_and_services/facturation_private_cloud).                          |
| **Étape 12** | ?           | ❌                 | **HCX**                                       | **Pas d’utilisation HCX dans vSphere sur OVHcloud.**                                                                                                | - Si tel est le cas, il ne peut pas être migré vers VCD.                                                                                                                                         |                                                                                                                                                                                      |
