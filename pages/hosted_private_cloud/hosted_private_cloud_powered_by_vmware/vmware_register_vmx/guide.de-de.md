...
Gehen Sie im [vSphere Interface](/pages/hosted_private_cloud/hosted_private_cloud_powered_by_vmware/vsphere_interface_connexion) in die Ansicht `Storage`{.action}.
...
Überprüfen Sie die Einstellungen Ihrer VMs (Name, [Portgroup](/pages/hosted_private_cloud/hosted_private_cloud_powered_by_vmware/creation_vlan) etc.), indem Sie mit der rechten Maustaste auf jede Ihrer VMs klicken und dann auf `Edit Settings`{.action}.
