...
1. From the `General information`{.action} tab, the vRack attached to your organization will appear with an ID in the form `pn-xxxxxxx`.

    ![Public VCF as-a-Service vRack attached](images/vcd-link-ip-vrack-02.png){.thumbnail .w-640}

2. Expand the `Network`{.action} menu in the left-hand column and select your vRack `pn-xxxxxxx`.

    ![Network vRack](images/vcd-link-ip-vrack-03.png){.thumbnail .w-640}

3. Select the IP block to link to your vRack/organization and click `Add`{.action}.

    ![add IP to vRack](images/vcd-link-ip-vrack-04.png){.thumbnail .w-640}

4. The public IP block is now linked to your vRack.
