...
| Logstash managed by OVHcloud                                          | - vRealize Log Insight Log Forwarder (vrli Forwarder)<br/>- LDP subscription for dedicated cloud                |         All         | Option |   ❌   |                                           ✅                                           |                                                              ✅                                                              |   | - Send VMware logs to managed Logstash. The use of a managed Logstash need to be added and configured manually from the OVHcloud control panel with the Log Forwarder feature. You can read the guide [Logs Data Platform - Collect VMware on OVHcloud logs](/pages/hosted_private_cloud/hosted_private_cloud_powered_by_vmware/vmware_ldp), a dedicated cloud LDP souscription is needed (check guide for POST api call). |
