...
- **Opcje [NSX](/links/websiteenterprise/products/hosted-private-cloud/nsx-datacenter-vsphere/){.external} i [vROps](/links/websiteenterprise/products/hosted-private-cloud/nsx-datacenter-vsphere/){.external} muszą być zainstalowane**\: z poziomu karty `Informacje ogólne`{.action} w sekcji „Opcje chmury prywatnej” możesz sprawdzić stan aktywacji tych opcji. Jeśli nie są aktywne, naciśnij przycisk `...`{.action}, a następnie `Aktywuj`{.action}, aby je włączyć.
